...
Example: `/debug-simtest MSIM_TEST_SEED=1768248386016 RUST_LOG=myso=debug,info cargo simtest --test address_balance_tests test_deposit_and_withdraw`
...
Run the repro command. If `RUST_LOG=...` is missing, add `RUST_LOG=myso=debug,info`. if `--no-capture` is missing, add it.
