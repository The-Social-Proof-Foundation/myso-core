...
The release branch naming convention is `releases/myso-v<version>.0-release`.
```bash
git fetch origin releases/myso-v<version>.0-release
git checkout releases/myso-v<version>.0-release
```
...
```bash
gh pr create --base releases/myso-v<version>.0-release \
