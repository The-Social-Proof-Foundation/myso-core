...
With any change of this magnitude there are bound to be some issues that crop up. Report any issues you encounter while using the new package system [here](https://github.com/the-social-proof-foundation/myso-core/issues/24540).
...
Publication workflows on Mainnet and Testnet should not change with the new package system. However, publishing on ephemeral networks like `localnet` or `devnet` should use the `myso client test-publish` command to avoid polluting the `Published.toml` file with local addresses.
...
MySo advises against disabling implicit dependencies.
