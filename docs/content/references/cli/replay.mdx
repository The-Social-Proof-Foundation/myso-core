---
title: MySo Replay CLI
description: Use the MySo CLI to replay a given transaction to view its effects.
---
...
The main command for replaying a transaction follows. For all available flags and arguments, run `myso replay --help`.

```sh
$ myso replay --digest <digest>
```
...
the gas cost, please refer to the [gas in myso](../../concepts/tokenomics/gas-in-myso.mdx) page.
...
You can provide default values for the replay tool flags by specifying them in the `~/.myso/myso_config/replay.toml` file. You need to specify them in the `[flags]` section (the only section currently supported) using their "long" names with initial dashes (`--`) removed. For example, you can disable showing transaction effects with `show-effects = false` but not with `e = false`. Here is a complete example of a config file:
```toml
[flags]
show-effects = false
overwrite = true
