...
```sh
$ cargo install --git https://github.com/the-social-proof-foundation/myso-core.git myso-move-lsp
```

By default, the Move extension expects to find the `move-analyzer` binary in `~/.myso/bin`. You can either copy the binary to this location, or configure the extension to use a different path. 
...
These commands find the `Move.toml` file for the open Move source file and open a terminal to run the appropriate `myso move` command.

To execute these commands you must have `myso` binary installed, in particular, to generate a trace, the `myso` binary version must be trace-generation capable. See [Debugger](./debugger.mdx#install) for installation instructions. The extension will search for the `myso` binary on a system path but you can configure the extension to use a different path.
