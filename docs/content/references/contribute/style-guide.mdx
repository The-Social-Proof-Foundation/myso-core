...
title: Style Guide
slug: /style-guide
description: All contributions to the MySo documentation must adhere to the style guide.
...
> ✅ You can view the transaction history in the MySo Explorer.

> ❌ We can view the transaction history in the MySo Explorer.
...
> ✅ You must install Cargo, Rust, Docker, and the MySo CLI to create a MySo node.
...
For consistency, the MySo documentation must use the following terms and phrases with the indicated capitalization (unless sentence structure determines otherwise). This list is not exhaustive and will be updated over time. 

|     **Category**       |             **Words and terms**               |    **Notes**   |
| ---------------------- |---------------------------------------------- | -------------- |
| **Always capitalized** | [Proper nouns](#proper-nouns), [product names](#product-names), names of example apps (Coin Flip, Blackjack, etc), Archival Store and Service, Archival Store, Archival Service, Coin Registry, Currency Standard, Orderbook Indexer, OrderbookV3, Devnet, GraphQL RPC, General-purpose Indexer, ID, Localnet, Mainnet, Mysticeti, One-Time Witness, Operation Cap,MySo, MySo CLI, MySo Client PTB CLI, MySo Closed-Loop Token / Closed-Loop Token, MySo dApp Kit, MySo Explorer, MySoJSON, MySo Keystore, MySo Keytool, MySoLink, MySo Object Display, MySoPlay0X1, MYSO, MYSO token, Testnet, Wallet Standard, Web2, Web3, zkSend SDK | Use capitalization consistently for branding and product-specific references |
| **Always lowercase**   | casual history, casual order, certificate, epoch, equivocation, eventual consistency, finality, gas, genesis, object, oracle, recovery passphrase (mnemonic), smart contract, soulbound, MySo framework, MySo object, total order, transaction, transfer, validator, wallet | Use lowercase when referring to general concepts rather than branded terms   |
...
| **Word preference**   | Use "app" rather than "dApp"    | Decentralization is implied when building an app on MySo. |
...
- Capitalize _MySo Full Node_ when referring to the official software binary/package that MySo distributes.
    “Download and run the MySo Full Node from GitHub to connect to Mainnet.”
...
Product names are proper nouns. Capitalize all words of a product name. When referring to a product, use only the product name without "the". When referring specifically to a MySo wallet, use MySo Wallet or Ethos Wallet and not just wallet. Users likely have multiple wallets, and we want to make it clear which wallet. Use wallet generically when referring to the concept of a wallet.

> There are several types of wallets to choose from.

> Never share the recovery passphrase for your wallet with anyone.

> The MySo network supports the following wallets:
> - MySo Wallet
> - Ethos Wallet
> - Coinbase Wallet
...
> Example: You can mint non-fungible tokens (NFTs) using your MySo Wallet. To view an NFT after you mint it, click the NFTs tab of your wallet.
...
A better option is **Get Started withMySo**, but even this is still too broad. Readers want guidance for a specific task or journey. Instead, use precise titles such as **Create a MySo Full Node** or **Install MySo Tooling**. These tell the reader exactly what they will learn or accomplish.
...
> ✅ Correct usage: 
> Page 1: Page title "MySo Gas Profiling" with heading "Environment configuration"
> Page 2: Page title "MySo Indexing" with heading "Environment configuration"

> ❌ Incorrect usage: 
> Page 1: Page title "MySo Gas Profiling" with heading "Environment configuration"
> Page 2: Page title: "MySo Features" with heading "MySo gas profiling"
...
MySo Explorer supports the following browsers:
...
An event object in MySo consists of the following attributes:
...
- `sender`: The MySo network address that triggered the event.
...
- CLI commands when used within a sentence: If using the suggested location, type `export PATH=$PATH:~/myso` and press Enter.
...
To get the latest version of the MySo Wallet extension:

1. Open Google Chrome.
1. Click **Extensions**, then click **Manage Extensions**.
1. Click **Details** for the MySo Wallet extension, then click **View in Chrome Web Store**.
...
Always use full, relative links when linking to topics on [docs.mysocial.network].
...
> To learn more, see [Examples of MySo Smart Contracts](/guides/developer/app-examples.mdx).
...
> Before you installMySo, make sure to install the [prerequisites](/guides/developer/getting-started/myso-install.mdx#prerequisites).
...
You can also just refer to a topic by title where it makes sense. See Installing MySo for more information.
...
Running `target/release/myso-node`
...
INFO: MySo node is now running
```
...
You must install Rust before you can build MySo from source.
