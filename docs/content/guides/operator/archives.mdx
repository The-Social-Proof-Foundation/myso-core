---
title: MySo Archives
description: The archive is a historical record of all transactions on MySo. Enable archiving on your Full nodes as a best practice.
keywords: [ archive, archive data, full node archive, node archive, fallback, archival fallback ] 
---

A MySo archive is a history of all transaction data on MySo. In some cases, peer nodes may not catch up with all transactions and effects through synchronization if they lag behind the current epoch by more than the latest few epochs. In such cases, instead of relying on synchronization, peer nodes can fallback to downloading the relevant information from an archive.
...
```yaml
state-archive-read-config:
  - ingestion-url: "https://storage.googleapis.com/mysocial-testnet-checkpoints"
    # How many objects to read ahead when catching up
    concurrency: 5
```
