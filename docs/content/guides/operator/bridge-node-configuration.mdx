---
title: MySo Bridge Validator Node Configuration
sidebar_label: MySo Bridge Node Configuration
...
- [x] Install `myso` and `myso-bridge-cli`.
...
```sh
$ cargo install --locked --git "https://github.com/the-social-proof-foundation/myso-core.git" myso myso-bridge-cli
```

Install with a commit sha:

```sh
$ cargo install --locked --git "https://github.com/the-social-proof-foundation/myso-core.git" --rev {SHA} myso myso-bridge-cli
```
...
- A REST API URL where the bridge node listens to and serves requests. Example: `https://bridge.example-myso-validator.io:443`. Make sure the port is correct and the URL does not contain any invalid characters, like quotes for example.

To create a `BridgeAuthorityKey`, run
```sh
$ myso-bridge-cli create-bridge-validator-key <PATH-TO-WRITE>
```
...
After you have both authority key file and REST API URL ready, you can register them by using MySo CLI:

```sh
$ myso validator register-bridge-committee --bridge-authority-key-path <BRIDGE-AUTHORITY-KEY-PATH> --bridge-authority-url <BRIDGE-AUTHORITY-URL>
```
...
```sh
$ myso client execute-signed-tx
```

#### Update metadata (before committee is finalized)

Both key and URL are changeable **before the committee is finalized**. If you wish to update metadata, simply rerun `myso validator register-bridge-committee`.
...
```sh
$ myso-bridge-cli view-bridge-registration --myso-rpc-url {MYSO-FULLNODE-URL}
```
...
```sh
$ myso validator update-bridge-committee-node-url
```
...
Use `myso-bridge-cli` command to create a template. If you want to run `BridgeClient` (see the following section), pass `--run-client` as a parameter.

```sh
$ myso-bridge-cli create-bridge-node-config-template {PATH}
$ myso-bridge-cli create-bridge-node-config-template --run-client {PATH}
```
...
| `bridge-authority-key-path` | The path to the Bridge Validator key, generated from `myso-bridge-cli create-bridge-validator-key` command referenced previously. |
...
| `myso:myso-rpc-url` | MySo RPC URL. |
| `myso:myso-bridge-chain-id` | `0` for MySo Mainnet, `1` for MySo Testnet. |
...
| `metrics:push-url` | The url of the remote MySo metrics pipeline (for example, `https://metrics-proxy.[testnet_OR_mainnet].mysocial.network:8443/publish/metrics`). See the [metrics push section](#metrics-push) that follows for more details. | 
...
| `myso:bridge-client-key-path` | The file path of Bridge Client key. This key can be generated with `myso-bridge-cli create-bridge-client-key` as previously shown. When `run-client` is true but you do not provide `myso:bridge-client-key-path`, it defaults to use the Bridge Validator key to submit transactions on MySo. This is not recommended for the sake of key separation. | 

### BridgeClient

`BridgeClient` orchestrates bridge transfer requests. It is **optional** to run for a `BridgeNode`. `BridgeClient` submits transaction on the MySo network. Thus when it's enabled, you need a MySo account key with enough MYSO balance.
...
```sh
$ myso-bridge-cli create-bridge-client-key <PATH_TO_BRIDGE_CLIENT_KEY>
```
This prints the newly created MySo Address. Then we need to fund this address with some MYSO for operations.
...
- Use `cargo install`.
    ```sh
    $ cargo install --locked --git "https://github.com/the-social-proof-foundation/myso-core.git" --branch {BRANCH-NAME} myso-bridge
    ```
    Or
    ```sh
    $ cargo install --locked --git "https://github.com/the-social-proof-foundation/myso-core.git" --rev {SHA-NAME} myso-bridge
    ```
- Compile from source code
    ...
    ```
    ```sh
    $ cargo build --release --bin myso-bridge
    ```
- Use `curl`/`wget` pre-built binaries (for Linux/AMD64 only).
    ```sh
    $ curl https://myso-releases.s3.us-east-1.amazonaws.com/{SHA}/myso-bridge -o myso-bridge
    ```
- Use pre-built Docker image. Pull from Docker Hub: `mysten/myso-tools:{SHA}`


### Run bridge node

Running bridge node is similar to running a MySo node using systemd or Ansible. The command to start the bridge node is:

```sh
$ RUST_LOG=info,myso_bridge=debug myso-bridge --config-path {BRIDGE-NODE-CONFIG-PATH}
```
...
You can find a full list of bridge node metrics and their descriptions in the [`myso-bridge` crate](https://github.com/the-social-proof-foundation/myso-core/blob/main/crates/myso-bridge/src/metrics.rs).
