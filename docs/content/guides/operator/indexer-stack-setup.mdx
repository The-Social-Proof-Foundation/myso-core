...
```sh
$ myso-indexer-alt-consistent-store restore \
    --azure <AZURE_BUCKET> \
...
| --------------------------- | ----------------------------------------------------------------------------------------------------------------------------------------- |
| `<AZURE_BUCKET>` \*         | Name or URL of Azure bucket containing [managed snapshots](https://docs.mysocial.network/guides/operator/snapshots#mysten-labs-managed-snapshots).  |
| `<DATABASE_PATH>`           | Path to RocksDB database.                                                                                                                 |
| `<GCS_ACCOUNT>` \*          | Name or URL of GCS bucket containing [managed snapshots](https://docs.mysocial.network/guides/operator/snapshots#mysten-labs-managed-snapshots).    |
...
| `<REMOTE_STORE_URL>`        | URL of a checkpoint bucket to index from, one of multiple possible [data sources](/guides/developer/accessing-data/custom-indexing-framework#data-sources).      |
| `<S3_BUCKET>` \*            | Name or URL of AWS S3 bucket containing [managed snapshots](https://docs.mysocial.network/guides/operator/snapshots#mysten-labs-managed-snapshots). |
...
$ myso-indexer-alt-consistent-store restore \
    ...
    --http https://formal-snapshot.mainnet.mysocial.network \
...
```sh
$ myso-indexer-alt-consistent-store run \
    --config <CONFIG_FILE> \
    --database-path <DATABASE_PATH> \
    --remote-store-url <REMOTE_STORE_URL>
```
...
### Run `myso-indexer-alt-graphql`

:::info

If you use the MySo Foundation–hosted public good archival service on Testnet or Mainnet, you may encounter performance issues. The team will address these before the GraphQL RPC and Archival Service reach general availability.

:::

Use the following command to run a GraphQL RPC server node:
```sh
myso-indexer-alt-graphql rpc \
...
myso-indexer-alt-graphql rpc \
    ...
    --ledger-grpc-url https://archive.mainnet.mysocial.network:443 \
...
myso-indexer-alt-graphql generate-config > <PATH_TO_GRAPHQL_CONFIG_FILE>
```
...
<RelatedLink
	href="https://github.com/the-social-proof-foundation/myso-core/tree/main/crates/myso-indexer-alt"
	label="MySo Indexer Alt"
	desc="The `myso-indexer-alt` crate in the MySo repo."
/>
<RelatedLink
...
/>
<RelatedLink
	href="https://github.com/MystenLabs/orderbookv3/tree/main/crates/indexer"
	label="Orderbook Indexer"
	desc="The indexer that Orderbook implements."
/>
<RelatedLink
	href="/references/myso-api/myso-graphql/beta/reference"
