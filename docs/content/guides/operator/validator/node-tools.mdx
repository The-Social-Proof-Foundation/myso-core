...
description: Use the MySo CLI to interact with a full node's validator commands. 
keywords: [ myso cli, full node, delegator, validator, operation cap, report validators, join a committee, leave a committee, report a validator, proof of possession, generate proof of possession ]
sidebar_position: 5
sidebar_label: Tools
---

This guide focuses on using the MySo CLI `validator` commands. 
...
- [x] Complete all [MySo installation prerequisites](/guides/developer/getting-started/myso-install.mdx).

- [x] Build the `myso` binary, which you need for the genesis ceremony. You can do this on any computer.

<details className="nudge-details">

<summary>

View `myso` binary installation details.

</summary>

    1. Clone the git repo:

           `$ git clone git@github.com:the-social-proof-foundation/myso-core.git && cd myso`
...
View MySo account and CLI environment instructions.

</summary>

    - If this is the first time running this program, it asks you to provide a MySo full node server URL and a meaningful environment alias. It also generates a random key pair in `myso.keystore` and a config `client.yaml`. Swap in your validator account key if you already have one.
    ...
    - If you already set it up, just make sure:
      ...
      - `myso.keystore` contains your account key pair.
...
```sh
$ myso validator display-metadata
```
...
## Using MySo CLI 
Use the MySo CLI to perform validator tasks.
...
```sh
$ myso validator make-validator-info <name> <description> <image-url> <project-url> <host-name> <gas_price>
```
...
```sh
$ myso validator become-candidate path/to/validator.info
```
...
Then, you must acquire 30M MYSO and stake it to the validator staking pool. You can add MySo to the validator staking pool with the command:

```sh
$ myso client call --package 0x3 --module myso_system --function request_add_stake --args 0x5 {myso_object_id} {validator_address}
```

This creates a stake object (of type `0x3::staking_pool::StakedMySo`). This object can be withdrawn with the following command:

```sh
$ myso client call --package 0x3 --module myso_system --function request_withdraw_stake --args 0x5 {stake_object_id}
```
...
```sh
$ myso validator leave-committee
```
...
```sh
$ myso validator serialize-payload-pop --account-address $ACCOUNT_ADDRESS --protocol-public-key $BLS_PUBKEY
```
...
```sh
$ myso validator display-metadata <validator-address>
```
...
```sh
$ myso validator update-metadata --help
```
...
```sh
$ myso validator update-gas-price <gas-price>
```

If the account is a delegatee:

```sh
$ myso validator update-gas-price --operation-cap-id <operation-cap-id> <gas-price>
```
...
```sh
$ myso validator report-validator <reportee-address>
```
