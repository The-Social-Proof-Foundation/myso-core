...
MySo uses a delegated proof-of-stake (DPoS) system, where validators receive voting power from MYSO token holders who delegate their stake. At the end of each epoch, validators receive rewards based on their performance and commission rate. Rewards auto-compound because validator staking pools receive new delegated stakes.

The total voting power on MySo is always 10,000, regardless of the amount staked. Therefore, the quorum threshold is 6,667. There is no limit to the amount of MYSO users can stake with a validator. Each validator has consensus voting power proportional to MYSO in its staking pool, with one exception: the voting power of an individual validator is capped at 1,000 (10% of the total). If a validator accumulates more than 10% of total stake, the validator's voting power remains fixed at 10%, and the remaining voting power is spread across the rest of the validator set.

## Staking on MySo compared to other blockchains

The MySo staking mechanism includes features that might differ from other blockchains:

- Staking is self-custodial. Stakers keep their staked MYSO tokens in an owned object.
...
When users stake MYSO tokens, the tokens are wrapped into `StakedSUI` objects. A user’s share of the staking pool is calculated using the timestamp of their `StakedSUI` object, which records when the deposit occurred, along with the change in exchange rates between the deposit epoch and the withdrawal epoch.  
...
Validator rewards are distributed as regular stake objects, so withdrawal works the same for validators and stakers. A validator can call the [`request_withdraw_stake` function](https://github.com/the-social-proof-foundation/myso-core/blob/6234ae2cc8137f3a2a34cd0aa1ac4fd5f31260e2/crates/myso-framework/packages/myso-system/sources/myso_system.move#L272) in the `myso_system` module to withdraw stake and receive rewards.
...
$$
MYSO withdrawn at E' = ( MYSO deposited at E ) * ( Exchange Rate at E'-1 / Exchange Rate at E )
$$

Users can't withdraw a portion of their active stake. They must withdraw all staked MYSO at the same time. Users can, however, stake using multiple `StakedMySo` objects by splitting their MYSO into multiple coins. They can then perform a partial withdrawal from a validator by un-staking only some of the `StakedSUI` objects.
...
Stake subsidies are intended to subsidize the network during its early phases and are funded by a 10% allocation of MYSO tokens. After this allocation depletes, the entirety of stake rewards is made up of gas fees collected through regular network operations.
...
- **Gas fees:** Each epoch's amount depends on the total gas fees collected throughout the epoch. Each MySo transaction pays gas fees depending on 2 variables, the amount of executed gas units and the gas price:
...
MySo is designed such that end-users can expect the gas price to be stable and predictable during regular network operations. This is achieved by having validators set the network's reference gas price at the beginning of each epoch.
...
```sh
$ myso client call --package <PACKAGE-ID> --module myso_system --function request_set_gas_price --args 0x5 \"42\" --gas-budget <GAS-AMOUNT>
```
...
For example, to report a validator whose MySo address is `0x44840a79dd5cf1f5efeff1379f5eece04c72db13512a2e31e8750f5176285446` as bad or non-performant, run:

```sh
$ myso client call --package <PACKAGE-ID> --module myso_system --function report_validator --args 0x5 0x44840a79dd5cf1f5efeff1379f5eece04c72db13512a2e31e8750f5176285446 --gas-budget <GAS-AMOUNT>
```
