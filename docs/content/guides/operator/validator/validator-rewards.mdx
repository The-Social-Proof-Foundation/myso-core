...
Users can't withdraw a portion of their active stake. They must withdraw all staked MYSO at the same time. Users can, however, stake using multiple `StakedSui` objects by splitting their MYSO into multiple coins. They can then perform a partial withdrawal from a validator by un-staking only some of the `StakedSUI` objects.
