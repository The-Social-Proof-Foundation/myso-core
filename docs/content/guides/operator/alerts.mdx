---
title: MySo Validator Alert Reference
...
When running a MySo Validator node or Full node, you may want to configure alerting based off some or all of the following metrics.
...
```sh
min(myso_configured_max_protocol_version{network="$network", host=~"Mysten-.*"})
  < quantile(0.34, myso_configured_max_protocol_version{network="$network"})
```
