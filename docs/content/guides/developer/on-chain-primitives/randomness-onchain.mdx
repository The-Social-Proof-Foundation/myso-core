...
`Random` has a reserved address `0x8`. See <UnsafeLink href="/references/framework/myso-framework/myso_sui/random">`random.move`</UnsafeLink> for the Move APIs that access randomness on MySo.
...
<ImportContent source="crates/myso-protocol-config/src/lib.rs" mode="code" struct="ProtocolConfig" />
...
module games::dice {
  ...
  public fun play_dice(guess: u8, fee: Coin<MYSO>, r: &Random, ctx: &mut TxContext): Ticket {
    // Pay for the turn
...
A similar attack to the one previously described involves PTBs even when `play_dice` is defined as a private `entry` function. For example, consider the `entry play_dice(guess: u8, fee: Coin<MYSO>, r: &Random, ctx: &mut TxContext): Ticket { … }` function defined earlier, the attacker can publish the following function and send a PTB with commands `play_dice(...), attack(Result(0))` where `Result(0)` is the output of the first command:
...
To protect against PTB-based composition attacks, MySo rejects PTBs that have commands that are not `TransferObjects` or `MergeCoins` following a `MoveCall` command that uses `Random` as an input.
...
<RelatedLink href="https://docs.mysocial.network/references/framework/myso_sui/random" label="random.move" desc="Documentation for the `random` module." />
<RelatedLink href="https://github.com/the-social-proof-foundation/myso-core/blob/main/examples/move/random/raffles" label="Raffle example" desc="GitHub repo containing a raffle smart contract demonstrating randomness." />
