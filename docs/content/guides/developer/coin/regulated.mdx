...
description: You can create regulated currencies on MySo using the Coin Registry system. These coins include the ability to control access using a deny list.
keywords: [ regulated coin list, regulated coins, regulate coin, allow list, deny list, stablecoins, token example, create a token, how to create token, MYSO token standards, coin registry ]
---

You can create regulated currencies onMySo, such as stablecoins. These coins are similar to other coins, like MYSO, but include the ability to control access to the currency using a deny list.

The Coin Registry system provides enhanced regulatory features through the `myso::coin_registry` module:
...
The `DenyList` is a singleton, shared object that the bearer of a `DenyCapV2` can access to specify a list of addresses that are unable to use a MySo core type. The initial use case for `DenyList`, however, focuses on limiting access to coins of a specified type. This is useful when creating a regulated coin on MySo that requires the ability to block certain addresses from using it as inputs to transactions. Regulated coins on MySo satisfy any regulations that require the ability to prevent known bad actors from having access to those coins.
...
To learn about the features available, see the [Currency Standard](/standards/currency.mdx) documentation and the `coin_registry` module in the MySo framework.

## Regulated coin example

The regulated coin example is in the `examples/regulated-coin` directory of the MySo repo. The example provides both TypeScript- and Rust-based command line access to an on-chain package that demonstrates some of the features of regulated coins on MySo.

<ImportContent source="prerequisites.mdx" mode="snippet" />

This topic assumes you are accessing the code from your own fork of the MySo repo. 

You do not need a MySo wallet to use this project, but having one available might help you visualize results.

This example assumes you're familiar with publishing packages on MySo and the Move language. For more detailed guides on example apps, see [App Examples](../app-examples.mdx). For more information on the Move language, see [The Move Book](https://move-book.com/).
...
| --- | --- |
| `PACKAGE_ID` | Object ID of the package you publish. This data is part of the response that MySo provides on publish. |
| `ADMIN_SECRET_KEY` | The secret key for the address that publishes the package. You can use `myso keytool export --key-identity <MYSO-ADDRESS>` or a wallet UI to get the value. Take care not to expose the value to the public. |
| `ADMIN_ADDRESS` | The address that publishes the contract. |
| `DENY_CAP_ID` | Deny capability object ID. This data is part of the response that MySo provides on publish. |
| `TREASURY_CAP_ID` | The treasury cap object ID that allows the bearer to mint new coins. This data is part of the response that MySo provides on publish. |
| `MODULE_NAME` | The name of the module you publish. |
| `COIN_NAME` | The name of your regulated coin.  |
| `MYSO_FULLNODE_URL` | The URL to the full node network that processes transactions. For Testnet this value is `https://fullnode.testnet.mysocial.network:443`. |
...
For all `Coin` functions available, see the MySo framework [`coin` module documentation](https://github.com/the-social-proof-foundation/myso-core/blob/main/crates/myso-framework/docs/myso/coin.md). The following functions are the most common. 
...
<ImportContent source="crates/myso-framework/packages/myso-framework/sources/coin.move" mode="code" fun="mint" />
...
<ImportContent source="crates/myso-framework/packages/myso-framework/sources/coin.move" mode="code" fun="mint_balance" />
...
<ImportContent source="crates/myso-framework/packages/myso-framework/sources/coin.move" mode="code" fun="mint_and_transfer" />
...
<ImportContent source="crates/myso-framework/packages/myso-framework/sources/coin.move" mode="code" fun="burn" />
...
<RelatedLink href="https://github.com/the-social-proof-foundation/myso-core/tree/main/examples/regulated-coin" label="Source code" desc="The source code in GitHub for this example." />
