...
description: Use the MySo Closed-Loop Token standard to create tokens that you can use as currency within a game application.
keywords: [ closed-loop tokens, closed-loop, closed loop, in-game tokens, in-game currency, game currency, token example, create a token, how to create token, MYSO token standards ]
---

Using the MySo [Closed-Loop Token](/standards/closed-loop-token.mdx) standard, you can create in-game currency (such as gems or diamonds in mobile games) that you can grant to players for their actions or make available to purchase. You mint the tokens onMySo, but players can only use the tokens within the economy of the game itself. These types of tokens are usually not transferable and you typically mint them in predefined amounts to maintain scarcity and game balance.

The following example creates an in-game currency called a GEM, which represents a certain number of MYSO. In the example, the user can buy fungible GEMs using MYSO, which can then be used as currency within the game. Use the code comments to follow the logic of the example.

## Example 

The MySo repo hosts a basic example of creating in-game currency. The Move modules that create the economy of the example are in the `gems.move` source file.
...
The `examples::gem` module creates the in-game currency, GEMs. Users spend MYSO to purchase GEMs, which can then be traded for swords. The module defines 3 groups of GEMs (small, medium, and large), with each group representing a different in-game value. Constants hold both the value of each package and the actual number of GEMs the groups contain. 
...
<RelatedLink href="https://github.com/the-social-proof-foundation/myso-core/blob/main/examples/move/token/sources/gems.move" label="Source code" desc="The source code in GitHub for this example." />
