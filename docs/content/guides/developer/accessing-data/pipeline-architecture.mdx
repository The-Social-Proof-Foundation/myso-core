...
description: The `myso-indexer-alt-framework` provides two distinct pipeline architectures. Understand the differences between the sequential and concurrent pipelines that the `myso-indexer-alt-framework` provides to decide which best suits your project needs.
keywords: [ indexer, index data, myso-indexer, custom indexer, sequential pipeline, concurrent pipeline ]
---

The `myso-indexer-alt-framework` provides 2 distinct pipeline architectures. Understanding their differences is crucial for choosing the right approach.
...
<li><span className="text-myso-success-dark">✓</span> You want straightforward implementation with direct commits and simple queries.</li>
<li><span className="text-myso-success-dark">✓</span> Team prefers predictable, easy-to-debug behavior.</li>
<li><span className="text-myso-success-dark">✓</span> Current performance meets your requirements.</li>
<li><span className="text-myso-success-dark">✓</span> Operational simplicity is valued.</li>
</ul>
...
<ul className="list-none pl-2">
<li><span className="text-myso-success-dark">✓</span> Performance optimization is essential.</li>
<li><span className="text-myso-success-dark">✓</span> Sequential processing cannot keep up with your data volume.</li>
<li><span className="text-myso-success-dark">✓</span> Your team is willing to handle the additional implementation complexity for the performance benefits.</li>
</ul>
...
<ImportContent source="crates/myso-indexer-alt-framework/src/pipeline/sequential/mod.rs" mode="code" fun="batch" noComments />

`commit()`: Database write logic.

<ImportContent source="crates/myso-indexer-alt-framework/src/pipeline/sequential/mod.rs" mode="code" fun="commit" noComments />
...
<ImportContent source="crates/myso-indexer-alt-framework/src/pipeline/sequential/committer.rs" tag="send" />
...
<ImportContent source="crates/myso-indexer-alt-framework/src/ingestion/broadcaster.rs" tag="regulator" />
...
<ImportContent mode="code" source="crates/myso-indexer-alt-framework/src/pipeline/concurrent/mod.rs" variable="MIN_EAGER_ROWS" />
...
<ImportContent mode="code" source="crates/myso-indexer-alt-framework/src/pipeline/concurrent/mod.rs" variable="MAX_PENDING_ROWS" />
...
<ImportContent mode="code" source="crates/myso-indexer-alt-framework/src/pipeline/mod.rs" struct="CommitterConfig" />
...
<ImportContent mode="code" source="crates/myso-indexer-alt-framework/src/pipeline/mod.rs" struct="WatermarkPart"/>
...
<ImportContent mode="code" source="crates/myso-indexer-alt-framework/src/pipeline/mod.rs" struct="CommitterConfig" highlight="watermark_interval_ms" />
...
<ImportContent mode="code" source="crates/myso-indexer-alt-framework/src/pipeline/concurrent/mod.rs" struct="PrunerConfig" highlight="delay_ms" />
...
<ImportContent mode="code" source="crates/myso-indexer-alt-framework/src/pipeline/concurrent/mod.rs" struct="PrunerConfig" highlight="prune_concurrency" />
...
<ImportContent mode="code" source="crates/myso-pg-db/src/schema.rs" />
...
<ImportContent mode="code" source="crates/myso-indexer-alt-framework/src/ingestion/mod.rs" struct="IngestionConfig" highlight="checkpoint_buffer_size" />   
    
**`Processor` to `Collector`**: `FANOUT + PIPELINE_BUFFER` slots → All workers block on `send()`.

<ImportContent mode="code" source="crates/myso-indexer-alt-framework/src/pipeline/concurrent/mod.rs" variable="processor_tx" />
    
**`Collector` to `Committer`**: `write_concurrency + PIPELINE_BUFFER` slots → `Collector` stops accepting.

<ImportContent mode="code" source="crates/myso-indexer-alt-framework/src/pipeline/concurrent/mod.rs" tag="buff" /> 
...
<ImportContent mode="code" source="crates/myso-indexer-alt-framework/src/pipeline/concurrent/collector.rs" tag="collector" /> 
...
<ImportContent mode="code" source="crates/myso-indexer-alt-framework/src/ingestion/broadcaster.rs" tag="bound" /> 
