...
keywords: [ using events, notify events, notify assets, notify on-chain assets, activity events, events for activities, move events, events in move, query events, myso RPC, graphql, move, filter events, rust, typescript ]
---

Move code can interact with objects stored on MySo. Some applications might want to track an object's activity and trigger certain workflows within the application based on that activity, such as application updates based on how many times an NFT is minted or the amount of MYSO a smart contract generates. 
...
<ImportContent source="crates/myso-indexer-alt-e2e-tests/packages/emit_event/sources/emit_event.move" mode="code" struct="TestEvent" noComments />

## Using the `emit` function

Use the `myso::event::emit` function to emit an event when the action you want to monitor occurs:
...
- **Polling**: Query the MySo network periodically for emitted events. This approach requires a database to store retrieved data.
...
The MySo RPC provides a [`queryEvents`](/myso-api-ref#suix_queryEvents) method to query on-chain packages and return available events. As an example, the following `curl` command queries the OrderbookV3 package on Mainnet for a specific type of event:
		...
		"data": [
			{
				...
				"transactionModule": "orderbook_utils",
			...
			},
			{
				...
				"transactionModule": "orderbook_utils",
				"sender": "0x8b35e67a519fffa11a9c74f169228ff1aa085f3a3d57710af08baab8c02211b9",
				"type": "0x0b0c::clob_v2::OrderFilled<0x2::myso::MYSO, 0x5d4b302506645c37ff133b98c4b50a5ae14841659738d6d733d59d0d217a93bf::coin::COIN>",
				"parsedJson": {
...
{
  events(
    filter: {type: "0x3164fcf73eb6b41ff3d2129346141bd68469964c2d95a5b1533e8d16e6ea6e13::Market::ChangePriceEvent<0x2::myso::MYSO>"}
  ) {
    nodes {
      transactionModule {
        name
...
The [MySo by Example](https://github.com/gdanezis/myso-by-example/blob/main/src/05_reading_events/bin/main.rs) repo on GitHub contains a code sample that demonstrates how to query events using the `query_events` function. The package that `PACKAGE_ID_CONST` points to exists on Mainnet, so you can test this code using Cargo. To do so, clone the `myso-by-example` repo locally and follow the [Example 05 directions](https://github.com/gdanezis/myso-by-example/tree/main/src/05_reading_events).

```rust
use myso_sdk::{rpc_types::EventFilter, types::Identifier, MySoClientBuilder};

const PACKAGE_ID_CONST: &str = "0x279525274aa623ef31a25ad90e3b99f27c8dbbad636a6454918855c81d625abc";

#[tokio::main]
async fn main() -> Result<(), anyhow::Error> {
    let myso_mainnet = MySoClientBuilder::default()
        .build("https://fullnode.mainnet.mysocial.network:443")
        .await?;

    let events = myso_mainnet
        .event_api()
        .query_events(
...
| :---------------- | :------------------------------------------------------- | :---------------------------------------------------------------------------------------------------- |
| `All`             | All events                                               | `{"All": []}`                                                                                         |
| `Any`             | Events emitted from any of the given filter              | `{"Any": MySoEventFilter[]}`                                                                           |
...
The [TypeScript SDK](https://sdk.mystenlabs.com/typedoc/modules/_mysten_sui.graphql.html) also can be used to interact with the MySo GraphQL service and filter events.
