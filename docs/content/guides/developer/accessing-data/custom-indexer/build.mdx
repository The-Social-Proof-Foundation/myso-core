...
- Connects to MySo Testnet: Uses the remote checkpoint store at https://checkpoints.testnet.mysocial.network.
...
```sh
$ cargo run -- --remote-store-url https://checkpoints.testnet.mysocial.network
```

**Using gRPC streaming from a full node (with remote checkpoint store fallback):**

```sh
$ cargo run -- --remote-store-url https://checkpoints.testnet.mysocial.network --streaming-url https://fullnode.testnet.mysocial.network:443
```
