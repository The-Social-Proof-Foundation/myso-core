...
description: Build a custom indexer using the `myso-indexer-alt-framework` module. The example indexer demonstrates a sequential pipeline that extracts transaction digests from MySo checkpoints and stores them in a local PostgreSQL.
...
- Connects to MySo Testnet: Uses the remote checkpoint store at https://storage.googleapis.com/mysocial-testnet-checkpoints.
...
MySo provides checkpoint stores for both Mainnet and Testnet.

- Testnet: `https://storage.googleapis.com/mysocial-testnet-checkpoints`
...
```sh
$ cargo new simple-myso-indexer
$ cd simple-myso-indexer
```
...
<ImportContent source="examples/rust/basic-myso-indexer/Cargo.toml" mode="code" />

The manifest now includes the following dependencies:

- `myso-indexer-alt-framework`: Core framework providing pipeline infrastructure.
...
```sh
$ createdb myso_indexer
```

Get your connection details:

```sh
$ psql myso_indexer -c "\conninfo"
```

If successful, your console should display a message similar to the following:

```sh
You are connected to database "myso_indexer" as user "username" via socket in "/tmp" at port "5432".
```
...
```sh
$ PSQL_URL=postgres://username@localhost:5432/myso_indexer
```
...
<ImportContent source="examples/rust/basic-myso-indexer/diesel.toml" mode="code" />
...
<ImportContent source="examples/rust/basic-myso-indexer/migrations/YYYY-MM-DD-HHMMSS_transaction_digests/up.sql" mode="code" />
...
<ImportContent source="examples/rust/basic-myso-indexer/migrations/YYYY-MM-DD-HHMMSS_transaction_digests/down.sql" mode="code" />
...
<ImportContent source="examples/rust/basic-myso-indexer/src/schema.rs" mode="code" />
...
<ImportContent source="examples/rust/basic-myso-indexer/src/models.rs" mode="code" />

Key annotations:

- `FieldCount`: Required by `myso-indexer-alt-framework` for memory optimization and batch processing efficiency. It is used to limit the max size of a batch so that we don't exceed the postgres limit on the number of bind parameters a single SQL statement can have.
...
<ImportContent source="examples/rust/basic-myso-indexer/src/handlers.rs" mode="code" struct="TransactionDigestHandler" />
...
<ImportContent source="examples/rust/basic-myso-indexer/src/handlers.rs" mode="code" tag="processordeps" />

After the `TransactionDigestHandler` struct, add the `Processor` code:

<ImportContent source="examples/rust/basic-myso-indexer/src/handlers.rs" mode="code" tag="processor" />
...
<ImportContent source="crates/myso-indexer-alt-framework/src/pipeline/processor.rs" mode="code" trait="Processor" />
...
<ImportContent source="examples/rust/basic-myso-indexer/src/handlers.rs" mode="code" tag="handlerdeps" />

Add the logic for `Handler` after the `Processor` code. The complete code is available at the end of this step.

<ImportContent source="examples/rust/basic-myso-indexer/src/handlers.rs" mode="code" tag="handler" />
...
<ImportContent source="crates/myso-indexer-alt-framework/src/pipeline/sequential/committer.rs" mode="code" tag="batch" />
...
<ImportContent source="examples/rust/basic-myso-indexer/src/handlers.rs" mode="code" />
...
<ImportContent source="crates/myso-indexer-alt-framework/src/pipeline/sequential/committer.rs" mode="code" trait="Handler" />
...
<ImportContent source="examples/rust/basic-myso-indexer/src/main.rs" mode="code" />
...
```sh
$ cargo run -- --remote-store-url https://storage.googleapis.com/mysocial-testnet-checkpoints
```

**Using gRPC streaming from a full node (with remote checkpoint store fallback):**

```sh
$ cargo run -- --remote-store-url https://storage.googleapis.com/mysocial-testnet-checkpoints --streaming-url https://fullnode.testnet.mysocial.network:443
```

:::info

Allow incoming network requests if your operating system requests it for the `basic-myso-indexer` application.
...
To confirm your data is accurate, copy any transaction digest from your database and verify it on MySoScan: [https://suiscan.xyz/testnet/home](https://suiscan.xyz/testnet/home)
...
<RelatedLink href="https://github.com/the-social-proof-foundation/myso-core/tree/main/crates/myso-indexer-alt" label="MySo Indexer Alt" desc="The `myso-indexer-alt` crate in the MySo repo." />
...
<RelatedLink href="https://github.com/MystenLabs/orderbookv3/tree/main/crates/indexer" label="Orderbook Indexer" desc="The indexer that Orderbook implements." />
