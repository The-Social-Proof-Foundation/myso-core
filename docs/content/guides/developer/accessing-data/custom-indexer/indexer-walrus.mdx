...
description: Walrus is a content-addressable storage protocol, where data is retrieved using a unique identifier derived from the content itself, rather than from a file path or location. Integrating a custom MySo Indexer with Walrus can provide novel user experiences.
...
For uniqueness, each blob uploaded to Walrus also creates a corresponding [`Blob` NFT object onMySo](https://docs.wal.app/dev-guide/myso-struct.html#blob-and-storage-objects) with a unique ID. Furthermore, the associated `Blob` object can optionally have a `Metadata` [dynamic field](/guides/developer/objects/dynamic-fields.mdx). 
...
<RelatedLink href="https://github.com/the-social-proof-foundation/myso-core/tree/main/examples/rust/walrus-attributes-indexer" label="Indexer with Walrus example files" desc="Directory in the MySo repo containing the files for this example." />
