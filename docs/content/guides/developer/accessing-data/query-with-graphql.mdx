---
title: Querying MySo RPC with GraphQL
description: Practical guide to making queries of the MySo RPC using the GraphQL service, with examples for common tasks.
keywords: [ rpc, myso rpc, graphql, query transactions, query blocks, query block info, query dynamic field, execute transaction, graphql examples ]
---

This guide provides practical examples for querying the MySo network using GraphQL. For core concepts like headers, variables, fragments, pagination, scope, and limits, see [GraphQL for MySo RPC](/concepts/data-access/graphql-rpc.mdx).
...
- Mainnet: https://graphql.mainnet.mysocial.network/graphql
- Testnet: https://graphql.testnet.mysocial.network/graphql
- Devnet: https://graphql.devnet.mysocial.network/graphql

:::caution

Switch any apps that still use the GraphQL Alpha endpoints (`https://graphql.mainnet.mysocial.network/graphql`, `https://myso-testnet.mystenlabs.com/graphql`, and `https://myso-devnet.mystenlabs.com/graphql`) as soon as possible. Migrate your apps to the GraphQL Beta endpoints to avoid disruption of service.

:::

Consult the [GraphQL reference](../../../references/myso-graphql.mdx) for full documentation on the supported schema.
...
Find all the transactions that touched (modified, transferred, or deleted) a given object. Useful when you want to trace the flow of a `Coin`, `StakeMySo`, `NFT`, or similar object.
...
query ($address: MySoAddress!) {
  transactions(filter: {
    function: "0x3::myso_system::request_add_stake"
    sentAddress: $address
  }) {
    nodes {
      digest
...
Paginate over the dynamic fields of an object. This works even when the object in question is [wrapped](/guides/developer/objects/object-ownership/wrapped.mdx) by using the <a href="/references/myso-api/myso-graphql/alpha/reference/operations/queries/owner.mdx" data-noBrokenLinkCheck='true'>owner query</a>. It can be used for iterating over the elements of on-chain data structures, like tables and bags. See [The Move Book](https://move-book.com/programmability/dynamic-collections.html) to learn more about dynamic collections available in Move.
...
Transaction execution takes 2 arguments, `transactionDataBcs` and `signatures`. `transactionDataBcs` is the serialized unsigned transaction data, which the [MySo Client CLI](/references/cli/client.mdx) `client call` command can generate. Pass the `--serialize-unsigned-transaction` flag to the command to call a Move function. The [MySo Keytool CLI](/references/cli/keytool.mdx) command `myso keytool sign` can generate the `signatures`.
...
The examples in the following sections demonstrate typical API calls. The calls are tabbed to show both the JSON-RPC call and the equivalent GraphQL structure. For a comprehensive list of all available GraphQL features, consult the [reference](/references/myso-graphql).
...
Return all `Coin<0x2::myso::MYSO>` objects that an address owns.
...
query {
  address(address: "0x5094652429957619e6efa79a404a6714d1126e63f551f4b6c7fb76440f8118c9") {
    objects(
      ...
      filter: { type: "0x2::coin::Coin<0x2::myso::MYSO>" }
    ) {
      nodes {
        address
      }
...
Find all versions of the MySo framework and list their modules:
