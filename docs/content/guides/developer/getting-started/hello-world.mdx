...
keywords: [ first dapp, first package, move package, first move package, hello world, hello, world, what is move, what is a package, what is an object, move object, myso move object, myso move package, myso move module, move module, build package, publish package, call package, make call to package, how to call package ]
pagination_prev: null
pagination_next: null
---

You'll build a "Hello, World!" program to learn the fundamentals of programming on MySo. You create programs on MySo by writing and deploying smart contracts to the network.

The most basic unit of storage on MySo is an [_object_](/guides/developer/objects/object-model.mdx). Other blockchains typically structure storage using key-value stores. MySo centers storage around objects with unique ID addresses on-chain. Every MySo smart contract is an object that manipulates other objects.
...
- **Mutable** objects can be transferred, changed, and deleted. A MySo address can own them, or they can be shared for public access.
...
MySo has limits on the maximum transaction size (128KB) and number of objects (2,048) used in a transaction. For more information on limits, see [Building Against Limits](https://move-book.com/guides/building-against-limits/) in The Move Book.

## What is Move? 

[Move](/concepts/myso-move-concepts.mdx) is the programming language MySo uses to create smart contracts. It is platform agnostic and enables common libraries, tooling, and developer communities across blockchains with vastly different data and execution models. There are three ways to use Move in the context ofMySo: Move packages, Move modules, and Move objects.

A MySo **Move package** is also referred to as a Move smart contract. It is a set of Move bytecode published to the MySo network. It is immutable and cannot be changed or removed, however you can upgrade it. Upgrading creates a new version of the package object on-chain, leaving the original intact. All prior versions of a package still exist on-chain. Once you publish it, other packages can import and use the modules it provides. Anyone can view a package's contents and use a MySo Explorer to see how its logic manipulates other objects. 

Every Move package on MySo includes one or more **MySo Move modules** that define the package's interaction with on-chain objects. A module's name is always unique within the package that contains it.

A MySo Move module governs a MySo **Move object**, which is typed data from a MySo Move package. Each Move object value is a struct with fields that can contain primitive types, such as integers and addresses, other objects, and non-object structs.
...
- [x] [Install the latest version ofMySo](/guides/developer/getting-started/myso-install.mdx).

- [x] [Configure the MySo client](/guides/developer/getting-started/configure-myso-client.mdx).

- [x] [Create a MySo address](/guides/developer/getting-started/get-address.mdx).

- [x] [Get MYSO Testnet tokens](/guides/developer/getting-started/get-coins.mdx).
...
To demonstrate creating objects, packages, and how to build your first MySo application, start by cloning the "Hello, World!" example:

```
$ git clone \
  https://github.com/the-social-proof-foundation/myso-core-stack-hello-world.git
$ cd myso-stack-hello-world/move/hello-world
```
...
<ImportContent source="/move/hello-world/Move.toml" mode="code" org="MystenLabs" repo="myso-stack-hello-world" />
...
<ImportContent source="/move/hello-world/sources/greeting.move" mode="code" org="MystenLabs" repo="myso-stack-hello-world" />
...
<ImportContent source="/move/hello-world/sources/greeting.move" mode="code" struct="Greeting" org="MystenLabs" repo="myso-stack-hello-world" />
...
<ImportContent source="/move/hello-world/sources/greeting.move" mode="code" fun="new" org="MystenLabs" repo="myso-stack-hello-world" />

Lastly, the package defines a function called `update_text` that can be called to update the text stored in `Greeting`:

<ImportContent source="/move/hello-world/sources/greeting.move" mode="code" fun="update_text" org="MystenLabs" repo="myso-stack-hello-world" />

### Resource safety 

A unique aspect of programming applications on MySo is the resource safety enforced by the Move Bytecode Verifier. Move packages must satisfy the following resource safety parameters:
...
[Learn more](https://github.com/the-social-proof-foundation/myso-core/blob/main/external-crates/move/move-execution/v1/crates/move-bytecode-verifier/README.md) about the Move Bytecode Verifier.
...
To build your "Hello, World!" package, first confirm your working directory is `~/myso-stack-hello-world/move/hello-world`, then run the following command:

```
$ myso move build 
```

The build process fetches and compiles the dependencies defined in the `Move.toml` file. The Move compiler checks your `.move` code for type errors, syntax errors, and enforces [resource safety](#resource-safety), then translates your `.move` code into bytecode that MySo can execute.

:::info 

You must build your package before you can publish it, but also before you test it. You cannot run tests (`myso move test`) on your code until it has been built.
...
This should return `testnet`. If it does not return `testnet`, follow the [client configuration instructions](/guides/developer/getting-started/configure-myso-client.mdx) before continuing.

Then, check your balance of MYSO tokens to confirm you have enough to publish to Testnet:
...
If you do not have a balance, follow the [MYSO faucet instructions](/guides/developer/getting-started/get-coins.mdx).
...
When you publish a Move package to the network, the network uploads and stores the bytecode as a Move package with a unique package ID and version number. The network consumes MYSO tokens as gas and processes the transaction on-chain.
...
```
$ myso client call --package <PACKAGE_ID> --module greeting --function new  
```

Replace `<PACKAGE_ID>` with the package ID the output of the `myso client publish` command returned. You must include the `--package`, `--module`, and `--function` flags. 
...
If you receive this error, you must wait until the current epoch is over before submitting your transaction again. You can see how long is left in the current epoch using [MySo Explorer](https://suivision.xyz/) or another network explorer like [MySoScan](https://suiscan.xyz/mainnet/home). 
...
<div className="next-steps-module">
  ...
  <div className="next-steps-grid">
    ...
    <Card className="plausible-event-name=hello+data+button"
      title="Access MySo Data" 
      href="/concepts/data-access/data-serving"
    >
      Learn more about accessing data on MySo.
    </Card>
    <Card className="plausible-event-name=hello+community+button"
      title="Join the Community" 
      href="/guides/developer/getting-started/next-steps"
    >
      Join the MySo developer community, try out other example projects, or read more documentation.
    </Card>
  </div>
</div>
