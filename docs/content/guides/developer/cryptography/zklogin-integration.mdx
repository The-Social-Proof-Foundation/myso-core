...
description: zkLogin can be integrated into applications deployed on MySo.
keywords: [ zklogin, how to use zklogin, generate ephemeral key pair, fetch JWT, decode JWT, generate salt, generate user MySo address, fetch ZK proof, assemble zkLogin signature, oauth, openid ]
---
...
1. The wallet obtains a unique user salt based on a JWT. Use the OAuth subject identifier and salt to compute the zkLogin MySo address.
...
```sh npm2yarn
$ npm install @mysten/myso@experimental
```

## Get JWT

1. Generate an ephemeral key pair. Follow the same process as you would generating a key pair in a traditional wallet. See [MySo SDK](https://sdk.mystenlabs.com/typescript/cryptography/keypairs) for details.
...
const FULLNODE_URL = 'https://fullnode.devnet.mysocial.network:443'; // replace with the RPC URL you want to use
const suiClient = new MySoGrpcClient({ baseUrl: FULLNODE_URL, network: 'devnet' });
const { epoch, epochDurationMs, epochStartTimestampMs } = await suiClient.core.getLatestMySoSystemState();
...
zkLogin uses the user salt to compute the zkLogin MySo address (see [definition](../../../concepts/cryptography/zklogin.mdx#address-definition)). The salt must be a 16-byte value or a integer smaller than `2n**128n`. There are several options for the application to maintain the user salt:
...
User salt is used to disconnect the OAuth identifier (sub) from the on-chain MySo address to avoid linking Web2 credentials with Web3 credentials. While losing or misusing the salt could enable this link, it wouldn't compromise fund control or zkLogin asset authority. 

## Get the user's MySo address
...
```typescript
import { jwtToAddress } from '@socialproof/myso/zklogin';
...
```typescript
import { getExtendedEphemeralPublicKey } from '@socialproof/myso/zklogin';
...
1. Download the [Groth16 proving key zkey file](https://docs.circom.io/getting-started/proving-circuits/). There are zkeys available for all MySo networks.

   - Main zkey (for Mainnet and Testnet)
     ```sh
     $ wget -O - https://raw.githubusercontent.com/myso-foundation/zklogin-ceremony-contributions/main/download-main-zkey.sh | bash
     ```
   - Test zkey (for Devnet)

     ```sh
     $ wget -O - https://raw.githubusercontent.com/myso-foundation/zklogin-ceremony-contributions/main/download-test-zkey.sh | bash
     ```
...
const client = new MySoGrpcClient({ baseUrl: '<YOUR_RPC_URL>', network: 'mainnet' });
...
```typescript
import { genAddressSeed, getZkLoginSignature } from '@socialproof/myso/zklogin';
...
The prover that Mysten Labs maintains is set to auto-scale to handle traffic surges. If you are not sure whether Mysten Labs can handle a specific number of requests or expect a sudden spike in the number of prover requests your application needs to make, please reach out to us on [Discord](https://discord.gg/myso). Our plan is to horizontally scale the prover to handle any RPS you require.
