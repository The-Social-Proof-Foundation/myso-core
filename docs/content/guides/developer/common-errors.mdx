...
Errors in this section relate to MySo addresses.
...
Indicates that the address you provided does not conform to a valid MySo address format. MySo addresses must:
...
- [Create a MySo Address](/guides/developer/getting-started/get-address.mdx)
...
- [`myso::url`](/references/framework/myso_sui/url.md)

- [`myso::url` source code](https://github.com/the-social-proof-foundation/myso-core/blob/c5f8b33b8090d75b1922f1497916b4010d9140d7/crates/myso-framework/sources/url.move).
...
- [Move Concepts](/concepts/myso-move-concepts.mdx)
...
Indicates that a module in your package does not have the address `0x0`. In Move, you must set all modules to have their self-address as `0x0` in the source code. When you publish the package, MySo automatically assigns it the correct address. 
...
- [Error definition](https://github.com/the-social-proof-foundation/myso-core/blob/63cbf0dc26d0f2329d3e0a03639e31ded9f93561/crates/myso-adapter-transactional-tests/tests/myso/publish_module_non_zero_address.snap)

---

#### `The signer only expects one signature, not two`

Indicates that a function you are calling in your Move module expects 1 signer argument, yet you provided 2. OnMySo, the number of signer arguments in your function's signature must match the number of signers you provide. 
...
- [Error definition](https://github.com/the-social-proof-foundation/myso-core/blob/63cbf0dc26d0f2329d3e0a03639e31ded9f93561/external-crates/move/crates/bytecode-verifier-transactional-tests/tests/script_signature/signer_double_signer.snap)
...
- [InstallMySo](/guides/developer/getting-started/myso-install.mdx)
...
Indicates the network does not receive a response within a certain time period. This error is not specific to MySo.
...
This error occurs when the network is down, unreachable due to an unstable internet connection, or there is a misconfiguration in your MySo client configuration.
...
devnet => https://fullnode.devnet.mysocial.network:443 (active)
```
...
- [Configure a MySo Client](/guides/developer/getting-started/configure-myso-client.mdx)
...
This error occurs when a Move transaction does not have a `drop` ability, or you do not explicitly drop it. The MySo runtime system enforces resource safety that throws this error when you leave values unused at the end of a transaction. 

##### Solution 

In MySo Move packages, you must either transfer every value that does not have the `drop` ability to another address, explicitly destroy it, or use it as input for another function to consume. 
...
- `UNKNOWN_VERSION`: Your module or enum uses a version or feature the system does not recognize. Ensure you are using supported features and the correct MySo toolchain version. 
...
- [Error: `ZERO_SIZED_STRUCT`](https://github.com/the-social-proof-foundation/myso-core/blob/63cbf0dc26d0f2329d3e0a03639e31ded9f93561/crates/myso-verifier-transactional-tests/tests/one_time_witness/no_field.snap)

- [Error: `FIELD_MISSING_TYPE_ABILITY`](https://github.com/the-social-proof-foundation/myso-core/blob/63cbf0dc26d0f2329d3e0a03639e31ded9f93561/crates/myso-verifier-transactional-tests/tests/struct_with_key/key_struct_with_drop.snap)

- [Error: `CONSTRAINT_NOT_SATISFIED`](https://github.com/the-social-proof-foundation/myso-core/blob/63cbf0dc26d0f2329d3e0a03639e31ded9f93561/crates/myso-verifier-transactional-tests/tests/private_generics/receive_without_key.snap)
...
Indicates that your transaction is trying to use one or more MySo objects that another transaction already uses. When you use an object in a transaction, the system locks that object to prevent multiple, possibly conflicting modifications to an object. For DeFi objects, this prevents double-spending.
...
- [Avoiding Equivocation](/concepts/myso-architecture/epochs.mdx)

---

#### `Unable to process transaction. Unexpected status code: 403`

Indicates the MySo network responds to an RPC or API request with `HTTP 403: Forbidden` status. This might happen if an RPC provider:
...
Indicates the MySo network rejects a transaction due to invalid transaction inputs. 
...
- [Get MYSO from Faucet](/guides/developer/getting-started/get-coins.mdx)
