...
description: Programmable transaction blocks are a group of commands that complete a transaction on MySo.
...
- `tx.moveCall({ target, arguments, typeArguments })`: Executes a Move call. Returns whatever the MySo Move call returns.
...
- **`GasCoin`:** References the MYSO coin used to pay for gas. This is separate from other inputs because it has special restrictions. It can only be taken by value through the `TransferObjects` command. To use a portion of the gas coin elsewhere, first split off a coin with [`SplitCoins`](/references/ptb-commands.mdx).
...
The most important thing to note at this stage is the effects on the gas coin. At the beginning of execution, the maximum gas budget (in terms of MYSO) is withdrawn from the gas coin. Any unused gas is returned to the gas coin at the end of execution, even if the coin has changed owners.
...
- If the signature expects a `T`, the runtime verifies the argument has type `T`, then copies the value if `T: copy` or moves it otherwise. Objects are always moved because `myso::object::UID` does not have the `copy` ability.
...
Object inputs have the type `T` of the underlying object. The exception is `ObjectArg::Receiving` inputs, which have type `myso::transfer::Receiving<T>`. This wrapper indicates the object is owned by another object, not the sender. Call `myso::transfer::receive` with the parent object to unwrap it and prove ownership.
...
Any remaining MYSO deducted from the gas coin at the beginning of execution is returned to the coin, even if the owner has changed. The maximum possible gas is deducted at the beginning of execution, and unused gas is returned at the end (all in MYSO). Because the gas coin can only be taken by value with `TransferObjects`, it has not been wrapped or deleted.

The total effects (created, mutated, and deleted objects) are then passed out of the execution layer and applied by the MySo network.
...
```rust
{
  ...
  commands: [
    ...
    MoveCall("myso", "tx_context", "sender", [], []),
    TransferObjects([NestedResult(1, 1)], NestedResult(3, 0)),
  ]
}
```

The inputs include the friend's address, the marketplace object, and the coin split value. The commands split off the coin, call the marketplace function, send the gas coin and one object, grab your address (through `myso::tx_context::sender`), and send the remaining object to yourself.
...
| Result | `[Coin<MYSO> { id: new_coin, value: 100 }]` |

**Command 1: `MoveCall("some_package", "some_marketplace", "buy_two", ...)`**

Calls `buy_two(marketplace: &mut Marketplace, coin: Coin<MYSO>, ctx: &mut TxContext): (Item, Item)`. Uses `Input(1)` by mutable reference (marketplace) and `NestedResult(0, 0)` by value (coin is moved and consumed).
...
**Command 3: `MoveCall("myso", "tx_context", "sender", [], [])`**
...
```rust
Gas Coin: Coin<MYSO> { id: gas_coin, balance: 1_000_000u64 }
...
```rust
Gas Coin: Coin<MYSO> { id: gas_coin, balance: 500_000u64 }
```
