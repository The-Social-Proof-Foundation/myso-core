...
description: Each transaction on MySo represents a call to a specific functionality that executes with inputs that define the result of the transaction.
...
Transactions in MySo represent calls to specific functionality that execute on inputs to define the result of the transaction.
...
The signature is provided with the private key owned by the wallet, and its public key must be consistent with the transaction sender's MySo address.

MySo uses a `MySoKeyPair` to produce the signature, which commits to the Blake2b hash digest of the intent message (`intent || bcs bytes of tx_data`). The signature schemes currently supported are:
...
The MySo SDK offers transaction executors to help process multiple transactions from the same address.
...
The following examples demonstrate how to sign and execute transactions using Rust, TypeScript, or the MySo CLI.

<Tabs groupId="code-language">

<TabItem value="ts-sdk" label="TypeScript">

There are various ways to instantiate a key pair and to derive its public key and MySo address using the MySo TypeScript SDK.
...
// define myso client for the desired network.
...
The full code example below can be found under [crates/myso-sdk](https://github.com/the-social-proof-foundation/myso-core/blob/main/crates/myso-sdk/examples/sign_tx_guide.rs).

There are various ways to instantiate a `MySoKeyPair` and to derive its public key and MySo address using the MySo Rust SDK.
...
```rust
    ...
    // if you would like to verify the signature locally before submission, use this function. if it fails to verify locally, the transaction will fail to execute in MySo.
...
<TabItem value="cli" label="MySo CLI">

When using the [MySo CLI](/references/cli.mdx) for the first time, it creates a local file in `~/.myso/keystore` on your machine with a list of private keys (encoded as Base64 encoded `flag || 32-byte-private-key`). You can use any key to sign transactions by specifying its address. Use `myso keytool list` to see a list of addresses.
...
```
```sh
$ myso client transfer-myso --to $MYSO_ADDRESS --myso-coin-object-id $GAS_COIN_ID --gas-budget $GAS_BUDGET --serialize-unsigned-transaction
```
```sh
$ myso keytool sign --address $MYSO_ADDRESS --data $TX_BYTES
```
```sh
$ myso client execute-signed-tx --tx-bytes $TX_BYTES --signatures $SERIALIZED_SIGNATURE
```
