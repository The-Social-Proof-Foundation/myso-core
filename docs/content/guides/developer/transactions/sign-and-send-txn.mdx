...
MySo uses a `SuiKeyPair` to produce the signature, which commits to the Blake2b hash digest of the intent message (`intent || bcs bytes of tx_data`). The signature schemes currently supported are:
...
There are various ways to instantiate a `SuiKeyPair` and to derive its public key and MySo address using the MySo Rust SDK.
...
```
```sh
$ myso client transfer-myso --to $SUI_ADDRESS --myso-coin-object-id $GAS_COIN_ID --gas-budget $GAS_BUDGET --serialize-unsigned-transaction
```
```sh
$ myso keytool sign --address $SUI_ADDRESS --data $TX_BYTES
```
```sh
