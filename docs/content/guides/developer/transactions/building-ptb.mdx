---
title: Building PTBs
description: Using the MySo TypeScript SDK, you can create programmable transaction blocks to perform multiple commands in a single transaction.
keywords:
  [
    ptbs,
...
This guide explores creating a [programmable transaction block (PTB)](/guides/developer/transactions/prog-txn-blocks.mdx) on MySo using the [TypeScript SDK](https://sdk.mystenlabs.com/typescript/install).

This example constructs a PTB to send MYSO tokens. First, import the `Transaction` class, and construct it:
...
tx.transferObjects([coin], tx.pure('address', '0xSomeMySoAddress'));
```
...
// Procure a list of some MySo transfers to make:
...
After you have the transaction defined, you can directly execute it with a MySo client and `KeyPair` using `client.signAndExecuteTransaction`.
...
Inputs are how you provide external values to PTBs, for example, an amount of MYSO to transfer, an object to pass into a Move call, or a shared object.
...
- For pure values: The `tx.pure(type, value)` function is used to construct an input for a non-object input.
  ...
  - In the new version, a more intuitive way of writing is provided, for example: `tx.pure.u64(100)`, `tx.pure.string('SomeString')`, `tx.pure.address('0xSomeMySoAddress')`, `tx.pure.vector('bool', [true, false])`...
...
MySo represents the gas budget in MYSO and you should take the gas price of the PTB into account.
...
To serialize a PTB for sending to a wallet, MySo recommends using the `tx.serialize()` function, which returns an opaque string representation of the PTB that can be passed from the Wallet Standard app context to your wallet. You can then convert this back into a `Transaction` using `Transaction.from()`.
