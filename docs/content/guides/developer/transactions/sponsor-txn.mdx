...
description: Sponsored transactions let you pay gas fees on behalf of another user, reducing onboarding friction for users who don't own MYSO.
...
A transaction on MySo requires a gas payment to execute. The payment is a list of `0x2::coin::Coin<0x2::myso::MYSO>` objects.

Sponsored transactions let you pay gas fees on behalf of another user. This reduces onboarding friction because users can execute transactions without owning MYSO or understanding gas mechanics. As a sponsor, you provide the gas payment object for the transaction.
...
`GasLessTransactionData` is `TransactionData` without `GasData`. It is not a `myso-core` data structure; it is only an interface between user and sponsor.
...
1. The user verifies and then signs `TransactionData` and sends the dual-signed transaction to MySo network through a full node or the sponsor.
...
1. The user submits the dual-signed transaction to a MySo full node or sponsor to execute it.
