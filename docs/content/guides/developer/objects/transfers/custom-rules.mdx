...
description: Custom transfer rules enable you to define a set of rules that must be met before MySo considers a transfer operation valid.
...
With these rules in place, there are different custom transfer rules for any object `Object`. Either it's unlocked and anyone can transfer it, or it's locked and it can only be transferred to `0xCAFE`. Importantly, these are the only ways of transferring any object of type `Object`. In particular, because `Object` does not have the `store` ability, you cannot transfer it using the [`myso::transfer::public_transfer`](/references/framework/myso_sui/transfer#myso_transfer_public_transfer) function.