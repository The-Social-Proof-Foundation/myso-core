...
description: OnMySo, you can transfer objects to objects in the same way you can transfer objects to addresses.
...
The core receiving interface in Move is defined in the [`transfer`](https://docs.mysocial.network/references/framework/myso_sui/transferx) module in the MySo framework:
...
///
/// This function has custom rules that the MySo Move bytecode verifier enforces to ensure
...
Each `Receiving` argument referring to a sent object of type `T` in a PTB results in exactly 1 argument with type `myso::transfer::Receiving<T>`. You can then use this argument to receive the sent object of type `T` with the `transfer::receive` function.
...
Just like with [custom transfer policies](/guides/developer/objects/transfers/custom-rules.mdx), MySo allows for the definition of custom receivership rules for `key`-only objects. In particular, you can use the `transfer::receive` function only on objects defined in the same module as the call to [`transfer::receive`](/references/framework/myso_sui/transfer#myso_transfer_receive), just like you can use the [`transfer::transfer`](/references/framework/myso_sui/transfer#myso_transfer_transfer) function only on objects defined in the module where it's being used.

Similarly for objects that also have the `store` ability, anyone can use the [`transfer::public_receive`](/references/framework/myso_sui/transfer#myso_transfer_public_receive) function to receive them, just like [`transfer::public_transfer`](/references/framework/myso_sui/transfer#myso_transfer_public_transfer) can transfer any objects that have the `store` ability on them.
...
Interact with `Receiving` transaction inputs as you would with other object arguments in the MySo TypeScript SDK. For example, if in the [Simple Account](#simple-account) example you want to send a transaction that receives a coin object with ID `0xc0ffee` that was sent to your account at `0xcafe`, you can do the following using either the [MySo TypeScript SDK](https://sdk.mystenlabs.com/typescript) or [MySo Rust SDK](https://github.com/the-social-proof-foundation/myso-core/tree/main/crates/myso-sdk):
...
client
  ...
  .move_call(
     ...
     vec!["0x2::myso::MYSO"],
     vec![
       MySoJsonValue::from_object_id("0xcafe"),
       MySoJsonValue::from_object_id("0xc0ffee") // 0xcoffee is turned into the `Receiving<...>` argument of `accept_payment` by the SDK
     ])
  ...
```
