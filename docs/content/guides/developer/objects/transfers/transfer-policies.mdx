...
description: Learn about transfer policies on MySo.
keywords: [ transfer, policy, transferpolicy, transfer receipt, transfer request, transferrequest ]
---

A [`TransferPolicy`](https://docs.mysocial.network/references/framework/myso_sui/transfer_policy) on MySo is a customizable primitive that enables the owner of a type to create custom rules that define how the type can be transferred. You can set any number of rules, such as paying a royalty or commission, and all must be satisfied in a single transaction for the transfer to succeed. A `TransferPolicy` can be used within a marketplace or any other system that integrates the `TransferPolicy` primitive. Creating and sharing a `TransferPolicy<T>` makes the type `T` tradable in that system. On every purchase, the `TransferPolicy` must confirm the `TransferRequest`, or the transaction fails.
...
module examples::dummy_rule {
    ...
    ///
    /// This example also illustrates that rules can add Coin<MYSO>
...
This example module has no configuration and accepts a `Coin<MYSO>` of any value.
...
- The object ID that was used for selling, such as a marketplace

The `myso::transfer_policy` module provides public functions to access the fields `paid()`, `item()`, and `from()`.
...
When a purchase occurs in a system that uses TransferPolicy, the system creates a `TransferRequest` hot potato, and only the matching `TransferPolicy` can confirm it to unblock the transaction.

A marketplace can trade an item of type `T` only if the `TransferPolicy` for `T` exists and the buyer can access it. A purchase issues a `TransferRequest` that must be resolved in a matching `TransferPolicy`. If no policy exists or the buyer can't access it, the transaction fails.
