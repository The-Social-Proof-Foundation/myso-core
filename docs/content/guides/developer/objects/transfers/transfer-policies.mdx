...
A [`TransferPolicy`](https://docs.mysocial.network/references/framework/myso_sui/transfer_policy) on MySo is a customizable primitive that enables the owner of a type to create custom rules that define how the type can be transferred. You can set any number of rules, such as paying a royalty or commission, and all must be satisfied in a single transaction for the transfer to succeed. A `TransferPolicy` can be used within a MySo Kiosk marketplace or any other system that integrates the `TransferPolicy` primitive. 
