...
Objects on MySo can either be immutable or mutable. An immutable object cannot be mutated, transferred, or deleted. These objects have no owner and are freely accessible to everyone.

## Create immutable objects

To make an object immutable, call the [`myso::transfer::public_freeze_object`](/references/framework/myso_sui/transfer#myso_transfer_public_freeze_object) function from the [transfer module](https://github.com/the-social-proof-foundation/myso-core/blob/main/crates/myso-framework/packages/myso-framework/sources/transfer.move):
...
You can see this function used in the [`color_object` example module](https://github.com/the-social-proof-foundation/myso-core/blob/main/examples/move/color_object/sources/example.move) tests. The test creates a new address-owned `ColorObject` object, then calls `public_freeze_object` to turn it into an immutable object.
...
The [`myso::transfer::public_freeze_object`](/references/framework/myso_sui/transfer#myso_transfer_public_freeze_object) function requires that you pass the object by value. If you are allowed to pass the object by a mutable reference, you could still mutate the object after the [`myso::transfer::public_freeze_object`](/references/framework/myso_sui/transfer#myso_transfer_public_freeze_object) call. This contradicts the fact that it should have become immutable.
...
After an object becomes immutable, the rules of who can use this object in MySo Move calls change:

1. You can only pass an immutable object as a read-only, immutable reference to MySo Move entry functions as `&T`.
...
```sh
$ export ADDR=`myso client active-address`
```
```sh
$ myso client objects $ADDR
```

Publish the `ColorObject` code on-chain using the MySo CLI:

<ImportContent source="info-gas-budget.mdx" mode="snippet" />

```sh
$ myso client publish $ROOT/examples/move/color_object --gas-budget <GAS-AMOUNT>
```

Set the package object ID to the `$PACKAGE` environment variable, if you have it set. Then create a new `ColorObject`:

```sh
$ myso client call --gas-budget <GAS-AMOUNT> --package $PACKAGE --module "color_object" --function "create" --args 0 255 0
```
...
```sh
$ myso client call --gas-budget <GAS-AMOUNT> --package $PACKAGE --module "color_object" --function "freeze_object" --args \"$OBJECT\"
```
...
```sh
$ myso client call --gas-budget <GAS-AMOUNT> --package $PACKAGE --module "color_object" --function "update" --args \"$OBJECT\" 0 0 0
```
