...
description: Dynamic fields and dynamic object fields on MySo are added and removed dynamically, affect gas only when accessed, and store heterogeneous values.
...
Fortunately, MySo provides dynamic fields with arbitrary names, not just identifiers, that are added and removed on-the-fly, which only affect gas when they are accessed. Dynamic fields can store heterogeneous values. 
...
| --- | --- | --- |
| Fields | Can store any value that has `store`, however an object stored in this kind of field is considered wrapped and is not accessible through its ID by external tools (explorers, wallets, and so on).| [`dynamic_field`](https://github.com/the-social-proof-foundation/myso-core/tree/main/crates/myso-framework/packages/myso-framework/sources/dynamic_field.move) |
| Object field | Values must be objects (have the `key` ability, and `id: UID` as the first field), but are still accessible at their ID to external tools.| [`dynamic_object_field`](https://github.com/the-social-proof-foundation/myso-core/tree/main/crates/myso-framework/packages/myso-framework/sources/dynamic_object_field.move) |
...
Use the `add` function from the relevant MySo framework module to add dynamic fields:

**Dynamic field**

<ImportContent source="crates/myso-framework/packages/myso-framework/sources/dynamic_field.move" mode="code" fun="add" noComments />

**Dynamic object field**

<ImportContent source="crates/myso-framework/packages/myso-framework/sources/dynamic_object_field.move" mode="code" fun="add" noComments />
...
<ImportContent source="crates/myso-framework/packages/myso-framework/sources/dynamic_field.move" mode="code" fun="borrow" noComments />

<ImportContent source="crates/myso-framework/packages/myso-framework/sources/dynamic_field.move" mode="code" fun="borrow_mut" noComments />

`object` is the UID of the object the field is defined on and `name` is the field's name.

:::info

`myso::dynamic_object_field` has equivalent functions for object fields, but with the added constraint `Value: key + store`.
...
<ImportContent source="crates/myso-framework/packages/myso-framework/sources/dynamic_field.move" mode="code" fun="remove" noComments />
...
`myso::dynamic_object_field` has an equivalent function for object fields.
...
MySo provides `Table` and `Bag` collections built using dynamic fields, but with additional support to count the number of entries they contain to protect against accidental deletion when non-empty. To learn more, see [Tables and Bags](/guides/developer/objects/tables-bags.mdx).

## Related links

<RelatedLink href="https://move-book.com/programmability/dynamic-fields.html" label="Dynamic fields" desc="Learn more about Dynamic Fields in The Move Book. The Move Book is a comprehensive guide to the Move programming language and the MySo blockchain." />
