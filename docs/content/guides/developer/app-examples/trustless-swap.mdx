...
description: An app that performs atomic swaps on MySo. Atomic swaps are similar to escrows but without requiring a trusted third party.
effort: large
keywords:
  [
    trustless,
...
This guide demonstrates how to make an app that performs atomic swaps on MySo. Atomic swaps are similar to escrows but without requiring a trusted third party.
...
- You have the latest version of MySo installed. If you run `myso --version` in your terminal or console, it responds with the currently installed version.
- Your active environment is pointing to the expected network. Run `myso client active-env` to make sure. If you receive a warning about a client and server API version mismatch, update MySo using the version in the relevant branch (`mainnet`, `testnet`, `devent`) of the MySo repo.
- Your active address has MYSO. Run `myso client balance` in your terminal or console. If there is no balance, [acquire MYSO](/guides/developer/getting-started/get-coins.mdx) from the faucet (not available in Mainnet).
...
In this part of the guide, you write the Move contracts that perform the trustless swaps. The guide describes how to create the package from scratch, but you can use a fork or copy of the example code in the MySo repo to follow along instead. See [Hello, World!](/guides/developer/getting-started/hello-world.mdx) to learn more about package structure and how to use the MySo CLI to scaffold a new project.
...
If you are targeting a network other than Testnet, be sure to update the `rev` value for the MySo dependency.
...
The test starts with a helper function for creating an object. The object type does not matter, as long as it has the `store` ability. The test uses `Coin<MYSO>`, because it comes with a `#[test_only]` function for minting:
...
- The `test_scenario` module is used to provide access to a `&mut TxContext` in the test (necessary for creating new objects). Tests that do not need to simulate multiple transactions but still need access to a `TxContext` can use `myso::tx_context::dummy` to create a test context instead.
...
From your `escrow` folder, run `myso move test` in your terminal or console. If successful, you get a response similar to the following that confirms the package builds and your tests pass:
...
- [Full source code](https://github.com/the-social-proof-foundation/myso-core/blob/705ee1ed3ce8cfadc4597c6facb6769d7dfb5896/examples/trading/contracts/escrow/sources/shared.move)
...
`Locked` and `Key` objects are typically owned by the transaction sender, and so can be queried through the MySo RPC, but `Escrow` objects are shared, and it is useful to be able to query them by their sender and recipient (so that users can see the trades they have offered and received).
...
You now have `shared.move` and `locked.move` files in your `sources` folder. From the parent `escrow` folder, run `myso move test` in your terminal or console. If successful, you get a response similar to the following that confirms the package builds and your tests pass:
...
- [Full source code](https://github.com/the-social-proof-foundation/myso-core/tree/705ee1ed3ce8cfadc4597c6facb6769d7dfb5896/examples/trading/api)
- Reference: [JSON-RPC](https://docs.mysocial.network/myso-api-ref)
...
<ImportContent source="initialize-myso-client-cli.mdx" mode="snippet" />

Next, configure the MySo CLI to use `testnet` as the active environment.
...
```sh
$ myso client new-env --alias testnet --rpc https://fullnode.testnet.mysocial.network:443
```
...
Before being able to publish your package to Testnet, you need Testnet MYSO tokens. To get some, run the following command:

```sh
$ myso client faucet
```

For other ways to get MYSO in your Testnet account, see [Get MYSO Tokens](/guides/developer/getting-started/get-coins).

Now that you have an account with some Testnet MYSO, you can deploy your contracts.

There are some helper functions to publish the smart contracts so you can create some demo data (for Testnet). The helper function to publish the smart contracts expects built smart contracts in both the `escrow` and `demo` directories. Run `myso move build` in both directories, if necessary. Be sure to update the MySo dependency in the manifest to point to the correct source based on your environment.
...
- [MySo TypeScript SDK](https://sdk.mystenlabs.com/typescript). For basic usage on how to interact with MySo with TypeScript.
- [MySo dApp Kit](https://sdk.mystenlabs.com/dapp-kit). To learn basic building blocks for developing an app in the MySo ecosystem with React.js.
- [`@mysten/dapp`](https://sdk.mystenlabs.com/dapp-kit/create-dapp). This is used within this project to quickly scaffold a React-based MySo app.
...
`MySoObjectDisplay.tsx`

</summary>

<ImportContent source="examples/trading/frontend/src/components/MySoObjectDisplay.tsx" mode="code" />
...
The dApp Kit provides a set of hooks for making query and mutation calls to the MySo blockchain. These hooks are thin wrappers around query and mutation hooks from `@tanstack/react-query`.
...
export function LockOwnedObjects() {
	...
	return (
		...
		>
			{data?.map((obj) => (
				<MySoObjectDisplay key={obj.objectId} object={obj}></MySoObjectDisplay>
			))}
		</InfiniteScrollArea>
	);
}
...
Fetch the owned objects directly from the MySo blockchain using `useInfiniteQuery` from TanStack Query with the `useCurrentClient()` hook from dApp Kit. The `useCurrentClient()` hook returns the configured MySo client, and you use its `core.listOwnedObjects()` method to fetch paginated owned objects. Supply the connected wallet account as the `owner`. The returned data is stored inside the cache at query key `getOwnedObjects`. In a future step you invalidate this cache after a mutation succeeds, so the data is re-fetched automatically.
...
Use the `useCurrentClient()` hook from dApp Kit to retrieve the MySo client instance configured in `src/main.tsx`. The `useSignTransaction()` function is another hook from dApp kit that helps to sign the transaction using the currently connected wallet. It displays the UI for users to review and sign their transactions with their selected wallet. To execute a transaction, use the `executeTransaction()` on the client instance of the MySo TypeScript SDK.

### Generate demo data

:::info

The full source code of the demo bear smart contract is available at [Trading Contracts Demo directory](https://github.com/the-social-proof-foundation/myso-core/tree/main/examples/trading/contracts/demo)
...
As previously mentioned, this example uses `@tanstack/react-query` to query, cache, and mutate server state. Server state is data only available on remote servers, and the only way to retrieve or update this data is by interacting with these remote servers. In this case, it could be from an API or directly from MySo blockchain RPC.

When you execute a transaction call to mutate data on the MySo blockchain, use the `useMutation()` hook. The `useMutation()` hook accepts several inputs. However, you only need 2 of them for this example. The first parameter, `mutationFn`, accepts the function to execute the main mutating logic, while the second parameter, `onSuccess`, is a callback that runs when the mutating logic succeeds.
...
import { MySoObjectData } from '@socialproof/myso/jsonRpc';
...
export function Locked({
	...
	return (
		<MySoObjectDisplay object={suiObject.data!} label={getLabel()} labelClasses={getLabelClasses()}>
...
export function useCreateEscrowMutation() {
	...
	return useMutation({
		mutationFn: async ({ object, locked }: { object: MySoObjectData; locked: ApiLockedObject }) => {
