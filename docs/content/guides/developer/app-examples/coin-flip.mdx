---
title: Coin Flip
description: Learn MySo through a coin flip app that covers the full end-to-end flow of building a MySo Move module and connecting it to a React MySo app.
...
- [Frontend repository](https://github.com/myso-foundation/satoshi-coin-flip-frontend-example)
...
- **One-time witnesses:** The guide teaches you how to use [one-time witnesses](concepts/myso-move-concepts.mdx) to ensure only a single instance of the `HouseData` object ever exists.
...
1. Seven lines begin with the `use` keyword, which enables this module to use types and functions declared in other modules (in this case, they are all coming from the MySo standard library).
...
```move title='house_data.move'
  public fun initialize_house_data(house_cap: HouseCap, coin: Coin<MYSO>, public_key: vector<u8>, ctx: &mut TxContext) {
    ...
    let house_data = HouseData {
      ...
      house: ctx.sender(),
      public_key,
      max_stake: 50_000_000_000, // 50 MYSO, 1 MYSO = 10^9.
      min_stake: 1_000_000_000, // 1 MYSO.
      fees: balance::zero(),
...
```move title='house_data.move'
  public fun top_up(house_data: &mut HouseData, coin: Coin<MYSO>, _: &mut TxContext) {
...
- `top_up`: Add to the balance of the house to ensure that there is enough MYSO for future games.
...
```move title='house_data.move'
  ...
  public(package) fun borrow_balance_mut(house_data: &mut HouseData): &mut Balance<MYSO> {
    &mut house_data.balance
  }

  public(package) fun borrow_fees_mut(house_data: &mut HouseData): &mut Balance<MYSO> {
    &mut house_data.fees
  }
...
module satoshi_flip::single_player_satoshi {
  ...
  use myso::myso::MYSO;
  use myso::bls12381::bls12381_min_pk_verify;
  use myso::event::emit;
  use myso::hash::{blake2b256};
  use myso::dynamic_object_field::{Self as dof};
...
```move title='single_player_satoshi.move'
  ...
  fun internal_start_game(guess: String, counter: &mut Counter, coin: Coin<MYSO>, house_data: &mut HouseData, fee_bp: u16, ctx: &mut TxContext): (ID, Game) {
...
Next, configure the MySo CLI to use `testnet` as the active environment, as well. If you haven't already set up a `testnet` environment, do so by running the following command in a terminal or console:
...
To skip building the frontend and test out your newly deployed package, use the provided [Satoshi Coin Flip Frontend Example repository](https://github.com/myso-foundation/satoshi-coin-flip-frontend-example) and follow the instructions in the example's `README.md` file
...
- [MySo TypeScript SDK](https://sdk.mystenlabs.com/typescript) for basic usage on how to interact with MySo using TypeScript.
...
- [`@mysten/dapp`](https://sdk.mystenlabs.com/dapp-kit/create-dapp), used within this project to quickly scaffold a React-based MySo app.
...
The following frontend code snippets include only the most relevant sections. Refer to the [Satoshi Coin Flip Frontend Example repository](https://github.com/myso-foundation/satoshi-coin-flip-frontend-example) for complete source code.
...
<form
  onSubmit={(e) => {
    ...
    const [houseStakeCoin] = txb.splitCoins(txb.gas, [
      MIST_PER_MYSO * BigInt(houseStake),
    ]);
    ...
    execInitializeHouse(
      ...
      },
      {
        ...
        onSuccess: (result: MySoTransactionBlockResponse) => {
          let houseDataObjId;
...
To use a [programmable transaction block](/guides/developer/transactions/prog-txn-blocks.mdx) (PTB) inMySo, create a `Transaction`. To initiate a Move call, you must know the global identifier of a public function in your smart contract. The global identifier usually takes the following form:
...
// Player stake
const [stakeCoin] = txb.splitCoins(txb.gas, [MIST_PER_MYSO * BigInt(stake)]);
...
export function HouseFinishGame() {
  ...
  useEffect(() => {
    const gqlClient = new MySoGraphQLClient({
      url: 'https://graphql.mainnet.mysocial.network/graphql',
      network: 'mainnet',
    });
...
The component uses `MySoGraphQLClient` to poll for `NewGame` events every 5 seconds, tracking the cursor to only process new events. When a new game is detected, it signs the VRF input with the BLS private key and calls `single_player_satoshi::finish_game()` to settle the game.

The cleanup function sets `cancelled = true` to stop polling when the component unmounts.

Congratulations, you completed the frontend. You can carry the lessons learned here forward when using the dApp Kit to build your next MySo project.
