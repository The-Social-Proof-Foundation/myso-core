...
This example walks you through building a basic distributed counter app, covering the full end-to-end flow of building your MySo Move module and connecting it to your React MySo app. The app allows users to create counters that anyone can increment, but only the owner can reset. 
...
[Example source code](https://github.com/the-social-proof-foundation/myso-core/blob/60bb8bdc274b9e5706fd916cd84c13f81e832529/sdk/create-dapp/templates/react-e2e-counter)
...
- Your active address has MYSO. Run `myso client balance` in your terminal or console. If there is no balance, [acquire MYSO](../getting-started/get-coins.mdx) from the faucet (not available in Mainnet).
...
Your smart contract is complete. You should be able to run the `myso move build` command from `react-e2e-counter/move/counter` and receive a response similar to the following:
```sh
...
BUILDING counter
```
You always run `myso move build` at the same level as your `Move.toml` file. After a successful build, you now have a `build` folder inside `react-e2e-counter/move/counter`.
...
The `create` function now creates a new MySo `Transaction` and calls the `create` function from your Move module. The PTB is then signed and executed via the `useSignAndExecuteTransaction` hook. The `onCreated` callback is called with the new counter's ID when the transaction is successful.
...
```tsx
  ...
  import { isValidSuiObjectId } from "@socialproof/myso/utils";
...
This snippet shows the key concepts. It uses the `useCurrentClient` hook to get the MySo client, combined with TanStack Query's `useQuery` hook to fetch the object. The `useDAppKit` hook provides access to transaction signing.
