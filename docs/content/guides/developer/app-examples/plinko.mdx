---
title: Plinko
description: Learn MySo through an example implementation of the popular casino game, Plinko.
...
The Plinko game, implemented through smart contracts on the MySo blockchain, leveragesMySo's native on-chain randomness to ensure fairness and transparency. The game calculates the trace path for each ball using secure random values generated directly on-chain, determining the game's outcome based on the number of Plinko balls a player chooses to drop.

The game mechanics involve a player starting a game by specifying the number of balls and staking a certain amount. The on-chain randomness is provided throughMySo's Move randomness framework, which ensures each game's randomness is cryptographically secure and cannot be predicted or manipulated. The number of Plinko balls, chosen by the player, directly influences the game's complexity and potential payout, as each ball's final position is determined by the on-chain random values that dictate the ball's path through the pegged board.
...
sequenceDiagram
    ...
    participant Blockchain as MySo Blockchain
...
The `plinko::plinko` module combines various MySo blockchain features, such as coin handling, event emissions, on-chain randomness, and game logic, to create fair and transparent on-chain Plinko games.
...
public fun start_game(coin: Coin<MYSO>, house_data: &mut HouseData, ctx: &mut TxContext): ID {
...
entry fun finish_game(
    ...
    let stake_per_ball = stake.value<MYSO>() / num_balls;
    ...
    while (ball_index < num_balls) {
        ...
        // Divide by 100 to adjust for multiplier scale and MYSO units
    ...
    let payout_coin: Coin<MYSO> = coin::take(payout_balance_mut, total_funds_amount, ctx);
...
- `finish_game`: Completes the game using on-chain randomness to calculate outcomes, traces the path each ball travels, and distributes the total winnings to the player. This is an entry function that takes the Random object fromMySo's randomness framework.
...
public fun initialize_house_data(
    ...
    let mut house_data = HouseData {
        ...
        house: ctx.sender(),
        max_stake: 10_000_000_000, // 10 MYSO = 10^9.
        min_stake: 100_000_000, // 0.1 MYSO.
        fees: balance::zero(),
...
The Plinko game demonstrates a frictionless user experience by leveraging [Enoki](https://docs.enoki.mystenlabs.com/) for transaction sponsorship. This allows players to interact with the game without needing to hold MYSO tokens for gas fees, significantly reducing barriers to entry.
...
export const POST = async (req: NextRequest) => {
  try {
    ...
    const sponsored = await enokiClient.createSponsoredTransaction({
      network: process.env.NEXT_PUBLIC_MYSO_NETWORK_NAME as
        | "mainnet"
        | "testnet"
        | "devnet",
...
The Plinko frontend is built with React and integrates with the MySo blockchain using sponsored transactions through Enoki. The application provides an interactive and responsive gaming experience while abstracting away blockchain complexity from end users.
...
- **Sponsored transactions:** The frontend leverages Enoki's `/sponsor` and `/execute` APIs to create a gasless experience. Players can interact with the game without needing MYSO tokens, as the backend sponsors all transaction fees.
