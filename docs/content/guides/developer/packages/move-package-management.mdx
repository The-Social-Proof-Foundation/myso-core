...
The Move package system supports 4 types of dependencies, each suited for different use cases:
 ...
 - System dependencies are used for built-in packages that are part of MySo.
...
```toml
[dependencies]
usdc = { git = "https://github.com/circlefin/stablecoin-myso.git", subdir = "packages/usdc", rev = "master" }
```
...
```toml
[dependencies.usdc]
git = "https://github.com/circlefin/stablecoin-myso.git"
...
Several packages are built into MySo. The `system` dependency type can be used to depend on these packages. The available system packages are `std`, `myso`, `myso_system`, `bridge`, and `orderbook`.

However:

 - The `std` and `myso` packages are implicitly included unless you write `implicit-dependencies = false` in your `[package]` section. You do not need to explicitly include them.
 - The `orderbook` system package is for the deprecated Orderbook version 2. New applications should use Orderbook version 3 by adding `orderbook = { mvr = "@orderbook/core" }`.

To include a system dependency, write `{ system = "<name>" }`. For example, to use `myso_system`, add `myso_system = { system = "myso_system" }` to your `[dependencies]` section.
...
This dependency is included whenever you run `myso move test` or pass `-m test` to any `myso move` command.
...
The right-hand side of the entry is the chain identifier for the network. You can find the chain ID using `myso client chain-identifier`. The chain identifier is used to ensure dependencies agree on the meaning of the environment name, and to reset package addresses if the network is wiped and restarted.
...
```toml
[dependencies]
codec = { git = "https://github.com/myso-potatoes/app.git", subdir = "packages/codec", rev = "codec@testnet-v2" }

[dep-replacements.mainnet]
codec = { git = "https://github.com/myso-potatoes/app.git", subdir = "packages/codec", rev = "codec@mainnet-v2" }
```
...
Whenever you run `myso move build` or `myso move test`, the system ensures that you have all dependencies cached in your `~/.move` directory. It first checks whether the lockfile is current, and if it is not, it repins all of the dependencies for your build environment. It then checks whether the packages are cached, and if not it downloads them.
...
Run `myso move update-deps` to repin all of your dependencies. This does not change your manifest file, it only redownloads the latest versions that are compatible with what you have written.

### Publishing and upgrading

During normal package publication and upgrades (`myso client publish` or `myso client upgrade`), the system rechecks that the dependencies are published on the given network and that the relevant chain IDs are consistent. The system also ensures that the additional on-chain linkage requirements are satisfied. For example, on-chain packages can have extra dependencies in their linkage that they do not actually use, so the system needs to include the on-chain linkages.
...
build-config = { flavor = "myso", edition = "2024" }
...
You can also publish or upgrade using the `myso client ptb` command. See [Building Programmable Transaction Blocks](/guides/developer/transactions/building-ptb.mdx) for details.
...
Instead, the package manager supports ephemeral publication of a package and its dependencies for testing. The `myso client test-publish` command works like a normal publish, except that the system reads the publication addresses for packages and writes them to a separate file.

The command `myso client test-publish --pubfile-path <pubfile> --build-env <env>` builds the root package for the `<env>` environment (see [the build environment](#the-build-environment)) and then publishes it to a chain using the addresses from the ephemeral publication file `<pubfile>`.
...
| **Created by** | `myso client publish` | `myso client test-publish` |
...
```sh
myso client env switch localnet
myso client test-publish --build-env testnet
```
...
```sh
cd packages/a
myso client test-publish --build-env testnet --pubfile-path ../Pub.localnet.toml
...
```sh
cd packages/b
myso client test-publish --build-env testnet --publish-unpublished-deps`
```
