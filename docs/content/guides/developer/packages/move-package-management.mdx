...
Several packages are built into MySo. The `system` dependency type can be used to depend on these packages. The available system packages are `std`, `myso`, `myso_system`, `bridge`, and `deepbook`.
