...
Using the Closed-Loop Token standard, you can limit the applications that can use the token and set up custom policies for transfers, spends, and conversions. The [`myso::token` module](https://github.com/the-social-proof-foundation/myso-core/blob/main/crates/myso-framework/docs/myso/token.md) in the MySo framework defines the standard.

## Background and use cases

The [Currency Standard](/standards/currency.mdx) on MySo is an example of an open-loop system. Coins are free-flowing, [wrappable](/guides/developer/objects/object-ownership/wrapped.mdx), [freely transferable](/guides/developer/objects/transfers/custom-rules.mdx) and you can store them in any application. 
