---
title: Margin Risks
description: Understand the risks of margin trading on Orderbook, including liquidation, interest rate fluctuations, and how to protect your positions.
keywords:
  [
    margin risks,
    liquidation,
...
Margin trading amplifies both gains and losses. Before using Orderbook Margin, it's critical to understand the risks involved. This guide explains the key risks and provides concrete examples to help you make informed decisions.
...
### Example: Getting liquidated on MYSO/USDC

Let's walk through a concrete example using the MYSO/USDC trading pair, which has a **Liquidation Risk Ratio of 1.1**.

**Opening position:**
- You deposit **100 USDC** as collateral
- You borrow **400 USDC** and open a 5x long position on MYSO at 1.50 USDC per MYSO
...
| MYSO Price (USDC) | MYSO Value (USDC) | Total Assets (USDC) | Risk Ratio | Status |
|------------------|------------------|---------------------|------------|--------|
...
**What happens at 1.275 USDC per MYSO:**
- Your risk ratio hits 1.1 (the liquidation threshold for MYSO/USDC)
...
- **Long positions**: Price moves down (you borrowed USDC to buy MYSO, and MYSO drops)
- **Short positions**: Price moves up (you borrowed MYSO to sell for USDC, and MYSO rises)
...
Orderbook Margin uses a [kinked interest rate model](./contract-information/interest-rates.mdx) where rates increase gradually up to an optimal utilization point, then spike dramatically.
...
Orderbook Margin uses Pyth price oracles to value your assets and debts. While the protocol includes several protections, some oracle-related risks remain:
...
- **Price manipulation:** Although Pyth is designed to be manipulation-resistant and Orderbook validates prices against confidence intervals and EWMA (exponentially weighted moving average) prices, extreme market conditions could still affect price accuracy.

### Oracle protections

Orderbook Margin implements multiple safeguards against oracle issues:
...
Orderbook Margin supports [TPSL orders](./contract-information/tpsl.mdx) that automatically close your position:
