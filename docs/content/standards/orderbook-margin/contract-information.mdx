...
sidebar_position: 2
description: Orderbook Margin contract addresses, supported coins, margin pools, and risk parameters on MySo Mainnet.
keywords:
  [
    orderbook margin,
    contract address,
...
This page contains the contract addresses, supported coins, margin pools, and risk parameters for Orderbook Margin on MySo Mainnet.

## Contract versions

Orderbook Margin uses upgradeable contracts. When a contract is upgraded, only `MARGIN_PACKAGE_ID` needs to be updated - previous versions remain compatible unless noted.
...
<RelatedLink
	...
	label="Orderbook Margin repository"
	desc="The Orderbook Margin package in the OrderbookV3 repository on GitHub."
/>
