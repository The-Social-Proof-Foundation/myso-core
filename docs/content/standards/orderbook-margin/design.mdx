---
title: Design
description: Learn about Orderbook Margin design, including MarginPool, MarginManager, and MarginRegistry shared objects.
keywords:
  [
    orderbook margin design,
    margin pool,
    margin manager,
    margin registry,
    leveraged trading,
...
At a high level, the Orderbook Margin design revolves around four main shared objects that work together to enable leveraged trading:
...
- `BalanceManager`: Inherited from OrderbookV3, used to source funds for trading. See [BalanceManager](../orderbookv3/contract-information/balance-manager.mdx) to learn more.
...
The `MarginManager` wraps a `BalanceManager` and adds margin trading capabilities. Each `MarginManager` is associated with a specific Orderbook pool and tracks:
...
- **Orderbook pool:** Which trading pool this manager is authorized to trade on.
...
- **Pool enablement:** Determines which Orderbook pools are enabled for margin trading.
...
The registry enforces that only one margin pool can exist per asset type and ensures margin managers can only trade on enabled Orderbook pools.
...
Interest accrues continuously in Orderbook Margin based on the utilization rate:
