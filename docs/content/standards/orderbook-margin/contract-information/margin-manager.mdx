...
description: Learn about the margin manager and how it enables leveraged trading on Orderbook.
keywords:
  [
    margin manager,
    marginmanager,
...
Each `MarginManager` is associated with a specific Orderbook pool and can borrow from margin pools that allow trading on that pool. The margin manager tracks borrowed positions and enforces risk ratio limits to maintain system solvency.
...
<ImportContent
	source="packages/orderbook_margin/sources/margin_manager.move"
	mode="code"
	org="MystenLabs"
	repo="orderbookv3"
	fun="new"
...
The owner of a `MarginManager` can set or unset a pool-specific referral for trading fee benefits. The referral must be a `OrderbookPoolReferral` minted for the pool associated with the margin manager. Each margin manager can have different referrals for different pools.
...
Borrow base or quote assets from margin pools to increase position sizes. Borrowing is subject to risk ratio limits and the margin pool must allow trading on the manager's Orderbook pool.
...
<ImportContent
	...
	org="MystenLabs"
	repo="orderbookv3"
	fun="balance_manager,calculate_assets,calculate_debts,owner,orderbook_pool,margin_pool_id,borrowed_shares,borrowed_base_shares,borrowed_quote_shares,has_base_debt"
/>
