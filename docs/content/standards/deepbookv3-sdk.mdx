...
To work with DeepBookV3, use the client extension to add DeepBook functionality to a MySo client. The [MySo TypeScript SDK](https://sdk.mystenlabs.com/typescript) provides the `MySoGrpcClient` and key functionality necessary to process transactions. The following example imports those libraries, as well.
...
(async () => {
	...
	// Read only call
	console.log(await mmClient.client.deepbook.checkManagerBalance('MANAGER_1', 'MYSO'));
	console.log(await mmClient.client.deepbook.getLevel2Range('MYSO_DBUSDC', 0.1, 100, true));
...
client.deepbook.mintReferral('MYSO_DBUSDC', 0.1)(tx);

// Update the multiplier for an existing referral
client.deepbook.updatePoolReferralMultiplier('MYSO_DBUSDC', referralId, 0.2)(tx);

// Claim accumulated referral rewards (returns base, quote, and DEEP coins)
const { baseRewards, quoteRewards, deepRewards } = tx.add(
	client.deepbook.claimPoolReferralRewards('MYSO_DBUSDC', referralId)
);

// Get the current balances for a referral
client.deepbook.getPoolReferralBalances('MYSO_DBUSDC', referralId)(tx);

// Get the multiplier for a referral
client.deepbook.poolReferralMultiplier('MYSO_DBUSDC', referralId)(tx);
```
...
client.deepbook.balanceManager.unsetBalanceManagerReferral('MANAGER_1', 'MYSO_DBUSDC', tradeCap)(tx);

// Get the referral ID associated with a balance manager for a specific pool
client.deepbook.balanceManager.getBalanceManagerReferralId('MANAGER_1', 'MYSO_DBUSDC')(tx);
