---
title: Swaps
description: Learn how to use the OrderbookV3 SDK to interact a swap-like interface.
keywords: [ automatic market makers, amms, swap_exact_amount, balancemanager, base_in, quote_in, deep_in, swaps, swap base quantity, deep quantity, DEEP, swap exact quantity, orderbook, orderbookv3  ]
---

OrderbookV3 provides a swap-like interface commonly seen in automatic market makers (AMMs). The OrderbookV3 SDK provides functions to leverage the features of this interface. See [Swaps](../orderbookv3/contract-information/swaps.mdx) in the API section for more details.
...
<ImportContent source="packages/orderbook-v3/src/transactions/orderbook.ts" mode="code" org="MystenLabs" repo="ts-sdks" fun="swapExactQuantity" signatureOnly/>
...
<ImportContent source="packages/orderbook-v3/src/transactions/orderbook.ts" mode="code" org="MystenLabs" repo="ts-sdks" fun="swapExactQuantityWithManager" signatureOnly/>

### Examples

The following examples demonstrate custom swap functions that you can place into the `OrderbookMarketMaker` class. Base coin, quote coin, and deep coin are automatically determined by the coin available in the user address unless you explicitly pass one in as an argument. You can transfer the coin outputs to their address or execute other operations using the outputs.

```tsx
swapExactBaseForQuote = (tx: Transaction) => {
  const [baseOut, quoteOut, deepOut] = this.orderbook.swapExactBaseForQuote({
    poolKey: 'MYSO_DBUSDC',
    amount: 1, // amount of MYSO to swap
...
swapExactQuoteForBase = (tx: Transaction) => {
  const [baseOut, quoteOut, deepOut] = this.orderbook.swapExactQuoteForBase({
    poolKey: 'MYSO_DBUSDC',
    amount: 1, // amount of DBUSDC to swap
    deepAmount: 1, // amount of DEEP to pay as fees, excess is returned
    minOut: 0.1, // minimum amount of MYSO to receive or transaction fails
  })(tx);
