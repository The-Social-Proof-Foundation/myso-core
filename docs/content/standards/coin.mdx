---
title: Coin Standard
description: The Coin standard enables you to create a broad range of fungible tokens on the MySo network to satisfy a number of use cases. 
sidebar_label: Coin
keywords: [ myso coin standard, erc-20, erc-20 equivalent, creating coins, coin standard, fungible tokens, fungible token standard, treasury capability, regulated coins, denylist object, coin metadata, create coin with SDK, use SDK to create coin, mint coin, minting coins ]
...
Coin creates tokens using `coin::create_currency` while Currency uses `myso::coin_registry`.

Coin uses `CoinMetadata` while Currency uses `Currency`. 

Fungible tokens created on MySo using the Coin or Currency standard are referred to as _coins_. 
...
The `TreasuryCap` object is required to mint new coins or to burn current ones. Consequently, only addresses that have access to this object can maintain the coin supply on the MySo network.
...
The `coin` module provides the logic for creating and destroying coins on the MySo network (as long as you own the associated `TreasuryCap`). These functions are the same for all coins and each requires the `TreasuryCap` as an input.

### Mint

Use the `coin::mint` function to create new tokens.

<ImportContent source="crates/myso-framework/packages/myso-framework/sources/coin.move" mode="code" fun="mint" noComments />
...
<ImportContent source="crates/myso-framework/packages/myso-framework/sources/coin.move" mode="code" fun="burn" noComments />
...
<RelatedLink href="https://github.com/the-social-proof-foundation/myso-core/blob/main/crates/myso-framework/docs/myso/coin.md" label="`coin` module rustdoc documentation" desc="Automated documentation output for the MySo framework `coin` module." />
<RelatedLink href="https://github.com/the-social-proof-foundation/myso-core/blob/main/crates/myso-framework/docs/myso/token.md" label="`token` module rustdoc documentation" desc="Automated documentation output for the MySo framework `token` module." />
