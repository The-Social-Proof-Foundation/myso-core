...
description: Learn how to use the Orderbook Margin SDK to manage margin pools and system configuration.
keywords:
  [
    maintainer,
    margin pool admin,
...
The Maintainer SDK provides administrative functions for managing margin pools, configuring interest rates, and controlling which Orderbook pools can access margin lending. These functions are restricted to maintainers with the appropriate capabilities.

## Maintainer functions

The Orderbook Margin SDK provides the following functions for pool administration and configuration.
...
<ImportContent source="/packages/orderbook-v3/src/transactions/marginMaintainer.ts" mode="code" fun="createMarginPool" org="MystenLabs" repo="ts-sdks" signatureOnly />
...
<ImportContent source="/packages/orderbook-v3/src/transactions/marginMaintainer.ts" mode="code" fun="newProtocolConfig" org="MystenLabs" repo="ts-sdks" signatureOnly />
...
<ImportContent source="/packages/orderbook-v3/src/transactions/marginMaintainer.ts" mode="code" fun="newMarginPoolConfig" org="MystenLabs" repo="ts-sdks" signatureOnly />
...
<ImportContent source="/packages/orderbook-v3/src/transactions/marginMaintainer.ts" mode="code" fun="newInterestConfig" org="MystenLabs" repo="ts-sdks" signatureOnly />

### `enableOrderbookPoolForLoan`, `disableOrderbookPoolForLoan`

Use these functions to control which Orderbook pools can borrow from the margin pool. The call returns a function that takes a `Transaction` object.

**Parameters**

- `orderbookPoolKey`: String that identifies the Orderbook pool.
- `coinKey`: String that identifies the margin pool asset.
- `marginPoolCap`: String representing the margin pool capability ID.

<ImportContent source="/packages/orderbook-v3/src/transactions/marginMaintainer.ts" mode="code" fun="enableOrderbookPoolForLoan,disableOrderbookPoolForLoan" org="MystenLabs" repo="ts-sdks" signatureOnly />
...
<ImportContent source="/packages/orderbook-v3/src/transactions/marginMaintainer.ts" mode="code" fun="updateInterestParams" org="MystenLabs" repo="ts-sdks" signatureOnly />
...
<ImportContent source="packages/orderbook-v3/src/transactions/marginMaintainer.ts" mode="code" fun="updateMarginPoolConfig" org="MystenLabs" repo="ts-sdks" signatureOnly />
...
<ImportContent source="packages/orderbook-v3/src/transactions/marginMaintainer.ts" mode="code" fun="withdrawMaintainerFees" org="MystenLabs" repo="ts-sdks" signatureOnly />
...
<ImportContent source="packages/orderbook-v3/src/transactions/marginMaintainer.ts" mode="code" fun="withdrawProtocolFees" org="MystenLabs" repo="ts-sdks" signatureOnly />
...
<ImportContent source="packages/orderbook-v3/src/transactions/marginMaintainer.ts" mode="code" fun="adminWithdrawDefaultReferralFees" org="MystenLabs" repo="ts-sdks" signatureOnly />
...
### Enable a Orderbook pool for borrowing

```tsx
// Example: Allow MYSO/USDC pool to borrow from USDC margin pool
enablePoolForBorrowing = (tx: Transaction) => {
	const orderbookPoolKey = 'MYSO_DBUSDC';
	const coinKey = 'USDC';
	const marginPoolCapId = '0x...'; // Margin pool cap ID

	tx.add(
		this.maintainerContract.enableOrderbookPoolForLoan(orderbookPoolKey, coinKey, marginPoolCapId),
	);
};
```
...
setupNewMarginPool = (tx: Transaction) => {
	...
	// Step 3: Enable specific Orderbook pools for borrowing
	const marginPoolCapId = '0x...'; // Get from pool creation event
	tx.add(this.maintainerContract.enableOrderbookPoolForLoan('MYSO_DBUSDC', coinKey, marginPoolCapId));
	tx.add(this.maintainerContract.enableOrderbookPoolForLoan('MYSO_USDT', coinKey, marginPoolCapId));
};
```
