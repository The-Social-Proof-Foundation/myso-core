...
enablePoolForBorrowing = (tx: Transaction) => {
	const deepbookPoolKey = 'MYSO_DBUSDC';
	const coinKey = 'USDC';
...
setupNewMarginPool = (tx: Transaction) => {
	...
	tx.add(this.maintainerContract.enableDeepbookPoolForLoan('MYSO_DBUSDC', coinKey, marginPoolCapId));
	tx.add(this.maintainerContract.enableDeepbookPoolForLoan('MYSO_USDT', coinKey, marginPoolCapId));
};
```
