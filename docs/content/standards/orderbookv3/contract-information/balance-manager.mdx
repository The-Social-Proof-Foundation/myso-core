...
With exception to swaps, all interactions with OrderbookV3 require a `BalanceManager` as one of its inputs. When orders are matched, funds are transferred to or from the `BalanceManager`. You can use a single `BalanceManager` between all pools.
...
The `new_with_custom_owner_caps<App>()` function creates a `BalanceManager` with a custom owner and returns all three capabilities (`DepositCap`, `WithdrawCap`, and `TradeCap`) in a single call. This function requires authorization through the Orderbook Registry with a specific `App` type. Combine the balance manager with `share`, or else the transaction fails. This is a convenient way to set up a complete balance manager with all necessary capabilities in one transaction.

:::caution
Move code using OrderbookV3 uses `DepositCap`, `WithdrawCap`, and `TradeCap`, while the OrderbookV3 SDK uses `depositCap`, `withdrawCap`, and `tradeCap`.
:::
...
The owner of a `TradeCap` can set or unset a pool-specific referral for the balance manager. Setting a referral allows the balance manager to be associated with a `OrderbookPoolReferral` for that pool, which can track and earn referral fees. Each balance manager can have different referrals for different pools.
