...
description: OrderbookV3 contract addresses, supported coins, and pool information on MySo Mainnet.
keywords:
  [
    orderbook,
    orderbookv3,
...
This page contains the contract addresses, supported coins, and pool information for OrderbookV3 on MySo Mainnet.

## Contract versions

OrderbookV3 uses upgradeable contracts. When a contract is upgraded, only `ORDERBOOK_PACKAGE_ID` needs to be updated - previous versions remain compatible unless noted. A redeployment would require updating `ORDERBOOK_PACKAGE_ID`, `REGISTRY_ID`, and all pool IDs.
