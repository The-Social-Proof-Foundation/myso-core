---
title: Orderbook Margin Indexer
sidebar_label: Indexer
description: Orderbook Margin Indexer provides access to margin trading events including loans, liquidations, and margin pool operations.
keywords: [Orderbook Margin Indexer, margin manager, margin pool, liquidation, loan, borrow, repay, supply, withdraw, collateral]
---

The Orderbook Margin Indexer extends the OrderbookV3 Indexer with endpoints for margin trading data. These endpoints provide access to margin manager events, margin pool operations, and liquidation monitoring.

## Public endpoints

The margin endpoints are available through the same public indexer as OrderbookV3.

- **Mainnet**: `https://orderbook-indexer.mainnet.mystenlabs.com/`

- **Testnet**: `https://orderbook-indexer.testnet.mystenlabs.com/`
...
```http
/margin_manager_states?max_risk_ratio=<FLOAT>&orderbook_pool_id=<ID>
```
...
- `orderbook_pool_id`: Filter by specific Orderbook pool
...
<details>
<summary>Get Orderbook pool updated events</summary>

```http
/orderbook_pool_updated?margin_pool_id=<ID>&orderbook_pool_id=<ID>
```

Returns events for when Orderbook pools are enabled or disabled for a margin pool.
...
<details>
<summary>Get Orderbook pool registered events</summary>

```http
/orderbook_pool_registered?pool_id=<ID>
```

Returns events for when Orderbook pools are registered in the margin registry.
...
<details>
<summary>Get Orderbook pool registry updated events</summary>

```http
/orderbook_pool_updated_registry?pool_id=<ID>
```

Returns events for when Orderbook pool registry entries are updated.
...
<details>
<summary>Get Orderbook pool config updated events</summary>

```http
/orderbook_pool_config_updated?pool_id=<ID>
```

Returns events for when Orderbook pool configurations are updated.
