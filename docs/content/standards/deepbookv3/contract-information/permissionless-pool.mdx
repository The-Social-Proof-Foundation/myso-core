...
The `Pool` shared object represents a market, such as a MYSO/USDC market. That `Pool` is the only one representing that unique pairing (MYSO/USDC) and the pairing is the only member of that particular `Pool`. See [DeepBookV3 Design](../design.mdx#pool) to learn more about the structure of pools.
