...
Browser extension wallets built for MySo use the [Wallet Standard](https://github.com/wallet-standard/wallet-standard/). This is a cross-chain standard that defines how apps can automatically discover and interact with wallets.
...
```tsx
import { MYSO_DEVNET_CHAIN, Wallet } from '@mysten/wallet-standard';
...
class YourWallet implements Wallet {
	...
	// Return the MySo chains that your wallet supports.
	get chains() {
		return [MYSO_DEVNET_CHAIN];
	}
}
```
...
Current methods:
- `myso:signTransaction` - Use to prompt the user to sign a transaction and return the serialized
  transaction and signature back to the app. This method does not submit the transaction for
  execution.
- `myso:signAndExecuteTransaction` - Use to prompt the user to sign a transaction, then submit it
  for execution to the blockchain.

Legacy methods:
- `myso:signTransactionBlock` - The legacy version of `myso:signTransaction`. Many existing apps still rely on this method.
- `myso:signAndExecuteTransactionBlock` - The legacy version of `myso:signAndExecuteTransaction`. Many existing apps still rely on this method.

**Personal message signing**
- `myso:signPersonalMessage` - Use to prompt the user to sign a personal message and return the
		...
		return someWalletAccounts.map(
			(walletAccount) =>
				new ReadonlyWalletAccount({
					address: walletAccount.mysoAddress,
					publicKey: walletAccount.pubkey,
					// The MySo chains that this account supports. This can be a subset of the wallet's supported chains.
          // These chains must exist on the wallet as well.
					chains: [MYSO_DEVNET_CHAIN],
...
Construct the transaction separately with the `@mysten/myso` library and then sign it with the private key of the user. Use the `myso:signTransaction` feature to achieve this:

```tsx
wallet.features['myso:signTransaction'].signTransaction({
...
To execute the transaction, use a MySo client such as `MySoGrpcClient` from `@socialproof/myso/grpc`:
...
Many wallets abstract the above flow into one feature: `myso:signAndExecuteTransaction`. The required arguments for this feature are the raw transaction and the options with the desired information to be included in the response:
