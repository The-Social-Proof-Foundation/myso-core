---
title: Orderbook Margin SDK
description: Use the Orderbook TypeScript SDK to interact directly with the Orderbook Margin package.
keywords:
  [
    orderbook margin,
    margin trading,
    typescript,
    typescript SDK,
    mysten/orderbook,
    orderbook margin SDK,
    margin manager,
    margin pool,
  ]
---

The Orderbook Margin TypeScript SDK abstracts away the transaction calls, allowing for direct interactions with the `Orderbook Margin` package for leveraged trading.

- [SDK repository](https://github.com/MystenLabs/ts-sdks/tree/main/packages/orderbook-v3)
- [NPM version](https://www.npmjs.com/package/@mysten/orderbook-v3)

## Install

To use the SDK in your projects, install the `@mysten/orderbook-v3` package, which includes the margin trading functionality.

```sh npm2yarn
npm install @mysten/orderbook-v3
```

## Constants

The Orderbook SDK includes a constants file (`/utils/constants.ts`) that maintains the latest deployed addresses for Orderbook Margin, as well as margin pools and configurations.

<details>
<summary>
`constants.ts`
</summary>

```ts reference
https://github.com/MystenLabs/ts-sdks/blob/main/packages/orderbook-v3/src/utils/constants.ts
```

</details>

## OrderbookClient

To work with Orderbook Margin, use the client extension to add Orderbook functionality to a MySo client. The [MySo TypeScript SDK](https://sdk.mystenlabs.com/typescript) provides the `MySoGrpcClient` and key functionality necessary to process transactions. The following example imports those libraries, as well.

```tsx
import { orderbook, type OrderbookClient } from '@mysten/orderbook-v3';
import type { ClientWithExtensions } from '@socialproof/myso/client';
import { decodeMySoPrivateKey } from '@socialproof/myso/cryptography';
import { MySoGrpcClient } from '@socialproof/myso/grpc';
import { Ed25519Keypair } from '@socialproof/myso/keypairs/ed25519';

class OrderbookMarginTrader {
	client: ClientWithExtensions<{ orderbook: OrderbookClient }>;
	keypair: Ed25519Keypair;

	constructor(privateKey: string, env: 'testnet' | 'mainnet') {
		this.keypair = this.getSignerFromPK(privateKey);
		this.client = new MySoGrpcClient({
			network: env,
			baseUrl:
				env === 'mainnet'
					? 'https://fullnode.mainnet.mysocial.network:443'
					: 'https://fullnode.testnet.mysocial.network:443',
		}).$extend(
			orderbook({
				address: this.getActiveAddress(),
			}),
		);
	}

	getSignerFromPK = (privateKey: string): Ed25519Keypair => {
		const { scheme, secretKey } = decodeMySoPrivateKey(privateKey);
		if (scheme === 'ED25519') return Ed25519Keypair.fromSecretKey(secretKey);

		throw new Error(`Unsupported scheme: ${scheme}`);
	};

	getActiveAddress() {
		return this.keypair.toMySoAddress();
	}
}
```

## Keys: Coin, Pool, and MarginManager {#keys}

Functions that require the input of a coin, pool, or a margin manager require the key of any such object as the parameter. The SDK manages a `key:value` relationship of this data in memory. Some default data comes with the SDK (as seen in `utils/constants.ts`). Coins are stored in a `CoinMap`, pools in a `PoolMap`, and margin managers in a `MarginManagerMap` in the config.

### Margin manager

Before placing any margin trade, you must supply a margin manager address to the client. The manager key points to an object defined by the `MarginManager` interface in the client. [MarginManager docs](./orderbook-margin/contract-information/margin-manager.mdx). Initialize the margin manager with the client. If you don't create a margin manager, you can rely on the client to create one, but then the user must reinitialize the client.

Example using an existing margin manager:

```tsx
import { orderbook, type OrderbookClient } from '@mysten/orderbook-v3';
import type { MarginManager } from '@mysten/orderbook-v3';
import type { ClientWithExtensions } from '@socialproof/myso/client';
import { decodeMySoPrivateKey } from '@socialproof/myso/cryptography';
import { MySoGrpcClient } from '@socialproof/myso/grpc';
import { Ed25519Keypair } from '@socialproof/myso/keypairs/ed25519';
import { config } from 'dotenv';

config();

const MARGIN_MANAGER_KEY = 'MARGIN_MANAGER_1';

class OrderbookMarginTrader {
	client: ClientWithExtensions<{ orderbook: OrderbookClient }>;
	keypair: Ed25519Keypair;

	constructor(privateKey: string, env: 'testnet' | 'mainnet') {
		this.keypair = this.getSignerFromPK(privateKey);
		this.client = new MySoGrpcClient({
			network: env,
			baseUrl:
				env === 'mainnet'
					? 'https://fullnode.mainnet.mysocial.network:443'
					: 'https://fullnode.testnet.mysocial.network:443',
		}).$extend(
			orderbook({
				address: this.getActiveAddress(),
				marginManagers: this.getMarginManagers(),
			}),
		);
	}

	getSignerFromPK = (privateKey: string): Ed25519Keypair => {
		const { scheme, secretKey } = decodeMySoPrivateKey(privateKey);
		if (scheme === 'ED25519') return Ed25519Keypair.fromSecretKey(secretKey);

		throw new Error(`Unsupported scheme: ${scheme}`);
	};

	getActiveAddress() {
		return this.keypair.toMySoAddress();
	}

	getMarginManagers(): { [key: string]: MarginManager } {
		const marginManagerAddress = process.env.MARGIN_MANAGER_ADDRESS;
		const poolKey = process.env.POOL_KEY || 'MYSO_DBUSDC';
		if (!marginManagerAddress) {
			throw new Error('No margin manager address found');
		}
		return {
			[MARGIN_MANAGER_KEY]: {
				address: marginManagerAddress,
				poolKey: poolKey,
			},
		};
	}
}
```

Example creating a margin manager:

```tsx
import { orderbook, type OrderbookClient } from '@mysten/orderbook-v3';
import type { MarginManager } from '@mysten/orderbook-v3';
import type { ClientWithExtensions } from '@socialproof/myso/client';
import { decodeMySoPrivateKey } from '@socialproof/myso/cryptography';
import { MySoGrpcClient } from '@socialproof/myso/grpc';
import { Ed25519Keypair } from '@socialproof/myso/keypairs/ed25519';
import { Transaction } from '@socialproof/myso/transactions';

const MARGIN_MANAGER_KEY = 'MARGIN_MANAGER_1';

class OrderbookMarginTrader {
	client: ClientWithExtensions<{ orderbook: OrderbookClient }>;
	keypair: Ed25519Keypair;
	env: 'testnet' | 'mainnet';

	constructor(privateKey: string, env: 'testnet' | 'mainnet') {
		this.env = env;
		this.keypair = this.getSignerFromPK(privateKey);
		this.client = this.#createClient(env);
	}

	#createClient(env: 'testnet' | 'mainnet', marginManagers?: { [key: string]: MarginManager }) {
		return new MySoGrpcClient({
			network: env,
			baseUrl:
				env === 'mainnet'
					? 'https://fullnode.mainnet.mysocial.network:443'
					: 'https://fullnode.testnet.mysocial.network:443',
		}).$extend(
			orderbook({
				address: this.getActiveAddress(),
				marginManagers,
			}),
		);
	}

	getSignerFromPK = (privateKey: string): Ed25519Keypair => {
		const { scheme, secretKey } = decodeMySoPrivateKey(privateKey);
		if (scheme === 'ED25519') return Ed25519Keypair.fromSecretKey(secretKey);

		throw new Error(`Unsupported scheme: ${scheme}`);
	};

	getActiveAddress() {
		return this.keypair.toMySoAddress();
	}

	async createMarginManagerAndReinitialize() {
		let tx = new Transaction();
		const poolKey = 'MYSO_DBUSDC';
		tx.add(this.client.orderbook.marginManager.newMarginManager(poolKey));

		const result = await this.client.core.signAndExecuteTransaction({
			transaction: tx,
			signer: this.keypair,
			include: { effects: true, objectTypes: true },
		});

		if (result.$kind === 'FailedTransaction') {
			throw new Error('Transaction failed');
		}

		const objectTypes = result.Transaction?.objectTypes ?? {};
		const marginManagerAddress = result.Transaction?.effects?.changedObjects?.find(
			(obj) =>
				obj.idOperation === 'Created' && objectTypes[obj.objectId]?.includes('MarginManager'),
		)?.objectId;

		if (!marginManagerAddress) {
			throw new Error('Failed to create margin manager');
		}

		const marginManagers: { [key: string]: MarginManager } = {
			[MARGIN_MANAGER_KEY]: {
				address: marginManagerAddress,
				poolKey: poolKey,
			},
		};

		this.client = this.#createClient(this.env, marginManagers);
	}
}
```

### Coin

The SDK comes with four default coins on Testnet and five default coins on Mainnet.

**Default Testnet coins**

- DEEP
- MYSO
- DBUSDC
- DBUSDT

**Default Mainnet coins**

- DEEP
- MYSO
- USDC
- USDT
- WETH

You can also initialize the SDK with custom coins to interact with margin pools that are not supported by default. To do this, create a `CoinMap` object and pass it to the constructor of the client.

### Pool

Similar to coins, the SDK comes with default pools. You can provide a `PoolMap` during construction to override this behavior.

```tsx
import { orderbook, type OrderbookClient } from '@mysten/orderbook-v3';
import type { MarginManager } from '@mysten/orderbook-v3';
import type { ClientWithExtensions } from '@socialproof/myso/client';
import { decodeMySoPrivateKey } from '@socialproof/myso/cryptography';
import type { Keypair } from '@socialproof/myso/cryptography';
import { MySoGrpcClient } from '@socialproof/myso/grpc';
import { Ed25519Keypair } from '@socialproof/myso/keypairs/ed25519';
import type { Transaction } from '@socialproof/myso/transactions';

export class OrderbookMarginTrader {
	keypair: Keypair;
	client: ClientWithExtensions<{ orderbook: OrderbookClient }>;

	constructor(
		keypair: string | Keypair,
		env: 'testnet' | 'mainnet',
		marginManagers?: { [key: string]: MarginManager },
		maintainerCap?: string,
	) {
		if (typeof keypair === 'string') {
			this.keypair = OrderbookMarginTrader.#getSignerFromPK(keypair);
		} else {
			this.keypair = keypair;
		}

		this.client = new MySoGrpcClient({
			network: env,
			baseUrl:
				env === 'mainnet'
					? 'https://fullnode.mainnet.mysocial.network:443'
					: 'https://fullnode.testnet.mysocial.network:443',
		}).$extend(
			orderbook({
				address: this.getActiveAddress(),
				marginManagers,
				marginMaintainerCap: maintainerCap,
			}),
		);
	}

	static #getSignerFromPK = (privateKey: string) => {
		const { scheme, secretKey } = decodeMySoPrivateKey(privateKey);
		if (scheme === 'ED25519') return Ed25519Keypair.fromSecretKey(secretKey);

		throw new Error(`Unsupported scheme: ${scheme}`);
	};

	signAndExecute = async (tx: Transaction) => {
		const result = await this.client.core.signAndExecuteTransaction({
			transaction: tx,
			signer: this.keypair,
			include: { effects: true },
		});
		if (result.$kind === 'FailedTransaction') {
			throw new Error('Transaction failed');
		}
		return result.Transaction;
	};

	getActiveAddress() {
		return this.keypair.getPublicKey().toMySoAddress();
	}
}
```

### Example setup

The following example uses the default pools and coins provided, and demonstrates margin trading operations.

```tsx
import { Transaction } from '@socialproof/myso/transactions';

import { OrderbookMarginTrader } from './orderbookMarginTrader.js';

(async () => {
	const privateKey = ''; // Can encapsulate this in a .env file

	// Initialize with margin managers if created
	const marginManagers = {
		MARGIN_MANAGER_1: {
			address: '',
			poolKey: 'MYSO_DBUSDC',
		},
	};
	const traderClient = new OrderbookMarginTrader(privateKey, 'testnet', marginManagers);

	const tx = new Transaction();

	// Margin manager contract calls
	traderClient.client.orderbook.marginManager.deposit('MARGIN_MANAGER_1', 'DBUSDC', 10000)(tx);
	traderClient.client.orderbook.marginManager.borrowBase('MARGIN_MANAGER_1', 'MYSO_DBUSDC', 100)(tx);

	// Place leveraged orders
	traderClient.client.orderbook.poolProxy.placeLimitOrder({
		poolKey: 'MYSO_DBUSDC',
		marginManagerKey: 'MARGIN_MANAGER_1',
		clientOrderId: '12345',
		price: 2.5,
		quantity: 100,
		isBid: true,
		payWithDeep: true,
	})(tx);

	// Margin pool operations
	const supplierCap = tx.add(traderClient.client.orderbook.marginPool.mintSupplierCap());
	traderClient.client.orderbook.marginPool.supplyToMarginPool('DBUSDC', supplierCap, 5000)(tx);

	let res = await traderClient.signAndExecute(tx);

	console.dir(res, { depth: null });
})();
```

## Margin manager referral functions

The SDK provides functions for managing referrals with margin managers. Referrals are pool-specific and must first be minted using the core Orderbook SDK before they can be associated with a margin manager.

```tsx
// Set a referral for a margin manager (pool-specific)
// The referral must be a OrderbookPoolReferral minted for the pool the margin manager is associated with
traderClient.client.orderbook.marginManager.setMarginManagerReferral(
	'MARGIN_MANAGER_1',
	referralId,
)(tx);

// Unset the referral for a margin manager for a specific pool
traderClient.client.orderbook.marginManager.unsetMarginManagerReferral(
	'MARGIN_MANAGER_1',
	'MYSO_DBUSDC',
)(tx);
```

:::info

To mint a referral, use the core Orderbook SDK's `mintReferral` function. See the [OrderbookV3 SDK documentation](./orderbookv3-sdk.mdx#referral-functions) for more details on minting and managing referrals.

:::

## Related links

<RelatedLink
	href="https://www.npmjs.com/package/@mysten/orderbook-v3"
	label="OrderbookV3 SDK node package"
	desc="The OrderbookV3 SDK node package on NPM."
/>
<RelatedLink
	href="https://github.com/MystenLabs/orderbookv3"
	label="Orderbook Margin package"
	desc="The Orderbook Margin package on GitHub."
/>
