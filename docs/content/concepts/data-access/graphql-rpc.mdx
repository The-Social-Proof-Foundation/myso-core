---
title: GraphQL for MySo RPC (Beta)
beta: devnet, testnet, mainnet
description: Use GraphQL to make MySo RPC calls. This feature is currently in Beta.
keywords: [ graphql, graphql headers, x-myso-rpc-version, x-myso-rpc-show-usage, variables, fragments, pagination, graphql limits, make rpc call, myso rpc calls ]
---

GraphQL provides a flexible and powerful way to query the MySo network. This page covers the core concepts you need to understand when working with GraphQL for MySo RPC, including request headers, query composition with variables and fragments, pagination strategies, query scope, and service limits.

For practical examples and a quick start guide, see [Querying MySo RPC with GraphQL](/guides/developer/accessing-data/query-with-graphql.mdx).
...
Switch any apps that still use the GraphQL Alpha endpoints (`https://graphql.mainnet.mysocial.network/graphql`, `https://myso-testnet.mystenlabs.com/graphql`, and `https://myso-devnet.mystenlabs.com/graphql`) to the GraphQL Beta endpoints as soon as possible to avoid disruption of service.

- GraphQL Beta Mainnet: `https://graphql.mainnet.mysocial.network/graphql`
- GraphQL Beta Testnet: `https://graphql.testnet.mysocial.network/graphql`
- GraphQL Beta Devnet: `https://graphql.devnet.mysocial.network/graphql`
...
- `x-myso-rpc-version`: Specifies which RPC version to use. Currently only one version is supported.
- `x-myso-rpc-show-usage`: Returns the response with extra query complexity information.

By default, each response contains the following HTTP response headers:

- `x-myso-rpc-request-id`: A unique identifier for the request. This appears in application logs for debugging.
- `x-myso-rpc-version`: The version of the service that handled the request.

```sh
$ curl -i -X POST https://graphql.testnet.mysocial.network/graphql\
     --header 'x-myso-rpc-show-usage: true'                 \
...
content-length: 179
x-myso-rpc-request-id: f5442058-47ab-4360-8295-61c009f38516
x-myso-rpc-version: 1.56.1-
...
When using the [GraphQL online integrated development environment (IDE)](https://graphql.mainnet.mysocial.network/graphql), supply variables as a JSON object to the query in the **Variables** pane at the bottom of the main editing window. You receive a warning if you supply a variable but don't declare it.
...
```sh
$ curl -X POST https://myso-testnet.mystenlabs.com/graphql \
    --header 'Content-Type: application/json' \
    --data '{
...
- `pageInfo` of type <UnsafeLink href="/references/myso-api/myso-graphql/beta/reference/types/objects/page-info">PageInfo</UnsafeLink>, which indicates whether there are more pages before or after the page returned.
...
query NextBalancesPage($address: MySoAddress!, $after: String!) {
...
```graphql
query FirstBalancesPageAtCheckpoint($address: MySoAddress!, $cp: UInt53) {
  checkpoint(sequenceNumber: $cp) {
    query {
      address(address: $address) {
        balances(first: 10) {
...
```graphql
query ObjectsDynamicFields($id: MySoAddress!, $v: UInt53!) {
...
```graphql
query NestedDynamicFields($id: MySoAddress!, $r: UInt53!) {
...
```graphql
query WrappedDynamicField($id: MySoAddress!, $r: UInt53!) {
...
```graphql
query LatestDynamicField($id: MySoAddress!, $v: UInt53!) {
  object(address: $id, version: $v) {
    objectAt {
      version
    }
...
The GraphQL service for MySo RPC is rate-limited on all available instances to keep network throughput optimized and to protect against excessive or abusive calls to the service.
...
- `address.coins` and `address.stakedMySos` are being deprecated in favor of `address.objects` with the corresponding `type` filter.
...
Dry-run now supports the same input schema as `gRPC`'s [`SimulateTransaction`](/references/fullnode-protocol#myso-rpc-v2-Transaction) in JSON form. This means that it can also accept transactions where the inputs have not been fully resolved or serialized, perform gas estimation, and coin selection. The feature is now called `simulateTransaction`.

## Related links

<RelatedLink to="/guides/developer/accessing-data/query-with-graphql.mdx" />
<RelatedLink to="/references/myso-graphql.mdx" />
<RelatedLink to="/concepts/data-access/graphql-indexer" />
<RelatedLink href="https://graphql.testnet.mysocial.network/graphql" label="MySo Testnet GraphiQL" desc="MySo GraphiQL IDE for Testnet." />
<RelatedLink href="https://graphql.mainnet.mysocial.network/graphql" label="MySo Mainnet GraphiQL" desc="MySo GraphiQL IDE for Mainnet." />
