...
description: The GraphQL RPC Beta service offers a structured way for your clients to interact with data on the MySo blockchain. It accesses data processed by a general-purpose indexer and can connect to an archival store for historical network state.
...
- **General-purpose Indexer:** Ingests and transforms MySo checkpoint data using configurable and parallel pipelines, then writes it into a Postgres-compatible database. It can be configured to use the MySo remote checkpoint store and a full node as its sources.
...
- **GraphQL service:** Serves structured queries over indexed data. It follows the [GraphQL specification](https://graphql.org/) and the supported schema is documented in the [GraphQL API reference](/references/myso-graphql). Refer to the [GraphQL getting started guide](/guides/developer/accessing-data/query-with-graphql.mdx).
...
<ImportContent source="crates/myso-indexer-alt-schema/src/schema.rs" mode="code" />
</details>
...
The pipeline composition, concurrency, and deployment grouping is configured through a TOML config file. A built-in [`GenerateConfig` command](https://github.com/the-social-proof-foundation/myso-core/blob/58eabc882671ceb6266473a5531253a90b22a66f/crates/myso-indexer-alt/src/main.rs#L109) is provided to output sample configuration files for different deployment setups. The configuration generated by this command includes all pipelines in a single indexer deployment.
...
This document focuses on the general-purpose Indexer that powers GraphQL. If you want to build your own pipelines for application-specific data (such as Orderbook order books, Walrus blob metadata, Seal access events, and so on), refer to the [Build Your First Custom Indexer](/guides/developer/accessing-data/custom-indexer/build.mdx).
...
Current implementation supports [GCP Bigtable](https://cloud.google.com/bigtable) which is a highly scalable and performant data store. If you plan to operate your own archival store, refer to `myso-kvstore` and `myso-kv-rpc` for indexer setup and RPC service implementation respectively. For the indexer setup, make sure to use the [custom indexing framework](/guides/developer/accessing-data/custom-indexing-framework.mdx). If you're interested in contributing support for other scalable data stores, reach out on GitHub by creating a new issue.

<details>
<summary>
`main.rs` in `myso-kvstore`
</summary>
<ImportContent source="crates/myso-kvstore/src/main.rs" mode="code" />
</details>

<details>
<summary>
`main.rs` in `myso-kv-rpc`
</summary>
<ImportContent source="crates/myso-kv-rpc/src/main.rs" mode="code" />
</details>
...
<RelatedLink href="/references/myso-api/myso-graphql/beta/reference" label="GraphQL Beta schema" desc="Schema documentation for GraphQL Beta" />