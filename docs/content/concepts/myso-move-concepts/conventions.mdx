...
description: Recommended Move 2024 best practices for MySo development.
---

This guide outlines recommended conventions and best practices for writing Move smart contracts on MySo. Following these guidelines helps create more maintainable, secure, and composable code that aligns with ecosystem standards.

While these conventions are recommendations rather than strict rules, they represent patterns that have proven effective across many MySo projects. They help create consistency across the ecosystem and make code easier to understand and maintain.

## Organization principles

### Package

A MySo package consists of:
...
- `Move.lock` file that the MySo Move toolchain automatically generates to lock the versions of the dependencies and track the different published and upgraded versions of the package that exist on the different networks.
...
The following section covers common patterns and best practices specific to Move development onMySo, including object ownership models and function design principles.
...
public fun withdraw(state: &mut State, account: &mut Account, ctx: &mut TxContext): Coin<MYSO> {
...
public fun wrong_withdraw(state: &mut State, ctx: &mut TxContext): Coin<MYSO> {
...
public fun new(deposit: Coin<MYSO>, ctx: &mut TxContext): OwnedWallet {
    // Implementation omitted.
    abort(0)
}
...
public fun new_shared(deposit: Coin<MYSO>, ctx: &mut TxContext) {
    // Implementation omitted.
...
public fun start_battle(
    _hero: &mut Hero, // hero to use in battle
    hero_id: ID, // hero object id
