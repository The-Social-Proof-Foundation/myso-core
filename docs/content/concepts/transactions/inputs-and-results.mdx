...
- If the object is owned by another object, as in it was sent to an object's ID through the `TransferObjects` command or the `myso::transfer::transfer` function, then use `ObjectArg::Receiving(ObjectID, SequenceNumber, ObjectDigest)`. The object data is the same as for the `ImmOrOwnedObject` case.

For pure inputs, the only data provided is the [BCS](https://github.com/the-social-proof-foundation/myso-core/blob/main/crates/myso-framework/docs/std/bcs.md) bytes, which are deserialized to construct Move values. Not all Move values can be constructed from BCS bytes. The following types are allowed to be used with pure values:
...
- `SplitCoins`: Produces one or more coins from a single coin. The type of each coin is `myso::coin::Coin<T>` where the specific coin type `T` matches the coin being split.

- `Publish`: Returns the upgrade capability `myso::package::UpgradeCap` for the newly published package.

- `Upgrade`: Returns the upgrade receipt `myso::package::UpgradeReceipt` for the upgraded package.
