...
description: The life of a transaction on the MySo network has some differences compared to those from other blockchains.
keywords: [ transaction lifecycle, transaction flow, transaction finality, settlement, consensus, certification, execution ]
---

At a high level, the following diagram outlines the life cycle of a transaction on the MySo blockchain.
...
1. MySo sends the transaction to each [validator](/guides/operator/validator-index.mdx), often through a [full node](/guides/operator/myso-full-node.mdx). Validators perform a series of validity and safety checks, sign it, and return the signed transaction to the client.

1. The client collects the responses from a set of validators that account for at least 2/3 of the [validator committee](/concepts/myso-architecture/consensus.mdx) to form a transaction certificate. As a result, MySo validators do not need to propagate signatures on a best-effort basis (gossip signatures) or aggregate certificates. This effort is now the responsibility of the client or gateway.
...
1. After assembling the certificate, the client sends it back to all validators, who check its validity and acknowledge its receipt to the client.
    ...
    1. If the transaction involves [owned objects](/guides/developer/objects/object-ownership/address-owned.mdx) or immutable objects exclusively, MySo can process the transaction certificate immediately and execute it without waiting for the [consensus engine](/concepts/myso-architecture/consensus.mdx).

1. All certificates are forwarded to the MySo DAG-based consensus protocol, which also operated by the validators. Consensus outputs a complete order of certificates; the validators check and execute those that contain [shared objects].
...
1. Subsequently, MySo forms [checkpoints](/concepts/cryptography/system/checkpoint-verification.mdx) for every consensus commit, which it uses to drive the [reconfiguration protocol](/concepts/myso-architecture/epochs.mdx).

The [MySo Lutris](https://docs.mysocial.network/paper/myso-lutris.pdf) paper provides additional details on how the safety and liveness protocols operate, as well as proof of security for the standard distributed systems model with Byzantine participants in partial synchrony.

## Transaction submission

All transactions on MySo begin when they are submitted to the network. For example, imagine you want to transfer an NFT that you own in your wallet to your friend. First, you would create a transaction using a wallet or some other app. The transaction includes your gas object and a command to transfer the NFT object to your friend's address. Before the wallet or app submits the transaction to the network, it must also sign it.

After the transaction is signed, the wallet or app submits the transaction to a MySo full node on behalf of the user.
...
- The gas coin is a `Coin<MYSO>` object, and it contains at least as much gas as specified in the transaction's gas budget.

If all the validity checks pass, the validator then attempts to lock all the owned input objects to the given transaction digest. This ensures each owned input object can only be used by one transaction at a time and is the way that MySo prevents double-spending. In other words, this makes sure that you send your NFT to only one friend instead of trying to send the same NFT to all your friends.
...
The validator submits the transaction toMySo's consensus layer, which orders the transaction with respect to other transactions using the same shared objects and then executes it. Transactions that don't overlap on inputs at all can be verified and executed completely in parallel.
...
At this point, the transaction has reached the end of its life cycle and is included in the permanent record of transaction activity on the MySo network.
...
Periodically (~24 hours), the MySo [Testnet](/concepts/myso-architecture/networks.mdx) and [Mainnet](/concepts/myso-architecture/networks.mdx) networks enter the process of [epoch](/concepts/myso-architecture/epochs.mdx) change. During epoch change, the network calculates and distributes [staking rewards](/concepts/tokenomics/staking-unstaking.mdx), validator metadata takes effect, and other network processes are performed. All user transactions are delayed until the new epoch begins.
...
As a real-world example, suppose you want to pay the local coffee shop 10 MYSO for your morning coffee. The following steps demonstrate how the coffee shop can be sure that the payment is complete.

### Transaction creation and signing

You open the wallet app on your phone and scan the coffee shop's QR code that contains the recipient on-chain address. The wallet app constructs a transaction that transfers 10 MYSO from your MySo address to the coffee shop's address. You review the transaction details and approve it. The wallet app then signs the transaction with your private key and submits it to a full node. The full node broadcasts the transaction to all validators in the network.
