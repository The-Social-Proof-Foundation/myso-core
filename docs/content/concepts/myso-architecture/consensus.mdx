---
title: Consensus
description: Overview of the MySo consensus mechanism.
...
To participate in consensus, validators must [stake MYSO tokens](/guides/operator/validator/validator-rewards.mdx) on the network. MySo uses delegated proof-of-stake (DPoS) to determine which validators operate the network and their voting power. Validators are incentivized to participate in good faith through a share of transaction fees, [staking rewards](/guides/operator/validator/validator-rewards.mdx), and slashing of stake and staking rewards in case of misbehavior.

### Epochs

An [epoch](/concepts/myso-architecture/epochs.mdx) is a duration of time where the MySo validators and their stakes do not change. An epoch is about 24 hours on both [Mainnet](/concepts/myso-architecture/networks.mdx) and [Testnet](/concepts/myso-architecture/networks.mdx). At an epoch boundary, [reconfiguration](/concepts/myso-architecture/epochs.mdx) might occur and can change the set of validators participating in the network and their voting power. Conceptually, reconfiguration starts a new instance of the MySo protocol with the previous epoch's final state as genesis and the new set of validators as the operators. Besides validator set changes, [tokenomics](/concepts/tokenomics.mdx) operations such as staking and un-staking, and distribution of staking rewards are also processed at epoch boundaries.

### Quorums

A quorum is a set of [validators](/guides/operator/validator/validator-config.mdx) whose combined voting power is greater than two-thirds (>2/3) of the total during a particular epoch. For example, in a MySo instance operated by 4 validators that all have the same voting power, any group containing 3 validators is a quorum.

The quorum size of >2/3 ensures Byzantine fault tolerance (BFT). A validator commits a transaction only if the transaction is accompanied by cryptographic signatures from a quorum. MySo calls the combination of the transaction and the quorum signatures on its bytes a certificate. The policy of committing only certificates ensures Byzantine fault tolerance: if >2/3 of the validators faithfully follow the protocol, they are guaranteed to eventually agree on both the set of committed certificates and their effects.
...
<RelatedLink to="/concepts/myso-architecture/myso-security.mdx" />

<RelatedLink href="https://docs.mysocial.network/paper/mysticeti.pdf" label="MYSTICETI: Reaching the Latency Limits with Uncertified DAGs" desc="Whitepaper that documents the Mysticeti protocol." />