...
description: The MySo protocol, framework, and execution engine are frequently extended to include new functionality and bug fixes. The upgrade process ensures all clients use the same source.
keywords: [ protocol upgrades, framework upgrades, protocol config, feature flags, validator upgrades, protocol version ]
---

The MySo protocol, framework, and execution engine are frequently extended to include new functionality and bug fixes. This functionality is added as new code released to validator operators as part of regular software releases.

As [consensus](/concepts/myso-architecture/consensus.mdx) requires that all validators agree on the results of executing each transaction, how is code released that changes transaction execution, given that operators cannot all upgrade at the same instant? Further, how is it ensured that all transaction history can be replayed even after functionality has changed?

To solve this, MySo uses a process called _protocol upgrades_.
...
1. A new release of the MySo validator software is built and released to validator and full node operators.
...
Not all new MySo functionality comes from validator code changes. Developers also extend the MySo framework. For example, they might add new native functions to expose functionality to smart contracts. The process for framework updates is similar to protocol upgrades.

Instead of feature flags, MySo objects coordinate framework changes. The MySo framework is a special object with ID `0x2`. The Move source code for the framework is built into the validator binary.
...
<RelatedLink to="/concepts/myso-architecture/consensus.mdx" />