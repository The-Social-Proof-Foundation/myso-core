...
description: Epochs define time periods on MySo where the validator set remains unchanged. Equivocation occurs when objects are used incorrectly across transactions. Reconfiguration adjusts network parameters at epoch boundaries.
keywords: [ epochs, equivocation, reconfiguration, validators, object versioning ]
---

OnMySo, an _epoch_ is a period of time defined by the network. During this time, the MySo [validator](/guides/operator/validator/validator-config.mdx) set and their stakes remain unchanged. On both [Mainnet](/concepts/myso-architecture/networks.mdx) and [Testnet](/concepts/myso-architecture/networks.mdx), an epoch is about 24 hours. Epochs on Devnet are 1 hour. This timeframe allows validators to process transactions efficiently without worrying about ad hoc validator changes.
...
1. **Distribution of gas rewards:** Computation gas fees are distributed to the validator staking reward pool, from which stakers can withdraw. Storage fees are allocated to a storage fund, playing a vital role in the [MySo tokenomics](/concepts/tokenomics.mdx).
