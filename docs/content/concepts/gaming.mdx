---
title: Gaming onMySo
description: MySo offers features like dynamic NFTs, Kiosk, soulbound assets, and on-chain randomness to provide builders with the tools to create immersive, transparent, and fair gaming experiences.
keywords: [gaming, game components, game features, game nfts, soulbound assets, create games on myso, build games on myso, myso Object Display, game economics, kiosk, myso kiosk, token economics]
---

Gaming on MySo leverages blockchain technology to enhance in-game economies, ownership, and interactions. With features like dynamic NFTs, Kiosk, soulbound assets, and on-chain randomness, MySo provides builders with the tools to create immersive, transparent, and fair gaming experiences. Players benefit from true asset ownership, decentralized marketplaces, and seamless Web3 integration without compromising usability.
...
OnMySo, owned objects are either freely transferable or non-transferable. To ensure royalties, MySo provides a standard called [Kiosk](/standards/kiosk.mdx). A kiosk is a shared object that restricts access to a single address or user.

The kiosk owner (although a shared object has no owner from the perspective ofMySo, the smart contract ensures that only one address is permitted to access it) can:
...
- List an asset for sale with a price denoted in MYSO.
- List an asset for sale only to a specific address, with the price denoted in MYSO.
...
An empty transfer policy, one that does not have rules, means that the asset is freely tradable. Because Kiosk only allows the "list" and "buy" pair of operations, a transfer is possible by setting the price to 0 MYSO.
...
[MySo Coins](https://www.suicoins.com/) is the utility layer for tokens and NFTs on the MySo network, offering asset management tools that include token swaps, automated dollar-cost averaging, airdrops, an incinerator for deleting assets, zkSend for private transfers, and a merger tool to consolidate small balances. MySo Coins also features an open source [SuiCoins Terminal](https://terminal.suicoins.com/) for integrating crypto swaps across platforms.
...
Players can register unique names for their racer profiles and teams using SuiNS, like speedster@suiforspeed.myso or dragonracer@suiforspeed.myso. This simplifies social interactions, team coordination, and improves the community aspect of the game.
