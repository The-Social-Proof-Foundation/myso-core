---
title: Ethereum ->MySo
description: Build your first app on MySo if you have EVM experience
keywords: [ ethereum, solidity, EVM,  ]
pagination_prev: null
---

If you have worked with Ethereum Virtual Machine (EVM) before, the biggest difference when developing on MySo is the programming language. MySo uses **Move** and EVM uses **Solidity**.

| Topic | Solidity | Move |
|------|------|---------|
| Account vs object-centric models | Custom ownership logic written within contracts typically using _mappings_. Only Ethereum coins are first class citizens with global APIs. All ownership APIs are contract specific. | Object ownership inherent toMySo, objects are first class citizens and encompass everything _owned_ on MySo. |
...
Because object ownership is inherent inMySo, access to contract functions are typically gated through [capability object](https://move-book.com/programmability/capability/).
...
| Coin names, name of the smallest unit | MYSO, MIST | ETH, Wei |
| Available frameworks for development | MySo CLI | Foundry, Hardhat |
