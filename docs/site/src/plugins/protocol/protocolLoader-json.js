...
const protocolInject = async function (source) {
  ...
  const mysoSorted = (array) => {
    return array.sort((a, b) => {
      const aStartsWithMySo = a.name.startsWith("myso");
      const bStartsWithMySo = b.name.startsWith("myso");

      if (aStartsWithMySo && !bStartsWithMySo) return -1;
      if (!aStartsWithMySo && bStartsWithMySo) return 1;
      return 0;
    });
  };
  ...
  const files = mysoSorted(spec.files);
  const bordercolor = "border-myso-blue-dark dark:border-myso-blue";

  const tabStyle = `grid grid-cols-6 border border-solid border-b-0 border-myso-blue-dark ${bordercolor}`;
  const tabRowStyle = `p-4 border-0 border-b border-solid ${bordercolor} col-span-full`;
  const tabHeaderStyle = `${tabRowStyle} bg-myso-gray-50 dark:bg-myso-ghost-dark`;
  const tabAltHeaderStyle = `${tabRowStyle} bg-myso-ghost-white dark:bg-myso-gray-95`;
      ...
      if (fields.length > 0) {
        for (const field of fields.entries()) {
          ...
          if (hasLabel) {
            ...
            let labelBg = "bg-myso-ghost-white dark:bg-myso-ghost-dark";
            if (field[1].label === "optional") {
              label = "Proto3 optional";
              labelBg = "bg-myso-blue-light dark:bg-myso-blue-dark";
            } else if (field[1].label === "repeated") {
              label = "Repeated []";
              labelBg = "bg-myso-warning-light dark:bg-myso-warning-dark";
            }
            content.push(
  ...
  content.push("\n## Scalar Value Types");
  const cellStyle =
    "m-2 min-w-24 max-w-[13rem] rounded-lg border border-solid align-center text-center relative border-myso-gray-65";
  const titleStyle =
    "p-4 pb-2 font-bold text-myso-ghost-dark dark:text-myso-ghost-white bg-myso-gray-50 dark:bg-myso-ghost-dark border border-solid border-transparent rounded-t-lg";
  const valStyle =
    "p-4 pt-2 border border-solid border-transparent border-t-myso-gray-65 whitespace-break-spaces";
