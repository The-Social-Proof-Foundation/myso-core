...
const BRIDGE_PATH = path.join(
  __dirname,
  "../../../../../crates/myso-framework/docs/bridge",
);
const FRAMEWORK_PATH = path.join(
  __dirname,
  "../../../../../crates/myso-framework/docs/myso",
);
...
const MYSOSYS_PATH = path.join(
  __dirname,
  "../../../../../crates/myso-framework/docs/myso_system",
);
...
const prefixRootDir = (seg) => `myso_${seg}`;
...
const CRATE_PREFIX_MAP = {
  ...
  myso: "myso_myso",
  std: "myso_std",
  myso_system: "myso_myso_system",
};

const CRATE_PACKAGES_PATH = {
  bridge: "myso/crates/myso-framework/packages/bridge",
  myso: "myso/crates/myso-framework/packages/myso",
  std: "myso/crates/myso-framework/packages/std",
  myso_system: "myso/crates/myso-framework/packages/myso_system",
};
...
const toLowerTitleText = (s) =>
  s.replace(/^myso_/, "").replace(/[-_]+/g, " ").toLowerCase();
  ...
  return {
    ...
    async loadContent() {
          ...
          parts.forEach((part, i) => {
            if (!part.match(/\.md$/)) {
              ...
              if (!fs.existsSync(newDir)) {
                ...
                const topUnpref = top.replace(/^myso_/, "");

                // Category label: lowercased dirname without myso_ prefix
                const unprefixed = part.replace(/^myso_/, "");
      ...
      function buildIndexForDir(absDir) {
        ...
        const titleText = `myso:${toLowerTitleText(dirName)}`;

        const entries = fs.readdirSync(absDir, { withFileTypes: true });
        const children = [];
        const topDir = relParts[0] || "";
        const frameworkName = topDir.replace(/^myso_/, "");
        ...
        const fm = [
          ...
          `slug: ${slug}`,
          `description: "${(`Documentation for the modules in the ${CRATE_PACKAGES_PATH[topDir?.replace(/^myso_/, "")] ?? ""} crate. Select a module from the list to see its details.`).replace(/"/g, '\\"')}"`,
          "---",
          "",
        ].join("\n");
