...
- For faster iteration, use -p to select only the most relevant packages for testing. Use multiple `-p` flags if necessary, e.g. `cargo nextest run -p myso-types -p myso-core`
...
│   ├── myso-core/                       # Core blockchain logic
│   ├── myso-node/                       # Validator node implementation
│   ├── myso-framework/                  # Move system packages & stdlib
│   ├── myso-types/                      # Core type definitions
│   ├── myso-json-rpc/                   # JSON-RPC API server
│   ├── myso-indexer-alt-graphql/        # GraphQL API server
│   └── myso-indexer-alt/                # Blockchain data indexer
├── consensus/                          # Consensus mechanism (Mysticeti)
├── myso-execution/                      # Move execution layer with versions (v0, v1, v2 and latest)
...
1. **Authority System**: MySo uses a set of validators (authorities) that process transactions in parallel. Each authority maintains its own state and participates in Byzantine consensus.

2. **Object Model**: Unlike account-based blockchains, MySo uses an object-centric model where:
