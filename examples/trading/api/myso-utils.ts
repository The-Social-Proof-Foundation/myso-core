...
import { fromBase64 } from '@mysten/myso/utils';

export type Network = 'mainnet' | 'testnet' | 'devnet' | 'localnet';

export const ACTIVE_NETWORK = (process.env.NETWORK as Network) || 'testnet';

export const MYSO_BIN = `myso`;

export const getActiveAddress = () => {
	return execSync(`${MYSO_BIN} client active-address`, { encoding: 'utf8' }).trim();
};

/** Returns a signer based on the active address of system's myso. */
...
export const getClient = (network: Network) => {
	return new MySoClient({ url: getFullnodeUrl(network) });
};
...
export const publishPackage = async ({
	...
	const { modules, dependencies } = JSON.parse(
		execSync(`${MYSO_BIN} move build --dump-bytecode-as-base64 --path ${packagePath}`, {
			encoding: 'utf-8',
		}),
	);
