...
import { EventId, MySoClient, MySoEvent, MySoEventFilter } from '@mysten/myso/client';

import { CONFIG } from '../config';
import { prisma } from '../db';
import { getClient } from '../myso-utils';
import { handleEscrowObjects } from './escrow-handler';
import { handleLockObjects } from './locked-handler';

type MySoEventsCursor = EventId | null | undefined;

type EventExecutionResult = {
	cursor: MySoEventsCursor;
	hasNextPage: boolean;
};
...
type EventTracker = {
	...
	type: string;
	filter: MySoEventFilter;
	callback: (events: MySoEvent[], type: string) => any;
};
...
const runEventJob = async (client: MySoClient, tracker: EventTracker, cursor: MySoEventsCursor) => {
