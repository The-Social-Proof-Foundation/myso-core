...
import { PublicKey } from "@mysten/myso/cryptography";
import { ObjectRef, Transaction } from "@mysten/myso/transactions";
...
export class Transactions {
    ...
    newMultiSigGame(player: PublicKey, opponent: PublicKey): Transaction {
        ...
        const game = tx.moveCall({
            ...
            arguments: [
                tx.pure.address(player.toMySoAddress()),
                tx.pure.address(opponent.toMySoAddress()),
                tx.pure(bcs.vector(bcs.u8()).serialize(admin.toRawBytes()).toBytes()),
            ],
        });

        tx.transferObjects([game], admin.toMySoAddress());
