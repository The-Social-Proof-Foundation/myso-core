...
use myso_sdk::{
    rpc_types::{
        DevInspectArgs, DevInspectResults, DryRunTransactionBlockResponse, ObjectChange, MySoData,
        MySoExecutionStatus, MySoObjectData, MySoObjectDataFilter, MySoObjectDataOptions,
        MySoObjectResponse, MySoObjectResponseQuery, MySoProtocolConfigValue,
...
pub struct Connection {
    /// The MySo CLI config file, (default: ~/.myso/myso_config/client.yaml)
    #[clap(long)]
...
impl Client {
        ...
        let Some(config) = conn.config.or_else(|| {
            ...
            bail!(
                "Cannot find wallet config. No config was supplied, and the default path \
                 (~/.myso/myso_config/client.yaml) does not exist.",
            );
        };
    ...
    pub(crate) async fn turn_cap(&mut self, game: &Game) -> Result<ObjectRef> {
        ...
        let query = Some(MySoObjectResponseQuery::new(
            Some(MySoObjectDataFilter::StructType(turn_cap_type.clone())),
            Some(MySoObjectDataOptions::new().with_bcs()),
        ));
        ...
        loop {
            ...
            for MySoObjectResponse { data, error } in response.data {
    ...
    pub(crate) async fn new_shared_game(&mut self, opponent: MySoAddress) -> Result<ObjectID> {
        let player = self.wallet.active_address()?;
    ...
    pub async fn new_owned_game(&mut self, opponent_key: PublicKey) -> Result<ObjectID> {
        ...
        let opponent = MySoAddress::from(&opponent_key);
        ...
        let admin = MySoAddress::from(&admin_key);
        let admin_bytes =
    ...
    async fn max_gas_budget(&self) -> Result<u64> {
        ...
        let Some(Some(MySoProtocolConfigValue::U64(max))) = cfg.attributes.get("max_tx_gas") else {
    ...
    async fn execute_transaction(&self, tx: Transaction) -> Result<MySoTransactionBlockResponse> {
        let response = self
            .wallet
    ...
    async fn client(&self) -> Result<MySoClient> {
        self.wallet
            .get_client()
            .await
