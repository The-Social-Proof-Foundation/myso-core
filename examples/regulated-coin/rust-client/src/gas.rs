...
use myso_sdk::rpc_types::{
    MySoObjectDataFilter, MySoObjectDataOptions, MySoObjectResponseQuery, MySoRawData,
};
use myso_sdk::types::base_types::{ObjectID, ObjectRef, MySoAddress};
use myso_sdk::types::gas_coin::GasCoin;
...
pub async fn select_gas(
    ...
    if let Some(gas) = input_gas {
        ...
        let object = read_api
            .get_object_with_options(gas, MySoObjectDataOptions::new())
            .await?
    ...
    let gas_objs = read_api
        .get_owned_objects(
            ...
            Some(MySoObjectResponseQuery {
                filter: Some(MySoObjectDataFilter::StructType(GasCoin::type_())),
                options: Some(MySoObjectDataOptions::new().with_bcs()),
            }),
            None,
            None,
        )
    ...
    for obj in gas_objs {
        let MySoRawData::MoveObject(raw_obj) = &obj
            .data
            .as_ref()
