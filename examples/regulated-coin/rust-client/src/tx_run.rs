...
use myso_sdk::rpc_types::MySoTransactionBlockResponse;
use myso_sdk::types::base_types::{MySoAddress, ObjectID};
use myso_sdk::wallet_context::WalletContext;

#[derive(Debug)]
pub enum AppCommand {
    DenyListAdd(MySoAddress),
    DenyListRemove(MySoAddress),
    MintAndTransfer(u64, MySoAddress),
    Transfer(ObjectID, MySoAddress),
    Burn(ObjectID)
}

pub struct AppConfig {
    pub client: MySoClient,
