...
enum CliCommand {
    ...
    /// Transfer coin from the myso client's active address
    Transfer {
    ...
    /// Burn coin inside the myso client's active address
    Burn {
        /// The Coin to burn
        #[arg(value_parser)]
        coin: String,
...
async fn cli_parse() -> Result<(AppConfig, AppCommand)> {
    ...
    let command = match command {
        CliCommand::DenyListAdd { address } => {
            AppCommand::DenyListAdd(MySoAddress::from_str(&address)?)
        }
        CliCommand::DenyListRemove { address } => {
            AppCommand::DenyListRemove(MySoAddress::from_str(&address)?)
        }
        CliCommand::MintAndTransfer { balance, address } => {
            AppCommand::MintAndTransfer(balance, MySoAddress::from_str(&address)?)
        }
        CliCommand::Transfer { coin, address } => AppCommand::Transfer(
            ObjectID::from_hex_literal(&coin)?,
            MySoAddress::from_str(&address)?,
        ),
