...
use myso_config::{myso_config_dir, MYSO_CLIENT_CONFIG, MYSO_KEYSTORE_FILENAME};
...
fn cmd_myso_client_switch(new_addr: MySoAddress) -> Result<()> {
    println!("SWITCHING TO ADDRESS: {new_addr}");
    let myso_client_switch = format!("myso client switch --address {new_addr}");
    let _ = std::process::Command::new("sh")
        .arg("-c")
        .arg(myso_client_switch)
        .output()?;
    Ok(())
}

fn get_other_address(different_from: MySoAddress) -> Result<MySoAddress> {
    let keystore =
        FileBasedKeystore::load_or_create(&myso_config_dir()?.join(MYSO_KEYSTORE_FILENAME))?;
    Ok(keystore
        .keys()
        .into_iter()
        .find(|pub_key| MySoAddress::from(pub_key) != different_from)
        .map(|pub_key| MySoAddress::from(&pub_key))
...
async fn get_config() -> Result<AppConfig> {
    ...
    let wallet_context =
        WalletContext::new(&myso_config_dir()?.join(MYSO_CLIENT_CONFIG), None, None).await?;
...
async fn test_is_blocked() -> Result<()> {
    ...
    let resp2 = run_as_deny_addr(coin.0, admin_addr).await; // Notice we do not use '?' so that
                                                            // cmd_myso_client_switch runs again
    cmd_myso_client_switch(admin_addr)?;
    assert!(resp2.is_err());
