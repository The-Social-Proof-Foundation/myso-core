    ...
    program.command('deny-list-add')
        ...
        .action((options) => {
            ...
            txb.moveCall({
                ...
                arguments: [
                    txb.object(MYSO_DENY_LIST_OBJECT_ID),
...
async function executeTx(txb: Transaction) {
    ...
    console.log("Connecting to MySo network: ", MYSO_NETWORK);
    const mysoClient = new MySoClient({url: MYSO_NETWORK});
    ...
    mysoClient.signAndExecuteTransaction({
        signer: adminKeypair,
        transaction: txb,
