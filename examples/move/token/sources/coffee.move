...
use myso::coin::{Self, TreasuryCap, Coin};
use myso::myso::MYSO;
use myso::token::{Self, Token};
use myso::tx_context::sender;
...
fun init(otw: COFFEE, ctx: &mut TxContext) {
    ...
    myso::transfer::public_freeze_object(metadata);
    myso::transfer::share_object(CoffeeShop {
        coffee_points,
        id: object::new(ctx),
...
public fun buy_coffee(app: &mut CoffeeShop, payment: Coin<MYSO>, ctx: &mut TxContext) {
    // Check if the customer has enough MYSO to pay for the coffee.
    ...
    myso::event::emit(CoffeePurchased {})
}
