...
accept and process payments on MySo. We'll focus on highlighting the trade-offs
...
using the function `fun make_payment(payment_id: u64, coin: Coin<MYSO>, to: address)`.
...
- Payments can still be made using the `identified_payment::make_payment`
  function that uses `myso::transfer::transfer` under the hood, so payments can
...
object is `key`-only, then the `myso::transfer::receive` function can be called
in the module that defines the object, but not elsewhere. Elsewhere, the
`myso::transfer::public_receive` function must be called and can only be used on
