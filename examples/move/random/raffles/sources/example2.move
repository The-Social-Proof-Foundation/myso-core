...
/// Basic raffles game that depends on MySo randomness.
///
...
public fun create(end_time: u64, cost_in_myso: u64, ctx: &mut TxContext) {
    let game = Game {
        id: object::new(ctx),
        cost_in_myso,
        participants: 0,
...
entry fun close(game: Game, r: &Random, clock: &Clock, ctx: &mut TxContext) {
    ...
    let Game { id, cost_in_myso: _, participants, end_time: _, balance, participants_table } = game;
    if (participants > 0) {
...
/// Anyone can play.
public fun play(game: &mut Game, coin: Coin<MYSO>, clock: &Clock, ctx: &mut TxContext) {
    assert!(game.end_time > clock.timestamp_ms(), EGameAlreadyCompleted);
    assert!(coin.value() == game.cost_in_myso, EInvalidAmount);
...
#[test_only]
public fun cost_in_myso(game: &Game): u64 {
    game.cost_in_myso
}
