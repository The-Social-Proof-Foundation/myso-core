...
/// A betting game that depends on MySo randomness.
///
/// Anyone can create a new game for the current epoch by depositing MYSO as the initial balance. The creator can
/// withdraw the remaining balance after the epoch is over.
///
/// Anyone can play the game by betting on X MYSO. They win X with probability 49% and lose the X MYSO otherwise.
///
module slot_machine::example;

use myso::balance::Balance;
use myso::coin::{Self, Coin};
use myso::random::{Random, new_generator};
use myso::myso::MYSO;
...
public fun create(reward: Coin<MYSO>, ctx: &mut TxContext) {
...
public fun close(game: Game, ctx: &mut TxContext): Coin<MYSO> {
...
entry fun play(game: &mut Game, r: &Random, coin: &mut Coin<MYSO>, ctx: &mut TxContext) {
    assert!(ctx.epoch() == game.epoch, EInvalidEpoch);
