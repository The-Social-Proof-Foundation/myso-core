...
use myso::random::{Self, update_randomness_state_for_testing, Random};
use myso::myso::MYSO;
use myso::test_scenario as ts;

fun mint(addr: address, amount: u64, scenario: &mut ts::Scenario) {
    transfer::public_transfer(coin::mint_for_testing<MYSO>(amount, scenario.ctx()), addr);
...
fun test_game() {
    ...
    let coin = ts.take_from_sender<Coin<MYSO>>();
    ...
    let mut coin: Coin<MYSO> = ts.take_from_sender();
