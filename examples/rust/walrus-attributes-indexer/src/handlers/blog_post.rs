...
use myso_indexer_alt_framework::Result;
use myso_indexer_alt_framework::pipeline::{Processor, sequential::Handler};
use myso_indexer_alt_framework::postgres;
use myso_indexer_alt_framework::types::base_types::{ObjectID, SequenceNumber, MySoAddress};
...
use myso_indexer_alt_framework::types::parse_myso_struct_tag;
...
pub enum ProcessedWalrusMetadata {
    Upsert {
        ...
        /// ID of the Blob object on MySo, used during reads to fetch the actual blob content. If
...
impl BlogPostPipeline {
    pub fn new(type_string: &str) -> Result<Self> {
        let metadata_type = parse_myso_struct_tag(type_string)?;
    ...
    fn to_stored(&self) -> Result<StoredBlogPost> {
        match self {
            ProcessedWalrusMetadata::Upsert {
                ...
                // This is meant to validate that the publisher address stored is a valid MySoAddress
