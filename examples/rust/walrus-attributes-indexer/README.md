...
Given a service that allows users to upload blog posts to Walrus and creates an associated `Metadata` dynamic field with `view_count`, `title`, and `publisher` (MySo address that created the Walrus blob), you can write a corresponding indexer that commits these attributes to a store of your choice to emulate a blog post platform. Then, users can:
...
# Set a path: value attribute pair on the Metadata dynamic field of a Blob object on MySo.
walrus set-blob-attribute {MySo blob object id} --attr "title" {title} --attr "view_count" {view_count}
```
...
# MySo object ID: 0xcfb3d474c9a510fde93262d4b7de66cad62a2005a54f31a63e96f3033f465ed3
