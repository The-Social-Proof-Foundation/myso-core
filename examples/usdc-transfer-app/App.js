...
// Define the USDC token type on MySo Testnet
// This is the unique identifier for the USDC token on MySo
...
function HomeContent() {
  ...
  // Get the MySo client for interacting with the MySo network
  ...
  const handleSendTokens = async () => {
    ...
    try {
      ...
      // This uses the MySoClient to get coins of the specified type owned by the current address
      const { data: coins } = await mysoClient.getCoins({
      ...
      // Transaction is used to construct and execute transactions on MySo
  ...
  return (
    <main className="mainwrapper">
      <div className="outerwrapper">
        <h1 className="h1">MySo USDC Sender (Testnet)</h1>
        <ConnectButton />
