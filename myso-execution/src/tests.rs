...
#[test]
/// Make sure that all accesses to execution layer crates in the `myso-node` and `myso-replay` crates
/// go via the `myso-execution` crate (in other words, the `myso-execution` crate dominates execution
/// layer crates in the dependency graphs of `myso-node` and `myso-replay`).
///
...
fn test_encapsulation() {
    ...
    let mut exec_crates: BTreeSet<_> = packages.normal_deps("myso-execution").collect();

    // Remove the crates that the execution layer depends on but which are not directly part of the
    // execution layer -- these don't need to be accessed exclusively via `myso-execution`.
    exec_crates.remove("myso-protocol-config");
    exec_crates.remove("myso-types");
    ...
    panic!(
        "protocol-sensitive binaries depend on execution crates outside of 'myso-execution', e.g.:\n\
         \n  {}\n\
         \n\
         This can cause execution to fork by not respecting the execution layer version set in the \
         protocol config.  Fix this by depending on these crates via 'myso-execution'.\n\
         \n\
         P.S. if you believe one of these crates should not be part of 'myso-execution' then update \
         the test to exclude this crate.",
        examples.join("\n  "),
    );
}

/// Parse `cargo metadata` for the `myso` repo.
fn cargo_metadata() -> cargo_metadata::Result<Metadata> {
    let myso_execution = PathBuf::from(env!("CARGO_MANIFEST_DIR"));

    MetadataCommand::new()
        .manifest_path(myso_execution.join("../Cargo.toml"))
        .current_dir(myso_execution.join(".."))
        .no_deps()
        .exec()
}
