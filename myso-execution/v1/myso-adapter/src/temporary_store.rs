    ...
    fn into_effects_v1(
        ...
        let updated_gas_object_info = if let Some(coin_id) = gas_charger.gas_coin() {
            ...
            (
                ...
                Owner::AddressOwner(MySoAddress::default()),
            )
        };
...
impl TemporaryStore<'_> {
    ...
    fn get_objects_to_authenticate(
    ...
    ) -> MySoResult<(Vec<ObjectID>, HashSet<ObjectID>)> {
        ...
        for id in &self.execution_results.modified_objects {
            ...
            match &old_obj.owner {
                ...
                Owner::Immutable => {
                    assert!(is_epoch_change, "Immutable objects cannot be written, except for MySo Framework/Move stdlib upgrades at epoch change boundaries");
...
impl Storage for TemporaryStore<'_> {
    ...
    fn check_coin_deny_list(
        &self,
        _receiving_funds_type_and_owners: BTreeMap<TypeTag, BTreeSet<MySoAddress>>,
    ) -> DenyListResult {
        unreachable!("Coin denylist v2 is not supported in myso-execution v1");
    }

    fn record_generated_object_ids(&mut self, _generated_ids: BTreeSet<ObjectID>) {
        unreachable!(
            "Generated object IDs are not recorded in ExecutionResults in myso-execution v1"
        );
    }
}
