...
mod checked {
    ...
    use crate::{adapter::new_native_extensions, gas_meter::MySoGasMeter};
    ...
    use myso_types::execution::ExecutionResults;
    ...
    use myso_types::{error::command_argument_error, execution_status::CommandArgumentError};
    ...
    impl<'vm, 'state, 'a> ExecutionContext<'vm, 'state, 'a> {
        ...
        pub fn new(
            ...
            let mut linkage_view = LinkageView::new(Box::new(state_view.as_myso_resolver()));
    ...
    pub(crate) struct MySoDataStore<'state, 'a> {
        linkage_view: &'a LinkageView<'state>,
        new_packages: &'a [MovePackage],
    }

    impl<'state, 'a> MySoDataStore<'state, 'a> {
        pub(crate) fn new(
            linkage_view: &'a LinkageView<'state>,
            new_packages: &'a [MovePackage],
        ) -> Self {
    ...
    impl DataStore for MySoDataStore<'_, '_> {
