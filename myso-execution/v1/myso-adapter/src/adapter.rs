...
mod checked {
    ...
    use myso_move_natives::{object_runtime::ObjectRuntime, NativesCostTable};
    use myso_protocol_config::ProtocolConfig;
    use myso_types::{
        base_types::*,
        error::ExecutionError,
        error::{ExecutionErrorKind, MySoError},
        metrics::LimitsMetrics,
        storage::ChildObjectResolver,
    };
    use myso_verifier::verifier::myso_verify_module_metered_check_timeout_only;

    pub fn new_move_vm(
        natives: NativeFunctionTable,
        protocol_config: &ProtocolConfig,
    ) -> Result<MoveVM, MySoError> {
        ...
        .map_err(|_| MySoErrorKind::ExecutionInvariantViolation.into())
    }
            ...
            if let Err(e) = verify_module_with_config_metered(verifier_config, module, meter) {
                ...
                if check_for_verifier_timeout(&e.major_status()) {
                    ...
                    metrics
                        ...
                        .inc();
                    return Err(MySoErrorKind::ModuleVerificationFailure {
                        error: format!("Verification timedout: {}", e),
                    }
                    .into());
                };
            } else if let Err(err) =
                myso_verify_module_metered_check_timeout_only(module, &BTreeMap::new(), meter)
            {
                ...
                metrics
                    .verifier_timeout_metrics
                    .with_label_values(&[
                        BytecodeVerifierMetrics::MYSO_VERIFIER_TAG,
                        BytecodeVerifierMetrics::TIMEOUT_TAG,
                    ])
                    .inc();
                return Err(err.into());
