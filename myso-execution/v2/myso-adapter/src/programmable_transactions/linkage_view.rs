...
use myso_types::storage::{get_module, PackageObject};
use myso_types::{
    base_types::ObjectID,
...
pub struct LinkageView<'state> {
    ...
    resolver: Box<dyn MySoResolver + 'state>,
...
impl<'state> LinkageView<'state> {
    pub fn new(resolver: Box<dyn MySoResolver + 'state>) -> Self {
        Self {
            resolver,
            linkage_info: None,
    ...
    pub(crate) fn relocate(&self, module_id: &ModuleId) -> Result<ModuleId, MySoError> {
