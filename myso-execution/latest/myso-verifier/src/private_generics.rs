...
use myso_types::{MYSO_FRAMEWORK_ADDRESS, error::ExecutionError};
...
/// All transfer functions (the functions in `myso::transfer`) are "private" in that they are
...
pub fn verify_module(
    ...
    if *module.address() == MYSO_FRAMEWORK_ADDRESS
    ...
    // do not need to check the myso::transfer module itself
...
fn verify_function(
    ...
    for instr in &code.code {
        if let Bytecode::CallGeneric(finst_idx) = instr {
            ...
            if ident == (MYSO_FRAMEWORK_ADDRESS, TRANSFER_MODULE) {
                verify_private_transfer(view, fhandle, type_arguments, allow_receiving_object_id)?
            } else if ident == (MYSO_FRAMEWORK_ADDRESS, EVENT_MODULE) {
                verify_private_event_emit(view, fhandle, type_arguments)?
            } else if ident == (MYSO_FRAMEWORK_ADDRESS, COIN_REGISTRY_MODULE) {
...
fn verify_private_transfer(
    ...
    if addr_module(view, self_handle) == (MYSO_FRAMEWORK_ADDRESS, TRANSFER_MODULE) {
        return Ok(());
    }
    ...
    if !is_defined_in_current_module(view, type_arg) {
        return Err(format!(
            "Invalid call to '{myso}::transfer::{f}' on an object of type '{t}'. \
            ...
            instead, i.e. '{myso}::transfer::public_{f}'",
            myso = MYSO_FRAMEWORK_ADDRESS,
            f = fident,
...
fn verify_private_event_emit(
    ...
    if fident == EMIT_AUTHENTICATED_IMPL_FUNCTION {
        ...
        if (module_id.address(), module_id.name()) != (&MYSO_FRAMEWORK_ADDRESS, EVENT_MODULE) {
            debug_assert!(
                false,
