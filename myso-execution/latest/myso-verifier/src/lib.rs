...
fn verification_failure(error: String) -> ExecutionError {
    ExecutionError::new_with_source(ExecutionErrorKind::MySoMoveVerificationError, error)
}

fn to_verification_timeout_error(error: String) -> ExecutionError {
    ExecutionError::new_with_source(ExecutionErrorKind::MySoMoveVerificationTimedout, error)
}
