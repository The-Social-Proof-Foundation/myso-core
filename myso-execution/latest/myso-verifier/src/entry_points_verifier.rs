...
use myso_types::randomness_state::is_mutable_random;
use myso_types::{
...
fn verify_param_type(
    ...
    // Only `myso::myso_system` is allowed to expose entry functions that accept a mutable clock
    // parameter.
    ...
    // Only `myso::myso_system` is allowed to expose entry functions that accept a mutable Random
    // parameter.
