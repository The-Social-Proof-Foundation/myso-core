...
use myso_types::{
    MOVE_STDLIB_ADDRESS, MYSO_FRAMEWORK_ADDRESS, error::ExecutionError, make_invariant_violation,
};
...
// Event function
pub const MYSO_EVENT_EMIT_EVENT: FunctionIdent =
    (MYSO_FRAMEWORK_ADDRESS, EVENT_MODULE, ident_str!("emit"));
pub const MYSO_EVENT_EMIT_AUTHENTICATED: FunctionIdent = (
    MYSO_FRAMEWORK_ADDRESS,
    EVENT_MODULE,
    ident_str!("emit_authenticated"),
);
pub const MYSO_EVENT_NUM_EVENTS: FunctionIdent = (
    MYSO_FRAMEWORK_ADDRESS,
    EVENT_MODULE,
    ident_str!("num_events"),
);
pub const MYSO_EVENT_EVENTS_BY_TYPE: FunctionIdent = (
    MYSO_FRAMEWORK_ADDRESS,
    EVENT_MODULE,
...
pub const MYSO_TRANSFER_PUBLIC_TRANSFER: FunctionIdent = (
    MYSO_FRAMEWORK_ADDRESS,
    TRANSFER_MODULE,
    ident_str!("public_transfer"),
);
pub const MYSO_TRANSFER_PUBLIC_FREEZE_OBJECT: FunctionIdent = (
    MYSO_FRAMEWORK_ADDRESS,
    TRANSFER_MODULE,
    ident_str!("public_freeze_object"),
);
pub const MYSO_TRANSFER_PUBLIC_SHARE_OBJECT: FunctionIdent = (
    MYSO_FRAMEWORK_ADDRESS,
    TRANSFER_MODULE,
    ident_str!("public_share_object"),
);
pub const MYSO_TRANSFER_PUBLIC_RECEIVE: FunctionIdent = (
    MYSO_FRAMEWORK_ADDRESS,
    TRANSFER_MODULE,
    ident_str!("public_receive"),
);
pub const MYSO_TRANSFER_RECEIVING_OBJECT_ID: FunctionIdent = (
    MYSO_FRAMEWORK_ADDRESS,
    TRANSFER_MODULE,
    ident_str!("receiving_object_id"),
);
pub const MYSO_TRANSFER_PUBLIC_PARTY_TRANSFER: FunctionIdent = (
    MYSO_FRAMEWORK_ADDRESS,
    TRANSFER_MODULE,
...
pub const MYSO_TRANSFER_TRANSFER: FunctionIdent = (
    MYSO_FRAMEWORK_ADDRESS,
    TRANSFER_MODULE,
    ident_str!("transfer"),
);
pub const MYSO_TRANSFER_FREEZE_OBJECT: FunctionIdent = (
    MYSO_FRAMEWORK_ADDRESS,
    TRANSFER_MODULE,
    ident_str!("freeze_object"),
);
pub const MYSO_TRANSFER_SHARE_OBJECT: FunctionIdent = (
    MYSO_FRAMEWORK_ADDRESS,
    TRANSFER_MODULE,
    ident_str!("share_object"),
);
pub const MYSO_TRANSFER_RECEIVE: FunctionIdent = (
    MYSO_FRAMEWORK_ADDRESS,
    TRANSFER_MODULE,
    ident_str!("receive"),
);
pub const MYSO_TRANSFER_PARTY_TRANSFER: FunctionIdent = (
    MYSO_FRAMEWORK_ADDRESS,
    TRANSFER_MODULE,
...
pub const MYSO_COIN_REGISTRY_NEW_CURRENCY: FunctionIdent = (
    MYSO_FRAMEWORK_ADDRESS,
...
pub const EXHAUSTIVE_MODULES: &[(AccountAddress, &IdentStr)] = &[
    (MYSO_FRAMEWORK_ADDRESS, EVENT_MODULE),
    (MYSO_FRAMEWORK_ADDRESS, TRANSFER_MODULE),
];
...
pub const FUNCTIONS_TO_CHECK: &[(FunctionIdent, &[/* is internal */ bool])] = &[
    ...
    // event functions
    (MYSO_EVENT_EMIT_EVENT, &[true]),
    (MYSO_EVENT_EMIT_AUTHENTICATED, &[true]),
    (MYSO_EVENT_NUM_EVENTS, &[]),
    (MYSO_EVENT_EVENTS_BY_TYPE, &[false]),
    // public transfer functions
    (MYSO_TRANSFER_PUBLIC_TRANSFER, &[false]),
    (MYSO_TRANSFER_PUBLIC_FREEZE_OBJECT, &[false]),
    (MYSO_TRANSFER_PUBLIC_SHARE_OBJECT, &[false]),
    (MYSO_TRANSFER_PUBLIC_RECEIVE, &[false]),
    (MYSO_TRANSFER_RECEIVING_OBJECT_ID, &[false]),
    (MYSO_TRANSFER_PUBLIC_PARTY_TRANSFER, &[false]),
    // private transfer functions
    (MYSO_TRANSFER_TRANSFER, &[true]),
    (MYSO_TRANSFER_FREEZE_OBJECT, &[true]),
    (MYSO_TRANSFER_SHARE_OBJECT, &[true]),
    (MYSO_TRANSFER_RECEIVE, &[true]),
    (MYSO_TRANSFER_PARTY_TRANSFER, &[true]),
    // coin registry functions
    (MYSO_COIN_REGISTRY_NEW_CURRENCY, &[true]),
];
...
/// Several functions in the MySo Framework have `internal` type parameters, whose arguments must be
...
/// To avoid, issues, all `su::transfer` and `myso::event` functions must be configured in `INTERNAL_FUNCTIONS`.
pub fn verify_module(
    ...
    // Skip myso::test_scenario
    if module_address == MYSO_FRAMEWORK_ADDRESS && module_name.as_str() == TEST_SCENARIO_MODULE_NAME
    {
        // exclude test_module which is a test-only module in the MySo framework which "emulates"
...
pub fn callee_package_name(callee_addr: &AccountAddress) -> Cow<'static, str> {
    match *callee_addr {
        MYSO_FRAMEWORK_ADDRESS => Cow::Borrowed("myso"),
...
pub fn help_message(
    ...
    if *callee_addr == MYSO_FRAMEWORK_ADDRESS && callee_module == TRANSFER_MODULE {
        format!(
            " If the type has the 'store' ability, use the public variant instead: 'myso::transfer::public_{}'.",
            callee_function
        )
    } else {
        String::new()
