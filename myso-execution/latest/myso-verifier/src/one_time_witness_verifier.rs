...
use myso_types::bridge::BRIDGE_SUPPORTED_ASSET;
use myso_types::{
    BRIDGE_ADDRESS, MYSO_FRAMEWORK_ADDRESS,
...
pub fn verify_module(
    ...
    // In MySo's framework code there is an exception to the one-time witness type rule - we have a
    // MYSO type in the myso module but it is instantiated outside of the module initializer (in fact,
    // the module has no initializer). The reason for it is that the MYSO coin is only instantiated
    ...
    if ModuleId::new(MYSO_FRAMEWORK_ADDRESS, ident_str!("myso").to_owned()) == self_id {
        return Ok(());
    }
