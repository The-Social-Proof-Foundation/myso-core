...
const MYSO_DENY_LIST_CREATE: FunctionIdent = (
    MYSO_FRAMEWORK_ADDRESS,
    DENY_LIST_MODULE,
...
const MYSO_COIN_REGISTRY_CREATE: FunctionIdent = (
    MYSO_FRAMEWORK_ADDRESS,
    ident_str!("coin_registry"),
    ident_str!("create"),
);
const MYSO_ALIAS_CREATE: FunctionIdent = (
    MYSO_FRAMEWORK_ADDRESS,
    ...
    if handle.abilities.has_key() && last_value != AbstractValue::Fresh {
        ...
        let msg = format!(
            "Invalid object creation in {cur_package}::{cur_module}::{cur_function}. \
                ...
                The UID must come directly from `myso::{}::{}`, or `myso::{}::{}`. \
                For tests, it can also come from `myso::{}::{}`",
            OBJECT_NEW.1,
            OBJECT_NEW.2,
