...
pub fn record_settlement_myso_conservation(
    context: &mut NativeContext,
    ty_args: Vec<Type>,
    mut args: VecDeque<Value>,
) -> PartialVMResult<NativeResult> {
    ...
    let output_myso = args.pop_back().unwrap().value_as::<u64>().unwrap();
    let input_myso = args.pop_back().unwrap().value_as::<u64>().unwrap();

    let obj_runtime: &mut ObjectRuntime = context.extensions_mut().get_mut()?;

    obj_runtime.record_settlement_myso_conservation(input_myso, output_myso);
