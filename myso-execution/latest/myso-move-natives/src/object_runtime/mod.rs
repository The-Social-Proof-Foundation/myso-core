...
use myso_types::{
    ...
    MYSO_AUTHENTICATOR_STATE_OBJECT_ID, MYSO_BOOTSTRAP_KEY_OBJECT_ID, MYSO_BRIDGE_OBJECT_ID,
    MYSO_CLOCK_OBJECT_ID,
    MYSO_COIN_REGISTRY_OBJECT_ID, MYSO_DENY_LIST_OBJECT_ID, MYSO_DISPLAY_REGISTRY_OBJECT_ID,
    MYSO_ORDERBOOK_REGISTRY_OBJECT_ID, MYSO_RANDOMNESS_STATE_OBJECT_ID,
    MYSO_SYSTEM_STATE_OBJECT_ID, TypeTag,
...
pub(crate) struct TestInventories {
    ...
    pub(crate) address_inventories: BTreeMap<MySoAddress, BTreeMap<MoveObjectType, Set<ObjectID>>>,
...
pub struct RuntimeResults {
    ...
    pub settlement_input_myso: u64,
    pub settlement_output_myso: u64,
}
...
pub(crate) struct ObjectRuntimeState {
    ...
    // Used to track MYSO conservation in settlement transactions. Settlement transactions
    // gather up withdraws and deposits from other transactions, and record them to accumulator
    // fields. The settlement transaction records the total amount of MYSO being disbursed here,
    // so that we can verify that the amount stored in the fields at the end of the transaction
    // is correct.
    settlement_input_myso: u64,
    settlement_output_myso: u64,
...
impl<'a> ObjectRuntime<'a> {
    pub fn new(
        ...
        Self {
            ...
            state: ObjectRuntimeState {
                ...
                settlement_input_myso: 0,
                settlement_output_myso: 0,
    ...
    pub fn transfer(
        ...
        let is_framework_obj = [
            ...
            MYSO_DISPLAY_REGISTRY_OBJECT_ID,
            MYSO_ADDRESS_ALIAS_STATE_OBJECT_ID,
        ]
        .contains(&id);
    ...
    pub fn record_settlement_myso_conservation(&mut self, input_myso: u64, output_myso: u64) {
        self.state.settlement_input_myso += input_myso;
        self.state.settlement_output_myso += output_myso;
    }
...
impl ObjectRuntimeState {
    ...
    pub(crate) fn finish(
        ...
        let ObjectRuntimeState {
            ...
            received,
            accumulator_events,
            settlement_input_myso,
            settlement_output_myso,
