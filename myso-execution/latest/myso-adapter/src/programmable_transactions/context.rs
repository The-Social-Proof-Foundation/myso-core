...
mod checked {
    use crate::{
        ...
        data_store::{
            ...
            legacy::{linkage_view::LinkageView, myso_data_store::MySoDataStore},
        },
    ...
    pub struct ExecutionContext<'vm, 'state, 'a> {
        ...
        additional_transfers: Vec<(/* new owner */ MySoAddress, ObjectValue)>,
    ...
    impl<'vm, 'state, 'a> ExecutionContext<'vm, 'state, 'a> {
        ...
        pub(crate) fn execute_function_bypass_visibility(
            ...
            let mut data_store = MySoDataStore::new(&self.linkage_view, &self.new_packages);
        ...
        let accumulator_events = accumulator_events
            ...
            .map(|accum_event| {
                ...
                let write = AccumulatorWriteV1 {
                    address,
                    operation: accum_event.action.into_myso_accumulator_action(),
                    value,
                };
    ...
    pub fn load_type_from_struct(
        ...
        let data_store = MySoDataStore::new(linkage_view, new_packages);
        ...
        Ok(match call_arg {
            ...
            CallArg::FundsWithdrawal(FundsWithdrawalArg {
                ...
                let limit = match reservation {
                    myso_types::transaction::Reservation::MaxAmountU64(u) => U256::from(u),
                };
    ...
    fn convert_vm_error<S: MoveResolver<Err = MySoError>>(
        error: VMError,
        vm: &MoveVM,
        state_view: &S,
    ...
    /// Special case errors for type arguments to Move functions
    fn convert_type_argument_error<S: MoveResolver<Err = MySoError>>(
        idx: usize,
        error: VMError,
        vm: &MoveVM,
        state_view: &S,
        ...
        use myso_types::execution_status::TypeArgumentError;
