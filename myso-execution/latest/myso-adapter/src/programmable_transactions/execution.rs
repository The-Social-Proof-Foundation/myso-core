...
mod checked {
    use crate::{
        ...
        data_store::{PackageStore, legacy::myso_data_store::MySoDataStore},
    ...
    fn publish_and_verify_modules(
        ...
        // TODO(https://github.com/the-social-proof-foundation/myso-core/issues/69): avoid this redundant serialization by exposing VM API that allows us to run the linker directly on `Vec<CompiledModule>`
        ...
        // run the MySo verifier
        for module in modules {
            // Run MySo bytecode verifier, which runs some additional checks that assume the Move
            // bytecode verifier has passed.
            myso_verifier::verifier::myso_verify_module_unmetered(
                module,
                &BTreeMap::new(),
                &context
                    .protocol_config
    ...
    fn check_visibility_and_signature<Mode: ExecutionMode>(
        ...
        let data_store = MySoDataStore::new(&context.linkage_view, &no_new_packages);
        let module = context
            .vm
            .get_runtime()
    ...
    pub fn check_private_generics(
        ...
        if module_ident == (&MYSO_FRAMEWORK_ADDRESS, EVENT_MODULE) {
            return Err(ExecutionError::new_with_source(
                ExecutionErrorKind::NonEntryFunctionInvoked,
                format!("Cannot directly call functions in myso::{}", EVENT_MODULE),
            ));
        }

        if module_ident == (&MYSO_FRAMEWORK_ADDRESS, TRANSFER_MODULE)
            && PRIVATE_TRANSFER_FUNCTIONS.contains(&function)
        {
            let msg = format!(
                "Cannot directly call myso::{m}::{f}. \
                Use the public variant instead, myso::{m}::public_{f}",
                m = TRANSFER_MODULE,
                f = function
            );
            return Err(ExecutionError::new_with_source(
        ...
        match value {
            ...
            Value::Receiving(_, _, assigned_type) => {
                ...
                let Some(s) = context.vm.get_runtime().get_type(*sidx) else {
                    invariant_violation!("myso::transfer::Receiving struct not found in session")
                };
    ...
    pub fn is_tx_context(
        ...
        let is_tx_context_type = module_addr == &MYSO_FRAMEWORK_ADDRESS
        ...
        fn deserialize<D: serde::de::Deserializer<'d>>(
            ...
            match self {
                ...
                PrimitiveArgumentLayout::Address => {
                    MySoAddress::deserialize(deserializer)?;
                    Ok(())
                }
            }
        }
