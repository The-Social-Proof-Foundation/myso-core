...
/// `myso::coin::Coin<T>` and swaps references to that input to the conversion result.
...
/// Returns the inner type `T` if the type is `myso::coin::Coin<T>`, else `None`
...
/// Returns the inner type `T` if the type is `myso::balance::Balance<T>`, else `None`
...
/// Returns the inner type `T` if the type is `myso::funds_accumulator::Withdrawal<T>`, else `None`
