...
macro_rules! charge_gas_ {
    ($gas_charger:expr, $env:expr, $call:ident($($args:expr),*)) => {{
        MySoGasMeter($gas_charger.move_gas_status_mut())
            .$call($($args),*)
...
impl<'env, 'pc, 'vm, 'state, 'linkage, 'gas> Context<'env, 'pc, 'vm, 'state, 'linkage, 'gas> {
    ...
    fn publish_and_verify_modules(
        ...
        self.env
            ...
            .publish_module_bundle(
                ...
                &mut data_store,
                &mut MySoGasMeter(self.gas_charger.move_gas_status_mut()),
            )
            .map_err(|e| self.env.convert_linked_vm_error(e, linkage))?;
    ...
    pub fn argument_updates(
        &mut self,
        args: Vec<T::Argument>,
    ) -> Result<Vec<(myso_types::transaction::Argument, Vec<u8>, TypeTag)>, ExecutionError> {
        args.into_iter()
    ...
    ) -> Result<Option<(myso_types::transaction::Argument, Vec<u8>, TypeTag)>, ExecutionError> {
        use myso_types::transaction::Argument as TxArgument;
        let ty = match ty {
...
fn load_object_arg_impl(
    ...
    let myso_types::object::ObjectInner {
        data: myso_types::object::Data::Move(move_obj),
        ..
    } = obj.as_inner()
    else {
