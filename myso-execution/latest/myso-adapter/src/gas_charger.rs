...
pub mod checked {
    ...
    use myso_types::deny_list_v2::CONFIG_SETTING_DYNAMIC_FIELD_SIZE_FOR_GAS;
    use myso_types::gas::{GasCostSummary, MySoGasStatus, deduct_gas};
    use myso_types::gas_model::gas_predicates::{
    ...
    impl GasCharger {
        ...
        pub fn new_unmetered(tx_digest: TransactionDigest) -> Self {
            Self {
                ...
                smashed_gas_coin: None,
                gas_status: MySoGasStatus::new_unmetered(),
            }
        }
        ...
        pub fn into_gas_status(self) -> MySoGasStatus {
            self.gas_status
        }
        ...
        pub fn charge_input_objects(
            ...
            let total_size = temporary_store
                .objects()
                .iter()
                // don't charge for loading MySo Framework or Move stdlib
        ...
        pub fn charge_gas<T>(
            ...
            if self.smashed_gas_coin.is_some() || self.payment_method.is_address_balance() {
                ...
                let is_move_abort = execution_result
                    ...
                    .map(|err| {
                        matches!(
                            err.kind(),
                            myso_types::execution_status::ExecutionFailureStatus::MoveAbort(_, _)
                        )
                    })
                    .unwrap_or(false);
                // bucketize computation cost
            ...
            if execution_result
                ...
                .map(|err| {
                    matches!(
                        err.kind(),
                        myso_types::execution_status::ExecutionFailureStatus::InsufficientFundsForWithdraw
                    )
                })
                .unwrap_or(false)
            ...
            match self.payment_method {
                PaymentMethod::AddressBalance(payer_address) => {
                    if net_change != 0 {
                        let balance_type = myso_types::balance::Balance::type_tag(
