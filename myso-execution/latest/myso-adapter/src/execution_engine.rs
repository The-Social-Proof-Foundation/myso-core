...
mod checked {
    ...
    use crate::execution_value::MySoResolver;
    ...
    use myso_types::bridge::BRIDGE_COMMITTEE_MINIMAL_VOTING_POWER;
    use myso_types::bridge::{
        BRIDGE_CREATE_FUNCTION_NAME, BRIDGE_INIT_COMMITTEE_FUNCTION_NAME, BRIDGE_MODULE_NAME,
        BridgeChainId,
    };
    use myso_types::clock::{CLOCK_MODULE_NAME, CONSENSUS_COMMIT_PROLOGUE_FUNCTION_NAME};
    ...
    use myso_types::error::{ExecutionError, ExecutionErrorKind};
    use myso_types::execution::{ExecutionTiming, ResultWithTimings};
    ...
    use myso_types::transaction::{CheckedInputObjects, RandomnessStateUpdate};
    use myso_types::{
    ...
    pub fn execute_transaction_to_effects<Mode: ExecutionMode>(
        ...
        let status = if let Err(error) = &execution_result {
            ...
            match error.kind() {
                ...
                K::MySoMoveVerificationError | K::VMVerificationOrDeserializationError => {
                    #[skip_checked_arithmetic]
                    tracing::debug!(
                        kind = ?error.kind(),
                        tx_digest = ?transaction_digest,
    ...
    fn execute_transaction<Mode: ExecutionMode>(
        ...
        // information provided to check_myso_conserved, because we mint rewards, and burn
    ...
    fn run_conservation_checks<Mode: ExecutionMode>(
        ...
        let mut result: std::result::Result<(), myso_types::error::ExecutionError> = Ok(());
        if !is_genesis_tx && !Mode::skip_conservation_checks() {
            // ensure that this transaction did not create or destroy MYSO, try to recover if the check fails
            let conservation_result = {
                temporary_store
                    .check_myso_conserved(simple_conservation_checks, cost_summary)
                    .and_then(|()| {
                        if enable_expensive_checks {
                            // ensure that this transaction did not create or destroy MYSO, try to recover if the check fails
                            let mut layout_resolver =
                                TypeLayoutResolver::new(move_vm, Box::new(&*temporary_store));
                            temporary_store.check_myso_conserved_expensive(
                                cost_summary,
                                advance_epoch_gas_summary,
                                &mut layout_resolver,
                            )
            ...
            if let Err(conservation_err) = conservation_result {
                ...
                if let Err(recovery_err) = {
                    ...
                    // we will create or destroy MYSO otherwise
                    panic!(
                        "MYSO conservation fail in tx block {}: {}\nGas status is {}\nTx was ",
                        tx_digest,
                        recovery_err,
                        gas_charger.summary()
                    )
        ...
        } // else, we're in the genesis transaction which mints the MYSO supply, and hence does not satisfy MYSO conservation, or
            ...
            TransactionKind::Genesis(GenesisTransaction { objects }) => {
                ...
                for genesis_object in objects {
                    match genesis_object {
                        myso_types::transaction::GenesisObject::RawObject { data, owner } => {
    ...
    fn setup_bridge_create(
        ...
        let bridge_chain_id = if chain_id == get_mainnet_chain_identifier() {
            BridgeChainId::MySoMainnet as u8
        } else if chain_id == get_testnet_chain_identifier() {
            BridgeChainId::MySoTestnet as u8
        } else {
    ...
    fn setup_bridge_committee_update(
        ...
        let bridge = builder
            .obj(ObjectArg::SharedObject {
                ...
                mutability: myso_types::transaction::SharedObjectMutability::Mutable,
            })
            .expect("Unable to create Bridge object arg!");
        let system_state = builder
            .obj(ObjectArg::MYSO_SYSTEM_MUT)
