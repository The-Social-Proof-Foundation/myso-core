...
use myso_types::deny_list_v2::check_coin_deny_list_v2_during_execution;
use myso_types::effects::{
    AccumulatorOperation, AccumulatorValue, AccumulatorWriteV1, TransactionEffects,
    TransactionEvents,
};
use myso_types::error::ExecutionErrorKind;
use myso_types::execution::{
...
impl<'backing> TemporaryStore<'backing> {
    ...
    pub fn conserve_unmetered_storage_rebate(&mut self, unmetered_storage_rebate: u64) {
        ...
        let mut system_state_wrapper = self
            .read_object(&MYSO_SYSTEM_STATE_OBJECT_ID)
        ...
        // If not, we would be dropping MYSO on the floor by overriding it.
                    ...
                    Owner::Immutable => {
                        assert!(
                            ...
                            "Immutable objects cannot be written, except for \
                             MySo Framework/Move stdlib upgrades at epoch change boundaries"
                        );
...
impl TemporaryStore<'_> {
    ...
    /// `MySoGasStatus` `storage_rebate` and `storage_gas_units` track the transaction
...
impl TemporaryStore<'_> {
    pub fn advance_epoch_safe_mode(
        ...
        let wrapper = get_myso_system_state_wrapper(self.store.as_object_store())
...
impl TemporaryStore<'_> {
    fn get_input_myso(
        ...
        if let Some(obj) = self.input_objects.get(id) {
            ...
            obj.get_total_myso(layout_resolver).map_err(|e| {
                make_invariant_violation!(
                    "Failed looking up input MYSO in MYSO conservation checking for input with \
                         type {:?}: {e:#?}",
                    obj.struct_tag(),
                )
            })
            ...
            let Some(obj) = self.store.get_object_by_key(id, expected_version) else {
                invariant_violation!(
                    "Failed looking up dynamic field {id} in MYSO conservation checking"
                );
            };
            obj.get_total_myso(layout_resolver).map_err(|e| {
                make_invariant_violation!(
                    "Failed looking up input MYSO in MYSO conservation checking for type \
                         {:?}: {e:#?}",
                    obj.struct_tag(),
                )
            })
    ...
    /// Check that this transaction neither creates nor destroys MYSO. This should hold for all txes
    ...
    ///
    /// 1. all MYSO in storage rebate fields of input objects should flow either to the transaction
    ///    storage rebate, or the transaction non-refundable storage rebate
    /// 2. all MYSO charged for storage should flow into the storage rebate field of some output
    ///    object
    ///
    ...
    pub fn check_myso_conserved(
        ...
        // total amount of MYSO in storage rebate of input objects
        let mut total_input_rebate = 0;
        // total amount of MYSO in storage rebate of output objects
        ...
        if gas_summary.storage_cost == 0 {
                    ...
                    + gas_summary.non_refundable_storage_fee
            {
                return Err(ExecutionError::invariant_violation(format!(
                    "MYSO conservation failed -- no storage charges in gas summary \
            ...
            // all MYSO in storage rebate fields of input objects should flow either to
            ...
            {
                return Err(ExecutionError::invariant_violation(format!(
                    "MYSO conservation failed -- {} MYSO in storage rebate field of input objects, \
                        {} MYSO in tx storage rebate or tx non-refundable storage rebate",
                    total_input_rebate, gas_summary.non_refundable_storage_fee,
                )));
            }

            // all MYSO charged for storage should flow into the storage rebate field
            // of some output object
            if gas_summary.storage_cost != total_output_rebate {
                return Err(ExecutionError::invariant_violation(format!(
                    "MYSO conservation failed -- {} MYSO charged for storage, \
                        {} MYSO in storage rebate field of output objects",
    ...
    /// Check that this transaction neither creates nor destroys MYSO.
    /// This more expensive check will check a third invariant on top of the 2 performed
    /// by `check_myso_conserved` above:
    ///
    /// * all MYSO in input objects (including coins etc in the Move part of an object) should flow
    ...
    /// advance epoch transaction would mint `epoch_fees` amount of MYSO, and burn `epoch_rebates`
    /// amount of MYSO. We need these information for this check.
    pub fn check_myso_conserved_expensive(
        &self,
        gas_summary: &GasCostSummary,
        advance_epoch_gas_summary: Option<(u64, u64)>,
        layout_resolver: &mut impl LayoutResolver,
    ) -> Result<(), ExecutionError> {
        // total amount of MYSO in input objects, including both coins and storage rebates
        let mut total_input_myso = 0;
        // total amount of MYSO in output objects, including both coins and storage rebates
        let mut total_output_myso = 0;

        // settlement input/output myso is used by the settlement transactions to account for
        // MySo that has been gathered from the accumulator writes of transactions which it is
        // settling.
        total_input_myso += self.execution_results.settlement_input_myso;
        total_output_myso += self.execution_results.settlement_output_myso;

        for (id, input, output) in self.get_modified_objects() {
            if let Some(input) = input {
                total_input_myso += self.get_input_myso(&id, input.version, layout_resolver)?;
            }
            if let Some(object) = output {
                total_output_myso += object.get_total_myso(layout_resolver).map_err(|e| {
                    make_invariant_violation!(
                        "Failed looking up output MYSO in MYSO conservation checking for \
                         mutated type {:?}: {e:#?}",
                        object.struct_tag(),
                    )
                })?;
        ...
        for event in &self.execution_results.accumulator_events {
            let (input, output) = event.total_myso_in_event();
            total_input_myso += input;
            total_output_myso += output;
        }
        ...
        total_output_myso += gas_summary.computation_cost + gas_summary.non_refundable_storage_fee;
        if let Some((epoch_fees, epoch_rebates)) = advance_epoch_gas_summary {
            total_input_myso += epoch_fees;
            total_output_myso += epoch_rebates;
        }
        if total_input_myso != total_output_myso {
            return Err(ExecutionError::invariant_violation(format!(
                "MYSO conservation failed: input={}, output={}, \
                    this transaction either mints or burns MYSO",
                total_input_myso, total_output_myso,
            )));
        }
        Ok(())
    }
...
impl Storage for TemporaryStore<'_> {
    ...
    fn record_execution_results(
        ...
        let ExecutionResults::V2(mut results) = results else {
            panic!("ExecutionResults::V2 expected in myso-execution v1 and above");
        };
    ...
    fn check_coin_deny_list(
        &self,
        receiving_funds_type_and_owners: BTreeMap<TypeTag, BTreeSet<MySoAddress>>,
    ) -> DenyListResult {
        ...
        if result.num_non_gas_coin_owners > 0
            && !self.input_objects.contains_key(&MYSO_DENY_LIST_OBJECT_ID)
        {
            self.loaded_per_epoch_config_objects
                .write()
                .insert(MYSO_DENY_LIST_OBJECT_ID);
        }
        result
    }
