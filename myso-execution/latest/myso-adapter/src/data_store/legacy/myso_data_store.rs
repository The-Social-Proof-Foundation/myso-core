...
impl PackageStore for MySoDataStore<'_, '_> {
    fn get_package(&self, id: &ObjectID) -> MySoResult<Option<Rc<MovePackage>>> {
        for package in self.new_packages {
            if package.id() == *id {
    ...
    fn resolve_type_to_defining_id(
        ...
        unimplemented!(
            "resolve_type_to_defining_id is not implemented for legacy::MySoDataStore and should never be called"
        )
    }
}
