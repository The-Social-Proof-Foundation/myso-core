...
The `myso-execution` crate is responsible for abstracting access to the
...
- The adapter, that integrates Move into MySo.
...
- `./myso-execution`, (latest version and cuts/copies of myso-specific
  crates).
...
code must be via `myso-execution`, and not by directly depending a
constituent crate.
...
and elsewhere, **default to accessing execution via `myso-execution`.**
...
`myso-execution tests::test_encapsulation` is a test that detects
...
`./crates/myso-types` are shared across all versions.  Changes to
...
## `myso-execution/src/lib.rs`

The entry-point to the execution crate -- `myso-execution/src/lib.rs`
...
- `myso-execution/src/lib.template.rs` -- a template file with
