...
use myso_types::{
    ...
    MYSO_CLOCK_OBJECT_ID, MYSO_SYSTEM_STATE_OBJECT_ID,
};
...
pub(crate) struct TestInventories {
    ...
    pub(crate) address_inventories: BTreeMap<MySoAddress, BTreeMap<Type, Set<ObjectID>>>,
...
impl<'a> ObjectRuntime<'a> {
    ...
    pub fn transfer(
        ...
        //   child object
        let is_framework_obj = [MYSO_SYSTEM_STATE_OBJECT_ID, MYSO_CLOCK_OBJECT_ID].contains(&id);
            ...
            .map(|(id, (owner, type_, value))| {
                let write_kind =
                    if input_objects.contains_key(&id) || loaded_child_objects.contains_key(&id) {
                        ...
                        WriteKind::Mutate
                    } else if id == MYSO_SYSTEM_STATE_OBJECT_ID || new_ids.contains_key(&id) {
                        // MYSO_SYSTEM_STATE_OBJECT_ID is only transferred during genesis
