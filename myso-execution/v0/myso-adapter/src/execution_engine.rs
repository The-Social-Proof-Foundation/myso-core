...
mod checked {
    ...
    use myso_types::{MYSO_FRAMEWORK_PACKAGE_ID, MYSO_SYSTEM_PACKAGE_ID};
    ...
    fn execute_transaction<Mode: ExecutionMode>(
        ...
        // === begin MYSO conservation checks ===
        if !is_genesis_tx && !Mode::allow_arbitrary_values() {
            ...
            let conservation_result = {
                ...
                temporary_store.check_myso_conserved(
                    &cost_summary,
            ...
            if let Err(conservation_err) = conservation_result {
                ...
                let mut layout_resolver =
                    TypeLayoutResolver::new(move_vm, Box::new(&*temporary_store));
                if let Err(recovery_err) = temporary_store.check_myso_conserved(
                    &cost_summary,
                    advance_epoch_gas_summary,
                    &mut layout_resolver,
                    enable_expensive_checks,
