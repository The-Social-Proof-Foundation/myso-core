...
use myso_types::execution::{DynamicallyLoadedObjectMetadata, ExecutionResults, SharedInput};
...
use myso_types::storage::{BackingStore, DeleteKindWithOldVersion, DenyListResult, PackageObject};
...
impl TemporaryStore<'_> {
    ...
    ///
    /// 3. all MYSO in input objects (including coins etc in the Move part of an object) should flow
    ...
    /// amount of MYSO. We need these information for conservation check.
    pub fn check_myso_conserved(
        ...
        for (id, input, output) in self.get_modified_objects() {
            if let Some((version, storage_rebate)) = input {
                ...
                if do_expensive_checks {
                    total_input_myso += self.get_input_myso(&id, version, layout_resolver)?;
                }
            }
        ...
        if do_expensive_checks {
            ...
            if total_input_myso != total_output_myso {
                ...
                format!("MYSO conservation failed: input={}, output={}, this transaction either mints or burns MYSO",
                total_input_myso,
                total_output_myso))
            );
            }
        ...
        // all MYSO in storage rebate fields of input objects should flow either to the transaction storage rebate, or the non-refundable
        // storage rebate pool
        if total_input_rebate != gas_summary.storage_rebate + gas_summary.non_refundable_storage_fee
        {
            ...
            /*return Err(ExecutionError::invariant_violation(
                format!("MYSO conservation failed--{} MYSO in storage rebate field of input objects, {} MYSO in tx storage rebate or tx non-refundable storage rebate",
                total_input_rebate,
                gas_summary.non_refundable_storage_fee))
            );*/
        }

        // all MYSO charged for storage should flow into the storage rebate field of some output object
        if gas_summary.storage_cost != total_output_rebate {
            // TODO: re-enable once we fix the edge case with OOG, gas smashing, and storage rebate
            /*return Err(ExecutionError::invariant_violation(
                format!("MYSO conservation failed--{} MYSO charged for storage, {} MYSO in storage rebate field of output objects",
...
impl Storage for TemporaryStore<'_> {
    ...
    fn record_execution_results(
        ...
        let ExecutionResults::V1(results) = results else {
            panic!("ExecutionResults::V1 expected in myso-execution v0");
        };
    ...
    fn check_coin_deny_list(
        ...
        unreachable!("Coin denylist v2 is not supported in myso-execution v0");
    }

    fn record_generated_object_ids(&mut self, _generated_ids: BTreeSet<ObjectID>) {
        unreachable!(
            "Generated object IDs are not recorded in ExecutionResults in myso-execution v0"
        );
    }
}
