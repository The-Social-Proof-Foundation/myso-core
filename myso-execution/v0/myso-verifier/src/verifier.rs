...
pub fn myso_verify_module_metered_check_timeout_only(
    ...
    if let Err(error) = myso_verify_module_metered(config, module, fn_info_map, meter) {
        if matches!(
            error.kind(),
...
pub fn myso_verify_module_unmetered(
    ...
    myso_verify_module_metered(config, module, fn_info_map, &mut DummyMeter).inspect_err(|err| {
