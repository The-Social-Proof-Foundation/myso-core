...
//! Objects whose struct type has key ability represent MySo objects.
//! They have unique IDs that should never be reused. This verifier makes
//! sure that the id field of MySo objects never get leaked.
//! Unpack is the only bytecode that could extract the id field out of
//! a MySo object. From there, we track the flow of the value and make
...
const MYSO_SYSTEM_CREATE: FunctionIdent = (
    &MYSO_SYSTEM_ADDRESS,
    MYSO_SYSTEM_MODULE_NAME,
    ident_str!("create"),
);
const MYSO_CLOCK_CREATE: FunctionIdent = (
    &MYSO_FRAMEWORK_ADDRESS,
    CLOCK_MODULE_NAME,
    ident_str!("create"),
);
...
#[cfg(msim)]
const MYSO_AUTHENTICATOR_STATE_CREATE: FunctionIdent = (
    &MYSO_FRAMEWORK_ADDRESS,
    AUTHENTICATOR_STATE_MODULE_NAME,
    ident_str!("create"),
);
#[cfg(msim)]
const MYSO_RANDOMNESS_STATE_CREATE: FunctionIdent = (
    &MYSO_FRAMEWORK_ADDRESS,
    RANDOMNESS_MODULE_NAME,
    ident_str!("create"),
);
#[cfg(msim)]
const MYSO_DENY_LIST_OBJECT_CREATE: FunctionIdent = (
    &MYSO_FRAMEWORK_ADDRESS,
    COIN_MODULE_NAME,
...
#[cfg(not(msim))]
const FUNCTIONS_TO_SKIP: &[FunctionIdent] = &[MYSO_SYSTEM_CREATE, MYSO_CLOCK_CREATE];

#[cfg(msim)]
const FUNCTIONS_TO_SKIP: &[FunctionIdent] = &[
    MYSO_SYSTEM_CREATE,
    MYSO_CLOCK_CREATE,
    MYSO_AUTHENTICATOR_STATE_CREATE,
    MYSO_RANDOMNESS_STATE_CREATE,
    MYSO_DENY_LIST_OBJECT_CREATE,
];
    ...
    if handle.abilities.has_key() && last_value != AbstractValue::Fresh {
        ...
        let msg = format!(
            "Invalid object creation in {cur_package}::{cur_module}::{cur_function}. \
                ...
                The UID must come directly from myso::{}::{}. \
                Or for tests, it can come from myso::{}::{}",
