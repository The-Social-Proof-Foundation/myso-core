...
"""
Script to kick off Antithesis tests via the myso-operations repo.

Requires myso-operations repo to be locally checked out. If MYSO_OPS_REPO
environment variable is not set, it defaults to ~/dev/myso-operations.
"""
...
def main():
    parser = argparse.ArgumentParser(
        description="Run Antithesis tests via myso-operations workflow",
    ...
    parser.add_argument(
        "-l", "--log-level",
        type=str,
        help="Logging filter for myso-node (no spaces allowed)",
    )
    parser.add_argument(
        "-T", "--tidehunter-commit",
        type=str,
    ...
    parser.add_argument(
        "-S", "--stress-commit",
        type=str,
        help="MySo repo sha for stress image",
    )
    parser.add_argument(
        "-p", "--protocol-override",
        type=str,
    ...
    parser.add_argument(
        "-C", "--myso-commit",
        type=str,
        help="MySo repo sha (default: current HEAD)",
    )
    parser.add_argument(
        "-r", "--workflow-ref",
        type=str,
        help="Branch/ref in myso-operations repo to run workflow from",
    )
    parser.add_argument(
        "-n", "--dry-run",
        action="store_true",
    ...
    # Determine myso-operations repo path
    myso_ops_repo = os.environ.get("MYSO_OPS_REPO", os.path.expanduser("~/dev/myso-operations"))

    if not os.path.isdir(myso_ops_repo):
        print(
            f"myso-operations repo directory {myso_ops_repo} does not exist. "
            "Please make sure you have correctly set the myso-operations repo directory.",
            file=sys.stderr,
        )
        return 1
    ...
    # Determine myso_commit
    myso_commit = args.myso_commit
    if not myso_commit:
        myso_commit = get_git_output(["rev-parse", "HEAD"])
    ...
    if not args.skip_validation:
        valid = True
        if not validate_commit_on_remote(commit, "MystenLabs/myso", "commit"):
            valid = False
        if alt_commit and not validate_commit_on_remote(alt_commit, "MystenLabs/myso", "alt_commit"):
            valid = False
        if args.stress_commit and not validate_commit_on_remote(args.stress_commit, "MystenLabs/myso", "stress_commit"):
            valid = False
        if args.cli_commit and not validate_commit_on_remote(args.cli_commit, "MystenLabs/myso", "cli_commit"):
            valid = False
        if args.workflow_ref and not validate_ref_on_remote(args.workflow_ref, myso_ops_repo):
            valid = False
        if not valid:
            return 1
    ...
    if args.cli_commit:
        cmd.extend(["-f", f"myso_cli_commit={args.cli_commit}"])
    ...
    result = subprocess.run(cmd, cwd=myso_ops_repo)
    if result.returncode != 0:
