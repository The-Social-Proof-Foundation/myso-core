...
echo "myso-test-validator binary has been deprecated in favor of myso start, which is a more powerful command that allows you to start the local network with more options.
This script offers backward compatibility, but ideally, you should migrate to myso start instead. Use myso start --help to see all the flags and options. 

To recreate the exact basic functionality of myso-test-validator, you must use the following options:
...
# Basic command that replicates the command line arguments of myso-test-validator
cmd="myso start --with-faucet --force-regenesis"
