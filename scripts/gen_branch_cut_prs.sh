...
#   cargo run --bin myso-framework-snapshot
...
MYSO_VERSION=$(sed -nE 's/^version = "([0-9]+\.[0-9]+\.[0-9]+)"/\1/p' ./Cargo.toml)
STAMP="$(date +%Y%m%d%H%M%S)"

if [[ "$PR_TYPE" == *snapshot* ]]; then
  echo "Generating framework bytecode snapshot..."
  # Set up branch for changes.
  BRANCH="${GITHUB_ACTOR}/myso-v${MYSO_VERSION}-bytecode-framework-snapshot-${STAMP}"
  ...
  # Generate PR body
  BODY="MySo v${MYSO_VERSION} Framework Bytecode snapshot"
  ...
  echo "Pull request for MySo v${MYSO_VERSION} Framework Bytecode snapshot created: $PR_URL"

elif [[ "$PR_TYPE" == *version-bump* ]]; then
  # Generate the version bump PR
  echo "Generating version bump..."
  # Bump main branhch version
  IFS=. read -r major minor patch <<<"$MYSO_VERSION"; NEW_MYSO_VERSION="$major.$((minor+1)).$patch"

  # Setup new branch for staging
  BRANCH="${GITHUB_ACTOR}/myso-v${NEW_MYSO_VERSION}-version-bump-${STAMP}"
  git checkout -b "$BRANCH"

  # Update the version in Cargo.toml and openrpc.json
  sed -i -E "s/^(version = \")[0-9]+\.[0-9]+\.[0-9]+(\"$)/\1${NEW_MYSO_VERSION}\2/" Cargo.toml
  sed -i -E "s/(\"version\": \")([0-9]+\.[0-9]+\.[0-9]+)(\")/\1${NEW_MYSO_VERSION}\3/" crates/myso-open-rpc/spec/openrpc.json
  ...
  # Generate PR body
  BODY="MySo v${NEW_MYSO_VERSION} Version Bump"
  ...
  echo "Pull request for MySo v${NEW_MYSO_VERSION} Version Bump created: $PR_URL"
else
