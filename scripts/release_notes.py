...
PROTOCOL_CONFIG_PATH = "crates/myso-protocol-config/src/lib.rs"
...
def extract_notes_for_pr(pr):
    ...
    variables = {"owner": "MystenLabs", "name": "myso", "number": int(pr)}
...
def fetch_release_notes_for_commits(commits):
    ...
    for start in range(0, len(commits), BATCH_SIZE):
        ...
        variables = {"owner": "MystenLabs", "name": "myso"}
...
def extract_protocol_version(commit):
    ...
    Assumes that it is being called from the root of the myso repository."""
    for line in git(
        "show", f"{commit}:crates/myso-protocol-config/src/lib.rs"
    ).splitlines():
...
def pr_bumps_protocol_version(pr):
    ...
    try:
        diff = subprocess.check_output(
            [GH_CLI_PATH, "pr", "diff", str(pr), "--repo", "MystenLabs/myso"],
            text=True,
...
def do_list_prs(from_, to):
    ...
    mysotable for use in CI workflows.
...
def do_generate(from_, to):
    ...
    for impact_area in NOTE_ORDER:
        ...
        if impact_area == "Protocol":
            print(f"#### MySo Protocol Version in this release: `{protocol_version}`")
