...
def read_sparse_config(sparse_file=".sparse"):
    ...
    if os.path.isfile(sparse_file):
        ...
        # myso-benchmark tests use myso-surfer which requires move sources to be checked out
        if "crates/myso-surfer" not in crates and "crates/myso-benchmark" in crates:
            crates.append("crates/myso-surfer")
...
def update_git_sparse_checkout(crates_to_checkout):
    ...
    # if we don't have myso-framework, we probably need to add the move sources in order for tests
    # to run
    if "crates/myso-framework" not in crates_to_checkout:
        default_directories.append("crates/myso-framework/packages")
