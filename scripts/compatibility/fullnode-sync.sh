...
CLEAN=0
LOG_LEVEL="info"
MYSO_RUN_PATH="/opt/myso"
VERBOSE=""

function cleanup {
    echo "Performing exit cleanup..."
    [ ! -z $MYSO_NODE_PID ] && kill $MYSO_NODE_PID && echo "Shutdown mysonode process running on pid $MYSO_NODE_PID"
}
...
while getopts "hvn:e:p:t:" OPT; do
    case $OPT in
        p) 
            MYSO_BIN_PATH=$OPTARG ;;
        v)
            LOG_LEVEL="myso=debug,error"
            VERBOSE="--verbose" 
            ;;
        n)
            NETWORK=$OPTARG ;;
        ...
        h)
            ...
            >&2 echo ""
            >&2 echo "Options:"
            >&2 echo " -p                 Path to myso binary to run. If unspecified, will build from source."
...
[ ! -d "${MYSO_RUN_PATH}/mysodb" ] && mkdir -p ${MYSO_RUN_PATH}/mysodb
...
if [[ ! -f "${MYSO_RUN_PATH}/fullnode.yaml" ]]; then
    echo "Generating fullnode.yaml at ${MYSO_RUN_PATH}/fullnode.yaml"
    cp crates/myso-config/data/fullnode-template.yaml ${MYSO_RUN_PATH}/fullnode.yaml
    sed -i "s|genesis.blob|${MYSO_RUN_PATH}/genesis.blob|g" ${MYSO_RUN_PATH}/fullnode.yaml
    sed -i "s|mysodb|${MYSO_RUN_PATH}/mysodb|g" ${MYSO_RUN_PATH}/fullnode.yaml

    if [[ $NETWORK != "devnet" ]]; then
        cat >> "${MYSO_RUN_PATH}/fullnode.yaml" <<- EOM

p2p-config:
  seed-peers:
    - address: /dns/ewr-tnt-ssfn-00.testnet.myso.io/udp/8084
      peer-id: df8a8d128051c249e224f95fcc463f518a0ebed8986bbdcc11ed751181fecd38
    - address: /dns/lax-tnt-ssfn-00.testnet.myso.io/udp/8084
      peer-id: f9a72a0a6c17eed09c27898eab389add704777c03e135846da2428f516a0c11d
    - address: /dns/lhr-tnt-ssfn-00.testnet.myso.io/udp/8084
      peer-id: 9393d6056bb9c9d8475a3cf3525c747257f17c6a698a7062cbbd1875bc6ef71e
    - address: /dns/mel-tnt-ssfn-00.testnet.myso.io/udp/8084
...
if [[ -z $MYSO_BIN_PATH ]]; then
    echo "Building myso..."
    cargo build --release --bin myso-node
    MYSO_BIN_PATH="target/release/myso-node"
    echo "Done"
fi


echo "Starting mysonode..."
RUST_LOG=$LOG_LEVEL $MYSO_BIN_PATH --config-path ${MYSO_RUN_PATH}/fullnode.yaml &
MYSO_NODE_PID=$!
