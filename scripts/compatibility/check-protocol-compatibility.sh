...
if [ -z "$RELEASED_COMMIT" ]; then
  ...
  case "$NETWORK" in
    devnet)
      URL="https://$API_USER:$API_KEY@gateway.mimir.myso.io/prometheus/api/v1/query"
      ;;
    testnet)
      URL="http://$API_USER:$API_KEY@metrics-gw-2.testnet.myso.io/prometheus/api/v1/query"
      ;;
    mainnet)
      URL="https://$API_USER:$API_KEY@metrics-gw-2.mainnet.myso.io/prometheus/api/v1/query"
      ;;
  esac
...
# check out all files in crates/myso-protocol-config/src/snapshots at origin commit
echo "Checking out $NETWORK snapshot files"
git checkout $RELEASED_COMMIT -- crates/myso-protocol-config/src/snapshots || exit 1
...
cargo test --package myso-protocol-config snapshot_tests || exit 1
