...
def parse_args():
    ...
    generate_lib = subparsers.add_parser(
        "generate-lib",
        help=(
            "Generate `myso-execution/src/lib.rs` based on the current set of "
...
def do_cut(args):
    ...
    update_toml(args.feature, Path() / "myso-execution" / "Cargo.toml")
    generate_impls(args.feature, impl_module)

    with open(Path() / "myso-execution" / "src" / "lib.rs", mode="w") as lib:
        generate_lib(lib)
...
def do_generate_lib(args):
    if args.dry_run:
        ...
        lib_path = Path() / "myso-execution" / "src" / "lib.rs"
...
def cut_command(f):
    ...
    return [
        ...
        *[repo_root()],
        *["-d", f"myso-execution/latest:myso-execution/{f}:-latest"],
        *["-d", f"external-crates/move:external-crates/move/move-execution/{f}"],
        *["-p", "myso-adapter-latest"],
        *["-p", "myso-move-natives-latest"],
        *["-p", "myso-verifier-latest"],
...
def cut_directories(f):
    ...
    myso_base = Path() / "myso-execution"
    external = Path() / "external-crates"

    crates = [
        myso_base / f / "myso-adapter",
        myso_base / f / "myso-move-natives",
        myso_base / f / "myso-verifier",
    ]
...
def impl(feature):
    ...
    return Path() / "myso-execution" / "src" / (feature.replace("-", "_") + ".rs")
...
def update_toml(feature, toml_path):
    """Add dependencies for 'feature' to myso-execution's manifest."""
...
def generate_impls(feature, copy):
    ...
    orig = Path() / "myso-execution" / "src" / "latest.rs"
...
def generate_lib(output_file: TextIO):
    ...
    Generates the contents of myso-execution/src/lib.rs to assign a numeric
    ...
    template_path = Path() / "myso-execution" / "src" / "lib.template.rs"
    cuts = discover_cuts()
...
# Modules in `myso-execution` that don't count as "cuts" (they are
...
def discover_cuts():
    ...
    Finds all modules within the `myso-execution` crate that count as
