...
contract BridgeUtilsTest is BridgeBaseTest {
    ...
    function testConvertERC20ToMySoDecimalAmountTooLargeForUint64() public {
        vm.expectRevert(bytes("BridgeUtils: Amount too large for uint64"));
        BridgeUtils.convertERC20ToMySoDecimal(18, 8, type(uint256).max);
    }

    function testConvertERC20ToMySoDecimalInvalidMySoDecimal() public {
        vm.expectRevert(bytes("BridgeUtils: Invalid MySo decimal"));
        BridgeUtils.convertERC20ToMySoDecimal(10, 11, 100);
    }

    function testconvertMySoToERC20DecimalInvalidMySoDecimal() public {
        vm.expectRevert(bytes("BridgeUtils: Invalid MySo decimal"));
        BridgeUtils.convertMySoToERC20Decimal(10, 11, 100);
    }

    function testConvertERC20ToMySoDecimal() public view {
        // ETH
        assertEq(IERC20Metadata(wETH).decimals(), 18);
        uint256 ethAmount = 10 ether;
        uint64 mysoAmount = BridgeUtils.convertERC20ToMySoDecimal(
            IERC20Metadata(config.tokenAddressOf(BridgeUtils.ETH)).decimals(),
            config.tokenMySoDecimalOf(BridgeUtils.ETH),
            ethAmount
        );
        assertEq(mysoAmount, 10_000_000_00); // 10 * 10 ^ 8

        // USDC
        assertEq(IERC20Metadata(USDC).decimals(), 6);
        ethAmount = 50_000_000; // 50 USDC
        mysoAmount = BridgeUtils.convertERC20ToMySoDecimal(
            IERC20Metadata(config.tokenAddressOf(BridgeUtils.USDC)).decimals(),
            config.tokenMySoDecimalOf(BridgeUtils.USDC),
            ethAmount
        );
        assertEq(mysoAmount, ethAmount);
        ...
        mysoAmount = BridgeUtils.convertERC20ToMySoDecimal(
            ...
            config.tokenMySoDecimalOf(BridgeUtils.USDT),
            ethAmount
        );
        assertEq(mysoAmount, ethAmount);
        ...
        mysoAmount = BridgeUtils.convertERC20ToMySoDecimal(
            ...
            config.tokenMySoDecimalOf(BridgeUtils.BTC),
            ethAmount
        );
        assertEq(mysoAmount, ethAmount);
    }

    function testconvertMySoToERC20Decimal() public view {
        // ETH
        assertEq(IERC20Metadata(wETH).decimals(), 18);
        uint64 mysoAmount = 11_000_000_00; // 11 eth
        uint256 ethAmount = BridgeUtils.convertMySoToERC20Decimal(
            IERC20Metadata(config.tokenAddressOf(BridgeUtils.ETH)).decimals(),
            config.tokenMySoDecimalOf(BridgeUtils.ETH),
            mysoAmount
        );
        ...
        // USDC
        assertEq(IERC20Metadata(USDC).decimals(), 6);
        mysoAmount = 50_000_000; // 50 USDC
        ethAmount = BridgeUtils.convertMySoToERC20Decimal(
            IERC20Metadata(config.tokenAddressOf(BridgeUtils.USDC)).decimals(),
            config.tokenMySoDecimalOf(BridgeUtils.USDC),
            mysoAmount
        );
