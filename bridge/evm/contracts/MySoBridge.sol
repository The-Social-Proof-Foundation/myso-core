...
import "./interfaces/IMySoBridge.sol";
...
/// tokens. Bridge operations are managed by a committee of MySo validators that are responsible
...
contract MySoBridge is IMySoBridge, CommitteeUpgradeable, PausableUpgradeable {
    /* ========== STATE VARIABLES ========== */
    ...
    /// @notice Initializes the MySoBridge contract with the provided parameters.
    ...
    /// @param recipientAddress The address on the MySo chain where the tokens will be sent.
        ...
        onlySupportedChain(destinationChainID)
    {
        ...
        require(success, "MySoBridge: Failed to transfer ETH to vault");
        ...
        uint64 mysoAdjustedAmount = BridgeUtils.convertERC20ToMySoDecimal(
            ...
            config.tokenMySoDecimalOf(BridgeUtils.ETH),
            amount
        );
    ...
    modifier limitNotExceeded(uint8 chainID, uint8 tokenID, uint256 amount) {
        require(
            ...
            "MySoBridge: Amount exceeds bridge limit"
        );
        _;
    }
    ...
    modifier onlySupportedChain(uint8 targetChainID) {
        require(
            ...
            "MySoBridge: Target chain not supported"
        );
        _;
    }
}
