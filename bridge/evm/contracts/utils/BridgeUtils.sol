...
/// @title BridgeUtils
/// @notice This library defines the message format and constants for the MySo native bridge. It also
...
library BridgeUtils {
    ...
    /// @notice Converts the provided token amount to the MySo decimal adjusted amount.
    /// @param erc20Decimal The erc20 decimal value for the token.
    /// @param mysoDecimal The myso decimal value for the token.
    /// @param amount The ERC20 amount of the tokens to convert to MySo.
    /// @return MySo converted amount.
    function convertERC20ToMySoDecimal(uint8 erc20Decimal, uint8 mysoDecimal, uint256 amount)
        internal
        pure
        returns (uint64)
    {
        if (erc20Decimal == mysoDecimal) {
        ...
        require(erc20Decimal > mysoDecimal, "BridgeUtils: Invalid MySo decimal");

        // Difference in decimal places
        uint256 factor = 10 ** (erc20Decimal - mysoDecimal);
    ...
    /// @notice Converts the provided MySo decimal adjusted amount to the ERC20 token amount.
    /// @param erc20Decimal The erc20 decimal value for the token.
    /// @param mysoDecimal The myso decimal value for the token.
    /// @param amount The MySo amount of the tokens to convert to ERC20.
    /// @return ERC20 converted amount.
    function convertMySoToERC20Decimal(uint8 erc20Decimal, uint8 mysoDecimal, uint64 amount)
        internal
        pure
        returns (uint256)
    {
        if (mysoDecimal == erc20Decimal) {
    ...
    ///     bytes 1-32   : sender address (as we only support MySo now, it has to be 32 bytes long)
        ...
        returns (TokenTransferPayload memory)
    {
        ...
        require(
            ...
            "BridgeUtils: Invalid sender address length, MySo address must be 32 bytes"
        );
    ...
    ///     byte m + 1       : number of myso decimals
    ///     bytes m + 2 -> i : myso decimals
    ...
    /// @return mysoDecimals the MySo decimal places of the tokens to be added.
        ...
        )
    {
        ...
        mysoDecimals = new uint8[](decimalCount);
        for (uint8 i; i < decimalCount; i++) {
            mysoDecimals[i] = uint8(_payload[offset++]);
        }
