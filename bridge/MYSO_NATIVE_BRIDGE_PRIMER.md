# MySo Native Bridge Primer for Agents

This document is a fast-start artifact for any agent (human or AI) that needs to reason about the MySo Native Bridge. It maps the three primary code surfaces—the Ethereum smart contracts, the MySo Move package, and the Rust bridge node—so you can quickly answer “where does this live?” and “who is responsible for what?” before making changes or asking follow-up questions.

## Purpose & Scope

- Clarify the ownership boundaries between the EVM contracts, the Move bridge module, and the Rust bridge node.
- Highlight the critical data flows for MySo→EVM and EVM→MySo transfers, plus emergency/governance operations.
...
| --------------------- | --------------------------------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ----------------------------------------------------------------------------------------------------------------------------- |
| EVM `MySoBridge`       | `bridge/evm/contracts/MySoBridge.sol`                                                          | Custody vault orchestrator on Ethereum: collects deposits, enforces limits, releases assets after committee-signed messages.                                                                | `CommitteeUpgradeable`, `IBridgeVault`, `IBridgeLimiter`, `IBridgeConfig`, OpenZeppelin upgradeable stack.                    |
| Move `bridge::bridge` | `crates/myso-framework/packages/bridge/sources/bridge.move`                                    | On-chain MySo contract that mints/burns bridged assets, stores bridge messages, enforces limiter + committee approvals, handles governance ops.                                              | `bridge::committee`, `bridge::limiter`, `bridge::treasury`, `myso::event`, `myso_system::myso_system`.                           |
| MySo Bridge Node       | `crates/myso-bridge/src` (`main.rs`, `node.rs`, `server`, `client`, `orchestrator`, `syncers`) | Off-chain daemon that (a) exposes a signing server for bridge actions, (b) optionally runs a client/orchestrator that watches both chains, aggregates signatures, and submits transactions. | Axum server, Reqwest client, `BridgeAuthorityAggregator`, `EthSyncer`, `MySoSyncer`, `BridgeOrchestrator`, Prometheus metrics. |

## EVM Contract: `bridge/evm/contracts/MySoBridge.sol`

### State & Initialization

`MySoBridge` keeps track of processed transfer nonces, references to the vault and limiter contracts, and enforces a fixed MySo address length. Deployment follows the OpenZeppelin upgradeable pattern via `initialize`, which wires in the committee contract and pause controller:

```24:45:bridge/evm/contracts/MySoBridge.sol
mapping(uint64 nonce => bool isProcessed) public isTransferProcessed;
IBridgeVault public vault;
IBridgeLimiter public limiter;

uint8 constant MYSO_ADDRESS_LENGTH = 32;
...
`transferBridgedTokensWithSignatures` is invoked by the bridge node once it has gathered quorum signatures for a MySo→EVM transfer message. It verifies the message, ensures the nonce was not processed, decodes the payload, checks the target chain, converts MySo decimals into ERC20 decimals, pulls tokens from the vault, and marks the nonce consumed:

```55:103:bridge/evm/contracts/MySoBridge.sol
...
{
    require(!isTransferProcessed[message.nonce], "MySoBridge: Message already processed");
    ...
    require(
        tokenTransferPayload.targetChain == config.chainID(), "MySoBridge: Invalid target chain"
    );
    uint256 erc20AdjustedAmount = BridgeUtils.convertMySoToERC20Decimal(
        IERC20Metadata(config.tokenAddressOf(tokenTransferPayload.tokenID)).decimals(),
        config.tokenMySoDecimalOf(tokenTransferPayload.tokenID),
...
Users call `bridgeERC20` or `bridgeETH` to lock assets into the vault. The contract checks the destination route, token support, allowance, and recipient length. It measures the vault delta, converts ERC20 decimals back into MySo decimals, and emits `TokensDeposited` with the current nonce before incrementing it:

```135:189:bridge/evm/contracts/MySoBridge.sol
function bridgeERC20(
    ...
    require(recipientAddress.length == MYSO_ADDRESS_LENGTH, "MySoBridge: Invalid recipient address length");
    IBridgeConfig config = committee.config();
    require(config.isTokenSupported(tokenID), "MySoBridge: Unsupported token");
    address tokenAddress = config.tokenAddressOf(tokenID);
    require(
        IERC20(tokenAddress).allowance(msg.sender, address(this)) >= amount,
        "MySoBridge: Insufficient allowance"
    );
    ...
    uint64 mysoAdjustedAmount = BridgeUtils.convertERC20ToMySoDecimal(
        IERC20Metadata(tokenAddress).decimals(),
        config.tokenMySoDecimalOf(tokenID),
        amountTransfered
    );
...
```244:265:bridge/evm/contracts/MySoBridge.sol
function _transferTokensFromVault(
    ...
    require(tokenAddress != address(0), "MySoBridge: Unsupported token");
...
```105:125:bridge/evm/contracts/MySoBridge.sol
...
- Decimal conversions happen _twice_ (ERC20→MySo during deposit, MySo→ERC20 during withdrawal); mismatched decimals lead to silent value drift.
...
- Registry for supported tokens (`tokenAddressOf`, `tokenMySoDecimalOf`, `tokenPriceOf`) and supported chain IDs. The contract also stores per-token USD prices (8 decimal precision) and can add new ERC20s via committee-signed `ADD_EVM_TOKENS` messages.
- `updateTokenPriceWithSignatures` and `addTokensWithSignatures` mirror the governance actions emitted on MySo; both routes end with dedicated events so the off-chain client can pick them up.
- When adding a token, `_addToken` cross-checks ERC20 decimals against the provided MySo decimals to prevent overflows during conversion.
...
- `recordBridgeTransfers` is `onlyOwner` (the MySoBridge proxy) and converts each transfer into USD using `BridgeConfig.tokenPriceOf`. If the USD total for the past 24 hours would exceed `chainLimits[chainID]`, it reverts with `"BridgeLimiter: amount exceeds rolling window limit"`.
...
## Move Package: `crates/myso-framework/packages/bridge/sources/bridge.move`
...
```46:87:crates/myso-framework/packages/bridge/sources/bridge.move
...
When a MySo user bridges out, the module ensures the bridge is not paused, validates the route and EVM address length, increments the per-message-type sequence number, builds a `BridgeMessage`, burns the coins in the treasury, stores the record, and emits an event:

```218:333:crates/myso-framework/packages/bridge/sources/bridge.move
...
Bridge authorities submit committee-signed `BridgeMessage`s. The Move module verifies signatures, ensures message type/version/chain constraints, and either attaches verified signatures to an existing record (MySo-originated) or inserts a new record (foreign-originated). Duplicate approvals short-circuit with events:

```338:395:crates/myso-framework/packages/bridge/sources/bridge.move
public fun approve_token_transfer(
    ...
    if (message.source_chain() == inner.chain_id) {
        ...
        assert!(!record.claimed, EInvariantMySoInitializedTokenTransferShouldNotBeClaimed);
...
```436:473:crates/myso-framework/packages/bridge/sources/bridge.move
public fun execute_system_message(
    ...
    else if (message_type == message_types::add_tokens_on_myso()) { /* ... */ }
    else {
...
- Stores the bridge committee members, their voting power snapshot, URLs, and blocklist state. Registration is limited to validators (sender must appear in `MySoSystemState.active_validator_addresses`), and the final committee is instantiated via `try_create_next_committee` during end-of-epoch.
- `verify_signatures` recreates the same prefix-hash scheme as the EVM contracts (`"MYSO_BRIDGE_MESSAGE"` prefix + BCS payload) and enforces that the accumulated voting power across non-blocklisted members meets `BridgeMessage.required_voting_power()`:

```89:120:crates/myso-framework/packages/bridge/sources/committee.move
...
- `register_foreign_token` accepts a `TreasuryCap<T>` and `UpgradeCap`, ensures the cap has zero supply, drops the metadata into a “waiting room,” and emits `TokenRegistrationEvent`. Only after governance approves the token via `AddTokenOnMySo` does `add_new_token` move it into `supported_tokens`, freeze the UpgradeCap, and emit `NewTokenEvent`.
...
- `create_token_bridge_message` encodes sender bytes, target chain/address, token type, and amount into a canonical BCS layout that both MySo and Ethereum verify.
...
## MySo Bridge Node (`crates/myso-bridge`)
...
```53:150:crates/myso-bridge/src/node.rs
pub async fn run_bridge_node(
    ...
    let myso_chain_identifier = server_config.myso_client.get_chain_identifier().await?;
    ...
    let committee = Arc::new(server_config.myso_client.get_bridge_committee().await?);
    let mut handles = vec![];
    let eth_provider = server_config.eth_client.provider();
    let eth_bridge_proxy_address = server_config.eth_bridge_proxy_address;
    let myso_client = server_config.myso_client.clone();
    handles.push(spawn_logged_monitored_task!(start_watchdog(/* ... */)));
    let myso_system = server_config.myso_client.jsonrpc_client().governance_api().get_latest_myso_system_state().await?;
    if let Some(client_config) = client_config {
        let committee_keys_to_names = Arc::new(get_validator_names_by_pub_keys(&committee, &myso_system).await);
...
```91:139:crates/myso-bridge/src/server/mod.rs
pub fn run_server(
...
If `run_client` is true in the config, `start_client_components` spins up the full cross-chain pipeline: persistent storage (`BridgeOrchestratorTables`), `EthSyncer` + `MySoSyncer` watchers, `BridgeAuthorityAggregator` (fetches signatures from peer bridge nodes), `BridgeActionExecutor` (submits transactions to MySo/EVM), `BridgeMonitor`, and orchestrator channels. It also queries token metadata, checks the pause flag, and seeds watch cursors:

```258:360:crates/myso-bridge/src/node.rs
async fn start_client_components(
    ...
    let myso_client = client_config.myso_client.clone();
    ...
    let (task_handles, myso_events_rx) = MySoSyncer::new(client_config.myso_client, myso_modules_to_watch, metrics.clone())
    ...
    let myso_token_type_tags = myso_client.get_token_id_map().await.unwrap();
    let is_bridge_paused = myso_client.is_bridge_paused().await.unwrap();
    let (bridge_pause_tx, bridge_pause_rx) = tokio::sync::watch::channel(is_bridge_paused);
    let (myso_monitor_tx, myso_monitor_rx) = mysten_metrics::metered_channel::channel(10000, /* ... */);
...
```4:105:crates/myso-bridge/src/orchestrator.rs
//! `BridgeOrchestrator` monitors MySo & Ethereum events, updates WAL/cursors, hands actions to the executor.
pub struct BridgeOrchestrator<C> {
    _myso_client: Arc<MySoClient<C>>,
    myso_events_rx: mysten_metrics::metered_channel::Receiver<(Identifier, Vec<MySoEvent>)>,
    eth_events_rx: mysten_metrics::metered_channel::Receiver<(EthAddress, u64, Vec<EthLog>)>,
    store: Arc<BridgeOrchestratorTables>,
    myso_monitor_tx: mysten_metrics::metered_channel::Sender<MySoBridgeEvent>,
    ...
    C: MySoClientInner + 'static,
{
    ...
    pub async fn run(
        ...
        task_handles.push(spawn_logged_monitored_task!(Self::run_myso_watcher(/* ... */)));
...
- `start_watchdog` (see `node.rs`) monitors Ethereum vault balances for WETH/USDT/WBTC/LBTC, ETH & MySo pause status, and optional total supply invariants, exporting Prometheus gauges.
...
Bridge clients (the optional `run_client` portion of `myso-bridge`) always submit an **approval transaction** before any assets are released or minted. On both chains this approval is the `bridge::approve_token_transfer` Move call—which either attaches signatures to an existing MySo-originated record or inserts a new record for EVM-originated transfers. Only after this approval lands do claims/unlocks occur. Keep that invariant in mind when reasoning about replay protection or idempotency.

### MySo → Ethereum (Mint on MySo, Unlock on EVM)

1. User calls `bridge::send_token` to burn/lock the MySo asset and emit `TokenDepositedEvent` (`bridge.move` lines 218–333).
2. `MySoSyncer` (client-side) tails the `bridge`, `bridge_committee`, `bridge_treasury`, and `bridge_limiter` modules, forwarding every deposit event into the orchestrator channel.
3. `BridgeOrchestrator` converts the raw event into `BridgeAction::MySoToEthTokenTransfer` and persists it in the WAL to guarantee retries.
4. `BridgeAuthorityAggregator` gathers the validator signatures needed for quorum by calling each Axum `/sign/bridge_action/myso/eth/...` endpoint.
5. **Approval:** The bridge client sends `bridge::approve_token_transfer` on MySo. Because the message originated on MySo, this transaction simply attaches the aggregated signatures to the existing `BridgeRecord` (`bridge.move` lines 338–372). This is the first on-chain transaction the client executes in this direction.
6. After approval is on-chain, the executor calls `MySoBridge.transferBridgedTokensWithSignatures` on Ethereum, which verifies the same signatures, withdraws from the vault, and pays the EVM recipient (`MySoBridge.sol` lines 55–103).

```mermaid title="MySo → Ethereum Flow" type="diagram"
flowchart LR
    ...
    F --> G["MySoBridge.sol transferBridgedTokensWithSignatures\n(EVM unlock)"]
    G --> H["EVM Recipient"]
```

### Ethereum → MySo (Lock on EVM, Mint on MySo)

1. User calls `MySoBridge.bridgeERC20`/`bridgeETH` to send funds into the EVM vault and emit `TokensDeposited` (`MySoBridge.sol` lines 135–235).
...
3. `BridgeOrchestrator` maps logs into `BridgeAction::EthToMySo*` records, writes them into the WAL, and forwards them to the executor.
4. `BridgeAuthorityAggregator` again fans out to Axum peers for signatures covering the decoded `BridgeMessage`.
5. **Approval:** The bridge client now calls `bridge::approve_token_transfer` on MySo. Because the message originated on Ethereum, this transaction inserts a brand-new `BridgeRecord` containing the message + signatures (`bridge.move` lines 374–395). This is the first client-submitted transaction in this direction and unlocks the subsequent claim path.
6. Recipients (or relayers via `claim_and_transfer_token`) call `bridge::claim_token`/`claim_and_transfer_token`, which runs limiter checks, mints from the treasury, and transfers the MySo coins to the target address (`bridge.move` lines 398–434 & 548–634).

```mermaid title="Ethereum → MySo Flow" type="diagram"
flowchart LR
    A["EVM User\nbridgeERC20/ETH"] -->|TokensDeposited event| B["MySoBridge.sol + Vault"]
    B --> C["EthSyncer\n(node client)"]
    C --> D["BridgeOrchestrator\nWAL"]
    D --> E["BridgeAuthorityAggregator\n(sigs)"]
    E --> F["bridge::approve_token_transfer\n(first client tx)"
...
- Emergency pause/unpause: bridge node hits `/sign/emergency_button/...`, collects signatures, then calls `bridge::execute_system_message` (Move) or `MySoBridge.executeEmergencyOpWithSignatures` (EVM) to toggle `paused`.
...
- Adding tokens: `/sign/add_tokens_on_myso` or `/sign/add_tokens_on_evm` feeds into corresponding Move/EVM modules; keep token IDs, type names, and prices arrays length-matched (see Move code lines 676–693).

```mermaid title="MySo Native Bridge Data Flow" type="diagram"
flowchart LR
    MySoUser["MySo User\nsend_token"] -->|burn & emit event| MySoBridge["Move bridge::bridge"]
    MySoBridge -->|TokenDepositedEvent| MySoSyncer["MySoSyncer\n(node client)"]
    MySoSyncer --> Orchestrator["BridgeOrchestrator\nWAL + actions"]
    ...
    AxumServer --> EvmBridge["EVM MySoBridge.sol"]
    ...
    MoveBridgeOps -->|mint/claim| MySoRecipient["MySo Recipient"]
...
```67:86:crates/myso-framework/packages/bridge/sources/bridge.move
...
```275:333:crates/myso-framework/packages/bridge/sources/bridge.move
...
```548:582:crates/myso-framework/packages/bridge/sources/bridge.move
...
- `MySoBridgeV2.sol` duplicates the deposit/claim entry points with timestamp-aware variants. Deposits emit `TokensDepositedV2` (including `block.timestamp` in seconds), while withdrawals call `_transferTokensFromVault` guarded by `limitNotExceededV2`:
...
// MySoBridgeV2.sol - limitNotExceededV2 modifier
...
- **MySo/Move**: Uses milliseconds (`timestamp_ms`)
...
- **Rust bridge node**: Converts ETH seconds → MySo milliseconds (`* 1000`) when building MySo transactions

### Bridge Node Updates

- New bridge actions (`MySoToEthTokenTransferV2`, `EthToMySoTokenTransferV2`) propagate the timestamp through the orchestrator WAL, and the authority aggregator requests signatures against the V2 payload format (`BridgeUtilsV2`).
...
- Watchers must track both the legacy and V2 events. `get_myso_modules_to_watch` and `get_eth_contracts_to_watch` remain the same; the differentiator is the event decoding logic in `MySoBridgeEvent` / `EthBridgeEvent`.
...
**The EVM MySoBridge proxy has different storage layouts between mainnet and testnet (Sepolia).** This is because a `__gap` storage array was added to `CommitteeUpgradeable.sol` between the testnet and mainnet deployments.
...
| Network | MySoBridge Proxy | vault slot | limiter slot |
|---------|-----------------|------------|--------------|
...
- **Explain the nonce story:** Can you point to `BridgeInner.sequence_nums` and `MySoBridge.nonces` and describe how they prevent replays across message types?
- **Describe limiter enforcement on both sides:** On EVM it’s `limitNotExceeded` + `limiter.recordBridgeTransfers` (`MySoBridge.sol` lines 244–265); on MySo it’s `limiter.check_and_record_sending_transfer` inside `claim_token_internal` (`bridge.move` lines 603–624). Make sure you can articulate when the 48h bypass kicks in.
...
- **Know where to adjust watchers:** `get_myso_modules_to_watch` & `get_eth_contracts_to_watch` in `node.rs` (lines 362–404) determine cursor overrides. If you change event structure, update these lists.
- **Be ready to run tests:** `forge test` in `bridge/evm`, `cargo test -p myso-bridge` for Rust, `myso move test` for the Move package. Long-running tasks need ≥10m timeouts per repo guidelines.
...
- **Bridge node:** `cargo run -p myso-bridge -- --config-path <path>`; ensure your config passes `BridgeNodeConfig::validate` (see `crates/myso-bridge/src/config.rs`).
- **Move package:** `myso move test --path crates/myso-framework/packages/bridge`.
- **Metrics:** Run `cargo run -p myso-bridge ...` and hit `http://<host>:<metrics_port>/metrics` or `/ping` to confirm the node advertises its version and metrics key.

## Running Rust Tests for myso-bridge

The `myso-bridge` crate has specific test configuration requirements that differ from typical Cargo test invocations.
...
```toml
# From crates/myso-bridge/Cargo.toml
[features]
...
```bash
cargo nextest run -p myso-bridge --features test-utils e2e_tests::basic
```

### E2E Test Considerations

E2E tests spin up local MySo clusters and anvil (Ethereum simulator). They require:
...
| Integration  | `action_executor::`, `monitor::`, `myso_client::` | 1–10s each       | Uses mock clients        |
...
With this primer in hand, you—and future agents—should be able to navigate the MySo Native Bridge code paths, justify design decisions, and verify understanding before proposing changes.
