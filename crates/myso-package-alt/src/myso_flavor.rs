...
const FLAVOR: &str = "myso";

#[derive(Debug, Serialize, Deserialize)]
pub struct MySoFlavor;
...
impl MySoFlavor {
    ...
    fn system_deps_by_name() -> BTreeMap<String, SystemDepName> {
        BTreeMap::from([
            ("MySo".into(), "myso".into()),
            ("MySoSystem".into(), "myso_system".into()),
...
impl MoveFlavor for MySoFlavor {
    fn name() -> String {
        FLAVOR.to_string()
    }
    ...
    fn implicit_dependencies(_: &EnvironmentID) -> BTreeMap<PackageName, ReplacementDependency> {
        BTreeMap::from([
            (
                PackageName::new("myso").expect("myso is a valid identifier"),
                ReplacementDependency::override_system_dep("myso"),
            ),
            (
    ...
    fn system_deps(_: &EnvironmentID) -> BTreeMap<SystemDepName, LockfileDependencyInfo> {
        ...
        for package in pkgs {
            ...
            let info = LockfileDependencyInfo::Git(LockfileGitDepInfo {
                ...
                rev: GitSha::try_from(sha_clean.to_string()).expect("manifest has valid sha"),
            });
...
fn validate_modern_manifest_does_not_use_legacy_system_names(
    ...
    let legacy_names = MySoFlavor::system_deps_by_name();

    for name in dep_names {
        if legacy_names.contains_key(&name) {
            return Err(format!(
                "Dependency `{name}` is a legacy system name and cannot be used. See https://docs.mysocial.network/guides/developer/myso-101/move-package-management#system-dependencies"
            ));
        }
    }
