...
use myso_simulator::fastcrypto::encoding::{Base64, Encoding};
...
async fn test_get_objects() -> Result<(), anyhow::Error> {
    ...
    let objects = http_client
        .get_owned_objects(
            address,
            Some(MySoObjectResponseQuery::new_with_options(
...
async fn test_get_package_with_display_should_not_fail() -> Result<(), anyhow::Error> {
    ...
    let response = http_client
        .get_object(
            ObjectID::from(MYSO_FRAMEWORK_ADDRESS),
            Some(MySoObjectDataOptions::new().with_display()),
        )
        .await;
    assert!(response.is_ok());
    let response: MySoObjectResponse = response?;
    assert!(
        response
            .into_object()
            .unwrap()
...
async fn test_public_transfer_object() -> Result<(), anyhow::Error> {
    ...
    let tx_response: MySoTransactionBlockResponse = http_client
...
async fn test_publish() -> Result<(), anyhow::Error> {
    ...
    let tx_response = http_client
        .execute_transaction_block(
            tx_bytes,
            signatures,
            Some(MySoTransactionBlockResponseOptions::new().with_effects()),
            Some(ExecuteTransactionRequestType::WaitForLocalExecution),
        )
        .await?;
    matches!(tx_response, MySoTransactionBlockResponse {effects, ..} if effects.as_ref().unwrap().created().len() == 6);
    Ok(())
}
...
async fn test_move_call() -> Result<(), anyhow::Error> {
    ...
    // now do the call
    let package_id = ObjectID::new(MYSO_FRAMEWORK_ADDRESS.into_bytes());
    ...
    let tx_response = http_client
        ...
        .await?;
    matches!(tx_response, MySoTransactionBlockResponse {effects, ..} if effects.as_ref().unwrap().created().len() == 1);
    Ok(())
}
...
async fn test_get_object_info() -> Result<(), anyhow::Error> {
    ...
    for obj in objects {
        ...
        let result = http_client
            .get_object(
                oref.object_id,
                Some(MySoObjectDataOptions::new().with_owner()),
            )
            .await?;
        assert!(
            matches!(result, MySoObjectResponse { data: Some(object), .. } if oref.object_id == object.object_id && object.owner.clone().unwrap().get_owner_address()? == address)
        );
    }
    Ok(())
}
...
async fn test_get_object_data_with_content() -> Result<(), anyhow::Error> {
    ...
    let objects = http_client
        .get_owned_objects(
            address,
            Some(MySoObjectResponseQuery::new_with_options(
                MySoObjectDataOptions::new().with_content().with_owner(),
            )),
            None,
            None,
        )
    ...
    for obj in objects {
        ...
        let result = http_client
            .get_object(
                oref.object_id,
                Some(MySoObjectDataOptions::new().with_content().with_owner()),
            )
            .await?;
        assert!(
            matches!(result, MySoObjectResponse { data: Some(object), .. } if oref.object_id == object.object_id && object.owner.clone().unwrap().get_owner_address()? == address)
...
async fn test_get_coins() -> Result<(), anyhow::Error> {
    ...
    let result: CoinPage = http_client
        .get_coins(address, Some("0x2::myso::TestCoin".into()), None, None)
        .await?;
    assert_eq!(0, result.data.len());

    let result: CoinPage = http_client
        .get_coins(address, Some("0x2::myso::MYSO".into()), None, None)
        .await?;
    assert_eq!(5, result.data.len());
    assert!(!result.has_next_page);
    ...
    // Test paging
    let result: CoinPage = http_client
        .get_coins(address, Some("0x2::myso::MYSO".into()), None, Some(3))
        .await?;
...
async fn test_sorted_get_coin_response() {
    ...
    for amount in amounts {
        let tx = make_transfer_myso_transaction(&cluster.wallet, Some(address), Some(amount)).await;
        let (tx_bytes, signatures) = tx.to_tx_bytes_and_signatures();
...
async fn test_get_balance() -> Result<(), anyhow::Error> {
    ...
    assert_eq!("0x2::myso::MYSO", result.coin_type);
    assert_eq!(
...
async fn test_get_metadata() -> Result<(), anyhow::Error> {
    ...
    let result: MySoCoinMetadata = http_client
...
async fn test_get_total_supply() -> Result<(), anyhow::Error> {
    ...
    let treasury_cap = object_changes
        ...
        .find_map(|e| {
            ...
            {
                if &TreasuryCap::type_(parse_myso_struct_tag(&coin_name).unwrap()) == object_type {
                    Some(object_id)
                } else {
                    None
                }
    ...
    let MySoTransactionBlockResponse { effects, .. } = tx_response;

    assert_eq!(MySoExecutionStatus::Success, *effects.unwrap().status());
...
async fn test_staking() -> Result<(), anyhow::Error> {
    ...
    // Check StakedMySo object before test
    let staked_myso: Vec<DelegatedStake> = http_client.get_stakes(address).await?;
    assert!(staked_myso.is_empty());

    let validator = http_client
        .get_latest_myso_system_state()
        .await?
        .active_validators[0]
        .myso_address;
    ...
    http_client
        .execute_transaction_block(
            tx_bytes,
            signatures,
            Some(MySoTransactionBlockResponseOptions::new()),
            Some(ExecuteTransactionRequestType::WaitForLocalExecution),
        )
        .await?;
    ...
    let staked_myso: Vec<DelegatedStake> = http_client.get_stakes(address).await?;
    assert_eq!(1, staked_myso.len());
    assert_eq!(1000000000, staked_myso[0].stakes[0].principal);
    assert!(matches!(
        staked_myso[0].stakes[0].status,
        StakeStatus::Pending
    ));
    let staked_myso_copy = http_client
        .get_stakes_by_ids(vec![staked_myso[0].stakes[0].staked_myso_id])
        .await?;
    assert_eq!(
        staked_myso[0].stakes[0].staked_myso_id,
        staked_myso_copy[0].stakes[0].staked_myso_id
    );
    Ok(())
}
...
async fn test_unstaking() -> Result<(), anyhow::Error> {
    ...
    let staked_myso_copy = http_client
        .get_stakes_by_ids(vec![
            staked_myso[0].stakes[0].staked_myso_id,
            staked_myso[0].stakes[1].staked_myso_id,
            staked_myso[0].stakes[2].staked_myso_id,
        ])
        .await?;

    assert!(matches!(
        &staked_myso_copy[0].stakes[0].status,
        StakeStatus::Active {
            estimated_reward: _
        }
    ));
    assert!(matches!(
        &staked_myso_copy[0].stakes[1].status,
        StakeStatus::Active {
            estimated_reward: _
        }
    ));
    assert!(matches!(
        &staked_myso_copy[0].stakes[2].status,
        StakeStatus::Active {
            estimated_reward: _
        }
    ));
    ...
    let transaction_bytes: TransactionBlockBytes = http_client
        .request_withdraw_stake(
            address,
            staked_myso_copy[0].stakes[2].staked_myso_id,
            None,
            1_000_000.into(),
        )
        .await?;
...
async fn test_zklogin_verify() -> Result<(), anyhow::Error> {
    ...
    let tx_data = TestTransactionBuilder::new(zklogin_addr, gas, rgp)
        .transfer_myso(None, MySoAddress::ZERO)
        .build();
    let msg = IntentMessage::new(Intent::myso_transaction(), tx_data.clone());
