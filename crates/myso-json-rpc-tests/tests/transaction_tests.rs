...
use myso_json_rpc_api::ReadApiClient;
use myso_json_rpc_types::MySoTransactionBlockDataAPI;
use myso_json_rpc_types::MySoTransactionBlockResponseQuery;
use myso_json_rpc_types::TransactionFilter;
use myso_json_rpc_types::{
    MySoObjectDataOptions, MySoObjectResponseQuery, MySoTransactionBlockResponse,
...
use myso_json_rpc_api::{IndexerApiClient, TransactionBuilderClient, WriteApiClient};
...
async fn test_get_transaction_block() -> Result<(), anyhow::Error> {
    ...
    let mut tx_responses: Vec<MySoTransactionBlockResponse> = Vec::new();
    ...
    // let batch_responses: Vec<MySoTransactionBlockResponse> = http_client
    //     .multi_get_transaction_blocks(tx, Some(MySoTransactionBlockResponseOptions::new()))
    //     .await?;
    ...
    //     assert!(tx_responses
    //         .iter()
    //         .any(|resp| matches!(resp, MySoTransactionBlockResponse {digest, ..} if *digest == r.digest)))
    // }

    // // test get_transaction
    // for tx_digest in tx {
    //     let response: MySoTransactionBlockResponse = http_client
    //         .get_transaction_block(
    //             tx_digest,
    //             Some(MySoTransactionBlockResponseOptions::new().with_raw_input()),
    //         )
    //         .await?;
    //     assert!(tx_responses.iter().any(
    //         |resp| matches!(resp, MySoTransactionBlockResponse {digest, ..} if *digest == response.digest)
    //     ));
...
async fn test_get_fullnode_transaction() -> Result<(), anyhow::Error> {
    ...
    let query = MySoTransactionBlockResponseQuery {
        options: Some(MySoTransactionBlockResponseOptions {
            show_input: true,
            show_effects: true,
            show_events: true,
            ..Default::default()
    ...
    let first_page = client
        ...
        .query_transaction_blocks(
            MySoTransactionBlockResponseQuery::default(),
            None,
            Some(5),
            false,
        )
    ...
    let address_txs_asc = client
        .read_api()
        .query_transaction_blocks(
            MySoTransactionBlockResponseQuery::new_with_filter(TransactionFilter::FromAddress(
...
async fn test_query_transaction_blocks() -> Result<(), anyhow::Error> {
    ...
    let myso_type_args = type_args![GAS::type_tag()]?;
    let type_args = myso_type_args
        .into_iter()
        .map(|ty| ty.try_into())
        .collect::<Result<Vec<_>, _>>()?;

    let myso_call_args_1 = call_args!(coin.data.clone().unwrap().object_id, 10)?;
    ...
    let myso_call_args_2 = call_args!(coin_2.data.clone().unwrap().object_id, 10)?;
    ...
    let move_call_query = MySoTransactionBlockResponseQuery::new_with_filter(filter);
    let tx = client
        .read_api()
        .query_transaction_blocks(move_call_query, None, Some(20), true)
        .await
        .unwrap();
    // verify that only 1 tx is returned and no MySoRpcInputError::ContainsDuplicates error
...
async fn test_display_transaction_block_with_empty_balance_changes() {
    ...
    let tx_block = client
        .get_transaction_block(
            *digest,
            Some(MySoTransactionBlockResponseOptions::full_content()),
        )
        .await
        .unwrap();
