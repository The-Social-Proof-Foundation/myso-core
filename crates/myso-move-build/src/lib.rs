...
use myso_types::{
    BRIDGE_ADDRESS, CONTRA_ADDRESS, MOVE_STDLIB_ADDRESS, MYDATA_ADDRESS, MYSO_FRAMEWORK_ADDRESS,
...
use myso_verifier::verifier as myso_bytecode_verifier;
...
pub mod test_utils {
    ...
    pub async fn compile_managed_coin_package() -> CompiledPackage {
        compile_example_package("../../crates/myso-core/src/unit_tests/data/managed_coin").await
    }
...
/// Wrapper around the core Move `CompiledPackage` with some MySo-specific traits and info
#[derive(Debug, Clone)]
...
/// Wrapper around the core Move `BuildConfig` with some MySo-specific info
#[derive(Clone)]
...
impl BuildConfig {
    pub fn new_for_testing() -> Self {
        ...
        let config = MoveBuildConfig {
            default_flavor: Some(move_compiler::editions::Flavor::MySo),
    ...
    fn compile_package<W: Write + Send, F: MoveFlavor>(
        ...
        // set the default flavor to MySo if not already set by the user
        if config.default_flavor.is_none() {
            config.default_flavor = Some(move_compiler::editions::Flavor::MySo);
        }
    ...
    pub async fn build_async_from_root_pkg(
        self,
        root_pkg: &mut RootPackage<MySoFlavor>,
    ...
    /// If we are building the MySo framework, we skip the check that the addresses should be 0
    ...
    fn internal_build(
        ...
        let (package, fn_info) = result.map_err(|error| {
            MySoError::from(MySoErrorKind::ModuleBuildFailure {
...
fn verify_bytecode(package: &MoveCompiledPackage, fn_info: &FnInfoMap) -> MySoResult<()> {
    ...
    for m in compiled_modules.iter_modules() {
        move_bytecode_verifier::verify_module_unmetered(m).map_err(|err| {
            MySoErrorKind::ModuleVerificationFailure {
                error: err.to_string(),
            }
        })?;
...
impl CompiledPackage {
    ...
    /// Note: these are not topologically sorted by dependency--use `get_dependency_sorted_modules` to produce a list of modules mysotable
    ...
    pub fn get_dependency_sorted_modules(
        ...
        if with_unpublished_deps {
            ...
            // sufficient as we may have modules with the same names in user code and in MySo
    ...
    /// Get bytecode modules from the MySo System that are used by this package
    pub fn get_myso_system_modules(&self) -> impl Iterator<Item = &CompiledModule> {
        self.get_modules_and_deps()
            .filter(|m| *m.self_id().address() == MYSO_SYSTEM_ADDRESS)
    }

    /// Get bytecode modules from the MySo Framework that are used by this package
    pub fn get_myso_framework_modules(&self) -> impl Iterator<Item = &CompiledModule> {
        self.get_modules_and_deps()
            .filter(|m| *m.self_id().address() == MYSO_FRAMEWORK_ADDRESS)
    }
    ...
    /// Get bytecode modules from Contra that are used by this package
    pub fn get_contra_modules(&self) -> impl Iterator<Item = &CompiledModule> {
        self.get_modules_and_deps()
            .filter(|m| *m.self_id().address() == CONTRA_ADDRESS)
    }
    ...
    pub fn verify_unpublished_dependencies(
        ...
        Err(MySoErrorKind::ModulePublishFailure {
