...
fn from_addr_seq(s: &str) -> Result<(MySoAddress, TxSequenceNumber), anyhow::Error> {
    // Remove whitespaces
    let s = s.trim();
    ...
    let address = MySoAddress::from_str(tokens[0].trim())?;
...
fn from_addr_oid(s: &str) -> Result<(MySoAddress, ObjectID), anyhow::Error> {
    // Remove whitespaces
    let s = s.trim();
    let tokens = s.split(',').collect::<Vec<&str>>();
    if tokens.len() != 2 {
    ...
    let addr = MySoAddress::from_str(tokens[0].trim())?;
...
fn from_address_and_event_id(
    s: &str,
) -> Result<(MySoAddress, (TxSequenceNumber, usize)), anyhow::Error> {
    // Example: "0x1 1234 5"
    let tokens = s.split(' ').collect::<Vec<&str>>();
    if tokens.len() != 3 {
        return Err(anyhow!("Invalid input"));
