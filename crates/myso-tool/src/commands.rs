...
use myso_replay::{ReplayToolCommand, execute_replay_command};
...
pub enum ToolCommand {
    ...
    FetchObject {
        ...
        /// prints tabular output mysotable for processing with unix tools. For
        ...
        ///      --genesis $HOME/.myso/myso_config/genesis.blob \
    ...
    /// contains Move bytecode (mysotable for passing into a disassembler).
    ...
    Replay {
        ...
        #[arg(
            ...
            If not specified we will use the default network config file at ~/.myso-replay/network-config.yaml"
        )]
...
async fn check_locked_object(
    ...
    let clients = Arc::new(make_clients(myso_client).await?);
    ...
    let res = myso_client.clone().execute_transaction(&tx).await;
    match res {
        Ok(_) => {
    ...
    pub async fn execute(self, tracing_handle: TracingHandle) -> Result<(), anyhow::Error> {
        match self {
            ...
            ToolCommand::DumpPackages {
                ...
                myso_package_dump::dump(rpc_url, output_dir, before_checkpoint).await?;
            }
                ...
                if !concise {
                    ...
                    for (i, val_info) in genesis.validator_set_for_tooling().iter().enumerate() {
                        ...
                        println!(
                            ...
                            i,
                            metadata.name,
                            metadata.myso_pubkey_bytes().concise(),
            ...
            ToolCommand::RestoreFromDBCheckpoint {
                ...
                let config = myso_config::NodeConfig::load(config_path)?;
