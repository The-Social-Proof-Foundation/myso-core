// Copyright (c) The Social Proof Foundation, LLC.
// SPDX-License-Identifier: Apache-2.0

use chrono::{DateTime, Utc};
use sqlx::PgPool;
use uuid::Uuid;

#[derive(Debug, Clone, sqlx::FromRow, serde::Serialize)]
pub struct MarketRow {
    pub id: Uuid,
    pub post_id: String,
    pub spot_record_id: Option<String>,
    pub creator: Option<String>,
    pub claim_text: String,
    pub betting_options: serde_json::Value,
    pub status: String,
    pub review_id: Option<Uuid>,
    pub resolver_definition_id: Option<Uuid>,
    pub resolution_window_ms: i64,
    pub max_resolution_window_ms: i64,
    pub created_at: DateTime<Utc>,
    pub updated_at: DateTime<Utc>,
}

pub async fn market_exists(pool: &PgPool, post_id: &str) -> anyhow::Result<bool> {
    let row: (bool,) = sqlx::query_as("SELECT EXISTS(SELECT 1 FROM markets WHERE post_id = $1)")
        .bind(post_id)
        .fetch_one(pool)
        .await?;
    Ok(row.0)
}

pub async fn insert_market(
    pool: &PgPool,
    post_id: &str,
    creator: &str,
    claim_text: &str,
) -> anyhow::Result<Uuid> {
    let row: Option<(Uuid,)> = sqlx::query_as(
        r#"
        INSERT INTO markets (post_id, creator, claim_text, status)
        VALUES ($1, $2, $3, 'pending_review')
        ON CONFLICT (post_id) DO NOTHING
        RETURNING id
        "#,
    )
    .bind(post_id)
    .bind(creator)
    .bind(claim_text)
    .fetch_optional(pool)
    .await?;
    if let Some(id) = row {
        return Ok(id.0);
    }
    let existing: (Uuid,) = sqlx::query_as("SELECT id FROM markets WHERE post_id = $1")
        .bind(post_id)
        .fetch_one(pool)
        .await?;
    Ok(existing.0)
}

pub async fn get_market_by_post_id(pool: &PgPool, post_id: &str) -> anyhow::Result<Option<MarketRow>> {
    let row = sqlx::query_as::<_, MarketRow>(
        r#"
        SELECT id, post_id, spot_record_id, creator, claim_text, betting_options, status,
               review_id, resolver_definition_id, resolution_window_ms, max_resolution_window_ms,
               created_at, updated_at
        FROM markets WHERE post_id = $1
        "#,
    )
    .bind(post_id)
    .fetch_optional(pool)
    .await?;
    Ok(row)
}

pub async fn get_market(pool: &PgPool, market_id: Uuid) -> anyhow::Result<Option<MarketRow>> {
    let row = sqlx::query_as::<_, MarketRow>(
        r#"
        SELECT id, post_id, spot_record_id, creator, claim_text, betting_options, status,
               review_id, resolver_definition_id, resolution_window_ms, max_resolution_window_ms,
               created_at, updated_at
        FROM markets WHERE id = $1
        "#,
    )
    .bind(market_id)
    .fetch_optional(pool)
    .await?;
    Ok(row)
}

pub async fn list_markets(
    pool: &PgPool,
    status: Option<&str>,
    limit: i64,
) -> anyhow::Result<Vec<MarketRow>> {
    let rows = if let Some(status) = status {
        sqlx::query_as::<_, MarketRow>(
            r#"
            SELECT id, post_id, spot_record_id, creator, claim_text, betting_options, status,
                   review_id, resolver_definition_id, resolution_window_ms, max_resolution_window_ms,
                   created_at, updated_at
            FROM markets WHERE status = $1
            ORDER BY created_at DESC LIMIT $2
            "#,
        )
        .bind(status)
        .bind(limit)
        .fetch_all(pool)
        .await?
    } else {
        sqlx::query_as::<_, MarketRow>(
            r#"
            SELECT id, post_id, spot_record_id, creator, claim_text, betting_options, status,
                   review_id, resolver_definition_id, resolution_window_ms, max_resolution_window_ms,
                   created_at, updated_at
            FROM markets ORDER BY created_at DESC LIMIT $1
            "#,
        )
        .bind(limit)
        .fetch_all(pool)
        .await?
    };
    Ok(rows)
}

pub async fn update_market_status(
    pool: &PgPool,
    market_id: Uuid,
    status: &str,
    review_id: Option<Uuid>,
    resolver_definition_id: Option<Uuid>,
    betting_options: Option<&serde_json::Value>,
) -> anyhow::Result<()> {
    sqlx::query(
        r#"
        UPDATE markets SET
            status = $2,
            review_id = COALESCE($3, review_id),
            resolver_definition_id = COALESCE($4, resolver_definition_id),
            betting_options = COALESCE($5, betting_options),
            updated_at = NOW()
        WHERE id = $1
        "#,
    )
    .bind(market_id)
    .bind(status)
    .bind(review_id)
    .bind(resolver_definition_id)
    .bind(betting_options)
    .execute(pool)
    .await?;
    Ok(())
}

pub async fn set_spot_record_id(
    pool: &PgPool,
    market_id: Uuid,
    spot_record_id: &str,
) -> anyhow::Result<()> {
    sqlx::query(
        "UPDATE markets SET spot_record_id = $2, status = 'active', updated_at = NOW() WHERE id = $1",
    )
    .bind(market_id)
    .bind(spot_record_id)
    .execute(pool)
    .await?;
    Ok(())
}

pub async fn max_ingested_created_at_ms(pool: &PgPool) -> anyhow::Result<Option<i64>> {
    let row: Option<(Option<i64>,)> = sqlx::query_as(
        r#"
        SELECT MAX((payload->>'created_at_ms')::bigint)
        FROM spot_jobs
        WHERE job_type = 'ReviewPost'
        "#,
    )
    .fetch_optional(pool)
    .await?;
    Ok(row.and_then(|r| r.0))
}
