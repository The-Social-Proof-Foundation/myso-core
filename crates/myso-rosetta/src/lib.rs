...
use myso_sdk_types::{StructTag, TypeTag as SDKTypeTag};
...
use crate::types::{Currency, CurrencyMetadata, MySoEnv};
...
pub static MYSO: Lazy<Currency> = Lazy::new(|| Currency {
...
impl RosettaOnlineServer {
    pub fn new(env: MySoEnv, client: Client) -> Self {
    ...
    pub async fn serve(self, addr: SocketAddr) {
        ...
        info!(
            "MySo Rosetta online server listening on {}",
...
impl RosettaOfflineServer {
    ...
    pub async fn serve(self, addr: SocketAddr) {
        ...
        info!(
            "MySo Rosetta offline server listening on {}",
            listener.local_addr().unwrap()
        );
        axum::serve(listener, app).await.unwrap();
    }
