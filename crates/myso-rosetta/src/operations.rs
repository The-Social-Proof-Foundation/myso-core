...
use myso_rpc::proto::myso::rpc::v2::BalanceChange;
use myso_rpc::proto::myso::rpc::v2::ExecutedTransaction;
use myso_rpc::proto::myso::rpc::v2::Input;
use myso_rpc::proto::myso::rpc::v2::MoveCall;
use myso_rpc::proto::myso::rpc::v2::ProgrammableTransaction;
use myso_rpc::proto::myso::rpc::v2::TransactionKind;
use myso_rpc::proto::myso::rpc::v2::argument::ArgumentKind;
use myso_rpc::proto::myso::rpc::v2::command::Command;
...
use myso_types::{MYSO_SYSTEM_ADDRESS, MYSO_SYSTEM_PACKAGE_ID};

use crate::types::internal_operation::{PayCoin, PayMySo, Stake, WithdrawStake};
use crate::types::{
...
use crate::{CoinMetadataCache, Error, MYSO};
...
impl Operations {
    ...
    pub fn into_internal(self) -> Result<InternalOperation, Error> {
        ...
        match type_ {
            OperationType::PayMySo => self.pay_myso_ops_to_internal(),
    ...
    fn pay_myso_ops_to_internal(self) -> Result<InternalOperation, Error> {
        ...
        Ok(InternalOperation::PayMySo(PayMySo {
            sender,
            recipients,
            amounts,
        }))
    ...
    fn parse_programmable_transaction(
        ...
        fn transfer_object(
            aggregated_recipients: &mut HashMap<MySoAddress, u64>,
            inputs: &[Input],
            known_results: &[Vec<KnownValue>],
            objs: &[Argument],
            recipient: &Argument,
            ...
            let addr = match recipient.kind() {
                ArgumentKind::Input => {
                    ...
                    match input.kind() {
                        InputKind::Pure => {
                            let bytes = input.pure();
                            bcs::from_bytes::<MySoAddress>(bytes).ok()?
                        }
                        _ => return None,
                    }
                }
        ...
        fn stake_call(
            ...
            known_results: &[Vec<KnownValue>],
            call: &MoveCall,
        ) -> Result<Option<(Option<u64>, MySoAddress)>, Error> {
                        ...
                        ArgumentKind::Input => {
                            ...
                            let validator_addr = match inputs.get(i as usize) {
                                Some(input) if input.kind() == InputKind::Pure => {
                                    bcs::from_bytes::<MySoAddress>(input.pure()).ok()
                                }
                                _ => None,
                            };
        ...
        let mut aggregated_recipients: HashMap<MySoAddress, u64> = HashMap::new();
                ...
                aggregated_recipients
                    ...
                    .map(|(recipient, amount)| {
                        ...
                        match currency {
                            ...
                            None => Operation::pay_myso(status, recipient, amount.into()),
                        }
                    }),
            );
            match currency {
                ...
                _ => operations.push(Operation::pay_myso(status, sender, -(total_paid as i128))),
            }
    ...
    fn is_stake_call(tx: &MoveCall) -> bool {
        ...
        package_id == MYSO_SYSTEM_PACKAGE_ID
            && tx.module() == MYSO_SYSTEM_MODULE_NAME.as_str()
    ...
    fn process_balance_change(
        gas_owner: MySoAddress,
        gas_used: i128,
        balance_changes: &[(BalanceChange, Currency)],
        status: Option<OperationStatus>,
        balances: HashMap<(MySoAddress, Currency), i128>,
                ...
                .fold(balances, |mut balances, (balance_change, ccy)| {
                    if let (Some(addr_str), Some(amount_str)) =
                        ...
                        && let (Ok(owner), Ok(amount)) =
                            (MySoAddress::from_str(addr_str), i128::from_str(amount_str))
                    {
        ...
        *balances.entry((gas_owner, MYSO.clone())).or_default() -= gas_used;
    ...
    fn validate_operations(
        ...
        let balances: HashMap<(MySoAddress, Currency), i128> = HashMap::new();
    ...
    fn process_gascoin_transfer(
        ...
        prev_gas_owner: MySoAddress,
        new_gas_owner: MySoAddress,
        gas_used: i128,
        initial_balance_changes: &[(BalanceChange, Currency)],
        ...
        if is_gascoin_transfer && prev_gas_owner != new_gas_owner {
            ...
            for operation in &mut operations {
                match operation.type_ {
                    ...
                    OperationType::MySoBalanceChange => {
                        ...
                        if account.address == prev_gas_owner && amount.currency == *MYSO {
                            amount.value -= gas_used;
                        } else if account.address == new_gas_owner && amount.currency == *MYSO {
                            amount.value += gas_used;
                        }
                    }
                    _ => {
...
impl Operations {
    pub async fn try_from_executed_transaction(
        ...
        let sender = MySoAddress::from_str(transaction.sender())?;

        let gas_owner = if effects.gas_object.is_some() {
            let gas_object = effects.gas_object();
            let owner = gas_object.output_owner();
            MySoAddress::from_str(owner.address())?
        } else if sender == MySoAddress::ZERO {
        ...
        let prev_gas_owner = MySoAddress::from_str(transaction.gas_payment().owner())?;
        ...
        for balance_change in &balance_changes {
            ...
            let type_tag = myso_types::TypeTag::from_str(coin_type)
        ...
        // and convert BalanceChange to PayMySo when GasCoin is transferred
        ...
        // This is a workaround for the payCoin cases that are mistakenly considered to be payMySo operations
        // In this case we remove any irrelevant, MYSO specific operation entries that sum up to 0 balance changes per address
        ...
        let mutually_cancelling_balances: HashMap<_, _> = ops
            ...
            .fold(
                HashMap::new(),
                |mut balances: HashMap<(MySoAddress, Currency), i128>, op| {
                    if let (Some(acc), Some(amount), Some(OperationStatus::Success)) =
                        (&op.account, &op.amount, &op.status)
...
fn is_unstake_event(tag: &StructTag) -> bool {
    tag.address == MYSO_SYSTEM_ADDRESS
...
impl Operation {
    ...
    pub fn genesis(index: u64, sender: MySoAddress, coin: GasCoin) -> Self {
        Operation {
    ...
    fn pay_myso(status: Option<OperationStatus>, address: MySoAddress, amount: i128) -> Self {
        Operation {
            operation_identifier: Default::default(),
            type_: OperationType::PayMySo,
            status,
    ...
    fn balance_change(
        ...
        Self {
            operation_identifier: Default::default(),
            type_: OperationType::MySoBalanceChange,
            status,
    ...
    fn gas(addr: MySoAddress, amount: i128) -> Self {
        Self {
            operation_identifier: Default::default(),
    ...
    fn stake_reward(status: Option<OperationStatus>, addr: MySoAddress, amount: i128) -> Self {
        Self {
            operation_identifier: Default::default(),
    ...
    fn stake_principle(status: Option<OperationStatus>, addr: MySoAddress, amount: i128) -> Self {
        Self {
            operation_identifier: Default::default(),
...
mod tests {
    ...
    #[tokio::test]
    async fn test_operation_data_parsing_pay_myso() -> Result<(), anyhow::Error> {
        let gas = (
            ObjectID::random(),
            SequenceNumber::new(),
            ObjectDigest::random(),
        ...
        let pt = {
            ...
            builder
                .pay_myso(vec![MySoAddress::random_for_testing_only()], vec![10000])
                .unwrap();
            builder.finish()
        };
        let gas_price = 10;
        ...
        ops.0
            .iter()
            .for_each(|op| assert_eq!(op.type_, OperationType::PayMySo));
    ...
    async fn test_operation_data_parsing_pay_coin() -> Result<(), anyhow::Error> {
        ...
        let pt = {
            ...
            builder.pure(serde_json::to_string(&MYSO.clone())?)?;
            builder.finish()
        };
        let gas_price = 10;
        let data = TransactionData::new_programmable(
