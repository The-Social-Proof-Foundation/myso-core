...
pub struct PayMySo {
    ...
    pub recipients: Vec<MySoAddress>,
    pub amounts: Vec<u64>,
}

#[async_trait]
impl TryConstructTransaction for PayMySo {
    async fn try_fetch_needed_objects(
        ...
        // PayMySo needs enough MYSO to cover both payment amount and gas. We select up to 1500
        ...
        // [40, 35, 25] MYSO coins and needing to pay 50 + gas, no discrete set works - we must
        ...
        // Use the full Coin<MYSO> struct tag
        ...
        let total_myso_balance = all_coins.iter().map(|c| c.balance()).sum::<u64>() as i128;
        ...
        let pt = pay_myso_pt(recipients, amounts, &extra_coins, &extra_party_coins)?;
...
/// Creates the `ProgrammableTransaction` for a pay-myso operation.
/// In case pay-myso needs more than 255 gas-coins to be smashed, it tries to merge the surplus
