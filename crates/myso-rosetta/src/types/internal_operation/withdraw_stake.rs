    ...
    async fn try_fetch_needed_objects(
        ...
        let stake_ids = if withdraw_all {
            ...
            let list_request = ListOwnedObjectsRequest::default()
                ...
                .with_object_type("0x3::staking_pool::StakedMySo".to_string())
        ...
        for stake_id in &stake_ids {
            let stake_address: myso_sdk_types::Address = (*stake_id).into();
        ...
        let total_myso_balance = gas_coin_objs.iter().map(|c| c.balance()).sum::<u64>() as i128;
...
pub fn withdraw_stake_pt(
    ...
    for stake_id in stake_objs {
        ...
        let (system_state, id) = if !withdraw_all {
            let system_state = builder.input(CallArg::MYSO_SYSTEM_MUT)?;
