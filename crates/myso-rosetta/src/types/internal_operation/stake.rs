...
impl TryConstructTransaction for Stake {
    async fn try_fetch_needed_objects(
        ...
        // Staking needs enough MYSO to cover both the stake amount and gas. We select up to 1500
        ...
        // [8, 6, 4] MYSO coins and wanting to stake 10 + gas, no discrete set works - we must
        ...
        let simulation_amount = amount.unwrap_or(1_000_000_000); // 1 MYSO minimum staking threshold
...
pub fn stake_pt(
    ...
    let (validator, system_state) = if !stake_all {
        ...
        let state = builder.input(CallArg::MYSO_SYSTEM_MUT)?;
        (validator, state)
    } else {
        let state = builder.input(CallArg::MYSO_SYSTEM_MUT)?;
        let validator = builder.input(CallArg::Pure(bcs::to_bytes(&validator)?))?;
