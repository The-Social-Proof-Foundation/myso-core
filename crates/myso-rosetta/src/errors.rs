...
/// MySo-Rosetta specific error types.
/// This contains all the errors returns by the myso-rosetta server.
...
pub enum Error {
    ...
    #[error(transparent)]
    MySoRpcError(#[from] tonic::Status),
    #[error(transparent)]
...
impl From<MySoErrorKind> for Error {
    fn from(e: MySoErrorKind) -> Self {
        Error::MySoError(MySoError::from(e))
    }
}
