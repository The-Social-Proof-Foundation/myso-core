...
use myso_rpc::proto::myso::rpc::v2::ExecutionStatus;
...
impl MySoEnv {
    pub fn check_network_identifier(
        ...
        if &network_identifier.blockchain != "myso" {
...
impl From<MySoAddress> for AccountIdentifier {
...
impl From<MySoPublicKey> for PublicKey {
    fn from(pk: MySoPublicKey) -> Self {
        match pk {
            MySoPublicKey::Ed25519(k) => PublicKey {
                hex_bytes: Hex::from_bytes(&k.0),
                curve_type: CurveType::Edwards25519,
            },
            MySoPublicKey::Secp256k1(k) => PublicKey {
                hex_bytes: Hex::from_bytes(&k.0),
                curve_type: CurveType::Secp256k1,
            },
            MySoPublicKey::Secp256r1(k) => PublicKey {
                hex_bytes: Hex::from_bytes(&k.0),
                curve_type: CurveType::Secp256r1,
            },
            MySoPublicKey::ZkLogin(k) => PublicKey {
                hex_bytes: Hex::from_bytes(&k.0),
                curve_type: CurveType::ZkLogin, // inaccurate but added for completeness.
            },
            MySoPublicKey::Passkey(k) => PublicKey {
                hex_bytes: Hex::from_bytes(&k.0),
                curve_type: CurveType::Secp256r1,
            },
        }
...
impl TryInto<MySoAddress> for PublicKey {
    type Error = Error;

    fn try_into(self) -> Result<MySoAddress, Self::Error> {
        let key_bytes = self.hex_bytes.to_vec()?;
        let pub_key = MySoPublicKey::try_from_bytes(self.curve_type.into(), &key_bytes)?;
        Ok((&pub_key).into())
    }
}
...
pub enum OperationType {
    ...
    StakeReward,
    StakePrinciple,
    // myso-rosetta supported operation type
    PayMySo,
    PayCoin,
    Stake,
    WithdrawStake,
    // All other MySo transaction types, readonly
    EpochChange,
    Genesis,
...
pub struct ConstructionMetadata {
    ...
    /// `Coin<MYSO>` objects to be used as gas
    ...
    /// Always refers to MYSO balance used
    #[serde(with = "str_format")]
