...
#[derive(Parser)]
#[clap(name = "myso-rosetta", rename_all = "kebab-case", author, version)]
        ...
        match self {
            RosettaServerCommand::GenerateRosettaCLIConfig {
                ...
                let path = keystore_path
                    .unwrap_or_else(|| myso_config_dir().unwrap().join(MYSO_KEYSTORE_FILENAME));

                let prefunded_accounts = read_prefunded_account(&path)?;

                info!(
                    "Retrieved {} MySo address from keystore file {:?}",
                ...
                let dsl_path = PathBuf::from(".").join("myso.ros");
                let dsl = include_str!("../resources/myso.ros");
                fs::write(
                    &dsl_path,
                    dsl.replace("{{myso.env}}", json!(env).as_str().unwrap()),
                )?;
            ...
            RosettaServerCommand::StartOnlineRemoteServer {
                ...
                info!(
                    "Starting Rosetta Online Server with remote MySo full node [{full_node_url}]."
                );
...
/// This method reads the keypairs from the MySo keystore to create the PrefundedAccount objects,
