...
MySo-Rosetta is an implementation of the Mesh API (f.k.a. Rosetta API) for the MySo network, the MySo-Rosetta server uses the MySo Full Node to
read and write transactions to the MySo network.
...
`./myso genesis -f`  
The MySo genesis process will create configs and coins for testing, the config files are stored in `~/.myso/myso_config` by
default.
...
`./myso-rosetta start-online-server`
...
`./myso-rosetta generate-rosetta-cli-config`
This will generate the `rosetta-cli.json` and `myso.ros` file to be used by the [Rosetta-CLI](https://github.com/coinbase/rosetta-cli)
...
```shell
cd <myso project directory>/crate/myso-rosetta/docker/myso-rosetta-local
```   
#### 2. Build the image
```shell
./build.sh
...
```shell
docker run mysten/myso-rosetta-devnet myso-rosetta start-online-server
```
Alternatively, to start the rosetta-offline server, run
```shell
docker run mysten/myso-rosetta-devnet myso-rosetta start-offline-server
```
...
| Method | Endpoint         | Description                    | MySo Supported? |  Server Type  |
|--------|------------------|--------------------------------|:--------------:|:-------------:|
...
| Method | Endpoint           | Description             |                                      MySo Supported?                                       |  Server Type  |
|--------|--------------------|-------------------------|:-----------------------------------------------------------------------------------------:|:-------------:|
| POST   | /block             | Get a Block             | Yes (One transaction per block in phase 1, will be replaced by MySo checkpoint in phase 2) |    Online     |
...
## MySo transaction <> Rosetta Operation conversion explained
There are 2 places we convert MySo's transaction to Rosetta's operations, 
...
the `/construction/parse` endpoint is used for checking transaction correctness during transaction construction, in our case we only support `TransferMySo` for now, the operations created looks like this (negative amount indicate sender):
```json
{
...
Since the intent operations of TransferMySo contains all the balance change information(amount field) already, 
