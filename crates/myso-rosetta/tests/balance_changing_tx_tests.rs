...
use myso_rosetta::types::{ConstructionMetadata, OperationStatus, OperationType};
...
async fn test_transfer_myso() {
    ...
    let pt = {
        ...
        builder.transfer_myso(recipient, Some(50000));
        builder.finish()
    };
    test_transaction(
        &network,
...
#[tokio::test]
async fn test_transfer_myso_whole_coin() {
    let network = TestClusterBuilder::new().build().await;
    let keystore = &network.wallet.config.keystore;
    let mut client = GrpcClient::new(network.rpc_url()).unwrap();
    let rgp = client.get_reference_gas_price().await.unwrap();
    ...
    let pt = {
        let mut builder = ProgrammableTransactionBuilder::new();
        builder.transfer_myso(recipient, None);
        builder.finish()
    };
    test_transaction(
        &network,
...
async fn test_transfer_object() {
    ...
    let sender = get_random_address(&addresses, vec![]);
    let recipient = get_random_address(&addresses, vec![sender]);
    let object_ref = get_random_myso(&mut client, sender, vec![]).await;
    let pt = {
        let mut builder = ProgrammableTransactionBuilder::new();
        builder
...
async fn test_split_coin() {
    ...
    let coin = get_random_myso(&mut client, sender, vec![]).await;
    let pt = {
        let mut builder = ProgrammableTransactionBuilder::new();
...
async fn test_merge_coin() {
    ...
    let sender = get_random_address(&network.get_addresses(), vec![]);
    let coin = get_random_myso(&mut client, sender, vec![]).await;
    let coin2 = get_random_myso(&mut client, sender, vec![coin.0]).await;
    let pt = {
        let mut builder = ProgrammableTransactionBuilder::new();
...
async fn test_pay_multiple_coin_multiple_recipient() {
    ...
    let coin1 = get_random_myso(&mut client, sender, vec![]).await;
    let coin2 = get_random_myso(&mut client, sender, vec![coin1.0]).await;
    let pt = {
        let mut builder = ProgrammableTransactionBuilder::new();
        builder
            .pay(
...
#[tokio::test]
async fn test_pay_myso_multiple_coin_same_recipient() {
    let network = TestClusterBuilder::new().build().await;
    let keystore = &network.wallet.config.keystore;
    let mut client = GrpcClient::new(network.rpc_url()).unwrap();
    let rgp = client.get_reference_gas_price().await.unwrap();
...
async fn test_pay_myso() {
    ...
    let pt = {
        let mut builder = ProgrammableTransactionBuilder::new();
        builder
            .pay_myso(vec![recipient1, recipient2], vec![1000000, 2000000])
            .unwrap();
        builder.finish()
    };
    test_transaction(
...
#[tokio::test]
async fn test_stake_myso_with_none_amount() {
    let network = TestClusterBuilder::new().build().await;
    let keystore = &network.wallet.config.keystore;
    let mut client = GrpcClient::new(network.rpc_url()).unwrap();
    let rgp = client.get_reference_gas_price().await.unwrap();
...
async fn test_delegation_parsing() -> Result<(), anyhow::Error> {
    ...
    let sender = get_random_address(&network.get_addresses(), vec![]);
    let mut client = GrpcClient::new(network.rpc_url()).unwrap();
    let gas = get_random_myso(&mut client, sender, vec![]).await;
    ...
    let proto_tx: myso_rpc::proto::myso::rpc::v2::Transaction = parsed_data.clone().into();
...
// Record current MySo balance of an address then execute the transaction,
...
async fn test_transaction(
    ...
    let gas = if !gas.is_empty() {
        ...
        let input_objects = tx
            ...
            .collect::<Vec<_>>();
        vec![get_random_myso(&mut client, sender, input_objects).await]
    };
    ...
    let signature = keystore
        .sign_secure(&data.sender(), &data, Intent::myso_transaction())
        .await
        .unwrap();
...
fn extract_balance_changes_from_ops(ops: Operations) -> HashMap<MySoAddress, i128> {
    ops.into_iter()
        .fold(HashMap::<MySoAddress, i128>::new(), |mut changes, op| {
            if let Some(OperationStatus::Success) = op.status {
                match op.type_ {
                    OperationType::MySoBalanceChange
                    | OperationType::Gas
                    | OperationType::PayMySo
                    ...
                    | OperationType::Stake => {
                        if let (Some(addr), Some(amount)) = (op.account, op.amount) {
                            ...
                            if amount.currency.metadata.coin_type
                                == myso_types::TypeTag::from(GAS::type_()).to_canonical_string(true)
                            {
...
fn get_random_address(addresses: &[MySoAddress], except: Vec<MySoAddress>) -> MySoAddress {
    *addresses
        .iter()
...
async fn get_balance(client: &mut GrpcClient, address: MySoAddress) -> u64 {
