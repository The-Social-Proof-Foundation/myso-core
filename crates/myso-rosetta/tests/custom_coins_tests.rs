...
async fn test_custom_coin_balance() {
    const MYSO_BALANCE: u64 = 150_000_000_000_000_000;
    const COIN1_BALANCE: u64 = 100_000_000;
    const COIN2_BALANCE: u64 = 200_000_000;
    let test_cluster = TestClusterBuilder::new().build().await;
    let keystore = &test_cluster.wallet.config.keystore;
    ...
    let request = AccountBalanceRequest {
        ...
        currencies: Currencies(vec![myso_currency, test_coin_currency]),
    };
    ...
    assert_eq!(response.balances[0].value, MYSO_BALANCE as i128);
    assert_eq!(
...
async fn test_custom_coin_without_symbol() {
    ...
    for op in ops {
        if op.type_ == OperationType::MySoBalanceChange {
