...
async fn test_make_change_exact_div() -> Result<()> {
    const MYSO_100: u64 = 100_000_000_000;
    const MYSO_10: u64 = 10_000_000_000;
    ...
    if get_coin_value(&new_coin) != MYSO_100 {
        return Err(anyhow!(
    ...
    assert!(
        new_coins_count == (MYSO_100 / MYSO_10) as usize - 1,
...
async fn test_make_change_remainder_div() -> Result<()> {
    const MYSO_100: u64 = 100_000_000_000;
    const MYSO_12: u64 = 12_000_000_000;
    ...
    assert!(
        new_coins.len() == (MYSO_100 / MYSO_12) as usize,
        "Expected {} new coins. Got: {}",
        MYSO_100 / MYSO_12,
        new_coins.len()
    );
    ...
    for result in batch_response.objects {
        ...
        match b {
            ...
            b => {
                return Err(anyhow!(
                    "Did not expect anything else other than 12 or 4 MYSO. Found {b}"
                ));
            }
        }
    }

    assert!(twelve_count == 8, "Expected 8 coins with 12 MYSO");
    assert!(four_count == 1, "Expected 1 coin with 4 MYSO");
