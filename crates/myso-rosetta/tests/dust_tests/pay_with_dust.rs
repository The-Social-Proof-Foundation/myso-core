...
use myso_rosetta::types::PreprocessMetadata;
...
async fn test_pay_with_many_small_coins_fail_insufficient_balance_budget_none() -> Result<()> {
    ...
    let mut ptb = ProgrammableTransactionBuilder::new();
    ptb.pay_myso(vec![recipient], vec![send_amount_to_decrease_balance])?;
    let tx_data = TransactionData::new_programmable(
        sender,
    ...
    let effects: myso_types::effects::TransactionEffects = effects_bcs.deserialize().unwrap();
