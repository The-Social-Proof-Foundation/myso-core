...
use myso_types::{Identifier, MYSO_SYSTEM_PACKAGE_ID};
use test_cluster::TestClusterBuilder;
use test_utils::{
    execute_transaction, get_all_coins, get_object_ref, get_random_myso, wait_for_transaction,
};
...
#[tokio::test]
async fn test_get_staked_myso() -> Result<()> {
    ...
    let network_identifier = NetworkIdentifier {
        blockchain: "myso".to_string(),
        network: MySoEnv::LocalNet,
    };
    ...
    let arguments = vec![
        ptb.input(CallArg::MYSO_SYSTEM_MUT)?,
    ...
    ptb.command(Command::move_call(
        ...
        MYSO_SYSTEM_MODULE_NAME.to_owned(),
...
async fn test_stake_all() {
    ...
    // Create test cluster with 150K MYSO instead of the default 150M
    // 150K MYSO = 150_000_000_000_000 MIST (150K * 1e9)
    const AMOUNT_150K_MYSO: u64 = 150_000_000_000_000;

    // Create 5 test accounts, but only the first one gets custom amount
    let accounts = (0..5)
        .map(|_| AccountConfig {
            address: None,
            gas_amounts: vec![AMOUNT_150K_MYSO], // Single gas object with 150K MYSO
        })
        .collect();
...
#[tokio::test]
async fn test_pay_myso_multiple_times() {
    let test_cluster = TestClusterBuilder::new()
...
async fn test_transfer_single_gas_coin() {
    ...
    let input_objects = pt
        ...
        .collect::<Vec<_>>();
    let gas = vec![get_random_myso(&mut client.clone(), sender, input_objects).await];
    ...
    let signature = keystore
        .sign_secure(&sender, &data, Intent::myso_transaction())
        .await
        .unwrap();
    ...
    operations.into_iter().for_each(|op| {
        ...
        if op.type_ == OperationType::PayMySo {
...
/// The below MySoTransactionBlockResponse is created by using the below contract:
/// ```move
/// module vault::vault;
///
...
/// }
///
/// public fun from_coin(coin: Coin<MYSO>, ctx: &mut TxContext): Vault {
///     Vault {
///         id: object::new(ctx),
...
/// }
///
/// public fun to_coin(self: Vault, ctx: &mut TxContext): Coin<MYSO> {
///     let Vault { id, balance } = self;
///     id.delete();
///     balance.into_coin(ctx)
/// }
///
/// public fun amount_to_coin(self: &mut Vault, amount: u64, ctx: &mut TxContext): Coin<MYSO> {
...
async fn test_balance_from_obj_paid_eq_gas() {
    ...
    let sender_address = MySoAddress::from_str(SENDER).unwrap();
    let recipient_address = MySoAddress::from_str(RECIPIENT).unwrap();
    ...
    let tx_data = TestTransactionBuilder::new(sender_address, gas_ref, 1000)
        .transfer_myso(Some(AMOUNT as u64), recipient_address)
        .build();
    ...
    let effects = TestEffectsBuilder::new(&signed_tx)
        .with_status(myso_types::execution_status::ExecutionStatus::Success)
        .build();
    ...
    let mut balance_changes: HashMap<MySoAddress, i128> = HashMap::new();
    operations.into_iter().for_each(|op| {
    ...
    assert_eq!(
        *balance_changes
            .get(&MySoAddress::from_str(RECIPIENT).unwrap())
            .unwrap(),
        AMOUNT
    );
    assert_eq!(
        *balance_changes
            .get(&MySoAddress::from_str(SENDER).unwrap())
            .unwrap_or(&0),
        0
    );
}
...
async fn test_stake_with_party_objects() {
    ...
    // Stake amount must exceed regular coins (60M MYSO) to force party coin usage
    let stake_amount = "70000000000000000"; // 70M MYSO in MIST
    let ops = serde_json::from_value(json!(
        [{
            "operation_identifier":{"index":0},
            "type":"Stake",
...
#[tokio::test]
async fn test_pay_myso_with_party_objects() {
    let test_cluster = TestClusterBuilder::new().build().await;
    let sender = test_cluster.get_address_0();
    let recipient = test_cluster.get_address_1();
    let keystore = &test_cluster.wallet.config.keystore;
    ...
    // Pay MYSO with party objects present
    // Payment amount must exceed regular coins (60M MYSO) to force party coin usage
    let payment_amount = "70000000000000000"; // 70M MYSO in MIST
    let ops = serde_json::from_value(json!(
        [{
            "operation_identifier":{"index":0},
            "type":"PayMySo",
    ...
    assert!(
        tx.effects().status().success(),
        "Pay MYSO with party objects failed: {:?}",
        tx.effects().status().error()
    );
}
...
async fn test_pay_coin_with_party_objects() {
    ...
    let coin_type_tag: myso_sdk_types::TypeTag = coin_type.parse().unwrap();
    let custom_coins = client
        .select_up_to_n_largest_coins(
            &myso_sdk_types::Address::from(sender),
            &coin_type_tag,
            5,
            &[],
        )
...
async fn create_party_objects_with_type(
    ...
    let exclude_addrs: Vec<myso_sdk_types::Address> = coins_to_convert
        .iter()
        .map(|coin_ref| myso_sdk_types::Address::from(coin_ref.0.into_bytes()))
        .collect();

    let myso_type = myso_sdk_types::TypeTag::from_str("0x2::myso::MYSO")?;
...
async fn test_network_list() -> Result<()> {
    ...
    assert_eq!(networks[0]["blockchain"].as_str().unwrap(), "myso");
...
async fn test_account_coins() -> Result<()> {
    ...
    for coin in coins {
        ...
        assert_eq!(currency_symbol, "MYSO");
