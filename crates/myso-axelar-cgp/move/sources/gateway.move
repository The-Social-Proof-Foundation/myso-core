...
///
/// - When call approvals is sent to MySo, it targets an object and not a module;
/// - To support cross-chain messaging, a Channel object has to be created;
/// - Channel can be either owned or shared but not frozen;
/// - Module developer on the MySo side will have to implement a system to support messaging;
...
module axelar::gateway {
    ...
    use axelar::utils::to_myso_signed;
    ...
    /// For when approval chainId is not MYSO.
    ...
    /// Emitted when a new message is sent from the MYSO network.
    ...
    entry fun process_commands(
        ...
        let mut allow_operatorship_transfer = validate_proof(validators, to_myso_signed(*&data), proof);
    ...
    fun test_execute() {
        use myso::test_scenario::{Self as ts, ctx};
