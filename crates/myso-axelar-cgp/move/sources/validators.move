...
module axelar::validators {
    ...
    #[test]
    /// Tests `ecrecover`, makes sure external signing process works with MySo ecrecover.
    ...
    fun test_ecrecover() {
        ...
        normalize_signature(&mut signature);
        let pubkey = ecdsa::secp256k1_ecrecover(&signature, &to_myso_signed(message), 0);

        assert!(pubkey == SIGNER, 0);
    }

    #[test]
    /// Tests "MySo Signed Message" prefix addition ecrecover.
    ...
    fun test_to_signed() {
        ...
        let pub_key = ecdsa::secp256k1_ecrecover(&signature, &to_myso_signed(message), 0);
