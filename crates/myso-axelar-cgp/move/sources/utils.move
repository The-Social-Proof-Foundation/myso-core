...
module axelar::utils {
    ...
    /// Prefix for MySo Messages.
    const PREFIX: vector<u8> = b"\x19MySo Signed Message:\n";

    /// Normalize last byte of the signature. Have it 1 or 0.
    /// See https://tech.mystenlabs.com/cryptography-in-myso-cross-chain-signature-verification/
    ...
    public fun to_myso_signed(bytes: vector<u8>): vector<u8> {
        let mut res = vector[];
