...
```
postgres://postgres:postgrespw@localhost:5432/myso_indexer_alt
```
...
diesel setup                                                                       \
    --database-url="postgres://postgres:postgrespw@localhost:5432/myso_indexer_alt" \
    --migration-dir ../myso-indexer-alt-schema/migrations
```
...
cargo nextest run                \
    -p myso-indexer-alt           \
    -p myso-indexer-alt-framework \
    -p myso-indexer-alt-e2e-tests
```
...
### Social indexing (profiles, governance, etc.)

To index social contract events (e.g. `ProfileCreatedEvent` from `0x50c1::profile`),
use the standalone `myso-indexer-alt-social` indexer and `myso-social-server` instead.

```sh
cargo run --bin myso-indexer-alt -- generate-config > indexer_alt_config.toml
```
