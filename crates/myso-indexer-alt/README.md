...
the `social_events` pipeline must be present in the config. The default config
generated by `generate-config` includes it. If using a custom config, ensure the
`[pipeline]` section contains `social_events = {}` to enable social indexing.
