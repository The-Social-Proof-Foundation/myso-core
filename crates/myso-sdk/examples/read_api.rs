...
use myso_sdk::rpc_types::{
    MySoGetPastObjectRequest, MySoObjectDataOptions, MySoTransactionBlockResponseOptions,
};
use myso_sdk::types::base_types::ObjectID;
...
async fn main() -> Result<(), anyhow::Error> {
    let (myso, active_address, _) = setup_for_write().await?;
    ...
    let object_data = object
        .data
        .as_ref()
        .unwrap_or_else(|| panic!("No object data for this MySoObjectResponse {:?}", object));
    let object_id = object_data.object_id;
    let version = object_data.version;

    let myso_data_options = MySoObjectDataOptions {
        show_type: true,
        show_owner: true,
    ...
    let past_object = myso
        .read_api()
        .try_get_parsed_past_object(object_id, version, myso_data_options.clone())
        .await?;
    println!(" *** Past Object *** ");
    println!("{:?}", past_object);
    println!(" *** Past Object ***\n");

    let myso_get_past_object_request = past_object.clone().into_object()?;
    let multi_past_object = myso
        .read_api()
        .try_multi_get_parsed_past_object(
            vec![MySoGetPastObjectRequest {
                object_id: myso_get_past_object_request.object_id,
                version: myso_get_past_object_request.version,
            }],
            myso_data_options.clone(),
        )
        .await?;
    ...
    // Object with options
    let object_with_options = myso
        .read_api()
        .get_object_with_options(myso_get_past_object_request.object_id, myso_data_options)
        .await?;
    ...
    println!("{:?}", myso.read_api().get_chain_identifier().await?);
    println!(" *** Chain identifier ***\n ");

    println!(" *** Protocol Config *** ");
    println!("{:?}", myso.read_api().get_protocol_config(None).await?);
    println!(" *** Protocol Config ***\n ");

    // we make a dummy transaction which returns a transaction digest
    let tx_digest = split_coin_digest(&myso, &active_address).await?;
    println!(" *** Transaction data *** ");
    let tx_response = myso
        .read_api()
    ...
    let tx_blocks = myso.read_api().get_total_transaction_blocks().await?;
