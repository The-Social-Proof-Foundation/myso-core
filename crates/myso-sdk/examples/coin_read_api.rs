...
async fn main() -> Result<(), anyhow::Error> {
    ...
    // use `None` for the default `Coin<MYSO>` which is represented as
    // "0x2::myso::MYSO"
    let coin_type = Some("0x2::myso::MYSO".to_string());
    let coins = myso
        .coin_read_api()
    ...
    let coins_stream = myso.coin_read_api().get_coins_stream(active_address, None);
    ...
    // Select coins based on the provided coin type (MYSO in this example). Use `None` for the default MySo coin
    let select_coins = myso
        .coin_read_api()
    ...
    // or if None is passed, it will use Coin<MYSO> as the coin type
    let balance = myso
        .coin_read_api()
    ...
    let total_balance = myso.coin_read_api().get_all_balances(active_address).await?;
    println!(" *** Balance + Total Balance *** ");
    println!("Balance: {:?}", balance);
    println!("Total Balance: {:?}", total_balance);
    println!(" *** Balance + Total Balance ***\n ");

    // Return the coin metadata for the Coin<MYSO>
