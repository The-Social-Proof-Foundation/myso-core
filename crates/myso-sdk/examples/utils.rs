...
/// Return a myso client to interact with the APIs,
...
pub async fn setup_for_write() -> Result<(MySoClient, MySoAddress, MySoAddress), anyhow::Error> {
    let (client, active_address) = setup_for_read().await?;
    // make sure we have some MYSO (5_000_000 MIST) on this address
...
/// Return a myso client to interact with the APIs and an active address from the local wallet.
///
/// This function sets up a wallet in case there is no wallet locally,
/// and ensures that the active address of the wallet has MYSO on it.
/// If there is no MYSO owned by the active address, then it will request
/// MYSO from the faucet.
pub async fn setup_for_read() -> Result<(MySoClient, MySoAddress), anyhow::Error> {
    let client = MySoClientBuilder::default().build_testnet().await?;
    println!("MySo testnet version is: {}", client.api_version());
...
pub async fn request_tokens_from_faucet(
    ...
    loop {
        let owner = myso_client
            .read_api()
            .get_object_with_options(
                ObjectID::from_str(&coin_id)?,
                MySoObjectDataOptions::new().with_owner(),
            )
            .await?;
...
pub async fn split_coin_digest(
    ...
    let coin = match fetch_coin(myso, sender).await? {
        None => {
            request_tokens_from_faucet(*sender, myso).await?;
            fetch_coin(myso, sender)
                .await?
                .expect("Supposed to get a coin with MYSO, but didn't. Aborting")
        }
        Some(c) => c,
    };
    ...
    // first, we want to split the coin, and we specify how much MYSO (in MIST) we want
    // for the new coin
...
pub async fn retrieve_wallet() -> Result<WalletContext, anyhow::Error> {
    ...
    // check if a wallet exists and if not, create a wallet and a myso client config
    ...
    if !wallet_conf.exists() {
        let keystore = FileBasedKeystore::load_or_create(&keystore_path)?;
        let mut client_config = MySoClientConfig::new(keystore.into());

        client_config.add_env(MySoEnv::testnet());
        client_config.add_env(MySoEnv::devnet());
        client_config.add_env(MySoEnv::localnet());
    ...
    let mut client_config: MySoClientConfig = PersistedConfig::read(&wallet_conf)?;
...
async fn just_for_clippy() -> Result<(), anyhow::Error> {
    ...
    let _digest = split_coin_digest(&myso, &sender).await?;
    Ok(())
}
