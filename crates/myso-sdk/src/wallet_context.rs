...
use crate::myso_client_config::{MySoClientConfig, MySoEnv};
...
pub struct WalletContext {
    pub config: PersistedConfig<MySoClientConfig>,
...
impl WalletContext {
    pub fn new(config_path: &Path) -> Result<Self, anyhow::Error> {
        let config: MySoClientConfig = PersistedConfig::read(config_path).map_err(|err| {
            anyhow!(
    ...
    pub fn new_for_tests(
        ...
        let mut config = MySoClientConfig::new(keystore)
    ...
    pub fn addresses_with_alias(&self) -> Vec<(&MySoAddress, &Alias)> {
    ...
    async fn internal_load_or_cache_chain_id(
        ...
        let mut config_result = MySoClientConfig::load_with_lock(path)?;
    ...
    // TODO: Ger rid of mut
    pub fn active_address(&mut self) -> Result<MySoAddress, anyhow::Error> {
    ...
    pub async fn get_object_owner(&self, id: &ObjectID) -> Result<MySoAddress, anyhow::Error> {
        self.grpc_client()?
            .get_object(*id)
            .await?
            .owner()
    ...
    pub async fn try_get_object_owner(
        &self,
        id: &Option<ObjectID>,
    ) -> Result<Option<MySoAddress>, anyhow::Error> {
        if let Some(id) = id {
    ...
    pub async fn infer_sender(&mut self, gas: &[ObjectID]) -> Result<MySoAddress, anyhow::Error> {
        if gas.is_empty() {
    ...
    pub async fn gas_for_owner_budget(
        ...
        Err(anyhow!(
            "No non-argument gas objects found for this address with value >= budget {budget}. Run myso client gas to check for gas objects."
        ))
    }
    ...
    pub async fn get_one_account(&self) -> anyhow::Result<(MySoAddress, Vec<ObjectRef>)> {
    ...
    pub async fn get_one_gas_object(&self) -> anyhow::Result<Option<(MySoAddress, ObjectRef)>> {
    ...
    pub async fn get_all_accounts_and_gas_objects(
        &self,
    ) -> anyhow::Result<Vec<(MySoAddress, Vec<ObjectRef>)>> {
        let mut result = vec![];
        for address in self.get_addresses() {
            let objects = self
                .gas_objects(address)
    ...
    /// Add an account
    pub async fn add_account(&mut self, alias: Option<String>, keypair: MySoKeyPair) {
