...
//! * <a href="../myso_transaction_builder/struct.TransactionBuilder.html" title="struct myso_transaction_builder::TransactionBuilder">TransactionBuilder</a> - provides functions for building transactions
//!
//! # Usage
//! The main way to interact with the API is through the [MySoClientBuilder],
//! which returns a [MySoClient] object from which the user can access the
//! various APIs.
//!
//! ## Getting Started
//! Add the Rust SDK to the project by running `cargo add myso-sdk` in the root
//! folder of your Rust project.
//!
//! The main building block for the MySo Rust SDK is the [MySoClientBuilder],
//! which provides a simple and straightforward way of connecting to a MySo
//! network and having access to the different available APIs.
//!
//! A simple example that connects to a running MySo local network,
//! the MySo devnet, and the MySo testnet is shown below.
...
//! (see [here](https://github.com/stefan-mysten/myso/tree/rust_sdk_api_examples/crates/myso-sdk/examples#preqrequisites) for more information).
//!
//! ```rust,no_run
...
pub mod myso_client_config;
pub mod verify_personal_message_signature;
pub mod wallet_context;

pub const MYSO_COIN_TYPE: &str = "0x2::myso::MYSO";
...
impl Default for MySoClientBuilder {
    fn default() -> Self {
        Self {
...
impl MySoClientBuilder {
    ...
    /// Set the WebSocket URL for the MySo network
    ...
    /// Returns a [MySoClient] object connected to the MySo network running at the URI provided.
    ///
    /// # Examples
    ///
    /// ```rust,no_run
    ...
    ///         .await?;
    ///
    ///     println!("MySo local version: {:?}", myso.api_version());
    ///     Ok(())
    /// }
    /// ```
    pub async fn build(self, http: impl AsRef<str>) -> MySoRpcResult<MySoClient> {
    ...
    /// Returns a [MySoClient] object that is ready to interact with the local
    /// development network (by default it expects the MySo network to be
    ...
    /// }
    /// ```
    pub async fn build_localnet(self) -> MySoRpcResult<MySoClient> {
        self.build(MYSO_LOCAL_NETWORK_URL).await
    }

    /// Returns a [MySoClient] object that is ready to interact with the MySo devnet.
    ///
    /// For connecting to a custom URI, use the `build` function instead..
    ///
    /// # Examples
    ...
    ///         .await?;
    ///
    ///     println!("{:?}", myso.api_version());
    ///     Ok(())
    /// }
    /// ```
    pub async fn build_devnet(self) -> MySoRpcResult<MySoClient> {
        self.build(MYSO_DEVNET_URL).await
    }

    /// Returns a [MySoClient] object that is ready to interact with the MySo testnet.
    ///
    /// For connecting to a custom URI, use the `build` function instead.
    ///
    /// # Examples
    ...
    /// }
    /// ```
    pub async fn build_testnet(self) -> MySoRpcResult<MySoClient> {
        self.build(MYSO_TESTNET_URL).await
    }

    /// Returns a [MySoClient] object that is ready to interact with the MySo mainnet.
    ///
    /// For connecting to a custom URI, use the `build` function instead.
    ///
    /// # Examples
    ...
    /// }
    /// ```
    pub async fn build_mainnet(self) -> MySoRpcResult<MySoClient> {
        self.build(MYSO_MAINNET_URL).await
    }
...
/// MySoClient is the basic type that provides all the necessary abstractions for interacting with the MySo network.
///
/// # Usage
///
/// Use [MySoClientBuilder] to build a [MySoClient].
///
/// # Examples
///
/// ```rust,no_run
...
///      .await?;
///
///     println!("{:?}", myso.available_rpc_methods());
///     println!("{:?}", myso.available_subscriptions());
///     println!("{:?}", myso.api_version());
///
...
impl MySoClient {
    ...
    pub fn check_api_version(&self) -> MySoRpcResult<()> {
...
impl DataReader for ReadApi {
    async fn get_owned_objects(
        ...
        let mut result = vec![];
        let query = Some(MySoObjectResponseQuery {
            filter: Some(MySoObjectDataFilter::StructType(object_type)),
            options: Some(
    ...
    async fn get_object(&self, object_id: ObjectID) -> Result<Object, anyhow::Error> {
        let resp = self
            .get_object_with_options(object_id, MySoObjectDataOptions::bcs_lossless())
            .await?;
