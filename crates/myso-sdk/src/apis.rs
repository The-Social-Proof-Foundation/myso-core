...
use myso_json_rpc_types::CheckpointPage;
use myso_json_rpc_types::DevInspectArgs;
use myso_json_rpc_types::MySoData;
...
use crate::RpcClient;
use crate::error::{Error, MySoRpcResult};
...
impl ReadApi {
    ...
    ///     let myso = MySoClientBuilder::default().build_localnet().await?;
    ///     let address = MySoAddress::from_str("0x0000....0000")?;
    ///     let owned_objects = myso
    ///         .read_api()
    ...
    ///         "No object data for this MySoObjectResponse {:?}",
    ///         object
    ///     ));
    ///     let object_id = object_data.object_id;
    ///     let dynamic_fields = myso
    ///         .read_api()
    ...
    pub async fn get_dynamic_fields(
        ...
        cursor: Option<ObjectID>,
        limit: Option<usize>,
    ) -> MySoRpcResult<DynamicFieldPage> {
        Ok(self
            .api
            .http
    ...
    ) -> MySoRpcResult<MySoPastObjectResponse> {
        Ok(self
            .api
            .http
    ...
    /// Return a list of [MySoPastObjectResponse] objects, or an error upon failure.
    ///
    ...
    /// use myso_sdk::MySoClientBuilder;
    /// use myso_types::base_types::{ObjectID, MySoAddress};
    /// use myso_json_rpc_types::{MySoObjectDataOptions, MySoGetPastObjectRequest};
    /// use std::str::FromStr;
    ///
    /// #[tokio::main]
    /// async fn main() -> Result<(), anyhow::Error> {
    ...
    ///     let multi_past_object = myso
    ///         .read_api()
    ///         .try_multi_get_parsed_past_object(
    ///             vec![MySoGetPastObjectRequest {
    ...
    pub async fn try_multi_get_parsed_past_object(
        &self,
        past_objects: Vec<MySoGetPastObjectRequest>,
        options: MySoObjectDataOptions,
    ) -> MySoRpcResult<Vec<MySoPastObjectResponse>> {
        Ok(self
            .api
            .http
    ...
    /// Return a [MySoObjectResponse] based on the provided [ObjectID] and [MySoObjectDataOptions], or an error upon failure.
    ///
    /// The [MySoObjectResponse] contains two fields:
    /// 1) `data` for the object's data (see [MySoObjectData](myso_json_rpc_types::MySoObjectData)),
    /// 2) `error` for the error (if any) (see [MySoObjectResponseError](myso_types::error::MySoObjectResponseError)).
    ///
    /// # Examples
    ///
    /// ```rust,no_run
    ...
    ///     ));
    ///     let object_id = object_data.object_id;
    ///     let object = myso.read_api().get_object_with_options(object_id,
    ///             MySoObjectDataOptions {
    ///                 show_type: true,
    ///                 show_owner: true,
    ///                 show_previous_transaction: true,
    ...
    /// Return a list of [MySoObjectResponse] from the given vector of [ObjectID]s and [MySoObjectDataOptions], or an error upon failure.
    ///
    ...
    ///     let object = myso.read_api().multi_get_object_with_options(object_ids,
    ///             MySoObjectDataOptions {
    ///                 show_type: true,
    ///                 show_owner: true,
    ///                 show_previous_transaction: true,
    ...
    pub async fn multi_get_object_with_options(
        ...
        options: MySoObjectDataOptions,
    ) -> MySoRpcResult<Vec<MySoObjectResponse>> {
        Ok(self
            .api
            .http
    ...
    pub async fn get_move_object_bcs(&self, object_id: ObjectID) -> MySoRpcResult<Vec<u8>> {
        let resp = self
            .get_object_with_options(object_id, MySoObjectDataOptions::default().with_bcs())
            .await?
            .into_object()
            .map_err(|e| {
    ...
    /// async fn main() -> Result<(), anyhow::Error> {
    ///     let myso = MySoClientBuilder::default().build_localnet().await?;
    ///     let total_transaction_blocks = myso
    ///         .read_api()
    ...
    /// }
    /// ```
    pub async fn get_total_transaction_blocks(&self) -> MySoRpcResult<u64> {
        Ok(*self.api.http.get_total_transaction_blocks().await?)
    }

    /// Return a transaction and its effects in a [MySoTransactionBlockResponse] based on its
    ...
    ) -> MySoRpcResult<MySoTransactionBlockResponse> {
        Ok(self
            .api
            .http
    ...
    /// Return a list of [MySoTransactionBlockResponse] based on the given vector of [TransactionDigest], or an error upon failure.
    ///
    ...
    pub async fn multi_get_transactions_with_options(
        ...
        options: MySoTransactionBlockResponseOptions,
    ) -> MySoRpcResult<Vec<MySoTransactionBlockResponse>> {
        Ok(self
            .api
            .http
    ...
    /// Return the [MySoCommittee] information for the provided `epoch`, or an error upon failure.
    ///
    /// The [MySoCommittee] contains the validators list and their information (name and stakes).
    ///
    ...
    ) -> MySoRpcResult<TransactionBlocksPage> {
        Ok(self
            .api
            .http
    ...
    pub async fn get_chain_identifier(&self) -> MySoRpcResult<String> {
        Ok(self.api.http.get_chain_identifier().await?)
    }

    /// Return a checkpoint, or an error upon failure.
    ///
    /// A MySo checkpoint is a sequence of transaction sets that a quorum of validators
    /// agree upon as having been executed within the MySo system.
    pub async fn get_checkpoint(&self, id: CheckpointId) -> MySoRpcResult<Checkpoint> {
    ...
    pub async fn get_latest_checkpoint_sequence_number(
        &self,
    ) -> MySoRpcResult<CheckpointSequenceNumber> {
        Ok(*self
            .api
            .http
    ...
    /// Return a stream of [MySoTransactionBlockResponse], or an error upon failure.
    pub fn get_transactions_stream(
        &self,
        query: MySoTransactionBlockResponseQuery,
        cursor: Option<TransactionDigest>,
        descending_order: bool,
    ) -> impl Stream<Item = MySoTransactionBlockResponse> + '_ {
        stream::unfold(
    ...
    ) -> MySoRpcResult<impl Stream<Item = MySoRpcResult<MySoTransactionBlockEffects>>> {
        let Some(c) = &self.api.ws else {
            return Err(Error::Subscription(
                "Subscription only supported by WebSocket client.".to_string(),
            ));
        };
        let subscription: Subscription<MySoTransactionBlockEffects> =
    ...
    pub async fn get_normalized_move_modules_by_package(
        &self,
        package: ObjectID,
    ) -> MySoRpcResult<BTreeMap<String, MySoMoveNormalizedModule>> {
        Ok(self
            .api
            .http
    ...
    pub async fn get_reference_gas_price(&self) -> MySoRpcResult<u64> {
    ...
    pub async fn dev_inspect_transaction_block(
        &self,
        sender_address: MySoAddress,
        tx: TransactionKind,
        gas_price: Option<BigInt<u64>>,
        epoch: Option<BigInt<u64>>,
        additional_args: Option<DevInspectArgs>,
    ) -> MySoRpcResult<DevInspectResults> {
        Ok(self
            .api
            .http
    ...
    ) -> MySoRpcResult<ProtocolConfigResponse> {
    ...
    /// Verify a zkLogin signature against bytes that is parsed using intent_scope, and the myso address.
    pub async fn verify_zklogin_signature(
        ...
        address: MySoAddress,
    ) -> MySoRpcResult<ZkLoginVerifyResult> {
        Ok(self
            .api
            .http
...
/// Coin Read API provides the functionality needed to get information from the MySo network regarding the coins owned by an address.
#[derive(Debug, Clone)]
...
impl CoinReadApi {
    ...
    /// or use `None` for the default `Coin<MYSO>`.
    ///
    /// # Examples
    ///
    /// ```rust,no_run
    ...
    ///
    /// The coins can be filtered by `coin_type` (e.g., 0x168da5bf1f48dafc111b0a488fa454aca95e0b5e::usdc::USDC)
    /// or use `None` to use the default `Coin<MYSO>`.
    ///
    /// # Examples
    ///
    /// ```rust,no_run
    ...
    /// it will use the default `Coin<MYSO>`.
    ///
    /// # Examples
    ///
    /// ```rust,no_run
    ...
    /// }
    /// ```
    pub async fn get_all_balances(&self, owner: MySoAddress) -> MySoRpcResult<Vec<Balance>> {
    ...
    /// async fn main() -> Result<(), anyhow::Error> {
    ///     let myso = MySoClientBuilder::default().build_localnet().await?;
    ///     let coin_metadata = myso
    ///         .coin_read_api()
    ///         .get_coin_metadata("0x2::myso::MYSO".to_string())
    ///         .await?;
    ///     Ok(())
    /// }
    /// ```
    pub async fn get_coin_metadata(
        &self,
        coin_type: String,
    ) -> MySoRpcResult<Option<MySoCoinMetadata>> {
    ...
    /// async fn main() -> Result<(), anyhow::Error> {
    ///     let myso = MySoClientBuilder::default().build_localnet().await?;
    ///     let total_supply = myso
    ///         .coin_read_api()
    ///         .get_total_supply("0x2::myso::MYSO".to_string())
    ///         .await?;
    ///     Ok(())
    /// }
    /// ```
    pub async fn get_total_supply(&self, coin_type: String) -> MySoRpcResult<Supply> {
        Ok(self.api.http.get_total_supply(coin_type).await?)
    }
}

/// Event API provides the functionality to fetch, query, or subscribe to events on the MySo network.
#[derive(Clone)]
pub struct EventApi {
    api: Arc<RpcClient>,
}
...
impl EventApi {
    ...
    ///         .ws_url("wss://rpc.mainnet.myso.io:443")
    ///         .build("https://fullnode.mainnet.myso.io:443")
    ///         .await?;
    ///     let mut subscribe_all = myso
    ///         .event_api()
    ...
    pub async fn subscribe_event(
        &self,
        filter: EventFilter,
    ) -> MySoRpcResult<impl Stream<Item = MySoRpcResult<MySoEvent>>> {
        match &self.api.ws {
            Some(c) => {
                let subscription: Subscription<MySoEvent> = c.subscribe_event(filter).await?;
                Ok(subscription.map(|item| Ok(item?)))
            }
    ...
    pub async fn get_events(&self, digest: TransactionDigest) -> MySoRpcResult<Vec<MySoEvent>> {
...
impl GovernanceApi {
    ...
    pub async fn get_stakes(&self, owner: MySoAddress) -> MySoRpcResult<Vec<DelegatedStake>> {
        Ok(self.api.http.get_stakes(owner).await?)
    }

    /// Return the [MySoCommittee] information for the given `epoch`, or an error upon failure.
    ///
    ...
    /// Return the latest MYSO system state object on-chain, or an error upon failure.
    ///
    ...
    /// and much more. See the [MySoSystemStateSummary] for all the available fields.
    pub async fn get_latest_myso_system_state(&self) -> MySoRpcResult<MySoSystemStateSummary> {
        Ok(self.api.http.get_latest_myso_system_state().await?)
    }
