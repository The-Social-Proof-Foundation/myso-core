...
// Re-export things used by myso-macros
...
pub use myso_framework;
pub use myso_move_build;
pub use myso_types;
...
pub mod configs {
    ...
    /// Select from among a number of configs using the MYSO_SIM_CONFIG env var.
    pub fn env_config(
        // Config to use when MYSO_SIM_CONFIG is not set.
        default: SimConfig,
        // List of (&str, SimConfig) pairs - the SimConfig associated with the value
        // of the MYSO_SIM_CONFIG var is chosen.
        ...
        if let Some(env) = std::env::var("MYSO_SIM_CONFIG").ok() {
            if let Some(cfg) = env_configs.remove(env.as_str()) {
                info!("Using test config for MYSO_SIM_CONFIG={}", env);
                cfg
            } else {
                panic!(
                    "No config found for MYSO_SIM_CONFIG={}. Available configs are: {:?}",
                    env,
                    env_configs.keys()
                );
            }
...
pub fn has_mainnet_protocol_config_override() -> bool {
    use myso_types::{digests::ChainIdentifier, supported_protocol_versions::Chain};
