...
- Provides deterministic, randomized execution of an entire MySo network in a single process.
...
1. Procedural macros that replace `#[tokio::test]` and run test code inside a testing environment. These are `#[myso_test]` and `#[sim_test]` and are documented below. The test harness created by these macros initializes the simulator runtime with a starting seed, generates the simulator configuration, and runs the test inside a newly created thread. The test must be run in its own thread in order to provide each test case with fresh thread local storage.
...
The easiest way to write tests that run in the simulation testing framework is to use [SwarmBuilder](https://github.com/the-social-proof-foundation/myso-core/blob/main/crates/myso-swarm/src/memory/swarm.rs#L47) to start your validators.
...
https://github.com/the-social-proof-foundation/myso-core/blob/main/crates/myso/tests/checkpoints_tests.rs is a good example of this.
...
To address this, most such test code launches validators via the `spawn_test_authorities` function in the `test-utils` carate, which returns `Vec<MySoNodeHandle>` rather than `Vec<MySoNode>`.
`MySoNodeHandle` hides the `MySoNode` from the test code.
...
`MySoNodeHandle` runs the provided callbacks/futures inside the context of the appropriate simulator node, so that network requests, spawned tasks, etc continue running in the correct context.
...
Network latency and packet loss can be configured using [SimConfig](https://github.com/MystenLabs/mysten-sim/blob/main/msim/src/sim/config.rs#L8), which is re-exported from this crate as `myso_simulator::config::SimConfig`.
...
There is a small but growing library of functions for building network configs in [myso_simulator::configs](https://github.com/the-social-proof-foundation/myso-core/blob/main/crates/myso-simulator/src/lib.rs).

There are also some examples of network configuration at https://github.com/the-social-proof-foundation/myso-core/blob/main/crates/myso-benchmark/tests/simtest.rs#L52.
