    ...
    input.block = syn::parse2(quote! {
        {
            ...
            std::thread::spawn(|| {
                ...
                ::myso_simulator::telemetry_subscribers::init_for_testing();
                ::myso_simulator::myso_types::execution_params::get_denied_certificates_for_sim_test();
                ::myso_simulator::myso_framework::BuiltInFramework::all_package_ids();
                ::myso_simulator::myso_types::gas::MySoGasStatus::new_unmetered();
                ...
                let mut cache = ::myso_simulator::lru::LruCache::new(1.try_into().unwrap());
                cache.put(1, 1);

                use std::sync::Arc;

                use ::myso_simulator::anemo_tower::callback::CallbackLayer;
                use ::myso_simulator::anemo_tower::trace::DefaultMakeSpan;
                use ::myso_simulator::anemo_tower::trace::DefaultOnFailure;
                use ::myso_simulator::anemo_tower::trace::TraceLayer;
                use ::myso_simulator::fastcrypto::traits::KeyPair;
                use ::myso_simulator::mysten_network::metrics::MetricsMakeCallbackHandler;
                use ::myso_simulator::mysten_network::metrics::NetworkMetrics;
                use ::myso_simulator::rand_crate::rngs::{StdRng, OsRng};
                use ::myso_simulator::rand::SeedableRng;
                use ::myso_simulator::tower::ServiceBuilder;
                ...
                let rt = ::myso_simulator::runtime::Runtime::new();
                rt.block_on(async move {
                    use ::myso_simulator::anemo::{Network, Request};

                    {
                        ...
                        use myso_simulator::myso_move_build::BuildConfig;
                    ...
                    let make_network = |port: u16| {
                        ...
                        let inbound_network_metrics =
                            NetworkMetrics::new("myso", "inbound", &registry);
                        let outbound_network_metrics =
                            NetworkMetrics::new("myso", "outbound", &registry);
                        ...
                        let service = ServiceBuilder::new()
                            ...
                            .service(::myso_simulator::anemo::Router::new());
                        ...
                        Network::bind(format!("127.0.0.1:{}", port))
                            ...
                            .private_key(
                                ::myso_simulator::fastcrypto::ed25519::Ed25519KeyPair::generate(&mut StdRng::from_rng(OsRng).unwrap())
                                    .private()
                                    .0
                                    .to_bytes(),
                            )
...
/// The myso_test macro will invoke either `#[msim::test]` or `#[tokio::test]`,
...
#[proc_macro_attribute]
pub fn myso_test(args: TokenStream, item: TokenStream) -> TokenStream {
    ...
    let header = if cfg!(msim) {
        quote! {
            #[::myso_simulator::sim_test(crate = "myso_simulator", #(#args)* )]
        }
    } else {
        quote! {
            #[::tokio::test(#(#args)*)]
    ...
    let result = quote! {
        #header
        #[::myso_macros::init_static_initializers]
        #input
    };
    ...
    let result = if cfg!(msim) {
        ...
        quote! {
            ...
            #sig {
                ...
                ::myso_simulator::task::shutdown_all_nodes();
                ...
                assert_eq!(
                    myso_simulator::NodeLeakDetector::get_current_node_count(),
                    0,
                    "MySoNode leak detected"
                );
        ...
        quote! {
            ...
            #ignore
            #sig {
                if std::env::var("MYSO_SKIP_SIMTESTS").is_ok() {
                    println!("not running test {} in `cargo test`: MYSO_SKIP_SIMTESTS is set", stringify!(#fn_name));
...
pub fn enum_variant_order_derive(input: TokenStream) -> TokenStream {
    ...
    if let Data::Enum(DataEnum { variants, .. }) = ast.data {
        ...
        let deriv = quote! {
            impl myso_enum_compat_util::EnumOrderMap for #name {
