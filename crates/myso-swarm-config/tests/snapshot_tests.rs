...
// This file contains tests that detect changes in MySo configs.
...
// 1. Run `cargo insta test --review` under `./myso-config`.
...
fn populated_genesis_snapshot_matches() {
    ...
    let genesis = Builder::new()
        ...
        .build();
    assert_yaml_snapshot!(genesis.myso_system_wrapper_object());
