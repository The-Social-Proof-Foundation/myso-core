...
use myso_config::node::{RunWithRange, TransactionDriverConfig, default_zklogin_oauth_providers};
use myso_config::p2p::{P2pConfig, SeedPeer, StateSyncConfig};
...
impl ValidatorConfigBuilder {
    ...
    pub fn build(
        ...
        genesis: myso_config::genesis::Genesis,
    ) -> NodeConfig {
        ...
        NodeConfig {
            ...
            genesis: myso_config::node::Genesis::new(genesis),
            grpc_load_shed: None,
...
impl FullnodeConfigBuilder {
    ...
    pub fn with_rpc_config(mut self, rpc_config: myso_config::RpcConfig) -> Self {
    ...
    pub fn with_network_key_pair(mut self, network_key_pair: Option<NetworkKeyPair>) -> Self {
        if let Some(network_key_pair) = network_key_pair {
            self.network_key_pair =
                Some(KeyPairWithPath::new(MySoKeyPair::Ed25519(network_key_pair)));
        }
        self
    }
    ...
    pub fn build<R: rand::RngCore + rand::CryptoRng>(
        ...
        NodeConfig {
            ...
            worker_key_pair: KeyPairWithPath::new(MySoKeyPair::Ed25519(
                validator_config.worker_key_pair,
            )),
            network_key_pair: self.network_key_pair.unwrap_or(KeyPairWithPath::new(
                MySoKeyPair::Ed25519(validator_config.network_key_pair),
            )),
            db_path: self
                .db_path
            ...
            remove_deprecated_tables: false,
            enable_index_processing: default_enable_index_processing(),
            genesis: self.genesis.unwrap_or(myso_config::node::Genesis::new(
