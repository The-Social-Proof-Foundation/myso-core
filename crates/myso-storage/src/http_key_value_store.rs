...
trait IntoMySoResult<T> {
    fn into_myso_result(self) -> MySoResult<T>;
}

impl<T, E> IntoMySoResult<T> for Result<T, E>
where
    E: std::error::Error,
{
    fn into_myso_result(self) -> MySoResult<T> {
        self.map_err(|e| MySoErrorKind::Storage(e.to_string()).into())
    }
}
...
impl HttpKVStore {
    pub fn new_kv(
        ...
        cache_size: u64,
        metrics: Arc<KeyValueStoreMetrics>,
    ) -> MySoResult<TransactionKeyValueStore> {
    ...
    pub fn new(
        ...
        let base_url = Url::parse(&base_url).into_myso_result()?;
    ...
    fn get_url(&self, key: &Key) -> MySoResult<Url> {
        let (digest, item_type) = key.to_path_elements();
        let joined = self
            .base_url
            .join(&format!("{}/{}", digest, item_type))
            .into_myso_result()?;
        Url::from_str(joined.as_str()).into_myso_result()
    }

    async fn multi_fetch(&self, uris: Vec<Key>) -> Vec<MySoResult<Option<Bytes>>> {
    ...
    async fn fetch(&self, key: Key) -> MySoResult<Option<Bytes>> {
        ...
        if resp.status().is_success() {
            let bytes = resp.bytes().await.into_myso_result()?;
...
fn map_fetch<'a, K>(fetch: (&'a MySoResult<Option<Bytes>>, &'a K)) -> Option<(&'a Bytes, &'a K)>
where
    K: std::fmt::Debug,
{
    let (fetch, key) = fetch;
