...
use myso_types::digests::{CheckpointDigest, TransactionDigest};
...
impl TransactionKeyValueStore {
    ...
    ) -> MySoResult<Vec<Option<CertifiedCheckpointSummary>>> {
    ...
    pub async fn multi_get_checkpoints_contents(
        &self,
        keys: &[CheckpointSequenceNumber],
    ) -> MySoResult<Vec<Option<CheckpointContents>>> {
        self.multi_get_checkpoints(&[], keys, &[])
            .await
    ...
    ) -> MySoResult<Vec<Option<Transaction>>> {
    ...
    pub async fn multi_get_fx_by_tx_digest(
        &self,
        keys: &[TransactionDigest],
    ) -> MySoResult<Vec<Option<TransactionEffects>>> {
    ...
    pub async fn get_tx(&self, digest: TransactionDigest) -> MySoResult<Transaction> {
        self.multi_get_tx(&[digest])
            .await?
            .into_iter()
            .next()
            .flatten()
            .ok_or(MySoErrorKind::TransactionNotFound { digest }.into())
    }
    ...
    ) -> MySoResult<CertifiedCheckpointSummary> {
