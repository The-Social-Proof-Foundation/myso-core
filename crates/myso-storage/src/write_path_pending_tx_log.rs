...
impl WritePathPendingTransactionLog {
    ...
    pub fn finish_transaction(&self, tx: &TransactionDigest) -> MySoResult {
        let mut transactions_set = self.transactions_set.lock();
        ...
        write_batch.write().map_err(MySoError::from)?;
        transactions_set.remove(tx);
        Ok(())
    }

    pub fn load_all_pending_transactions(&self) -> MySoResult<Vec<VerifiedTransaction>> {
        let mut transactions_set = self.transactions_set.lock();
