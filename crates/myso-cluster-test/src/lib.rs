...
impl TestContext {
    async fn get_myso_from_faucet(&self, minimum_coins: Option<usize>) -> Vec<GasCoin> {
        ...
        if gas_coins.len() < minimum_coins {
            panic!(
                "Expect to get at least {minimum_coins} MySo Coins for address {addr}, but only got {}",
                gas_coins.len()
            )
        }
    ...
    fn get_fullnode_client(&self) -> &MySoClient {
        self.client.get_fullnode_client()
    }

    fn clone_fullnode_client(&self) -> MySoClient {
    ...
    async fn get_latest_myso_system_state(&self) -> MySoSystemStateSummary {
        self.client
            .get_fullnode_client()
            .governance_api()
    ...
    fn get_wallet_address(&self) -> MySoAddress {
    ...
    async fn get_tx_with_retry_times(
        ...
        match self
            ...
            .get_fullnode_client()
            .read_api()
            .get_transaction_with_options(digest, MySoTransactionBlockResponseOptions::new())
            .await
        {
