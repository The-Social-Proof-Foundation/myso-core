...
use myso_json_rpc_types::{BalanceChange, MySoData, MySoObjectData, MySoObjectDataOptions};
use myso_sdk::MySoClient;
use myso_types::error::MySoObjectResponseError;
use myso_types::gas_coin::GasCoin;
use myso_types::{base_types::ObjectID, object::Owner, parse_myso_type_tag};
use tracing::{debug, trace};

/// A util struct that helps verify MySo Object.
...
/// `check_into_gas_coin` expect to get a `MySoObjectData` and `GasCoin`
/// respectfully.
#[derive(Debug)]
...
impl ObjectChecker {
    ...
    pub fn is_myso_coin(mut self, is_myso_coin: bool) -> Self {
        self.is_myso_coin = Some(is_myso_coin);
        self
    }

    pub async fn check_into_gas_coin(self, client: &MySoClient) -> GasCoin {
        if self.is_myso_coin == Some(false) {
            panic!("'check_into_gas_coin' shouldn't be called with 'is_myso_coin' set as false");
        }
        self.is_myso_coin(true)
            .check(client)
            .await
            .unwrap()
            .into_gas_coin()
    ...
    pub async fn check_into_object(self, client: &MySoClient) -> MySoObjectData {
        self.check(client).await.unwrap().into_object()
    }

    pub async fn check(self, client: &MySoClient) -> Result<CheckerResultObject, anyhow::Error> {
        debug!(?self);
        ...
        match (object_info.data, object_info.error) {
            (None, Some(MySoObjectResponseError::NotExists { object_id })) => {
                panic!(
            ...
            (
                None,
                Some(MySoObjectResponseError::DynamicFieldNotFound {
            ...
            (Some(object), _) => {
                ...
                if self.is_myso_coin == Some(true) {
                    let move_obj = object
                        .bcs
                        .as_ref()
            ...
            (None, Some(MySoObjectResponseError::DisplayError { error })) => {
                panic!("Display Error: {error:?}");
            }
            (None, None) | (None, Some(MySoObjectResponseError::Unknown)) => {
...
pub struct CheckerResultObject {
    ...
    object: Option<MySoObjectData>,
}

impl CheckerResultObject {
    pub fn new(gas_coin: Option<GasCoin>, object: Option<MySoObjectData>) -> Self {
        Self { gas_coin, object }
    }
    pub fn into_gas_coin(self) -> GasCoin {
        self.gas_coin.unwrap()
    }
    pub fn into_object(self) -> MySoObjectData {
        self.object.unwrap()
    }
}
...
impl BalanceChangeChecker {
    ...
    pub fn coin_type(mut self, coin_type: &str) -> Self {
        self.coin_type = Some(parse_myso_type_tag(coin_type).unwrap());
        self
    }
