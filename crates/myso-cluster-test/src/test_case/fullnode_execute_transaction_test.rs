...
use myso_json_rpc_types::{
    MySoExecutionStatus, MySoTransactionBlockEffectsAPI, MySoTransactionBlockResponseOptions,
};
...
impl FullNodeExecuteTransactionTest {
    async fn verify_transaction(fullnode: &MySoClient, tx_digest: TransactionDigest) {
        fullnode
            .read_api()
            .get_transaction_with_options(tx_digest, MySoTransactionBlockResponseOptions::new())
            .await
            .unwrap_or_else(|e| {
                panic!(
...
impl TestCaseImpl for FullNodeExecuteTransactionTest {
    ...
    async fn run(&self, ctx: &mut TestContext) -> Result<(), anyhow::Error> {
        let txn_count = 4;
        ctx.get_myso_from_faucet(Some(1)).await;
        ...
        if !matches!(effects.status(), MySoExecutionStatus::Success) {
            panic!(
