...
use myso_json_rpc_types::{Balance, MySoTransactionBlockResponseOptions};
use myso_move_build::test_utils::compile_managed_coin_package;
...
impl TestCaseImpl for CoinIndexTest {
    ...
    async fn run(&self, ctx: &mut TestContext) -> Result<(), anyhow::Error> {
        ...
        ctx.get_myso_from_faucet(None).await;
        ...
        assert_eq!(
            ...
            "total_balance: {}, old_total_balance: {}, myso_balance_change.amount: {}",
            total_balance,
            old_total_balance,
        ...
        let myso_type_str = "0x2::myso::MYSO";
        ...
        let args = vec![
            MySoJsonValue::from_object_id(cap.0),
            MySoJsonValue::new(json!("10000"))?,
            MySoJsonValue::new(json!(account))?,
        ];
        let txn = client
            .transaction_builder()
            .move_call(
        ...
        let myso_balance_change = balance_changes
            .iter()
            .find(|b| b.coin_type.to_string().contains("MYSO"))
            .unwrap();
        ...
        assert_eq!(myso_balance_change.owner, Owner::AddressOwner(account));
        ...
        assert_eq!(
            total_balance,
            (old_total_balance as i128 + myso_balance_change.amount) as u128,
            "total_balance: {}, old_total_balance: {}, myso_balance_change.amount: {}",
            total_balance,
            old_total_balance,
            myso_balance_change.amount
        );
        old_coin_object_count = coin_object_count;
        ...
        let args = vec![MySoJsonValue::from_object_id(envelope.0)];
        let txn = client
            .transaction_builder()
            .move_call(
                account,
        ...
        let txn = client
            ...
            .move_call(
                ...
                vec![
                    MySoJsonValue::from_object_id(cap.0),
                    MySoJsonValue::from_object_id(envelope.0),
                ],
                None,
                rgp * 2_000_000,
                None,
        ...
        let txn = client
            ...
            .move_call(
                ...
                vec![
                    MySoJsonValue::from_object_id(cap.0),
                    MySoJsonValue::from_object_id(managed_coin_id_10k),
                ],
                None,
                rgp * 2_000_000,
                None,
        ...
        let myso_coins = client
            .coin_read_api()
            .get_coins(account, Some(myso_type_str.into()), None, None)
            .await
            .unwrap()
            .data;
        ...
        assert_eq!(
            // this is only MYSO coins at the moment
            myso_coins,
            client
                .coin_read_api()
        ...
        let myso_balance = client
            .coin_read_api()
            .get_balance(account, None)
            .await
            .unwrap();
        assert_eq!(
            myso_balance.total_balance,
            myso_coins.iter().map(|c| c.balance as u128).sum::<u128>()
        );
        ...
        let txn = client
            ...
            .move_call(
                ...
                vec![
                    MySoJsonValue::from_object_id(cap.0),
                    MySoJsonValue::new(json!("5"))?,  // balance = 5
                    MySoJsonValue::new(json!("40"))?, // num = 40
                    MySoJsonValue::new(json!(account))?,
                ],
                None,
                rgp * 2_000_000,
        ...
        assert_eq!(
            ...
            client
                .coin_read_api()
                .get_coins(account, None, None, Some(myso_coins.len() + 1))
                .await
                .unwrap()
                .data,
        );
        ...
        assert_eq!(myso_coins.len() + managed_coins.len(), total_coins,);

        let myso_coins_with_managed_coin_1 = client
            .coin_read_api()
            .get_all_coins(account, None, Some(myso_coins.len() + 1))
            .await
            .unwrap();
        assert_eq!(
            myso_coins_with_managed_coin_1.data.len(),
            myso_coins.len() + 1
        );
        assert!(myso_coins_with_managed_coin_1.has_next_page);
        let cursor = myso_coins_with_managed_coin_1.next_cursor;
...
async fn add_to_envelope(
    ...
    let txn = client
        ...
        .move_call(
            ...
            vec![],
            vec![
                MySoJsonValue::from_object_id(envelope),
                MySoJsonValue::from_object_id(coin),
            ],
            None,
            rgp * 2_000_000,
            None,
