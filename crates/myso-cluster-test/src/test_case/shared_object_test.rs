...
use myso_test_transaction_builder::{increment_counter, publish_basics_package_and_make_counter};
...
impl TestCaseImpl for SharedCounterTest {
    ...
    async fn run(&self, ctx: &mut TestContext) -> Result<(), anyhow::Error> {
        ...
        let myso_objs = ctx.get_myso_from_faucet(Some(1)).await;
        assert!(!myso_objs.is_empty());
