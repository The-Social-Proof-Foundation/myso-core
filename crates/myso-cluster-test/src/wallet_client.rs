...
impl WalletClient {
    pub async fn new_from_cluster(cluster: &(dyn Cluster + Sync + Send)) -> Self {
        ...
        let address: MySoAddress = key.public().into();
        ...
        let fullnode_client = MySoClientBuilder::default().build(rpc_url).await.unwrap();
    ...
    pub fn get_wallet_address(&self) -> MySoAddress {
        self.address
    }

    pub fn get_fullnode_client(&self) -> &MySoClient {
    ...
    pub async fn sign(&self, txn_data: &TransactionData, desc: &str) -> Signature {
        self.get_wallet()
            .config
            .keystore
            .sign_secure(&self.address, txn_data, Intent::myso_transaction())
            .await
