...
impl TestTransactionBuilder {
    pub fn new(sender: MySoAddress, gas_object: ObjectRef, gas_price: u64) -> Self {
        Self::new_impl(sender, Some(gas_object), gas_price)
    }

    fn new_impl(sender: MySoAddress, gas_object: Option<ObjectRef>, gas_price: u64) -> Self {
        Self {
    ...
    pub fn call_staking(self, stake_coin: ObjectRef, validator: MySoAddress) -> Self {
        self.move_call(
            MYSO_SYSTEM_PACKAGE_ID,
            MYSO_SYSTEM_MODULE_NAME.as_str(),
            "request_add_stake",
            vec![
    ...
    pub fn transfer(mut self, object: FullObjectRef, recipient: MySoAddress) -> Self {
        self.ptb_builder.transfer_object(recipient, object).unwrap();
        self
    }

    pub fn transfer_myso(mut self, amount: Option<u64>, recipient: MySoAddress) -> Self {
        self.ptb_builder.transfer_myso(recipient, amount);
        self
    }

    pub fn transfer_myso_to_address_balance(
        self,
        source: FundSource,
    ...
    pub fn build_and_sign_multisig(
        ...
        let data = self.build();
        let intent_msg = IntentMessage::new(Intent::myso_transaction(), data.clone());
...
pub async fn make_transfer_myso_transaction(
    context: &WalletContext,
    recipient: Option<MySoAddress>,
    amount: Option<u64>,
) -> Transaction {
    ...
    context
        .sign_transaction(
            &TestTransactionBuilder::new(sender, gas_object, gas_price)
                .transfer_myso(amount, recipient.unwrap_or(sender))
                .build(),
        )
        .await
}

pub async fn make_transfer_myso_address_balance_transaction(
    context: &WalletContext,
    recipient: Option<MySoAddress>,
    amount: u64,
) -> Transaction {
...
pub async fn create_nft(
    context: &WalletContext,
    package_id: ObjectID,
) -> (MySoAddress, ObjectID, TransactionDigest) {
    let (sender, gas_object) = context.get_one_gas_object().await.unwrap().unwrap();
    let rgp = context.get_reference_gas_price().await.unwrap();
