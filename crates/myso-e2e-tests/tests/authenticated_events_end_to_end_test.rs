...
async fn load_event_stream_head_by_object_id(
    state: &myso_core::authority::AuthorityState,
    object_id: ObjectID,
...
async fn authenticated_events_single_event_test() {
    ...
    assert_eq!(
        ...
        MySoAddress::from(AccountAddress::from(package_id))
    );

    let state = test_cluster.fullnode_handle.myso_node.state();
...
async fn authenticated_events_mmr_validation_test() {
    ...
    let myso_stream_head_x = load_event_stream_head_by_object_id(&state, accumulator_obj_id)
        .await
    ...
    assert_eq!(
        calculated_stream_head_x.num_events, myso_stream_head_x.num_events,
        "Event count should match at checkpoint X"
    );

    assert_eq!(
        calculated_stream_head_x.mmr, myso_stream_head_x.mmr,
        "MMR should match at checkpoint X"
    );

    let stream_head_x = &myso_stream_head_x;
