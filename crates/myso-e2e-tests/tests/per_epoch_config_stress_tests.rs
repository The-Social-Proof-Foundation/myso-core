...
const DENY_ADDRESS: MySoAddress = MySoAddress::ZERO;
...
struct TestEnv {
    ...
    regulated_coin_owner: MySoAddress,
...
async fn create_test_env() -> TestEnv {
    ...
    let deny_list_object_init_version = test_cluster
        .get_object_from_fullnode_store(&MYSO_DENY_LIST_OBJECT_ID)
        .await
        .unwrap()
        .version();
