...
async fn test_unchanged_loaded_runtime_objects() {
    ...
    let objects = client
        .state_client()
        .list_owned_objects(
            myso_rpc::proto::myso::rpc::v2::ListOwnedObjectsRequest::default()
    ...
    builder
        .move_call(
            "0x3".parse().unwrap(),
            Identifier::new("myso_system").unwrap(),
...
async fn test_tto_receive_twice() {
    ...
    let t = client
        .ledger_client()
        .get_transaction(
            myso_rpc::proto::myso::rpc::v2::GetTransactionRequest::new(&(*txn.digest()).into())
                .with_read_mask(FieldMask::from_paths(["*"])),
        )
        .await
        .unwrap()
