...
use myso_rpc::proto::myso::rpc::v2::changed_object::{IdOperation, OutputObjectState};
...
async fn test_filter_by_type() {
    ...
    let myso = "0x2::coin::Coin<0x2::myso::MYSO>"
        .parse::<TypeTag>()
        .unwrap()
        .to_string();
    let mut client = Client::new(cluster.rpc_url().to_owned()).unwrap();
    ...
    // We start with some MYSO coins
    assert!(!objects.is_empty());
    assert!(objects.iter().all(|o| o.object_type() == myso));
    ...
    let treasury_cap_type =
        myso_types::coin::TreasuryCap::type_(myso_types::parse_myso_struct_tag(&trusted).unwrap())
...
async fn test_reverse_sorted_coins_by_balance() {
    ...
    let address = myso_types::base_types::MySoAddress::random_for_testing_only();
