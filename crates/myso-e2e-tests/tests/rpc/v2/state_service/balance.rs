...
use myso_rpc::Client;
use myso_rpc::proto::myso::rpc::v2::Balance;
use myso_rpc::proto::myso::rpc::v2::{ExecutedTransaction, GasCostSummary};
use myso_rpc::proto::myso::rpc::v2::{GetBalanceRequest, ListBalancesRequest};
...
const INITIAL_MYSO_BALANCE: u64 = 150000000000000000;
...
async fn test_balance_apis() {
    ...
    verify_balances(
        ...
        address,
        &[Balance::default()
            .with_coin_type(MYSO_COIN_TYPE)
            .with_balance(INITIAL_MYSO_BALANCE)
            .with_coin_balance(INITIAL_MYSO_BALANCE)],
    )
    .await;
}
...
async fn test_balance_changes_on_transfer() {
    ...
    let txn = myso_test_transaction_builder::make_transfer_myso_transaction(
        &test_cluster.wallet,
        Some(receiver),
    ...
    verify_balances(
        ...
        &[Balance::default()
            .with_coin_type(MYSO_COIN_TYPE)
            .with_balance(INITIAL_MYSO_BALANCE - transfer_amount - gas_used)
            .with_coin_balance(INITIAL_MYSO_BALANCE - transfer_amount - gas_used)],
    )
    .await;
    ...
    verify_balances(
        ...
        &[Balance::default()
            .with_coin_type(MYSO_COIN_TYPE)
            .with_balance(INITIAL_MYSO_BALANCE + transfer_amount)
            .with_coin_balance(INITIAL_MYSO_BALANCE + transfer_amount)],
    )
    .await;
}
...
async fn test_custom_coin_balance() {
    ...
    verify_balances(
        ...
        &[
            Balance::default()
                .with_coin_type(MYSO_COIN_TYPE)
                .with_balance(INITIAL_MYSO_BALANCE - publish_gas_used - mint_gas_used)
                .with_coin_balance(INITIAL_MYSO_BALANCE - publish_gas_used - mint_gas_used),
            Balance::default()
    ...
    let myso_client = test_cluster.grpc_client();
    ...
    verify_balances(
        ...
        &[
            Balance::default()
                .with_coin_type(MYSO_COIN_TYPE)
                .with_balance(
                    INITIAL_MYSO_BALANCE - publish_gas_used - mint_gas_used - transfer_gas_used,
                )
                .with_coin_balance(
                    INITIAL_MYSO_BALANCE - publish_gas_used - mint_gas_used - transfer_gas_used,
                ),
...
async fn test_multiple_concurrent_balance_changes() {
    ...
    verify_balances(
        ...
        &[Balance::default()
            .with_coin_type(MYSO_COIN_TYPE)
            .with_balance(INITIAL_MYSO_BALANCE - transfer_0_to_1 - gas_used_0)
            .with_coin_balance(INITIAL_MYSO_BALANCE - transfer_0_to_1 - gas_used_0)],
    )
    .await;
    ...
    verify_balances(
        ...
        &[Balance::default()
            .with_coin_type(MYSO_COIN_TYPE)
            .with_balance(
                INITIAL_MYSO_BALANCE + transfer_0_to_1 - transfer_1_to_2 + transfer_2_to_1
                    - gas_used_1,
            )
            .with_coin_balance(
                INITIAL_MYSO_BALANCE + transfer_0_to_1 - transfer_1_to_2 + transfer_2_to_1
    ...
    verify_balances(
        ...
        &[Balance::default()
            .with_coin_type(MYSO_COIN_TYPE)
            .with_balance(INITIAL_MYSO_BALANCE + transfer_1_to_2 - transfer_2_to_1 - gas_used_2)
            .with_coin_balance(
                INITIAL_MYSO_BALANCE + transfer_1_to_2 - transfer_2_to_1 - gas_used_2,
            )],
    )
    .await;
}
...
async fn test_fresh_address_with_no_coins() {
    ...
    let fresh_address = MySoAddress::random_for_testing_only();

    // Get balance for MYSO
    let response = grpc_client
        .state_client()
        .get_balance({
            let mut message = GetBalanceRequest::default();
            message.owner = Some(fresh_address.to_string());
            message.coin_type = Some(MYSO_COIN_TYPE.to_string());
            message
        })
        .await
        .unwrap()
...
async fn test_invalid_requests() {
    ...
    request.coin_type = Some(MYSO_COIN_TYPE.to_string());
...
async fn execute_transaction(
    ...
    txn: &myso_types::transaction::Transaction,
...
/// Build a transaction that splits and transfers MYSO
...
async fn split_and_transfer_coin(
    ...
    recipient: MySoAddress,
    gas_object: (
        myso_types::base_types::ObjectID,
        myso_types::base_types::SequenceNumber,
        myso_types::base_types::ObjectDigest,
    ),
    coin: (
        myso_types::base_types::ObjectID,
        myso_types::base_types::SequenceNumber,
