...
use myso_rpc::proto::myso::rpc::v2::coin_metadata::MetadataCapState;
...
// not been migrated.
#[sim_test]
async fn get_coin_info_myso() {
    let test_cluster = TestClusterBuilder::new().build().await;

    let mut grpc_client = get_grpc_client(&test_cluster).await;

    let coin_type_sdk: TypeTag = "0x2::myso::MYSO".parse().unwrap();
    ...
    assert_eq!(metadata.symbol, Some("MYSO".to_owned()));
    assert_eq!(metadata.name, Some("MySo".to_string()));
    ...
    assert_eq!(
        ...
        Some(myso_types::gas_coin::TOTAL_SUPPLY_MIST)
    );
    assert_eq!(
        treasury.supply_state,
        Some(SupplyState::Fixed as i32),
        "MYSO should have Fixed supply state"
    );
    ...
    let registry_obj_response = {
        ...
        ledger_client
            .get_object({
                ...
                request.object_id = Some(MYSO_COIN_REGISTRY_OBJECT_ID.to_string());
    ...
    let registry_initial_version = registry_obj_response
        ...
        .and_then(|owner| {
            if owner.kind == Some(OwnerKind::Shared as i32) {
                owner
                    .version
                    .map(myso_types::base_types::SequenceNumber::from)
            } else {
                None
            }
        })
...
async fn finalize_registration(
    ...
    let registry_owned = grpc_client
        .list_owned_objects({
            ...
            request.owner = Some(MySoAddress::from(MYSO_COIN_REGISTRY_OBJECT_ID).to_string());
        ...
        .find(|obj| {
            ...
            obj_type
                .and_then(|t| {
                    ...
                    match myso_types::parse_myso_struct_tag(t) {
                        Ok(struct_tag) => {
    ...
    let mut client = myso_rpc::Client::new(test_cluster.rpc_url().to_owned()).unwrap();
