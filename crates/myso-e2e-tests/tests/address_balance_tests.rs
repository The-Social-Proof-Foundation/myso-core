...
use myso_types::accumulator_metadata::get_accumulator_object_count;
use myso_types::{
...
async fn get_sender_and_all_gas(context: &mut WalletContext) -> (MySoAddress, Vec<ObjectRef>) {
    get_nth_sender_and_all_gas(context, 0).await
}

async fn get_sender_and_one_gas(context: &mut WalletContext) -> (MySoAddress, ObjectRef) {
...
async fn test_accumulators_root_created() {
    ...
    test_cluster.fullnode_handle.myso_node.with(|node| {
...
async fn test_accumulators_disabled() {
    ...
    let tx = TestTransactionBuilder::new(sender, gas, rgp)
        .transfer_myso_to_address_balance(FundSource::coin(gas), vec![(1000, recipient)])
        .build();
    ...
    let tx = TestTransactionBuilder::new(sender, gas, rgp)
        .transfer_myso_to_address_balance(FundSource::coin(gas), vec![(1000, sender)])
        .build();
...
async fn test_deposits() {
    ...
    test_env.cluster.fullnode_handle.myso_node.with(|node| {
        let state = node.state();
        let child_object_resolver = state.get_child_object_resolver().as_ref();
        ...
        let accumulator_object =
            AccumulatorValue::load_object(child_object_resolver, None, recipient, &myso_coin_type)
        ...
        input_consensus_objects.iter().find(|input_consensus_object| {
            matches!(input_consensus_object, InputConsensusObject::ReadOnly(obj_ref) if obj_ref.0 == MYSO_ACCUMULATOR_ROOT_OBJECT_ID)
...
async fn test_multiple_settlement_txns() {
    ...
    let amounts_and_recipients = (0..20)
        .map(|_| (1u64, MySoAddress::random_for_testing_only()))
        .collect::<Vec<_>>();

    let tx = test_env
        .tx_builder(sender)
        .transfer_myso_to_address_balance(FundSource::coin(gas), amounts_and_recipients.clone())
        .build();
    test_env.exec_tx_directly(tx).await.unwrap();
...
async fn test_withdraw_insufficient_balance() {
    ...
    let tx = test_env
        ...
        .transfer_myso_to_address_balance(FundSource::coin(gas1), vec![(1000, sender)])
        .build();
    test_env.exec_tx_directly(tx).await.unwrap();
...
async fn test_sponsored_address_balance_storage_rebates() {
    ...
    let deposit_tx_sender = test_env
        ...
        .transfer_myso_to_address_balance(FundSource::coin(sender_gas), vec![(100_000_000, sender)])
        .build();
    ...
    let sender_sig = test_env
        ...
        .config
        .keystore
        .sign_secure(&sender, &create_txn, Intent::myso_transaction())
        .await
        .unwrap();
    let sponsor_sig = test_env
        ...
        .config
        .keystore
        .sign_secure(&sponsor, &create_txn, Intent::myso_transaction())
        .await
        .unwrap();
    ...
    let sponsor_actual = test_env.get_myso_balance(sponsor);
    let sender_actual = test_env.get_myso_balance(sender);
    ...
    let sender_delete_sig = test_env
        ...
        .config
        .keystore
        .sign_secure(&sender, &delete_txn, Intent::myso_transaction())
        .await
        .unwrap();
    let sponsor_delete_sig = test_env
        ...
        .config
        .keystore
        .sign_secure(&sponsor, &delete_txn, Intent::myso_transaction())
        .await
        .unwrap();
    ...
    let sponsor_final = test_env.get_myso_balance(sponsor);
    let sender_final = test_env.get_myso_balance(sender);
...
async fn setup_address_balance_account(
    ...
    let deposit_tx = test_env
        .tx_builder(sender)
        .transfer_myso_to_address_balance(FundSource::coin(gas), vec![(deposit_amount, sender)])
        .build();
...
fn create_withdraw_balance_transaction(
    ...
    let split_withdraw_arg = builder.programmable_move_call(
        ...
        vec!["0x2::balance::Balance<0x2::myso::MYSO>".parse().unwrap()],
    ...
    let coin = builder.programmable_move_call(
        ...
        vec!["0x2::myso::MYSO".parse().unwrap()],
...
fn create_address_balance_transaction_with_expiration(
    ...
    use myso_types::transaction::{GasData, TransactionDataV1, TransactionExpiration};
...
async fn test_sponsor_insufficient_balance_charges_zero_gas() {
    ...
    let sender_sig1 = test_env
        ...
        .config
        .keystore
        .sign_secure(&sender, &tx1, Intent::myso_transaction())
        .await
        .unwrap();
    let sponsor_sig1 = test_env
        ...
        .config
        .keystore
        .sign_secure(&sponsor, &tx1, Intent::myso_transaction())
        .await
        .unwrap();
    ...
    let sender_sig2 = test_env
        ...
        .config
        .keystore
        .sign_secure(&sender, &tx2, Intent::myso_transaction())
        .await
        .unwrap();
    let sponsor_sig2 = test_env
        ...
        .config
        .keystore
        .sign_secure(&sponsor, &tx2, Intent::myso_transaction())
        .await
        .unwrap();
    ...
    let final_sponsor_balance = test_env.get_myso_balance(sponsor);
    ...
    let final_sender_balance = test_env.get_myso_balance(sender);
...
async fn test_soft_bundle_different_gas_payers() {
    ...
    let actual_balance1 = test_env.get_myso_balance(sender1);
    let actual_balance2 = test_env.get_myso_balance(sender2);
...
async fn test_multiple_deposits_merged_in_effects() {
    ...
    let initial_deposit = TestTransactionBuilder::new(sender, gas, rgp)
        .transfer_myso_to_address_balance(FundSource::coin(gas), vec![(10000, recipient)])
        .build();
    ...
    for amount in &withdraw_amounts {
        let withdraw_arg = myso_types::transaction::FundsWithdrawalArg::balance_from_sender(
            *amount,
            myso_types::gas_coin::GAS::type_tag(),
        );
        let withdraw_arg = builder.funds_withdrawal(withdraw_arg).unwrap();
    ...
    match &event.write.value {
        myso_types::effects::AccumulatorValue::Integer(value) => {
            assert_eq!(
                *value, expected_net,
    ...
    match &event.write.operation {
        myso_types::effects::AccumulatorOperation::Merge => {}
...
async fn test_address_balance_gas_budget_enforcement_with_storage() {
    ...
    assert!(
        matches!(
            error_kind,
            myso_types::execution_status::ExecutionFailureStatus::InsufficientGas
        ),
...
async fn test_address_balance_large_rebate() {
    ...
    let initial_balance = test_env.get_myso_balance(sender);
    ...
    let final_balance = test_env.get_myso_balance(sender);
...
async fn test_sponsored_address_balance_storage_oog() {
    ...
    let sender_sig = test_env
        ...
        .config
        .keystore
        .sign_secure(&sender, &tx, Intent::myso_transaction())
        .await
        .unwrap();
    let sponsor_sig = test_env
        .cluster
        ...
        .config
        .keystore
        .sign_secure(&sponsor, &tx, Intent::myso_transaction())
        .await
        .unwrap();
...
async fn test_get_all_balances() {
    ...
    let _gas = {
        let tx = TestTransactionBuilder::new(sender, gas, rgp)
            .transfer_myso_to_address_balance(FundSource::coin(gas), vec![(1001, recipient)])
            .build();
    ...
    test_cluster.fullnode_handle.myso_node.with(|node| {
        ...
        assert!(
            balances
                .iter()
                .any(|(t, _)| t.to_canonical_string(true).contains("::myso::MYSO"))
        );
...
async fn address_balance_stress_test() {
    ...
    for coin_type in &coin_types {
        let recipients: Vec<(u64, MySoAddress)> =
            ...
            let handle = tokio::spawn(async move {
                ...
                while start_time.elapsed() < duration {
                    let (selected_types, recipients) = {
                        ...
                        let other_addresses: Vec<MySoAddress> = all_addresses
                            .iter()
                            .filter(|a| **a != addr)
                            .copied()
                            .collect();
