...
use myso_types::{
    ...
    base_types::{ObjectDigest, ObjectID, ObjectRef, MySoAddress},
    governance::StakedMySo,
    object::{Object, Owner},
    programmable_transaction_builder::ProgrammableTransactionBuilder,
    storage::ObjectStore,
    myso_system_state::{
        MySoSystemStateTrait,
        myso_system_state_summary::{MySoSystemStateSummary, MySoValidatorSummary},
    },
    transaction::{
...
const MAX_DELEGATION_AMOUNT: u64 = 1_000_000_000_000_000; // 1M MYSO
const MIN_DELEGATION_AMOUNT: u64 = 500_000_000_000_000; // 0.5M MYSO
...
struct StressTestRunner {
    ...
    pub accounts: Vec<MySoAddress>,
    pub active_validators: BTreeSet<MySoAddress>,
    pub preactive_validators: BTreeMap<MySoAddress, u64>,
    pub removed_validators: BTreeSet<MySoAddress>,
    pub delegation_requests_this_epoch: BTreeMap<ObjectID, MySoAddress>,
    pub delegation_withdraws_this_epoch: u64,
    pub delegations: BTreeMap<ObjectID, (MySoAddress, ObjectID, ObjectDigest, SequenceNumber)>,
    pub reports: BTreeMap<MySoAddress, BTreeSet<MySoAddress>>,
    pub rng: StdRng,
}
...
impl StressTestRunner {
    ...
    pub fn pick_random_sender(&mut self) -> MySoAddress {
        self.accounts[self.rng.gen_range(0..self.accounts.len())]
    }

    pub fn system_state(&self) -> MySoSystemStateSummary {
        self.state()
    ...
    pub fn pick_random_active_validator(&mut self) -> MySoValidatorSummary {
    ...
    pub async fn run(&self, sender: MySoAddress, pt: ProgrammableTransaction) -> TransactionEffects {
    ...
    pub fn display_effects(&self, effects: &TransactionEffects) {
        ...
        for (obj_ref, _) in effects.created() {
            ...
            println!(">> {struct_tag} TOTAL_MYSO: {total_myso}");
        }
    ...
    pub fn state(&self) -> Arc<AuthorityState> {
        self.test_cluster.fullnode_handle.myso_node.state()
    }
...
mod add_stake {
    ...
    impl GenStateChange for RequestAddStakeGen {
        ...
        fn create(&self, runner: &mut StressTestRunner) -> Self::StateChange {
            ...
            let staked_with = runner.pick_random_active_validator().myso_address;
    ...
    impl StatePredicate for RequestAddStake {
        async fn run(&mut self, runner: &mut StressTestRunner) -> Result<TransactionEffects> {
            let pt = {
                ...
                builder.obj(ObjectArg::MYSO_SYSTEM_MUT).unwrap();
                builder.pure(self.staked_with).unwrap();
                let coin = StressTestRunner::split_off(&mut builder, self.stake_amount);
                move_call! {
                    builder,
                    (MYSO_SYSTEM_PACKAGE_ID)::myso_system::request_add_stake(Argument::Input(0), coin, Argument::Input(1))
                };
                builder.finish()
            };
        ...
        async fn pre_epoch_post_condition(
            ...
            // Assert that a `StakedMySo` object matching the amount delegated is created.
            // Assert that this staked myso
            let object = runner
                .get_created_object_of_type_name(effects, "StakedMySo")
                .await
                .unwrap();
            ...
            let staked_myso: StakedMySo =
                bcs::from_bytes(object.data.try_as_move().unwrap().contents()).unwrap();

            assert_eq!(staked_myso.principal(), self.stake_amount);
    ...
    impl StatePredicate for RequestWithdrawStake {
        async fn run(&mut self, runner: &mut StressTestRunner) -> Result<TransactionEffects> {
            let pt = {
                ...
                move_call! {
                    builder,
                    (MYSO_SYSTEM_PACKAGE_ID)::myso_system::request_withdraw_stake(Argument::Input(0), staked_myso)
                };
                builder.finish()
            };
...
async fn fuzz_dynamic_committee() {
    ...
    let mut initial_committee = runner
        ...
        .active_validators
        .iter()
        .map(|v| (v.myso_address, v.voting_power))
        .collect::<Vec<_>>();
    ...
    let total_stake = active_validators
        .iter()
        .fold(0, |acc, v| acc + v.staking_pool_myso_balance);

    // Use the formula for voting_power from MySo System to check if the voting power is correctly
    // set. See `crates/myso-framework/packages/myso-system/sources/voting_power.move`.
    ...
    active_validators.iter().for_each(|v| {
        ...
        let calculated_power =
            ((v.staking_pool_myso_balance as u128 * 10_000) / total_stake as u128).min(1_000) as u64;
