...
use myso_system::myso_system_state_inner::{Self, MySoSystemStateInner};
...
fun load_inner_maybe_upgrade(self: &mut MySoSystemState): &mut MySoSystemStateInner {
    let version = self.version;
    // TODO: This is where we check the version and perform upgrade if necessary.

    let inner: &mut MySoSystemStateInner = dynamic_field::borrow_mut(&mut self.id, version);
