...
async fn test_client_multiple_streams() {
    ...
    let sender = test_cluster.wallet.config.keystore.addresses()[0];
    let stream_id_1 = MySoAddress::from(package_id_1);
    let stream_id_2 = MySoAddress::from(package_id_2);
...
async fn test_client_resume_from_checkpoint_without_events() {
    ...
    let myso_light_client::authenticated_events::ClientError::InternalError(msg) = e else {
...
async fn test_client_pruned_checkpoint_error() {
    ...
    assert!(
        matches!(
            e,
            myso_light_client::authenticated_events::ClientError::RpcError(_)
        ),
...
async fn test_client_detects_corrupted_event_data() {
    use myso_macros::register_fail_point_if;
...
async fn test_client_pagination_limit_forward_progress() {
    ...
    let config = myso_light_client::authenticated_events::ClientConfig::new(
        5,                                     /* page_size */
