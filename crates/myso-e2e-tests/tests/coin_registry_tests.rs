...
use myso_types::{MYSO_COIN_REGISTRY_OBJECT_ID, MYSO_FRAMEWORK_PACKAGE_ID};
...
async fn test_create_coin_registry_object() {
    let _guard =
        myso_protocol_config::ProtocolConfig::apply_overrides_for_testing(|_, mut config| {
    ...
    let framework = myso_framework_snapshot::load_bytecode_snapshot(95)
        .unwrap()
        .into_iter()
    ...
    let package = framework
        .iter()
        .find(|f| f.id() == MYSO_FRAMEWORK_PACKAGE_ID)
        .unwrap()
        .data
        .try_as_package()
        .unwrap();
        ...
        h.with(|node| {
            assert!(
                node.state()
                    ...
                    .get_latest_object_ref_or_tombstone(MYSO_COIN_REGISTRY_OBJECT_ID)
                    .is_none()
            );
        });
    }
