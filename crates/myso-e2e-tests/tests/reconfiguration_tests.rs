    ...
    *expired_data.expiration_mut_for_testing() = TransactionExpiration::Epoch(0);
    ...
    assert!(
        result
            .unwrap_err()
            .to_string()
            .contains(&MySoErrorKind::TransactionExpired.to_string())
    );
...
async fn test_create_advance_epoch_tx_race() {
    ...
    let register_wait = |failpoint, node_id, tx: Arc<broadcast::Sender<()>>| {
        let node = myso_simulator::task::NodeId(node_id);
...
async fn test_validator_candidate_pool_read() {
    ...
    let address: MySoAddress = (&new_validator.account_key_pair.public()).into();
    let test_cluster = TestClusterBuilder::new()
    ...
    test_cluster.fullnode_handle.myso_node.with(|node| {
        let system_state = node
            ...
            .unwrap();
        let system_state_summary = system_state.clone().into_myso_system_state_summary();
        ...
        .unwrap();
        assert_eq!(validator.myso_address, address);
    });
}
...
async fn test_inactive_validator_pool_read() {
    ...
    let address = validator.with(|node| node.get_config().myso_address());
    let staking_pool_id = test_cluster.fullnode_handle.myso_node.with(|node| {
        node.state()
            .get_myso_system_state_object_for_testing()
            .unwrap()
...
const VALIDATOR_STARTING_STAKE: u64 = 1_000_000_000_000_000; // 1M MYSO
...
async fn test_reconfig_with_committee_change_basic() {
    ...
    let total_stake = test_cluster.fullnode_handle.myso_node.with(|node| {
        node.state()
            .get_myso_system_state_object_for_testing()
            .unwrap()
            .into_myso_system_state_summary()
...
async fn test_epoch_flag_upgrade() {
    ...
    register_fail_point_arg("initial_epoch_flags", move || {
        ...
        let current_node = myso_simulator::current_simnode_id();
...
async fn safe_mode_reconfig_test() {
    use myso_test_transaction_builder::make_staking_transaction;
    use myso_types::myso_system_state::advance_epoch_result_injection;
...
async fn add_validator_candidate(
    ...
    let cur_validator_candidate_count = test_cluster.fullnode_handle.myso_node.with(|node| {
        node.state()
            .get_myso_system_state_object_for_testing()
            .unwrap()
            .into_myso_system_state_summary()
    ...
    test_cluster.fullnode_handle.myso_node.with(|node| {
        let system_state = node
            ...
            .get_myso_system_state_object_for_testing()
            .unwrap();
        let system_state_summary = system_state.into_myso_system_state_summary();
        assert_eq!(
...
async fn execute_remove_validator_tx(test_cluster: &TestCluster, handle: &MySoNodeHandle) {
    let address = handle.with(|node| node.get_config().myso_address());
    let gas = test_cluster
        .wallet
        .get_one_gas_object_owned_by_address(address)
        .await
...
async fn execute_add_stake_transaction(
    ...
    let tx = {
        ...
        let system_arg = ptb.obj(ObjectArg::MYSO_SYSTEM_MUT).unwrap();
...
async fn execute_add_validator_transactions(
    ...
    let pending_active_count = test_cluster.fullnode_handle.myso_node.with(|node| {
        let system_state = node
            .state()
            .get_myso_system_state_object_for_testing()
            .unwrap();
    ...
    test_cluster.fullnode_handle.myso_node.with(|node| {
        ...
        assert_eq!(
            pending_active_validators[pending_active_validators.len() - 1].myso_address,
            address
        );
    });
}
