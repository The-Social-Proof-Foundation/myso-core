...
use myso_types::base_types::{FullObjectRef, MySoAddress};
...
async fn party_object_transfer_multi_certs() {
    ...
    myso_macros::register_fail_point_async("transaction_execution_delay", move || async move {
        let delay = {
...
async fn party_object_grpc() {
    ...
    assert_eq!(new_owner.address, Some(MySoAddress::ZERO.to_string()));
...
async fn party_coin_grpc() {
    ...
    use myso_types::transaction::{CallArg, ObjectArg, TransactionData};
    ...
    builder.programmable_move_call(
        ...
        vec!["0x2::coin::Coin<0x2::myso::MYSO>".parse().unwrap()],
    ...
    let kind = myso_types::transaction::TransactionKind::ProgrammableTransaction(ptb);
...
async fn party_object_jsonrpc() {
    ...
    #[allow(deprecated)]
    let client = test_cluster.myso_client();

    let object = client
        .read_api()
        .get_object_with_options(
            object_id,
            myso_json_rpc_types::MySoObjectDataOptions::new().with_owner(),
        )
        .await
        .unwrap()
        .data
    ...
    assert_eq!(new_owner_address, MySoAddress::ZERO);
