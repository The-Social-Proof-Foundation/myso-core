...
async fn basic_checkpoints_integration_test() {
    ...
    let tx = make_transfer_myso_transaction(&test_cluster.wallet, None, None).await;
...
async fn test_checkpoint_split_brain() {
    #[cfg(msim)]
    {
        ...
        // loss of liveness
        use myso_core::authority::{CheckpointTimeoutConfig, init_checkpoint_timeout_config};
...
async fn test_checkpoint_contents_v2_alias_versions() {
    ...
    let test_cluster = TestClusterBuilder::new()
        ...
        .with_state_sync_config(myso_config::p2p::StateSyncConfig {
            ...
            &TestTransactionBuilder::new(account, gas_objects[0], gas_price)
                .move_call(
                    ...
                    vec![CallArg::Object(ObjectArg::SharedObject {
                        id: MYSO_ADDRESS_ALIAS_STATE_OBJECT_ID,
    ...
    let mut client = test_cluster
        ...
        .unwrap();
    let request = myso_types::messages_grpc::RawSubmitTxRequest {
        transactions: vec![
            bcs::to_bytes(&enable_tx).unwrap().into(),
            bcs::to_bytes(&transfer_tx).unwrap().into(),
        ],
        submit_type: myso_types::messages_grpc::SubmitTxType::SoftBundle.into(),
    };
    let result = client
    ...
    assert!(matches!(
        ...
        Some(myso_types::messages_grpc::RawValidatorSubmitStatus::Submitted(_))
    ));
    assert!(matches!(
    ...
    let post_enable_tx = test_cluster
        ...
        .sign_transaction(
            &TestTransactionBuilder::new(account, gas_objects[2], gas_price)
                .transfer_myso(None, account)
                .build(),
        )
        .await;
    let post_enable_digest = *post_enable_tx.digest();
    let result = client
        .submit_transaction(myso_types::messages_grpc::RawSubmitTxRequest {
            transactions: vec![bcs::to_bytes(&post_enable_tx).unwrap().into()],
            submit_type: myso_types::messages_grpc::SubmitTxType::Default.into(),
        })
        .await
        .unwrap()
        .into_inner();
