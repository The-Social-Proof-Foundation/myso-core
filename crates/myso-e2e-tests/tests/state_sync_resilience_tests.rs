...
mod test {
    ...
    async fn test_state_sync_with_degraded_peers() {
        ...
        let degraded_id_1 = degraded_fullnode_1.myso_node.with(|n| n.get_sim_node_id());
        let degraded_id_2 = degraded_fullnode_2.myso_node.with(|n| n.get_sim_node_id());
        let syncing_node_id = syncing_node.myso_node.with(|n| n.get_sim_node_id());
        ...
        loop {
            let syncing = syncing_node.myso_node.with(|n| {
                n.state()
                    .get_checkpoint_store()
                    .get_highest_synced_checkpoint_seq_number()
                    .unwrap()
