...
async fn validate_chunked_execution_time_storage(test_cluster: &TestCluster) {
    ...
    let system_state = myso_system_state::get_myso_system_state(object_store.as_ref())
        .expect("System state must exist");

    let extra_fields_id = match &system_state {
        myso_types::myso_system_state::MySoSystemState::V2(system_state) => {
            system_state.extra_fields.id.id.bytes
        }
        #[cfg(msim)]
        myso_types::myso_system_state::MySoSystemState::SimTestDeepV2(system_state) => {
            system_state.extra_fields.id.id.bytes
        }
        #[cfg(msim)]
        myso_types::myso_system_state::MySoSystemState::SimTestShallowV2(system_state) => {
            system_state.extra_fields.id.id.bytes
        }
        myso_types::myso_system_state::MySoSystemState::V1(_) => {
            panic!("MySoSystemState V1 not supported for chunking validation");
        }
        #[cfg(msim)]
        myso_types::myso_system_state::MySoSystemState::SimTestV1(_) => {
            panic!("MySoSystemState V1 not supported for chunking validation");
        }
    };
...
async fn test_chunked_execution_time() {
    ...
    let (package_id, _, _) =
        myso_test_transaction_builder::publish_package(&test_cluster.wallet, path).await;
...
async fn send_transactions(
    ...
    for i in 0..6 {
        ...
        let counter_arg = ptb
            .obj(myso_types::transaction::ObjectArg::SharedObject {
                id: counter_id,
                initial_shared_version,
