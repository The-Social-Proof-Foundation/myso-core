...
use myso_json_rpc_types::{
    EventPage, MySoEvent, MySoTransactionBlockEffectsAPI, MySoTransactionBlockResponse,
...
use myso_types::messages_grpc::TransactionInfoRequest;
use myso_types::object::{Object, ObjectRead, Owner, PastObjectRead};
...
async fn test_full_node_follows_txes() -> Result<(), anyhow::Error> {
    ...
    let fullnode = test_cluster.spawn_new_fullnode().await.myso_node;

    let context = &mut test_cluster.wallet;

    // TODO: test fails on CI due to flakiness without this. Once https://github.com/the-social-proof-foundation/myso-core/pull/7056 is
...
async fn test_full_node_move_function_index() -> Result<(), anyhow::Error> {
    telemetry_subscribers::init_for_testing();
    let mut test_cluster = TestClusterBuilder::new().build().await;
    let node = &test_cluster.fullnode_handle.myso_node;
    let sender = test_cluster.get_address_0();
    let context = &mut test_cluster.wallet;
...
async fn test_full_node_indexes() -> Result<(), anyhow::Error> {
    ...
    let sender_balance_change = BalanceChange {
        ...
        owner: sender,
        coin_type: parse_struct_tag("0x2::myso::MYSO").unwrap(),
...
async fn test_full_node_event_read_api_ok() {
    ...
    let events: Vec<MySoEvent> = jsonrpc_client
        .request("myso_getEvents", params)
        .await
        .unwrap();
...
async fn test_full_node_event_query_by_module_ok() {
    ...
    let page: EventPage = jsonrpc_client
        .request("mysox_queryEvents", params)
        .await
        .unwrap();
...
async fn test_execute_tx_with_serialized_signature() -> Result<(), anyhow::Error> {
    ...
    for txn in txns {
        ...
        let response: MySoTransactionBlockResponse = jsonrpc_client
            .request("myso_executeTransactionBlock", params)
            .await
            .unwrap();

        let MySoTransactionBlockResponse {
            digest,
...
async fn test_full_node_transaction_orchestrator_rpc_ok() -> Result<(), anyhow::Error> {
    ...
    let _response: MySoTransactionBlockResponse = jsonrpc_client
        .request("myso_getTransactionBlock", rpc_params![*tx_digest])
        .await
        .unwrap();
...
async fn test_get_objects_read() -> Result<(), anyhow::Error> {
    ...
    // Transfer some MYSO to recipient
...
async fn test_pass_back_no_object() -> Result<(), anyhow::Error> {
    ...
    // TODO: this is publishing the wrong package - we should be publishing the one in `myso-core/src/unit_tests/data` instead.
...
async fn test_access_old_object_pruned() {
    ...
    let effects = test_cluster
        .sign_and_execute_transaction(&tx_builder.transfer_myso(None, sender).build())
        .await
        .effects;
...
async fn test_full_node_run_with_range_checkpoint() -> Result<(), anyhow::Error> {
    ...
    assert!(test_cluster.fullnode_handle.myso_node.with(|node| {
        node.state()
