...
fn build(path: &Path) -> anyhow::Result<CompiledPackage> {
    let mut config = myso_move_build::BuildConfig::new_for_testing();
...
fn test_metered_move_bytecode_verifier() {
    ...
    let path = tempdir
        .path()
        .join("crates/myso-framework/packages/myso-framework");
    ...
    let mut meter = MySoVerifierMeter::new(meter_config.clone());
    ...
    let mut meter = MySoVerifierMeter::new(meter_config);
    let timer_start = Instant::now();
    let r = run_metered_move_bytecode_verifier(
        &compiled_modules,
        &verifier_config,
