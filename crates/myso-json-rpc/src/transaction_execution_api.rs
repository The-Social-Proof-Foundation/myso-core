...
use myso_json_rpc_api::{JsonRpcMetrics, WriteApiOpenRpc, WriteApiServer};
use myso_json_rpc_types::{
    DevInspectArgs, DevInspectResults, DryRunTransactionBlockResponse, MySoTransactionBlock,
...
impl TransactionExecutionApi {
    ...
    > {
        ...
        let transaction = if opts.show_input {
            ...
            Some(MySoTransactionBlock::try_from(
                txn.data().clone(),
    ...
    ) -> Result<MySoTransactionBlockResponse, Error> {
        let request_type =
    ...
    async fn handle_post_orchestration(
        ...
        transaction: Option<MySoTransactionBlock>,
        ...
        let events = if opts.show_events {
            ...
            Some(MySoTransactionBlockEvents::try_from(
        ...
        Ok(MySoTransactionBlockResponse {
            digest,
            transaction,
            raw_transaction,
            effects: opts
    ...
    pub fn prepare_dry_run_transaction_block(
        &self,
        tx_bytes: Base64,
    ) -> Result<(TransactionData, TransactionDigest, Vec<InputObjectKind>), MySoRpcInputError> {
        let tx_data: TransactionData = self.convert_bytes(tx_bytes)?;
...
impl MySoRpcModule for TransactionExecutionApi {
