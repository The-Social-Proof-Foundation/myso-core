...
use myso_types::base_types::{ObjectID, SequenceNumber, TransactionDigest};
use myso_types::crypto::AggregateAuthoritySignature;
use myso_types::display::DisplayVersionUpdatedEvent;
use myso_types::effects::{TransactionEffects, TransactionEffectsAPI, TransactionEvents};
use myso_types::error::{MySoError, MySoObjectResponseError};
...
impl ReadApi {
    ...
    async fn multi_get_transaction_blocks_internal(
        ...
        opts: Option<MySoTransactionBlockResponseOptions>,
    ) -> Result<Vec<MySoTransactionBlockResponse>, Error> {
        trace!("start");

        let num_digests = digests.len();
        if num_digests > *QUERY_MAX_RESULT_LIMIT {
            Err(MySoRpcInputError::SizeLimitExceeded(
        ...
        if temp_response.len() < num_digests {
            Err(MySoRpcInputError::ContainsDuplicates)?
        }
            ...
            for (_, cache_entry) in temp_response.iter_mut() {
                    ...
                    cache_entry.effects.as_ref().and_then(|e| e.events_digest())
                {
                    match events.next() {
                        Some(Some(ev)) => {
                            cache_entry.events =
                                Some(to_myso_transaction_events(self, cache_entry.digest, ev)?)
                        }
                        None | Some(None) => {
                            error!(
                    ...
                    cache_entry.events = Some(MySoTransactionBlockEvents::default());
                }
            }
        }
        ...
        if opts.show_balance_changes || opts.show_object_changes {
            ...
            for resp in temp_response.values() {
                let effects = resp.effects.as_ref().ok_or_else(|| {
                    MySoRpcInputError::GenericNotFound(
...
impl ReadApiServer for ReadApi {
                ...
                ObjectRead::Exists(object_ref, o, layout) => {
                    ...
                    if options.show_display {
                            ...
                            .await
                        {
                            ...
                            Err(e) => {
                                return Ok(MySoObjectResponse::new(
                                    Some((object_ref, o, layout, options, None).try_into()?),
                                    Some(MySoObjectResponseError::DisplayError {
                                        error: e.to_string(),
                                    }),
                                ));
                            }
                    ...
                    Ok(MySoObjectResponse::new_with_data(
    ...
    async fn multi_get_objects(
        ...
        options: Option<MySoObjectDataOptions>,
    ) -> RpcResult<Vec<MySoObjectResponse>> {
        with_tracing!(async move {
            if object_ids.len() <= *QUERY_MAX_RESULT_LIMIT {
                ...
                let objects_result: Result<Vec<MySoObjectResponse>, String> = results
                    .into_iter()
    ...
    async fn try_get_past_object(
        ...
        options: Option<MySoObjectDataOptions>,
    ) -> RpcResult<MySoPastObjectResponse> {
        with_tracing!(async move {
            let state = self.state.clone();
            let past_read = state
            ...
            match past_read {
                PastObjectRead::ObjectNotExists(id) => {
                    Ok(MySoPastObjectResponse::ObjectNotExists(id))
                }
                PastObjectRead::VersionFound(object_ref, o, layout) => {
                    ...
                    Ok(MySoPastObjectResponse::VersionFound(
                        (object_ref, o, layout, options, display_fields).try_into()?,
                    ))
                }
                PastObjectRead::ObjectDeleted(oref) => {
                    Ok(MySoPastObjectResponse::ObjectDeleted(oref.into()))
                }
                PastObjectRead::VersionNotFound(id, seq_num) => {
                    Ok(MySoPastObjectResponse::VersionNotFound(id, seq_num))
                }
                PastObjectRead::VersionTooHigh {
                    object_id,
                    asked_version,
                    latest_version,
                } => Ok(MySoPastObjectResponse::VersionTooHigh {
                    object_id,
                    asked_version,
                    latest_version,
                }),
    ...
    async fn try_get_object_before_version(
        ...
        self.try_get_past_object(
            object_id,
            version,
            Some(MySoObjectDataOptions::bcs_lossless()),
        )
        .await
    }
    ...
    async fn try_multi_get_past_objects(
        ...
        options: Option<MySoObjectDataOptions>,
    ) -> RpcResult<Vec<MySoPastObjectResponse>> {
        with_tracing!(async move {
            if past_objects.len() <= *QUERY_MAX_RESULT_LIMIT {
                ...
                if !errors.is_empty() {
                    let error_string = errors
                        ...
                        .collect::<Vec<String>>()
                        .join("; ");
                    Err(anyhow!("{error_string}").into()) // Collects errors not related to MySoPastObjectResponse variants
                } else {
                    Ok(success)
                }
            } else {
        ...
        with_tracing!(async move {
            ...
            if opts.show_events && temp_response.effects.is_some() {
                ...
                let events = transaction_kv_store
                    ...
                    .flatten();
                match events {
                    None => temp_response.events = Some(MySoTransactionBlockEvents::default()),
                    Some(events) => match to_myso_transaction_events(self, digest, events) {
    ...
    async fn multi_get_transaction_blocks(
        ...
        digests: Vec<TransactionDigest>,
        opts: Option<MySoTransactionBlockResponseOptions>,
    ) -> RpcResult<Vec<MySoTransactionBlockResponse>> {
        with_tracing!(async move {
    ...
    #[instrument(skip(self))]
    async fn get_events(&self, transaction_digest: TransactionDigest) -> RpcResult<Vec<MySoEvent>> {
        with_tracing!(async move {
            let state = self.state.clone();
            let transaction_kv_store = self.transaction_kv_store.clone();
            async move {
                ...
                Ok(match events {
                    Some(events) => events
                        ...
                        .map(|(seq, e)| {
                            ...
                            MySoEvent::try_from(e, transaction_digest, seq as u64, None, layout)
                        })
                        .collect::<Result<Vec<_>, _>>()
                        .map_err(Error::MySoError)?,
                    None => vec![],
                })
            }
            .await
    ...
    async fn get_latest_checkpoint_sequence_number(&self) -> RpcResult<BigInt<u64>> {
        with_tracing!(async move {
            Ok(self
                ...
                .map_err(|e| {
                    MySoRpcInputError::GenericNotFound(format!(
    ...
    async fn get_checkpoints(
        ...
        with_tracing!(async move {
            let limit = validate_limit(limit, QUERY_MAX_RESULT_LIMIT_CHECKPOINTS)
                .map_err(MySoRpcInputError::from)?;
        ...
        with_tracing!(async move {
            version
                .map(|v| {
                    ...
                    .ok_or(MySoRpcInputError::ProtocolVersionUnsupported(
    ...
    async fn verify_zklogin_signature(
        ...
        let zklogin_env_native = match self
            ...
            .chain()
        {
            myso_protocol_config::Chain::Mainnet | myso_protocol_config::Chain::Testnet => {
                ZkLoginEnv::Prod
            }
            _ => ZkLoginEnv::Test,
        };
        let GenericSignature::ZkLoginAuthenticator(zklogin_sig) =
            ...
            return Err(MySoRpcInputError::GenericNotFound(
            ...
            ZkLoginIntentScope::TransactionData => {
                let tx_data: TransactionData =
                    bcs::from_bytes(&Base64::decode(&bytes).map_err(Error::from)?)
                        .map_err(Error::from)?;
                let intent_msg = IntentMessage::new(Intent::myso_transaction(), tx_data.clone());
...
impl MySoRpcModule for ReadApi {
...
#[instrument(skip_all)]
fn to_myso_transaction_events(
    fullnode_api: &ReadApi,
    tx_digest: TransactionDigest,
    events: TransactionEvents,
) -> Result<MySoTransactionBlockEvents, Error> {
    ...
    Ok(MySoTransactionBlockEvents::try_from(
        events,
        tx_digest,
        None,
...
pub enum ObjectDisplayError {
    ...
    #[error(transparent)]
    Deserialization(#[from] MySoError),
...
async fn get_display_fields(
    ...
    let format = match Format::parse(MAX_DISPLAY_NESTED_LEVEL, &display_object.fields) {
        ...
        Err(e) => {
            return Ok(DisplayFieldsResponse {
                data: None,
                error: Some(MySoObjectResponseError::DisplayError {
                    error: e.to_string(),
                }),
            });
        }
    ...
    Ok(DisplayFieldsResponse {
        ...
        error: (!errors.is_empty()).then(|| MySoObjectResponseError::DisplayError {
            error: errors.join("; "),
        }),
    })
}
...
fn convert_to_response(
    ...
    opts: &MySoTransactionBlockResponseOptions,
    module_cache: &impl GetModule,
) -> RpcInterimResult<MySoTransactionBlockResponse> {
    let mut response = MySoTransactionBlockResponse::new(cache.digest);
    ...
    if let Some(transaction) = cache.transaction {
        ...
        if opts.show_input {
            response.transaction = Some(MySoTransactionBlock::try_from(
