...
pub enum Error {
    ...
    #[error(transparent)]
    MySoObjectResponseError(#[from] MySoObjectResponseError),

    #[error(transparent)]
    MySoRpcInputError(#[from] MySoRpcInputError),
...
impl From<MySoErrorKind> for Error {
    fn from(e: MySoErrorKind) -> Self {
        match e {
            ...
            other => Self::MySoError(MySoError(Box::new(other))),
        }
    }
}

impl From<MySoError> for Error {
...
impl From<Error> for ErrorObjectOwned {
    ...
    fn from(e: Error) -> ErrorObjectOwned {
        match e {
            ...
            Error::MySoObjectResponseError(err) => match err {
                MySoObjectResponseError::NotExists { .. }
            ...
            Error::MySoError(myso_error) => match myso_error.as_inner() {
                MySoErrorKind::TransactionNotFound { .. }
                | MySoErrorKind::TransactionsNotFound { .. }
                | MySoErrorKind::TransactionEventsNotFound { .. } => invalid_params(myso_error),
                _ => failed(myso_error),
            },
...
pub enum MySoRpcInputError {
    ...
    #[error("{0}")]
    CannotParseMySoStructTag(String),
...
impl From<MySoRpcInputError> for RpcError {
    fn from(e: MySoRpcInputError) -> Self {
        RpcError::Call(invalid_params(e))
    }
}

impl From<MySoRpcInputError> for ErrorObjectOwned {
    fn from(e: MySoRpcInputError) -> Self {
        invalid_params(e)
    }
}
...
mod tests {
    ...
    use myso_types::crypto::AuthorityPublicKey;
    ...
    mod match_transaction_submission_error_tests {
        ...
        fn test_objects_double_used() {
            use myso_types::crypto::VerifyingKey;
        ...
        fn test_non_recoverable_transaction_error_with_transient_errors() {
            let quorum_driver_error = TransactionSubmissionError::NonRecoverableTransactionError {
                errors: vec![
                    ...
                    (
                        MySoErrorKind::RpcError("Hello".to_string(), "Testing".to_string()).into(),
                        0,
                        vec![],
                    ),
                ],
        ...
        fn test_transaction_driver_internal_error() {
            let quorum_driver_error = TransactionSubmissionError::TransactionDriverInternalError(
                MySoErrorKind::UnexpectedMessage("test".to_string()).into(),
            );
