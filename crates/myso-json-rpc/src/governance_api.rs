...
use myso_json_rpc_api::{GovernanceReadApiOpenRpc, GovernanceReadApiServer, JsonRpcMetrics};
...
impl GovernanceReadApi {
    ...
    async fn get_staked_myso(&self, owner: MySoAddress) -> Result<Vec<StakedMySo>, Error> {
        let state = self.state.clone();
        let result = state.get_staked_myso(owner).await?;

        self.metrics
            .get_stake_myso_result_size
            .observe(result.len() as f64);
        self.metrics
            .get_stake_myso_result_size_total
    ...
    async fn get_stakes_by_ids(
        ...
        let state = self.state.clone();
        let stakes_read: Vec<_> = staked_myso_ids
            .iter()
        ...
        let mut stakes: Vec<(StakedMySo, bool)> = vec![];
        for stake in stakes_read.into_iter() {
            match stake {
                ObjectRead::Exists(_, o, _) => stakes.push((StakedMySo::try_from(&o)?, true)),
                ObjectRead::Deleted(oref) => {
                    match self
                        ...
                        .await?
                    {
                        Some(o) => stakes.push((StakedMySo::try_from(&o)?, false)),
                        None => Err(MySoRpcInputError::UserInputError(
                ...
                ObjectRead::NotExists(id) => Err(MySoRpcInputError::UserInputError(
                    UserInputError::ObjectNotFound {
                        object_id: id,
                        version: None,
                    },
    ...
    async fn get_stakes(&self, owner: MySoAddress) -> Result<Vec<DelegatedStake>, Error> {
        let timer = self.metrics.get_stake_myso_latency.start_timer();
        let stakes = self.get_staked_myso(owner).await?;
        if stakes.is_empty() {
            return Ok(vec![]);
        }
        drop(timer);

        let _timer = self.metrics.get_delegated_myso_latency.start_timer();
    ...
    async fn get_delegated_stakes(
        ...
        let system_state_summary: MySoSystemStateSummary =
            system_state.clone().into_myso_system_state_summary();
    ...
    fn get_system_state(&self) -> Result<MySoSystemState, Error> {
...
impl GovernanceReadApiServer for GovernanceReadApi {
    ...
    async fn get_stakes_by_ids(
        ...
        with_tracing!(async move { self.get_stakes_by_ids(staked_myso_ids).await })
    }
    ...
    #[instrument(skip(self))]
    async fn get_committee_info(&self, epoch: Option<BigInt<u64>>) -> RpcResult<MySoCommittee> {
    ...
    async fn get_validators_apy(&self) -> RpcResult<ValidatorApys> {
        ...
        let system_state_summary: MySoSystemStateSummary =
            self.get_latest_myso_system_state().await?;
    ...
    for (address, pool_id, exchange_rates_id, exchange_rates_size, active) in tables {
        let mut rates = state
            ...
            .map(|df| {
                ...
                Ok::<_, MySoError>((epoch, exchange_rate))
            })
            .collect::<Result<Vec<_>, _>>()?;
...
impl MySoRpcModule for GovernanceReadApi {
