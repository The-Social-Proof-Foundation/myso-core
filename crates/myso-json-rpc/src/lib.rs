...
pub fn myso_rpc_doc(version: &str) -> Project {
    Project::new(
        version,
        "MySo JSON-RPC",
...
impl JsonRpcServerBuilder {
    ...
    pub fn register_module<T: MySoRpcModule>(&mut self, module: T) -> Result<(), Error> {
    ...
    pub async fn start(
        ...
        info!(local_addr =? addr, "MySo JSON-RPC server listening on {addr}");
        Ok(handle)
    }
}
