...
use myso_json_rpc_api::{CoinReadApiOpenRpc, CoinReadApiServer, JsonRpcMetrics, cap_page_limit};
use myso_json_rpc_types::Balance;
use myso_json_rpc_types::{CoinPage, MySoCoinMetadata};
...
use myso_types::gas_coin::{GAS, TOTAL_SUPPLY_MIST};
...
pub fn parse_to_struct_tag(coin_type: &str) -> Result<StructTag, MySoRpcInputError> {
    parse_myso_struct_tag(coin_type)
        .map_err(|e| MySoRpcInputError::CannotParseMySoStructTag(format!("{e}")))
}

pub fn parse_to_type_tag(coin_type: Option<String>) -> Result<TypeTag, MySoRpcInputError> {
...
impl MySoRpcModule for CoinReadApi {
...
impl CoinReadApiServer for CoinReadApi {
            ...
            let cursor = match cursor {
                Some(c) => {
                    let decoded = CoinCursor::decode(&c).ok_or_else(|| {
                        MySoRpcInputError::GenericInvalid("invalid cursor".to_string())
                    })?;
    ...
    #[instrument(skip(self))]
    async fn get_all_balances(&self, owner: MySoAddress) -> RpcResult<Vec<Balance>> {
        with_tracing!(async move {
    ...
    #[instrument(skip(self))]
    async fn get_coin_metadata(&self, coin_type: String) -> RpcResult<Option<MySoCoinMetadata>> {
        with_tracing!(async move {
...
async fn find_package_object_id(
    ...
    async move {
        ...
        Err(MySoRpcInputError::GenericNotFound(format!(
...
mod tests {
    ...
    use myso_types::messages_checkpoint::{CheckpointDigest, CheckpointSequenceNumber};
    use myso_types::object::MoveObject;
    use myso_types::object::Object;
    use myso_types::object::Owner;
    use myso_types::utils::create_fake_transaction;
    use myso_types::{TypeTag, parse_myso_struct_tag};
    ...
    mock! {
        ...
        impl TransactionKeyValueStoreTrait for KeyValueStore {
            ...
            ) -> MySoResult<KVStoreTransactionData>;
            ...
            ) -> MySoResult<KVStoreCheckpointData>;
            ...
            async fn get_object(&self, object_id: ObjectID, version: SequenceNumber) -> MySoResult<Option<Object>>;
            async fn multi_get_objects(&self, object_keys: &[myso_types::storage::ObjectKey]) -> MySoResult<Vec<Option<Object>>>;
            ...
            async fn multi_get_events_by_tx_digests(&self,digests: &[TransactionDigest]) -> MySoResult<Vec<Option<TransactionEvents>>>;
        }
    }
    ...
    fn get_test_coin_type_tag(coin_type: String) -> TypeTag {
        TypeTag::Struct(Box::new(parse_myso_struct_tag(&coin_type).unwrap()))
    }
    ...
    fn get_test_treasury_cap_peripherals(
        ...
        let input_coin_struct = parse_myso_struct_tag(&coin_name).expect("should not fail");
    ...
    fn create_currency_object(currency: Currency) -> Object {
        use myso_types::object::{Data, ObjectInner};
    ...
    mock! {
        pub ObjectStore {}
        impl myso_types::storage::ObjectStore for ObjectStore {
    ...
    mod get_coins_tests {
        ...
        async fn test_coin_no_cursor() {
            ...
            let coin_type_tag =
                TypeTag::Struct(Box::new(parse_myso_struct_tag(&coin.coin_type).unwrap()));
            let mut mock_state = MockStateRead::new();
            mock_state
                .expect_get_owned_coins()
                .with(
        ...
        async fn test_unrecognized_token() {
            ...
            expected.assert_eq(&error_object.code().to_string());
            let expected =
                expect!["Invalid struct type: 0x2::myso:🤵. Got error: unrecognized token: :🤵"];
            expected.assert_eq(error_object.message());
        }
            ...
            mock_state
                ...
                .returning(move |_, _, _, _| {
                    Err(StateReadError::Client(
                        MySoErrorKind::IndexStoreNotAvailable.into(),
                    ))
                });
            let coin_read_api = CoinReadApi::new_for_tests(Arc::new(mock_state), None);
            let response = coin_read_api
        ...
        async fn test_get_coins_iterator_typed_store_error() {
            ...
            mock_state
                .expect_get_owned_coins()
                .returning(move |_, _, _, _| {
                    Err(MySoErrorKind::Storage("mock rocksdb error".to_string()).into())
                });
            let coin_read_api = CoinReadApi::new_for_tests(Arc::new(mock_state), None);
            let response = coin_read_api
                .get_coins(owner, Some(coin_type.to_string()), None, None)
    ...
    mod get_balance_tests {
        ...
        async fn test_get_balance_execution_error() {
            ...
            let mut mock_state = MockStateRead::new();
            mock_state.expect_get_balance().returning(move |_, _| {
                Err(MySoErrorKind::ExecutionError("mock db error".to_string()).into())
            });
            let coin_read_api = CoinReadApi::new_for_tests(Arc::new(mock_state), None);
            let response = coin_read_api
                .get_balance(owner, Some(coin_type.to_string()))
    ...
    mod get_all_balances_tests {
        ...
        async fn test_index_store_not_available() {
            ...
            mock_state.expect_get_all_balance().returning(move |_| {
                Err(StateReadError::Client(
                    MySoError(Box::new(MySoErrorKind::IndexStoreNotAvailable)).into(),
                ))
            });
            let coin_read_api = CoinReadApi::new_for_tests(Arc::new(mock_state), None);
            let response = coin_read_api.get_all_balances(owner).await;
    ...
    mod get_coin_metadata_tests {
        ...
        async fn test_valid_coin_metadata_object() {
            ...
            let metadata = MySoCoinMetadata::try_from(coin_metadata_object.clone()).unwrap();
        ...
        #[tokio::test]
        async fn test_find_package_object_not_myso_coin_metadata() {
            let package_id = get_test_package_id();
            let coin_name = get_test_coin_type(package_id);
            let input_coin_struct = parse_myso_struct_tag(&coin_name).expect("should not fail");
            let coin_metadata_struct = CoinMetadata::type_(input_coin_struct.clone());
        ...
        async fn test_coin_metadata_with_registry_data() {
            use myso_types::coin_registry::{self, Currency, MetadataCapState, SupplyState};
            use myso_types::collection_types::VecMap;

            let package_id = get_test_package_id();
            let coin_name = get_test_coin_type(package_id);
            let coin_struct = parse_myso_struct_tag(&coin_name).expect("should not fail");
