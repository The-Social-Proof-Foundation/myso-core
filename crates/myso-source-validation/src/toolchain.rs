...
const CANONICAL_UNIX_BINARY_NAME: &str = "myso";
const CANONICAL_WIN_BINARY_NAME: &str = "myso.exe";
...
impl ToolchainVersion {
    ...
    pub fn read(root_path: &Path, env: &Environment) -> anyhow::Result<Option<ToolchainVersion>> {
        ...
        if published_path.exists() {
            ...
            let parsed: ParsedPublishedFile<MySoFlavor> =
...
pub(crate) fn current_toolchain() -> ToolchainVersion {
    ToolchainVersion {
        ...
        flavor: Flavor::MySo,     /* does not matter, unused for current_toolchain */
    }
}
...
pub(crate) fn units_for_toolchain(
    ...
    if std::env::var("MYSO_RUN_TOOLCHAIN_BUILD").is_err() {
    ...
    for (package, local_unit) in compiled_units {
        ...
        if myso_types::is_system_package(local_unit.unit.address.into_inner()) {
    ...
    if !dest_canonical_binary.exists() {
        ...
        let mut response = match ureq::get(&mainnet_url).call() {
            ...
            Err(ureq::Error::Status(404, _)) => {
                println!(
                    "{} myso mainnet compiler {} not available, attempting to download testnet compiler release...",
                    "WARNING".bold().yellow(),
                    compiler_version.yellow()
                );
                println!(
        ...
        if platform == "windows-x86_64" {
            dest_binary.push(format!("myso-{platform}.exe"));
        } else {
            dest_binary.push(format!("myso-{platform}"));
        }
...
fn detect_platform(
    ...
    let s = match (std::env::consts::OS, std::env::consts::ARCH) {
        ...
        (os, arch) => {
            ...
            bail!(
                "The package {} needs to be built with myso compiler version {compiler_version} but there \
