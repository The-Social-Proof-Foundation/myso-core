...
impl<'a> BytecodeSourceVerifier<'a> {
    ...
    async fn pkg_for_address(&self, addr: AccountAddress) -> Result<MovePackage, Error> {
        ...
        // fetched from a myso network via myso_getObject, which takes an object ID
        let obj_id = ObjectID::from(addr);

        // fetch the MySo object at the address specified for the package in the local resolution table
        ...
        match &obj.data {
            myso_types::object::Data::Package(pkg) => Ok(pkg.to_owned()),
            myso_types::object::Data::Move(_) => Err(Error::ObjectFoundWhenPackageExpected(obj_id)),
        }
    }
}
