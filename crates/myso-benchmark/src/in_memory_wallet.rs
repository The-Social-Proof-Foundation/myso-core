...
/// A MySo account and all of the objects it owns
#[derive(Debug)]
pub struct MySoAccount {
...
pub struct InMemoryWallet {
    accounts: BTreeMap<MySoAddress, MySoAccount>, // TODO: track shared and immutable objects as well
}
...
impl InMemoryWallet {
    ...
    pub fn add_account(
        ...
        self.accounts.insert(addr, MySoAccount::new(key, gas, objs));
    }
    ...
    pub fn account_mut(&mut self, addr: &MySoAddress) -> Option<&mut MySoAccount> {
        self.accounts.get_mut(addr)
    }

    pub fn account(&self, addr: &MySoAddress) -> Option<&MySoAccount> {
        self.accounts.get(addr)
    }

    pub fn accounts(&self) -> impl Iterator<Item = &MySoAddress> {
        self.accounts.keys()
    }

    pub fn gas(&self, addr: &MySoAddress) -> Option<&ObjectRef> {
        self.accounts.get(addr).map(|a| &a.gas)
    }

    pub fn owned_object(&self, addr: &MySoAddress, id: &ObjectID) -> Option<&ObjectRef> {
        self.accounts.get(addr).and_then(|a| a.owned.get(id))
    }

    pub fn owned_objects(&self, addr: &MySoAddress) -> Option<impl Iterator<Item = &ObjectRef>> {
    ...
    pub fn keypair(&self, addr: &MySoAddress) -> Option<Arc<AccountKeyPair>> {
    ...
    pub fn addresses(&self) -> impl Iterator<Item = &MySoAddress> {
        self.accounts.keys()
    }
