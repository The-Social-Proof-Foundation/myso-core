...
use myso_rpc_api::{Client, client::ExecutedTransaction};
use myso_types::transaction::Argument;
use myso_types::transaction::CallArg;
use myso_types::transaction::ObjectArg;
use myso_types::transaction_driver_types::EffectsFinalityInfo;
use myso_types::transaction_driver_types::FinalizedEffects;
use myso_types::{
    base_types::ObjectID,
...
use myso_types::{base_types::ObjectRef, crypto::AuthorityStrongQuorumSignInfo, object::Owner};
use myso_types::{base_types::SequenceNumber, gas_coin::GasCoin};
use myso_types::{
...
use myso_types::{
    base_types::{AuthorityName, MySoAddress},
    myso_system_state::MySoSystemStateTrait,
};
use myso_types::{
...
impl ExecutionEffects {
        ...
        match self {
            ExecutionEffects::FinalizedTransactionEffects(effects, ..) => {
                match effects.data().status() {
                    myso_types::execution_status::ExecutionStatus::Success => false,
                    myso_types::execution_status::ExecutionStatus::Failure {
                        error:
    ...
    pub fn is_invalid_transaction(&self) -> bool {
        match self {
            ExecutionEffects::FinalizedTransactionEffects(effects, ..) => {
                match effects.data().status() {
                    myso_types::execution_status::ExecutionStatus::Failure { error, .. } => {
                        matches!(
                            error,
...
pub trait ValidatorProxy {
    ...
    async fn get_myso_address_balance(&self, address: MySoAddress) -> Result<u64, anyhow::Error>;
    ...
    async fn get_validators(&self) -> Result<Vec<MySoAddress>, anyhow::Error>;
...
impl ValidatorProxy for LocalValidatorAggregatorProxy {
    async fn get_myso_address_balance(&self, _: MySoAddress) -> Result<u64, anyhow::Error> {
...
async fn execute_soft_bundle_with_retries(
    ...
    use myso_network::tonic::IntoRequest;
    ...
    loop {
            ...
            .await
        {
            ...
            Err(err) => {
                ...
                // Convert tonic error to MySoError to check if retriable
                let myso_error: myso_types::error::MySoError = err.into();
                if myso_error.is_retryable().0
                    && (retry_cnt < max_retries || start.elapsed() < min_retry_duration)
                {
                    let delay = Duration::from_millis(rand::thread_rng().gen_range(100..1000));
                    warn!(
        ...
        enum SubmissionOutcome {
            Submitted(myso_types::messages_consensus::ConsensusPosition),
...
impl FullNodeProxy {
    pub async fn from_url(
        ...
        let myso_client = Client::new(&http_url)?;

        let committee = myso_client.get_committee(None).await?;

        let chain_identifier = myso_client.get_chain_identifier().await?;

        let protocol_config = {
            let resp = myso_client.get_protocol_config(None).await?;
        ...
        let myso_system_state = myso_client.get_system_state_summary(None).await?;
...
impl ValidatorProxy for FullNodeProxy {
    async fn get_myso_address_balance(&self, address: MySoAddress) -> Result<u64, anyhow::Error> {
        let balance = self.myso_client.get_balance(address, &GAS::type_()).await?;
    ...
    async fn execute_transaction_block(
        ...
        while retry_cnt < 10 || start.elapsed() < Duration::from_secs(60) {
            ...
            // MySoClient times out after 60s
            match self
                .myso_client
                .clone()
    ...
    fn clone_new(&self) -> Box<dyn ValidatorProxy + Send + Sync> {
        Box::new(Self {
            myso_client: self.myso_client.clone(),
    ...
    async fn get_validators(&self) -> Result<Vec<MySoAddress>, anyhow::Error> {
        let validators = self
            ...
            .await?
            .active_validators;
        Ok(validators.into_iter().map(|v| v.myso_address).collect())
    }
