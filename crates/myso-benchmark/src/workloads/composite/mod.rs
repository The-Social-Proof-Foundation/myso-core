...
use myso_types::transaction::{Argument, Command, ObjectArg, SharedObjectMutability, Transaction};
use myso_types::{Identifier, MYSO_FRAMEWORK_PACKAGE_ID};
...
pub struct OperationPool {
    ...
    pub chain_identifier: myso_types::digests::ChainIdentifier,
}
...
impl AccountState {
    pub async fn new(
        ...
        while retries < 3 {
            ...
            let Ok(myso_balance) = proxy.get_myso_address_balance(sender).await else {
                info!("Failed to get myso balance for address {sender}");
                retries += 1;
...
pub struct CompositeWorkload {
    ...
    num_payloads: u64,
    metrics: Arc<Mutex<CompositionMetrics>>,
    chain_identifier: Option<myso_types::digests::ChainIdentifier>,
}
