...
use myso_types::transaction::{CallArg, ObjectArg, SharedObjectMutability, Transaction};
use myso_types::{Identifier, MYSO_RANDOMNESS_STATE_OBJECT_ID};
use rand::Rng;
use std::sync::Arc;
...
impl WorkloadBuilder<dyn Payload> for RandomizedTransactionWorkloadBuilder {
    ...
    async fn generate_coin_config_for_payloads(&self) -> Vec<GasCoinConfig> {
        ...
        for _i in 0..self.num_payloads {
            let (address, keypair): (MySoAddress, AccountKeyPair) = get_key_pair();
...
pub struct RandomizedTransactionWorkload {
    ...
    pub transfer_to: Option<MySoAddress>,
