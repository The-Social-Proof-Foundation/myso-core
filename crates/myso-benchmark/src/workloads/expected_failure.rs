...
use myso_types::crypto::{AccountKeyPair, Ed25519MySoSignature};
use myso_types::signature::GenericSignature;
use myso_types::{base_types::ObjectRef, crypto::get_key_pair, transaction::Transaction};
use tracing::debug;
...
impl Workload<dyn Payload> for ExpectedFailureWorkload {
    ...
    async fn make_test_payloads(
        ...
        let mut gas_by_address: HashMap<MySoAddress, Vec<Gas>> = HashMap::new();
        ...
        let addresses: Vec<MySoAddress> = gas_by_address.keys().cloned().collect();
