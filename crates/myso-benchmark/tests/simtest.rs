...
mod test {
    ...
    use myso_benchmark::workloads::adversarial::AdversarialPayloadCfg;
    use myso_benchmark::workloads::benchmark_move_base_dir;
    use myso_benchmark::workloads::composite::CompositeWorkloadConfig;
    use myso_benchmark::workloads::expected_failure::ExpectedFailurePayloadCfg;
    use myso_benchmark::workloads::workload::ExpectedFailureType;
    use myso_benchmark::workloads::workload_configuration::{
    ...
    use myso_config::node::{AuthorityOverloadConfig, ForkCrashBehavior, ForkRecoveryConfig};
    use myso_config::{AUTHORITIES_DB_NAME, MYSO_KEYSTORE_FILENAME};
    ...
    use myso_core::checkpoints::{CheckpointStore, CheckpointWatermark};
    ...
    use myso_simulator::tempfile::TempDir;
    use myso_simulator::{SimConfig, configs::*};
    use myso_surfer::surf_strategy::SurfStrategy;
    use myso_swarm_config::network_config_builder::ConfigBuilder;
    use myso_types::base_types::{AuthorityName, ConciseableName, ObjectID, SequenceNumber};
    ...
    struct DeadValidator {
        node_id: myso_simulator::task::NodeId,
    ...
    async fn test_simulated_load_with_reconfig() {
        myso_protocol_config::ProtocolConfig::poison_get_for_min_version();
    ...
    fn get_keep_alive_nodes(cluster: &TestCluster) -> HashSet<myso_simulator::task::NodeId> {
        ...
        keep_alive_nodes.insert(myso_simulator::current_simnode_id());
        keep_alive_nodes
    }

    fn handle_failpoint(
        dead_validator: Arc<Mutex<Option<DeadValidator>>>,
        keep_alive_nodes: HashSet<myso_simulator::task::NodeId>,
        ...
        let cur_node = myso_simulator::current_simnode_id();
        ...
        if rng.gen_range(0.0..1.0) < probability {
            ...
            *dead_validator = Some(DeadValidator {
            ...
            myso_simulator::task::kill_current_node(Some(restart_after));
        }
    }
    ...
    async fn test_simulated_load_reconfig_with_prune_and_compact() {
        ...
        let node_state = test_cluster.fullnode_handle.myso_node.clone().state();
    ...
    async fn test_data_ingestion_pipeline() {
        ...
        let _checkpoint: myso_types::full_checkpoint_content::Checkpoint = (&proto_checkpoint)
            .try_into()
    ...
    async fn test_protocol_upgrade_compatibility_impl() {
        ...
        let manifest = myso_framework_snapshot::load_bytecode_snapshot_manifest();
        ...
        let init_framework =
            myso_framework_snapshot::load_bytecode_snapshot(starting_version).unwrap();
        let test_cluster = Arc::new(
        ...
        let _handle = tokio::task::spawn(async move {
            ...
            for version in starting_version..=max_ver {
                ...
                let new_framework = myso_framework_snapshot::load_bytecode_snapshot(next_version);
    ...
    fn handle_bool_failpoint(
        eligible_nodes: &HashSet<myso_simulator::task::NodeId>, // only given eligible nodes may fail
        probability: f64,
    ) -> bool {
        if !eligible_nodes.contains(&myso_simulator::current_simnode_id()) {
    ...
    impl SimulatedLoadConfig {
        fn composite_only(
            config: myso_benchmark::workloads::composite::CompositeWorkloadConfig,
        ) -> Self {
            Self {
                composite_weight: 1,
        ...
        Fut: std::future::Future<Output = ()> + Send,
    {
        ...
        let keystore_path = test_cluster.swarm.dir().join(MYSO_KEYSTORE_FILENAME);
        ...
        if enable_surfer {
            let surfer_task = tokio::spawn(async move {
                // now do a myso-surfer test
                let mut test_packages_dir = benchmark_move_base_dir();
                test_packages_dir.extend(["..", "..", "crates", "myso-surfer", "tests"]);
                let test_package_paths: Vec<PathBuf> = std::fs::read_dir(test_packages_dir)
                    ...
                    .collect();
                info!("using myso_surfer test packages: {test_package_paths:?}");

                let surf_strategy = SurfStrategy::new(Duration::from_millis(400));
                let results = myso_surfer::run_with_test_cluster_and_strategy(
                    surf_strategy,
                    test_duration,
                    test_package_paths,
                    test_cluster,
                ...
                .await;
                info!("myso_surfer test complete with results: {results:?}");
                ...
                move |_cluster: Arc<TestCluster>| async move {
                    ...
                    register_fail_point_arg("kill_checkpoint_fork_node", {
                        let forked_validators: Arc<
                            Mutex<HashSet<myso_types::crypto::AuthorityPublicKeyBytes>>,
                        > = forked_validators.clone();
                        let checkpoint_overrides = checkpoint_overrides.clone();
                        let node_to_authority_map = node_to_authority_map.clone();
                        move || {
                            let current_node_id = myso_simulator::current_simnode_id();
                            let authority_name =
                        ...
                        move || {
                            forked_validators.lock().unwrap().contains(
                                node_to_authority_map
                                    .get(&myso_simulator::current_simnode_id())
                                    .unwrap(),
                            )
                        }
                    });
    ...
    async fn test_composite_workload() {
        ...
        let protocol_config = myso_protocol_config::ProtocolConfig::get_for_version(
            myso_protocol_config::ProtocolVersion::max(),
        ...
        let metrics = Arc::new(Mutex::new(
            myso_benchmark::workloads::composite::CompositionMetrics::new(),
        ));

        use myso_benchmark::workloads::composite::*;
