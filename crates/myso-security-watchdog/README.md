...
```shell
cargo run --release -p myso-security-watchdog
```
## Usage
...
The config file allows setting up time based schedule for expected results. For example, when periodically checking total myso in the network we want it to be an exact value i.e 10B whereas when periodically checking balance of an account
...
[
  {
    ...
    "sql_query": "SELECT total_myso FROM total_myso_mainnet ORDER BY epoch DESC LIMIT 1",
    "metric_name": "total_myso_10B",
    "timed_exact_limits": {
      // total myso should always be exact 10B since
      // the dawn of time
  ...
  },
  {
    ...
    "timed_lower_limits": {
      ...
      // i.e. balance should not drop below 50 MYSO before 1/1/2024,
      // balance should not drop below 100MYSO before 2/1/2024
      // and it should not drop below 150MYSO before 3/1/2024
