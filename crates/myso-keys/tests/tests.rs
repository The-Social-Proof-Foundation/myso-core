...
async fn create_alias_keystore_file_test() {
    let temp_dir = TempDir::new().unwrap();
    let mut keystore_path = temp_dir.path().join("myso.keystore");
    let mut keystore = Keystore::from(FileBasedKeystore::load_or_create(&keystore_path).unwrap());
    keystore
...
async fn check_reading_aliases_file_correctly() {
    ...
    let temp_dir = TempDir::new().unwrap();
    let mut keystore_path = temp_dir.path().join("myso.keystore");
    let keystore_path_keep = temp_dir.path().join("myso.keystore");
    let mut keystore = Keystore::from(FileBasedKeystore::load_or_create(&keystore_path).unwrap());
...
fn keystore_no_aliases() {
    ...
    // myso.aliases file with the existing address in the myso.keystore,
...
/// This test confirms rust's implementation of mnemonic is the same with the MySo Wallet
#[tokio::test]
async fn myso_wallet_address_mnemonic_test() -> Result<(), anyhow::Error> {
    ...
    assert_eq!(pubkey.flag(), Ed25519MySoSignature::SCHEME.flag());

    let mut hasher = DefaultHash::default();
    hasher.update([pubkey.flag()]);
    hasher.update(pubkey);
    let g_arr = hasher.finalize();
    let mut res = [0u8; MYSO_ADDRESS_LENGTH];
    res.copy_from_slice(&AsRef::<[u8]>::as_ref(&g_arr)[..MYSO_ADDRESS_LENGTH]);
    let address = MySoAddress::try_from(res.as_slice())?;
