...
pub trait AccountKeystore: Send + Sync {
    ...
    /// Import a keypair into the keystore from a `MySoKeyPair` and optional alias.
    async fn import(
        &mut self,
        alias: Option<String>,
        keypair: MySoKeyPair,
    ...
    async fn remove(&mut self, address: MySoAddress) -> Result<(), anyhow::Error>;
    /// Return an array of `PublicKey`, consisting of every public key in the keystore.
    fn entries(&self) -> Vec<PublicKey>;
    /// Return `MySoKeyPair` for the given address.
    fn export(&self, address: &MySoAddress) -> Result<&MySoKeyPair, anyhow::Error>;
    ...
    /// Return an array of `MySoAddress`, consisting of every address in the keystore.
    fn addresses(&self) -> Vec<MySoAddress> {
        self.entries().iter().map(|k| k.into()).collect()
    }
    /// Return an array of (&MySoAddress, &Alias), consisting of every address and its corresponding alias in the keystore.
    ...
    fn get_by_identity(&self, key_identity: &KeyIdentity) -> Result<MySoAddress, anyhow::Error> {
        match key_identity {
...
pub struct FileBasedKeystore {
    keys: BTreeMap<MySoAddress, MySoKeyPair>,
    aliases: BTreeMap<MySoAddress, Alias>,
    path: Option<PathBuf>,
}
...
impl AccountKeystore for FileBasedKeystore {
    ...
    async fn import(
        ...
        let address: MySoAddress = (&keypair.public()).into();
...
impl FileBasedKeystore {
    pub fn load_or_create(path: &PathBuf) -> Result<Self, anyhow::Error> {
        let keys = if path.exists() {
            ...
            kp_strings
                .iter()
                .map(|kpstr| {
                    let key = MySoKeyPair::decode_base64(kpstr);
                    key.map(|k| (MySoAddress::from(&k.public()), k))
                })
        ...
        let aliases = if aliases_path.exists() {
            ...
            aliases
                ...
                .map(|alias| {
                    ...
                    key.map(|k| (Into::<MySoAddress>::into(&k), alias))
                })
                .collect::<Result<BTreeMap<_, _>, _>>()
                .map_err(|e| {
                    anyhow!(
    ...
    /// To see Bech32 format encoding, use `myso keytool export $MYSO_ADDRESS` where
    /// $MYSO_ADDRESS can be found with `myso keytool list`. Or use `myso keytool convert $BASE64_STR`
    ...
    pub fn key_pairs(&self) -> Vec<&MySoKeyPair> {
...
impl InMemKeystore {
    pub fn new_insecure_for_tests(initial_key_number: usize) -> Self {
        ...
        let keys = (0..initial_key_number)
            ...
            .map(|(ad, k)| (ad, MySoKeyPair::Ed25519(k)))
            .collect::<BTreeMap<MySoAddress, MySoKeyPair>>();
...
mod tests {
    ...
    fn validate_alias_test() {
        ...
        assert!(validate_alias("abc_123.myso").is_ok());
        // Not allowed
