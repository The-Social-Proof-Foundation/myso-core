...
use myso_types::crypto::{AuthorityKeyPair, NetworkKeyPair, MySoKeyPair, ToFromBytes};
...
/// Read from file as Base64 encoded `flag || privkey` and return a MySoKeypair.
pub fn read_keypair_from_file<P: AsRef<std::path::Path>>(path: P) -> anyhow::Result<MySoKeyPair> {
    let contents = std::fs::read_to_string(path)?;
    MySoKeyPair::decode_base64(contents.as_str().trim()).map_err(|e| anyhow!(e))
}
...
pub fn read_network_keypair_from_file<P: AsRef<std::path::Path>>(
    ...
    if let MySoKeyPair::Ed25519(kp) = kp {
        Ok(kp)
    } else {
...
/// Read a MySoKeyPair from a file. The content could be any of the following:
...
pub fn read_key(path: &PathBuf, require_secp256k1: bool) -> Result<MySoKeyPair, anyhow::Error> {
    if !path.exists() {
    ...
    // Try base64 encoded MySoKeyPair `flag || privkey`
    if let Ok(key) = MySoKeyPair::decode_base64(contents) {
        if require_secp256k1 && !matches!(key, MySoKeyPair::Secp256k1(_)) {
    ...
    if let Ok(key) = Secp256k1KeyPair::decode_base64(contents) {
        return Ok(MySoKeyPair::Secp256k1(key));
    }
