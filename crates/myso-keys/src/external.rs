...
pub struct External {
    ...
    pub aliases: BTreeMap<MySoAddress, Alias>,
    // Holds a map of addresses to [`StoredKey`]
    pub keys: BTreeMap<MySoAddress, StoredKey>,
...
pub struct StoredKey {
    ...
    /// External signer binary (e.g., "myso-ledger-signer")
...
impl External {
    ...
    pub fn load_or_create(path: &PathBuf) -> Result<Self, Error> {
        ...
        let aliases: BTreeMap<MySoAddress, Alias> = if aliases_store_directory.exists() {
        ...
        let keys: BTreeMap<MySoAddress, StoredKey> = if path.exists() {
    ...
    /// Add a Key ID from the given an external signer to the MySo CLI index
    ...
    pub fn is_indexed(&self, key: &StoredKey) -> bool {
        self.keys.contains_key(&MySoAddress::from(&key.public_key))
    }
...
impl AccountKeystore for External {
    ...
    async fn generate(
        ...
        let address: MySoAddress = (&stored_key.public_key).into();
    ...
    async fn import(&mut self, _alias: Option<String>, _keypair: MySoKeyPair) -> Result<(), Error> {
        Err(anyhow!("Import not supported for external keys."))
    }

    async fn remove(&mut self, address: MySoAddress) -> Result<(), Error> {
    ...
    /// Export the key pair for the given address as a `MySoKeyPair`.
    fn export(&self, _address: &MySoAddress) -> Result<&MySoKeyPair, Error> {
    ...
    async fn sign_hashed(
        ...
        let address = MySoAddress::from(&public_key);
        match &result.signature {
            Signature::Ed25519MySoSignature(s) => {
                ...
                let signature_address = MySoAddress::from(&pk);
            ...
            Signature::Secp256k1MySoSignature(s) => {
                let (sig, pk) = s.get_verification_inputs().map_err(|e| {
                    signature::Error::from_source(anyhow!(
                        "Failed to get verification inputs: {}",
                        e
            ...
            Signature::Secp256r1MySoSignature(s) => {
                let (sig, pk) = s.get_verification_inputs().map_err(|e| {
                    signature::Error::from_source(anyhow!(
                        "Failed to get verification inputs: {}",
                        e
    ...
    fn addresses_with_alias(&self) -> Vec<(&MySoAddress, &Alias)> {
    ...
    fn get_alias(&self, address: &MySoAddress) -> Result<String, anyhow::Error> {
...
mod tests {
    ...
    async fn test_serialize() {
        ...
        external.keys.insert(
            MySoAddress::from_str(ADDRESS).unwrap(),
            StoredKey {
    ...
    async fn test_exec() {
        ...
        assert!(
            external
                .exec("myso-key-tool", "test_method", params)
                .await
                .is_ok()
        );
    }
    ...
    async fn test_generate_success() {
        ...
        assert_eq!(address, MySoAddress::from_str(ADDRESS).unwrap());
    ...
    async fn test_add_existing_key() {
        ...
        let key = keys.get(&MySoAddress::from_str(ADDRESS).expect("Invalid address format"));
        assert!(key.is_some());

        let alias = external
            .aliases
            .get(&MySoAddress::from_str(ADDRESS).expect("Invalid address format"));
    ...
    async fn test_add_keypair_not_supported() {
        ...
        let result = external
            .import(None, MySoKeyPair::Ed25519(ed25519_keypair))
            .await;
    ...
    async fn test_remove_key() {
        ...
        let address = MySoAddress::from_str(ADDRESS).unwrap();
        external.keys.insert(
            address,
            StoredKey {
                public_key: PublicKey::decode_base64(PUBLIC_KEY).unwrap(),
    ...
    fn test_get_key_fails() {
        ...
        let result = external.export(&MySoAddress::from_str(ADDRESS).unwrap());
        assert!(result.is_err())
    }

    #[tokio::test]
    async fn test_sign_hashed() {
        let skp = MySoKeyPair::Ed25519(Ed25519KeyPair::generate(&mut StdRng::from_seed([0; 32])));
    ...
    async fn test_sign_secure() {
        ...
        let intent = Intent::myso_transaction();
