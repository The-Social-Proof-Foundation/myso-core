...
use myso_types::{
    ...
    crypto::{SignatureScheme, MySoKeyPair},
    error::{MySoError, MySoErrorKind},
};
...
) -> Result<(MySoAddress, MySoKeyPair), MySoError> {
    ...
    match key_scheme {
        SignatureScheme::ED25519 => {
            ...
            let sk = Ed25519PrivateKey::from_bytes(&derived)
                .map_err(|e| MySoErrorKind::SignatureKeyGenError(e.to_string()))?;
            let kp: Ed25519KeyPair = sk.into();
            Ok((kp.public().into(), MySoKeyPair::Ed25519(kp)))
        }
        SignatureScheme::Secp256k1 => {
            ...
            Ok((kp.public().into(), MySoKeyPair::Secp256k1(kp)))
        }
        SignatureScheme::Secp256r1 => {
            ...
            Ok((kp.public().into(), MySoKeyPair::Secp256r1(kp)))
        }
        SignatureScheme::BLS12381
        | SignatureScheme::MultiSig
        | SignatureScheme::ZkLoginAuthenticator
        | SignatureScheme::PasskeyAuthenticator => Err(MySoErrorKind::UnsupportedFeatureError {
...
) -> Result<DerivationPath, MySoError> {
    match key_scheme {
        SignatureScheme::ED25519 => {
            match path {
                Some(p) => {
                    ...
                    if let &[purpose, coin_type, account, change, address] = p.as_ref() {
                            ...
                            && address.is_hardened()
                        {
                            ...
                            Err(
                                MySoErrorKind::SignatureKeyGenError("Invalid path".to_string())
                                    .into(),
                            )
                        }
                    } else {
                        Err(MySoErrorKind::SignatureKeyGenError("Invalid path".to_string()).into())
                    }
                }
                None => Ok(format!(
                ...
                .parse()
                .map_err(|_| {
                    MySoErrorKind::SignatureKeyGenError("Cannot parse path".to_string())
                })?),
            }
        }
        SignatureScheme::Secp256k1 => {
...
) -> Result<(MySoAddress, MySoKeyPair, SignatureScheme, String), anyhow::Error> {
