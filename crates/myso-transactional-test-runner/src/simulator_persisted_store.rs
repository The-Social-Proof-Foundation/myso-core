...
use myso_swarm_config::network_config_builder::{ConfigBuilder, KeyPairWrapper};
...
use myso_types::{
    ...
    error::{MySoError, UserInputError},
    messages_checkpoint::{
...
pub struct PersistedStoreInner {
    ...
    // Transaction data
    transactions: DBMap<TransactionDigest, myso_types::transaction::TrustedTransaction>,
...
impl SimulatorStore for PersistedStore {
    ...
    fn get_system_state(&self) -> myso_types::myso_system_state::MySoSystemState {
        myso_types::myso_system_state::get_myso_system_state(self).expect("system state must exist")
    }

    fn get_clock(&self) -> myso_types::clock::Clock {
        SimulatorStore::get_object(self, &myso_types::MYSO_CLOCK_OBJECT_ID)
            .expect("clock should exist")
            .to_rust()
            .expect("clock object should deserialize")
    }

    fn owned_objects(&self, owner: MySoAddress) -> Box<dyn Iterator<Item = Object> + '_> {
    ...
    fn backing_store(&self) -> &dyn myso_types::storage::BackingStore {
        self
    }
}
...
impl BackingPackageStore for PersistedStore {
    fn get_package_object(
        &self,
        package_id: &ObjectID,
    ) -> myso_types::error::MySoResult<Option<PackageObject>> {
...
impl ChildObjectResolver for PersistedStore {
    ...
    ) -> myso_types::error::MySoResult<Option<Object>> {
...
impl ParentSync for PersistedStore {
    fn get_latest_parent_entry_ref_deprecated(
        &self,
        _object_id: ObjectID,
    ) -> Option<myso_types::base_types::ObjectRef> {
