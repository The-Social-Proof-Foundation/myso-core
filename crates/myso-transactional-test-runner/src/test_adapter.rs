...
use myso_swarm_config::network_config_builder::KeyPairWrapper;
use myso_types::base_types::{SequenceNumber, VersionNumber};
...
use myso_types::digests::{ConsensusCommitDigest, TransactionDigest};
use myso_types::effects::{
    AccumulatorOperation, TransactionEffects, TransactionEffectsAPI, TransactionEvents,
};
use myso_types::execution_status::ExecutionFailureStatus;
...
use myso_types::storage::{ObjectStore, RpcStateReader};
...
pub struct OffChainConfig {
    ...
    /// URL for the MySo REST API. To be passed to the offchain indexer if it uses the REST API.
...
struct TxnSummary {
    ...
    events: Vec<Event>,
    accumulators_written: Vec<(ObjectID, MySoAddress, TypeTag, AccumulatorOperation)>,
    gas_summary: GasCostSummary,
}

impl AdapterInitConfig {
    fn from_args(init_cmd: InitCommand, myso_args: MySoInitArgs) -> Self {
...
#[async_trait]
impl MoveTestAdapter<'_> for MySoTestAdapter {
    type ExtraPublishArgs = MySoPublishArgs;
    type ExtraRunArgs = MySoRunArgs;
    type ExtraInitArgs = MySoInitArgs;
    type ExtraValueArgs = MySoExtraValueArgs;
    type Subcommand = MySoSubcommand<Self::ExtraValueArgs, Self::ExtraRunArgs>;
    ...
    fn render_command_input(
        ...
        match &task.command {
            TaskCommand::Subcommand(MySoSubcommand::ProgrammableTransaction(..)) => {
    ...
    async fn init(
        ...
        assert!(
            ...
            "Must populate 'pre_compiled_deps' with MySo framework"
        );
        ...
        let AdapterInitConfig {
            ...
            Some((init_cmd, myso_args)) => AdapterInitConfig::from_args(init_cmd, myso_args),
    ...
    async fn publish_modules(
        ...
        // we are assuming that all packages depend on Move Stdlib and MySo Framework, so these
        // don't have to be provided explicitly as parameters
        dependencies.extend([MOVE_STDLIB_PACKAGE_ID, MYSO_FRAMEWORK_PACKAGE_ID]);
    ...
    async fn call_function(
        ...
        self.next_task();
        let MySoRunArgs { summarize, .. } = extra;
    ...
    async fn handle_subcommand(
        ...
        match command {
            MySoSubcommand::RunGraphql(RunGraphqlCommand {
                show_usage,
                show_headers,
            ...
            MySoSubcommand::RunJsonRpc(RunJsonRpcCommand {
                show_headers,
                cursors,
            }) => {
            ...
            MySoSubcommand::CreateCheckpoint(CreateCheckpointCommand { count }) => {
            ...
            MySoSubcommand::AdvanceEpoch(cmd) => {
            ...
            MySoSubcommand::AdvanceClock(advance_clock_command) => {
            ...
            MySoSubcommand::SetRandomState(SetRandomStateCommand {
                randomness_round,
                random_bytes,
            ...
            MySoSubcommand::AuthenticatorStateUpdate(AuthenticatorStateUpdateCommand {
                round,
                jwk_iss,
            ...
            MySoSubcommand::ViewObject(ViewObjectCommand {
                id: fake_id,
                hide_contents,
            }) => {
            ...
            MySoSubcommand::TransferObject(TransferObjectCommand {
                id: fake_id,
                recipient,
                sender,
                gas_budget,
                ...
                let mut builder = ProgrammableTransactionBuilder::new();
                let obj_arg = MySoValue::Object(fake_id, None).into_argument(&mut builder, self)?;
                ...
                let transaction = self.sign_txn(sender, |sender, gas| {
                    ...
                    builder.command(myso_types::transaction::Command::TransferObjects(
                        vec![obj_arg],
                        rec_arg,
                    ));
                    let pt = builder.finish();
            ...
            MySoSubcommand::ConsensusCommitPrologue(ConsensusCommitPrologueCommand {
                timestamp_ms,
            }) => {
            ...
            MySoSubcommand::ProgrammableTransaction(ProgrammableTransactionCommand {
                sender,
                sponsor,
                gas_budget,
                gas_budget_from_address_balance,
            ...
            MySoSubcommand::UpgradePackage(UpgradePackageCommand {
                package,
                upgrade_capability,
                dependencies,
                sender,
            ...
            MySoSubcommand::StagePackage(StagePackageCommand {
                syntax,
                dependencies,
            }) => {
                let syntax = syntax.unwrap_or_else(|| self.default_syntax());
            ...
            MySoSubcommand::SetAddress(SetAddressCommand { address, input }) => {
                ...
                let (value, package) = match input {
                    ...
                    MySoValue::MoveValue(v) => {
                        let bytes = v.simple_serialize().unwrap();
                        let value: AccountAddress = bcs::from_bytes(&bytes)?;
                        (value, None)
                    }
                    MySoValue::Digest(_) => bail!("digest is not supported as an input"),
                    MySoValue::ObjVec(_) => bail!("obj vec is not supported as an input"),
                    MySoValue::Receiving(_, _) => bail!("receiving is not supported as an input"),
                    MySoValue::Owned(_, _) => bail!("owned object is not supported as an input"),
                    MySoValue::Shared(_, _, _) => {
...
impl MySoTestAdapter {
    ...
    async fn upgrade_package(
        ...
        // Argument::Input(0)
        MySoValue::Object(upgrade_capability, None).into_argument(&mut builder, self)?;
    ...
    fn tx_summary_from_effects(
        ...
        if let MySoExecutionStatus::Failure { error } = effects.status() {
        ...
        let mut accumulators_written: Vec<_> = accumulator_events
            ...
            .map(|event| {
                let operation = match event.operation {
                    MySoAccumulatorOperation::Merge => AccumulatorOperation::Merge,
                    MySoAccumulatorOperation::Split => AccumulatorOperation::Split,
                };
                (
    ...
    fn list_accumulator_events(
        &self,
        accumulators: &[(ObjectID, MySoAddress, TypeTag, AccumulatorOperation)],
        summarize: bool,
    ) -> String {
        if summarize {
    ...
    fn stabilize_str(&self, input: impl AsRef<str>) -> String {
        fn candidate_is_hex(s: &str) -> bool {
            const HEX_STR_LENGTH: usize = MYSO_ADDRESS_LENGTH * 2;
            let n = s.len();
...
impl<'a> GetModule for &'a MySoTestAdapter {
...
static NAMED_ADDRESSES: Lazy<BTreeMap<String, NumericalAddress>> = Lazy::new(|| {
    ...
    // TODO fix MySo framework constants
    map.insert(
        "myso".to_string(),
        NumericalAddress::new(
            MYSO_FRAMEWORK_ADDRESS.into_bytes(),
            move_compiler::shared::NumberFormat::Hex,
        ),
    );
    map.insert(
        "myso_system".to_string(),
        NumericalAddress::new(
            MYSO_SYSTEM_ADDRESS.into_bytes(),
            move_compiler::shared::NumberFormat::Hex,
        ),
    );
    map.insert(
...
pub static PRE_COMPILED: Lazy<PreCompiledProgramInfo> = Lazy::new(|| {
    ...
    let myso_files: &Path = Path::new(DEFAULT_FRAMEWORK_PATH);
    let myso_system_sources = {
        let mut buf = myso_files.to_path_buf();
        buf.extend(["packages", "myso-system", "sources"]);
        buf.to_string_lossy().to_string()
    };
    let myso_sources = {
        let mut buf = myso_files.to_path_buf();
        buf.extend(["packages", "myso-framework", "sources"]);
        buf.to_string_lossy().to_string()
    };
    let myso_deps = {
        let mut buf = myso_files.to_path_buf();
    ...
    let pre_compiled_program = move_compiler::construct_pre_compiled_lib(
        vec![PackagePaths {
            name: Some(("myso-framework".into(), config)),
            paths: vec![
                myso_system_sources,
                myso_sources,
                myso_deps,
    ...
    match pre_compiled_program {
        Err((files, diags)) => {
            eprintln!("!!!MySo framework failed to compile!!!");
...
async fn init_val_fullnode_executor(
    ...
    // For each named MySo account without an address value, create an account with an address
    // and a gas object
...
async fn init_sim_executor(
    ...
    // For each named MySo account without an address value, create a key pair
    for n in account_names {
    ...
    let addr_keys: Vec<_> = (0..num_custom_validator_accounts)
        ...
        .map(|_| {
            let (addr, account_key_pair): (MySoAddress, AccountKeyPair) =
...
impl ObjectStore for MySoTestAdapter {
...
impl ReadStore for MySoTestAdapter {
    ...
    fn get_committee(
        &self,
        epoch: myso_types::committee::EpochId,
    ...
    fn get_lowest_available_checkpoint(
        &self,
    ) -> myso_types::storage::error::Result<CheckpointSequenceNumber> {
        self.executor.get_lowest_available_checkpoint()
    }

    fn get_checkpoint_by_digest(
        &self,
        digest: &myso_types::messages_checkpoint::CheckpointDigest,
