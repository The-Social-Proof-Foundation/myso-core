...
impl InMemoryStore {
    ...
    pub fn get_system_state(&self) -> myso_types::myso_system_state::MySoSystemState {
        myso_types::myso_system_state::get_myso_system_state(self).expect("system state must exist")
    }

    pub fn get_clock(&self) -> myso_types::clock::Clock {
        self.get_object(&myso_types::MYSO_CLOCK_OBJECT_ID)
            .expect("clock should exist")
            .to_rust()
            .expect("clock object should deserialize")
    }

    pub fn owned_objects(&self, owner: MySoAddress) -> impl Iterator<Item = &Object> {
        self.live_objects
            .iter()
...
pub struct KeyStore {
    ...
    #[allow(unused)]
    account_keys: BTreeMap<MySoAddress, AccountKeyPair>,
}

impl KeyStore {
    pub fn from_network_config(
        network_config: &myso_swarm_config::network_config::NetworkConfig,
    ) -> Self {
    ...
    pub fn accounts(&self) -> impl Iterator<Item = (&MySoAddress, &AccountKeyPair)> {
