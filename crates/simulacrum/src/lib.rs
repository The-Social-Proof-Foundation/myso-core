...
//! provides the [`Simulacrum`] type which is a implementation or instantiation of a myso
...
use myso_types::base_types::{AuthorityName, ObjectID, ObjectRef, SequenceNumber, VersionNumber};
use myso_types::crypto::{AccountKeyPair, AuthoritySignature, get_account_key_pair};
...
/// A `Simulacrum` of MySo.
///
/// This type represents a simulated instantiation of a MySo blockchain that needs to be driven
...
impl<R, S: store::SimulatorStore> Simulacrum<R, S> {
    pub fn new_with_network_config_store(config: &NetworkConfig, rng: R, store: S) -> Self {
        ...
        let epoch_state = EpochState::new(genesis.myso_system_object());
    ...
    /// let (account, kp, gas) = simulacrum.funded_account(MIST_PER_MYSO).unwrap();
    ///
    /// // `account` is a fresh MySoAddress that owns a Coin<MYSO> object with single MYSO in it,
    /// // referred to by `gas`.
    /// // ...
    /// # }
    /// ```
    pub fn funded_account(
        &mut self,
        amount: u64,
    ) -> Result<(MySoAddress, AccountKeyPair, ObjectRef)> {
    ...
    /// # fn main() {
    /// let mut simulacrum = Simulacrum::new();
    /// let address = MySoAddress::generate(simulacrum.rng());
    /// simulacrum.request_gas(address, MIST_PER_MYSO).unwrap();
    ///
    /// // `account` now has a Coin<MYSO> object with single MYSO in it.
    /// // ...
    /// # }
    /// ```
    pub fn request_gas(&mut self, address: MySoAddress, amount: u64) -> Result<TransactionEffects> {
        ...
        let object = self
            ...
            .find(|object| {
                object.is_gas_coin() && object.get_coin_value_unsafe() > amount + MIST_PER_MYSO
            })
            .ok_or_else(|| {
                anyhow!("unable to find a coin with enough to satisfy request for {amount} Mist")
            })?;

        let gas_data = myso_types::transaction::GasData {
            payment: vec![object.compute_object_reference()],
            owner: *sender,
            price: self.reference_gas_price(),
            budget: MIST_PER_MYSO,
        };

        let pt = {
            let mut builder =
                myso_types::programmable_transaction_builder::ProgrammableTransactionBuilder::new();
            builder.transfer_myso(address, Some(amount));
            builder.finish()
        };

        let kind = myso_types::transaction::TransactionKind::ProgrammableTransaction(pt);
        let tx_data =
            myso_types::transaction::TransactionData::new_with_gas_data(kind, *sender, gas_data);
...
impl<T, V: store::SimulatorStore> ReadStore for Simulacrum<T, V> {
    ...
    fn get_checkpoint_contents_by_digest(
        &self,
        digest: &myso_types::messages_checkpoint::CheckpointContentsDigest,
    ...
    fn get_events(
        &self,
        event_digest: &myso_types::digests::TransactionDigest,
    ...
    fn get_full_checkpoint_contents(
        ...
        _digest: &myso_types::messages_checkpoint::CheckpointContentsDigest,
    ...
    fn get_unchanged_loaded_runtime_objects(
        &self,
        _digest: &myso_types::digests::TransactionDigest,
...
impl<T: Send + Sync, V: store::SimulatorStore + Send + Sync> RpcStateReader for Simulacrum<T, V> {
    ...
    fn get_chain_identifier(
        &self,
    ) -> myso_types::storage::error::Result<myso_types::digests::ChainIdentifier> {
        Ok(self
            .store()
    ...
    fn indexes(&self) -> Option<&dyn myso_types::storage::RpcIndexes> {
        None
    }
    ...
    ) -> myso_types::storage::error::Result<Option<move_core_types::annotated_value::MoveTypeLayout>>
    {
        Ok(None)
    }
}
...
impl Simulacrum {
    ...
    /// existing code for generating transactions in myso-test-transaction-builder by defining a trait
    /// that both WalletContext and Simulacrum implement. Then we can remove this function.
    pub fn transfer_txn(&mut self, recipient: MySoAddress) -> (Transaction, u64) {
        let (sender, key) = self.keystore().accounts().next().unwrap();
        let sender = *sender;
        ...
        let pt = {
            ...
            builder.transfer_myso(recipient, Some(transfer_amount));
            builder.finish()
        };
