...
#[open_rpc(namespace = "mysox", tag = "Name Service API")]
#[rpc(server, namespace = "mysox")]
trait NameServiceApi {
    /// Resolve a MySoNS name to its address
    ...
    /// Find the MySoNS name that points to this address.
    ///
...
impl NameServiceApiServer for NameService {
    async fn resolve_name_service_address(&self, name: String) -> RpcResult<Option<MySoAddress>> {
        let Self(ctx) = self;
        Ok(response::resolved_address(ctx, &name)
            .await
            .with_internal_context(|| format!("Resolving MySoNS name {name:?}"))?)
    }
