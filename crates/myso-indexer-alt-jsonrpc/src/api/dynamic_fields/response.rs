...
use anyhow::anyhow;
use myso_json::MoveTypeLayout;
use myso_json::MySoJsonValue;
use myso_json_rpc_types::BcsName;
use myso_json_rpc_types::DynamicFieldInfo as DynamicFieldInfoResponse;
use myso_json_rpc_types::MySoMoveValue;
...
pub(super) async fn dynamic_field_object(
...
) -> Result<MySoObjectResponse, RpcError<Error>> {
    let layout = resolve_type(ctx, &name.type_).await?;
    let bytes = MySoJsonValue::new(name.value)
    ...
    let options = MySoObjectDataOptions::full_content();
...
/// `myso::dynamic_field::Field<K, V>`, and extract the name and value from it.
