...
use myso_indexer_alt_reader::displays::DisplayKey;
use myso_json_rpc_types::DisplayFieldsResponse;
...
use myso_json_rpc_types::MySoParsedData;
use myso_json_rpc_types::MySoPastObjectResponse;
use myso_json_rpc_types::MySoRawData;
...
) -> Result<MySoObjectResponse, RpcError> {
    let Some(object) = load_live(ctx, object_id)
        .await
        .context("Failed to load latest object")?
    else {
        return Ok(MySoObjectResponse::new_with_error(
...
pub(super) async fn past_object(
    ...
    options: &MySoObjectDataOptions,
) -> Result<MySoPastObjectResponse, RpcError> {
    let Some(object) = ctx
        .kv_loader()
        ...
        return Ok(MySoPastObjectResponse::VersionNotFound(object_id, version));
    };
...
pub(crate) async fn object_data_with_options(
    ...
    object: Object,
    options: &MySoObjectDataOptions,
) -> Result<MySoObjectData, RpcError> {
    ...
    let content: OptionFuture<_> = options
        .show_content
        .then(|| object_data::<MySoParsedData>(ctx, &object))
        .into();

    let bcs: OptionFuture<_> = options
        .show_bcs
        .then(|| object_data::<MySoRawData>(ctx, &object))
        .into();
...
async fn object_data<D: MySoData>(ctx: &Context, object: &Object) -> Result<D, RpcError> {
...
async fn display(ctx: &Context, object: &Object) -> DisplayFieldsResponse {
    ...
    DisplayFieldsResponse {
        ...
        error: (!field_errors.is_empty()).then_some(MySoObjectResponseError::DisplayError {
            error: field_errors,
        }),
    }
}
