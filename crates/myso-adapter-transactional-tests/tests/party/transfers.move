...
public fun create_party(ctx: &mut TxContext) {
    let p = Pub { id: object::new(ctx) };
    transfer::public_party_transfer(p, myso::party::single_owner(@0))
}

public fun pub_party(obj: Pub, p: myso::party::Party) {
    transfer::public_party_transfer(obj, p)
}

public fun priv_party(obj: Priv, p: myso::party::Party) {
...
//> 0: myso::party::single_owner(Input(1));
//> myso::transfer::public_party_transfer<ex::m::Pub>(Input(0), Result(0))
