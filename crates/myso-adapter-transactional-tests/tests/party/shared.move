...
public fun create_party(ctx: &mut TxContext) {
    let s = S { id: object::new(ctx) };
    transfer::party_transfer(s, myso::party::single_owner(ctx.sender()))
}
...
//> myso::transfer::public_party_transfer<ex::m::S>(Input(0), Result(0))
...
//> myso::transfer::public_share_object<ex::m::S>(Input(0))