...
public fun create_party(ctx: &mut TxContext) {
    ...
    transfer::party_transfer(a, myso::party::single_owner(ctx.sender()))
}

public fun wrap(a: A, ctx: &mut TxContext) {
    let wrapper = AWrapper { id: object::new(ctx), a };
    transfer::party_transfer(wrapper, myso::party::single_owner(ctx.sender()))
}

public fun unwrap(wrapper: AWrapper, ctx: &mut TxContext) {
    let AWrapper { id, a } = wrapper;
    transfer::party_transfer(a, myso::party::single_owner(tx_context::sender(ctx)));
    object::delete(id)
}
...
//> myso::transfer::public_party_transfer<ex::m::A>(Input(0), Result(0))
