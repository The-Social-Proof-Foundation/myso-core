...
module test::m1 {
    ...
    public entry fun test_dof(parent: &mut Object, ctx: &mut TxContext) {
        ...
        let C1 { id } = myso::dynamic_object_field::remove(&mut parent.id, 0u64);
        object::delete(id);
    ...
    public entry fun test_df(parent: &mut Object) {
        myso::dynamic_field::add(&mut parent.id, 0u64, b"true");
        let _: vector<u8> = myso::dynamic_field::remove(&mut parent.id, 0u64);
        myso::dynamic_field::add(&mut parent.id, 0u64, true);
        let _: bool = myso::dynamic_field::remove(&mut parent.id, 0u64);
    }
}
