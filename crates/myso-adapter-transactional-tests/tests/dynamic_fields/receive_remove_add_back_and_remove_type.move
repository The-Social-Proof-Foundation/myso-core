...
module test::m1 {
    ...
    public entry fun test_dof(parent: &mut Object, c1: Receiving<C1>, c2: Receiving<C2>) {
        ...
        myso::dynamic_object_field::add(&mut parent.id, 0u64, c1);
        let c1: C1 = myso::dynamic_object_field::remove(&mut parent.id, 0u64);
        transfer::public_transfer(c1, @test);

        let c2 = transfer::receive(&mut parent.id, c2);
        myso::dynamic_object_field::add(&mut parent.id, 0u64, c2);
        let C2 { id } = myso::dynamic_object_field::remove(&mut parent.id, 0u64);
        object::delete(id);
    }

    public entry fun test_df(parent: &mut Object, c1: Receiving<C1>, c2: Receiving<C2>) {
        let c1 = transfer::receive(&mut parent.id, c1);
        myso::dynamic_field::add(&mut parent.id, 0u64, c1);
        let c1: C1 = myso::dynamic_field::remove(&mut parent.id, 0u64);
        transfer::public_transfer(c1, @test);

        let c2 = transfer::receive(&mut parent.id, c2);
        myso::dynamic_field::add(&mut parent.id, 0u64, c2);
        let C2 { id } = myso::dynamic_field::remove(&mut parent.id, 0u64);
        object::delete(id);
    }
    ...
    public entry fun test_dof_wrapper(parent: &mut Object, c1: Receiving<C1>, _c2: Receiving<C2>, ctx: &mut TxContext) {
        ...
        myso::dynamic_object_field::add(&mut parent.id, 0u64, w);
        let w: Wrapper<C1> = myso::dynamic_object_field::remove(&mut parent.id, 0u64);
    ...
    public entry fun test_df_wrapper(parent: &mut Object, c1: Receiving<C1>, _c2: Receiving<C2>, ctx: &mut TxContext) {
        ...
        let c1: C1 = myso::dynamic_field::remove(&mut parent.id, 0u64);
        let w = Wrapper { id: object::new(ctx), value: c1 };
        myso::dynamic_field::add(&mut parent.id, 0u64, w);
        let w: Wrapper<C1> = myso::dynamic_field::remove(&mut parent.id, 0u64);
        transfer::public_transfer(w, @test);
    }
}
