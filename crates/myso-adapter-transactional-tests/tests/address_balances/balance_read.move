...
// Test settled_funds_value from myso::balance
...
public fun balance_check(acc: &myso::accumulator::AccumulatorRoot, addr: address, expected: u64) {
  let balance = myso::balance::settled_funds_value<myso::myso::MYSO>(acc, addr);

  assert!(balance == expected, 100);
}

public fun balance_check_fake(
  acc: &myso::accumulator::AccumulatorRoot,
  addr: address,
  expected: u64,
) {
  let balance = myso::balance::settled_funds_value<u64>(acc, addr);
...
public fun send_max(holder: &mut SupplyHolder, key: u64, recipient: address) {
  ...
  let mut supply = myso::balance::create_supply<u64>(0);
  let balance = supply.increase_supply(increase);
  myso::balance::send_funds(balance, recipient);
  myso::dynamic_field::add(&mut holder.id, key, supply);
}

//# programmable --sender A --inputs 1042 @B
// Send some MYSO from A to B
//> 0: SplitCoins(Gas, [Input(0)]);
//> 1: myso::coin::send_funds<myso::myso::MYSO>(Result(0), Input(1));
