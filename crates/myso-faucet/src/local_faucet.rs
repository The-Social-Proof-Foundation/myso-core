...
use myso_sdk::types::effects::TransactionEffectsAPI;
...
impl LocalFaucet {
    ...
    pub async fn local_request_execute_tx(
        ...
        ptb.pay_myso(recipients, amounts)
        ...
        client
            ...
            .map_err(|e| {
                FaucetError::internal(format!(
                    "Failed to execute PayMySo transaction for coin {:?}, with err {:?}",
                    coin_id, e
                ))
            })
            .map_err(Into::into)
...
async fn find_gas_coins_and_address(
    ...
    config: &FaucetConfig,
) -> Result<(Vec<GasCoin>, MySoAddress), FaucetError> {
