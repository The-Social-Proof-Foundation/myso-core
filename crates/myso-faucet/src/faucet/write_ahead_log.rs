...
/// Persistent log of transactions paying out MySo from the faucet, keyed by the coin serving the
...
mod tests {
    ...
    fn random_request(coin: ObjectRef) -> (MySoAddress, TransactionData) {
        let gas_price = 1;
        let send = MySoAddress::random_for_testing_only();
        let recv = MySoAddress::random_for_testing_only();
        (
            recv,
