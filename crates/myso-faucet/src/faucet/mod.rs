...
#[async_trait]
pub trait Faucet {
    /// Send `Coin<MYSO>` of the specified amount to the recipient
    async fn send(
        &self,
        id: Uuid,
    ...
    /// Send `Coin<MYSO>` of the specified amount to the recipient in a batch request
    async fn batch_send(
        &self,
        id: Uuid,
        recipient: MySoAddress,
...
#[clap(
    name = "MYSO Faucet",
    about = "Faucet for requesting test tokens on MYSO",
