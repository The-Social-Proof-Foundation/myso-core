...
use myso_bridge_schema::models::{BridgeDataSource, TokenTransfer, TokenTransferStatus};
use myso_bridge_schema::schema::token_transfer;
            ...
            for ev in tx.events.iter().flat_map(|e| &e.data) {
                let (chain_id, nonce) = if self.deposited_event_type == ev.type_ {
                    ...
                    self.metrics
                        ...
                        .with_label_values(&["myso_to_eth", "true"])
                    ...
                    info!("Observed MySo Approval {:?}", ev);
                    let event: MoveTokenTransferApproved = bcs::from_bytes(&ev.contents)?;

                    // Bridge committee approval metrics
                    self.metrics
                        .bridge_events_total
                        .with_label_values(&["transfer_approved", "myso"])
                        .inc();
                    self.metrics
                        .token_transfers_total
                        .with_label_values(&["eth_to_myso", "approved", "unknown"])
                        .inc();
                    ...
                    info!("Observed MySo Claim {:?}", ev);
                    let event: MoveTokenTransferClaimed = bcs::from_bytes(&ev.contents)?;

                    // Bridge transfer completion metrics
                    self.metrics
                        .bridge_events_total
                        .with_label_values(&["transfer_claimed", "myso"])
                        .inc();
                    self.metrics
                        .token_transfers_total
                        .with_label_values(&["eth_to_myso", "claimed", "unknown"])
                        .inc();
