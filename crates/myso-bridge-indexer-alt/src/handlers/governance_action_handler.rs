...
use myso_bridge_schema::models::{BridgeDataSource, GovernanceAction};
use myso_bridge_schema::schema;
            ...
            for ev in tx.events.iter().flat_map(|e| &e.data) {
                ...
                let (action, data) = match &ev.type_ {
                    ...
                    t if t == &self.emergency_op_event_type => {
                        ...
                        self.metrics
                            .governance_actions_total
                            .with_label_values(&["emergency_operation", "myso"])
                            .inc();
                        self.metrics
                            .bridge_pause_status
                    ...
                    t if t == &self.blocklist_event_type => {
                        info!(?ev, "Observed MySo Blocklist Validator");
                    ...
                    t if t == &self.token_reg_event_type => {
                        info!(?ev, "Observed MySo Token Registration");
                        let event: MoveTokenRegistrationEvent = bcs::from_bytes(&ev.contents)?;
                        (AddMySoTokens, serde_json::to_value(event)?)
                    }
                    t if t == &self.update_price_event_type => {
                        info!(?ev, "Observed MySo Token Price Update");
                    ...
                    t if t == &self.new_token_event_type => {
                        info!(?ev, "Observed MySo New token event");
                ...
                results.push(GovernanceAction {
                    nonce: None,
                    data_source: BridgeDataSource::MYSO,
