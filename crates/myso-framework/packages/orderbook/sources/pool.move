...
use orderbook::{
    ...
    book::{Self, Book},
    constants,
    myso_price::{Self, MySoPrice, OrderMySoPrice, emit_myso_price_added},
    ewma::{init_ewma_state, EWMAState},
    math,
    order::Order,
    order_info::{Self, OrderInfo},
    registry::{OrderbookAdminCap, Registry},
    state::{Self, State},
...
public struct MysoBurned<phantom BaseAsset, phantom QuoteAsset> has copy, drop, store {
    pool_id: ID,
    myso_burned: u64,
}
...
/// MYSO quantity can be overestimated. Returns three `Coin` objects:
...
public fun swap_exact_quantity<BaseAsset, QuoteAsset>(
    ...
    let pay_with_myso = myso_in.value() > 0;
    let is_bid = quote_quantity > 0;
    if (is_bid) {
        (base_quantity, _, _) = if (pay_with_myso) {
    ...
    if (base_quantity < self.load_inner().book.min_size()) {
        return (base_in, quote_in, myso_in)
    };
    ...
    let myso_out = temp_balance_manager.withdraw_all<MYSO>(ctx);
...
public fun add_myso_price_point<BaseAsset, QuoteAsset, ReferenceBaseAsset, ReferenceQuoteAsset>(
    ...
    let myso_type = type_name::with_defining_ids<MYSO>();
    ...
    let reference_myso_is_base = reference_base_type == myso_type;
    let reference_other_type = if (reference_myso_is_base) {
    ...
    // For MYSO/USDC pool, reference_myso_is_base is true, MYSO per USDC is
    // reference_pool_price
    // For USDC/MYSO pool, reference_myso_is_base is false, USDC per MYSO is
    // reference_pool_price
    let myso_per_reference_other_price = if (reference_myso_is_base) {
        math::div(1_000_000_000, reference_pool_price)
    } else {
        reference_pool_price
    };

    // For USDC/MYSO pool, reference_other_is_target_base is true, add price
    // point to myso per base
    // For MYSO/USDC pool, reference_other_is_target_base is false, add price
...
public fun burn_myso<BaseAsset, QuoteAsset>(
    self: &mut Pool<BaseAsset, QuoteAsset>,
    ctx: &mut TxContext,
): u64 {
    let self = self.load_inner_mut();
    let balance_to_burn = self.state.history_mut().reset_balance_to_burn();
    let myso_to_burn = self.vault.withdraw_myso_to_burn(balance_to_burn).into_coin(ctx);
    let amount_burned = myso_to_burn.value();
    transfer::public_transfer(myso_to_burn, @0x0);

    event::emit(MysoBurned<BaseAsset, QuoteAsset> {
...
#[
    deprecated(
        note = b"This function is deprecated, use `update_orderbook_referral_multiplier` instead.",
    ),
]
...
public fun update_orderbook_referral_multiplier<BaseAsset, QuoteAsset>(
    _self: &mut Pool<BaseAsset, QuoteAsset>,
    _referral: &OrderbookReferral,
    _multiplier: u64,
    _ctx: &TxContext,
...
public fun claim_pool_referral_rewards<BaseAsset, QuoteAsset>(
    ...
    let myso_rewards = referral_rewards.myso.withdraw_all().into_coin(ctx);
...
public fun get_quantity_out<BaseAsset, QuoteAsset>(
    ...
    let myso_price = self.myso_price.get_order_myso_price(whitelist);
    self
        .book
        .get_quantity_out(
            base_quantity,
...
public fun get_quantity_out_input_fee<BaseAsset, QuoteAsset>(
    ...
    let params = self.state.governance().trade_params();
    let taker_fee = params.taker_fee();
    let myso_price = self.myso_price.empty_myso_price();
    self
        .book
        .get_quantity_out(
            base_quantity,
...
public fun get_base_quantity_in<BaseAsset, QuoteAsset>(
    ...
    let params = self.state.governance().trade_params();
    let taker_fee = params.taker_fee();
    let myso_price = if (pay_with_myso) {
        self.myso_price.get_order_myso_price(whitelist)
    } else {
        self.myso_price.empty_myso_price()
    };
    self
        .book
...
public fun get_order_myso_price<BaseAsset, QuoteAsset>(
    self: &Pool<BaseAsset, QuoteAsset>,
): OrderMySoPrice {
    let whitelist = self.whitelisted();
    let self = self.load_inner();
...
public fun get_order_myso_required<BaseAsset, QuoteAsset>(
    self: &Pool<BaseAsset, QuoteAsset>,
    base_quantity: u64,
    price: u64,
): (u64, u64) {
    let order_myso_price = self.get_order_myso_price();
    let self = self.load_inner();
    let maker_fee = self.state.governance().trade_params().maker_fee();
    let taker_fee = self.state.governance().trade_params().taker_fee();
    let myso_quantity = order_myso_price
        .fee_quantity(
            base_quantity,
    ...
    (math::mul(taker_fee, myso_quantity), math::mul(maker_fee, myso_quantity))
}
...
public fun locked_balance<BaseAsset, QuoteAsset>(
    ...
    account_orders.do_ref!(|order| {
        ...
        myso_quantity = myso_quantity + locked_balance.myso();
    });
...
public fun can_place_limit_order<BaseAsset, QuoteAsset>(
    ...
    let order_myso_price = if (pay_with_myso) {
        pool_inner.myso_price.get_order_myso_price(whitelist)
    } else {
        pool_inner.myso_price.empty_myso_price()
    };
    ...
    let fee_balances = order_myso_price.fee_quantity(quantity, quote_quantity, is_bid);

    // Calculate required balances
    let mut required_base = 0;
    let mut required_quote = 0;
    let mut required_myso = 0;

    if (is_bid) {
        required_quote = quote_quantity;
        if (pay_with_myso) {
            required_myso = math::mul(fee_balances.myso(), taker_fee);
        } else {
    ...
    let available_myso = balance_manager.balance<MYSO>() + settled_balances.myso();
...
public fun can_place_market_order<BaseAsset, QuoteAsset>(
    ...
    if (is_bid) {
        ...
        // get_quote_quantity_in returns (base_out, quote_needed, myso_required)
        let (base_out, quote_needed, myso_required) = self.get_quote_quantity_in(
            quantity,
            pay_with_myso,
            clock,
        );
        ...
        let (_, _, myso_required) = if (pay_with_myso) {
        ...
        required_base = if (pay_with_myso) {
            quantity
        } else {
...
public fun get_pool_referral_balances<BaseAsset, QuoteAsset>(
    ...
    let myso_rewards = referral_rewards.myso.value();
