...
module orderbook::margin_registry;

use orderbook::{constants, math, pool::Pool};
use orderbook::margin_constants;
...
public struct OrderbookPoolRegistered has copy, drop {
    pool_id: ID,
    config: PoolConfig,
    timestamp: u64,
}

public struct OrderbookPoolUpdated has copy, drop {
    pool_id: ID,
    enabled: bool,
    timestamp: u64,
}

public struct OrderbookPoolConfigUpdated has copy, drop {
    pool_id: ID,
    config: PoolConfig,
    timestamp: u64,
}
...
public fun register_orderbook_pool<BaseAsset, QuoteAsset>(
    self: &mut MarginRegistry,
    _admin_cap: &MarginAdminCap,
    ...
    event::emit(OrderbookPoolRegistered {
        pool_id,
        config: pool_config,
        timestamp: clock.timestamp_ms(),
    });
...
/// Enables a orderbook pool for margin trading.
public fun enable_orderbook_pool<BaseAsset, QuoteAsset>(
    self: &mut MarginRegistry,
    _admin_cap: &MarginAdminCap,
    ...
    event::emit(OrderbookPoolUpdated {
        pool_id,
        enabled: true,
        timestamp: clock.timestamp_ms(),
    });
...
/// Disables a orderbook pool from margin trading. Only reduce only orders, cancels, and withdraw settled amounts are allowed.
public fun disable_orderbook_pool<BaseAsset, QuoteAsset>(
    self: &mut MarginRegistry,
    _admin_cap: &MarginAdminCap,
    pool: &mut Pool<BaseAsset, QuoteAsset>,
    clock: &Clock,
...
/// Updates risk params for a orderbook pool as the admin.
public fun update_risk_params<BaseAsset, QuoteAsset>(
    ...
    event::emit(OrderbookPoolConfigUpdated {
        pool_id,
        config: pool_config,
        timestamp: clock.timestamp_ms(),
    });
...
/// Check if a orderbook pool is registered for margin trading
...
/// Get the margin pool IDs for a orderbook pool
public fun get_orderbook_pool_margin_pool_ids(
    self: &MarginRegistry,
    orderbook_pool_id: ID,
): (ID, ID) {
    self.load_inner();
    let config = self.get_pool_config(orderbook_pool_id);
...
public fun can_liquidate(self: &MarginRegistry, orderbook_pool_id: ID, risk_ratio: u64): bool {
    let config = self.get_pool_config(orderbook_pool_id);
    risk_ratio < config.risk_ratios.liquidation_risk_ratio
}

public fun base_margin_pool_id(self: &MarginRegistry, orderbook_pool_id: ID): ID {
    let config = self.get_pool_config(orderbook_pool_id);
    config.base_margin_pool_id
}

public fun quote_margin_pool_id(self: &MarginRegistry, orderbook_pool_id: ID): ID {
    let config = self.get_pool_config(orderbook_pool_id);
    config.quote_margin_pool_id
}

public fun min_withdraw_risk_ratio(self: &MarginRegistry, orderbook_pool_id: ID): u64 {
    let config = self.get_pool_config(orderbook_pool_id);
    config.risk_ratios.min_withdraw_risk_ratio
}

public fun min_borrow_risk_ratio(self: &MarginRegistry, orderbook_pool_id: ID): u64 {
    let config = self.get_pool_config(orderbook_pool_id);
    config.risk_ratios.min_borrow_risk_ratio
}

public fun liquidation_risk_ratio(self: &MarginRegistry, orderbook_pool_id: ID): u64 {
    let config = self.get_pool_config(orderbook_pool_id);
    config.risk_ratios.liquidation_risk_ratio
}

public fun target_liquidation_risk_ratio(self: &MarginRegistry, orderbook_pool_id: ID): u64 {
    let config = self.get_pool_config(orderbook_pool_id);
    config.risk_ratios.target_liquidation_risk_ratio
}

public fun user_liquidation_reward(self: &MarginRegistry, orderbook_pool_id: ID): u64 {
    let config = self.get_pool_config(orderbook_pool_id);
    config.user_liquidation_reward
}

public fun pool_liquidation_reward(self: &MarginRegistry, orderbook_pool_id: ID): u64 {
    let config = self.get_pool_config(orderbook_pool_id);
...
/// Get the pool configuration for a orderbook pool
public fun get_pool_config(self: &MarginRegistry, orderbook_pool_id: ID): &PoolConfig {
    let inner = self.load_inner();
    assert!(inner.pool_registry.contains(orderbook_pool_id), EPoolNotRegistered);
    inner.pool_registry.borrow(orderbook_pool_id)
}
...
public(package) fun can_borrow(self: &MarginRegistry, orderbook_pool_id: ID, risk_ratio: u64): bool {
    let config = self.get_pool_config(orderbook_pool_id);
