...
module orderbook::margin_pool;

use orderbook::{constants, math};
use orderbook::{
...
const EOrderbookPoolAlreadyAllowed: u64 = 4;
const EOrderbookPoolNotAllowed: u64 = 5;
...
public struct MarginPool<phantom Asset> has key, store {
    ...
    allowed_orderbook_pools: VecSet<ID>,
...
public fun create_margin_pool<Asset>(
    ...
    let margin_pool = MarginPool<Asset> {
        ...
        allowed_orderbook_pools: vec_set::empty(),
...
/// Allow a margin manager tied to a orderbook pool to borrow from the margin pool.
public fun enable_orderbook_pool_for_loan<Asset>(
    ...
    assert!(!self.allowed_orderbook_pools.contains(&orderbook_pool_id), EOrderbookPoolAlreadyAllowed);
    self.allowed_orderbook_pools.insert(orderbook_pool_id);
...
/// Disable a margin manager tied to a orderbook pool from borrowing from the margin pool.
public fun disable_orderbook_pool_for_loan<Asset>(
    self: &mut MarginPool<Asset>,
    registry: &MarginRegistry,
    orderbook_pool_id: ID,
    margin_pool_cap: &MarginPoolCap,
    ...
    registry.load_inner();
    assert!(margin_pool_cap.margin_pool_id() == self.id(), EInvalidMarginPoolCap);
    assert!(self.allowed_orderbook_pools.contains(&orderbook_pool_id), EOrderbookPoolNotAllowed);
    self.allowed_orderbook_pools.remove(&orderbook_pool_id);
...
/// Return whether a margin manager for a given orderbook pool is allowed to borrow from the margin pool.
public fun orderbook_pool_allowed<Asset>(self: &MarginPool<Asset>, orderbook_pool_id: ID): bool {
    self.allowed_orderbook_pools.contains(&orderbook_pool_id)
}
