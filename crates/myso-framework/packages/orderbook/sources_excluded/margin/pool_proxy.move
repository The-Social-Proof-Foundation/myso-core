...
module orderbook::pool_proxy;

use orderbook::{math, order_info::OrderInfo, pool::Pool};
use orderbook::{
...
// === Errors ===
const ECannotStakeWithMysoMarginManager: u64 = 1;
const EPoolNotEnabledForMarginTrading: u64 = 2;
const ENotReduceOnlyOrder: u64 = 3;
const EIncorrectOrderbookPool: u64 = 4;
...
public fun place_limit_order<BaseAsset, QuoteAsset>(
    ...
    assert!(margin_manager.orderbook_pool() == pool.id(), EIncorrectOrderbookPool);
...
public fun stake<BaseAsset, QuoteAsset>(
    ...
    assert!(
        base_asset_type != myso_asset_type && quote_asset_type != myso_asset_type,
        ECannotStakeWithMysoMarginManager,
    );
...
/// Unstake MYSO tokens from the pool.
...
fun calculate_effective_price<BaseAsset, QuoteAsset>(
    ...
    if (is_bid) {
        let (base_out, quote_in, _) = pool.get_quote_quantity_in(quantity, pay_with_myso, clock);
