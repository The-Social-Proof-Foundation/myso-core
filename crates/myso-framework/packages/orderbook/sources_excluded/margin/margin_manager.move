...
module orderbook::margin_manager;
...
use myso::{clock::Clock, coin::Coin, event, vec_map::{Self, VecMap}};
use myso::myso::MYSO;
...
const EIncorrectOrderbookPool: u64 = 5;
const EOrderbookPoolNotAllowedForLoan: u64 = 6;
...
// === Structs ===
/// Witness type for authorizing MarginManager to call protected features of the Orderbook
...
public fun add_conditional_order<BaseAsset, QuoteAsset>(
    ...
    assert!(pool.id() == self.orderbook_pool(), EIncorrectOrderbookPool);
    self
...
public fun new<BaseAsset, QuoteAsset>(
    ...
    let manager = new_margin_manager(pool, orderbook_registry, margin_registry, clock, ctx);
...
public fun set_margin_manager_referral<BaseAsset, QuoteAsset>(
    self: &mut MarginManager<BaseAsset, QuoteAsset>,
    referral_cap: &OrderbookPoolReferral,
    ctx: &mut TxContext,
) {
    self.validate_owner(ctx);
...
// === Public Functions - Margin Manager ===
/// Deposit a coin into the margin manager. The coin must be of the same type as either the base, quote, or MYSO.
public fun deposit<BaseAsset, QuoteAsset, DepositAsset>(
    ...
    // nor the quote asset is deposited. This handles the case for MYSO deposits, when MYSO is not part of the base
    // or quote assets.
...
/// Withdraw a specified amount of an asset from the margin manager. The asset must be of the same type as either the base, quote, or MYSO.
...
public fun withdraw<BaseAsset, QuoteAsset, WithdrawAsset>(
    ...
    // nor the quote asset is withdrawn. This handles the case for MYSO withdrawals, when MYSO is not part of the base
    // or quote assets.
...
public fun borrow_base<BaseAsset, QuoteAsset>(
    ...
    assert!(
        base_margin_pool.orderbook_pool_allowed(self.orderbook_pool),
        EOrderbookPoolNotAllowedForLoan,
    );
...
public fun borrow_quote<BaseAsset, QuoteAsset>(
    ...
    assert!(
        quote_margin_pool.orderbook_pool_allowed(self.orderbook_pool),
        EOrderbookPoolNotAllowedForLoan,
    );
...
public fun liquidate<BaseAsset, QuoteAsset, DebtAsset>(
    ...
    assert!(self.orderbook_pool == pool.id(), EIncorrectOrderbookPool);
    assert!(self.margin_pool_id.contains(&margin_pool.id()), EIncorrectMarginPool);
    let risk_ratio = self.risk_ratio_int(
        registry,
        base_oracle,
    ...
    let (assets_in_debt_unit, base_asset, quote_asset) = self.assets_in_debt_unit(
        ...
        quote_oracle,
        clock,
    ); // MYSO/USDC pool. We have 90 MYSO and 40 USDC, 350 USDC debt. This should be 400 USDC. (assume 1 MYSO = 4 USDC)
    ...
    // We have 40 USDC which is used first in the second loop. Then MYSO to reach the total of 101.941 USDC.
...
public fun myso_balance<BaseAsset, QuoteAsset>(self: &MarginManager<BaseAsset, QuoteAsset>): u64 {
    self.balance_manager.balance<MYSO>()
}
...
/// Returns (manager_id, orderbook_pool_id, risk_ratio, base_asset, quote_asset,
...
public fun manager_state<BaseAsset, QuoteAsset>(
    ...
    let manager_id = self.id();
    let orderbook_pool_id = self.orderbook_pool;
...
public fun orderbook_pool<BaseAsset, QuoteAsset>(self: &MarginManager<BaseAsset, QuoteAsset>): ID {
    self.orderbook_pool
}
...
/// Unwraps balance manager for trading in orderbook.
...
public(package) fun deposit_int<BaseAsset, QuoteAsset, DepositAsset>(
    ...
    let myso_asset_type = type_name::with_defining_ids<MYSO>();
    assert!(
        deposit_asset_type == base_asset_type || deposit_asset_type == quote_asset_type ||
        deposit_asset_type == myso_asset_type,
        EInvalidDeposit,
    );
