...
module myso_system::validator_set;

use myso::bag::{Self, Bag};
use myso::balance::Balance;
use myso::event;
use myso::priority_queue as pq;
use myso::myso::MYSO;
use myso::table::{Self, Table};
use myso::table_vec::{Self, TableVec};
use myso::vec_map::{Self, VecMap};
use myso::vec_set::VecSet;
use myso_system::staking_pool::{
    PoolTokenExchangeRate,
    StakedMySo,
    pool_id,
    FungibleStakedMySo,
    fungible_staked_myso_pool_id
};
use myso_system::validator::{Validator, staking_pool_id, myso_address};
...
public(package) fun new(
    ...
    init_active_validators.do_ref!(|v| {
        staking_pool_mappings.add(v.staking_pool_id(), v.myso_address());
    });
...
/// Called by `myso_system` to add a new validator candidate.
public(package) fun request_add_validator_candidate(
    ...
    self.validator_candidates.add(validator.myso_address(), validator.wrap_v1(ctx));
}

/// Called by `myso_system` to remove a validator candidate, and move them to `inactive_validators`.
...
/// Called by `myso_system` to add a new validator to `pending_active_validators`, which will be
...
/// Called by `myso_system`, to remove a validator.
...
/// Called by `myso_system`, to add a new stake to the validator.
...
public(package) fun request_add_stake(
    ...
    assert!(myso_amount >= MIN_STAKING_THRESHOLD, EStakingBelowThreshold);
    self
        .get_candidate_or_active_validator_mut(validator_address)
        .request_add_stake(stake, ctx.sender(), ctx)
}

/// Called by `myso_system`, to withdraw some share of a stake from the validator. The share to withdraw
/// is denoted by `principal_withdraw_amount`. One of two things occurs in this function:
/// 1. If the `staked_myso` is staked with an active validator, the request is added to the validator's
///    staking pool's pending stake withdraw entries, processed at the end of the epoch.
/// 2. If the `staked_myso` was staked with a validator that is no longer active,
...
public(package) fun request_withdraw_stake(
    ...
    let staking_pool_id = staked_myso.pool_id();
    let validator = if (self.staking_pool_mappings.contains(staking_pool_id)) {
        // This is an active validator.
        let validator_address = self.staking_pool_mappings[staked_myso.pool_id()];
    ...
    validator.request_withdraw_stake(staked_myso, ctx)
}
...
public(package) fun convert_to_fungible_staked_myso(
    ...
    validator.convert_to_fungible_staked_myso(staked_myso, ctx)
}
...
public(package) fun redeem_fungible_staked_myso(
    ...
    let staking_pool_id = fungible_staked_myso.pool_id();
    ...
    validator.redeem_fungible_staked_myso(fungible_staked_myso, ctx)
}
...
public(package) fun advance_epoch(
    self: &mut ValidatorSet,
    computation_reward: &mut Balance<MYSO>,
    storage_fund_reward: &mut Balance<MYSO>,
...
fun update_validator_positions_and_calculate_total_stake(
    ...
    while (i > 0) {
        ...
        let validator_address = validator_ref.myso_address();
    ...
    pending_active_validators.do!(|mut validator| {
        ...
        if (voting_power >= min_joining_voting_power_threshold) {
            ...
            event::emit(ValidatorJoinEvent {
                epoch: ctx.epoch(),
                validator_address: validator.myso_address(),
...
/// Called by `myso_system` to derive reference gas price for the new epoch.
...
public(package) fun is_active_validator_by_myso_address(
    self: &ValidatorSet,
    validator_address: address,
): bool {
...
/// It differs from `is_active_validator_by_myso_address` in that the former checks
...
fun find_validator_from_table_vec(
    ...
    while (i < length) {
        let v = &validators[i];
        if (v.myso_address() == validator_address) {
...
fun compute_slashed_validators(
    ...
    while (!validator_report_records.is_empty()) {
        ...
        assert!(
            self.is_active_validator_by_myso_address(validator_address),
...
fun distribute_reward(
    ...
    staking_rewards: &mut Balance<MYSO>,
    storage_fund_reward: &mut Balance<MYSO>,
    ctx: &mut TxContext,
) {
    let length = validators.length();
...
fun emit_validator_epoch_events(
    ...
    length.do!(|i| {
        let v = &vs[i];
        let validator_address = v.myso_address();
...
public(package) fun is_active_validator(self: &ValidatorSet, addr: address): bool {
    self.active_validators.any!(|v| v.myso_address() == addr)
}
