...
#[allow(unused_const)]
module myso_system::staking_pool;
...
const MIN_STAKING_THRESHOLD: u64 = 1_000_000_000; // 1 MYSO

const EInsufficientPoolTokenBalance: u64 = 0;
const EWrongPool: u64 = 1;
const EWithdrawAmountCannotBeZero: u64 = 2;
const EInsufficientMySoTokenBalance: u64 = 3;
...
const EIncompatibleStakedMySo: u64 = 12;
...
const EDelegationOfZeroMySo: u64 = 17;
const EStakedMySoBelowThreshold: u64 = 18;
const ECannotMintFungibleStakedMySoYet: u64 = 19;
...
public struct StakingPool has key, store {
    ...
    /// in the `StakedMySo` object, updated at epoch boundaries.
    myso_balance: u64,
    /// The epoch stake rewards will be added here at the end of each epoch.
    rewards_pool: Balance<MYSO>,
    ...
    pending_total_myso_withdraw: u64,
...
public struct StakedMySo has key, store {
    id: UID,
    ...
    principal: Balance<MYSO>,
}

/// An alternative to `StakedMySo` that holds the pool token amount instead of the MYSO balance.
...
public struct FungibleStakedMySo has key, store {
    id: UID,
    /// ID of the staking pool we are staking with.
    pool_id: ID,
...
public struct FungibleStakedMySoData has key, store {
    id: UID,
...
// === dynamic field keys ===
public struct FungibleStakedMySoDataKey has copy, drop, store {}
...
public struct UnderflowMySoBalance has copy, drop, store {}
...
public(package) fun request_add_stake(
    ...
    let myso_amount = stake.value();
    assert!(!pool.is_inactive(), EDelegationToInactivePool);
    assert!(myso_amount > 0, EDelegationOfZeroMySo);

    pool.pending_stake = pool.pending_stake + myso_amount;
    StakedMySo {
        id: object::new(ctx),
...
public(package) fun request_withdraw_stake(
    ...
    if (staked_myso.stake_activation_epoch > ctx.epoch() && !pool.is_preactive()) {
        let principal = staked_myso.into_balance();
    ...
    let total_myso_withdraw_amount = principal_withdraw_amount + rewards_withdraw.value();
...
public(package) fun redeem_fungible_staked_myso(
    pool: &mut StakingPool,
    fungible_staked_myso: FungibleStakedMySo,
    ctx: &TxContext,
): Balance<MYSO> {
    let FungibleStakedMySo { id, pool_id, value } = fungible_staked_myso;
    assert!(pool_id == object::id(pool), EWrongPool);

    id.delete();

    let latest_exchange_rate = pool.pool_token_exchange_rate_at_epoch(ctx.epoch());
    let fungible_staked_myso_data: &mut FungibleStakedMySoData =
        &mut pool.extra_fields[FungibleStakedMySoDataKey {}];

    let (
        principal_amount,
        rewards_amount,
    ) = latest_exchange_rate.calculate_fungible_staked_myso_withdraw_amount(
        value,
    ...
    let mut myso_out = fungible_staked_myso_data.principal.split(principal_amount);
    myso_out.join(pool.rewards_pool.split(rewards_amount));
...
fun calculate_fungible_staked_myso_withdraw_amount(
    latest_exchange_rate: PoolTokenExchangeRate,
    fungible_staked_myso_value: u64,
    ...
    // 1. if the entire FungibleStakedMySoData supply is redeemed, how much myso should we receive?
    let total_myso_amount = latest_exchange_rate.get_myso_amount(
        fungible_staked_myso_data_total_supply,
    );

    // min with total_myso_amount to prevent underflow
    let fungible_staked_myso_data_principal_amount = fungible_staked_myso_data_principal_amount.min(
        total_myso_amount,
    );

    // 2. how much do we need to withdraw from the rewards pool?
    let total_rewards = total_myso_amount - fungible_staked_myso_data_principal_amount;

    // 3. proportionally withdraw from both wrt the fungible_staked_myso_value.
    ...
    let expected_myso_amount = latest_exchange_rate.get_myso_amount(fungible_staked_myso_value);
    assert!(
...
public(package) fun convert_to_fungible_staked_myso(
    pool: &mut StakingPool,
    staked_myso: StakedMySo,
    ctx: &mut TxContext,
): FungibleStakedMySo {
    let StakedMySo { id, pool_id, stake_activation_epoch, principal } = staked_myso;

    assert!(pool_id == object::id(pool), EWrongPool);
    assert!(ctx.epoch() >= stake_activation_epoch, ECannotMintFungibleStakedMySoYet);
    ...
    let key = FungibleStakedMySoDataKey {};
    ...
    if (!pool.extra_fields.contains(key)) {
        ...
        let fungible_staked_myso_data: &mut FungibleStakedMySoData = &mut pool.extra_fields[key];
        fungible_staked_myso_data.total_supply =
            fungible_staked_myso_data.total_supply + pool_token_amount;
...
public(package) fun withdraw_from_principal(
    ...
    assert!(staked_myso.pool_id == object::id(pool), EWrongPool);

    let exchange_rate_at_staking_epoch = pool.pool_token_exchange_rate_at_epoch(staked_myso.stake_activation_epoch);
    let principal_withdraw = staked_myso.into_balance();
...
/// Allows calling `.into_balance()` on `StakedMySo` to invoke `unwrap_staked_myso`
use fun unwrap_staked_myso as StakedMySo.into_balance;

fun unwrap_staked_myso(staked_myso: StakedMySo): Balance<MYSO> {
    let StakedMySo { id, principal, .. } = staked_myso;
    id.delete();
    principal
}
...
public(package) fun deposit_rewards(pool: &mut StakingPool, rewards: Balance<MYSO>) {
    pool.myso_balance = pool.myso_balance + rewards.value();
...
fun process_pending_stake_withdraw(pool: &mut StakingPool) {
    pool.myso_balance = if (pool.myso_balance >= pool.pending_total_myso_withdraw) {
        pool.myso_balance - pool.pending_total_myso_withdraw
    } else {
        // the diff will be applied in the `process_pending_stake` function.
        let diff = pool.pending_total_myso_withdraw - pool.myso_balance;
        pool.extra_fields.add(UnderflowMySoBalance {}, diff);
        0
    };
...
public(package) fun process_pending_stake(pool: &mut StakingPool) {
    ...
    // This key is only present if the `myso_balance` underflowed, hence, the current value of `myso_balance`
    // is `0`. Pool token balance will be recalculated automatically for `0` value.
    let myso_diff = {
        let key = UnderflowMySoBalance {};
        if (pool.extra_fields.contains(key)) pool.extra_fields.remove(key) else 0
    };

    pool.myso_balance = pool.myso_balance + pool.pending_stake - myso_diff;
    pool.pool_token_balance = latest_exchange_rate.get_token_amount(pool.myso_balance);
...
fun withdraw_rewards(
    ...
    let total_myso_withdraw_amount = exchange_rate.get_myso_amount(pool_token_withdraw_amount);
    let mut reward_withdraw_amount = if (total_myso_withdraw_amount >= principal_withdraw_amount) {
...
public fun myso_balance(pool: &StakingPool): u64 { pool.myso_balance }

public fun pool_id(staked_myso: &StakedMySo): ID { staked_myso.pool_id }

public use fun fungible_staked_myso_pool_id as FungibleStakedMySo.pool_id;

public fun fungible_staked_myso_pool_id(fungible_staked_myso: &FungibleStakedMySo): ID {
    fungible_staked_myso.pool_id
}

/// Allows calling `.amount()` on `StakedMySo` to invoke `staked_myso_amount`
public use fun staked_myso_amount as StakedMySo.amount;

/// Returns the principal amount of `StakedMySo`.
public fun staked_myso_amount(staked_myso: &StakedMySo): u64 { staked_myso.principal.value() }

public use fun stake_activation_epoch as StakedMySo.activation_epoch;

/// Returns the activation epoch of `StakedMySo`.
public fun stake_activation_epoch(staked_myso: &StakedMySo): u64 {
    staked_myso.stake_activation_epoch
}
...
public use fun fungible_staked_myso_value as FungibleStakedMySo.value;

public fun fungible_staked_myso_value(fungible_staked_myso: &FungibleStakedMySo): u64 {
    fungible_staked_myso.value
}

public use fun split_fungible_staked_myso as FungibleStakedMySo.split;

public fun split_fungible_staked_myso(
    fungible_staked_myso: &mut FungibleStakedMySo,
    split_amount: u64,
    ctx: &mut TxContext,
): FungibleStakedMySo {
    assert!(split_amount <= fungible_staked_myso.value, EInsufficientPoolTokenBalance);

    fungible_staked_myso.value = fungible_staked_myso.value - split_amount;

    FungibleStakedMySo {
        id: object::new(ctx),
        pool_id: fungible_staked_myso.pool_id,
        value: split_amount,
    }
}

public use fun join_fungible_staked_myso as FungibleStakedMySo.join;

public fun join_fungible_staked_myso(self: &mut FungibleStakedMySo, other: FungibleStakedMySo) {
    let FungibleStakedMySo { id, pool_id, value } = other;
...
/// Split StakedMySo `self` to two parts, one with principal `split_amount`,
/// and the remaining principal is left in `self`.
/// All the other parameters of the StakedMySo like `stake_activation_epoch` or `pool_id` remain the same.
public fun split(self: &mut StakedMySo, split_amount: u64, ctx: &mut TxContext): StakedMySo {
    let original_amount = self.principal.value();
    assert!(split_amount <= original_amount, EInsufficientMySoTokenBalance);
    let remaining_amount = original_amount - split_amount;
    // Both resulting parts should have at least MIN_STAKING_THRESHOLD.
    assert!(remaining_amount >= MIN_STAKING_THRESHOLD, EStakedMySoBelowThreshold);
    assert!(split_amount >= MIN_STAKING_THRESHOLD, EStakedMySoBelowThreshold);
    StakedMySo {
        id: object::new(ctx),
...
/// Allows calling `.split_to_sender()` on `StakedMySo` to invoke `split_staked_myso`
public use fun split_staked_myso as StakedMySo.split_to_sender;

#[allow(lint(public_entry))]
/// Split the given StakedMySo to the two parts, one with principal `split_amount`,
/// transfer the newly split part to the sender address.
public entry fun split_staked_myso(stake: &mut StakedMySo, split_amount: u64, ctx: &mut TxContext) {
    transfer::transfer(stake.split(split_amount, ctx), ctx.sender());
}

/// Allows calling `.join()` on `StakedMySo` to invoke `join_staked_myso`
public use fun join_staked_myso as StakedMySo.join;

#[allow(lint(public_entry))]
/// Consume the staked myso `other` and add its value to `self`.
/// Aborts if some of the staking parameters are incompatible (pool id, stake activation epoch, etc.)
public entry fun join_staked_myso(self: &mut StakedMySo, other: StakedMySo) {
    assert!(is_equal_staking_metadata(self, &other), EIncompatibleStakedMySo);
    let StakedMySo { id, principal, .. } = other;
...
public fun is_equal_staking_metadata(self: &StakedMySo, other: &StakedMySo): bool {
...
public fun pending_stake_withdraw_amount(staking_pool: &StakingPool): u64 {
    staking_pool.pending_total_myso_withdraw
}
...
public fun myso_amount(exchange_rate: &PoolTokenExchangeRate): u64 {
    exchange_rate.myso_amount
}
...
fun get_myso_amount(exchange_rate: &PoolTokenExchangeRate, token_amount: u64): u64 {
    // When either amount is 0, that means we have no stakes with this pool.
    // The other amount might be non-zero when there's dust left in the pool.
    if (exchange_rate.myso_amount == 0 || exchange_rate.pool_token_amount == 0) {
        return token_amount
    };

    mul_div!(exchange_rate.myso_amount, token_amount, exchange_rate.pool_token_amount)
}

fun get_token_amount(exchange_rate: &PoolTokenExchangeRate, myso_amount: u64): u64 {
    // When either amount is 0, that means we have no stakes with this pool.
    // The other amount might be non-zero when there's dust left in the pool.
    if (exchange_rate.myso_amount == 0 || exchange_rate.pool_token_amount == 0) {
        return myso_amount
    ...
    mul_div!(exchange_rate.pool_token_amount, myso_amount, exchange_rate.myso_amount)
}

fun initial_exchange_rate(): PoolTokenExchangeRate {
    PoolTokenExchangeRate { myso_amount: 0, pool_token_amount: 0 }
}
...
fun check_balance_invariants(pool: &StakingPool, epoch: u64) {
    ...
    let expected = exchange_rate.get_token_amount(pool.myso_balance);
...
// Given the `staked_myso` receipt calculate the current rewards (in terms of MYSO) for it.
public(package) fun calculate_rewards(
    pool: &StakingPool,
    staked_myso: &StakedMySo,
    current_epoch: u64,
): u64 {
    let staked_amount = staked_myso.amount();
    ...
    let total_myso_withdraw_amount = new_epoch_exchange_rate.get_myso_amount(
        pool_token_withdraw_amount,
    );

    let mut reward_withdraw_amount = if (total_myso_withdraw_amount >= staked_amount) {
...
#[test_only]
public(package) fun fungible_staked_myso_data(pool: &StakingPool): &FungibleStakedMySoData {
    bag::borrow(&pool.extra_fields, FungibleStakedMySoDataKey {})
}

#[test_only]
public use fun fungible_staked_myso_data_total_supply as FungibleStakedMySoData.total_supply;

#[test_only]
public(package) fun fungible_staked_myso_data_total_supply(
    fungible_staked_myso_data: &FungibleStakedMySoData,
): u64 {
    fungible_staked_myso_data.total_supply
}

#[test_only]
public use fun fungible_staked_myso_data_principal_value as FungibleStakedMySoData.principal_value;

#[test_only]
public(package) fun fungible_staked_myso_data_principal_value(
    fungible_staked_myso_data: &FungibleStakedMySoData,
): u64 {
    fungible_staked_myso_data.principal.value()
}
...
#[test_only]
public(package) fun create_fungible_staked_myso_for_testing(
    self: &StakingPool,
    value: u64,
    ctx: &mut TxContext,
): FungibleStakedMySo {
...
#[test_only]
public(package) fun increase_pending_total_myso_withdraw_for_testing(
    pool: &mut StakingPool,
    delta: u64,
) {
    pool.pending_total_myso_withdraw = pool.pending_total_myso_withdraw + delta
}
...
#[random_test]
fun test_calculate_fungible_staked_myso_withdraw_amount(
    mut total_myso_amount: u64,
    // these are all in basis points
    mut pool_token_frac: u16,
    mut fungible_staked_myso_data_total_supply_frac: u16,
    mut fungible_staked_myso_data_principal_frac: u16,
    mut fungible_staked_myso_value_bps: u16,
) {
    total_myso_amount = total_myso_amount.max(1);

    pool_token_frac = pool_token_frac % 10_000;
    fungible_staked_myso_data_total_supply_frac =
        fungible_staked_myso_data_total_supply_frac % 10_000;
    fungible_staked_myso_data_principal_frac = fungible_staked_myso_data_principal_frac % 10_000;
    fungible_staked_myso_value_bps = fungible_staked_myso_value_bps % 10_000;

    let total_pool_token_amount = mul_div!(total_myso_amount, pool_token_frac as u64, 10_000).max(1);

    let exchange_rate = PoolTokenExchangeRate {
        myso_amount: total_myso_amount,
        pool_token_amount: total_pool_token_amount,
    };

    let fungible_staked_myso_data_total_supply = mul_div!(
        total_pool_token_amount,
        fungible_staked_myso_data_total_supply_frac as u64,
        10_000,
    ).max(1);
    let fungible_staked_myso_value = mul_div!(
        fungible_staked_myso_data_total_supply,
        fungible_staked_myso_value_bps as u64,
        10_000,
    );

    let max_principal = exchange_rate.get_myso_amount(fungible_staked_myso_data_total_supply);
    let fungible_staked_myso_data_principal_amount = mul_div!(
        max_principal,
        fungible_staked_myso_data_principal_frac as u64,
        10_000,
    ).max(1);

    let (principal_amount, rewards_amount) = calculate_fungible_staked_myso_withdraw_amount(
        exchange_rate,
        fungible_staked_myso_value,
        fungible_staked_myso_data_principal_amount,
        fungible_staked_myso_data_total_supply,
    ...
    let expected_out = exchange_rate.get_myso_amount(fungible_staked_myso_value);
