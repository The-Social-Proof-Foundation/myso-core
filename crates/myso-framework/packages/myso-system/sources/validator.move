...
use myso_system::validator_cap::{Self, ValidatorOperationCap};

public use fun myso_system::validator_wrapper::create_v1 as Validator.wrap_v1;
...
public struct ConvertingToFungibleStakedMySoEvent has copy, drop {
    pool_id: ID,
    stake_activation_epoch: u64,
    staked_myso_principal_amount: u64,
    fungible_staked_myso_amount: u64,
}

/// Event emitted when a fungible staked MYSO is redeemed.
public struct RedeemingFungibleStakedMySoEvent has copy, drop {
    pool_id: ID,
    fungible_staked_myso_amount: u64,
    myso_amount: u64,
}
...
public(package) fun request_add_stake(
    ...
    let staked_myso = self.staking_pool.request_add_stake(stake, stake_epoch, ctx);
    ...
    event::emit(StakingRequestEvent {
        ...
        validator_address: self.metadata.myso_address,
        staker_address,
        epoch: ctx.epoch(),
        amount: stake_amount,
    });
...
public(package) fun convert_to_fungible_staked_myso(
    ...
    let stake_activation_epoch = staked_myso.activation_epoch();
    let staked_myso_principal_amount = staked_myso.amount();
    let fungible_staked_myso = self.staking_pool.convert_to_fungible_staked_myso(staked_myso, ctx);

    event::emit(ConvertingToFungibleStakedMySoEvent {
        pool_id: self.staking_pool_id(),
...
public(package) fun redeem_fungible_staked_myso(
    ...
    let fungible_staked_myso_amount = fungible_staked_myso.value();
    let myso = self.staking_pool.redeem_fungible_staked_myso(fungible_staked_myso, ctx);

    self.next_epoch_stake = self.next_epoch_stake - myso.value();

    event::emit(RedeemingFungibleStakedMySoEvent {
        pool_id: self.staking_pool_id(),
...
public(package) fun request_add_stake_at_genesis(
    ...
    // 0 = genesis epoch
    let staked_myso = self.staking_pool.request_add_stake(stake, 0, ctx);
...
public(package) fun request_withdraw_stake(
    ...
    let principal_amount = staked_myso.amount();
    let stake_activation_epoch = staked_myso.activation_epoch();
    let withdrawn_stake = self.staking_pool.request_withdraw_stake(staked_myso, ctx);
...
public(package) fun request_set_gas_price(
    ...
    assert!(validator_address == self.metadata.myso_address, EInvalidCap);
...
public(package) fun deposit_stake_rewards(self: &mut Validator, reward: Balance<MYSO>) {
...
public fun myso_address(self: &Validator): address {
    self.metadata.myso_address
}
...
public fun total_stake_amount(self: &Validator): u64 {
    self.staking_pool.myso_balance()
}
...
public fun is_duplicate(self: &Validator, other: &Validator): bool {
    ...
    self.myso_address == other.myso_address
...
public(package) fun new_unverified_validator_operation_cap_and_transfer(
    ...
    assert!(sender == self.metadata.myso_address, ENewCapNotCreatedByValidatorItself);
...
fun new_from_metadata(
    ...
    let myso_address = metadata.myso_address;
...
// Note: `proof_of_possession` MUST be a valid signature using myso_address and
...
public(package) fun new_for_testing(
    ...
    primary_address: vector<u8>,
    worker_address: vector<u8>,
    initial_stake_option: Option<Balance<MYSO>>,
    gas_price: u64,
    commission_rate: u64,
    ...
    initial_stake_option.do!(|balance| {
        request_add_stake_at_genesis(
            &mut validator,
            balance,
            myso_address, // give the stake to the validator
            ctx,
        );
    });
