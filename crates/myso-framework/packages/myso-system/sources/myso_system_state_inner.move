...
use myso_system::storage_fund::{Self, StorageFund};
use myso_system::validator::{Self, Validator};
use myso_system::validator_cap::{UnverifiedValidatorOperationCap, ValidatorOperationCap};
use myso_system::validator_set::{Self, ValidatorSet};
...
public struct MySoSystemStateInner has store {
    ...
    safe_mode: bool,
    safe_mode_storage_rewards: Balance<MYSO>,
    safe_mode_computation_rewards: Balance<MYSO>,
...
public(package) fun v1_to_v2(self: MySoSystemStateInner): MySoSystemStateInnerV2 {
...
public(package) fun request_add_validator(self: &mut MySoSystemStateInnerV2, ctx: &TxContext) {
    assert!(
...
/// of the validator.
public(package) fun request_remove_validator(self: &mut MySoSystemStateInnerV2, ctx: &TxContext) {
...
public(package) fun request_withdraw_stake(
    ...
    self.validators.request_withdraw_stake(staked_myso, ctx)
}
...
public(package) fun convert_to_fungible_staked_myso(
    ...
    self.validators.convert_to_fungible_staked_myso(staked_myso, ctx)
}
...
public(package) fun redeem_fungible_staked_myso(
    ...
    self.validators.redeem_fungible_staked_myso(fungible_staked_myso, ctx)
}
...
public(package) fun report_validator(
    ...
    assert!(self.validators.is_active_validator_by_myso_address(reportee_addr), ENotValidator);
...
public(package) fun rotate_operation_cap(self: &mut MySoSystemStateInnerV2, ctx: &mut TxContext) {
...
public(package) fun advance_epoch(
    ...
    mut storage_reward: Balance<MYSO>,
    mut computation_reward: Balance<MYSO>,
...
public(package) fun epoch(self: &MySoSystemStateInnerV2): u64 {
    self.epoch
}

public(package) fun protocol_version(self: &MySoSystemStateInnerV2): u64 {
    self.protocol_version
}

public(package) fun system_state_version(self: &MySoSystemStateInnerV2): u64 {
...
public(package) fun epoch_start_timestamp_ms(self: &MySoSystemStateInnerV2): u64 {
...
public(package) fun get_reporters_of(self: &MySoSystemStateInnerV2, addr: address): VecSet<address> {
    if (self.validator_report_records.contains(&addr)) self.validator_report_records[&addr]
    else vec_set::empty()
}

public(package) fun get_storage_fund_total_balance(self: &MySoSystemStateInnerV2): u64 {
    self.storage_fund.total_balance()
}

public(package) fun get_storage_fund_object_rebates(self: &MySoSystemStateInnerV2): u64 {
...
public(package) fun active_validator_addresses(self: &MySoSystemStateInnerV2): vector<address> {
...
public(package) fun validators(self: &MySoSystemStateInnerV2): &ValidatorSet {
    &self.validators
}

public(package) fun validators_mut(self: &mut MySoSystemStateInnerV2): &mut ValidatorSet {
    &mut self.validators
}

public(package) fun extra_fields(self: &MySoSystemStateInnerV2): &Bag {
    &self.extra_fields
}

public(package) fun extra_fields_mut(self: &mut MySoSystemStateInnerV2): &mut Bag {
...
#[test_only]
public(package) fun get_stake_subsidy_distribution_counter(self: &MySoSystemStateInnerV2): u64 {
    self.stake_subsidy.get_distribution_counter()
}

#[test_only]
public(package) fun set_epoch_for_testing(self: &mut MySoSystemStateInnerV2, epoch_num: u64) {
...
#[test_only]
public(package) fun epoch_duration_ms(self: &MySoSystemStateInnerV2): u64 {
