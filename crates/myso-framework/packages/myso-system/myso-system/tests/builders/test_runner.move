...
/// provides a set of convenience methods for testing the MySo System.
module myso_system::test_runner;

use myso::balance::{Self, Balance};
use myso::coin::{Self, Coin};
use myso::myso::MYSO;
use myso::test_scenario::{Self, Scenario};
use myso_system::stake_subsidy;
...
use myso_system::validator_builder::{Self, ValidatorBuilder};
...
public fun new(): TestRunnerBuilder {
    TestRunnerBuilder {
        ...
        myso_supply_amount: option::none(),
...
public fun build(builder: TestRunnerBuilder): TestRunner {
    ...
    let system_parameters = myso_system_state_inner::create_system_parameters(
    ...
    let genesis_validator_addresses = validators.map_ref!(|v| v.myso_address());

    // create myso system state
    myso_system::create(
        object::new(scenario.ctx()), // it doesn't matter what ID myso system state has in tests
        validators,
        balance::create_for_testing<MYSO>(storage_fund_amount.destroy_or!(0) * MIST_PER_MYSO), // storage_fund
...
public fun myso_supply_amount(
    mut builder: TestRunnerBuilder,
    myso_supply_amount: u64,
): TestRunnerBuilder {
    builder.myso_supply_amount = option::some(myso_supply_amount);
    builder
}
...
/// Mint a MYSO balance for testing.
public fun mint(amount: u64): Balance<MYSO> {
    balance::create_for_testing(amount * MIST_PER_MYSO)
}
...
public macro fun system_tx(
    ...
    $f: |&mut MySoSystemState, &mut TxContext|,
): &mut TestRunner {
    let sender = sender($runner);
    let scenario = scenario_mut($runner);
    scenario.next_tx(sender);
    let mut system_state = scenario.take_shared<MySoSystemState>();
    ...
    runner.system_tx!(|system, ctx| {
        ...
        storage_rebate_balance =
            system.advance_epoch_for_testing(
                ...
                storage_charge.destroy_or!(0) * MIST_PER_MYSO,
                computation_charge.destroy_or!(0) * MIST_PER_MYSO,
...
public fun stake_with(runner: &mut TestRunner, validator: address, amount: u64) {
    ...
    runner.system_tx!(|system, ctx| {
        system.request_add_stake(
            coin::mint_for_testing(amount * MIST_PER_MYSO, ctx),
            validator,
            ctx,
        );
    });
...
public fun unstake(runner: &mut TestRunner, staked_myso_idx: u64) {
    let sender = runner.sender;
    runner.set_sender(sender);
    let stake_myso_ids = runner.scenario.ids_for_sender<StakedMySo>();
    let staked_myso = runner.scenario.take_from_sender_by_id(stake_myso_ids[staked_myso_idx]);
    runner.system_tx!(|system, ctx| {
        system.request_withdraw_stake(staked_myso, ctx);
    });
}
...
public fun add_validator_candidate(runner: &mut TestRunner, validator: Validator) {
    let sender = runner.sender;
    runner.scenario.next_tx(validator.myso_address());
    runner.system_tx!(|system, ctx| {
...
/// Get the sum of the balances of all the MYSO coins in the sender's account.
public fun myso_balance(runner: &mut TestRunner): u64 {
    let sender = runner.sender;
    let scenario = runner.scenario_mut();
    scenario.next_tx(sender);
    scenario.ids_for_sender<Coin<MYSO>>().fold!(0, |mut sum, coin_id| {
        let coin = scenario.take_from_sender_by_id<Coin<MYSO>>(coin_id);
        sum = sum + coin.value();
...
/// Get the sum of the StakedMySo objects' principal and the rewards for these objects.
public fun staking_rewards_balance(runner: &mut TestRunner): u64 {
    let sender = runner.sender;
    let scenario = runner.scenario_mut();
    let mut system = scenario.take_shared<MySoSystemState>();

    scenario.next_tx(sender);
    let total_balance = scenario.ids_for_sender<StakedMySo>().fold!(0, |mut sum, staked_myso_id| {
        let staked_myso = scenario.take_from_sender_by_id<StakedMySo>(staked_myso_id);
        let rewards = system.calculate_rewards(&staked_myso, scenario.ctx());

        sum = sum + rewards + staked_myso.amount();
        scenario.return_to_sender(staked_myso);
        sum
    });
