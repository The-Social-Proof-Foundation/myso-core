...
fun create(
    ...
    let subsidy_fund = myso_supply.split(stake_subsidy_fund_mist);
    let storage_fund = balance::zero();

    // Allocate tokens and staking operations
    allocate_tokens(myso_supply, allocations, &mut validators, ctx);
...
fun allocate_tokens(
    ...
    allocations.destroy!(
        |TokenAllocation { recipient_address, amount_mist, staked_with_validator }| {
            let allocation_balance = myso_supply.split(amount_mist);
