...
#[test_only]
module myso_system::validator_set_tests;
...
use myso::vec_map;
use myso_system::validator::{Self, Validator, staking_pool_id};
use myso_system::validator_set::{Self, ValidatorSet};

const MIST_PER_MYSO: u64 = 1_000_000_000; // used internally for stakes.
...
fun validator_set_flow() {
    ...
    assert_eq!(validator_set.total_stake(), 100 * MIST_PER_MYSO);
    ...
    let scenario = &mut scenario_val;
    {
        ...
        let stake = validator_set.request_add_stake(
            @0x1,
            coin::mint_for_testing(500 * MIST_PER_MYSO, ctx1).into_balance(),
            ctx1,
        );
    ...
    assert_eq!(validator_set.total_stake(), 1500 * MIST_PER_MYSO);
    ...
    assert_eq!(validator_set.total_stake(), 900 * MIST_PER_MYSO);
...
fun staking_below_threshold() {
    ...
    let stake = validator_set.request_add_stake(
        @0x1,
        balance::create_for_testing(MIST_PER_MYSO - 1), // 1 MIST lower than the threshold
        ctx1,
    );
    transfer::public_transfer(stake, @0x1);
    destroy(validator_set);
...
fun staking_min_threshold() {
    ...
    let stake = validator_set.request_add_stake(
        @0x1,
        balance::create_for_testing(MIST_PER_MYSO), // min possible stake
        ctx1,
    );
    transfer::public_transfer(stake, @0x1);

    advance_epoch_with_dummy_rewards(&mut validator_set, scenario);
    assert!(validator_set.total_stake() == 101 * MIST_PER_MYSO);
...
fun add_validator_with_min_voting_power() {
    ...
    assert!(validator_set.total_stake() == 10_000 * MIST_PER_MYSO);
...
fun request_add_then_pull_stake() {
    ...
    let stake = validator_set.request_add_stake(
        @0xA,
        balance::create_for_testing(3 * MIST_PER_MYSO),
        scenario.ctx(),
    );
...
fun withdraw_all() {
    ...
    let stake = validator_set.request_add_stake(
        @0xB,
        balance::create_for_testing(4 * MIST_PER_MYSO),
        scenario.ctx(),
    );
    validator_set.request_add_validator(scenario.ctx()); // can be admitted
    advance_epoch_with_dummy_rewards(&mut validator_set, scenario);
...
fun very_low_voting_power_departure() {
    ...
    let bal = validator_set.request_withdraw_stake(
        stake.split(3 * MIST_PER_MYSO, scenario.ctx()),
        scenario.ctx(),
    );
    advance_epoch_with_low_stake_grace_period(&mut validator_set, grace_period, scenario);
    assert!(!validator_set.is_active_validator(@0xB));
...
fun low_voting_power_departure() {
    ...
    let bal = validator_set.request_withdraw_stake(
        stake.split(2 * MIST_PER_MYSO, scenario.ctx()),
        scenario.ctx(),
    );
    advance_epoch_with_low_stake_grace_period(&mut validator_set, grace_period, scenario);
    assert!(validator_set.is_active_validator(@0xB));
...
fun low_voting_power_recovery() {
    ...
    // withdraw part of the stake. validator will now have voting power 2 and is now at risk
    let bal = validator_set.request_withdraw_stake(
        stake1.split(2 * MIST_PER_MYSO, scenario.ctx()),
        scenario.ctx(),
    );
    advance_epoch_with_low_stake_grace_period(&mut validator_set, grace_period, scenario);
    assert!(validator_set.is_active_validator(@0xB));
...
fun add_then_increase_stake_of_others() {
    ...
    let stake = validator_set.request_add_stake(
        @0xB,
        balance::create_for_testing(1000 * MIST_PER_MYSO),
        scenario.ctx(),
    );
    destroy(stake);
    validator_set.request_add_validator(scenario.ctx()); // can be admitted
    ...
    num_validators.do!(|i| {
        let to_add = 10_000_000 * MIST_PER_MYSO;
        let stake = validator_set.request_add_stake(
...
fun create_validator(
    ...
    let stake_value = hint as u64 * 100 * MIST_PER_MYSO;
...
fun create_validator_with_initial_stake(
    ...
    let validator = validator::new_for_testing(
        ...
        vector[hint],
        if (initial_stake != 0) {
            option::some(balance::create_for_testing(initial_stake * MIST_PER_MYSO))
        } else { option::none() },
        1,
        0,
        is_initial_validator,
...
fun add_and_activate_validator(
    ...
    scenario.next_tx(validator.myso_address());
    let ctx = scenario.ctx();
