...
#[test_only]
#[deprecated(note = b"Use myso_system::test_runner instead")]
module myso_system::governance_test_utils;
...
use myso_system::staking_pool::{StakedMySo, StakingPool};
...
public fun create_validator_for_testing(
    ...
    let validator = validator::new_for_testing(
        ...
        b"/ip4/127.0.0.1/udp/80",
        b"/ip4/127.0.0.1/udp/80",
        option::some(balance::create_for_testing<MYSO>(init_stake_amount_in_myso * MIST_PER_MYSO)),
        1,
        0,
        true,
        ctx,
...
public fun create_myso_system_state_for_testing(
    ...
    let stake_subsidy = stake_subsidy::create(
        balance::create_for_testing<MYSO>(myso_supply_amount * MIST_PER_MYSO), // myso_supply
    ...
    myso_system::create(
        object::new(ctx), // it doesn't matter what ID myso system state has in tests
        validators,
        balance::create_for_testing<MYSO>(storage_fund_amount * MIST_PER_MYSO), // storage_fund
        1, // protocol version
...
public fun set_up_myso_system_state(mut addrs: vector<address>) {
    ...
    create_myso_system_state_for_testing(validators, 1000, 0, ctx);
    scenario.end();
}
...
public fun advance_epoch_with_reward_amounts(
    ...
    let storage_rebate = advance_epoch_with_reward_amounts_return_rebate(
        storage_charge * MIST_PER_MYSO,
        computation_charge * MIST_PER_MYSO,
        0,
        0,
        scenario,
    );
...
public fun unstake(staker: address, staked_myso_idx: u64, scenario: &mut Scenario) {
    scenario.next_tx(staker);
    let stake_myso_ids = scenario.ids_for_sender<StakedMySo>();
    let staked_myso = scenario.take_from_sender_by_id(stake_myso_ids[staked_myso_idx]);
    let mut system_state = scenario.take_shared<MySoSystemState>();

    let ctx = scenario.ctx();
    system_state.request_withdraw_stake(staked_myso, ctx);
    test_scenario::return_shared(system_state);
}
...
public fun add_validator_full_flow(
    ...
    system_state.request_add_stake(
        coin::mint_for_testing<MYSO>(init_stake_amount * MIST_PER_MYSO, ctx),
        validator,
        ctx,
    );
...
/// Return the rewards for the validator at `addr` in terms of MYSO.
public fun stake_plus_current_rewards_for_validator(
    addr: address,
    system_state: &mut MySoSystemState,
    scenario: &mut Scenario,
): u64 {
...
public fun stake_plus_current_rewards(
    ...
    let mut stake_ids = scenario.ids_for_sender<StakedMySo>();
    let current_epoch = scenario.ctx().epoch();

    while (!stake_ids.is_empty()) {
        let staked_myso_id = stake_ids.pop_back();
        let staked_myso = scenario.take_from_sender_by_id<StakedMySo>(staked_myso_id);
        sum =
            sum + staked_myso.amount() + staking_pool.calculate_rewards(&staked_myso, current_epoch);
        scenario.return_to_sender(staked_myso);
    };
    sum
}

public fun total_myso_balance(addr: address, scenario: &mut Scenario): u64 {
    let mut sum = 0;
    scenario.next_tx(addr);
    let coin_ids = scenario.ids_for_sender<Coin<MYSO>>();
    let mut i = 0;
    while (i < coin_ids.length()) {
        let coin = scenario.take_from_sender_by_id<Coin<MYSO>>(coin_ids[i]);
        sum = sum + coin.value();
