...
/// `MySoSystemState` is a thin wrapper around `MySoSystemStateInner` that provides a versioned interface.
/// The `MySoSystemState` object has a fixed ID 0x5, and the `MySoSystemStateInner` object is stored as a dynamic field.
/// There are a few different ways to upgrade the `MySoSystemStateInner` type:
///
/// The simplest and one that doesn't involve a real upgrade is to just add dynamic fields to the `extra_fields` field
/// of `MySoSystemStateInner` or any of its sub type. This is useful when we are in a rush, or making a small change,
/// or still experimenting a new field.
///
/// To properly upgrade the `MySoSystemStateInner` type, we need to ship a new framework that does the following:
/// 1. Define a new `MySoSystemStateInner`type (e.g. `MySoSystemStateInnerV2`).
/// 2. Define a data migration function that migrates the old `MySoSystemStateInner` to the new one (i.e. MySoSystemStateInnerV2).
/// 3. Replace all uses of `MySoSystemStateInner` with `MySoSystemStateInnerV2` in both myso_system.move and myso_system_state_inner.move,
///    with the exception of the `myso_system_state_inner::create` function, which should always return the genesis type.
...
/// 1. Define a new `MySoSystemStateInner` struct type that matches the new Move type, and implement the MySoSystemStateTrait.
/// 2. Update the `MySoSystemState` struct to include the new version as a new enum variant.
/// 3. Update the `get_myso_system_state` function to handle the new version.
/// To test that the upgrade will be successful, we need to modify `myso_system_state_production_upgrade_test` test in
...
use myso_system::stake_subsidy::StakeSubsidy;
use myso_system::staking_pool::{StakedMySo, FungibleStakedMySo, PoolTokenExchangeRate};
...
use myso_system::validator_cap::UnverifiedValidatorOperationCap;

#[test_only]
use myso::balance;
#[test_only]
use myso_system::validator_set::ValidatorSet;
#[test_only]
...
/// Note: `proof_of_possession` MUST be a valid signature using myso_address and protocol_pubkey_bytes.
...
public entry fun request_add_validator(wrapper: &mut MySoSystemState, ctx: &mut TxContext) {
...
/// At the end of the epoch, the `validator` object will be returned to the myso_address
/// of the validator.
public entry fun request_remove_validator(wrapper: &mut MySoSystemState, ctx: &mut TxContext) {
...
public entry fun request_add_stake(
    ...
    let staked_myso = request_add_stake_non_entry(wrapper, stake, validator_address, ctx);
    transfer::public_transfer(staked_myso, ctx.sender());
}

/// The non-entry version of `request_add_stake`, which returns the staked MYSO instead of transferring it to the sender.
...
public entry fun request_withdraw_stake(
    ...
    let withdrawn_stake = wrapper.request_withdraw_stake_non_entry(staked_myso, ctx);
    transfer::public_transfer(withdrawn_stake.into_coin(ctx), ctx.sender());
}

/// Convert StakedMySo into a FungibleStakedMySo object.
public fun convert_to_fungible_staked_myso(
    wrapper: &mut MySoSystemState,
    staked_myso: StakedMySo,
    ctx: &mut TxContext,
): FungibleStakedMySo {
    wrapper.load_system_state_mut().convert_to_fungible_staked_myso(staked_myso, ctx)
}

/// Convert FungibleStakedMySo into a StakedMySo object.
public fun redeem_fungible_staked_myso(
    wrapper: &mut MySoSystemState,
    fungible_staked_myso: FungibleStakedMySo,
    ctx: &TxContext,
): Balance<MYSO> {
    wrapper.load_system_state_mut().redeem_fungible_staked_myso(fungible_staked_myso, ctx)
}

/// Non-entry version of `request_withdraw_stake` that returns the withdrawn MYSO instead of transferring it to the sender.
public fun request_withdraw_stake_non_entry(
    wrapper: &mut MySoSystemState,
    staked_myso: StakedMySo,
    ctx: &mut TxContext,
): Balance<MYSO> {
    wrapper.load_system_state_mut().request_withdraw_stake(staked_myso, ctx)
}
...
public entry fun rotate_operation_cap(self: &mut MySoSystemState, ctx: &mut TxContext) {
...
public fun validator_address_by_pool_id(wrapper: &mut MySoSystemState, pool_id: &ID): address {
...
public fun active_validator_addresses_ref(wrapper: &MySoSystemState): vector<address> {
...
public fun active_validator_voting_powers(wrapper: &MySoSystemState): VecMap<address, u64> {
...
public(package) fun calculate_rewards(
    ...
    system_state
        ...
        .calculate_rewards(staked_myso, ctx.epoch())
}
...
fun load_system_state(self: &mut MySoSystemState): &MySoSystemStateInnerV2 {
...
fun load_system_state_ref(self: &MySoSystemState): &MySoSystemStateInnerV2 {
    let inner: &MySoSystemStateInnerV2 = dynamic_field::borrow(
        &self.id,
        self.version,
    );
...
fun load_inner_maybe_upgrade(self: &mut MySoSystemState): &mut MySoSystemStateInnerV2 {
    if (self.version == 1) {
        let v1: MySoSystemStateInner = dynamic_field::remove(&mut self.id, self.version);
        let v2 = v1.v1_to_v2();
        self.version = 2;
        dynamic_field::add(&mut self.id, self.version, v2);
    };

    let inner: &mut MySoSystemStateInnerV2 = dynamic_field::borrow_mut(
        &mut self.id,
        self.version,
    );
    assert!(inner.system_state_version() == self.version, EWrongInnerVersion);
...
fun validator_voting_powers(wrapper: &mut MySoSystemState): VecMap<address, u64> {
...
fun get_accumulator_storage_fund_amount(wrapper: &mut MySoSystemState): u64 {
...
#[test_only]
public fun validator_voting_powers_for_testing(wrapper: &mut MySoSystemState): VecMap<address, u64> {
...
public fun epoch(wrapper: &mut MySoSystemState): u64 {
    wrapper.load_system_state_mut().epoch()
}

#[test_only]
/// Returns unix timestamp of the start of current epoch
public fun epoch_start_timestamp_ms(wrapper: &mut MySoSystemState): u64 {
...
public fun validator_stake_amount(wrapper: &mut MySoSystemState, validator_addr: address): u64 {
...
/// Aborts if `validator_addr` is not an active validator.
public fun validator_staking_pool_id(wrapper: &mut MySoSystemState, validator_addr: address): ID {
    wrapper.load_system_state_mut().validator_staking_pool_id(validator_addr)
}

#[test_only]
/// Returns reference to the staking pool mappings that map pool ids to active validator addresses
public fun validator_staking_pool_mappings(wrapper: &mut MySoSystemState): &Table<ID, address> {
    wrapper.load_system_state_mut().validator_staking_pool_mappings()
}

#[test_only]
/// Returns all the validators who are currently reporting `addr`
public fun get_reporters_of(wrapper: &mut MySoSystemState, addr: address): VecSet<address> {
    wrapper.load_system_state_mut().get_reporters_of(addr)
}

#[test_only]
/// Return the current validator set
public fun validators(wrapper: &mut MySoSystemState): &ValidatorSet {
    wrapper.load_system_state_mut().validators()
}

#[test_only]
/// Return a mutable reference to the validator set
public fun validators_mut(wrapper: &mut MySoSystemState): &mut ValidatorSet {
...
#[test_only]
public fun set_epoch_for_testing(wrapper: &mut MySoSystemState, epoch_num: u64) {
    wrapper.load_system_state_mut().set_epoch_for_testing(epoch_num)
}

#[test_only]
public fun request_add_validator_for_testing(wrapper: &mut MySoSystemState, ctx: &TxContext) {
    wrapper.load_system_state_mut().request_add_validator(ctx)
}

#[test_only]
public fun get_storage_fund_total_balance(wrapper: &mut MySoSystemState): u64 {
    wrapper.load_system_state_mut().get_storage_fund_total_balance()
}

#[test_only]
public fun get_storage_fund_object_rebates(wrapper: &mut MySoSystemState): u64 {
    wrapper.load_system_state_mut().get_storage_fund_object_rebates()
}

#[test_only]
public fun get_stake_subsidy_distribution_counter(wrapper: &mut MySoSystemState): u64 {
    wrapper.load_system_state_mut().get_stake_subsidy_distribution_counter()
}

#[test_only]
public fun set_stake_subsidy_distribution_counter(wrapper: &mut MySoSystemState, counter: u64) {
    wrapper.load_system_state_mut().set_stake_subsidy_distribution_counter(counter)
}

#[test_only]
public fun inner_mut_for_testing(wrapper: &mut MySoSystemState): &mut MySoSystemStateInnerV2 {
