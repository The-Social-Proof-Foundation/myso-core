...
// This file contains tests testing functionalities in `myso_system` that are not
// already tested by the other more themed tests such as `stake_tests` or
// `rewards_distribution_tests`.

#[test_only]
module myso_system::myso_system_tests;
...
fun report_validator() {
    let mut runner = test_runner::new()
        .validators(vector[
            validator_builder::new().myso_address(@1),
            validator_builder::new().myso_address(@2),
            validator_builder::new().myso_address(@3),
        ])
        .build();
...
fun report_validator_by_stakee_ok(initial_stake: u8) {
    ...
    let new_validator = validator.myso_address();
...
#[test, expected_failure(abort_code = ::myso_system::validator_set::EInvalidCap)]
...
#[test, expected_failure(abort_code = myso_system::validator::EGasPriceHigherThanThreshold)]
fun set_gas_price_failure() {
    let validator = validator_builder::new().myso_address(@1);
...
#[test, expected_failure(abort_code = myso_system::validator::ECommissionRateTooHigh)]
...
#[test, expected_failure(abort_code = myso_system::myso_system_state_inner::ENotValidator)]
...
#[test, expected_failure(abort_code = myso_system::myso_system_state_inner::EReportRecordNotFound)]
...
#[test, expected_failure(abort_code = myso_system::myso_system_state_inner::ECannotReportOneself)]
...
fun staking_pool_mappings() {
    let mut runner = test_runner::new()
        .validators(vector[
            ...
            validator_builder::new().myso_address(@2),
            validator_builder::new().myso_address(@3),
            validator_builder::new().myso_address(@4),
        ])
        .build();
...
#[random_test]
// Stake random amount of MYSO and check that the pending and withdraw amounts are correct.
fun withdraw_inactive_stake(stake: u16) {
    let stake_amount = stake as u64;
    let validator = validator_builder::new().myso_address(@1).initial_stake(100);
    let mut runner = test_runner::new().validators(vector[validator]).build();
    ...
    // Check that pending stake amount is 1 MYSO.
    runner.system_tx!(|system, _| {
        let pool = system.active_validator_by_address(@1).get_staking_pool_ref();
        assert_eq!(pool.pending_stake_amount(), stake_amount * MIST_PER_MYSO);
        assert_eq!(pool.pending_stake_withdraw_amount(), 0);
        assert_eq!(pool.myso_balance(), 100 * MIST_PER_MYSO);
    });
...
#[random_test]
// Stake random amount of MYSO and check that the pending stake amount is correct.
// Convert to fungible staked MYSO and redeem it.
// Check that the stake amount is correct.
fun convert_to_fungible_staked_myso_and_redeem(stake: u16) {
    let stake_amount = stake as u64;
    let validator = validator_builder::new().myso_address(@1).initial_stake(100);
    let mut runner = test_runner::new().validators(vector[validator]).build();
    ...
    let staked_myso = runner.set_sender(@5).stake_with_and_take(@1, stake_amount);

    assert_eq!(staked_myso.amount(), stake_amount * MIST_PER_MYSO);
    ...
    runner.system_tx!(|system, _| {
        ...
        assert_eq!(pool.pending_stake_amount(), 0);
        assert_eq!(pool.pending_stake_withdraw_amount(), 0);
        assert_eq!(pool.myso_balance(), (100 + stake_amount) * MIST_PER_MYSO);
    });

    // Convert to fungible staked MYSO.
    let fungible_staked_myso;
    runner.system_tx!(|system, ctx| {
        fungible_staked_myso = system.convert_to_fungible_staked_myso(staked_myso, ctx);
    });

    assert_eq!(fungible_staked_myso.value(), stake_amount * MIST_PER_MYSO);

    let myso;
    runner.system_tx!(|system, ctx| {
        myso = system.redeem_fungible_staked_myso(fungible_staked_myso, ctx);
    });

    assert_eq!(myso.destroy_for_testing(), stake_amount * MIST_PER_MYSO);
