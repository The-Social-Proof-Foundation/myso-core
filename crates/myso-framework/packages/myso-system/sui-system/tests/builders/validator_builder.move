...
module myso_system::validator_builder;

use myso::bag;
use myso::balance;
use myso::myso::MYSO;
use myso::url;
use myso_system::validator::{Self, Validator, ValidatorMetadata};
...
public fun preset(): ValidatorBuilder {
    ValidatorBuilder {
        myso_address: option::some(VALID_ADDRESS),
...
public fun build(builder: ValidatorBuilder, ctx: &mut TxContext): Validator {
    ...
    validator::new_for_testing(
        myso_address.destroy_or!(ctx.fresh_object_address()),
        ...
        initial_stake.map!(|amount| balance::create_for_testing<MYSO>(amount * 1_000_000_000)),
...
public fun build_metadata(builder: ValidatorBuilder, ctx: &mut TxContext): ValidatorMetadata {
    ...
    validator::new_metadata(
        myso_address.destroy_or!(VALID_ADDRESS),
...
public fun myso_address(mut builder: ValidatorBuilder, myso_address: address): ValidatorBuilder {
    builder.myso_address = option::some(myso_address);
    builder
}
...
public fun valid_myso_address(): address { VALID_ADDRESS }
