...
#[test_only]
module myso::package_tests;

use std::unit_test::{assert_eq, assert_ref_eq};
use myso::package::{Self, UpgradeCap, UpgradeTicket};
...
fun test_upgrade_policy_reflected_in_ticket() {
    ...
    while (!policies.is_empty()) {
        ...
        let ticket = check_ticket(&mut cap, policy, myso::hash::blake2b256(&vector[policy]));
...
fun test_full_upgrade_flow() {
    ...
    let ticket = cap.authorize_upgrade(
        package::dep_only_policy(),
        myso::hash::blake2b256(&b"package contents"),
    );
...
#[test, expected_failure(abort_code = myso::package::ETooPermissive)]
...
#[test, expected_failure(abort_code = myso::package::EAlreadyAuthorized)]
fun test_failure_to_authorize_multiple_upgrades() {
    ...
    let _ticket0 = cap.authorize_upgrade(
        package::compatible_policy(),
        myso::hash::blake2b256(&b"package contents 0"),
    );
    ...
    let _ticket1 = cap.authorize_upgrade(
        package::compatible_policy(),
        myso::hash::blake2b256(&b"package contents 1"),
    );

    abort
}

#[test, expected_failure(abort_code = myso::package::EWrongUpgradeCap)]
