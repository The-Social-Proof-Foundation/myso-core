...
fun type_morphing() {
    ...
    let balance = balance::zero<MYSO>();
    let coin = balance.into_coin(scenario.ctx());
    let balance = coin.into_balance();

    balance.destroy_zero();

    let mut coin = coin::mint_for_testing<MYSO>(100, scenario.ctx());
...
#[test, expected_failure(abort_code = myso::balance::EOverflow)]
...
#[test]
fun test_balance() {
    let mut balance = balance::zero<MYSO>();
