...
#[test_only]
module myso::dynamic_field_tests;

use myso::dynamic_field::{add, exists_with_type, borrow, borrow_mut, remove};
...
#[test]
#[expected_failure(abort_code = myso::dynamic_field::EFieldAlreadyExists)]
fun add_duplicate() {
    let sender = @0x0;
    let mut scenario = test_scenario::begin(sender);
    let mut id = scenario.new_object();
...
#[test]
#[expected_failure(abort_code = myso::dynamic_field::EFieldDoesNotExist)]
fun borrow_missing() {
    let sender = @0x0;
    let mut scenario = test_scenario::begin(sender);
...
#[test]
#[expected_failure(abort_code = myso::dynamic_field::EFieldTypeMismatch)]
