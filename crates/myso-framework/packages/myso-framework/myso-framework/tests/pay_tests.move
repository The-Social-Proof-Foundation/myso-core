...
fun test_coin_split_n() {
    ...
    let mut coin = coin::mint_for_testing<MYSO>(10, ctx);

    scenario.next_tx(TEST_SENDER_ADDR);
    coin.divide_and_keep(3, scenario.ctx());

    scenario.next_tx(TEST_SENDER_ADDR);
    let coin1 = scenario.take_from_sender<Coin<MYSO>>();

    scenario.next_tx(TEST_SENDER_ADDR);
    let coin2 = scenario.take_from_sender<Coin<MYSO>>();

    scenario.next_tx(TEST_SENDER_ADDR);
    assert_eq!(coin1.value(), 3);
    assert_eq!(coin2.value(), 3);
    assert_eq!(coin.value(), 4);
    assert!(!scenario.has_most_recent_for_sender<Coin<MYSO>>(), 1);
...
fun test_split_and_transfer_fail() {
    ...
    scenario.next_tx(TEST_SENDER_ADDR);
    let coin_transfer_fail = scenario.take_from_sender<Coin<MYSO>>();
