...
/// Similar to `myso::table`, an `ObjectTable<K, V>` is a map-like collection. But unlike
/// `myso::table`, the values bound to these dynamic fields _must_ be objects themselves. This allows
...
/// Aborts with `myso::dynamic_field::EFieldAlreadyExists` if the table already has an entry with
/// that key `k: K`.
...
/// Aborts with `myso::dynamic_field::EFieldDoesNotExist` if the table does not have an entry with
/// that key `k: K`.
