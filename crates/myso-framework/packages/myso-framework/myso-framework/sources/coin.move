...
public use fun myso::pay::split_vec as Coin.split_vec;

// Allows calling `.join_vec(coins)` on `coin`
public use fun myso::pay::join_vec as Coin.join_vec;

// Allows calling `.split_and_transfer(amount, recipient, ctx)` on `coin`
public use fun myso::pay::split_and_transfer as Coin.split_and_transfer;

// Allows calling `.divide_and_keep(n, ctx)` on `coin`
public use fun myso::pay::divide_and_keep as Coin.divide_and_keep;
...
public fun redeem_funds<T>(
    withdrawal: myso::funds_accumulator::Withdrawal<Balance<T>>,
    ctx: &mut TxContext,
): Coin<T> {
...
/// The index into the deny list vector for the `myso::coin::Coin` type.
