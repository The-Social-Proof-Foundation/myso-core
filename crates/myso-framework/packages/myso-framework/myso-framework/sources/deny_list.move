...
fun add_per_type_config(
    ...
    myso::event::emit(PerTypeConfigCreated { key, config_id });
}
...
public struct PerTypeList has key, store {
    ...
    /// For example with `myso::coin::Coin`: If addresses A and B are banned from using
...
fun create(ctx: &mut TxContext) {
    ...
    let deny_list_object = DenyList {
        id: object::myso_deny_list_object_id(),
        lists,
    };
