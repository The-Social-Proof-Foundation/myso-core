...
/// A helper `macro` that calls `myso::transfer::party_transfer`.
public macro fun transfer<$T: key>($self: Party, $obj: $T) {
    let mp = $self;
    myso::transfer::party_transfer($obj, mp)
}

/// A helper `macro` that calls `myso::transfer::public_party_transfer`.
public macro fun public_transfer<$T: key + store>($self: Party, $obj: $T) {
    let mp = $self;
    myso::transfer::public_party_transfer($obj, mp)
}
