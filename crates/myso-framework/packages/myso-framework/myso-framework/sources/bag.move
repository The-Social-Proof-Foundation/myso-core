...
/// A bag is a heterogeneous map-like collection. The collection is similar to `myso::table` in that
/// its keys and values are not stored within the `Bag` value, but instead are stored using MySo's
...
/// assert!(&bag1 != &bag2);
/// ```
/// At it's core, `myso::bag` is a wrapper around `UID` that allows for access to
/// `myso::dynamic_field` while preventing accidentally stranding field values. A `UID` can be
/// deleted, even if it has dynamic fields associated with it, but a bag, on the other hand, must be
/// empty to be destroyed.
module myso::bag;

use myso::dynamic_field as field;
...
/// Aborts with `myso::dynamic_field::EFieldAlreadyExists` if the bag already has an entry with
/// that key `k: K`.
...
/// Aborts with `myso::dynamic_field::EFieldDoesNotExist` if the bag does not have an entry with
/// that key `k: K`.
/// Aborts with `myso::dynamic_field::EFieldTypeMismatch` if the bag has an entry for the key, but
