...
/// Utility for mocking a multi-transaction MySo execution in a single Move procedure.
...
public fun create_system_objects(scenario: &mut Scenario) {
    ...
    myso::clock::create_for_testing(scenario.ctx()).share_for_testing();
    myso::random::create_for_testing(scenario.ctx());
    myso::deny_list::create_for_testing(scenario.ctx());
...
public fun return_to_address<T: key>(account: address, t: T) {
    ...
    myso::transfer::transfer_impl(t, account)
}
...
public fun return_immutable<T: key>(t: T) {
    ...
    myso::transfer::freeze_object_impl(t)
}
...
public fun return_shared<T: key>(t: T) {
    ...
    myso::transfer::share_object_impl(t)
}
...
public fun most_recent_receiving_ticket<T: key>(owner: &ID): myso::transfer::Receiving<T> {
...
public fun receiving_ticket_by_id<T: key>(object_id: ID): myso::transfer::Receiving<T> {
    let version = allocate_receiving_ticket_for_object<T>(object_id);
    myso::transfer::make_receiver(object_id, version)
}
...
/// test scenario.
public fun return_receiving_ticket<T: key>(ticket: myso::transfer::Receiving<T>) {
    let id = myso::transfer::receiving_id(&ticket);
