...
/// `list_with_purchase_cap` therefore performing a trade (ismysong a
...
entry fun default(ctx: &mut TxContext) {
    ...
    myso::transfer::transfer(cap, ctx.sender());
    myso::transfer::share_object(kiosk);
}
...
public fun close_and_withdraw(self: Kiosk, cap: KioskOwnerCap, ctx: &mut TxContext): Coin<MYSO> {
...
public fun profits_mut(self: &mut Kiosk, cap: &KioskOwnerCap): &mut Balance<MYSO> {
    assert!(self.has_access(cap), ENotOwner);
    &mut self.profits
}
