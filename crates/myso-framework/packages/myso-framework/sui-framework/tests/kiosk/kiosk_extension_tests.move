...
#[test_only]
module myso::kiosk_marketplace_ext {
    use myso::coin::Coin;
    use myso::kiosk::{Self, KioskOwnerCap, Kiosk, PurchaseCap};
    use myso::kiosk_extension as ext;
    use myso::myso::MYSO;
    use myso::transfer_policy::{Self as policy, TransferPolicy, TransferRequest};

    /// Trying to access an owner-only action.
    const ENotOwner: u64 = 0;
    /// Trying to purchase an item with an incorrect amount of MYSO.
    ...
    /// Collection bidding: the Kiosk Owner offers a bid (in MYSO) for an item of type `T`.
    ///
    /// There can be only one bid per type.
    public fun bid<Market, T: key + store>(kiosk: &mut Kiosk, cap: &KioskOwnerCap, bid: Coin<MYSO>) {
    ...
    public fun accept_bid<Market, T: key + store>(
        ...
        let bid: Coin<MYSO> = ext::storage_mut(Ext<Market> {}, destination).remove(Bid<T> {});
...
#[test_only]
module myso::kiosk_extensions_tests {
    use myso::kiosk_extension as ext;
    use myso::kiosk_test_utils as test;
    ...
    #[test, expected_failure(abort_code = myso::kiosk_extension::EExtensionNotAllowed)]
    ...
    #[test, expected_failure(abort_code = myso::kiosk_extension::EExtensionNotInstalled)]
