...
fun test_bridge_and_claim() {
    ...
    //
    // move from eth and transfer to myso account
    let transfer_id1 = env.bridge_to_myso<ETH>(
        source_chain,
        eth_address,
        myso_address,
    ...
    let transfer_id = env.bridge_to_myso<ETH>(
        source_chain,
        eth_address,
        myso_address,
        amount,
    );
    let token = env.claim_token<ETH>(myso_address, source_chain, transfer_id);
    env.send_token<ETH>(
        myso_address,
        source_chain,
        eth_address,
