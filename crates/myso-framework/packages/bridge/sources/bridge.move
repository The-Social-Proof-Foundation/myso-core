...
use myso::versioned::{Self, Versioned};
...
const EInvariantMySoInitializedTokenTransferShouldNotBeClaimed: u64 = 10;
...
// Record bridge message approvals in MySo, called by the bridge client
...
public fun approve_token_transfer(
    ...
    // retrieve pending message if source chain is MySo, the initial message
    // must exist on chain
...
public fun execute_system_message(
    ...
    if (message_type == message_types::emergency_op()) {
        ...
        let payload = message.extract_add_tokens_on_myso();
        inner.execute_add_tokens_on_myso(payload);
    } else {
...
fun execute_add_tokens_on_myso(inner: &mut BridgeInner, payload: AddTokenOnMySo) {
...
#[test_only]
public fun test_execute_add_tokens_on_myso(bridge: &mut Bridge, payload: AddTokenOnMySo) {
    let inner = load_inner_mut(bridge);
    inner.execute_add_tokens_on_myso(payload);
}
