...
```json
"repository": {
  "url": "http://[your_token]@github.com/mystenlabs/myso.git",
  "commit": "main"
}
```
...
The `myso-aws-orchestrator` binary provides various functionalities for creating, starting, stopping, and destroying instances. You can use the following command to boot 2 instances per region (if the settings file specifies 10 regions, as shown in the example above, a total of 20 instances will be created):

```bash
cargo run --bin myso-aws-orchestrator -- testbed deploy --instances 2
```

To check the current status of the testbed instances, use the following command:

```bash
cargo run --bin myso-aws-orchestrator testbed status
```
...
```
// MySo
use protocol::myso::{MySoBenchmarkType, MySoProtocol};
type Protocol = MySoProtocol;
type BenchmarkType = MySoBenchmarkType;
// Narwhal
...
```bash
cargo run --bin myso-aws-orchestrator -- benchmark --committee 10 fixed-load --loads 200 --duration 180
```

In a network of 10 validators, each with a corresponding load generator, each load generator submits a fixed load of 20 tx/s. Performance measurements are collected by regularly scraping the Prometheus metrics exposed by the load generators. The `myso-aws-orchestrator` binary provides additional commands to run a specific number of load generators on separate machines.
...
```
cargo run --bin myso-aws-orchestrator -- testbed destroy
```
...
### I am getting an error "Failed to read settings file '"crates/myso-aws-orchestrator/assets/settings.json"': No such file or directory"

To run the tool a `settings.json` file with the deployment configuration should be under the directory `crates/myso-aws-orchestrator/assets`. Also, please make sure
that you run the orchestrator from the top level repo folder, ex `/myso $ cargo run --bin myso-aws-orchestrator`
...
for example when running the command `cargo run --bin myso-aws-orchestrator -- benchmark --committee 4 fixed-load --loads 500 --duration 500`, we'll need the following amount of instances available:
