...
use myso_storage::object_store::ObjectStoreListExt;
...
async fn test_archive_epoch_if_needed() -> Result<(), anyhow::Error> {
    ...
    let chain_identifier = myso_types::digests::get_testnet_chain_identifier();
    ...
    for (filename, content) in &test_files {
        ...
        myso_storage::object_store::util::put(&store, &file_path, bytes::Bytes::from(*content))
            .await?;
    }
    ...
    for (filename, expected_content) in &test_files {
        ...
        let archived_content = myso_storage::object_store::util::get(&store, &archive_path).await?;
