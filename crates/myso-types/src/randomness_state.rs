...
use crate::{
    MYSO_FRAMEWORK_ADDRESS, MYSO_RANDOMNESS_STATE_OBJECT_ID, error::MySoResult, object::Owner,
    storage::ObjectStore,
};
...
pub const RESOLVED_MYSO_RANDOMNESS_STATE: (&AccountAddress, &IdentStr, &IdentStr) = (
...
pub fn is_mutable_random(view: &CompiledModule, s: &SignatureToken) -> bool {
    match s {
        ...
        SignatureToken::Datatype(idx) => {
            resolve_struct(view, *idx) == RESOLVED_MYSO_RANDOMNESS_STATE
        }
        _ => false,
    }
}
