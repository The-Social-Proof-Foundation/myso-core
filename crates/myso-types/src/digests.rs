...
impl TryFrom<Vec<u8>> for Digest {
    ...
    fn try_from(bytes: Vec<u8>) -> Result<Self, MySoError> {
        let bytes: [u8; 32] =
            <[u8; 32]>::try_from(&bytes[..]).map_err(|_| MySoErrorKind::InvalidDigestLength {
                expected: 32,
                actual: bytes.len(),
            })?;
...
pub const MYSO_PROTOCOL_CONFIG_CHAIN_OVERRIDE_ENV_VAR_NAME: &str =
    "MYSO_PROTOCOL_CONFIG_CHAIN_OVERRIDE";

static MYSO_PROTOCOL_CONFIG_CHAIN_OVERRIDE: Lazy<Option<Chain>> = Lazy::new(|| {
    if let Ok(s) = env::var(MYSO_PROTOCOL_CONFIG_CHAIN_OVERRIDE_ENV_VAR_NAME) {
        info!("MYSO_PROTOCOL_CONFIG_CHAIN_OVERRIDE: {:?}", s);
        match s.as_str() {
            "mainnet" => Some(Chain::Mainnet),
            "testnet" => Some(Chain::Testnet),
            "" => None,
            _ => panic!("unrecognized MYSO_PROTOCOL_CONFIG_CHAIN_OVERRIDE: {s:?}"),
        }
    } else {
        None
    }
...
impl ChainIdentifier {
    ...
    pub fn chain(&self) -> Chain {
        ...
        if let Some(override_chain) = *MYSO_PROTOCOL_CONFIG_CHAIN_OVERRIDE {
...
impl TransactionDigest {
    ...
    // TODO(https://github.com/the-social-proof-foundation/myso-core/issues/65): we can pick anything here
...
impl TryFrom<&[u8]> for TransactionDigest {
    ...
    fn try_from(bytes: &[u8]) -> Result<Self, crate::error::MySoError> {
        let arr: [u8; 32] = bytes
            .try_into()
            .map_err(|_| crate::error::MySoErrorKind::InvalidTransactionDigest)?;
        Ok(Self::new(arr))
    }
}
...
impl CheckpointArtifactsDigest {
    ...
    pub fn from_artifact_digests(digests: Vec<Digest>) -> MySoResult<Self> {
        let bytes =
            bcs::to_bytes(&digests).map_err(|e| MySoError::from(format!("BCS error: {}", e)))?;
...
#[cfg(test)]
mod test {
    use crate::digests::{ChainIdentifier, MYSO_PROTOCOL_CONFIG_CHAIN_OVERRIDE};

    fn has_env_override() -> bool {
        MYSO_PROTOCOL_CONFIG_CHAIN_OVERRIDE.is_some()
    }
    ...
    fn test_chain_id_testnet() {
        ...
        assert_eq!(
            chain_id.unwrap().chain(),
            myso_protocol_config::Chain::Testnet
        );
    }
