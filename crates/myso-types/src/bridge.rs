...
pub const BRIDGE_CREATE_ADD_TOKEN_ON_MYSO_MESSAGE_FUNCTION_NAME: &IdentStr =
    ident_str!("create_add_tokens_on_myso_message");
...
impl BridgeChainId {
    pub fn is_myso_chain(&self) -> bool {
        matches!(
            self,
            BridgeChainId::MySoMainnet | BridgeChainId::MySoTestnet | BridgeChainId::MySoCustom
        )
    }
}
...
pub fn get_bridge_obj_initial_shared_version(
    ...
    Ok(object_store
        .get_object(&MYSO_BRIDGE_OBJECT_ID)
...
/// Rust version of the Move myso::bridge::Bridge type
...
pub trait BridgeTrait {
    ...
    fn frozen(&self) -> bool;
    fn try_into_bridge_summary(self) -> MySoResult<BridgeSummary>;
}
...
pub fn get_bridge_wrapper(object_store: &dyn ObjectStore) -> Result<BridgeWrapper, MySoError> {
    let wrapper = object_store
        .get_object(&MYSO_BRIDGE_OBJECT_ID)
        // Don't panic here on None because object_store is a generic store.
        .ok_or_else(|| {
            MySoErrorKind::MySoBridgeReadError("BridgeWrapper object not found".to_owned())
        })?;
    let move_object = wrapper.data.try_as_move().ok_or_else(|| {
        MySoErrorKind::MySoBridgeReadError("BridgeWrapper object must be a Move object".to_owned())
    })?;
    let result = bcs::from_bytes::<BridgeWrapper>(move_object.contents())
        .map_err(|err| MySoErrorKind::MySoBridgeReadError(err.to_string()))?;
    Ok(result)
}

pub fn get_bridge(object_store: &dyn ObjectStore) -> Result<Bridge, MySoError> {
    ...
    match version {
        ...
        _ => Err(MySoErrorKind::MySoBridgeReadError(format!(
            "Unsupported MySoBridge version: {}",
            version
        ))
        .into()),
    }
        ...
        let transfer_limit = self
            ...
            .map(|e| {
                let source = BridgeChainId::try_from(e.key.source).map_err(|_e| {
                    MySoErrorKind::GenericBridgeError {
...
pub struct MoveTypeBridgeCommittee {
    ...
    pub member_registrations: VecMap<MySoAddress, MoveTypeCommitteeMemberRegistration>,
...
pub struct BridgeCommitteeSummary {
    ...
    pub member_registration: Vec<(MySoAddress, MoveTypeCommitteeMemberRegistration)>,
    pub last_committee_update_epoch: u64,
}
...
pub fn is_bridge_committee_initiated(object_store: &dyn ObjectStore) -> MySoResult<bool> {
    match get_bridge(object_store).map_err(|e| *e.0) {
        Ok(bridge) => Ok(!bridge.committee().members.contents.is_empty()),
        Err(MySoErrorKind::MySoBridgeReadError(..)) => Ok(false),
