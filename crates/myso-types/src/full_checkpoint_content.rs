...
use crate::myso_system_state::MySoSystemStateTrait;
use crate::myso_system_state::get_myso_system_state;
...
impl CheckpointData {
    ...
    pub fn epoch_info(&self) -> Result<Option<EpochInfo>, StorageError> {
        ...
        let system_state =
            get_myso_system_state(&transaction.output_objects.as_slice()).map_err(|e| {
...
impl BackingPackageStore for CheckpointData {
    ...
    ) -> crate::error::MySoResult<Option<crate::storage::PackageObject>> {
        self.transactions
            .iter()
...
impl Checkpoint {
    pub fn epoch_info(&self) -> Result<Option<EpochInfo>, StorageError> {
        ...
        let output_objects: Vec<Object> = transaction
            ...
            .cloned()
            .collect();
        let system_state = get_myso_system_state(&output_objects.as_slice()).map_err(|e| {
            StorageError::custom(format!(
                "Failed to find system state object output from end of epoch transaction: {e}"
            ))
        })?;
