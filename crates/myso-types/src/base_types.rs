...
use crate::error::{ExecutionError, MySoResult};
use crate::gas_coin::GAS;
use crate::gas_coin::GasCoin;
use crate::governance::STAKED_MYSO_STRUCT_NAME;
use crate::governance::STAKING_POOL_MODULE_NAME;
use crate::governance::StakedMySo;
...
impl MoveObjectType {
    ...
    pub fn staked_myso() -> Self {
        Self(MoveObjectType_::StakedMySo)
    }

    pub fn address(&self) -> AccountAddress {
        match &self.0 {
            MoveObjectType_::GasCoin | MoveObjectType_::Coin(_) => MYSO_FRAMEWORK_ADDRESS,
            MoveObjectType_::StakedMySo => MYSO_SYSTEM_ADDRESS,
            MoveObjectType_::MySoBalanceAccumulatorField
            | MoveObjectType_::BalanceAccumulatorField(_) => MYSO_FRAMEWORK_ADDRESS,
    ...
    pub fn module(&self) -> &IdentStr {
        match &self.0 {
            ...
            MoveObjectType_::StakedMySo => STAKING_POOL_MODULE_NAME,
            MoveObjectType_::MySoBalanceAccumulatorField
    ...
    pub fn name(&self) -> &IdentStr {
        match &self.0 {
            ...
            MoveObjectType_::StakedMySo => STAKED_MYSO_STRUCT_NAME,
            MoveObjectType_::MySoBalanceAccumulatorField
    ...
    pub fn type_params(&self) -> Vec<Cow<'_, TypeTag>> {
        match &self.0 {
            ...
            MoveObjectType_::StakedMySo => vec![],
    ...
    pub fn coin_type_maybe(&self) -> Option<TypeTag> {
        match &self.0 {
            ...
            MoveObjectType_::StakedMySo => None,
            MoveObjectType_::MySoBalanceAccumulatorField => None,
    ...
    pub fn balance_accumulator_field_type_maybe(&self) -> Option<TypeTag> {
        match &self.0 {
            MoveObjectType_::MySoBalanceAccumulatorField => Some(GAS::type_tag()),
    ...
    pub fn is_myso_balance_accumulator_field(&self) -> bool {
        matches!(self.0, MoveObjectType_::MySoBalanceAccumulatorField)
    }
    ...
    pub fn size_for_gas_metering(&self) -> usize {
        ...
        match &self.0 {
            ...
            MoveObjectType_::StakedMySo => 1,
            MoveObjectType_::Coin(inner) => bcs::serialized_size(inner).unwrap() + 1,
            MoveObjectType_::MySoBalanceAccumulatorField => 1,
            MoveObjectType_::BalanceAccumulatorField(inner) => {
    ...
    /// Return true if `self` is `0x2::coin::Coin<T>` for some T (note: T can be MYSO)
    pub fn is_coin(&self) -> bool {
        match &self.0 {
            MoveObjectType_::GasCoin | MoveObjectType_::Coin(_) => true,
            MoveObjectType_::StakedMySo
            | MoveObjectType_::MySoBalanceAccumulatorField
            | MoveObjectType_::BalanceAccumulatorField(_)
    ...
    /// Return true if `self` is 0x2::coin::Coin<0x2::myso::MYSO>
    ...
    pub fn is_staked_myso(&self) -> bool {
        match &self.0 {
            MoveObjectType_::StakedMySo => true,
            MoveObjectType_::GasCoin
            | MoveObjectType_::Coin(_)
            | MoveObjectType_::MySoBalanceAccumulatorField
            | MoveObjectType_::BalanceAccumulatorField(_)
    ...
    pub fn is_dynamic_field(&self) -> bool {
        match &self.0 {
            MoveObjectType_::GasCoin | MoveObjectType_::StakedMySo | MoveObjectType_::Coin(_) => {
                false
            }
            MoveObjectType_::MySoBalanceAccumulatorField
    ...
    pub fn try_extract_field_name(&self, type_: &DynamicFieldType) -> MySoResult<TypeTag> {
        match &self.0 {
            MoveObjectType_::GasCoin | MoveObjectType_::StakedMySo | MoveObjectType_::Coin(_) => {
    ...
    pub fn try_extract_field_value(&self) -> MySoResult<TypeTag> {
        match &self.0 {
            MoveObjectType_::GasCoin | MoveObjectType_::StakedMySo | MoveObjectType_::Coin(_) => {
                Err(MySoErrorKind::ObjectDeserializationError {
    ...
    pub fn is(&self, s: &StructTag) -> bool {
        match &self.0 {
            ...
            MoveObjectType_::StakedMySo => StakedMySo::is_staked_myso(s),
            MoveObjectType_::Coin(inner) => {
                Coin::is_coin(s) && s.type_params.len() == 1 && inner == &s.type_params[0]
            }
            MoveObjectType_::MySoBalanceAccumulatorField => accumulator_value_balance_type_maybe(s)
...
impl From<StructTag> for MoveObjectType {
    fn from(mut s: StructTag) -> Self {
        ...
        } else if StakedMySo::is_staked_myso(&s) {
            MoveObjectType_::StakedMySo
...
impl From<MoveObjectType> for StructTag {
    fn from(t: MoveObjectType) -> Self {
        match t.0 {
            ...
            MoveObjectType_::StakedMySo => StakedMySo::type_(),
...
impl FromStr for ObjectType {
    ...
    fn from_str(s: &str) -> Result<Self, Self::Err> {
        if s.to_lowercase() == PACKAGE {
            ...
            let tag = parse_myso_struct_tag(s)?;
...
pub const MYSO_ADDRESS_LENGTH: usize = ObjectID::LENGTH;
...
#[cfg_attr(feature = "fuzzing", derive(proptest_derive::Arbitrary))]
pub struct MySoAddress(
    #[schemars(with = "Hex")]
    #[serde_as(as = "Readable<Hex, _>")]
    [u8; MYSO_ADDRESS_LENGTH],
);

impl MySoAddress {
    pub const ZERO: Self = Self([0u8; MYSO_ADDRESS_LENGTH]);
    ...
    /// Return a random MySoAddress.
    pub fn random_for_testing_only() -> Self {
        AccountAddress::random().into()
    }

    pub fn generate<R: rand::RngCore + rand::CryptoRng>(mut rng: R) -> Self {
        let buf: [u8; MYSO_ADDRESS_LENGTH] = rng.r#gen();
        Self(buf)
    }

    /// Serialize an `Option<MySoAddress>` in Hex.
    pub fn optional_address_as_hex<S>(
        key: &Option<MySoAddress>,
        serializer: S,
    ) -> Result<S::Ok, S::Error>
    where
    ...
    /// Deserialize into an `Option<MySoAddress>`.
    pub fn optional_address_from_hex<'de, D>(
        deserializer: D,
    ) -> Result<Option<MySoAddress>, D::Error>
    where
    ...
    /// Return the underlying byte array of a MySoAddress.
    pub fn to_inner(self) -> [u8; MYSO_ADDRESS_LENGTH] {
        self.0
    }

    /// Parse a MySoAddress from a byte array or buffer.
    pub fn from_bytes<T: AsRef<[u8]>>(bytes: T) -> Result<Self, MySoError> {
        <[u8; MYSO_ADDRESS_LENGTH]>::try_from(bytes.as_ref())
            .map_err(|_| MySoErrorKind::InvalidAddress.into())
            .map(MySoAddress)
    }
    ...
    pub fn try_from_padded(inputs: &ZkLoginInputs) -> MySoResult<Self> {
        Ok((&PublicKey::from_zklogin_inputs(inputs)?).into())
    }

    /// Define as iss_bytes_len || iss_bytes || unpadded_32_byte_address_seed.
    pub fn try_from_unpadded(inputs: &ZkLoginInputs) -> MySoResult<Self> {
        ...
        Ok(MySoAddress(hasher.finalize().digest))
    }
}

impl From<ObjectID> for MySoAddress {
    fn from(object_id: ObjectID) -> MySoAddress {
        Self(object_id.into_bytes())
    }
}

impl From<AccountAddress> for MySoAddress {
    fn from(address: AccountAddress) -> MySoAddress {
        Self(address.into_bytes())
    }
}

impl TryFrom<&[u8]> for MySoAddress {
    type Error = MySoError;

    /// Tries to convert the provided byte array into a MySoAddress.
    fn try_from(bytes: &[u8]) -> Result<Self, MySoError> {
        Self::from_bytes(bytes)
    }
}

impl TryFrom<Vec<u8>> for MySoAddress {
    type Error = MySoError;

    /// Tries to convert the provided byte buffer into a MySoAddress.
    fn try_from(bytes: Vec<u8>) -> Result<Self, MySoError> {
        Self::from_bytes(bytes)
    }
}

impl AsRef<[u8]> for MySoAddress {
    fn as_ref(&self) -> &[u8] {
        &self.0[..]
    }
}

impl FromStr for MySoAddress {
    type Err = anyhow::Error;
    fn from_str(s: &str) -> Result<Self, Self::Err> {
...
impl<T: MySoPublicKey> From<&T> for MySoAddress {
    fn from(pk: &T) -> Self {
        let mut hasher = DefaultHash::default();
        hasher.update([T::SIGNATURE_SCHEME.flag()]);
        hasher.update(pk);
        let g_arr = hasher.finalize();
        MySoAddress(g_arr.digest)
    }
}

impl From<&PublicKey> for MySoAddress {
...
impl From<&MultiSigPublicKey> for MySoAddress {
    /// Derive a MySoAddress from [struct MultiSigPublicKey]. A MultiSig address
    ...
    fn from(multisig_pk: &MultiSigPublicKey) -> Self {
        ...
        MySoAddress(hasher.finalize().digest)
    }
}

/// MySo address for [struct ZkLoginAuthenticator] is defined as the black2b hash of
/// [zklogin_flag || iss_bytes_length || iss_bytes || unpadded_address_seed_in_bytes].
impl TryFrom<&ZkLoginAuthenticator> for MySoAddress {
    type Error = MySoError;
    fn try_from(authenticator: &ZkLoginAuthenticator) -> MySoResult<Self> {
        MySoAddress::try_from_unpadded(&authenticator.inputs)
    }
}

impl TryFrom<&GenericSignature> for MySoAddress {
    type Error = MySoError;
    /// Derive a MySoAddress from a serialized signature in MySo [GenericSignature].
    fn try_from(sig: &GenericSignature) -> MySoResult<Self> {
        match sig {
            ...
            GenericSignature::ZkLoginAuthenticator(zklogin) => {
                MySoAddress::try_from_unpadded(&zklogin.inputs)
            }
            GenericSignature::PasskeyAuthenticator(s) => Ok(MySoAddress::from(&s.get_pk()?)),
        }
    }
}

impl fmt::Display for MySoAddress {
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
        write!(f, "0x{}", Hex::encode(self.0))
    }
}

impl fmt::Debug for MySoAddress {
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> Result<(), fmt::Error> {
        write!(f, "0x{}", Hex::encode(self.0))
    }
}

/// Generate a fake MySoAddress with repeated one byte.
pub fn dbg_addr(name: u8) -> MySoAddress {
    let addr = [name; MYSO_ADDRESS_LENGTH];
    MySoAddress(addr)
}
...
impl TxContext {
    ...
    pub fn sponsor(&self) -> Option<MySoAddress> {
        self.sponsor.map(MySoAddress::from)
    }
...
impl From<MySoAddress> for ObjectID {
    fn from(address: MySoAddress) -> ObjectID {
...
impl From<MySoAddress> for AccountAddress {
    fn from(address: MySoAddress) -> Self {
        Self::new(address.0)
    }
}
...
impl fmt::Display for MoveObjectType {
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> std::fmt::Result {
        ...
        write!(
            f,
            "{}",
            to_myso_struct_tag_string(&s).map_err(fmt::Error::custom)?
        )
    }
}
