...
use crate::base_types::{
    ConsensusObjectSequenceKey, FullObjectID, FullObjectRef, MySoAddress, TransactionDigest,
    VersionNumber,
};
use crate::committee::EpochId;
use crate::effects::{TransactionEffects, TransactionEffectsAPI};
use crate::error::{ExecutionError, MySoError, MySoErrorKind};
...
) -> MySoResult<PackageFetchResults<PackageObject>> {
...
pub fn get_module(
    store: impl BackingPackageStore,
    module_id: &ModuleId,
) -> Result<Option<Vec<u8>>, MySoError> {
    Ok(store
...
pub fn get_module_by_id<S: BackingPackageStore>(
    store: &S,
    id: &ModuleId,
) -> anyhow::Result<Option<CompiledModule>, MySoError> {
...
impl BackingPackageStore for TrackingBackingStore<'_> {
    fn get_package_object(
        &self,
        package_id: &ObjectID,
    ) -> crate::error::MySoResult<Option<PackageObject>> {
...
impl ChildObjectResolver for TrackingBackingStore<'_> {
    fn read_child_object(
        ...
        child: &ObjectID,
        child_version_upper_bound: SequenceNumber,
    ) -> crate::error::MySoResult<Option<Object>> {
        self.inner
