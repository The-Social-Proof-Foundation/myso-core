...
use crate::myso_serde::AsProtocolVersion;
use crate::myso_serde::BigInt;
use crate::myso_serde::Readable;
use crate::transaction::{Transaction, TransactionData};
use crate::{base_types::AuthorityName, committee::Committee, error::MySoErrorKind};
use anyhow::Result;
...
impl CheckpointArtifact {
    pub fn digest(&self) -> MySoResult<Digest> {
        match self {
            Self::ObjectStates(object_states) => {
                ...
                .map_err(|e| MySoErrorKind::GenericAuthorityError {
...
impl CheckpointArtifacts {
    ...
    pub fn add_artifact(&mut self, artifact: CheckpointArtifact) -> MySoResult<()> {
        if self
            .artifacts
            .iter()
            .any(|existing| existing.artifact_type() == artifact.artifact_type())
        {
            return Err(MySoErrorKind::GenericAuthorityError {
    ...
    pub fn object_states(&self) -> MySoResult<&BTreeMap<ObjectID, (SequenceNumber, ObjectDigest)>> {
        self.artifacts
            .iter()
    ...
    pub fn digest(&self) -> MySoResult<CheckpointArtifactsDigest> {
...
impl CheckpointSummary {
    ...
    pub fn verify_epoch(&self, epoch: EpochId) -> MySoResult {
        fp_ensure!(
            self.epoch == epoch,
    ...
    pub fn checkpoint_artifacts_digest(&self) -> MySoResult<&CheckpointArtifactsDigest> {
...
impl CertifiedCheckpointSummary {
    pub fn verify_authority_signatures(&self, committee: &Committee) -> MySoResult {
    ...
    pub fn try_into_verified(self, committee: &Committee) -> MySoResult<VerifiedCheckpoint> {
    ...
    pub fn verify_with_contents(
        ...
        if let Some(contents) = contents {
            ...
            fp_ensure!(
                ...
                MySoErrorKind::GenericAuthorityError{error:format!("Checkpoint contents digest mismatch: summary={:?}, received content digest {:?}, received {} transactions", self.data(), content_digest, contents.size())}.into()
            );
        }
...
impl SignedCheckpointSummary {
    ...
    pub fn try_into_verified(
        self,
        committee: &Committee,
    ) -> MySoResult<VerifiedEnvelope<CheckpointSummary, AuthoritySignInfo>> {
        self.verify_authority_signatures(committee)?;
...
impl CheckpointSignatureMessage {
    pub fn verify(&self, committee: &Committee) -> MySoResult {
