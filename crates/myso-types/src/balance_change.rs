...
fn coins(objects: &[Object]) -> impl Iterator<Item = (&MySoAddress, TypeTag, u64)> + '_ {
...
) -> impl Iterator<Item = (MySoAddress, TypeTag, i128)> + '_ {
    effects
        .accumulator_events()
        .into_iter()
        .filter_map(|event| {
...
mod tests {
    ...
    fn myso_balance_type() -> TypeTag {
        Balance::type_tag("0x2::myso::MYSO".parse().unwrap())
    }
    ...
    fn get_accumulator_obj_id(address: MySoAddress, balance_type: &TypeTag) -> ObjectID {
    ...
    fn test_derive_balance_changes_with_split_accumulator_event() {
        ...
        let balance_type = myso_balance_type();
        ...
        assert_eq!(
            result[0].coin_type,
            "0x2::myso::MYSO".parse::<TypeTag>().unwrap()
        );
    ...
    fn test_derive_balance_changes_with_multiple_addresses() {
        let address1 = MySoAddress::random_for_testing_only();
        let address2 = MySoAddress::random_for_testing_only();
        let balance_type = myso_balance_type();
    ...
    fn create_gas_coin_object(owner: MySoAddress, value: u64) -> Object {
    ...
    fn create_custom_coin_object(owner: MySoAddress, coin_type: TypeTag, value: u64) -> Object {
        use crate::base_types::SequenceNumber;
        use crate::object::MoveObject;
    ...
    fn test_derive_balance_changes_with_coin_objects_only() {
        ...
        // Create input coin with 5000 MYSO
        let input_coin = create_gas_coin_object(address, 5000);
        // Create output coin with 3000 MYSO (spent 2000)
    ...
    fn test_derive_balance_changes_coin_transfer_between_addresses() {
        ...
        let receiver = MySoAddress::random_for_testing_only();

        // Sender has 10000 MYSO initially
    ...
    fn test_derive_balance_changes_combines_coins_and_accumulator_events() {
        ...
        // Coin: spent 2000 MYSO (5000 -> 3000)
        let input_coin = create_gas_coin_object(address, 5000);
        let output_coin = create_gas_coin_object(address, 3000);

        // Accumulator: received 500 MYSO via address balance
        let write = AccumulatorWriteV1 {
            address: AccumulatorAddress::new(address, balance_type),
            operation: AccumulatorOperation::Merge,
            value: AccumulatorValue::Integer(500),
    ...
    fn test_derive_balance_changes_coins_and_accumulator_net_to_zero() {
        ...
        // Coin: spent 1000 MYSO (5000 -> 4000)
        let input_coin = create_gas_coin_object(address, 5000);
        let output_coin = create_gas_coin_object(address, 4000);

        // Accumulator: received exactly 1000 MYSO - perfectly offsets coin spend
        let write = AccumulatorWriteV1 {
            address: AccumulatorAddress::new(address, balance_type),
            operation: AccumulatorOperation::Merge,
            value: AccumulatorValue::Integer(1000),
    ...
    fn test_derive_balance_changes_different_coin_types() {
        ...
        // MYSO coin: spent 1000
        let myso_input = create_gas_coin_object(address, 5000);
        let myso_output = create_gas_coin_object(address, 4000);
        ...
        let result = derive_balance_changes(&effects, &[myso_input], &[myso_output, custom_output]);

        assert_eq!(result.len(), 2);

        let myso_change = result
            .iter()
            .find(|c| c.coin_type == "0x2::myso::MYSO".parse::<TypeTag>().unwrap())
            .unwrap();
        let custom_change = result.iter().find(|c| c.coin_type == custom_type).unwrap();

        assert_eq!(myso_change.amount, -1000);
