...
use crate::gas_coin::MIST_PER_MYSO;
use crate::id::{ID, UID};
...
pub const STAKED_MYSO_STRUCT_NAME: &IdentStr = ident_str!("StakedMySo");
...
impl StakedMySo {
    pub fn type_() -> StructTag {
        StructTag {
            ...
            name: STAKED_MYSO_STRUCT_NAME.to_owned(),
            type_params: vec![],
        }
    }

    pub fn is_staked_myso(s: &StructTag) -> bool {
        s.address == MYSO_SYSTEM_ADDRESS
            && s.module.as_ident_str() == STAKING_POOL_MODULE_NAME
            && s.name.as_ident_str() == STAKED_MYSO_STRUCT_NAME
...
impl TryFrom<&Object> for StakedMySo {
    type Error = MySoError;
    fn try_from(object: &Object) -> Result<Self, Self::Error> {
        match &object.data {
            Data::Move(o) => {
                if o.type_().is_staked_myso() {
                    return bcs::from_bytes(o.contents()).map_err(|err| {
                        MySoErrorKind::TypeError {
                            error: format!("Unable to deserialize StakedMySo object: {:?}", err),
                        }
                        .into()
                    });
                }
        ...
        Err(MySoErrorKind::TypeError {
            error: format!("Object type is not a StakedMySo: {:?}", object),
        }
        .into())
    }
}
