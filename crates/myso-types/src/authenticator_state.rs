...
use crate::{MYSO_AUTHENTICATOR_STATE_OBJECT_ID, MYSO_FRAMEWORK_ADDRESS, id::UID};
...
pub const RESOLVED_MYSO_AUTHENTICATOR_STATE: (&AccountAddress, &IdentStr, &IdentStr) = (
...
) -> MySoResult<Option<AuthenticatorStateInner>> {
    let outer = object_store.get_object(&MYSO_AUTHENTICATOR_STATE_OBJECT_ID);
...
pub fn get_authenticator_state_obj_initial_shared_version(
    ...
    Ok(object_store
        .get_object(&MYSO_AUTHENTICATOR_STATE_OBJECT_ID)
