...
use super::{MYSO_BRIDGE_OBJECT_ID, base_types::*, error::*};
...
use crate::gas_model::gas_v2::MySoCostTable;
...
use crate::{
    ...
    MYSO_CLOCK_OBJECT_SHARED_VERSION, MYSO_FRAMEWORK_PACKAGE_ID, MYSO_RANDOMNESS_STATE_OBJECT_ID,
    MYSO_SYSTEM_STATE_OBJECT_ID, MYSO_SYSTEM_STATE_OBJECT_SHARED_VERSION,
};
...
pub enum CallArg {
    ...
    // `myso::funds_accumulator::Withdrawal` struct and passed into Move.
    // It is allowed to have multiple withdraw arguments even for the same funds type.
    FundsWithdrawal(FundsWithdrawalArg),
}

impl CallArg {
    pub const MYSO_SYSTEM_MUT: Self = Self::Object(ObjectArg::MYSO_SYSTEM_MUT);
...
impl FundsWithdrawalArg {
    ...
    pub fn owner_for_withdrawal(&self, tx: &impl TransactionDataAPI) -> MySoAddress {
...
impl EndOfEpochTransactionKind {
        ...
        match self {
            Self::ChangeEpoch(_) => {
                vec![InputObjectKind::SharedMoveObject {
                    id: MYSO_SYSTEM_STATE_OBJECT_ID,
                    initial_shared_version: MYSO_SYSTEM_STATE_OBJECT_SHARED_VERSION,
                    mutability: SharedObjectMutability::Mutable,
                }]
            }
            ...
            Self::AuthenticatorStateExpire(expire) => {
                vec![InputObjectKind::SharedMoveObject {
                    id: MYSO_AUTHENTICATOR_STATE_OBJECT_ID,
    ...
    fn shared_input_objects(&self) -> impl Iterator<Item = SharedInputObject> + '_ {
        match self {
            Self::ChangeEpoch(_) => {
                Either::Left(vec![SharedInputObject::MYSO_SYSTEM_OBJ].into_iter())
            }
            ...
            Self::BridgeCommitteeInit(bridge_version) => Either::Left(
                vec![
                    ...
                    SharedInputObject::MYSO_SYSTEM_OBJ,
                ]
                .into_iter(),
            ),
            Self::StoreExecutionTimeObservations(_) => {
...
impl ObjectArg {
    pub const MYSO_SYSTEM_MUT: Self = Self::SharedObject {
        id: MYSO_SYSTEM_STATE_OBJECT_ID,
        initial_shared_version: MYSO_SYSTEM_STATE_OBJECT_SHARED_VERSION,
        mutability: SharedObjectMutability::Mutable,
    };
...
impl ProgrammableTransaction {
    ...
    fn validity_check(&self, config: &ProtocolConfig) -> UserInputResult {
        ...
        if let Some(random_index) = inputs.iter().position(|obj| {
            matches!(
                obj,
                CallArg::Object(ObjectArg::SharedObject { id, .. }) if *id == MYSO_RANDOMNESS_STATE_OBJECT_ID
            )
        }) {
            fp_ensure!(
...
impl SharedInputObject {
    pub const MYSO_SYSTEM_OBJ: Self = Self {
        id: MYSO_SYSTEM_STATE_OBJECT_ID,
        initial_shared_version: MYSO_SYSTEM_STATE_OBJECT_SHARED_VERSION,
        mutability: SharedObjectMutability::Mutable,
    };
...
impl TransactionKind {
    ...
    pub fn is_accumulator_barrier_settle_tx(&self) -> bool {
        matches!(self, TransactionKind::ProgrammableSystemTransaction(_))
            && self.shared_input_objects().any(|obj| {
                obj.id == MYSO_ACCUMULATOR_ROOT_OBJECT_ID
    ...
    pub fn shared_input_objects(&self) -> impl Iterator<Item = SharedInputObject> + '_ {
        match &self {
            Self::ChangeEpoch(_) => {
                Either::Left(Either::Left(iter::once(SharedInputObject::MYSO_SYSTEM_OBJ)))
            }
            ...
            Self::RandomnessStateUpdate(update) => {
                Either::Left(Either::Left(iter::once(SharedInputObject {
                    id: MYSO_RANDOMNESS_STATE_OBJECT_ID,
...
impl TransactionData {
    ...
    fn new_system_transaction(kind: TransactionKind) -> Self {
        ...
        let sender = MySoAddress::default();
    ...
    pub fn new_transfer_myso(
        ...
        Self::new_transfer_myso_allow_sponsor(
            recipient,
            sender,
            amount,
            gas_payment,
    ...
    pub fn new_transfer_myso_allow_sponsor(
        recipient: MySoAddress,
        sender: MySoAddress,
        amount: Option<u64>,
        gas_payment: ObjectRef,
        ...
        let pt = {
            let mut builder = ProgrammableTransactionBuilder::new();
            builder.transfer_myso(recipient, amount);
            builder.finish()
        };
    ...
    pub fn new_pay_all_myso(
        ...
        let pt = {
            let mut builder = ProgrammableTransactionBuilder::new();
            builder.pay_all_myso(recipient);
            builder.finish()
        };
    ...
    pub fn execution_parts(&self) -> (TransactionKind, MySoAddress, GasData) {
    ...
    pub fn uses_randomness(&self) -> bool {
        self.kind()
            ...
            .any(|obj| obj.id() == MYSO_RANDOMNESS_STATE_OBJECT_ID)
    }
...
pub trait TransactionDataAPI {
    ...
    fn required_signers(&self) -> NonEmpty<MySoAddress>;

    fn gas_data(&self) -> &GasData;

    fn gas_owner(&self) -> MySoAddress;
    ...
    fn validity_check(&self, context: &TxValidityCheckContext<'_>) -> MySoResult;
    ...
    fn sender_mut_for_testing(&mut self) -> &mut MySoAddress;
...
impl TransactionDataAPI for TransactionDataV1 {
    ...
    fn validity_check(&self, context: &TxValidityCheckContext<'_>) -> MySoResult {
        ...
        match self.expiration() {
            ...
            TransactionExpiration::Epoch(max_epoch) => {
                if context.epoch > *max_epoch {
                    return Err(MySoErrorKind::TransactionExpired.into());
                }
            }
        ...
        if !self.is_system_tx() {
            let cost_table = MySoCostTable::new(config, self.gas_data.price);
...
impl Serialize for SenderSignedTransaction {
        ...
        S: serde::Serializer,
    {
        ...
        if self.intent_message().intent != Intent::myso_transaction() {
...
impl<'de> Deserialize<'de> for SenderSignedTransaction {
        ...
        D: serde::Deserializer<'de>,
    {
        ...
        if intent_message.intent != Intent::myso_transaction() {
...
impl SenderSignedTransaction {
    ...
    pub(crate) fn get_signer_sig_mapping(
    ...
    ) -> MySoResult<BTreeMap<MySoAddress, (u8, &GenericSignature)>> {
        ...
        for (idx, sig) in self.tx_signatures.iter().enumerate() {
            if verify_legacy_zklogin_address && let GenericSignature::ZkLoginAuthenticator(z) = sig
            {
                ...
                mapping.insert(MySoAddress::try_from_padded(&z.inputs)?, (idx as u8, sig));
            }
...
impl SenderSignedData {
    pub fn new(tx_data: TransactionData, tx_signatures: Vec<GenericSignature>) -> Self {
        Self(SizeOneVec::new(SenderSignedTransaction {
            intent_message: IntentMessage::new(Intent::myso_transaction(), tx_data),
            tx_signatures,
        }))
    }
    ...
    pub fn full_message_digest_with_alias_versions(
        &self,
        alias_versions: &Vec<(MySoAddress, Option<SequenceNumber>)>,
    ...
    pub fn serialized_size(&self) -> MySoResult<usize> {
    ...
    fn check_user_signature_protocol_compatibility(&self, config: &ProtocolConfig) -> MySoResult {
    ...
    pub fn validity_check(&self, context: &TxValidityCheckContext<'_>) -> Result<usize, MySoError> {
...
impl<S> Envelope<SenderSignedData, S> {
    pub fn sender_address(&self) -> MySoAddress {
        self.data().intent_message().value.sender()
    }

    pub fn gas_owner(&self) -> MySoAddress {
...
impl Transaction {
    pub fn from_data_and_signer(
        ...
        let signatures = {
            let intent_msg = IntentMessage::new(Intent::myso_transaction(), &data);
            signers
                .into_iter()
        ...
        system_transaction
            ...
            .pipe(|data| {
                SenderSignedData::new_from_sender_signature(
                    data,
                    Ed25519MySoSignature::from_bytes(&[0; Ed25519MySoSignature::LENGTH])
                        .unwrap()
                        .into(),
                )
            })
...
impl Transaction {
    ...
    pub fn try_into_verified_for_testing(
        ...
        current_epoch: EpochId,
        verify_params: &VerifyParams,
    ) -> MySoResult<VerifiedTransaction> {
...
impl SignedTransaction {
    ...
    pub fn try_into_verified_for_testing(
        ...
        committee: &Committee,
        verify_params: &VerifyParams,
    ) -> MySoResult<VerifiedSignedTransaction> {
...
impl CertifiedTransaction {
    ...
    pub fn try_into_verified_for_testing(
        ...
        committee: &Committee,
        verify_params: &VerifyParams,
    ) -> MySoResult<VerifiedCertificate> {
    ...
    pub fn verify_committee_sigs_only(&self, committee: &Committee) -> MySoResult {
        self.auth_sig().verify_secure(
            self.data(),
            Intent::myso_app(IntentScope::SenderSignedTransaction),
            committee,
...
/// Deprecated version of WithAliases that uses MySoAddress instead of u8.
...
pub struct DeprecatedWithAliases<T>(
    T,
    #[serde(with = "nonempty_as_vec")] NonEmpty<(MySoAddress, Option<SequenceNumber>)>,
);

impl<T> DeprecatedWithAliases<T> {
    pub fn into_inner(self) -> (T, NonEmpty<(MySoAddress, Option<SequenceNumber>)>) {
        (self.0, self.1)
    }
}
...
impl<T> TransactionWithClaims<T> {
    ...
    #[allow(deprecated)]
    pub fn aliases_v1(&self) -> Option<NonEmpty<(MySoAddress, Option<SequenceNumber>)>> {
        self.claims
            .iter()
            .find_map(|c| match c {
...
// An InputObjects new-type that has been verified by myso-transaction-checks, and can be
// safely passed to execution.
pub struct CheckedInputObjects(InputObjects);

// DO NOT CALL outside of myso-transaction-checks, genesis, or replay.
//
// CheckedInputObjects should really be defined in myso-transaction-checks so that we can
// make public construction impossible. But we can't do that because it would result in circular
// dependencies.
impl CheckedInputObjects {
    // Only called by myso-transaction-checks.
