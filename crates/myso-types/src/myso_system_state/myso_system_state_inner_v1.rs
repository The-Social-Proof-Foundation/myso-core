...
use super::epoch_start_myso_system_state::EpochStartValidatorInfoV1;
use super::myso_system_state_summary::{MySoSystemStateSummary, MySoValidatorSummary};
use super::{AdvanceEpochParams, MySoSystemStateTrait, get_validators_from_table_vec};
use crate::balance::Balance;
use crate::base_types::{ObjectID, MySoAddress};
...
/// Rust version of the Move myso::myso_system::SystemParameters type
...
impl VerifiedValidatorMetadataV1 {
    pub fn myso_pubkey_bytes(&self) -> AuthorityPublicKeyBytes {
...
impl ValidatorMetadataV1 {
    ...
    pub fn verify(&self) -> Result<VerifiedValidatorMetadataV1, u64> {
        ...
        verify_proof_of_possession(&pop, &protocol_pubkey, self.myso_address)
            .map_err(|_| E_METADATA_INVALID_POP)?;
        ...
        Ok(VerifiedValidatorMetadataV1 {
            myso_address: self.myso_address,
            protocol_pubkey,
            network_pubkey,
            worker_pubkey,
...
/// Rust version of the Move myso::validator::Validator type
#[derive(Debug, Serialize, Deserialize, Clone, Eq, PartialEq)]
...
impl ValidatorV1 {
    ...
    pub fn into_myso_validator_summary(self) -> MySoValidatorSummary {
        let Self {
            metadata:
                ValidatorMetadataV1 {
                    myso_address,
            ...
            staking_pool:
                StakingPoolV1 {
                    ...
                    myso_balance: staking_pool_myso_balance,
                    rewards_pool,
                    pool_token_balance,
                    exchange_rates:
                        Table {
...
/// Rust version of the Move myso_system::staking_pool::StakingPool type
#[derive(Debug, Serialize, Deserialize, Clone, Eq, PartialEq)]
pub struct StakingPoolV1 {
    pub id: ObjectID,
    pub activation_epoch: Option<u64>,
    pub deactivation_epoch: Option<u64>,
    pub myso_balance: u64,
    pub rewards_pool: Balance,
    pub pool_token_balance: u64,
    pub exchange_rates: Table,
    pub pending_stake: u64,
    pub pending_total_myso_withdraw: u64,
    pub pending_pool_token_withdraw: u64,
    pub extra_fields: Bag,
}

/// Rust version of the Move myso_system::validator_set::ValidatorSet type
#[derive(Debug, Serialize, Deserialize, Clone, Eq, PartialEq)]
pub struct ValidatorSetV1 {
    ...
    pub at_risk_validators: VecMap<MySoAddress, u64>,
    pub extra_fields: Bag,
}

/// Rust version of the Move myso_system::storage_fund::StorageFund type
#[derive(Debug, Serialize, Deserialize, Clone, Eq, PartialEq)]
...
/// Rust version of the Move myso_system::myso_system::MySoSystemStateInner type
#[derive(Debug, Serialize, Deserialize, Clone, Eq, PartialEq)]
pub struct MySoSystemStateInnerV1 {
    pub epoch: u64,
    ...
    pub validator_report_records: VecMap<MySoAddress, VecSet<MySoAddress>>,
...
impl MySoSystemStateTrait for MySoSystemStateInnerV1 {
    fn epoch(&self) -> u64 {
        self.epoch
    }
    ...
    fn get_current_epoch_committee(&self) -> CommitteeWithNetworkMetadata {
        let validators = self
            ...
            .map(|validator| {
                ...
                let name = verified_metadata.myso_pubkey_bytes();
                (
                    name,
                    (
    ...
    fn get_pending_active_validators<S: ObjectStore + ?Sized>(
        ...
        Ok(validators
            .into_iter()
            .map(|v| v.into_myso_validator_summary())
            .collect())
    }
    ...
    fn into_myso_system_state_summary(self) -> MySoSystemStateSummary {
        // If you are making any changes to MySoSystemStateV1 or any of its dependent types before
        // mainnet, please also update MySoSystemStateSummary and its corresponding TS type.
...
/// Rust version of the Move myso_system::validator_cap::UnverifiedValidatorOperationCap type
#[derive(Debug, Serialize, Deserialize, Clone, Eq, PartialEq)]
pub struct UnverifiedValidatorOperationCapV1 {
    pub id: ObjectID,
    pub authorizer_validator_address: MySoAddress,
}
