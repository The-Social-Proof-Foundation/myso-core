...
/// This is the JSON-RPC type for the MYSO system state object.
/// It flattens all fields to make them top-level fields such that it as minimum
/// dependencies to the internal data structures of the MYSO system state type.

#[serde_as]
#[derive(Debug, Serialize, Deserialize, Clone, JsonSchema)]
#[serde(rename_all = "camelCase")]
pub struct MySoSystemStateSummary {
    ...
    pub active_validators: Vec<MySoValidatorSummary>,
    ...
    /// ID of the object that maps from staking pool's ID to the myso address of a validator.
    ...
    #[schemars(with = "Vec<(MySoAddress, BigInt<u64>)>")]
    #[serde_as(as = "Vec<(_, Readable<BigInt<u64>, _>)>")]
    pub at_risk_validators: Vec<(MySoAddress, u64)>,
    /// A map storing the records of validator reporting each other.
    pub validator_report_records: Vec<(MySoAddress, Vec<MySoAddress>)>,
}

impl MySoSystemStateSummary {
    pub fn get_myso_committee_for_benchmarking(&self) -> CommitteeWithNetworkMetadata {
        let validators = self
            .active_validators
            .iter()
            .map(|validator| {
...
/// This is the JSON-RPC type for the MYSO validator. It flattens all inner structures
/// to top-level fields so that they are decoupled from the internal definitions.
#[serde_as]
#[derive(Debug, Serialize, Deserialize, Clone, JsonSchema)]
#[serde(rename_all = "camelCase")]
pub struct MySoValidatorSummary {
    // Metadata
    ...
    #[schemars(with = "BigInt<u64>")]
    #[serde_as(as = "Readable<BigInt<u64>, _>")]
    pub staking_pool_myso_balance: u64,
...
impl Default for MySoSystemStateSummary {
    fn default() -> Self {
        Self {
            epoch: 0,
            protocol_version: 1,
...
impl Default for MySoValidatorSummary {
    fn default() -> Self {
        Self {
            myso_address: MySoAddress::default(),
...
/// Given the staking pool id of a validator, return the validator's `MySoValidatorSummary`,
/// works for validator candidates, active validators, as well as inactive validators.
pub fn get_validator_by_pool_id<S>(
    object_store: &S,
    system_state: &MySoSystemState,
    system_state_summary: &MySoSystemStateSummary,
    pool_id: ObjectID,
    ...
    S: ObjectStore + ?Sized,
{
    ...
    let candidate_address: MySoAddress = get_dynamic_field_from_store(
        &object_store,
