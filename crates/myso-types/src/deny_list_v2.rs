...
use crate::base_types::{EpochId, MySoAddress};
...
fn check_new_regulated_coin_owners(
    new_regulated_coin_owners: BTreeMap<String, (Config, BTreeSet<MySoAddress>)>,
    cur_epoch: EpochId,
    object_store: &dyn ObjectStore,
...
pub fn get_per_type_coin_deny_list_v2(
    ...
    let config: Config =
        get_dynamic_field_from_store(object_store, MYSO_DENY_LIST_OBJECT_ID, &config_key).ok()?;
    Some(config)
}
