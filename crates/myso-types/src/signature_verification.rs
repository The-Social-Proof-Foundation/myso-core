...
impl<D: Hash + Eq + Copy> VerifiedDigestCache<D, ()> {
    ...
    pub fn is_verified<F, G>(&self, digest: D, verify_callback: F, uncached_checks: G) -> MySoResult
    where
        F: FnOnce() -> MySoResult,
        G: FnOnce() -> MySoResult,
    {
...
pub fn verify_sender_signed_data_message_signatures(
    ...
    assert_eq!(intent_message.intent, Intent::myso_transaction());
    ...
    for signer in required_signers.iter() {
        ...
        let Some(sig_index) = alias_set
            ...
            return Err(MySoErrorKind::SignerSignatureAbsent {
                expected: alias_set
                    .iter()
                    .map(|s| s.to_string())
                    .collect::<Vec<_>>()
