...
use crate::{
    base_types::{MySoAddress, VersionDigest},
...
impl AccumulatorAddress {
    pub fn new(address: MySoAddress, ty: TypeTag) -> Self {
        Self { address, ty }
    }
}
...
mod tests {
    ...
    fn test_accumulator_address() -> AccumulatorAddress {
        AccumulatorAddress::new(
            ...
            "0x2::balance::Balance<0x2::myso::MYSO>".parse().unwrap(),
        )
    }
    ...
    fn test_merge_mismatched_types() {
        let addr = MySoAddress::random_for_testing_only();
