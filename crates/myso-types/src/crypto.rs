...
use crate::base_types::{AuthorityName, ConciseableName, MySoAddress};
...
use crate::myso_serde::{MySoBitmap, Readable};
...
pub const MYSO_PRIV_KEY_PREFIX: &str = "mysoprivkey";
...
pub fn generate_proof_of_possession(
    ...
    AuthoritySignature::new_secure(
        &IntentMessage::new(Intent::myso_app(IntentScope::ProofOfPossession), msg),
        &DEFAULT_EPOCH_ID,
        keypair,
    )
}
...
// * accounts to interact with MySo.
// * Currently we support eddsa and ecdsa on MySo.
//

#[allow(clippy::large_enum_variant)]
#[derive(Debug, From, PartialEq, Eq)]
pub enum MySoKeyPair {
...
impl MySoKeyPair {
    pub fn public(&self) -> PublicKey {
        match self {
            MySoKeyPair::Ed25519(kp) => PublicKey::Ed25519(kp.public().into()),
            MySoKeyPair::Secp256k1(kp) => PublicKey::Secp256k1(kp.public().into()),
            MySoKeyPair::Secp256r1(kp) => PublicKey::Secp256r1(kp.public().into()),
        }
    }

    pub fn copy(&self) -> Self {
        match self {
            MySoKeyPair::Ed25519(kp) => kp.copy().into(),
            MySoKeyPair::Secp256k1(kp) => kp.copy().into(),
            MySoKeyPair::Secp256r1(kp) => kp.copy().into(),
        }
    }
}

impl Signer<Signature> for MySoKeyPair {
    fn sign(&self, msg: &[u8]) -> Signature {
        match self {
            MySoKeyPair::Ed25519(kp) => kp.sign(msg),
            MySoKeyPair::Secp256k1(kp) => kp.sign(msg),
            MySoKeyPair::Secp256r1(kp) => kp.sign(msg),
        }
    }
}

impl EncodeDecodeBase64 for MySoKeyPair {
...
impl MySoKeyPair {
    pub fn to_bytes(&self) -> Vec<u8> {
        ...
        match self {
            MySoKeyPair::Ed25519(kp) => {
                bytes.extend_from_slice(kp.as_bytes());
            }
            MySoKeyPair::Secp256k1(kp) => {
                bytes.extend_from_slice(kp.as_bytes());
            }
            MySoKeyPair::Secp256r1(kp) => {
                bytes.extend_from_slice(kp.as_bytes());
            }
        }
        bytes
    ...
    pub fn from_bytes(bytes: &[u8]) -> Result<Self, eyre::Report> {
        ...
        {
            Ok(x) => match x {
                SignatureScheme::ED25519 => Ok(MySoKeyPair::Ed25519(Ed25519KeyPair::from_bytes(
                    bytes.get(1..).ok_or_else(|| eyre!("Invalid length"))?,
                )?)),
                SignatureScheme::Secp256k1 => {
                    Ok(MySoKeyPair::Secp256k1(Secp256k1KeyPair::from_bytes(
                        bytes.get(1..).ok_or_else(|| eyre!("Invalid length"))?,
                    )?))
                }
                SignatureScheme::Secp256r1 => {
                    Ok(MySoKeyPair::Secp256r1(Secp256r1KeyPair::from_bytes(
                        bytes.get(1..).ok_or_else(|| eyre!("Invalid length"))?,
                    )?))
                }
    ...
    pub fn to_bytes_no_flag(&self) -> Vec<u8> {
        match self {
            MySoKeyPair::Ed25519(kp) => kp.as_bytes().to_vec(),
            MySoKeyPair::Secp256k1(kp) => kp.as_bytes().to_vec(),
            MySoKeyPair::Secp256r1(kp) => kp.as_bytes().to_vec(),
        }
    }

    /// Encode a MySoKeyPair as `flag || privkey` in Bech32 starting with "mysoprivkey" to a string. Note that the pubkey is not encoded.
    pub fn encode(&self) -> Result<String, eyre::Report> {
        Bech32::encode(self.to_bytes(), MYSO_PRIV_KEY_PREFIX).map_err(|e| eyre!(e))
    }

    /// Decode a MySoKeyPair from `flag || privkey` in Bech32 starting with "mysoprivkey" to MySoKeyPair. The public key is computed directly from the private key bytes.
    pub fn decode(value: &str) -> Result<Self, eyre::Report> {
        let bytes = Bech32::decode(value, MYSO_PRIV_KEY_PREFIX)?;
        Self::from_bytes(&bytes)
    }
}

impl Serialize for MySoKeyPair {
...
impl<'de> Deserialize<'de> for MySoKeyPair {
        ...
        D: Deserializer<'de>,
    {
        ...
        MySoKeyPair::decode_base64(&s).map_err(|e| Error::custom(e.to_string()))
    }
}
...
impl ZkLoginPublicIdentifier {
    ...
    pub fn new(iss: &str, address_seed: &Bn254FrElement) -> MySoResult<Self> {
    ...
    pub fn validate(&self) -> MySoResult<()> {
        let bytes = &self.0;
        ...
        std::str::from_utf8(iss_bytes).map_err(|e| MySoErrorKind::InvalidSignature {
        ...
        if address_seed_bytes.len() > 32 {
            return Err(MySoErrorKind::InvalidSignature {
                error: format!(
...
impl PublicKey {
    ...
    pub fn scheme(&self) -> SignatureScheme {
        match self {
            PublicKey::Ed25519(_) => Ed25519MySoSignature::SCHEME,
            PublicKey::Secp256k1(_) => Secp256k1MySoSignature::SCHEME,
            PublicKey::Secp256r1(_) => Secp256r1MySoSignature::SCHEME,
            PublicKey::ZkLogin(_) => SignatureScheme::ZkLoginAuthenticator,
            PublicKey::Passkey(_) => SignatureScheme::PasskeyAuthenticator,
        }
    }

    pub fn from_zklogin_inputs(inputs: &ZkLoginInputs) -> MySoResult<Self> {
...
pub trait MySoAuthoritySignature {
    fn verify_secure<T>(
        &self,
...
impl MySoAuthoritySignature for AuthoritySignature {
...
pub fn get_key_pair<KP: KeypairTraits>() -> (MySoAddress, KP)
where
    <KP as KeypairTraits>::PubKey: MySoPublicKey,
{
...
pub fn random_committee_key_pairs_of_size(size: usize) -> Vec<AuthorityKeyPair> {
    ...
    (0..size)
        .map(|_| {
            ...
            // keys in ConfigBuilder::build (myso-config/src/network_config_builder). This is because
...
pub fn deterministic_random_account_key() -> (MySoAddress, AccountKeyPair) {
    let mut rng = StdRng::from_seed([0; 32]);
    get_key_pair_from_rng(&mut rng)
}

pub fn get_account_key_pair() -> (MySoAddress, AccountKeyPair) {
    get_key_pair()
}

pub fn get_authority_key_pair() -> (MySoAddress, AuthorityKeyPair) {
    get_key_pair()
}

/// Generate a keypair from the specified RNG (useful for testing with seedable rngs).
pub fn get_key_pair_from_rng<KP: KeypairTraits, R>(csprng: &mut R) -> (MySoAddress, KP)
where
...
// TODO: C-GETTER
pub fn get_key_pair_from_bytes<KP: KeypairTraits>(bytes: &[u8]) -> MySoResult<(MySoAddress, KP)>
where
    <KP as KeypairTraits>::PubKey: MySoPublicKey,
{
    ...
    if bytes.len() != priv_length + pub_key_length {
        return Err(MySoErrorKind::KeyConversionError(format!(
    ...
    let sk = <KP as KeypairTraits>::PrivKey::from_bytes(
        bytes
            .get(..priv_length)
            .ok_or(MySoErrorKind::InvalidPrivateKey)?,
    )
    .map_err(|_| MySoErrorKind::InvalidPrivateKey)?;
    let kp: KP = sk.into();
...
pub enum Signature {
    Ed25519MySoSignature,
    Secp256k1MySoSignature,
    Secp256r1MySoSignature,
}
...
impl AsRef<[u8]> for Signature {
    fn as_ref(&self) -> &[u8] {
        match self {
            Signature::Ed25519MySoSignature(sig) => sig.as_ref(),
            Signature::Secp256k1MySoSignature(sig) => sig.as_ref(),
            Signature::Secp256r1MySoSignature(sig) => sig.as_ref(),
        }
    }
}
impl AsMut<[u8]> for Signature {
    fn as_mut(&mut self) -> &mut [u8] {
        match self {
            Signature::Ed25519MySoSignature(sig) => sig.as_mut(),
            Signature::Secp256k1MySoSignature(sig) => sig.as_mut(),
            Signature::Secp256r1MySoSignature(sig) => sig.as_mut(),
        }
    }
}
...
impl ToFromBytes for Signature {
    fn from_bytes(bytes: &[u8]) -> Result<Self, FastCryptoError> {
        match bytes.first() {
            Some(x) => {
                if x == &Ed25519MySoSignature::SCHEME.flag() {
                    Ok(<Ed25519MySoSignature as ToFromBytes>::from_bytes(bytes)?.into())
                } else if x == &Secp256k1MySoSignature::SCHEME.flag() {
                    Ok(<Secp256k1MySoSignature as ToFromBytes>::from_bytes(bytes)?.into())
                } else if x == &Secp256r1MySoSignature::SCHEME.flag() {
                    Ok(<Secp256r1MySoSignature as ToFromBytes>::from_bytes(bytes)?.into())
                } else {
                    Err(FastCryptoError::InvalidInput)
                }
            }
...
impl MySoPublicKey for BLS12381PublicKey {
    const SIGNATURE_SCHEME: SignatureScheme = SignatureScheme::BLS12381;
}

//
// Ed25519 MySo Signature port
//
...
#[as_ref(forward)]
#[as_mut(forward)]
pub struct Ed25519MySoSignature(
    #[schemars(with = "Base64")]
    #[serde_as(as = "Readable<Base64, Bytes>")]
...
impl Default for Ed25519MySoSignature {
    fn default() -> Self {
        Self([0; Ed25519PublicKey::LENGTH + Ed25519Signature::LENGTH + 1])
    }
}

impl MySoSignatureInner for Ed25519MySoSignature {
...
impl MySoPublicKey for Ed25519PublicKey {
    const SIGNATURE_SCHEME: SignatureScheme = SignatureScheme::ED25519;
}

impl ToFromBytes for Ed25519MySoSignature {
    fn from_bytes(bytes: &[u8]) -> Result<Self, FastCryptoError> {
...
impl Signer<Signature> for Ed25519KeyPair {
    fn sign(&self, msg: &[u8]) -> Signature {
        Ed25519MySoSignature::new(self, msg).into()
    }
}

//
// Secp256k1 MySo Signature port
//
#[serde_as]
#[derive(Clone, Debug, Serialize, Deserialize, JsonSchema, PartialEq, Eq, Hash, AsRef, AsMut)]
#[as_ref(forward)]
#[as_mut(forward)]
pub struct Secp256k1MySoSignature(
    #[schemars(with = "Base64")]
    #[serde_as(as = "Readable<Base64, Bytes>")]
    [u8; Secp256k1PublicKey::LENGTH + Secp256k1Signature::LENGTH + 1],
);

impl MySoSignatureInner for Secp256k1MySoSignature {
...
impl MySoPublicKey for Secp256k1PublicKey {
    const SIGNATURE_SCHEME: SignatureScheme = SignatureScheme::Secp256k1;
}

impl ToFromBytes for Secp256k1MySoSignature {
    fn from_bytes(bytes: &[u8]) -> Result<Self, FastCryptoError> {
        if bytes.len() != Self::LENGTH {
            return Err(FastCryptoError::InputLengthWrong(Self::LENGTH));
        }
...
impl Signer<Signature> for Secp256k1KeyPair {
    fn sign(&self, msg: &[u8]) -> Signature {
        Secp256k1MySoSignature::new(self, msg).into()
    }
}

//
// Secp256r1 MySo Signature port
//
#[serde_as]
#[derive(Clone, Debug, Serialize, Deserialize, JsonSchema, PartialEq, Eq, Hash, AsRef, AsMut)]
#[as_ref(forward)]
#[as_mut(forward)]
pub struct Secp256r1MySoSignature(
    #[schemars(with = "Base64")]
    #[serde_as(as = "Readable<Base64, Bytes>")]
    [u8; Secp256r1PublicKey::LENGTH + Secp256r1Signature::LENGTH + 1],
);

impl MySoSignatureInner for Secp256r1MySoSignature {
...
impl MySoPublicKey for Secp256r1PublicKey {
    const SIGNATURE_SCHEME: SignatureScheme = SignatureScheme::Secp256r1;
}

impl ToFromBytes for Secp256r1MySoSignature {
    fn from_bytes(bytes: &[u8]) -> Result<Self, FastCryptoError> {
        if bytes.len() != Self::LENGTH {
            return Err(FastCryptoError::InputLengthWrong(Self::LENGTH));
        }
...
impl Signer<Signature> for Secp256r1KeyPair {
    fn sign(&self, msg: &[u8]) -> Signature {
        Secp256r1MySoSignature::new(self, msg).into()
    }
}
...
//
pub trait MySoSignatureInner: Sized + ToFromBytes + PartialEq + Eq + Hash {
    type Sig: Authenticator<PubKey = Self::PubKey>;
    type PubKey: VerifyingKey<Sig = Self::Sig> + MySoPublicKey;
    ...
    fn get_verification_inputs(&self) -> MySoResult<(Self::Sig, Self::PubKey)> {
        let pk = Self::PubKey::from_bytes(self.public_key_bytes())
            .map_err(|_| MySoErrorKind::KeyConversionError("Invalid public key".to_string()))?;
    ...
    fn new(kp: &Self::KeyPair, message: &[u8]) -> Self {
        ...
        signature_bytes
            .extend_from_slice(&[<Self::PubKey as MySoPublicKey>::SIGNATURE_SCHEME.flag()]);
...
pub trait MySoPublicKey: VerifyingKey {
    const SIGNATURE_SCHEME: SignatureScheme;
}

#[enum_dispatch(Signature)]
pub trait MySoSignature: Sized + ToFromBytes {
...
impl<S: MySoSignatureInner + Sized> MySoSignature for S {
    fn signature_bytes(&self) -> &[u8] {
        ...
        T: Serialize,
    {
        ...
        match scheme {
            ...
            _ => {
                let address = MySoAddress::from(pk);
                if author != address {
                    return Err(MySoErrorKind::IncorrectSigner {
                        error: format!(
...
impl AuthoritySignInfoTrait for AuthoritySignInfo {
    ...
    fn add_to_verification_obligation<'a>(
        ...
        obligation
            ...
            .ok_or(MySoErrorKind::InvalidAddress)?
...
#[serde_as]
#[derive(Clone, Debug, Serialize, Deserialize, JsonSchema)]
pub struct MySoAuthorityStrongQuorumSignInfo {
    pub epoch: EpochId,
...
impl From<&AuthorityStrongQuorumSignInfo> for MySoAuthorityStrongQuorumSignInfo {
...
impl TryFrom<&MySoAuthorityStrongQuorumSignInfo> for AuthorityStrongQuorumSignInfo {
    type Error = FastCryptoError;

    fn try_from(info: &MySoAuthorityStrongQuorumSignInfo) -> Result<Self, Self::Error> {
        Ok(Self {
            epoch: info.epoch,
    ...
    for AuthorityQuorumSignInfo<STRONG_THRESHOLD>
{
    ...
    fn add_to_verification_obligation<'a>(
        ...
        obligation
            .signatures
            .get_mut(message_index)
            .ok_or(MySoErrorKind::InvalidAuthenticator)?
        ...
        for authority_index in self.signers_map.iter() {
            ...
            let authority = committee
                ...
                .ok_or_else(|| MySoErrorKind::UnknownSigner {
                    signer: None,
        ...
        fp_ensure!(
            ...
            MySoErrorKind::CertificateRequiresQuorum.into()
        );
...
impl<const STRONG_THRESHOLD: bool> AuthorityQuorumSignInfo<STRONG_THRESHOLD> {
    ...
    pub fn authorities<'a>(
        &'a self,
        committee: &'a Committee,
    ) -> impl Iterator<Item = MySoResult<&'a AuthorityName>> {
        self.signers_map.iter().map(|i| {
            committee
                .authority_by_index(i)
                .ok_or(MySoErrorKind::InvalidAuthenticator.into())
        })
    }
...
/// * We use `BCS` to generate canonical bytes mysotable for hashing and signing.
///
/// # Safety
...
impl<'a> VerificationObligation<'a> {
    ...
    pub fn verify_all(self) -> MySoResult<()> {
...
pub enum SignatureScheme {
    ...
    Secp256k1,
    Secp256r1,
    BLS12381, // This is currently not supported for user MySo Address.
    MultiSig,
...
impl SignatureScheme {
    pub fn flag(&self) -> u8 {
        match self {
            ...
            SignatureScheme::BLS12381 => 0x04, // This is currently not supported for user MySo Address.
            SignatureScheme::ZkLoginAuthenticator => 0x05,
            SignatureScheme::PasskeyAuthenticator => 0x06,
        }
    }

    pub fn from_flag(flag: &str) -> Result<SignatureScheme, MySoError> {
        let byte_int = flag
            .parse::<u8>()
            .map_err(|_| MySoErrorKind::KeyConversionError("Invalid key scheme".to_string()))?;
        Self::from_flag_byte(&byte_int)
    }

    pub fn from_flag_byte(byte_int: &u8) -> Result<SignatureScheme, MySoError> {
        match byte_int {
            ...
            _ => Err(MySoErrorKind::KeyConversionError("Invalid key scheme".to_string()).into()),
        }
    }
}
