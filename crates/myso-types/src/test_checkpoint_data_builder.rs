...
pub struct TestCheckpointBuilder {
    ...
    gas_map: HashMap<MySoAddress, ObjectID>,
...
impl TestCheckpointBuilder {
    ...
    /// A proper MySoAddress will be derived from it.
    ...
    /// The object will be created as a MYSO coin object, with default balance,
    /// and the transaction sender as its owner.
    pub fn create_owned_object(self, object_idx: u64) -> Self {
        self.create_myso_object(object_idx, GAS_VALUE_FOR_TESTING)
    }
    ...
    /// Create a new MYSO coin object in the transaction.
    /// `object_idx` is a convenient representation of the object's ID.
    /// `balance` is the amount of MYSO to be created.
    pub fn create_myso_object(self, object_idx: u64, balance: u64) -> Self {
        let sender_idx = self
            .checkpoint_builder
            .next_transaction
            .as_ref()
    ...
    pub fn advance_epoch(
        ...
        let events = if !safe_mode {
            ...
            let struct_tag = StructTag {
                ...
                module: ident_str!("myso_system_state_inner").to_owned(),
                name: ident_str!("SystemEpochInfoEvent").to_owned(),
                type_params: vec![],
            };
            Some(vec![Event::new(
                &MYSO_SYSTEM_ADDRESS,
                ident_str!("myso_system_state_inner"),
    ...
    pub fn derive_address(address_idx: u8) -> MySoAddress {
