...
impl AuthenticatorTrait for MultiSig {
        ...
        T: Serialize,
    {
        ...
        if verify_params.validate_zklogin_public_identifier {
            for (pk, _weight) in &self.multisig_pk.pk_map {
                if let PublicKey::ZkLogin(zklogin_pk) = pk {
                    zklogin_pk
                        .validate()
                        .map_err(|e| MySoErrorKind::InvalidSignature {
        ...
        if MySoAddress::from(&self.multisig_pk) != multisig_address {
        ...
        for (sig, i) in self.sigs.iter().zip(as_indices(self.bitmap)?) {
            let (subsig_pubkey, weight) =
                self.multisig_pk
                    .pk_map
                    .get(i as usize)
                    .ok_or(MySoErrorKind::InvalidSignature {
            ...
            let res = match sig {
                CompressedSignature::Ed25519(s) => {
                        ...
                        && !matches!(subsig_pubkey.scheme(), SignatureScheme::ED25519)
                    {
                        return Err(MySoErrorKind::InvalidSignature {
                            error: format!(
                                ...
                                MySoAddress::from(subsig_pubkey)
                            ),
                        }.into());
                    }
                    let pk =
                    ...
                    pk.verify(
                        &digest,
                        &s.try_into().map_err(|_| MySoErrorKind::InvalidSignature {
        ...
        if weight_sum >= self.multisig_pk.threshold {
            ...
            Err(MySoErrorKind::InvalidSignature {
                error: format!(
...
pub fn as_indices(bitmap: u16) -> Result<Vec<u8>, MySoError> {
...
impl MultiSig {
    ...
    pub fn combine(
        ...
        for s in full_sigs {
            ...
            let index = multisig_pk
                .get_index(&pk)
                .ok_or(MySoErrorKind::IncorrectSigner {
    ...
    pub fn get_zklogin_sigs(&self) -> Result<Vec<ZkLoginAuthenticator>, MySoError> {
    ...
    pub fn get_indices(&self) -> Result<Vec<u8>, MySoError> {
...
impl FromStr for MultiSig {
    ...
    fn from_str(s: &str) -> Result<Self, Self::Err> {
        let bytes = Base64::decode(s).map_err(|_| MySoErrorKind::InvalidSignature {
            error: "Invalid base64 string".to_string(),
        })?;
        let sig = MultiSig::from_bytes(&bytes).map_err(|_| MySoErrorKind::InvalidSignature {
