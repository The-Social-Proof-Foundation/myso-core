...
pub enum UserInputError {
    ...
    #[error(
        "MYSO payment transactions use first input coin for gas payment, but found a different gas object"
    )]
...
pub enum MySoObjectResponseError {
    ...
    // TODO: also integrate MySoPastObjectResponse (VersionNotFound,  VersionTooHigh)
}

/// Custom error type for MySo.
#[derive(Eq, PartialEq, Clone, Serialize, Deserialize, Error, Hash)]
#[error(transparent)]
pub struct MySoError(#[from] pub Box<MySoErrorKind>);

/// Custom error type for MySo.
#[derive(
    Eq, PartialEq, Clone, Debug, Serialize, Deserialize, Error, Hash, AsRefStr, IntoStaticStr,
)]
pub enum MySoErrorKind {
    ...
    MySoObjectResponseError { error: MySoObjectResponseError },
    ...
    TooManyIncorrectAuthorities {
        errors: Vec<(AuthorityName, MySoError)>,
        action: String,
    },
    ...
    #[error("Failed to execute handle_consensus_transaction on MySo: {0}")]
...
pub type MySoResult<T = ()> = Result<T, MySoError>;
pub type UserInputResult<T = ()> = Result<T, UserInputError>;

impl From<MySoErrorKind> for MySoError {
    fn from(error: MySoErrorKind) -> Self {
        MySoError(Box::new(error))
    }
}

impl std::ops::Deref for MySoError {
    type Target = MySoErrorKind;
...
impl From<myso_protocol_config::Error> for MySoError {
    fn from(error: myso_protocol_config::Error) -> Self {
        MySoErrorKind::WrongMessageVersion { error: error.0 }.into()
    }
}

impl From<ExecutionError> for MySoError {
    fn from(error: ExecutionError) -> Self {
        MySoErrorKind::ExecutionError(error.to_string()).into()
    }
}

impl From<Status> for MySoError {
    fn from(status: Status) -> Self {
        if status.message() == "Too many requests" {
            return MySoErrorKind::TooManyRequests.into();
        }

        let result = bcs::from_bytes::<MySoError>(status.details());
        if let Ok(myso_error) = result {
            myso_error
        } else {
            MySoErrorKind::RpcError(
...
impl From<TypedStoreError> for MySoError {
    fn from(e: TypedStoreError) -> Self {
        MySoErrorKind::Storage(e.to_string()).into()
    }
}

impl From<crate::storage::error::Error> for MySoError {
    fn from(e: crate::storage::error::Error) -> Self {
        MySoErrorKind::Storage(e.to_string()).into()
    }
}

impl From<MySoErrorKind> for Status {
    fn from(error: MySoErrorKind) -> Self {
...
impl From<MySoError> for Status {
    fn from(error: MySoError) -> Self {
        Status::from(error.into_inner())
    }
}

impl From<ExecutionErrorKind> for MySoError {
    fn from(kind: ExecutionErrorKind) -> Self {
        ExecutionError::from_kind(kind).into()
    }
}

impl From<&str> for MySoError {
...
impl From<String> for MySoError {
    fn from(error: String) -> Self {
        MySoErrorKind::GenericAuthorityError { error }.into()
    }
}

impl TryFrom<MySoErrorKind> for UserInputError {
    type Error = anyhow::Error;

    fn try_from(err: MySoErrorKind) -> Result<Self, Self::Error> {
        match err {
            MySoErrorKind::UserInputError { error } => Ok(error),
            other => anyhow::bail!("error {:?} is not UserInputError", other),
        }
    }
}

impl TryFrom<MySoError> for UserInputError {
    type Error = anyhow::Error;

    fn try_from(err: MySoError) -> Result<Self, Self::Error> {
        err.into_inner().try_into()
    }
}

impl From<UserInputError> for MySoError {
    fn from(error: UserInputError) -> Self {
        MySoErrorKind::UserInputError { error }.into()
    }
}

impl From<MySoObjectResponseError> for MySoError {
    fn from(error: MySoObjectResponseError) -> Self {
        MySoErrorKind::MySoObjectResponseError { error }.into()
    }
}

impl PartialEq<MySoErrorKind> for MySoError {
    fn eq(&self, other: &MySoErrorKind) -> bool {
        &*self.0 == other
    }
}

impl PartialEq<MySoError> for MySoErrorKind {
    fn eq(&self, other: &MySoError) -> bool {
        self == &*other.0
    }
}

impl MySoError {
    pub fn as_inner(&self) -> &MySoErrorKind {
        &self.0
    }

    pub fn into_inner(self) -> MySoErrorKind {
        *self.0
    }
}
...
impl MySoErrorKind {
    ...
    pub fn to_variant_name(&self) -> &'static str {
        match &self {
            MySoErrorKind::UserInputError { error } => error.into(),
            _ => self.into(),
        }
    }
    ...
    pub fn individual_error_indicates_epoch_change(&self) -> bool {
        matches!(
            self,
            MySoErrorKind::ValidatorHaltedAtEpochEnd | MySoErrorKind::MissingCommitteeAtEpoch(_)
        )
    }
    ...
    pub fn is_retryable(&self) -> (bool, bool) {
        let retryable = match self {
            // Network error
            MySoErrorKind::RpcError { .. } => true,

            // Reconfig error
            MySoErrorKind::ValidatorHaltedAtEpochEnd => true,
            MySoErrorKind::MissingCommitteeAtEpoch(..) => true,
            MySoErrorKind::WrongEpoch { .. } => true,
            MySoErrorKind::EpochEnded(..) => true,

            MySoErrorKind::UserInputError { error } => {
                match error {
            ...
            MySoErrorKind::PotentiallyTemporarilyInvalidSignature { .. } => true,

            // Overload errors
            MySoErrorKind::TooManyTransactionsPendingExecution { .. } => true,
            MySoErrorKind::TooManyTransactionsPendingOnObject { .. } => true,
            MySoErrorKind::TooOldTransactionPendingOnObject { .. } => true,
            MySoErrorKind::TooManyTransactionsPendingConsensus => true,
            MySoErrorKind::ValidatorOverloadedRetryAfter { .. } => true,

            // Non retryable error
            MySoErrorKind::ExecutionError(..) => false,
            MySoErrorKind::ByzantineAuthoritySuspicion { .. } => false,
            MySoErrorKind::QuorumFailedToGetEffectsQuorumWhenProcessingTransaction { .. } => false,
            MySoErrorKind::TxAlreadyFinalizedWithDifferentUserSigs => false,
            MySoErrorKind::FailedToVerifyTxCertWithExecutedEffects { .. } => false,
            MySoErrorKind::ObjectLockConflict { .. } => false,
            ...
            MySoErrorKind::TooManyRequests => false,
    ...
    pub fn is_overload(&self) -> bool {
        matches!(
            self,
            MySoErrorKind::TooManyTransactionsPendingExecution { .. }
                | MySoErrorKind::TooManyTransactionsPendingOnObject { .. }
                | MySoErrorKind::TooOldTransactionPendingOnObject { .. }
                | MySoErrorKind::TooManyTransactionsPendingConsensus
        )
    }

    pub fn is_retryable_overload(&self) -> bool {
        matches!(self, MySoErrorKind::ValidatorOverloadedRetryAfter { .. })
    }

    pub fn retry_after_secs(&self) -> u64 {
        match self {
            MySoErrorKind::ValidatorOverloadedRetryAfter { retry_after_secs } => *retry_after_secs,
            _ => 0,
        }
    }

    /// Categorizes MySoError into ErrorCategory.
    pub fn categorize(&self) -> ErrorCategory {
        match self {
            ...
            | MySoErrorKind::SignerSignatureAbsent { .. }
            | MySoErrorKind::SignerSignatureNumberMismatch { .. }
            | MySoErrorKind::IncorrectSigner { .. }
            | MySoErrorKind::UnknownSigner { .. }
            | MySoErrorKind::TransactionExpired => ErrorCategory::InvalidTransaction,

            MySoErrorKind::ObjectLockConflict { .. } => ErrorCategory::LockConflict,

            MySoErrorKind::Unknown { .. }
            | MySoErrorKind::GrpcMessageSerializeError { .. }
            | MySoErrorKind::GrpcMessageDeserializeError { .. }
            | MySoErrorKind::ByzantineAuthoritySuspicion { .. }
            | MySoErrorKind::InvalidTxKindInSoftBundle
            | MySoErrorKind::UnsupportedFeatureError { .. }
            | MySoErrorKind::InvalidRequest { .. } => ErrorCategory::Internal,

            MySoErrorKind::TooManyTransactionsPendingExecution { .. }
            | MySoErrorKind::TooManyTransactionsPendingOnObject { .. }
            | MySoErrorKind::TooOldTransactionPendingOnObject { .. }
            | MySoErrorKind::TooManyTransactionsPendingConsensus
            | MySoErrorKind::ValidatorOverloadedRetryAfter { .. } => {
                ErrorCategory::ValidatorOverloaded
            }

            MySoErrorKind::TimeoutError => ErrorCategory::Unavailable,
...
impl Ord for MySoError {
    fn cmp(&self, other: &Self) -> std::cmp::Ordering {
        Ord::cmp(self.as_ref(), other.as_ref())
    }
}

impl PartialOrd for MySoError {
    fn partial_cmp(&self, other: &Self) -> Option<std::cmp::Ordering> {
        Some(self.cmp(other))
    }
}

impl std::fmt::Debug for MySoError {
