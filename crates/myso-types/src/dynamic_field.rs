...
/// Rust version of the Move myso::dynamic_field::Field type
...
/// Rust version of the Move myso::dynamic_object_field::Wrapper type
...
pub struct DynamicFieldName {
    ...
    #[serde_as(as = "Readable<MySoTypeTag, _>")]
    pub type_: TypeTag,
...
impl DynamicFieldInfo {
    ...
    pub fn try_extract_field_name(tag: &StructTag, type_: &DynamicFieldType) -> MySoResult<TypeTag> {
        match (type_, tag.type_params.first()) {
            ...
            (DynamicFieldType::DynamicObject, Some(TypeTag::Struct(s))) => Ok(s
                .type_params
                .first()
                .ok_or_else(|| MySoErrorKind::ObjectDeserializationError {
    ...
    pub fn try_extract_field_value(tag: &StructTag) -> MySoResult<TypeTag> {
        match tag.type_params.last() {
            Some(value_type) => Ok(value_type.clone()),
            None => Err(MySoErrorKind::ObjectDeserializationError {
    ...
    pub fn parse_move_object(
    ...
    ) -> MySoResult<(MoveValue, DynamicFieldType, ObjectID)> {
        let name = extract_field_from_move_struct(move_struct, "name").ok_or_else(|| {
            MySoErrorKind::ObjectDeserializationError {
                error: "Cannot extract [name] field from myso::dynamic_field::Field".to_string(),
            }
        })?;

        let value = extract_field_from_move_struct(move_struct, "value").ok_or_else(|| {
            MySoErrorKind::ObjectDeserializationError {
                error: "Cannot extract [value] field from myso::dynamic_field::Field".to_string(),
            }
        })?;
        ...
        Ok(if is_dynamic_object(move_struct) {
            ...
            .ok_or_else(|| MySoErrorKind::ObjectDeserializationError {
                error: "Cannot extract [name] field from myso::dynamic_object_field::Wrapper."
                    .to_string(),
            })?;
            ...
            let object_id = extract_id_value(value).ok_or_else(|| {
                MySoErrorKind::ObjectDeserializationError {
                    error: format!(
                        ...
                        myso::dynamic_field::Field, {value:?}"
                    ),
                }
            })?;
            ...
            let object_id = extract_object_id(move_struct).ok_or_else(|| {
                MySoErrorKind::ObjectDeserializationError {
                    error: format!(
                        "Cannot extract dynamic object's object id from \
                        myso::dynamic_field::Field, {move_struct:?}",
                    ),
                }
            })?;
...
where
    T: Into<MySoAddress>,
{
    let parent: MySoAddress = parent.into();
...
pub fn serialize_dynamic_field<K, V>(id: &UID, name: &K, value: V) -> Result<Vec<u8>, MySoError>
where
    K: Serialize + Clone,
    V: Serialize,
{
    ...
    K: Serialize + fmt::Debug,
{
    ...
    ) -> Result<BoundedDynamicFieldID<K>, MySoError> {
        let id = self.object_id()?;
    ...
    pub fn into_field<V>(self, value: V) -> Result<DynamicField<K, V>, MySoError>
    where
    ...
    V: Serialize,
{
    ...
    pub fn into_move_object_unsafe_for_testing(
        ...
        let field_bytes =
            bcs::to_bytes(&field).map_err(|e| MySoErrorKind::ObjectSerializationError {
                error: e.to_string(),
            })?;
        Ok(unsafe {
    ...
    K: Serialize + std::fmt::Debug,
{
    ...
    pub fn expect_object(
        ...
        key: &K,
        object_store: &dyn ObjectStore,
    ) -> Result<DynamicFieldObject<K>, MySoError> {
        let parent = self.0;
        let id = self.1;
    ...
    K: Serialize,
{
    ...
    ) -> Result<Option<DynamicFieldObject<K>>, MySoError> {
    ...
    K: Serialize + DeserializeOwned,
{
    ...
    pub fn load_value<V>(self) -> Result<V, MySoError>
    where
        V: Serialize + DeserializeOwned,
    {
    ...
    /// Deserialize the field value from the object. Requires that the value type is known.
    pub fn load_field<V>(self) -> Result<Field<K, V>, MySoError>
    where
        V: Serialize + DeserializeOwned,
    {
        let object = self.0;
        ...
        bcs::from_bytes::<Field<K, V>>(move_object.contents())
            .map_err(|err| MySoErrorKind::DynamicFieldReadError(err.to_string()).into())
    }
}
