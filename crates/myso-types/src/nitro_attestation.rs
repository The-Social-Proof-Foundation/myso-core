...
impl From<NitroAttestationVerifyError> for MySoError {
    fn from(err: NitroAttestationVerifyError) -> Self {
        MySoErrorKind::NitroAttestationFailedToVerify(err.to_string()).into()
    }
}
...
) -> MySoResult<(Vec<u8>, Vec<u8>, AttestationDocument)> {
...
impl CoseSign1 {
    ...
    fn to_signed_message(&self) -> MySoResult<Vec<u8>> {
        ...
        ciborium::ser::into_writer(&value, &mut bytes).map_err(|_| {
            MySoErrorKind::NitroAttestationFailedToVerify("cannot parse message".to_string())
        })?;
        Ok(bytes)
    }
}
