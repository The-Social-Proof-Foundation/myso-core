...
use crate::error::{ErrorCategory, MySoError};
...
pub enum TransactionSubmissionError {
    ...
    TransactionDriverInternalError(MySoError),
    #[error("Invalid user signature: {0}.")]
    InvalidUserSignature(MySoError),
    #[error(
...
pub type GroupedErrors = Vec<(MySoError, StakeUnit, Vec<ConciseAuthorityPublicKeyBytes>)>;
