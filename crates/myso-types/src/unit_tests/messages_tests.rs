...
use crate::crypto::{
    ...
    MySoAuthoritySignature, get_key_pair,
};
...
fn test_new_with_signatures() {
    ...
    assert_eq!(
        quorum
            ...
            .collect::<MySoResult<Vec<&AuthorityName>>>()
            .unwrap(),
...
fn test_handle_reject_malicious_signature() {
        ...
        AuthorityStrongQuorumSignInfo::new_from_auth_sign_infos(signatures, &committee).unwrap();
    {
        ...
        let sig = AuthoritySignature::new_secure(
            &IntentMessage::new(Intent::myso_transaction(), message.clone()),
            &committee.epoch,
            &sec,
        );
...
fn test_sponsored_transaction_message() {
    let epoch = 0;
    let sender_kp = MySoKeyPair::Ed25519(get_key_pair().1);
    let sender = (&sender_kp.public()).into();
    let sponsor_kp = MySoKeyPair::Ed25519(get_key_pair().1);
    ...
    let third_party_kp = MySoKeyPair::Ed25519(get_key_pair().1);
...
fn test_sponsored_transaction_validity_check() {
    ...
    let pt = {
        let mut builder = ProgrammableTransactionBuilder::new();
        builder.transfer_myso(MySoAddress::random_for_testing_only(), Some(50000));
        builder.finish()
    };
    let kind = TransactionKind::programmable(pt);
    TransactionData::new_with_gas_data(kind, sender, gas_data.clone())
    ...
    let pt = {
        let mut builder = ProgrammableTransactionBuilder::new();
        builder.pay_myso(vec![], vec![]).unwrap();
        builder.finish()
    };
    let kind = TransactionKind::programmable(pt);
    TransactionData::new_with_gas_data(kind, sender, gas_data.clone())
    ...
    let pt = {
        let mut builder = ProgrammableTransactionBuilder::new();
        builder.pay_all_myso(MySoAddress::random_for_testing_only());
        builder.finish()
    };
    let kind = TransactionKind::programmable(pt);
...
fn verify_sender_signature_correctly_with_flag() {
    ...
    let receiver_kp = MySoKeyPair::Secp256k1(get_key_pair().1);
    let receiver_address = (&receiver_kp.public()).into();

    // create a sender keypair with Secp256k1
    let sender_kp = MySoKeyPair::Secp256k1(get_key_pair().1);
    ...
    let sender_kp_2 = MySoKeyPair::Ed25519(get_key_pair().1);
    let mut tx_data_2 = tx_data.clone();
    *tx_data_2.sender_mut_for_testing() = (&sender_kp_2.public()).into();
    ...
    let sender_kp_3 = MySoKeyPair::Secp256r1(get_key_pair().1);
    ...
    *tx_data_3.sender_mut_for_testing() = (&sender_kp_3.public()).into();
    ...
    assert_eq!(s.scheme().flag(), Secp256k1MySoSignature::SCHEME.flag());
    ...
    assert_eq!(s.scheme().flag(), Ed25519MySoSignature::SCHEME.flag());
