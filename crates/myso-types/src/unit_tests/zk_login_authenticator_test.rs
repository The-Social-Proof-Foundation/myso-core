...
use crate::{
    base_types::MySoAddress, signature::GenericSignature, zk_login_util::DEFAULT_JWK_BYTES,
};
...
fn test_serde_zk_login_signature() {
    ...
    let addr: MySoAddress = (&authenticator).try_into().unwrap();
...
fn test_serde_zk_public_identifier() {
    ...
    assert_eq!(
        MySoAddress::try_from_unpadded(&modified_inputs).unwrap(),
        MySoAddress::from(&pk1)
    );
    ...
    assert_eq!(
        MySoAddress::try_from_padded(&modified_inputs).unwrap(),
        MySoAddress::from(&pk2)
    );
