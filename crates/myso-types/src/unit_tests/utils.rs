...
pub fn make_transaction_data(sender: MySoAddress) -> TransactionData {
    let object = Object::immutable_with_id_for_testing(ObjectID::random_from_rng(
        &mut StdRng::from_seed([0; 32]),
    ));
    let pt = {
        let mut builder = ProgrammableTransactionBuilder::new();
        builder.transfer_myso(dbg_addr(2), None);
        builder.finish()
    };
...
pub fn make_transaction(sender: MySoAddress, kp: &MySoKeyPair) -> Transaction {
...
mod zk_login {
    ...
    pub fn get_zklogin_user_address() -> MySoAddress {
        thread_local! {
            static USER_ADDRESS: MySoAddress = {
                ...
                MySoAddress::from_bytes(hasher.finalize().digest).unwrap()
            };
        }
        USER_ADDRESS.with(|a| *a)
    }

    fn get_zklogin_user_key() -> MySoKeyPair {
    ...
    pub fn get_legacy_zklogin_user_address() -> MySoAddress {
        thread_local! {
            static USER_ADDRESS: MySoAddress = {
                let inputs = get_inputs_with_bad_address_seed();
                MySoAddress::from(&PublicKey::from_zklogin_inputs(&inputs).unwrap())
            };
        }
        USER_ADDRESS.with(|a| *a)
    }

    pub fn sign_zklogin_personal_msg(data: PersonalMessage) -> (MySoAddress, GenericSignature) {
        let inputs = get_zklogin_inputs();
    ...
    pub fn sign_zklogin_tx_with_default_proof(
        data: TransactionData,
        legacy: bool,
    ) -> (MySoAddress, Transaction, GenericSignature) {
    ...
    pub fn keys() -> Vec<MySoKeyPair> {
        let mut seed = StdRng::from_seed([0; 32]);
        let kp1: MySoKeyPair = MySoKeyPair::Ed25519(get_key_pair_from_rng(&mut seed).1);
        let kp2: MySoKeyPair = MySoKeyPair::Secp256k1(get_key_pair_from_rng(&mut seed).1);
        let kp3: MySoKeyPair = MySoKeyPair::Secp256r1(get_key_pair_from_rng(&mut seed).1);
        vec![kp1, kp2, kp3]
    }
    ...
    pub fn make_upgraded_multisig_tx() -> Transaction {
        ...
        .unwrap();
        let addr = MySoAddress::from(&multisig_pk);
        let tx = make_transaction(addr, &keys[0]);

        let msg = IntentMessage::new(Intent::myso_transaction(), tx.transaction_data().clone());
