...
#[test]
fn test_myso_balance_accumulator_field_recognition() {
    // Create a Field<Key<Balance<MYSO>>, U128> type
    let myso_balance = Balance::type_tag(GAS::type_tag());
    let key_type = AccumulatorKey::get_type_tag(&[myso_balance]);
    ...
    assert!(move_type.is_myso_balance_accumulator_field());
...
#[test]
fn test_non_myso_balance_accumulator_field_recognition() {
    ...
    // Should be recognized as a balance accumulator field, but not MYSO
    assert!(move_type.is_balance_accumulator_field());
    assert!(!move_type.is_myso_balance_accumulator_field());
...
fn test_accumulator_storage_savings() {
    let owner = MySoAddress::random_for_testing_only();
    let balance = 1000u64;

    // Create the field struct tag for Field<Key<Balance<MYSO>>, U128>
    let myso_balance = Balance::type_tag(GAS::type_tag());
    let key_type = AccumulatorKey::get_type_tag(&[myso_balance]);
    let u128_type = U128::get_type_tag();
    ...
    assert!(
        efficient_move_object
            .type_()
            .is_myso_balance_accumulator_field()
    );
}
...
fn test_accumulator_field_wrong_value_type_not_u128() {
    // Field<Key<Balance<MYSO>>, SomeOtherType> should NOT be recognized
    let myso_balance = Balance::type_tag(GAS::type_tag());
    let key_type = AccumulatorKey::get_type_tag(&[myso_balance]);
...
fn test_accumulator_field_key_with_multiple_type_params() {
    // Key<Balance<MYSO>, ExtraParam> should not be recognized
    let myso_balance = Balance::type_tag(GAS::type_tag());
    ...
    let key_type = TypeTag::Struct(Box::new(StructTag {
        ...
        module: "accumulator".parse().unwrap(),
        name: "Key".parse().unwrap(),
        type_params: vec![myso_balance, extra_param],
    }));
    let u128_type = U128::get_type_tag();
...
fn test_accumulator_field_with_nested_balance_type() {
    // Balance<Balance<MYSO>> - nested balance should still work correctly
    ...
    // Should recognize as accumulator field with Balance<Balance<MYSO>> as the extracted type
    assert!(move_type.is_balance_accumulator_field());
    let extracted = move_type.balance_accumulator_field_type_maybe().unwrap();
    // The extracted type should be Balance<MYSO>, which is the inner type of Balance<Balance<MYSO>>
...
#[test]
fn test_is_method_myso_balance_accumulator_field() {
    let myso_balance = Balance::type_tag(GAS::type_tag());
    let key_type = AccumulatorKey::get_type_tag(&[myso_balance]);
    let u128_type = U128::get_type_tag();
    let field_type = DynamicFieldInfo::dynamic_field_type(key_type, u128_type);
...
#[test]
fn test_is_method_non_myso_balance_accumulator_field() {
    let custom_token = TypeTag::Struct(Box::new(StructTag {
        address: MYSO_FRAMEWORK_ADDRESS,
        module: "test".parse().unwrap(),
        name: "CustomToken".parse().unwrap(),
    ...
    // Should not match MYSO accumulator field
    let myso_balance = Balance::type_tag(GAS::type_tag());
    let myso_key_type = AccumulatorKey::get_type_tag(&[myso_balance]);
    let myso_field_type = DynamicFieldInfo::dynamic_field_type(myso_key_type, U128::get_type_tag());
    assert!(!move_type.is(&myso_field_type));
