...
async fn create_credential_and_sign_test_tx(
    ...
    // Derives its myso address and make a test transaction with it as sender.
...
async fn test_passkey_fails_invalid_json() {
    ...
    let res: Result<PasskeyAuthenticator, MySoError> = raw.try_into();
    ...
    let res_2: Result<PasskeyAuthenticator, MySoError> = raw_2.try_into();
    assert!(res_2.is_err());
    ...
    let res_3: Result<PasskeyAuthenticator, MySoError> = raw_3.try_into();
    assert!(res_3.is_ok());
}
...
async fn test_real_passkey_output() {
    ...
    let address =
        MySoAddress::from_str("0x9c0c00e929f08431583dad0e9409b5afb20cdbae0043fa5577f2577dbe88a0db")
            .unwrap();
