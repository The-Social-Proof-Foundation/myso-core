...
#[test]
fn serde_pubkey() {
    let skp = MySoKeyPair::Ed25519(get_key_pair().1);
...
fn public_key_equality() {
    let ed_kp1: MySoKeyPair = MySoKeyPair::Ed25519(get_key_pair().1);
    let ed_kp2: MySoKeyPair = MySoKeyPair::Ed25519(get_key_pair().1);
    let k1_kp1: MySoKeyPair = MySoKeyPair::Secp256k1(get_key_pair().1);
    let k1_kp2: MySoKeyPair = MySoKeyPair::Secp256k1(get_key_pair().1);
...
fn test_proof_of_possession() {
    let address =
        MySoAddress::from_str("0x1a4623343cd42be47d67314fce0ad042f3c82685544bc91d8c11d24e74ba7357")
            .unwrap();
...
proptest! {
    ...
    fn test_authority_pk_bytes(
        ...
        let _mysosig = Ed25519MySoSignature::from_bytes(&bytes);
        let _mysosig = Secp256k1MySoSignature::from_bytes(&bytes);
    ...
    fn test_deserialize_keypair(
        ...
        let _skp: Result<MySoKeyPair, _> = bcs::from_bytes(&bytes);
