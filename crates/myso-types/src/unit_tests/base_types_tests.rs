...
#[test]
fn test_bcs_enum() {
    let address = Owner::AddressOwner(MySoAddress::random_for_testing_only());
...
fn test_signatures() {
    ...
    let foo = IntentMessage::new(Intent::myso_transaction(), Foo("hello".into()));
    let foox = IntentMessage::new(Intent::myso_transaction(), Foo("hellox".into()));
    let bar = IntentMessage::new(Intent::myso_transaction(), Bar("hello".into()));
...
fn test_signatures_serde() {
    ...
    let s = Signature::new_secure(&IntentMessage::new(Intent::myso_transaction(), foo), &sec1);
...
fn test_object_id_zero_padding() {
    ...
    assert_eq!(MYSO_FRAMEWORK_ADDRESS, obj_id_1.0);
    assert_eq!(MYSO_FRAMEWORK_ADDRESS, obj_id_2.0);
    assert_eq!(MYSO_FRAMEWORK_ADDRESS, obj_id_3.0);
    assert_eq!(MYSO_FRAMEWORK_ADDRESS, obj_id_4.0);
    assert_eq!(MYSO_FRAMEWORK_ADDRESS, obj_id_5.0);
    assert_eq!(MYSO_FRAMEWORK_ADDRESS, obj_id_6.0);
}

#[test]
fn test_address_display() {
    let hex = SAMPLE_ADDRESS;
    let id = MySoAddress::from_str(hex).unwrap();
    assert_eq!(format!("{:?}", id), format!("0x{hex}"));
}
...
fn test_address_serde_not_human_readable() {
    ...
    let deserialized: MySoAddress = bincode::deserialize(&serialized).unwrap();
...
fn test_address_serde_human_readable() {
    ...
    let deserialized: MySoAddress = serde_json::from_str(&serialized).unwrap();
    assert_eq!(deserialized, address);
}

#[test]
fn test_address_serde_with_expected_value() {
    let address = MySoAddress::try_from(SAMPLE_ADDRESS_VEC.to_vec()).unwrap();
...
fn test_authority_signature_serde_not_human_readable() {
    ...
    let sig = AuthoritySignature::new_secure(
        &IntentMessage::new(Intent::myso_transaction(), Foo("some data".to_string())),
        &0,
        &key,
    );
...
fn derive_sample_address() -> (MySoAddress, AccountKeyPair) {
...
fn move_object_type_consistency() {
    ...
    let ty = assert_consistent(&StakedMySo::type_());
    assert!(ty.is_staked_myso());
