...
use crate::{
    ...
    committee::EpochId,
    crypto::{
        AccountKeyPair, AuthorityKeyPair, AuthoritySignature, MySoAuthoritySignature,
        MySoSignature, Signature, SignatureScheme,
    },
    object::Object,
...
fn test_personal_message_intent() {
    ...
    let intent = Intent::myso_app(IntentScope::PersonalMessage);
...
fn test_authority_signature_intent() {
    ...
    let signature = Signature::new_secure(
        &IntentMessage::new(Intent::myso_transaction(), data.clone()),
        &sender_key,
    );
