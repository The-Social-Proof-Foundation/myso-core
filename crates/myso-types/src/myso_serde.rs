...
use crate::{
    DEEPBOOK_ADDRESS, MYSO_CLOCK_ADDRESS, MYSO_FRAMEWORK_ADDRESS, MYSO_SYSTEM_ADDRESS,
...
pub(crate) struct MySoBitmap;

impl SerializeAs<roaring::RoaringBitmap> for MySoBitmap {
...
impl<'de> DeserializeAs<'de, roaring::RoaringBitmap> for MySoBitmap {
    fn deserialize_as<D>(deserializer: D) -> Result<roaring::RoaringBitmap, D::Error>
    where
        D: Deserializer<'de>,
    {
        let bytes: Vec<u8> = Bytes::deserialize_as(deserializer)?;
        deserialize_myso_bitmap(&bytes).map_err(to_custom_deser_error::<'de, D, _>)
    }
}
...
pub(crate) fn deserialize_myso_bitmap(bytes: &[u8]) -> std::io::Result<roaring::RoaringBitmap> {
...
mod test {
    ...
    #[test]
    fn test_myso_bitmap_unique_deserialize() {
        ...
        let myso_bitmap = deserialize_myso_bitmap(&bytes[..]).unwrap();
        assert_eq!(myso_bitmap.len(), 1);
        let bitmap_values: Vec<u32> = myso_bitmap.iter().collect();
