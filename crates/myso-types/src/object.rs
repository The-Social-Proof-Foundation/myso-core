...
use crate::error::{
    ExecutionError, ExecutionErrorKind, MySoErrorKind, UserInputError, UserInputResult,
};
use crate::error::{MySoError, MySoResult};
...
impl MoveObject {
    ...
    pub fn get_layout(&self, resolver: &impl GetModule) -> Result<MoveStructLayout, MySoError> {
    ...
    pub fn to_move_struct(&self, layout: &MoveStructLayout) -> Result<MoveStruct, MySoError> {
    ...
    pub fn get_total_myso(&self, layout_resolver: &mut dyn LayoutResolver) -> Result<u64, MySoError> {
        if self.type_.is_gas_coin() {
            ...
            // It's a coin, but its not MYSO
            Ok(0)
        } else if self.type_.is_myso_balance_accumulator_field() {
            ...
            // anywhere, which is 10B for MYSO.
            assert!(
                v.value <= u64::MAX as u128,
                "MYSO balance cannot exceed u64::MAX"
            );
            Ok(v.value as u64)
        } else {
...
pub enum Owner {
    ...
    /// The object ID is converted to MySoAddress as MySoAddress is universal.
    ObjectOwner(MySoAddress),
...
impl Owner {
    ...
    pub fn get_address_owner_address(&self) -> MySoResult<MySoAddress> {
        match self {
            Self::AddressOwner(address) => Ok(*address),
            Self::Shared { .. }
            | Self::Immutable
            | Self::ObjectOwner(_)
            | Self::ConsensusAddressOwner { .. } => Err(MySoErrorKind::UnexpectedOwnerType.into()),
        }
    }
    ...
    // type is MySoAddress.
    pub fn get_owner_address(&self) -> MySoResult<MySoAddress> {
        match self {
            Self::AddressOwner(address)
            | Self::ObjectOwner(address)
            | Self::ConsensusAddressOwner { owner: address, .. } => Ok(*address),
            Self::Shared { .. } | Self::Immutable => Err(MySoErrorKind::UnexpectedOwnerType.into()),
        }
    }
...
impl PartialEq<ObjectID> for Owner {
    fn eq(&self, other: &ObjectID) -> bool {
        let other_id: MySoAddress = (*other).into();
        match self {
...
fn is_object_debug_verbose() -> bool {
    static MYSO_OBJECT_DEBUG_VERBOSE: Lazy<bool> =
        Lazy::new(|| std::env::var("MYSO_OBJECT_DEBUG_VERBOSE").is_ok());
    *MYSO_OBJECT_DEBUG_VERBOSE
}
...
impl ObjectInner {
    ...
    pub fn get_single_owner(&self) -> Option<MySoAddress> {
    ...
    pub fn transfer(&mut self, new_owner: MySoAddress) {
    ...
    pub fn get_layout(
        &self,
        resolver: &impl GetModule,
    ) -> Result<Option<MoveStructLayout>, MySoError> {
        match &self.data {
    ...
    pub fn get_move_template_type(&self) -> MySoResult<TypeTag> {
        let move_struct = self
            .data
            .struct_tag()
            .ok_or_else(|| MySoErrorKind::TypeError {
                error: "Object must be a Move object".to_owned(),
            })?;
        fp_ensure!(
            move_struct.type_params.len() == 1,
            MySoErrorKind::TypeError {
...
impl Object {
    ...
    pub fn with_id_owner_gas_for_testing(id: ObjectID, owner: MySoAddress, gas: u64) -> Self {
        let data = Data::Move(MoveObject {
            type_: GasCoin::type_().into(),
            has_public_transfer: true,
            version: OBJECT_START_VERSION,
    ...
    pub fn with_id_owner_for_testing(id: ObjectID, owner: MySoAddress) -> Self {
    ...
    pub fn with_owner_for_testing(owner: MySoAddress) -> Self {
    ...
    pub fn new_gas_with_balance_and_owner_for_testing(value: u64, owner: MySoAddress) -> Self {
...
mod tests {
    ...
    fn test_object_digest_and_serialized_format() {
        ...
        let o = Object::new_move(
            g,
            Owner::AddressOwner(MySoAddress::ZERO),
