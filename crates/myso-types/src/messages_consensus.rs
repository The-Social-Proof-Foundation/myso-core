...
/// The value should be the same in MySo committee.
...
impl ConsensusPosition {
    pub fn into_raw(self) -> Result<Bytes, MySoError> {
        bcs::to_bytes(&self)
            .map_err(|e| {
...
pub struct ConsensusTransaction {
    /// Encodes an u64 unique tracking id to allow us trace a message between MySo and consensus.
...
impl AuthorityCapabilitiesV2 {
    pub fn new(
        ...
        let generation = SystemTime::now()
            ...
            .expect("MySo did not exist prior to 1970")
            .as_millis()
            .try_into()
            .expect("This build of myso is not supported in the year 500,000,000");
        Self {
            authority,
            generation,
            supported_protocol_versions:
