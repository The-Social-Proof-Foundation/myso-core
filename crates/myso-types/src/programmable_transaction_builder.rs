...
impl ProgrammableTransactionBuilder {
    ...
    pub fn transfer_arg(&mut self, recipient: MySoAddress, arg: Argument) {
        self.transfer_args(recipient, vec![arg])
    }

    pub fn transfer_args(&mut self, recipient: MySoAddress, args: Vec<Argument>) {
    ...
    pub fn transfer_myso(&mut self, recipient: MySoAddress, amount: Option<u64>) {
        let rec_arg = self.pure(recipient).unwrap();
    ...
    pub fn pay_all_myso(&mut self, recipient: MySoAddress) {
        let rec_arg = self.pure(recipient).unwrap();
    ...
    pub fn split_coin(&mut self, recipient: MySoAddress, coin: ObjectRef, amounts: Vec<u64>) {
    ...
    fn pay_impl(
        ...
        let mut recipient_map: IndexMap<MySoAddress, Vec<usize>> = IndexMap::new();
