...
pub const TOTAL_SUPPLY_MIST: u64 = TOTAL_SUPPLY_MYSO * MIST_PER_MYSO;

pub const GAS_MODULE_NAME: &IdentStr = ident_str!("myso");
pub const GAS_STRUCT_NAME: &IdentStr = ident_str!("MYSO");
...
mod checked {
    ...
    /// Rust version of the Move myso::coin::Coin<MySo::myso::MYSO> type
    ...
    impl GasCoin {
        ...
        /// Return `true` if `s` is the type of a gas coin (i.e., 0x2::coin::Coin<0x2::myso::MYSO>)
        pub fn is_gas_coin(s: &StructTag) -> bool {
            Coin::is_coin(s) && s.type_params.len() == 1 && GAS::is_gas_type(&s.type_params[0])
        }

        /// Return `true` if `s` is the type of a gas balance (i.e., 0x2::balance::Balance<0x2::myso::MYSO>)
