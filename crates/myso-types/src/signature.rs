...
use crate::crypto::{
    CompressedSignature, PasskeyAuthenticatorAsBytes, PublicKey, SignatureScheme, MySoSignature,
...
use crate::{base_types::MySoAddress, crypto::Signature, error::MySoResult, multisig::MultiSig};
...
impl GenericSignature {
    ...
    /// Parse [enum CompressedSignature] from trait MySoSignature `flag || sig || pk`.
    /// This is useful for the MultiSig to combine partial signature into a MultiSig public key.
    pub fn to_compressed(&self) -> Result<CompressedSignature, MySoError> {
        match self {
            GenericSignature::Signature(s) => {
                ...
                match s.scheme() {
                    ...
                    _ => Err(MySoErrorKind::UnsupportedFeatureError {
    ...
    /// Parse [struct PublicKey] from trait MySoSignature `flag || sig || pk`.
    /// This is useful for the MultiSig to construct the bitmap in [struct MultiPublicKey].
    pub fn to_public_key(&self) -> Result<PublicKey, MySoError> {
        match self {
            GenericSignature::Signature(s) => {
                ...
                match s.scheme() {
                    SignatureScheme::ED25519 => Ok(PublicKey::Ed25519(
                        (&Ed25519PublicKey::from_bytes(bytes).map_err(|_| {
                            MySoErrorKind::KeyConversionError("Cannot parse ed25519 pk".to_string())
                        })?)
                            .into(),
                    )),
...
impl AuthenticatorTrait for Signature {
    fn verify_user_authenticator_epoch(&self, _: EpochId, _: Option<EpochId>) -> MySoResult {
        Ok(())
    }
