...
mod checked {
    ...
    use myso_types::error::{MySoResult, UserInputError, UserInputResult};
    ...
    use myso_types::{
        MYSO_AUTHENTICATOR_STATE_OBJECT_ID, MYSO_CLOCK_OBJECT_ID, MYSO_CLOCK_OBJECT_SHARED_VERSION,
    ...
    ) -> MySoResult<(MySoGasStatus, CheckedInputObjects)> {
    ...
    pub fn check_dev_inspect_input(
        ...
        let mut used_objects: HashSet<MySoAddress> = HashSet::new();
    ...
    fn check_gas(
        ...
        if tx_kind.is_system_tx() {
            Ok(MySoGasStatus::new_unmetered())
        } else {
            let gas_status =
                MySoGasStatus::new(gas_budget, gas_price, reference_gas_price, protocol_config)?;
            ...
            InputObjectKind::SharedMoveObject {
                ...
                if system_transaction {
                    ...
                    return Err(UserInputError::InaccessibleSystemObject {
                        object_id: MYSO_AUTHENTICATOR_STATE_OBJECT_ID,
                    });
                }
            }
            InputObjectKind::SharedMoveObject {
    ...
    pub fn check_non_system_packages_to_be_published(
        ...
        let mut verifier = myso_execution::verifier(protocol_config, signing_limits, metrics);
