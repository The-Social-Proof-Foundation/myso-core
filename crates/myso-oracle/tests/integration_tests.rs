...
async fn test_publish_complex_value() {
    ...
    for i in 1..200 {
        ...
        builder.programmable_move_call(
            ...
            module.clone(),
            submit_data.clone(),
            vec![parse_myso_type_tag(&format!("{package}::decimal_value::DecimalValue")).unwrap()],
...
async fn test_consume_oracle_data() {
    ...
    for _ in 0..3 {
        ...
        let ticker = builder
            .input(CallArg::Pure(bcs::to_bytes("MYSOUSD".as_bytes()).unwrap()))
            .unwrap();
        let identifier = builder
            .input(CallArg::Pure(
                bcs::to_bytes("identifier".as_bytes()).unwrap(),
...
async fn get_gas(client: &Client, sender: MySoAddress) -> (ObjectRef, u64) {
    let gas = client
...
async fn init_test_client() -> (Client, Keystore, MySoAddress) {
    ...
    let keystore = Keystore::File(
        FileBasedKeystore::load_or_create(
            &dirs::home_dir()
                .unwrap()
                .join(".myso/myso_config/myso.keystore"),
        )
        .unwrap(),
    );
    let sender: MySoAddress = keystore.addresses()[0];
    let gas = client
        .get_owned_objects(sender, Some(GasCoin::type_()), None, None)
        .await
        .unwrap();
