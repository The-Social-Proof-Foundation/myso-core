...
use myso_types::transaction::{Argument, Transaction};
use myso_types::transaction::{Command, ObjectArg, SharedObjectMutability};
use myso_types::transaction_driver_types::NON_RECOVERABLE_ERROR_MSG;
use myso_types::{
...
use myso_types::base_types::{ObjectID, ObjectRef, random_object_ref};
    ...
    async fn upload(&mut self, data_points: Vec<DataPoint>) -> anyhow::Result<TransactionEffects> {
        ...
        for data_point in &data_points {
            ...
            builder.command(Command::move_call(
                ...
                vec![
                    parse_myso_type_tag(&format!("{package_id}::decimal_value::DecimalValue"))
                        .unwrap(),
                ],
                arguments,
            ));
    ...
    async fn execute(&mut self, tx: Transaction) -> anyhow::Result<ExecutedTransaction> {
        ...
        loop {
                ...
                .await
            {
                ...
                Err(err) => {
                    // jsonrpsee translate every MySoError into jsonrpsee::core::Error, so we need to further distinguish
