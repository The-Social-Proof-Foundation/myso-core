...
impl<'a> ProcessPayload<'a, &'a QueryTransactionBlocks> for RpcCommandProcessor {
    async fn process(
        ...
        let queries: Vec<MySoTransactionBlockResponseQuery> = filters
            .into_iter()
            .map(|filter| MySoTransactionBlockResponseQuery {
                filter,
                options: Some(MySoTransactionBlockResponseOptions::full_content()),
            })
            .collect();
...
) -> Result<Page<MySoTransactionBlockResponse, TransactionDigest>> {
