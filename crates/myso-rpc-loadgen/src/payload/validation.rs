...
pub(crate) async fn check_transactions(
    ...
    cross_validate: bool,
    verify_objects: bool,
) -> Vec<Vec<MySoTransactionBlockResponse>> {
    let transactions: Vec<Vec<MySoTransactionBlockResponse>> =
        join_all(clients.iter().map(|client| async move {
            client
                .read_api()
                .multi_get_transactions_with_options(
                    digests.to_vec(),
                    MySoTransactionBlockResponseOptions::full_content(), // todo(Will) support options for this
                )
                .await
        }))
        .await
...
pub(crate) fn get_all_object_ids(response: &MySoTransactionBlockResponse) -> Vec<ObjectID> {
...
pub(crate) async fn multi_get_object(
    clients: &[MySoClient],
    object_ids: &[ObjectID],
) -> Vec<Vec<MySoObjectResponse>> {
    let objects: Vec<Vec<MySoObjectResponse>> = join_all(clients.iter().map(|client| async move {
        ...
        client
            ...
            .multi_get_object_with_options(
                object_ids.to_vec(),
                MySoObjectDataOptions::full_content(), // todo(Will) support options for this
            )
            .await
    }))
    .await
