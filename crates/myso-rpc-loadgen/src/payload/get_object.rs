...
use myso_json_rpc_types::{MySoObjectDataOptions, MySoObjectResponse};
...
pub(crate) async fn get_object(
    client: &MySoClient,
    object_id: ObjectID,
) -> Result<MySoObjectResponse> {
    let result = client
        .read_api()
        .get_object_with_options(object_id, MySoObjectDataOptions::full_content())
        .await
        .unwrap();
    Ok(result)
}
