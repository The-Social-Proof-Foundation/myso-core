...
#[derive(Parser)]
#[clap(
    name = "MySo RPC Load Generator",
    version = "0.1",
    about = "A load test application for MySo RPC"
)]
struct Opts {
    ...
    #[clap(long, default_value = "~/.myso/myso_config/logs")]
    logs_directory: String,

    #[clap(long, default_value = "~/.myso/loadgen/data")]
...
fn get_keypair() -> Result<SignerInfo> {
    ...
    // Load keystore from ~/.myso/myso_config/myso.keystore
    let keystore_path = get_myso_config_directory().join("myso.keystore");
    ...
    let keypair: &MySoKeyPair = keystore.export(&active_address)?;
    println!("using address {active_address} for signing");
    Ok(SignerInfo::new(keypair.encode_base64()))
}

fn get_myso_config_directory() -> PathBuf {
    match dirs::home_dir() {
        Some(v) => v.join(".myso").join("myso_config"),
...
fn get_log_file_path(dir_path: String) -> String {
    ...
    let log_filename = format!("myso-rpc-loadgen.{}.log", timestamp);
...
async fn main() -> Result<(), Box<dyn Error>> {
    ...
    let (command, common, need_keystore) = match opts.command {
        ...
        ClapCommand::PayMySo { common } => (Command::new_pay_myso(), common, true),
