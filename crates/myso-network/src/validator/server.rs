...
impl<M: MetricsCallbackProvider> ServerBuilder<M> {
    ...
    pub async fn bind(self, addr: &Multiaddr, tls_config: Option<ServerConfig>) -> Result<Server> {
        ...
        let mut builder = myso_http::Builder::new().config(http_config);
...
pub const MYSO_TLS_SERVER_NAME: &str = "myso";

pub struct Server {
    server: myso_http::ServerHandle,
...
impl Server {
    ...
    pub fn handle(&self) -> &myso_http::ServerHandle {
        &self.server
    }
}
...
mod test {
    ...
    async fn test_metrics_layer_successful() {
        ...
        let server = super::ServerBuilder::from_config(&config, metrics.clone())
            .bind(
                &address,
                Some(myso_tls::create_rustls_server_config(
