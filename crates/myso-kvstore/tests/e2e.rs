...
use myso_kvstore::BigTableIndexer;
...
const TABLES: &[&str] = &[
    myso_kvstore::tables::objects::NAME,
    myso_kvstore::tables::object_types::NAME,
    myso_kvstore::tables::transactions::NAME,
    myso_kvstore::tables::checkpoints::NAME,
    myso_kvstore::tables::checkpoints_by_digest::NAME,
    myso_kvstore::tables::watermark_alt_legacy::NAME,
    myso_kvstore::tables::epochs::NAME,
];
...
async fn get_all_coins(client: &mut GrpcClient, address: MySoAddress) -> Result<Vec<Object>> {
...
impl TestHarness {
    ...
    /// Build, sign, and execute a MYSO transfer via gRPC.
    async fn transfer_myso(&mut self, recipient: MySoAddress, amount: u64) -> Result<Transaction> {
...
async fn test_indexer_e2e() -> Result<()> {
    ...
    for _ in 0..3 {
        ...
        signed_txns.push(harness.transfer_myso(recipient, 1).await?);
    }
