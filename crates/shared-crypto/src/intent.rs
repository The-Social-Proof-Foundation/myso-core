...
/// (i.e., Narwhal, MySo, Ethereum etc) should never collide, so that even when a signing
...
impl Intent {
    pub fn myso_app(scope: IntentScope) -> Self {
        Self {
...
/// be any type that implements [trait Serialize]. *ALL* signatures in MySo must commits
...
/// to ensure no hashing collision for any ObjectID vs MySoAddress which is derived
/// as the hash of `flag || pubkey`. See `myso_types::crypto::SignatureScheme::flag()`.
#[derive(Serialize_repr, Deserialize_repr, Copy, Clone, PartialEq, Eq, Debug, Hash)]
#[repr(u8)]
