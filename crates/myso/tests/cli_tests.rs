...
use myso::client_ptb::ptb::PTB;
use myso::myso_commands::RpcArgs;
...
use myso_sdk::myso_client_config::MySoClientConfig;
use myso_sdk::wallet_context::WalletContext;
use myso_swarm_config::genesis_config::{AccountConfig, GenesisConfig};
...
impl TreeShakingTest {
    ...
    async fn new() -> Result<Self, anyhow::Error> {
        ...
        let framework_pkgs = PathBuf::from("../myso-framework/packages");
...
upgrade-capability = "{}""#,
    ...
    async fn upgrade_package(
        ...
        let resp = MySoClientCommands::Upgrade(UpgradeArgs {
        ...
        let MySoClientCommandResult::TransactionBlock(publish_response) = resp else {
...
async fn test_publish_package(
    ...
    let resp = MySoClientCommands::TestPublish(TestPublishArgs {
        publish_args: PublishArgs {
            ...
            verify_deps: false,
            with_unpublished_dependencies,
            upgrade_admin_cap: None,
            publish_admin_cap: None,
            coin_admin_cap: None,
            payment: PaymentArgs {
                gas: vec![gas_obj_id],
            },
...
async fn publish_package(
    ...
    let resp = MySoClientCommands::Publish(PublishArgs {
        package_path: package_path.clone(),
        build_config: build_config.clone(),
        skip_dependency_verification: false,
        verify_deps: false,
...
async fn test_genesis() -> Result<(), anyhow::Error> {
    ...
    let config = working_dir.join(MYSO_NETWORK_CONFIG);

    // Start network without authorities
    let start = MySoCommand::Start {
    ...
    assert!(files.contains(&MYSO_CLIENT_CONFIG.to_string()));
    assert!(files.contains(&MYSO_NETWORK_CONFIG.to_string()));
    assert!(files.contains(&MYSO_FULLNODE_CONFIG.to_string()));
    assert!(files.contains(&MYSO_GENESIS_FILENAME.to_string()));
    assert!(files.contains(&MYSO_KEYSTORE_FILENAME.to_string()));
    assert!(files.contains(&MYSO_KEYSTORE_ALIASES_FILENAME.to_string()));

    // Check network config
    let network_conf =
        PersistedConfig::<NetworkConfig>::read(&working_dir.join(MYSO_NETWORK_CONFIG))?;
    assert_eq!(1, network_conf.validator_configs().len());

    // Check wallet config
    let wallet_conf =
        PersistedConfig::<MySoClientConfig>::read(&working_dir.join(MYSO_CLIENT_CONFIG))?;
    ...
    let result = MySoCommand::Genesis {
        working_dir: Some(working_dir.to_path_buf()),
        write_config: None,
        force: false,
        from_config: None,
...
async fn test_ptb_publish_and_complex_arg_resolution() -> Result<(), anyhow::Error> {
    ...
    let MySoClientCommandResult::TransactionBlock(response) = resp else {
        unreachable!("Invalid response");
    };
    ...
    let start_call_result = MySoClientCommands::Call {
        package: package.0.0,
    ...
    let shared_id_str =
        if let MySoClientCommandResult::TransactionBlock(response) = start_call_result {
    ...
    myso::client_ptb::ptb::PTB { args: args.clone() }
        .execute(context)
        .await?;
...
async fn test_ptb_publish() -> Result<(), anyhow::Error> {
    ...
    let res = myso::client_ptb::ptb::PTB { args: args.clone() }
        .execute(context)
        .await;
...
async fn test_gas_command() -> Result<(), anyhow::Error> {
    ...
    MySoClientCommands::Transfer {
        to: KeyIdentity::Address(MySoAddress::random_for_testing_only()),
...
async fn test_move_call_args_linter_command() -> Result<(), anyhow::Error> {
    ...
    let package = if let MySoClientCommandResult::TransactionBlock(response) = resp {
        assert!(
    ...
    // Create the args
    let args = vec![
        MySoJsonValue::new(json!("123"))?,
        MySoJsonValue::new(json!(address1))?,
    ];

    // Test case with no gas specified
    let resp = MySoClientCommands::Call {
        package,
    ...
    let created_obj: ObjectID = if let MySoClientCommandResult::TransactionBlock(resp) = resp {
    ...
    assert!(MySoJsonValue::new(args_json.as_array().unwrap().first().unwrap().clone()).is_err());

    // Try a bad argument: too few args
    let args_json = json!([300usize]);
    let mut args = vec![];
    for a in args_json.as_array().unwrap() {
        args.push(MySoJsonValue::new(a.clone()).unwrap());
    }
    ...
    let args = [
        MySoJsonValue::new(json!(obj))?,
        MySoJsonValue::new(json!(address2))?,
    ];
    ...
    let args = [
        MySoJsonValue::new(json!(created_obj))?,
        MySoJsonValue::new(json!(address2))?,
    ];
    ...
    // let err_string = format!("{} ", resp.err().unwrap());
    // let framework_addr = MYSO_FRAMEWORK_ADDRESS.to_hex_literal();
    // let package_addr = package.to_hex_literal();
    // assert!(err_string.contains(&format!("Expected argument of type {package_addr}::object_basics::Object, but found type {framework_addr}::coin::Coin<{framework_addr}::myso::MYSO>")));
    ...
    let result = MySoClientCommands::Call {
        package,
        module: "object_basics".to_string(),
        function: "create".to_string(),
        type_args: vec![],
    ...
    if let MySoClientCommandResult::TransactionBlock(txn_response) = result {
...
async fn test_package_publish_command() -> Result<(), anyhow::Error> {
    ...
    let obj_ids = if let MySoClientCommandResult::TransactionBlock(response) = resp {
        assert_eq!(response.effects.gas_object().0.0, gas_obj_id);
        response
            .effects
            .created()
...
async fn test_package_management_on_publish_command() -> Result<(), anyhow::Error> {
    ...
    let (expect_original_id, expect_version, _) =
        if let MySoClientCommandResult::TransactionBlock(response) = resp {
            assert_eq!(response.effects.gas_object().0.0, gas_obj_id);
            response
                .get_new_package_obj()
    ...
    let parsed: ParsedPublishedFile<MySoFlavor> = toml_edit::de::from_str(&pubfile_str).unwrap();
...
async fn test_delete_shared_object() -> Result<(), anyhow::Error> {
    ...
    let owned_obj_ids = if let MySoClientCommandResult::TransactionBlock(response) = resp {
        assert_eq!(response.effects.gas_object().0.0, gas_obj_id);
    ...
    let shared_id = if let MySoClientCommandResult::TransactionBlock(response) = start_call_result {
        response.effects.created()[0].0.0
    } else {
        unreachable!("Invalid response");
    };

    let delete_result = MySoClientCommands::Call {
        package: package_id.0,
        module: "sod".to_string(),
        function: "delete".to_string(),
        type_args: vec![],
        args: vec![MySoJsonValue::from_str(&shared_id.to_string()).unwrap()],
        payment: PaymentArgs::default(),
        gas_data: GasDataArgs {
            gas_budget: Some(rgp * TEST_ONLY_GAS_UNIT_FOR_PUBLISH),
            ..Default::default()
    ...
    if let MySoClientCommandResult::TransactionBlock(response) = delete_result {
...
async fn test_receive_argument() -> Result<(), anyhow::Error> {
    ...
    let receive_result = MySoClientCommands::Call {
        package: package_id.0,
        module: "tto".to_string(),
        function: "receiver".to_string(),
        type_args: vec![],
        args: vec![
            MySoJsonValue::from_str(&parent.0.to_string()).unwrap(),
            MySoJsonValue::from_str(&child.0.to_string()).unwrap(),
        ],
        payment: PaymentArgs::default(),
        gas_data: GasDataArgs {
            gas_budget: Some(rgp * TEST_ONLY_GAS_UNIT_FOR_PUBLISH),
    ...
    if let MySoClientCommandResult::TransactionBlock(response) = receive_result {
        assert!(response.effects.status().is_ok());
    } else {
        unreachable!("Invalid response");
    };
...
-> Result<(), anyhow::Error> {
    ...
    package_path.push("module_publish_with_unpublished_dependency");
    let build_config = BuildConfig::new_for_testing().config;
    let result = MySoClientCommands::TestPublish(TestPublishArgs {
        publish_args: PublishArgs {
            package_path,
            build_config,
            skip_dependency_verification: false,
...
async fn test_package_publish_test_flag() -> Result<(), anyhow::Error> {
    ...
    // this would have been the result of calling `myso client publish --test`
...
async fn test_package_management_on_upgrade_command() -> Result<(), anyhow::Error> {
    ...
    // Now run the upgrade
    let upgrade_response = MySoClientCommands::Upgrade(UpgradeArgs {
    ...
    let (expect_upgrade_latest_id, expect_upgrade_version, _) =
        if let MySoClientCommandResult::TransactionBlock(response) = upgrade_response {
            assert_eq!(response.effects.gas_object().0.0, gas_obj_id);
            response
                .get_new_package_obj()
                .ok_or_else(|| anyhow::anyhow!("No package object response"))?
    ...
    let published_file: ParsedPublishedFile<MySoFlavor> =
...
async fn test_native_transfer() -> Result<(), anyhow::Error> {
    ...
    let resp = MySoClientCommands::Transfer {
    ...
    let (mut_obj1, mut_obj2) = if let MySoClientCommandResult::TransactionBlock(response) = resp {
        assert!(
            response.effects.status().is_ok(),
            "Command failed: {:?}",
            response
    ...
    // Check the objects
    let resp = MySoClientCommands::Object {
        id: mut_obj1,
        bcs: false,
    }
    .execute(context)
    .await?;
    let mut_obj1 = if let MySoClientCommandResult::Object(object) = dbg!(resp) {
        object
    } else {
        panic!();
    };

    let resp2 = MySoClientCommands::Object {
        id: mut_obj2,
        bcs: false,
    }
    .execute(context)
    .await?;
    let mut_obj2 = if let MySoClientCommandResult::Object(object) = resp2 {
        object
    } else {
        panic!();
    };
    ...
    // Get the mutated objects
    let (_mut_obj1, _mut_obj2) = if let MySoClientCommandResult::TransactionBlock(response) = resp {
        (
            response.effects.mutated().first().unwrap().0.0,
            response.effects.mutated().get(1).unwrap().0.0,
        )
...
async fn test_switch_command() -> Result<(), anyhow::Error> {
    ...
    let os = MySoClientCommands::Objects { address: None }
        .execute(context)
        .await?;

    let mut cmd_objs = if let MySoClientCommandResult::Objects(v) = os {
        v
    } else {
    ...
    // Switch the address
    let resp = MySoClientCommands::Switch {
    ...
    assert_eq!(
        ...
        format!(
            "{}",
            MySoClientCommandResult::Switch(SwitchResponse {
    ...
    let os = MySoClientCommands::NewAddress {
    ...
    .execute(context)
    .await?;
    let new_addr = if let MySoClientCommandResult::NewAddress(x) = os {
        x.address
    } else {
        panic!("Command failed")
    };
...
async fn test_new_address_command_by_flag() -> Result<(), anyhow::Error> {
    ...
    assert_eq!(
        context
            ...
            .entries()
            .iter()
            .filter(|k| k.flag() == Ed25519MySoSignature::SCHEME.flag())
            .count(),
        5
    );
    ...
    assert_eq!(
        context
            ...
            .entries()
            .iter()
            .filter(|k| k.flag() == Secp256k1MySoSignature::SCHEME.flag())
            .count(),
        1
    );
...
async fn test_remove_address_command() -> Result<(), anyhow::Error> {
    ...
    MySoClientCommands::RemoveAddress {
...
async fn test_merge_coin() -> Result<(), anyhow::Error> {
    ...
    let resp = MySoClientCommands::MergeCoin {
        primary_coin,
        coin_to_merge,
        payment: PaymentArgs { gas: vec![gas] },
        gas_data: GasDataArgs {
    ...
    .execute(context)
    .await?;
    let g = if let MySoClientCommandResult::TransactionBlock(r) = resp {
...
async fn test_split_coin() -> Result<(), anyhow::Error> {
    ...
    // Test with gas specified
    let resp = MySoClientCommands::SplitCoin {
        coin_id: coin,
    ...
    let (updated_coin, new_coins) = if let MySoClientCommandResult::TransactionBlock(r) = resp {
        assert!(r.effects.status().is_ok(), "Command failed: {:?}", r);
        assert_eq!(r.effects.gas_object().0.0, gas);
...
async fn test_serialize_tx() -> Result<(), anyhow::Error> {
    ...
    let ptb = PTB { args };
    MySoClientCommands::PTB(ptb).execute(context).await.unwrap();
    let mut args = ptb_args.clone();
...
#[allow(deprecated)]
async fn test_with_myso_binary(args: &[&str]) -> Result<(), anyhow::Error> {
    let mut cmd = assert_cmd::Command::cargo_bin("myso").unwrap();
...
fn assert_dry_run(dry_run: MySoClientCommandResult, object_id: ObjectID, command: &str) {
    if let MySoClientCommandResult::DryRun(response) = dry_run {
        assert!(
...
async fn test_dry_run() -> Result<(), anyhow::Error> {
    ...
    // === TRANSFER === //
    let transfer_dry_run = MySoClientCommands::Transfer {
        to: KeyIdentity::Address(MySoAddress::random_for_testing_only()),
        object_id: object_to_send,
        payment: PaymentArgs {
            gas: vec![object_id],
    ...
    // === TRANSFER MYSO === //
    let transfer_myso_dry_run = MySoClientCommands::TransferMySo {
        to: KeyIdentity::Address(MySoAddress::random_for_testing_only()),
        myso_coin_object_id: object_to_send,
        amount: Some(1),
        gas_data: GasDataArgs {
            gas_budget: Some(rgp * TEST_ONLY_GAS_UNIT_FOR_TRANSFER),
            ..Default::default()
    ...
    assert_dry_run(transfer_myso_dry_run, object_to_send, "TransferMySo");

    // === PAY === //
    let pay_dry_run = MySoClientCommands::Pay {
        input_coins: vec![object_id],
        recipients: vec![KeyIdentity::Address(MySoAddress::random_for_testing_only())],
        amounts: vec![1],
        payment: PaymentArgs::default(),
        gas_data: GasDataArgs {
            gas_budget: Some(rgp * TEST_ONLY_GAS_UNIT_FOR_TRANSFER),
    ...
    if let MySoClientCommandResult::DryRun(response) = pay_dry_run {
    ...
    // === PAY MYSO === //
    let pay_myso_dry_run = MySoClientCommands::PayMySo {
        input_coins: vec![object_id],
        recipients: vec![KeyIdentity::Address(MySoAddress::random_for_testing_only())],
        amounts: vec![1],
        gas_data: GasDataArgs {
    ...
    assert_dry_run(pay_myso_dry_run, object_id, "PayMySo");

    // === PAY ALL MYSO === //
    let pay_all_myso_dry_run = MySoClientCommands::PayAllMySo {
        input_coins: vec![object_id],
        recipient: KeyIdentity::Address(MySoAddress::random_for_testing_only()),
        gas_data: GasDataArgs {
            gas_budget: Some(rgp * TEST_ONLY_GAS_UNIT_FOR_TRANSFER),
            ..Default::default()
        },
    ...
    assert_dry_run(pay_all_myso_dry_run, object_id, "PayAllMySo");
...
async fn test_cluster_helper() -> (
    ...
    let address2 = MySoAddress::random_for_testing_only();
    let address3 = MySoAddress::random_for_testing_only();
...
async fn test_pay() -> Result<(), anyhow::Error> {
    ...
    let context = &mut test_cluster.wallet;
    let pay = MySoClientCommands::Pay {
    ...
    if let MySoClientCommandResult::TransactionBlock(response) = pay {
        // check tx status
        assert!(response.effects.status().is_ok());
        // check gas coin used
...
#[sim_test]
async fn test_pay_myso() -> Result<(), anyhow::Error> {
    let (mut test_cluster, client, rgp, objects, recipients, addresses) =
        test_cluster_helper().await;
    ...
    let pay_myso = MySoClientCommands::PayMySo {
        input_coins: vec![object_id1, object_id2],
        recipients: vec![recipient1.clone(), recipient2.clone()],
        amounts: amounts.into(),
        gas_data: GasDataArgs {
    ...
    // pay myso takes the input coins and transfers from each of them (in order) the amounts to the
    ...
    if let MySoClientCommandResult::TransactionBlock(response) = pay_myso {
        assert!(response.effects.status().is_ok());
        // check gas coin used
        ...
        panic!("PayMySo test failed");
    }
    Ok(())
}

#[sim_test]
async fn test_pay_all_myso() -> Result<(), anyhow::Error> {
    let (mut test_cluster, client, rgp, objects, recipients, addresses) =
        test_cluster_helper().await;
    let (object_id1, object_id2) = (objects[0], objects[1]);
    ...
    let context = &mut test_cluster.wallet;
    let pay_all_myso = MySoClientCommands::PayAllMySo {
        input_coins: vec![object_id1, object_id2],
    ...
    // pay all myso will take the input coins and smash them into one coin and transfer that coin to
    // the recipient, so we check that the recipient has one object, if the tx status is success,
    // and if the gas object used was the first object in the input coins
    if let MySoClientCommandResult::TransactionBlock(response) = pay_all_myso {
        assert!(response.effects.status().is_ok());
        let objs_refs = client.get_owned_objects(address2, None, None, None).await?;
        assert!(objs_refs.next_page_token.is_none());
        assert_eq!(objs_refs.items.len(), 1);
...
async fn test_transfer() -> Result<(), anyhow::Error> {
    ...
    let address2 = addresses[0];
    let context = &mut test_cluster.wallet;
    let transfer = MySoClientCommands::Transfer {
    ...
    if let MySoClientCommandResult::TransactionBlock(response) = transfer {
        assert!(response.effects.status().is_ok());
...
#[sim_test]
async fn test_transfer_myso() -> Result<(), anyhow::Error> {
    let (mut test_cluster, client, rgp, objects, recipients, addresses) =
        test_cluster_helper().await;
    ...
    let context = &mut test_cluster.wallet;
    let amount = 1000;
    let transfer_myso = MySoClientCommands::TransferMySo {
        to: KeyIdentity::Address(address2),
        myso_coin_object_id: object_id1,
        amount: Some(amount),
        gas_data: GasDataArgs {
            gas_budget: Some(rgp * TEST_ONLY_GAS_UNIT_FOR_TRANSFER),
            ..Default::default()
    ...
    // transfer myso will transfer the amount from object_id1 to address2, and use the same object
    // as gas, and we check if the recipient address received the object, and the expected balance
    // is correct
    if let MySoClientCommandResult::TransactionBlock(response) = transfer_myso {
        assert!(response.effects.status().is_ok());
        assert_eq!(response.effects.gas_object().0.0, object_id1);
        let objs_refs = client.get_owned_objects(address2, None, None, None).await?;
        assert!(objs_refs.next_page_token.is_none());
        ...
        panic!("TransferMySo test failed");
    }
    ...
    if let MySoClientCommandResult::TransactionBlock(response) = transfer_myso {
        ...
        assert_eq!(
            objs_refs.items.len(),
            2,
            "Expected to have two coins when calling transfer myso the 2nd time"
        );
...
async fn test_transfer_gas_smash() -> Result<(), anyhow::Error> {
    ...
    let MySoClientCommandResult::TransactionBlock(response) = transfer else {
        panic!("Transfer test failed");
    };
...
async fn test_transfer_sponsored() -> Result<(), anyhow::Error> {
    ...
    // A0 sends O1 to A1
    let transfer = MySoClientCommands::TransferMySo {
        to: KeyIdentity::Address(a1),
        myso_coin_object_id: o[1],
        amount: None,
        gas_data: GasDataArgs {
            gas_budget: Some(rgp * TEST_ONLY_GAS_UNIT_FOR_TRANSFER),
            ..Default::default()
    ...
    let transfer_back = MySoClientCommands::Transfer {
    ...
    let MySoClientCommandResult::TransactionBlock(response) = transfer_back else {
...
async fn test_transfer_serialized_data() -> Result<(), anyhow::Error> {
    ...
    let MySoClientCommandResult::SerializedUnsignedTransaction(tx_data) = transfer else {
        panic!("Expected SerializedUnsignedTransaction result");
    };

    let tx_bytes = Base64::encode(bcs::to_bytes(&tx_data)?);
    let transfer_serialized = MySoClientCommands::SerializedTx {
        tx_bytes,
        processing: TxProcessingArgs::default(),
    }
    .execute(context)
    ...
    let MySoClientCommandResult::TransactionBlock(response) = transfer_serialized else {
...
async fn test_transfer_serialized_kind() -> Result<(), anyhow::Error> {
    ...
    let transfer_serialized = MySoClientCommands::SerializedTxKind {
        tx_bytes,
        payment: PaymentArgs { gas: vec![o[1]] },
        gas_data: GasDataArgs {
            gas_budget: Some(rgp * TEST_ONLY_GAS_UNIT_FOR_TRANSFER),
...
async fn test_gas_estimation() -> Result<(), anyhow::Error> {
    ...
    let tx_kind = tx_builder.transfer_myso_tx_kind(address2, Some(amount));
    let gas_estimate = estimate_gas_budget(context, sender, tx_kind, rgp, vec![], None).await;
    assert!(gas_estimate.is_ok());

    let transfer_myso_cmd = MySoClientCommands::TransferMySo {
        to: KeyIdentity::Address(address2),
        myso_coin_object_id: object_id1,
        amount: Some(amount),
        gas_data: GasDataArgs::default(),
    ...
    .await
    .unwrap();
    if let MySoClientCommandResult::TransactionBlock(response) = transfer_myso_cmd {
        assert!(response.effects.status().is_ok());
...
async fn test_custom_sender() -> Result<(), anyhow::Error> {
    ...
    let custom_sender = MySoAddress::random_for_testing_only();
...
async fn test_clever_errors() -> Result<(), anyhow::Error> {
    ...
    // Normal abort
    let non_clever_abort = MySoClientCommands::Call {
        package: package.0.0,
    ...
    // Line-only abort
    let line_only_abort = MySoClientCommands::Call {
        package: package.0.0,
        module: "clever_errors".to_string(),
    ...
    let clever_error_utf8 = MySoClientCommands::Call {
        package: package.0.0,
        module: "clever_errors".to_string(),
    ...
    let clever_error_non_utf8 = MySoClientCommands::Call {
        package: package.0.0,
        module: "clever_errors".to_string(),
...
async fn test_tree_shaking_package_deps_on_pkg_upgrade_1() -> Result<(), anyhow::Error> {
    ...
    // the old code for publishing a package from myso-test-transaction-builder does not know about
...
async fn test_party_transfer() -> Result<(), anyhow::Error> {
    ...
    let party_transfer = MySoClientCommands::PartyTransfer {
        to: recipient1.clone(),
        object_id: object_id1,
        payment: PaymentArgs::default(),
        gas_data: GasDataArgs {
    ...
    let MySoClientCommandResult::TransactionBlock(response) = party_transfer else {
...
fn create_temp_dir_with_framework_packages(
    ...
    copy_dir_all(
        PathBuf::from("../myso-framework/packages"),
...
async fn test_publish_sender_flag_respected_in_serialized_transaction() -> Result<(), anyhow::Error>
{
    ...
    let MySoClientCommandResult::SerializedUnsignedTransaction(tx_data) = resp else {
        panic!("Expected SerializedUnsignedTransaction result");
    };
