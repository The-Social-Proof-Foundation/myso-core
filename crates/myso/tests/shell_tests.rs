...
async fn shell_tests(path: &Path) -> datatest_stable::Result<()> {
    ...
    shell.env("CONFIG", config_file.join(MYSO_CLIENT_CONFIG));
...
fn make_temp_config_dir() -> TempDir {
    ...
    MySoClientConfig {
        keystore: Keystore::from(
            FileBasedKeystore::load_or_create(&config_dir.join(MYSO_KEYSTORE_FILENAME)).unwrap(),
        ),
        external_keys: None,
        envs: vec![MySoEnv {
    ...
    .persisted(&result.path().join(MYSO_CLIENT_CONFIG))
    .save()
