...
use myso_move_build::{BuildConfig, CompiledPackage, PackageDependencies};
use myso_package_management::LockCommand;
use myso_rpc_api::{
    Client,
...
use myso_keys::key_derive;
use myso_package_alt::{BuildParams, MySoFlavor, find_environment};
use myso_source_validation::{BytecodeSourceVerifier, ValidationMode};
...
pub enum MySoClientCommands {
    ...
    Balance {
        ...
        /// Show balance for the specified coin (e.g., 0x2::myso::MYSO).
    ...
    Call {
        ...
        #[clap(
            long,
            value_parser = parse_myso_type_tag,
            num_args(1..),
        )]
    ...
    /// List all MySo environments
    Envs,

    /// Execute a Signed Transaction. This is useful when the user prefers to sign elsewhere and use this command to execute.
    ExecuteSignedTx {
        /// BCS serialized transaction data bytes without its type tag, as base64 encoded string. This is the output of myso client command using --serialize-unsigned-transaction.
        #[clap(long)]
        tx_bytes: String,
    ...
    ExecuteCombinedSignedTx {
        /// BCS serialized sender signed data, as base64 encoded string. This is the output of myso client command using --serialize-signed-transaction.
        #[clap(long)]
    ...
    Objects {
        ...
        /// objects owned by `myso client active-address`.
        #[clap(name = "owner_address")]
        address: Option<KeyIdentity>,
    },
    ...
    /// Replay a given transaction to view transaction effects (deprecated; use `myso replay` instead)
    #[clap(name = "replay-transaction")]
    ReplayTransaction {},

    /// Replay transactions listed in a file (deprecated; use `myso replay` instead)
    #[clap(name = "replay-batch")]
    ReplayBatch {},

    /// Replay all transactions in a range of checkpoints (deprecated; use `myso replay` instead)
...
pub struct TxProcessingArgs {
    ...
    /// be used to execute transaction with `myso client execute-signed-tx --tx-bytes <TX_BYTES>`.
    #[arg(long)]
    ...
    /// `myso client execute-combined-signed-tx --signed-tx-bytes <SIGNED_TX_BYTES>`.
    #[arg(long)]
...
pub struct PublishArgs {
    ...
    #[clap(long)]
    pub coin_admin_cap: Option<ObjectID>,
...
impl MySoClientCommands {
    pub async fn execute(
        self,
        context: &mut WalletContext,
    ) -> Result<MySoClientCommandResult, anyhow::Error> {
        let ret = match self {
            MySoClientCommands::ReplayTransaction {} => {
                eprintln!("This command is deprecated. Use `myso replay` instead.");
                MySoClientCommandResult::NoOutput
            }
            MySoClientCommands::ReplayBatch {} => {
                eprintln!("This command is deprecated. Use `myso replay` instead.");
                MySoClientCommandResult::NoOutput
            }
            MySoClientCommands::ReplayCheckpoints {} => {
                eprintln!("This command is deprecated. Use `myso replay` instead.");
                MySoClientCommandResult::NoOutput
            }
            MySoClientCommands::Addresses { sort_by_alias } => {
                let active_address = context.active_address()?;
                let mut addresses: Vec<(String, MySoAddress)> = context
                ...
                MySoClientCommandResult::Addresses(output)
            }
            MySoClientCommands::Balance {
                address,
                coin_type,
                with_coins,
            } => {
                ...
                let objects: Vec<Coin> = client
                    ...
                    .try_filter_map(|o| async move {
                        let Ok(Some((coin_type, balance))) =
                            myso_types::coin::Coin::extract_balance_if_coin(&o)
                        else {
                            return Ok(None);
                        };
                        Ok(Some(Coin {
                ...
                let myso_type_tag = canonicalize_type(MYSO_COIN_TYPE)?;

                // show MYSO first
                let ordered_coins_myso_first = coins_by_type
                    .remove(&myso_type_tag)
                    .into_iter()
                    .chain(coins_by_type.into_values())
                    .collect();

                MySoClientCommandResult::Balance(ordered_coins_myso_first, with_coins)
            }

            MySoClientCommands::DynamicFieldQuery { id, cursor, limit } => {
                let client = context.grpc_client()?;
                let _ = context.cache_chain_id().await?;
            ...
            MySoClientCommands::TestPublish(args) => {
            ...
            MySoClientCommands::VerifyBytecodeMeter {
                protocol_version,
                module_paths,
                package_path,
                build_config,
                ...
                let mut verifier = myso_execution::verifier(
                    &protocol_config,
                    signing_limits,
                ...
                MySoClientCommandResult::VerifyBytecodeMeter {
                    success: !exceeded,
            ...
            MySoClientCommands::Object { id, bcs } => {
                // Fetch the object ref
                let _ = context.cache_chain_id().await?;
                let object = context.grpc_client()?.get_object(id).await?;
                if !bcs {
                    MySoClientCommandResult::Object(object)
                } else {
                    MySoClientCommandResult::RawObject(object)
                }
            }

            MySoClientCommands::TransactionBlock { digest } => {
                let _ = context.cache_chain_id().await?;
                let tx_read = context.grpc_client()?.get_transaction(&digest).await?;
                MySoClientCommandResult::TransactionBlock(tx_read)
            }

            MySoClientCommands::Call {
                package,
                module,
                function,
                type_args,
            ...
            MySoClientCommands::TransferMySo {
                to,
                myso_coin_object_id: object_id,
                amount,
                gas_data,
                processing,
            } => {
                ...
                let tx_kind = client
                    .transaction_builder()
                    .transfer_myso_tx_kind(to, amount);
            ...
            MySoClientCommands::Pay {
                ...
                let recipients = recipients
                    ...
                    .collect::<Result<Vec<MySoAddress>, anyhow::Error>>()
                    .map_err(|e| anyhow!("{e}"))?;
                ...
                let tx_kind = client
                    ...
                    "Gas coin is in input coins of Pay transaction, use PayMySo transaction instead!"
                );
            ...
            MySoClientCommands::PayMySo {
                ...
                let tx_kind = client
                    .transaction_builder()
                    .pay_myso_tx_kind(recipients, amounts)?;
            ...
            MySoClientCommands::PayAllMySo {
                ...
                processing,
                    ...
                    !input_coins.is_empty(),
                    "PayAllMySo transaction requires a non-empty list of input coins"
                );
                ...
                let tx_kind = client.transaction_builder().pay_all_myso_tx_kind(recipient);
                let gas_payment = client
                    .transaction_builder()
                    .input_refs(&input_coins)
                    .await?;
            ...
            MySoClientCommands::Objects { address } => {
                let address = context.get_identity_address(address)?;
                let client = context.grpc_client()?;
                let _ = context.cache_chain_id().await?;
                let objects = client
                    ...
                    .try_collect()
                    .await?;
                MySoClientCommandResult::Objects(objects)
            }

            MySoClientCommands::NewAddress {
                key_scheme,
                alias,
                derivation_path,
                word_length,
                ...
                MySoClientCommandResult::NewAddress(NewAddressOutput {
                    alias,
                    address,
                    key_scheme: scheme,
            ...
            MySoClientCommands::RemoveAddress { alias_or_address } => {
                let identity = KeyIdentity::from_str(&alias_or_address)
                    .map_err(|e| anyhow!("Invalid address or alias: {}", e))?;
                let address: MySoAddress = context.config.keystore.get_by_identity(&identity)?;

                context.config.keystore.remove(address).await?;

                MySoClientCommandResult::RemoveAddress(RemoveAddressOutput { alias_or_address })
            }

            MySoClientCommands::Gas { address } => {
                let address = context.get_identity_address(address)?;
                let coins = context
                    .gas_objects(address)
                    .await?
                    ...
                    .collect();
                let _ = context.cache_chain_id().await?;
                MySoClientCommandResult::Gas(coins)
            }
            ...
            MySoClientCommands::ChainIdentifier => {
                let ci = context.cache_chain_id().await?;
                MySoClientCommandResult::ChainIdentifier(ci)
            }
            MySoClientCommands::SplitCoin {
                coin_id,
                amounts,
                count,
                payment,
            ...
            MySoClientCommands::SerializedTx {
                tx_bytes,
                processing,
            } => {
            ...
            MySoClientCommands::SerializedTxKind {
                tx_bytes,
                payment,
                gas_data,
                processing,
            ...
            MySoClientCommands::Switch { address, env } => {
                let mut addr = None;
                ...
                context.config.save()?;
                MySoClientCommandResult::Switch(SwitchResponse { address: addr, env })
            }
            MySoClientCommands::ActiveAddress => {
                MySoClientCommandResult::ActiveAddress(context.active_address().ok())
            }

            MySoClientCommands::ExecuteSignedTx {
                tx_bytes,
                signatures,
            } => {
                let data = bcs::from_bytes(
                    ...
                    .to_vec()
                    .map_err(|_| anyhow!("Invalid Base64 encoding"))?
                ).map_err(|_| anyhow!("Failed to parse tx bytes, check if it matches the output of myso client commands with --serialize-unsigned-transaction"))?;
                ...
                MySoClientCommandResult::TransactionBlock(response)
            }
            MySoClientCommands::ExecuteCombinedSignedTx { signed_tx_bytes } => {
                let data: SenderSignedData = bcs::from_bytes(
                    &Base64::try_from(signed_tx_bytes)
                        .map_err(|_| anyhow!("Invalid Base64 encoding"))?
                        .to_vec()
                        .map_err(|_| anyhow!("Invalid Base64 encoding"))?
                ).map_err(|_| anyhow!("Failed to parse SenderSignedData bytes, check if it matches the output of myso client commands with --serialize-signed-transaction"))?;
                let transaction = Envelope::<SenderSignedData, EmptySignInfo>::new(data);
                let response = context.execute_transaction_may_fail(transaction).await?;
                MySoClientCommandResult::TransactionBlock(response)
            }
            MySoClientCommands::NewEnv {
                alias,
                rpc,
                ws,
                basic_auth,
                ...
                MySoClientCommandResult::NewEnv(env)
            }
            MySoClientCommands::ActiveEnv => MySoClientCommandResult::ActiveEnv(
                context.get_active_env().ok().map(|e| e.alias.clone()),
            ),
            MySoClientCommands::Envs => MySoClientCommandResult::Envs(
                context.config.envs.clone(),
                context.get_active_env().ok().map(|e| e.alias.clone()),
            ),
            MySoClientCommands::VerifySource {
                package_path,
                build_config,
                verify_deps,
                skip_source,
                ...
                MySoClientCommandResult::VerifySource
            }
            MySoClientCommands::PartyTransfer {
                to,
                object_id,
                payment,
                gas_data,
    ...
    pub fn switch_env(config: &mut MySoClientConfig, env: &str) -> Result<(), anyhow::Error> {
        let env = Some(env.into());
        ensure!(
            config.get_env(&env).is_some(),
            "Environment config not found for [{env:?}], add new environment config using the `myso client new-env` command."
        );
...
pub(crate) async fn upgrade_package(
    mut client: Client,
    root_pkg: &RootPackage<MySoFlavor>,
    build_config: MoveBuildConfig,
    package_path: &Path,
...
pub(crate) async fn compile_package(
    ...
    if compiled_package
        ...
        .is_empty()
    {
        return Err(MySoErrorKind::ModulePublishFailure {
...
impl Display for MySoClientCommandResult {
    fn fmt(&self, f: &mut Formatter<'_>) -> std::fmt::Result {
        let mut writer = String::new();
        match self {
            MySoClientCommandResult::Addresses(addresses) => {
            ...
            MySoClientCommandResult::Balance(coins, with_coins) => {
                if coins.is_empty() {
            ...
            MySoClientCommandResult::DynamicFieldQuery(df_refs) => {
            ...
            MySoClientCommandResult::Gas(gas_coins) => {
            ...
            MySoClientCommandResult::NewAddress(new_address) => {
                let mut builder = TableBuilder::default();
            ...
            MySoClientCommandResult::RemoveAddress(remove_address) => {
                let mut builder = TableBuilder::default();
            ...
            MySoClientCommandResult::RawObject(o) => match o.to_bcs_base64() {
                Ok(b64) => writeln!(writer, "{b64}")?,
                Err(e) => writeln!(writer, "{e}")?,
            },
            MySoClientCommandResult::ComputeTransactionDigest(tx_data) => {
                writeln!(writer, "{}", tx_data.digest())?;
            }
            MySoClientCommandResult::SerializedUnsignedTransaction(tx_data) => {
                writeln!(
                    writer,
                    "{}",
            ...
            MySoClientCommandResult::SerializedSignedTransaction(sender_signed_tx) => {
                writeln!(
                    writer,
                    "{}",
            ...
            MySoClientCommandResult::SyncClientState => {
                writeln!(writer, "Client state sync complete.")?;
            }
            MySoClientCommandResult::ChainIdentifier(ci) => {
                writeln!(writer, "{}", ci)?;
            }
            MySoClientCommandResult::Switch(response) => {
                write!(writer, "{}", response)?;
            }
            MySoClientCommandResult::ActiveAddress(response) => {
                match response {
                    Some(r) => write!(writer, "{}", r)?,
                    None => write!(writer, "None")?,
                };
            }
            MySoClientCommandResult::ActiveEnv(env) => {
                write!(writer, "{}", env.as_deref().unwrap_or("None"))?;
            }
            MySoClientCommandResult::NewEnv(env) => {
                writeln!(writer, "Added new MySo env [{}] to config.", env.alias)?;
            }
            MySoClientCommandResult::Envs(envs, active) => {
                let mut builder = TableBuilder::default();
            ...
            MySoClientCommandResult::NoOutput => {}
            MySoClientCommandResult::DryRun(response) => {
                writeln!(f, "{}", Pretty(response))?;
            }
            MySoClientCommandResult::DevInspect(response) => {
                writeln!(f, "{}", Pretty(response))?;
            }
        }
...
impl Debug for MySoClientCommandResult {
    fn fmt(&self, f: &mut Formatter<'_>) -> std::fmt::Result {
...
impl MySoClientCommandResult {
    pub fn objects_response(&self) -> Option<Vec<Object>> {
        use MySoClientCommandResult::*;
        match self {
...
pub struct AddressesOutput {
    pub active_address: MySoAddress,
    pub addresses: Vec<(String, MySoAddress)>,
}
...
pub struct ObjectOutput {
    ...
    pub content: myso_types::object::Data,
}
...
impl From<&GasCoin> for GasCoinOutput {
    fn from(gas_coin: &GasCoin) -> Self {
        Self {
            ...
            myso_balance: format_balance(gas_coin.value() as u128, 9, 2, None),
        }
    }
}
...
pub enum MySoClientCommandResult {
    ActiveAddress(Option<MySoAddress>),
    ...
    Envs(Vec<MySoEnv>, Option<String>),
    Gas(Vec<GasCoin>),
...
pub async fn execute_dry_run(
    ...
    Ok(MySoClientCommandResult::DryRun(response))
}
...
pub async fn estimate_gas_budget(
    ...
    if let Ok(MySoClientCommandResult::DryRun(dry_run)) = dry_run {
...
pub(crate) async fn dry_run_or_execute_or_serialize(
    ...
    if serialize_unsigned_transaction {
        Ok(MySoClientCommandResult::SerializedUnsignedTransaction(
            tx_data,
        ))
    } else if tx_digest {
        Ok(MySoClientCommandResult::ComputeTransactionDigest(tx_data))
    } else {
        ...
        if serialize_signed_transaction {
            Ok(MySoClientCommandResult::SerializedSignedTransaction(
                sender_signed_data,
            ))
        } else {
            ...
            Ok(MySoClientCommandResult::TransactionBlock(response))
        }
    }
}
...
async fn execute_dev_inspect(
    ...
    let result = client
        ...
        .await?;
    Ok(MySoClientCommandResult::DevInspect(result))
}
...
pub async fn load_root_pkg_for_publish_upgrade(
    ...
    build_config: &MoveBuildConfig,
    path: &Path,
) -> anyhow::Result<RootPackage<MySoFlavor>> {
...
pub fn update_publication(
    ...
    _build_config: &MoveBuildConfig,
    publication: Option<&mut Publication<MySoFlavor>>,
) -> Result<Publication<MySoFlavor>, anyhow::Error> {
    ...
    match command {
        LockCommand::Publish => {
            ...
            Ok(Publication::<MySoFlavor> {
                chain_id: chain_id.to_string(),
                metadata: myso_package_alt::PublishedMetadata {
                    toolchain_version: Some(env!("CARGO_PKG_VERSION").into()),
                    build_config: Some(myso_package_alt::BuildParams::default()),
...
async fn publish_command(
    args: PublishArgs,
    root_package: &mut RootPackage<MySoFlavor>,
    context: &mut WalletContext,
) -> Result<MySoClientCommandResult, anyhow::Error> {
    let PublishArgs {
        package_path,
    ...
    let package_path =
        package_path
            .canonicalize()
            .map_err(|e| MySoErrorKind::ModulePublishFailure {
    ...
    let response = if let MySoClientCommandResult::TransactionBlock(ref tx) = result {
        tx
    } else {
        return Ok(result);
    };
    ...
    if build_config.test_mode {
        return Err(MySoErrorKind::ModulePublishFailure {
            error:
                ...
                compiling the package normally with `myso move build`."
                    .to_string(),
        }
        .into());
    }
...
fn find_faucet_url(address: MySoAddress, rpc: &str) -> anyhow::Result<String> {
    let host = url_to_host(rpc)?;
    let devnet_host = url_to_host(MYSO_DEVNET_URL)?;
    let testnet_host = url_to_host(MYSO_TESTNET_URL)?;
    let localhost = url_to_host(MYSO_LOCAL_NETWORK_URL)?;
