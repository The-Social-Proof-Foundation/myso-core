...
use myso_types::multisig::{MultiSig, MultiSigPublicKey, ThresholdUnit, WeightUnit};
...
pub enum KeyToolCommand {
    ...
    /// MySo Wallet and MySo CLI Keystore. Use `myso keytool import` if you
    /// wish to import a key to MySo Keystore.
    ...
    ///
    /// Use `myso client new-address` if you want to generate and save the key into myso.keystore.
    Generate {
    ...
    /// Add a new key to MySo CLI Keystore using either the input mnemonic phrase or a Bech32 encoded 33-byte
    ...
    /// Output the private key of the given key identity in MySo CLI Keystore as Bech32
    /// encoded string starting with `mysoprivkey`.
    Export {
        #[clap(long)]
        key_identity: KeyIdentity,
    },
    /// List all keys by its MySo address, Base64 encoded public key, key scheme name in
    /// myso.keystore.
    List {
        /// Sort by alias
        #[clap(long, short = 's')]
    ...
    /// [enum MySoKeyPair] (Base64 encoded of 33-byte `flag || privkey`) or `type AuthorityKeyPair`
    ...
    /// To MultiSig MySo Address. Pass in a list of all public keys `flag || pk` in Base64.
    ...
    /// result can be used as signature field for `myso client execute-signed-tx`. The sum
    ...
    Show { file: PathBuf },
    /// Create signature using the private key for the given address (or its alias) in myso keystore.
    ...
    /// This takes [enum MySoKeyPair] of Base64 encoded of 33-byte `flag || privkey`). It
    /// outputs the keypair into a file at the current directory where the address is the filename,
    /// and prints out its MySo address, Base64 encoded public key, the key scheme, and the key scheme flag.
    ...
    ZkLoginEnterToken {
        ...
        kp_bigint: String,
        #[clap(long)]
        ephemeral_key_identifier: MySoAddress,
        #[clap(long, default_value = "devnet")]
        network: String,
        #[clap(long, default_value = "false")]
        test_multisig: bool,
    ...
    /// Example request: myso keytool zk-login-sig-verify --sig $SERIALIZED_ZKLOGIN_SIG --bytes $BYTES --intent-scope 0 --network devnet --curr-epoch 10
    ZkLoginSigVerify {
    ...
    /// e.g. myso keytool zk-login-insecure-sign-personal-message --data "hello" --max-epoch 5
...
pub struct ConvertOutput {
    bech32_with_flag: String, // latest MySo Keystore and MySo Wallet import/export format
    base64_with_flag: String, // MySo Keystore storage format
    hex_without_flag: String, // Legacy MySo Wallet format
    scheme: String,
}
...
pub struct SignData {
    ...
    // serialized MySo signature to be send for executing the transaction.
    myso_signature: String,
}
    ...
    pub async fn execute(
        ...
        Ok(match self {
            ...
            KeyToolCommand::DecodeMultiSig {
                ...
                let address = MySoAddress::from(multisig.get_pk());
            ...
            KeyToolCommand::Generate {
                ...
                SignatureScheme::BLS12381 => {
                    let (myso_address, kp) = get_authority_key_pair();
                    let file_name = format!("bls-{myso_address}.key");
                ...
                _ => {
                    let (myso_address, skp, _scheme, phrase) =
                        generate_new_key(key_scheme, derivation_path, word_length)?;
                    let file = format!("{myso_address}.key");
            ...
            KeyToolCommand::Import {
                ...
                if Hex::decode(&input_string).is_ok() {
                    return Err(anyhow!(
                        "MySo Keystore and MySo Wallet no longer support importing
                    private key as Hex, if you are sure your private key is encoded in Hex, use
                    `myso keytool convert $HEX` to convert first then import the Bech32 encoded
                    private key starting with `mysoprivkey`."
                    ));
                }

                match MySoKeyPair::decode(&input_string) {
                    Ok(skp) => {
                        ...
                        let alias = match alias {
                            Some(x) => x,
                            None => context.config.keystore.get_alias(&key.myso_address)?,
                        };
                    ...
                    Err(_) => {
                        ...
                        let myso_address = context
                            ...
                            .await?;
                        let skp = context.config.keystore.export(&myso_address)?;
                        let mut key = Key::from(skp);
            ...
            KeyToolCommand::Export { key_identity } => {
                ...
                let mut key = Key::from(skp);
                key.alias = context.config.keystore.get_alias(&key.myso_address).ok();
                let key = ExportedKey {
                ...
                let output = match read_keypair_from_file(&file) {
                    Ok(keypair) => {
                        ...
                        let network_worker_keypair = match &keypair {
                            MySoKeyPair::Ed25519(kp) => kp.encode_base64(),
                            MySoKeyPair::Secp256k1(kp) => kp.encode_base64(),
                            MySoKeyPair::Secp256r1(kp) => kp.encode_base64(),
                        };
                        KeypairData {
            ...
            KeyToolCommand::MultiSigAddress {
                ...
                for (pk, w) in pks.into_iter().zip(weights.into_iter()) {
                    output.multisig.push(MultiSigOutput {
                        address: Into::<MySoAddress>::into(&pk),
                        public_base64_key: pk.encode_base64(),
                        weight: w,
                    });
                }
                    ...
                    Err(_) => match read_authority_keypair_from_file(&file) {
                        Ok(keypair) => {
                            ...
                            CommandOutput::Show(Key {
                                ...
                                myso_address: (keypair.public()).into(),
                                public_base64_key,
                                key_scheme: SignatureScheme::BLS12381.to_string(),
                                flag: SignatureScheme::BLS12381.flag(),
                                peer_id: None,
            ...
            KeyToolCommand::Sign {
                ...
                let intent = intent.unwrap_or_else(Intent::myso_transaction);
                ...
                CommandOutput::Sign(SignData {
                    ...
                    myso_signature: myso_signature.encode_base64(),
                })
            }
            ...
            KeyToolCommand::SignKMS {
                ...
                let pk_owner = PublicKey::decode_base64(&base64pk)
                    .map_err(|e| anyhow!("Invalid base64 key: {:?}", e))?;
                let address_owner = MySoAddress::from(&pk_owner);
            ...
            KeyToolCommand::Unpack { keypair } => {
                let keypair = MySoKeyPair::decode_base64(&keypair)
                    .map_err(|_| anyhow!("Invalid Base64 encode keypair"))?;

                let key = Key::from(&keypair);
                let path_str = format!("{}.key", key.myso_address).to_lowercase();
            ...
            KeyToolCommand::ZkLoginInsecureSignPersonalMessage { data, max_epoch } => {
                ...
                let skp =
                    MySoKeyPair::Ed25519(Ed25519KeyPair::generate(&mut StdRng::from_seed([0; 32])));
            ...
            KeyToolCommand::ZkLoginSignAndExecuteTx {
                ...
                let skp = if fixed {
                    ...
                    MySoKeyPair::Ed25519(Ed25519KeyPair::generate(&mut rand::thread_rng()))
                };
                println!("Ephemeral keypair: {:?}", skp.encode());
                let pk = skp.public();
                let ephemeral_key_identifier: MySoAddress = (&skp.public()).into();
                ...
                println!(
                    "Finish login and paste the entire URL here (e.g. https://myso.io/#id_token=...):"
                );
...
impl From<&MySoKeyPair> for Key {
    fn from(skp: &MySoKeyPair) -> Self {
        Key::from(skp.public())
    }
}
...
impl From<PublicKey> for Key {
    fn from(pk: PublicKey) -> Self {
        Key {
            ...
            myso_address: MySoAddress::from(&pk),
            public_base64_key: pk.encode_base64(),
...
fn convert_private_key_to_bech32(value: String) -> Result<ConvertOutput, anyhow::Error> {
    let skp = match MySoKeyPair::decode(&value) {
        Ok(s) => s,
        Err(_) => match Hex::decode(&value) {
            Ok(decoded) => {
                ...
                MySoKeyPair::Ed25519(Ed25519KeyPair::from_bytes(&decoded)?)
            }
            Err(_) => match MySoKeyPair::decode_base64(&value) {
                Ok(skp) => skp,
                Err(_) => match Ed25519KeyPair::decode_base64(&value) {
                    Ok(kp) => MySoKeyPair::Ed25519(kp),
