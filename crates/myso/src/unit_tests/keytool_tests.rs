...
use myso_types::crypto::Ed25519MySoSignature;
use myso_types::crypto::EncodeDecodeBase64;
use myso_types::crypto::Secp256k1MySoSignature;
use myso_types::crypto::Secp256r1MySoSignature;
use myso_types::crypto::Signature;
use myso_types::crypto::SignatureScheme;
use myso_types::crypto::MySoKeyPair;
use myso_types::crypto::MySoSignatureInner;
...
async fn test_addresses_command() -> Result<(), anyhow::Error> {
    ...
    for _ in 0..3 {
        keystore
            .import(None, MySoKeyPair::Secp256k1(get_key_pair().1))
            .await?;
    }
...
async fn test_flag_in_signature_and_keypair() -> Result<(), anyhow::Error> {
    ...
    keystore
        ...
        .await?;
    keystore
        .import(None, MySoKeyPair::Ed25519(get_key_pair().1))
        .await?;

    for pk in keystore.entries() {
        let pk1 = pk.clone();
        let sig = keystore
            .sign_secure(&(&pk).into(), b"hello", Intent::myso_transaction())
            .await?;
        match sig {
            Signature::Ed25519MySoSignature(_) => {
                // signature contains corresponding flag
                assert_eq!(
                    *sig.as_ref().first().unwrap(),
                    Ed25519MySoSignature::SCHEME.flag()
                );
                // keystore stores pubkey with corresponding flag
                assert!(pk1.flag() == Ed25519MySoSignature::SCHEME.flag())
            }
            Signature::Secp256k1MySoSignature(_) => {
                assert_eq!(
                    *sig.as_ref().first().unwrap(),
                    Secp256k1MySoSignature::SCHEME.flag()
                );
                assert!(pk1.flag() == Secp256k1MySoSignature::SCHEME.flag())
            }
            Signature::Secp256r1MySoSignature(_) => {
                assert_eq!(
                    *sig.as_ref().first().unwrap(),
                    Secp256r1MySoSignature::SCHEME.flag()
                );
                assert!(pk1.flag() == Secp256r1MySoSignature::SCHEME.flag())
            }
        }
    }
    Ok(())
...
async fn test_read_write_keystore_with_flag() {
    ...
    let kp_secp = MySoKeyPair::Secp256k1(get_key_pair().1);
    let addr_secp: MySoAddress = (&kp_secp.public()).into();
    ...
    let kp_ed = MySoKeyPair::Ed25519(get_key_pair().1);
    let addr_ed: MySoAddress = (&kp_ed.public()).into();
...
#[test]
async fn test_myso_operations_config() {
    ...
    assert!(read.is_ok());
    assert_eq!(
        MySoAddress::from_str("7d20dcdb2bca4f508ea9613994683eb4e76e9c4ed371169677c1be02aaf0b58e")
            .unwrap(),
    ...
    let kp = MySoKeyPair::decode_base64("APCWxPNCbgGxOYKeMfPqPmXmwdNVyau9y4IsyBcmC14A").unwrap();
    let contents = vec![kp.encode_base64()];
    let res = std::fs::write(path2, serde_json::to_string_pretty(&contents).unwrap());
    assert!(res.is_ok());
    let read = FileBasedKeystore::load_or_create(&path3);
    assert_eq!(
        MySoAddress::from_str("160ef6ce4f395208a12119c5011bf8d8ceb760e3159307c819bd0197d154d384")
            .unwrap(),
        read.unwrap().addresses()[0]
    );
}
...
async fn test_private_keys_import_export() -> Result<(), anyhow::Error> {
    // private key in Bech32, private key in Hex, private key in Base64, derived MySo address in Hex
    const TEST_CASES: &[(&str, &str, &str, &str)] = &[
        (
            "mysoprivkey1qzwant3kaegmjy4qxex93s0jzvemekkjmyv3r2sjwgnv2y479pgsywhveae",
        ...
        (
            "mysoprivkey1qrh2sjl88rze74hwjndw3l26dqyz63tea5u9frtwcsqhmfk9vxdlx8cpv0g",
        ...
        (
            "mysoprivkey1qzg73qyvfz0wpnyectkl08nrhe4pgnu0vqx8gydu96qx7uj4wyr8gcrjlh3",
    ...
    for (private_key, private_key_hex, private_key_base64, address) in TEST_CASES {
        ...
        .execute(&mut context)
        .await?;
        let kp = MySoKeyPair::decode(private_key).unwrap();
        let kp_from_hex = MySoKeyPair::Ed25519(
            Ed25519KeyPair::from_bytes(&Hex::decode(private_key_hex).unwrap()).unwrap(),
        );
        assert_eq!(kp, kp_from_hex);

        let kp_from_base64 = MySoKeyPair::decode_base64(private_key_base64).unwrap();
        assert_eq!(kp, kp_from_base64);

        let addr = MySoAddress::from_str(address).unwrap();
        assert_eq!(MySoAddress::from(&kp.public()), addr);
...
async fn test_mnemonics_ed25519() -> Result<(), anyhow::Error> {
    // Test case matches with /mysten/myso/sdk/typescript/test/unit/cryptography/ed25519-keypair.test.ts
    const TEST_CASES: [[&str; 3]; 3] = [
        [
            "film crazy soon outside stand loop subway crumble thrive popular green nuclear struggle pistol arm wife phrase warfare march wheat nephew ask sunny firm",
            "mysoprivkey1qrwsjvr6gwaxmsvxk4cfun99ra8uwxg3c9pl0nhle7xxpe4s80y05ctazer",
            "a2d14fad60c56049ecf75246a481934691214ce413e6a8ae2fe6834c173a6133",
        ],
        [
            "require decline left thought grid priority false tiny gasp angle royal system attack beef setup reward aunt skill wasp tray vital bounce inflict level",
            "mysoprivkey1qzdvpa77ct272ultqcy20dkw78dysnfyg90fhcxkdm60el0qht9mvzlsh4j",
            "1ada6e6f3f3e4055096f606c746690f1108fcc2ca479055cc434a3e1d3f758aa",
        ],
        [
            "organ crash swim stick traffic remember army arctic mesh slice swear summer police vast chaos cradle squirrel hood useless evidence pet hub soap lake",
            "mysoprivkey1qqqscjyyr64jea849dfv9cukurqj2swx0m3rr4hr7sw955jy07tzgcde5ut",
    ...
    for t in TEST_CASES {
        ...
        .execute(&mut context)
        .await?;
        let kp = MySoKeyPair::decode(t[1]).unwrap();
        let addr = MySoAddress::from_str(t[2]).unwrap();
        assert_eq!(MySoAddress::from(&kp.public()), addr);
        assert!(context.config.keystore.addresses().contains(&addr));
    }
    Ok(())
...
async fn test_mnemonics_secp256k1() -> Result<(), anyhow::Error> {
    // Test case matches with /mysten/myso/sdk/typescript/test/unit/cryptography/secp256k1-keypair.test.ts
    const TEST_CASES: [[&str; 3]; 3] = [
        [
            "film crazy soon outside stand loop subway crumble thrive popular green nuclear struggle pistol arm wife phrase warfare march wheat nephew ask sunny firm",
            "mysoprivkey1qyqr6yvxdqkh32ep4pk9caqvphmk9epn6rhkczcrhaeermsyvwsg783y9am",
            "9e8f732575cc5386f8df3c784cd3ed1b53ce538da79926b2ad54dcc1197d2532",
        ],
        [
            "require decline left thought grid priority false tiny gasp angle royal system attack beef setup reward aunt skill wasp tray vital bounce inflict level",
            "mysoprivkey1q8hexn5m2u36tx39ln5e22hfseadknp7d2qlkhe30ejy7fc6am5aqkqpqsj",
            "9fd5a804ed6b46d36949ff7434247f0fd594673973ece24aede6b86a7b5dae01",
        ],
        [
            "organ crash swim stick traffic remember army arctic mesh slice swear summer police vast chaos cradle squirrel hood useless evidence pet hub soap lake",
            "mysoprivkey1qxx6yf53jgxvsmccst8cuwnj0rx4k4uzvn9aalvag7ns0xf0g8j2x246jst",
...
async fn test_mnemonics_secp256r1() -> Result<(), anyhow::Error> {
    // Test case matches with /mysten/myso/sdk/typescript/test/unit/cryptography/secp256r1-keypair.test.ts
    const TEST_CASES: [[&str; 3]; 3] = [
        [
            "act wing dilemma glory episode region allow mad tourist humble muffin oblige",
            "mysoprivkey1qgj6vet4rstf2p00j860xctkg4fyqqq5hxgu4mm0eg60fq787ujnqs5wc8q",
        ...
        [
            ...
            "mysoprivkey1qgmgr6dza8slgxn0rcxcy47xeas9l565cc5q440ngdzr575rc2356gzlq7a",
        ...
        [
            ...
            "mysoprivkey1qt2gsye4dyn0lxey0ht6d5f2ada7ew9044a49y2f3mymy2uf0hr55jmfze3",
    ...
    for [mnemonics, sk, address] in TEST_CASES {
        ...
        let kp = MySoKeyPair::decode(sk).unwrap();
        let addr = MySoAddress::from_str(address).unwrap();
        assert_eq!(MySoAddress::from(&kp.public()), addr);
        assert!(context.config.keystore.addresses().contains(&addr));
    }
...
async fn test_sign_command() -> Result<(), anyhow::Error> {
    ...
    let gas_price = 1;
    let tx_data = TransactionData::new_pay_myso(
        *sender,
        vec![gas],
        vec![MySoAddress::random_for_testing_only()],
        vec![10000],
        gas,
    ...
    KeyToolCommand::Sign {
        ...
        intent: Some(Intent::myso_app(IntentScope::PersonalMessage)),
    }
    .execute(&mut context)
    .await?;
