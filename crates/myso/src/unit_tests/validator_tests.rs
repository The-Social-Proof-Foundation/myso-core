...
use crate::validator_commands::{
    MySoValidatorCommand, MySoValidatorCommandResponse, get_validator_summary,
};
use anyhow::Ok;
...
use myso_types::{base_types::MySoAddress, crypto::Signature, transaction::Transaction};
use test_cluster::TestClusterBuilder;

#[tokio::test]
async fn test_print_raw_rgp_txn() -> Result<(), anyhow::Error> {
    let test_cluster = TestClusterBuilder::new().build().await;
    let keypair: &MySoKeyPair = test_cluster
        .swarm
        .config()
        .validator_configs
        .first()
        ...
        .account_key_pair
        .keypair();
    let validator_address: MySoAddress = MySoAddress::from(&keypair.public());
    let mut context = test_cluster.wallet;
    let myso_client = context.grpc_client()?;
    let (_, summary) = get_validator_summary(&myso_client, validator_address)
        .await?
        .unwrap();
    let operation_cap_id = summary.operation_cap_id().parse()?;

    // Execute the command and get the serialized transaction data.
    let response = MySoValidatorCommand::DisplayGasPriceUpdateRawTxn {
    ...
    .execute(&mut context)
    .await?;
    let MySoValidatorCommandResponse::DisplayGasPriceUpdateRawTxn {
        data,
        serialized_data,
    } = response
    else {
    ...
    let signature = Signature::new_secure(
        &IntentMessage::new(Intent::myso_transaction(), deserialized_data),
        keypair,
    );
