...
use myso_types::{
    base_types::ObjectID,
    coin::CoinMetadata,
    gas_coin::{GasCoin, MIST_PER_MYSO, TOTAL_SUPPLY_MIST},
    governance::StakedMySo,
...
const STR_MYSO: &str = "MySo";
const STR_STAKED_MYSO: &str = "StakedMySo";
const STR_PACKAGE: &str = "Package";
const STR_COIN_METADATA: &str = "CoinMetadata";
const STR_OTHER: &str = "Other";
const STR_MYSO_DISTRIBUTION: &str = "MySo Distribution";
...
pub(crate) fn examine_genesis_checkpoint(genesis: UnsignedGenesis) {
    ...
    // Prepare MySo distribution info
    let mut myso_distribution = BTreeMap::new();
    let entry = myso_distribution
        .entry("MySo System".to_string())
    ...
    entry.insert(
        ...
        (STR_MYSO, system_object.stake_subsidy.balance.value()),
    );

    // Prepare Object Info
    let mut owner_map = BTreeMap::new();
    let mut package_map = BTreeMap::new();
    let mut myso_map = BTreeMap::new();
    let mut staked_myso_map = BTreeMap::new();
    ...
    for object in genesis.objects() {
        ...
        match &object.data {
            myso_types::object::Data::Move(move_object) => {
                if let Ok(gas) = GasCoin::try_from(object) {
                    let entry = myso_distribution
                        .entry(object.owner.to_string())
                        .or_default();
                    entry.insert(object_id_str.clone(), (STR_MYSO, gas.value()));
                    myso_map.insert(object.id(), gas);
                } else if let Ok(coin_metadata) = CoinMetadata::try_from(object) {
                    coin_metadata_map.insert(object.id(), coin_metadata);
                } else if let Ok(staked_myso) = StakedMySo::try_from(object) {
                    let entry = myso_distribution
                        .entry(object.owner.to_string())
                        .or_default();
                    entry.insert(object_id_str, (STR_STAKED_MYSO, staked_myso.principal()));
                    // Assert pool id is associated with a knonw validator.
                    let validator = validator_pool_id_map.get(&staked_myso.pool_id()).unwrap();
                    assert_eq!(validator.staking_pool.id, staked_myso.pool_id());

                    staked_myso_map.insert(object.id(), staked_myso);
                } else {
                    other_object_map.insert(object.id(), move_object);
                }
            }
            myso_types::object::Data::Package(p) => {
    ...
    loop {
        ...
        .prompt();
        match ans {
            Ok(name) if name == STR_MYSO_DISTRIBUTION => {
                examine_total_supply(&myso_distribution, true)
            }
...
fn examine_validators(
    ...
    validator_map: &BTreeMap<&str, &MySoValidatorGenesis>,
) {
    loop {
...
fn examine_object(
    ...
    validator_pool_id_map: &BTreeMap<ObjectID, &MySoValidatorGenesis>,
    package_map: &BTreeMap<ObjectID, &MovePackage>,
    myso_map: &BTreeMap<ObjectID, GasCoin>,
    staked_myso_map: &BTreeMap<ObjectID, StakedMySo>,
    ...
    loop {
        ...
        match ans {
            Ok(name) if name == STR_EXIT => break,
            Ok(name) if name == STR_MYSO => {
                for gas_coin in myso_map.values() {
                    display_myso(gas_coin, owner_map);
                }
                print_divider("MySo");
            }
            Ok(name) if name == STR_STAKED_MYSO => {
                for staked_myso_coin in staked_myso_map.values() {
                    display_staked_myso(staked_myso_coin, validator_pool_id_map, owner_map);
                }
                print_divider(STR_STAKED_MYSO);
            }
...
fn examine_total_supply(
    myso_distribution: &BTreeMap<String, BTreeMap<String, (&str, u64)>>,
    print: bool,
) {
    let mut total_myso = 0;
    let mut total_staked_myso = 0;
    for (owner, coins) in myso_distribution {
        let mut amount_sum = 0;
        for (owner, value) in coins.values() {
            amount_sum += value;
            if *owner == STR_STAKED_MYSO {
                total_staked_myso += value;
            }
        }
        total_myso += amount_sum;
        if print {
            println!("Owner {:?}", owner);
            println!(
                "Total Amount of MySo/StakedMySo Owned: {amount_sum} MIST or {} MYSO:",
                amount_sum / MIST_PER_MYSO
            );
            println!("{:#?}\n", coins);
        }
    }
    assert_eq!(total_myso, TOTAL_SUPPLY_MIST);
    // Always print this.
    println!(
        "Total Supply of MySo: {total_myso} MIST or {} MYSO",
        total_myso / MIST_PER_MYSO
    );
    println!(
        "Total Amount of StakedMySo: {total_staked_myso} MIST or {} MYSO\n",
        total_staked_myso / MIST_PER_MYSO
    );
    if print {
        print_divider("MySo Distribution");
    }
}

fn display_validator(validator: &MySoValidatorGenesis) {
    ...
    println!(
        "Staking Pool MySo Balance: {:?}",
        validator.staking_pool.myso_balance
    );
    println!(
        "Rewards Pool: {}",
    ...
    println!(
        "Pending Total MySo Withdraw: {}",
        validator.staking_pool.pending_total_myso_withdraw
    );
    println!(
...
fn display_myso(gas_coin: &GasCoin, owner_map: &BTreeMap<ObjectID, Owner>) {
    println!("ID: {}", gas_coin.id());
    println!("Balance: {}", gas_coin.value());
    println!("Owner: {}\n", owner_map.get(gas_coin.id()).unwrap());
}

fn display_staked_myso(
    ...
    println!("Owner: {}\n", owner_map.get(&staked_myso.id()).unwrap());
}
