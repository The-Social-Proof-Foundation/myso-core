...
pub fn run(cmd: Ceremony) -> Result<()> {
    ...
    match cmd.command {
        ...
        CeremonyCommand::AddValidator {
            ...
            let account_keypair: MySoKeyPair = read_keypair_from_file(account_key_file)?;
            ...
            builder = builder.add_validator(
                myso_genesis_builder::validator_info::ValidatorInfo {
                    name,
                    protocol_key: keypair.public().into(),
                    worker_key: worker_keypair.public().clone(),
                    account_address: MySoAddress::from(&account_keypair.public()),
                    network_key: network_keypair.public().clone(),
                    gas_price: myso_config::node::DEFAULT_VALIDATOR_GAS_PRICE,
        ...
        CeremonyCommand::Finalize => {
            ...
            genesis.save(dir.join(MYSO_GENESIS_FILENAME))?;

            println!("Successfully built {MYSO_GENESIS_FILENAME}");
            println!(
                "{MYSO_GENESIS_FILENAME} blake2b-256: {}",
...
fn check_protocol_version(builder: &Builder, protocol_version: ProtocolVersion) -> Result<()> {
    ...
    // (e.g. using a `myso` binary built at the wrong commit).
    ...
    fn ceremony() -> Result<()> {
        ...
        let validators = (0..10)
            .map(|i| {
                ...
                write_keypair_to_file(&MySoKeyPair::Ed25519(worker_keypair), &worker_key_file)
                    .unwrap();

                let network_key_file = dir.path().join(format!("{}-1.key", info.name));
                write_keypair_to_file(&MySoKeyPair::Ed25519(network_keypair), &network_key_file)
                    .unwrap();

                let account_key_file = dir.path().join(format!("{}-2.key", info.name));
                write_keypair_to_file(&MySoKeyPair::Ed25519(account_keypair), &account_key_file)
                    .unwrap();
