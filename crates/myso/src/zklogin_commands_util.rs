...
pub async fn perform_zk_login_test_tx(
    ...
    let skp1 = MySoKeyPair::Ed25519(Ed25519KeyPair::generate(&mut StdRng::from_seed([1; 32])));
    ...
    let sender = if test_multisig {
        ...
        MySoAddress::from(&multisig_pk)
    } else {
        println!("Use single zklogin address as sender");
        MySoAddress::try_from_unpadded(&zk_login_inputs)?
    };
    println!("Sender: {:?}", sender);

    // Request some coin from faucet and build a test transaction.
    let mut myso = myso_rpc_api::Client::new(fullnode_url)?;
    ...
    if response.items.len() != 2 {
        panic!("Faucet did not work correctly and the provided MySo address has no coins")
    }
    ...
    let txb_res = myso
        ...
        .transfer_object(
            ...
            Some(gas_coin),
            5000000,
            MySoAddress::ZERO, // as a demo, send to a dummy address
        )
        .await?;
    println!(
...
fn get_config(network: &str) -> (&str, &str) {
    match network {
        "devnet" => (
            "https://faucet.devnet.myso.io/v2/gas",
            "https://rpc.devnet.myso.io:443",
        ),
