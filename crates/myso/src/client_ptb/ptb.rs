...
use crate::{
    client_commands::{
        GasDataArgs, MySoClientCommandResult, TxProcessingArgs, dry_run_or_execute_or_serialize,
    },
    client_ptb::{
...
impl PTB {
    ...
    pub async fn execute(self, context: &mut WalletContext) -> Result<(), Error> {
        ...
        let transaction_response = match transaction_response {
            MySoClientCommandResult::ComputeTransactionDigest(_)
            | MySoClientCommandResult::DryRun(_)
            | MySoClientCommandResult::SerializedUnsignedTransaction(_)
            | MySoClientCommandResult::SerializedSignedTransaction(_) => {
                println!("{transaction_response}");
                return Ok(());
            }
            MySoClientCommandResult::TransactionBlock(response) => response,
...
pub fn ptb_description() -> clap::Command {
    clap::Command::new("myso client ptb")
        .about(
        ...
        .long_help(
            ...
            \n --make-move-vec <myso::coin::Coin<myso::myso::MYSO>> [gas]"
        )
        ...
        .arg(arg!(
            ...
            "Publish the Move package. It takes as input the folder where the package exists.\
            \n\nExamples:\
            \n --move-call myso::tx_context::sender\
            \n --assign sender\
            \n --publish \".\"\
            \n --assign upgrade_cap\
