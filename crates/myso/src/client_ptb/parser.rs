...
use myso_types::{Identifier, base_types::ObjectID};
...
impl<'a, I: Iterator<Item = &'a str>> ProgramParser<'a, I> {
    ...
    fn parse_mvr_address(
        ...
        match prefix {
            ...
            Some(sp!(_, prefix)) => {
                ...
                match self.peek() {
                    sp!(sp, L(T::Ident, s)) => {
                        if s != "myso" {
                            error!(sp => help: { "Expected 'myso' extension" },
                    ...
                    sp!(sp, l) => {
                        error!(
                            sp,
                            "Expected a valid MVR domain extension ('.myso') but got {}", l
                        )
                    }
                }
            }
        ...
        self.expect(T::ForwardSlash).map_err(|e| {
            err!(e.span => help: {
                "MVR packages must contain a slash and be in the format '(@<domain>|<domain>.myso)/<package>[/<version>]'"
...
mod tests {
    ...
    fn test_parse_types() {
        let inputs = vec![
            ...
            "myso::object::ID",
            "0x2::object::UID",
            "3::staking_pool::StakedMySo",
            // Generic types
            "0x2::coin::Coin<2::myso::MYSO>",
            "myso::table::Table<myso::object::ID, vector<0x1::option::Option<u32>>>",
        ];
        let mut parsed = Vec::new();
        for input in inputs {
            let mut x = shlex::split(input).unwrap();
    ...
    fn test_parse_commands() {
        let inputs = vec![
            ...
            // Move Call
            "--move-call 0x3::myso_system::request_add_stake system coins.0 validator",
