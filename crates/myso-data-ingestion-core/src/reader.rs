...
use myso_types::full_checkpoint_content::{Checkpoint, CheckpointData};
...
enum RemoteStore {
    ...
    Rest(myso_rpc_api::Client),
    Hybrid(Box<dyn ObjectStore>, myso_rpc_api::Client),
}
...
impl CheckpointReader {
    ...
    fn start_remote_fetcher(&mut self) -> mpsc::Receiver<Result<(Arc<CheckpointData>, usize)>> {
        ...
        let store = if let Some((fn_url, remote_url)) = url.split_once('|') {
            ...
            RemoteStore::Hybrid(object_store, myso_rpc_api::Client::new(fn_url).unwrap())
        } else if url.ends_with("/rest") {
            RemoteStore::Rest(myso_rpc_api::Client::new(url).unwrap())
        } else {
            let object_store = create_remote_store_client(
                url,
                self.remote_store_options.clone(),
