    ...
    async fn process_object(
            ...
            .and_then(|tag| object.data.try_as_move().map(|mo| (tag, mo.contents())))
        {
            ...
            .await
            {
                ...
                Err(err)
                    if err
                        .downcast_ref::<myso_types::object::bounded_visitor::Error>()
                        .filter(|e| {
                            matches!(e, myso_types::object::bounded_visitor::Error::OutOfBudget)
                        })
                        .is_some() =>
                {
                    self.metrics
        ...
        let (struct_tag, myso_move_struct) = if let Some(move_struct) = move_struct {
            match move_struct.into() {
                MySoMoveStruct::WithTypes { type_, fields } => {
                    (Some(type_), Some(MySoMoveStruct::WithFields(fields)))
                }
        ...
        let row = ObjectRow {
            ...
            object_json: myso_move_struct.map(|x| x.to_json_value().to_string()),
        };
        Ok(Some(row))
    }
}
