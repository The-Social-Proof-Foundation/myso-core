...
use myso_analytics_indexer::config::BatchSizeConfig;
use myso_analytics_indexer::config::FileFormat;
use myso_analytics_indexer::config::IndexerConfig;
use myso_analytics_indexer::config::OutputStoreConfig;
use myso_analytics_indexer::config::PipelineConfig;
use myso_analytics_indexer::metrics::Metrics;
use myso_analytics_indexer::pipeline::Pipeline;
use myso_analytics_indexer::store::AnalyticsStore;
use myso_analytics_indexer::tables::CheckpointRow;
use myso_analytics_indexer::tables::DynamicFieldRow;
...
use myso_analytics_indexer::tables::TransactionObjectRow;
use myso_analytics_indexer::tables::TransactionRow;
use myso_analytics_indexer::tables::WrappedObjectRow;
...
impl TestHarness {
    ...
    async fn run_indexer(&self, config: IndexerConfig) {
        ...
        let service = myso_analytics_indexer::build_analytics_indexer(config, metrics, registry)
            .await
...
async fn test_migration_no_files_after_checkpoint_error() {
    ...
    let result =
        myso_analytics_indexer::build_analytics_indexer(migration_config, metrics, registry).await;
