...
use crate::reader::types::{
    ...
    ProfileBadgeRow, ProfileEventRow, ProfilePlatformEventRow, RecommendationDetail,
    ReservationPoolInfo, ReservationStatus, SelectedBadgeInfo, SocialGraphChartData,
    SocialGraphChartQuery, SocialProofTokenInfo, UniversalUserResult,
};
use myso_pg_db::Db;
...
pub(crate) async fn get_profile_platform_memberships(
    ...
    limit: i64,
    offset: i64,
) -> Result<Vec<ProfilePlatformMembershipRow>, SocialError> {
    let mut conn = db.connect().await?;
    #[derive(QueryableByName)]
    struct Row {
        ...
        #[diesel(sql_type = Timestamp)]
        platform_created_at: chrono::NaiveDateTime,
        #[diesel(sql_type = Timestamp)]
        platform_updated_at: chrono::NaiveDateTime,
        #[diesel(sql_type = Bool)]
        is_approved: bool,
        #[diesel(sql_type = Nullable<Timestamp>)]
        approval_changed_at: Option<chrono::NaiveDateTime>,
        #[diesel(sql_type = Nullable<Text>)]
    ...
    let recommendations = rows
        ...
        .map(|r| {
            ...
            RecommendationDetail {
                ...
                username: r.username.unwrap_or_else(|| r.owner_address.clone()),
