...
impl Builder {
    ...
    pub fn add_validator_signature(mut self, keypair: &AuthorityKeyPair) -> Self {
        ...
        let checkpoint_signature = {
            let intent_msg = IntentMessage::new(
                Intent::myso_app(IntentScope::CheckpointSummary),
                checkpoint.clone(),
            );
        ...
        let token_distribution_schedule =
            if let Some(token_distribution_schedule) = &self.token_distribution_schedule {
                ...
                TokenDistributionSchedule::new_for_validators_with_default_allocation(
                    validators.iter().map(|v| v.info.myso_address()),
                )
            };
    ...
    fn committee(objects: &[Object]) -> Committee {
        let myso_system_object =
            get_myso_system_state(&objects).expect("MySo System State object must always exist");
        myso_system_object
    ...
    fn validate_output(&self) {
        ...
        let system_state = match unsigned_genesis.myso_system_object() {
            MySoSystemState::V1(inner) => inner,
            MySoSystemState::V2(_) => unreachable!(),
            #[cfg(msim)]
            _ => {
            ...
            .zip(system_state.validators.active_validators.iter())
        {
            ...
            assert!(
                address_to_pool_id
                    .insert(metadata.myso_address, onchain_validator.staking_pool.id)
                    .is_none()
            );
            assert_eq!(validator.info.myso_address(), metadata.myso_address);
            assert_eq!(validator.info.protocol_key(), metadata.myso_pubkey_bytes());
        ...
        assert_eq!(
            system_state.stake_subsidy.max_apy_bps,
            stake_subsidy_max_apy_bps,
        );
        assert_eq!(
            system_state.stake_subsidy.min_apy_bps,
            stake_subsidy_min_apy_bps,
        );
        assert_eq!(
            system_state.stake_subsidy.intended_duration_years,
            stake_subsidy_intended_duration_years,
        );
        ...
        let mut gas_objects: BTreeMap<ObjectID, (&Object, GasCoin)> = unsigned_genesis
            ...
            .collect();
        let mut staked_myso_objects: BTreeMap<ObjectID, (&Object, StakedMySo)> = unsigned_genesis
            .objects()
            .iter()
            .filter_map(|o| StakedMySo::try_from(o).ok().map(|s| (o.id(), (o, s))))
            .collect();
        ...
        for allocation in token_distribution_schedule.allocations {
            if let Some(staked_with_validator) = allocation.staked_with_validator {
                ...
                let staked_myso_object_id = staked_myso_objects
                    .iter()
                    .find(|(_k, (o, s))| {
                ...
                assert_eq!(staked_myso_object.1.principal(), allocation.amount_mist);
                assert_eq!(staked_myso_object.1.pool_id(), staking_pool_id);
                assert_eq!(staked_myso_object.1.activation_epoch(), 0);
            } else {
        ...
        if !self.parameters.allow_insertion_of_extra_objects {
            ...
            assert!(staked_myso_objects.is_empty());
        }
...
fn get_genesis_protocol_config(version: ProtocolVersion) -> ProtocolConfig {
    ...
    // MYSO_PROTOCOL_CONFIG_CHAIN_OVERRIDE if necessary
...
fn build_unsigned_genesis_data(
    ...
    token_distribution_schedule.validate();
    token_distribution_schedule.check_all_stake_operations_are_for_valid_validators(
        genesis_validators.iter().map(|v| v.myso_address),
    );
    ...
    let mut system_packages =
        myso_framework_snapshot::load_bytecode_snapshot(parameters.protocol_version.as_u64())
...
// on `myso_system` which is mocked in some tests, but would be in the loader
    ...
    let genesis_transaction = {
        let genesis_objects = objects
            ...
            .map(|mut object| {
                ...
                myso_types::transaction::GenesisObject::RawObject {
                    data: object.data,
                    owner: object.owner,
                }
            })
        ...
        myso_types::transaction::VerifiedTransaction::new_genesis_transaction(genesis_objects)
            .into_inner()
    };
...
pub fn generate_genesis_system_object(
    ...
    let pt = {
        ...
        if protocol_config.enable_bridge() {
            ...
            let bridge_chain_id = builder.pure(BridgeChainId::MySoCustom).unwrap();
...
mod test {
    ...
    fn ceremony() {
        ...
        let validator = ValidatorInfo {
            ...
            account_address: MySoAddress::from(account_key.public()),
