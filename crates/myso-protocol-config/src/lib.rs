...
// Version 3: gas model v2, including all myso conservation fixes. Fix for loaded child object
...
//             Add `myso::table_vec::drop` to the framework via a system package upgrade.
...
// Version 19: Changes to myso-system package to enable liquid staking.
...
// Version 25: Add myso::table_vec::swap and myso::table_vec::swap_remove to system packages.
// Version 26: New gas model version.
//             Add support for receiving objects off of other objects in devnet only.
// Version 28: Add myso::zklogin::verify_zklogin_id and related functions to myso framework.
//             Enable transaction effects v2 in devnet.
// Version 29: Add verify_legacy_zklogin_address flag to myso framework, this add ability to verify
...
//             Add support for getting object ID referenced by receiving object in myso framework.
//             Create new execution layer version, and preserve previous behavior in v1.
//             Update semantics of `myso::transfer::receive` and add `myso::transfer::public_receive`.
...
// Version 42: Migrate myso framework and related code to Move 2024
...
//             Modified myso-system package to enable withdrawal of stake before it becomes active.
//             Enable soft bundle in devnet and testnet.
//             Core macro visibility in myso core framework.
...
//             Max gas budget moved to 50_000 MYSO
//             Max gas price moved to 50 MYSO
...
// Version 74: Enable load_nitro_attestation move function in myso framework in devnet.
...
//             Enable load_nitro_attestation move function in myso framework in testnet.
...
// Version 88: Update `myso-system` package to use `calculate_rewards` function.
...
// Version 91: Minor changes in MySo Framework. Include CheckpointDigest in consensus dedup key for checkpoint signatures (V2).
...
pub struct ProtocolConfig {
    ...
    /// Max size of objects a transaction can write to disk after completion. Enforce by the MySo adapter.
    ...
    /// Max size of objects a system transaction can write to disk after completion. Enforce by the MySo adapter.
    ...
    /// Maximum size of the `contents` part of an object, in bytes. Enforced by the MySo adapter when effects are produced.
    ...
    /// Maximum size of a Move package object, in bytes. Enforced by the MySo adapter at the end of a publish transaction.
    ...
    // === Execution gas costs ====
    /// Base cost for any MySo transaction
    ...
    // === Storage gas costs ===
    /// Per-byte cost of storing an object in the MySo global object store. Some of this cost may be refundable if the object is later freed
    obj_data_cost_refundable: Option<u64>,

    // Per-byte cost of storing an object in the MySo transaction log (e.g., in CertifiedTransactionEffects)
...
impl ProtocolConfig {
    ...
    pub fn get_for_version(version: ProtocolVersion, chain: Chain) -> Self {
        ...
        if std::env::var("MYSO_PROTOCOL_CONFIG_OVERRIDE_ENABLE").is_ok() {
            warn!(
            ...
            let overrides: ProtocolConfigOptional =
                serde_env::from_env_with_prefix("MYSO_PROTOCOL_CONFIG_OVERRIDE")
        ...
        for cur in 2..=version.0 {
            match cur {
                ...
                3 => {
                    ...
                    // max gas budget is in MIST and an absolute value 50MYSO
                    cfg.max_tx_gas = Some(50_000_000_000);
                    // min gas budget is in MIST and an absolute value 2000MIST or 0.000002MYSO
                ...
                72 => {
                    ...
                    // max gas budget is in MIST and an absolute value 50_000 MYSO
                    cfg.max_tx_gas = Some(50_000_000_000_000);
                    // max gas price is in MIST and an absolute value 50 MYSO
                ...
                112 => {
                    ...
                    cfg.max_move_package_size = Some(384 * 1024);
                }
...
macro_rules! check_limit_by_meter {
    ($is_metered:expr, $x:expr, $metered_limit:expr, $unmetered_hard_limit:expr, $metric:expr) => {{
        ...
        use myso_protocol_config::check_limit_in_range;
