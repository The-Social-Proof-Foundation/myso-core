...
use myso_telemetry::send_telemetry_event;
...
fn main() {
    ...
    let runtimes = MySoRuntimes::new(&config);
    ...
    info!("MySo Node version: {VERSION}");
    info!(
...

    {
        ...
        {
            myso_metrics_push_client::start_metrics_push_task(
    ...
    // if it deadlocks.
    let node_once_cell = Arc::new(AsyncOnceCell::<Arc<myso_node::MySoNode>>::new());
    let node_once_cell_clone = node_once_cell.clone();

    // let myso-node signal main to shutdown runtimes
    ...
    runtimes.myso_node.spawn(async move {
        match myso_node::MySoNode::start_async(config, registry_service, server_version).await {
            Ok(myso_node) => node_once_cell_clone
                .set(myso_node)
        ...
        // when we get a shutdown signal from myso-node, forward it on to the runtime_shutdown_channel here in
        // main to signal runtimes to all shutdown.
        tokio::select! {
           _ = shutdown_rx.recv() => {
                runtime_shutdown_tx.send(()).expect("failed to forward shutdown signal from myso-node to myso-node main");
            }
        }
    ...
    runtimes.metrics.spawn(async move {
        ...
        info!("MySo chain identifier: {chain_identifier}");
        prometheus_registry
        ...
        myso_node::admin::run_admin_server(node, admin_interface_port, Some(filter_handle)).await
    });
