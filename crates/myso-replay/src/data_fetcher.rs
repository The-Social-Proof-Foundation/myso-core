...
use myso_json_rpc_types::MySoObjectData;
...
use myso_types::base_types::{ObjectID, SequenceNumber, VersionNumber};
...
pub(crate) trait DataFetcher {
    ...
    ) -> Result<MySoTransactionBlockResponse, ReplayEngineError>;
    ...
    async fn get_epoch_change_events(
        &self,
        reverse: bool,
    ) -> Result<Vec<MySoEvent>, ReplayEngineError>;
...
impl RemoteFetcher {
    pub fn new(rpc_client: MySoClient) -> Self {
        Self {
            rpc_client,
...
impl DataFetcher for RemoteFetcher {
    ...
    async fn multi_get_versioned(
        ...
        let options = MySoObjectDataOptions::bcs_lossless();

        let objs: Vec<_> = to_fetch
            .iter()
            .map(|(object_id, version)| MySoGetPastObjectRequest {
                object_id: *object_id,
                version: *version,
            })
            .collect();
    ...
    async fn get_child_object(
        ...
        let response = self
            ...
            .await
            .map_err(|q| ReplayEngineError::MySoRpcError { err: q.to_string() })?;
    ...
    async fn multi_get_latest(
        ...
        join_all(objectsx)
            ...
            .iter()
            .flatten()
            .map(obj_from_myso_obj_response)
            .collect::<Result<Vec<_>, _>>()
            .map(|mut x| {
                // Add the cached objects to the result
                x.extend(cached);
    ...
    async fn get_transaction(
        ...
        let tx_fetch_opts = MySoTransactionBlockResponseOptions::full_content();
    ...
    async fn get_epoch_change_events(
        ...
        let mut epoch_change_events: Vec<MySoEvent> = vec![];
...
fn convert_past_obj_response(resp: MySoPastObjectResponse) -> Result<Object, ReplayEngineError> {
    match resp {
        MySoPastObjectResponse::VersionFound(o) => obj_from_myso_obj_data(&o),
        MySoPastObjectResponse::ObjectDeleted(r) => Err(ReplayEngineError::ObjectDeleted {
            id: r.object_id,
            version: r.version,
            digest: r.digest,
        }),
        ...
        MySoPastObjectResponse::VersionTooHigh {
            object_id,
            asked_version,
            latest_version,
...
#[allow(clippy::result_large_err)]
fn obj_from_myso_obj_response(o: &MySoObjectResponse) -> Result<Object, ReplayEngineError> {
    let o = o.object().map_err(ReplayEngineError::from)?.clone();
    obj_from_myso_obj_data(&o)
}

#[allow(clippy::result_large_err)]
fn obj_from_myso_obj_data(o: &MySoObjectData) -> Result<Object, ReplayEngineError> {
...
#[allow(clippy::result_large_err)]
pub fn extract_epoch_and_version(ev: MySoEvent) -> Result<(u64, u64), ReplayEngineError> {
