...
use myso_types::MYSO_DENY_LIST_OBJECT_ID;
use myso_types::error::MySoErrorKind;
use myso_types::execution_params::{
...
use myso_types::storage::{PackageObject, get_module};
...
pub struct ExecutionSandboxState {
    ...
    pub local_exec_effects: MySoTransactionBlockEffects,
...
impl ExecutionSandboxState {
    ...
    pub fn check_effects(&self) -> Result<(), ReplayEngineError> {
        let MySoTransactionBlockEffects::V1(mut local_effects) = self.local_exec_effects.clone();
        let MySoTransactionBlockEffects::V1(on_chain_effects) =
            self.transaction_info.effects.clone();

        // Handle backwards compatibility with the new `abort_error` field in
        // `MySoTransactionBlockEffects`
        if on_chain_effects.abort_error.is_none() {
            local_effects.abort_error = None;
        }
        let local_effects = MySoTransactionBlockEffects::V1(local_effects);
        let on_chain_effects = MySoTransactionBlockEffects::V1(on_chain_effects);
    ...
    pub fn diff_effects(
        on_chain_effects: &MySoTransactionBlockEffects,
        local_effects: &MySoTransactionBlockEffects,
    ) -> String {
...
impl LocalExec {
    ...
    pub async fn multi_download(
        ...
        while num_retries_for_timeout >= 0 {
            match self.fetcher.multi_get_versioned(objs).await {
                ...
                Err(ReplayEngineError::MySoRpcRequestTimeout) => {
                    warn!(
    ...
    pub fn download_latest_object(
        ...
        match resp {
            ...
            Err(err) => Err(ReplayEngineError::MySoRpcError {
                err: err.to_string(),
            }),
        }
    }
    ...
    pub async fn get_checkpoint_txs(
        ...
        self.fetcher
            ...
            .await
            .map_err(|e| ReplayEngineError::MySoRpcError { err: e.to_string() })
    }
    ...
    pub async fn execution_engine_execute_with_tx_info_impl(
        ...
        let effects =
            MySoTransactionBlockEffects::try_from(effects).map_err(ReplayEngineError::from)?;
    ...
    pub async fn certificate_execute_with_sandbox_state(
        ...
        let (gas_status, input_objects) = myso_transaction_checks::check_certificate_input(
            &executable,
            input_objects,
            &protocol_config,
            reference_gas_price,
        ...
        let executor = myso_execution::executor(&protocol_config, true).unwrap();
        let early_execution_error = get_early_execution_error(
            executable.digest(),
            &input_objects,
            &HashSet::new(),
    ...
    /// Must be called after `init_for_execution`
    /// This executes from `myso_core::authority::AuthorityState::try_execute_immediately`
    #[allow(clippy::result_large_err)]
    ...
    /// Must be called after `init_for_execution`
    /// This executes from `myso_adapter::execution_engine::execute_transaction_to_effects`
    #[allow(clippy::result_large_err)]
        ...
        match parse_effect_error_for_denied_coins(status) {
            Some(coin_type) => {
                ...
                if !config_id_and_version
                    .iter()
                    .any(|(id, _)| id == &MYSO_DENY_LIST_OBJECT_ID)
                {
                    let deny_list_oid_version = self.download_latest_object(&MYSO_DENY_LIST_OBJECT_ID)
                        .ok()
                        .flatten()
                        .expect("Unable to download the deny list object for a transaction that requires it")
                        .version();
                    config_id_and_version.push((MYSO_DENY_LIST_OBJECT_ID, deny_list_oid_version));
                }
    ...
    async fn resolve_tx_components(
        ...
        let sender = match tx_info.clone().transaction.unwrap().data {
            myso_json_rpc_types::MySoTransactionBlockData::V1(tx) => tx.sender,
        };
        let MySoTransactionBlockEffects::V1(effects) = tx_info.clone().effects.unwrap();
        ...
        let gas_data = match tx_info.clone().transaction.unwrap().data {
            myso_json_rpc_types::MySoTransactionBlockData::V1(tx) => tx.gas_data,
        };
        ...
        Ok(OnChainTransactionInfo {
            ...
            effects: MySoTransactionBlockEffects::V1(effects),
            receiving_objs,
            config_objects,
    ...
    async fn resolve_tx_components_from_dump(
        ...
        let effects = MySoTransactionBlockEffects::try_from(effects).unwrap();
...
impl BackingPackageStore for LocalExec {
    ...
    fn get_package_object(&self, package_id: &ObjectID) -> MySoResult<Option<PackageObject>> {
        fn inner(self_: &LocalExec, package_id: &ObjectID) -> MySoResult<Option<Object>> {
            // If package not present fetch it from the network
            self_
                .get_or_download_object(package_id, true /* we expect a Move package*/)
                .map_err(|e| MySoErrorKind::Storage(e.to_string()).into())
        }
    ...
    fn read_child_object(
        ...
        fn inner(
            ...
            if child_object.version() > child_version_upper_bound {
                return Err(MySoErrorKind::Unknown(format!(
            ...
            if child_object.owner != Owner::ObjectOwner(parent.into()) {
                return Err(MySoErrorKind::InvalidChildObjectAccess {
                    object: *child,
                    given_parent: parent,
...
impl ModuleResolver for LocalExec {
    ...
    /// We do not download
    fn get_module(&self, module_id: &ModuleId) -> MySoResult<Option<Vec<u8>>> {
        fn inner(self_: &LocalExec, module_id: &ModuleId) -> MySoResult<Option<Vec<u8>>> {
...
impl GetModule for LocalExec {
    ...
    fn get_module_by_id(&self, id: &ModuleId) -> MySoResult<Option<Self::Item>> {
...
pub fn get_executor(
    ...
    let silent = true;
    myso_execution::executor(&protocol_config, silent)
        .expect("Creating an executor should not fail here")
}

fn parse_effect_error_for_denied_coins(status: &MySoExecutionStatus) -> Option<String> {
    let MySoExecutionStatus::Failure { error } = status else {
        return None;
    };
