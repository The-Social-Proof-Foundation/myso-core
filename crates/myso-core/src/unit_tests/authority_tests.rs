...
use myso_json_rpc_types::{
    ...
    MySoTransactionBlockEffectsV1, MySoTypeTag,
};
...
use myso_types::execution::SharedInput;
...
use myso_types::{
    MOVE_STDLIB_PACKAGE_ID, MYSO_CLOCK_OBJECT_ID, MYSO_FRAMEWORK_PACKAGE_ID,
...
use myso_types::{MYSO_CLOCK_OBJECT_SHARED_VERSION, digests::Digest};
use myso_types::{dynamic_field::DynamicFieldType, messages_consensus::ConsensusTransaction};
...
async fn test_dry_run_transaction_block() {
    ...
    let (response, _, _, _) = fullnode
        ...
        .await
        .unwrap();
    assert_eq!(*response.effects.status(), MySoExecutionStatus::Success);
...
async fn test_dry_run_no_gas_big_transfer() {
    ...
    builder.transfer_myso(recipient, Some(amount));
    ...
    let (dry_run_res, _, _, _) = fullnode
        ...
        .await
        .unwrap();
    assert_eq!(*dry_run_res.effects.status(), MySoExecutionStatus::Success);
}
...
async fn test_dev_inspect_gas_coin_argument() {
    ...
    let pt = {
        let mut builder = ProgrammableTransactionBuilder::new();
        builder.pay_myso(vec![recipient], vec![amount]).unwrap();
        builder.finish()
    };
    let kind = TransactionKind::programmable(pt);
    ...
    assert_eq!(arg, &MySoArgument::GasCoin);
    check_coin_value(
        arg_value,
        arg_type,
...
fn check_coin_value(actual_value: &[u8], actual_type: &MySoTypeTag, expected_value: u64) {
...
async fn test_dry_run_dev_inspect_dynamic_field_too_new() {
    ...
    match effects {
        MySoTransactionBlockEffects::V1(MySoTransactionBlockEffectsV1 { abort_error, .. }) => {
...
async fn test_dry_run_dev_inspect_max_gas_version() {
    ...
    let DevInspectResults { effects, .. } = fullnode
        ...
        .await
        .unwrap();
    assert_eq!(effects.status(), &MySoExecutionStatus::Success);
...
async fn test_transfer_package() {
    ...
    let package_object_ref = authority_state
        .get_myso_system_package_object_ref()
        .await
        .unwrap();
...
#[tokio::test]
async fn test_handle_transfer_myso_with_amount_insufficient_gas() {
    let (sender, sender_key): (_, AccountKeyPair) = get_key_pair();
    let recipient = dbg_addr(2);
    let object_id = ObjectID::random();
    let authority_state = init_state_with_ids(vec![(sender, object_id)]).await;
...
async fn test_invalid_mutable_clock_parameter() {
    ...
    assert_eq!(
        ...
        UserInputError::ImmutableParameterExpectedError {
            object_id: MYSO_CLOCK_OBJECT_ID
        }
    );
}
...
async fn test_invalid_randomness_parameter() {
    ...
    assert_eq!(
        UserInputError::try_from(e).unwrap(),
        UserInputError::ImmutableParameterExpectedError {
            object_id: MYSO_RANDOMNESS_STATE_OBJECT_ID
        }
    );
}
...
#[tokio::test]
async fn test_genesis_myso_system_state_object() {
    // This test verifies that we can read the genesis MySoSystemState object.
    ...
    let _myso_system_state =
        bcs::from_bytes::<MySoSystemStateWrapper>(move_object.contents()).unwrap();
    assert!(move_object.type_().is(&MySoSystemStateWrapper::type_()));
    let myso_system_state = authority_state
...
#[tokio::test]
async fn test_transfer_myso_no_amount() {
    let (sender, sender_key): (_, AccountKeyPair) = get_key_pair();
    let recipient = dbg_addr(2);
    let gas_object_id = ObjectID::random();
    let gas_object = Object::with_id_owner_for_testing(gas_object_id, sender);
    let init_balance = myso_types::gas::get_gas_balance(&gas_object).unwrap();
    ...
    let gas_ref = gas_object.compute_object_reference();
    let tx_data = TransactionData::new_transfer_myso(
        recipient,
        sender,
        None,
        gas_ref,
...
#[tokio::test]
async fn test_transfer_myso_with_amount() {
    let (sender, sender_key): (_, AccountKeyPair) = get_key_pair();
    let recipient = dbg_addr(2);
    let gas_object_id = ObjectID::random();
    let gas_object = Object::with_id_owner_for_testing(gas_object_id, sender);
...
#[tokio::test]
async fn test_clear_cache_reverts_transfer_myso() {
...
async fn create_and_retrieve_df_info(function: &IdentStr) -> (MySoAddress, Vec<DynamicFieldInfo>) {
    let (sender, sender_key): (_, AccountKeyPair) = get_key_pair();
    let gas_object_id = ObjectID::random();
    let (authority_state, object_basics) =
        init_state_with_ids_and_object_basics(vec![(sender, gas_object_id)]).await;
...
pub async fn init_state_with_ids_and_object_basics<
    I: IntoIterator<Item = (MySoAddress, ObjectID)>,
>(
    objects: I,
) -> (Arc<AuthorityState>, ObjectRef) {
    let state = TestAuthorityBuilder::new().build().await;
...
async fn test_gas_smashing() {
    ...
    fn make_gas_coins(owner: MySoAddress, gas_amount: u64, coin_num: u64) -> Vec<Object> {
        let mut objects = vec![];
    ...
    async fn run_and_check(
        ...
        let balance =
            myso_types::gas::get_gas_balance(&state.get_object(&gas_coin_ids[0]).await.unwrap())
                .unwrap();
...
async fn test_publish_missing_dependency() {
    ...
    let mut builder = ProgrammableTransactionBuilder::new();
    builder.publish_immutable(modules, vec![MYSO_FRAMEWORK_PACKAGE_ID]);
    let kind = TransactionKind::programmable(builder.finish());
...
async fn test_publish_not_a_package_dependency() {
    ...
    deps.push(MYSO_SYSTEM_STATE_OBJECT_ID);
    builder.publish_immutable(modules, deps);
    let kind = TransactionKind::programmable(builder.finish());
    ...
    assert_eq!(
        MySoErrorKind::UserInputError {
            error: UserInputError::MoveObjectAsPackage {
                object_id: MYSO_SYSTEM_STATE_OBJECT_ID
            }
        },
        failure,
    )
...
pub fn create_gas_objects(num: u32, owner: MySoAddress) -> Vec<Object> {
    let mut objects = vec![];
    for _ in 0..num {
        let gas_object_id = ObjectID::random();
