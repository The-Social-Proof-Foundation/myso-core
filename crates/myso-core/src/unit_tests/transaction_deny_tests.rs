...
use crate::test_utils::make_transfer_myso_transaction;
...
use myso_types::utils::get_zklogin_user_address;
...
type Account = (MySoAddress, Ed25519KeyPair, Vec<ObjectRef>);
...
fn get_accounts_and_coins(
    ...
    let accounts: Vec<_> = network_config
        ...
        .iter()
        .map(|account| {
            let address: MySoAddress = account.public().into();
...
fn process_zklogin_tx(tx: Transaction, state: &Arc<AuthorityState>) -> MySoResult<()> {
...
fn transfer_with_account(
    ...
    let data = TransactionData::new_transfer_myso_allow_sponsor(
        sender_account.0,
        sender_account.0,
        None,
        sponsor_account.2[0],
...
fn assert_denied<T: std::fmt::Debug>(result: &MySoResult<T>) {
    assert!(matches!(
...
async fn test_shared_object_transaction_disabled() {
    ...
    let tx = TestTransactionBuilder::new(account.0, account.2[0], gas_price)
        .call_staking(account.2[1], MySoAddress::default())
...
async fn test_certificate_deny() {
    ...
    let (sender, key, gas_objects) = get_accounts_and_coins(&network_config, &state)
        .pop()
        .unwrap();
    let tx = make_transfer_myso_transaction(
        gas_objects[0],
        sender,
        None,
        sender,
