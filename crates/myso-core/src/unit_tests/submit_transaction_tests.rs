...
use myso_types::message_envelope::Message as _;
...
impl TestContext {
    ...
    fn build_test_transaction(&self) -> Transaction {
        ...
        .transfer_myso(None, self.sender)
        .build();
...
async fn test_submit_ping_request() {
    ...
    println!("Case 1. Ping request cannot contain transactions.");
    {
        ...
        let error: MySoError = response.unwrap_err().into();
        assert!(matches!(
            error.into_inner(),
...
async fn test_submit_transaction_wrong_epoch() {
    ...
    .transfer_myso(None, test_context.sender)
    .build();
...
async fn test_submit_transaction_gas_object_validation() {
    ...
    assert!(
        matches!(result, SubmitTxResult::Rejected { error } if matches!(error.as_inner(), MySoErrorKind::UserInputError {
...
async fn test_submit_batched_transactions_with_already_executed() {
    ...
    match &raw_response.results[0].inner {
        Some(myso_types::messages_grpc::RawValidatorSubmitStatus::Executed(_)) => {
