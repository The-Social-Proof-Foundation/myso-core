...
use myso_types::{MOVE_STDLIB_ADDRESS, MYSO_FRAMEWORK_ADDRESS};
...
fn test_to_json_value() {
    ...
    let myso_move_struct: MySoMoveStruct =
        BoundedVisitor::deserialize_struct(&event_bytes, &TestEvent::layout())
            .unwrap()
            .into();
    let json_value = myso_move_struct.to_json_value();
    assert_eq!(
        Some(&json!("1000000")),
...
impl TestEvent {
    fn type_() -> StructTag {
        StructTag {
            ...
            module: ident_str!("MYSO").to_owned(),
...
// TODO: Do we need this in the myso-types lib?
