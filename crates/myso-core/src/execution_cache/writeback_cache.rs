...
use myso_types::accumulator_root::{AccumulatorObjId, AccumulatorValue};
use myso_types::base_types::{
    EpochId, FullObjectID, ObjectID, ObjectRef, SequenceNumber, VerifiedExecutionData,
};
use myso_types::bridge::{Bridge, get_bridge};
use myso_types::digests::{ObjectDigest, TransactionDigest, TransactionEffectsDigest};
...
impl AccountFundsRead for WritebackCache {
    fn get_latest_account_amount(&self, account_id: &AccumulatorObjId) -> (u128, SequenceNumber) {
        let mut pre_root_version =
            ObjectCacheRead::get_object(self, &MYSO_ACCUMULATOR_ROOT_OBJECT_ID)
                .unwrap()
                .version();
        let mut loop_iter = 0;
        loop {
...
impl ObjectCacheRead for WritebackCache {
    ...
    fn get_lock(&self, obj_ref: ObjectRef, epoch_store: &AuthorityPerEpochStore) -> MySoLockResult {
        ...
        match self.get_object_by_id_cache_only("lock", &obj_ref.0) {
            ...
            CacheResult::NegativeHit => {
                Err(MySoError::from(UserInputError::ObjectNotFound {
                    object_id: obj_ref.0,
