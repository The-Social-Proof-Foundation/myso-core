...
use myso_types::{
    base_types::{FullObjectRef, MySoAddress, random_object_ref},
...
impl Scenario {
    ...
    fn new_outputs() -> TransactionOutputs {
        ...
        let (sender, keypair): (MySoAddress, AccountKeyPair) = get_key_pair_from_rng(&mut rng);
        let (receiver, _): (MySoAddress, AccountKeyPair) = get_key_pair_from_rng(&mut rng);
    ...
    pub async fn commit(&mut self, tx: TransactionDigest) -> MySoResult {
