...
impl SafeClientMetrics {
    pub fn new(metrics_base: &SafeClientMetricsBase, validator_address: AuthorityName) -> Self {
        ...
        let total_requests_handle_transaction_info_request = metrics_base
            ...
            .with_label_values(&[validator_address.as_str(), "handle_transaction_info_request"]);
...
impl<C: Clone> SafeClient<C> {
    ...
    fn get_committee(&self, epoch_id: &EpochId) -> MySoResult<Arc<Committee>> {
        self.committee_store
            .get_committee(epoch_id)?
            .ok_or(MySoErrorKind::MissingCommitteeAtEpoch(*epoch_id).into())
    }
    ...
    ) -> MySoResult<SignedTransactionEffects> {
    ...
    ) -> MySoResult<PlainTransactionInfoResponse> {
        fp_ensure!(
                ...
                match cert_opt {
                    Some(cert) => {
                        ...
                        ct.verify_committee_sigs_only(&committee).map_err(|e| {
                            MySoErrorKind::FailedToVerifyTxCertWithExecutedEffects {
    ...
    ) -> MySoResult<VerifiedObjectInfoResponse> {
    ...
    C: AuthorityAPI + Send + Sync + Clone + 'static,
{
    ...
    ) -> Result<SubmitTxResponse, MySoError> {
    ...
    pub async fn wait_for_effects(
        ...
        client_addr: Option<SocketAddr>,
    ) -> Result<WaitForEffectsResponse, MySoError> {
        let _timer = self.metrics.handle_certificate_latency.start_timer();
        ...
        match (events, events_digest) {
            ...
            (Some(events), None) => {
                if !events.data.is_empty() {
                    Err(MySoErrorKind::ByzantineAuthoritySuspicion {
                        authority: self.address,
    ...
    fn verify_objects<I>(&self, objects: &Option<Vec<Object>>, expected_refs: I) -> MySoResult
    where
        I: IntoIterator<Item = (ObjectID, ObjectRef)>,
    {
        if let Some(objects) = objects {
            ...
            for object in objects {
                ...
                {
                    return Err(MySoErrorKind::ByzantineAuthoritySuspicion {
                        authority: self.address,
    ...
    pub async fn handle_object_info_request(
        &self,
        request: ObjectInfoRequest,
    ) -> Result<VerifiedObjectInfoResponse, MySoError> {
    ...
    ) -> Result<PlainTransactionInfoResponse, MySoError> {
        self.metrics
            .total_requests_handle_transaction_info_request
            .inc();
    ...
    fn verify_checkpoint_sequence(
        ...
        if let (Some(e), Some(o)) = (expected_seq, observed_seq) {
            fp_ensure!(
                e == o,
                MySoError::from("Expected checkpoint number doesn't match with returned")
            );
        }
        Ok(())
    }
    ...
    fn verify_contents_exist<T, O>(
        ...
        match (request_content, checkpoint, contents) {
            ...
            (true, Some(_), None) | (false, _, Some(_)) | (_, None, Some(_)) => Err(
                MySoError::from("Checkpoint contents inconsistent with request"),
            ),
            _ => Ok(()),
        }
    }
    ...
    pub async fn handle_checkpoint(
        &self,
        request: CheckpointRequest,
    ) -> Result<CheckpointResponse, MySoError> {
        let resp = self
            .authority_client
    ...
    #[instrument(level = "trace", skip_all, fields(authority = ?self.address.concise()))]
    pub async fn handle_system_state_object(&self) -> Result<MySoSystemState, MySoError> {
        self.authority_client
    ...
    ) -> Result<ValidatorHealthResponse, MySoError> {
