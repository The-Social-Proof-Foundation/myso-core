...
use myso_network::tonic::metadata::KeyAndValueRef;
use myso_network::tonic::transport::Channel;
...
pub trait AuthorityAPI {
    ...
    ) -> Result<ObjectInfoResponse, MySoError>;

    /// Handle Object information requests for this account.
    async fn handle_transaction_info_request(
        &self,
        request: TransactionInfoRequest,
    ) -> Result<TransactionInfoResponse, MySoError>;
    ...
    ) -> Result<CheckpointResponseV2, MySoError>;
    ...
    ) -> Result<MySoSystemState, MySoError>;
...
pub struct NetworkAuthorityClient {
    client: MySoResult<ValidatorClient<Channel>>,
}
...
impl NetworkAuthorityClient {
    pub async fn connect(
        ...
        let tls_config = myso_tls::create_rustls_client_config(
            tls_target,
            myso_tls::MYSO_VALIDATOR_SERVER_NAME.to_string(),
            None,
        );
    ...
    pub fn connect_lazy(address: &Multiaddr, tls_target: NetworkPublicKey) -> Self {
        ...
        let client: MySoResult<_> = mysten_network::client::connect_lazy(address, tls_config)
    ...
    fn new_lazy(client: MySoResult<Channel>) -> Self {
        Self {
            client: client.map(ValidatorClient::new),
        }
    }

    pub(crate) fn client(&self) -> MySoResult<ValidatorClient<Channel>> {
        self.client.clone()
    }

    pub fn get_client_for_testing(&self) -> MySoResult<ValidatorClient<Channel>> {
        self.client()
    }
}
...
impl AuthorityAPI for NetworkAuthorityClient {
    /// Submits a transaction to the MySo network for certification and execution.
    async fn submit_transaction(
        &self,
        request: SubmitTxRequest,
        client_addr: Option<SocketAddr>,
...
pub fn make_network_authority_clients_with_network_config(
    ...
    for (name, (_state, network_metadata)) in committee.validators() {
        ...
        let tls_config = network_metadata
            ...
            .as_ref()
            .map(|key| {
                myso_tls::create_rustls_client_config(
                    key.clone(),
                    myso_tls::MYSO_VALIDATOR_SERVER_NAME.to_string(),
                    None,
                )
            })
            .ok_or(MySoError::from("network public key is not available"));
