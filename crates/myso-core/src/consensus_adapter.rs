...
pub trait ConsensusOverloadChecker: Sync + Send + 'static {
    fn check_consensus_overload(&self) -> MySoResult;
}
...
pub trait ConsensusClient: Sync + Send + 'static {
    async fn submit(
        ...
        transactions: &[ConsensusTransaction],
        epoch_store: &Arc<AuthorityPerEpochStore>,
    ) -> MySoResult<(Vec<ConsensusPosition>, BlockStatusReceiver)>;
}

/// Submit MySo certificates to the consensus.
            ...
            for transaction in &transactions[1..] {
                if is_user_tx_batch {
                    fp_ensure!(
                        ...
                        MySoErrorKind::InvalidTxKindInSoftBundle.into()
                    );
                    ...
                    return Err(MySoErrorKind::InvalidTxKindInSoftBundle.into());
                }
            }
        }
...
impl ConsensusOverloadChecker for ConsensusAdapter {
    fn check_consensus_overload(&self) -> MySoResult {
        fp_ensure!(
            self.check_limits(),
            MySoErrorKind::TooManyTransactionsPendingConsensus.into()
        );
        Ok(())
    }
}
...
impl SubmitToConsensus for Arc<ConsensusAdapter> {
    ...
    fn submit_best_effort(
        ...
        let permit = match self.submit_semaphore.clone().try_acquire_owned() {
            Ok(permit) => permit,
            Err(_) => {
                return Err(MySoErrorKind::TooManyTransactionsPendingConsensus.into());
            }
        };
    ...
    async fn test_await_submit_delay_user_transaction() {
        ...
        let consensus_adapter = ConsensusAdapter::new(
            ...
            *committee.authority_by_index(0).unwrap(),
            ...
            myso_protocol_config::ProtocolConfig::get_for_max_version_UNSAFE(),
        );
