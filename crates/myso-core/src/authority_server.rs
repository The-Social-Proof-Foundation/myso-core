...
use myso_network::{
    ...
    tonic,
    validator::server::MYSO_TLS_SERVER_NAME,
};
...
use myso_types::{
    ...
    error::{MySoErrorKind, UserInputError},
};
use myso_types::{
...
use myso_types::{effects::TransactionEvents, messages_grpc::SubmitTxType};
use myso_types::{error::*, transaction::*};
use myso_types::{
    fp_ensure,
    messages_checkpoint::{
...
use myso_config::local_ip_utils::new_local_tcp_address_for_testing;
use myso_types::messages_grpc::PingType;
...
pub struct AuthorityServerHandle {
    server_handle: myso_network::validator::server::Server,
}
...
impl AuthorityServer {
    ...
    pub async fn spawn_with_bind_address_for_test(
        ...
        let server = myso_network::validator::server::ServerBuilder::from_config(
            &config,
...
impl ValidatorService {
    ...
    pub fn handle_transaction_for_testing(&self, transaction: Transaction) -> MySoResult<()> {
    ...
    async fn collect_immutable_object_ids(
        ...
        objects
            ...
            .filter_map(|(obj, id)| {
                let Some(o) = obj else {
                    return Some(Err::<ObjectID, MySoError>(
            ...
            .collect::<MySoResult<Vec<ObjectID>>>()
    }
    ...
    async fn handle_submit_transaction(
        ...
        loop {
            ...
            match res {
                ...
                Err(err) => {
                    if max_retries > 0
                        && let MySoErrorKind::ValidatorHaltedAtEpochEnd = err.as_inner()
                    {
                        max_retries -= 1;
    ...
    async fn handle_submit_transaction_inner(
    ...
    ) -> MySoResult<(RawSubmitTxResponse, Weight)> {
        ...
        if is_ping_request {
            fp_ensure!(
                ...
                MySoErrorKind::InvalidRequest(format!(
        ...
        for (idx, tx_bytes) in request.transactions.iter().enumerate() {
            let transaction = match bcs::from_bytes::<Transaction>(tx_bytes) {
                ...
                Err(e) => {
                    ...
                    return Err(MySoErrorKind::TransactionDeserializationError {
    ...
    ) -> Result<RawSubmitTxResponse, MySoError> {
        let mut raw_results = Vec::new();
        for (i, result) in results.into_iter().enumerate() {
            let result = result.ok_or_else(|| MySoErrorKind::GenericAuthorityError {
    ...
    async fn handle_submit_to_consensus_for_position(
        ...
        Ok(rx_consensus_positions.await.map_err(|e| {
            MySoErrorKind::FailedToSubmitToConsensus(format!(
    ...
    ) -> MySoResult<(Option<TransactionEvents>, Vec<Object>, Vec<Object>)> {
...
impl ValidatorService {
    ...
    async fn wait_for_effects_impl(
        ...
        let response = timeout(
            ...
            epoch_store
                ...
                .map_err(|_| MySoErrorKind::EpochEnded(epoch_store.epoch())),
        )
        .await
    ...
    async fn wait_for_effects_response(
        ...
        epoch_store: &Arc<AuthorityPerEpochStore>,
    ) -> MySoResult<WaitForEffectsResponse> {
        if request.ping_type.is_some() {
            ...
            .await
            .map_err(|_| MySoErrorKind::TimeoutError)?;
        }
    ...
    ) -> WrappedServiceResponse<MySoSystemState> {
        let response = self
            .state
    ...
    async fn validator_health_impl(
        &self,
        _request: tonic::Request<myso_types::messages_grpc::RawValidatorHealthRequest>,
    ) -> WrappedServiceResponse<myso_types::messages_grpc::RawValidatorHealthResponse> {
        ...
        let typed_response = myso_types::messages_grpc::ValidatorHealthResponse {
        ...
        if let Some(traffic_controller) = &self.traffic_controller {
            if !traffic_controller.check(&client, &None).await {
                ...
                Err(tonic::Status::from_error(
                    MySoErrorKind::TooManyRequests.into(),
                ))
            } else {
                Ok(())
            }
    ...
    fn handle_traffic_resp<T>(
        ...
        let (error, spam_weight, unwrapped_response) = match wrapped_response {
            ...
            Err(status) => (
                Some(MySoError::from(status.clone())),
                Weight::zero(),
                Err(status.clone()),
            ),
        };
...
fn normalize(err: MySoError) -> Weight {
    match err.as_inner() {
        MySoErrorKind::UserInputError {
        ...
        | MySoErrorKind::WrongEpoch { .. } => Weight::one(),
        _ => Weight::zero(),
    }
}
...
impl Validator for ValidatorService {
    ...
    ) -> Result<tonic::Response<MySoSystemState>, tonic::Status> {
        handle_with_decoration!(self, get_system_state_object_impl, request)
    }

    async fn validator_health(
        &self,
        request: tonic::Request<myso_types::messages_grpc::RawValidatorHealthRequest>,
    ) -> Result<tonic::Response<myso_types::messages_grpc::RawValidatorHealthResponse>, tonic::Status>
    {
