...
use myso_authority_aggregation::quorum_map_then_reduce_with_timeout;
...
impl AuthorityAggregator<NetworkAuthorityClient> {
    ...
    pub fn new_from_epoch_start_state(
        ...
        let committee = epoch_start_state.get_myso_committee_with_network_metadata();
    ...
    A: AuthorityAPI + Send + Sync + 'static + Clone,
{
    ...
    pub async fn get_latest_object_version_for_testing(
        ...
        let result = quorum_map_then_reduce_with_timeout(
            ...
            .await.map_err(|_state| MySoError::from(UserInputError::ObjectNotFound {
                object_id,
                version: None,
            }))?;
        Ok(result.0)
    ...
    pub async fn get_latest_system_state_object_for_testing(
        &self,
    ) -> anyhow::Result<MySoSystemState> {
        #[derive(Debug, Default)]
        struct State {
            latest_system_state: Option<MySoSystemState>,
            total_weight: StakeUnit,
        }
        let initial_state = State::default();
        let result = quorum_map_then_reduce_with_timeout(
        ...
        if let Some(genesis) = self.get_genesis() {
            ...
            state
                ...
                .map(|v| {
                    ...
                    let name = metadata.myso_pubkey_bytes();
