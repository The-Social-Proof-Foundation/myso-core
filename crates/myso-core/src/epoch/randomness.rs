...
use myso_types::committee::{Committee, EpochId, StakeUnit};
use myso_types::crypto::{AuthorityKeyPair, RandomnessRound};
...
impl RandomnessManager {
    ...
    pub async fn start_dkg(&mut self) -> MySoResult {
    ...
    pub(crate) fn reserve_next_randomness(
        ...
        output: &mut ConsensusCommitOutput,
    ) -> MySoResult<Option<RandomnessRound>> {
        let epoch_store = self.epoch_store()?;
    ...
    fn epoch_store(&self) -> MySoResult<Arc<AuthorityPerEpochStore>> {
        self.epoch_store
            .upgrade()
            .ok_or(MySoErrorKind::EpochEnded(self.epoch).into())
    }
...
impl RandomnessReporter {
    ...
    /// the round.
    pub fn notify_randomness_in_checkpoint(&self, round: RandomnessRound) -> MySoResult {
        let epoch_store = self
            .epoch_store
            .upgrade()
            .ok_or(MySoErrorKind::EpochEnded(self.epoch))?;
    ...
    async fn test_dkg(version: u64) {
        ...
        for validator in network_config.validator_configs.iter() {
            ...
            let randomness_manager = RandomnessManager::try_new(
                ...
                myso_network::randomness::Handle::new_stub(),
