...
use myso_types::effects::{AccumulatorValue, TransactionEffectsAPI};
...
use myso_types::storage::BackingPackageStore;
use myso_types::storage::DynamicFieldKey;
use myso_types::storage::EpochInfo;
...
impl From<BalanceIndexInfo> for myso_types::storage::BalanceInfo {
    fn from(index_info: BalanceIndexInfo) -> Self {
        ...
        // Anyone can call `myso::balance::create_supply` and mint unbounded supply, potentially pushing
        ...
        myso_types::storage::BalanceInfo { balance }
    }
}
...
impl IndexStoreTables {
    ...
    fn init(
        ...
        rpc_config: &myso_config::RpcConfig,
    ) -> Result<(), StorageError> {
    ...
    fn extract_accumulator_version(
        &self,
        tx: &myso_types::full_checkpoint_content::CheckpointTransaction,
    ) -> Option<u64> {
        ...
        if pt.shared_input_objects().any(|obj| {
            obj.id == MYSO_ACCUMULATOR_ROOT_OBJECT_ID
                && obj.mutability == myso_types::transaction::SharedObjectMutability::Mutable
        }) {
            return tx.output_objects.iter().find_map(|obj| {
                if obj.id() == MYSO_ACCUMULATOR_ROOT_OBJECT_ID {
        ...
        for acc in acc_events {
            if let AccumulatorValue::EventDigest(event_digests) = &acc.write.value {
                ...
                if let Some(stream_id) =
                    myso_types::accumulator_root::stream_id_from_accumulator_event(&acc)
                {
    ...
    fn initialize_current_epoch(
        ...
        let system_state = myso_types::myso_system_state::get_myso_system_state(authority_store)
...
impl RpcIndexStore {
    ...
    pub fn new_without_init(dir: &Path) -> Self {
        ...
        Self {
            tables,
            pending_updates: Default::default(),
            rpc_config: myso_config::RpcConfig::default(),
        }
    }
    ...
    fn index_object(&mut self, object: Object) -> Result<(), StorageError> {
        match object.owner {
            ...
            Owner::ObjectOwner(parent) => {
                ...
                if parent == MYSO_ACCUMULATOR_ROOT_OBJECT_ID.into()
...
fn sparse_checkpoint_data_for_backfill(
    ...
    use myso_types::full_checkpoint_content::CheckpointTransaction;
    ...
    for ((tx, fx), ev) in transactions.into_iter().zip(effects).zip(events) {
        let input_objects =
            myso_types::storage::get_transaction_input_objects(authority_store, &fx)?;
        let output_objects =
            myso_types::storage::get_transaction_output_objects(authority_store, &fx)?;
...
fn get_address_balance_info(object: &Object) -> Option<(MySoAddress, StructTag, i128)> {
    ...
    let (key, value): (
        myso_types::accumulator_root::AccumulatorKey,
        myso_types::accumulator_root::AccumulatorValue,
...
mod tests {
    ...
    async fn test_events_compaction_filter() {
        ...
        let stream_id = MySoAddress::random_for_testing_only();
        ...
        let end_key = EventIndexKey {
            stream_id: MySoAddress::random_for_testing_only(),
