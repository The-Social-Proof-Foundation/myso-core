...
pub struct MySoTxValidator {
    ...
    metrics: Arc<MySoTxValidatorMetrics>,
}

impl MySoTxValidator {
    pub fn new(
        authority_state: Arc<AuthorityState>,
        epoch_store: Arc<AuthorityPerEpochStore>,
        checkpoint_service: Arc<dyn CheckpointServiceNotify + Send + Sync>,
        ...
        info!(
            "MySoTxValidator constructed for epoch {}",
            epoch_store.epoch()
        );
    ...
    fn validate_transactions(&self, txs: &[ConsensusTransactionKind]) -> Result<(), MySoError> {
        ...
        for tx in txs.iter() {
            match tx {
                ConsensusTransactionKind::CertifiedTransaction(_) => {
                    return Err(MySoErrorKind::UnexpectedMessage(
                ...
                ConsensusTransactionKind::RandomnessDkgMessage(_, bytes) => {
                    if bytes.len() > dkg_v1::DKG_MESSAGES_MAX_SIZE {
                        ...
                        return Err(MySoErrorKind::InvalidDkgMessageSize.into());
                    }
                }
                ...
                ConsensusTransactionKind::ExecutionTimeObservation(obs) => {
                    ...
                    if obs.estimates.len()
                        > epoch_store
                            ...
                            .unwrap()
                    {
                        return Err(MySoErrorKind::UnexpectedMessage(format!(
    ...
    fn vote_transaction(
        ...
        if epoch_store.protocol_config().address_aliases() {
            ...
            if !aliases_match || fail_point_always_report_aliases_changed {
                return Err(MySoErrorKind::AliasesChanged.into());
            }
        }
    ...
    fn verify_immutable_object_claims(
        ...
        for claimed_id in claimed_ids {
            if !input_refs_by_id.contains_key(claimed_id) {
                return Err(MySoErrorKind::ImmutableObjectClaimNotFoundInInput {
            ...
            .next()
        {
            ...
            return Err(MySoErrorKind::InvalidImmutableObjectClaim {
        ...
        if let Some(found_id) = found_immutable_ids
            ...
            .next()
        {
            return Err(MySoErrorKind::ImmutableObjectNotClaimed {
                object_id: *found_id,
            }
            .into());
        }
...
impl TransactionVerifier for MySoTxValidator {
...
pub struct MySoTxValidatorMetrics {
...
mod tests {
    ...
    use myso_config::transaction_deny_config::TransactionDenyConfigBuilder;
    ...
    async fn accept_valid_transaction() {
        ...
        let network_config =
            myso_swarm_config::network_config_builder::ConfigBuilder::new_with_temp_dir()
        ...
        let metrics = MySoTxValidatorMetrics::new(&Default::default());
        let validator = MySoTxValidator::new(
            state.clone(),
    ...
    async fn test_verify_and_vote_batch() {
        ...
        let validator = MySoTxValidator::new(
            ...
            state.epoch_store_for_testing().clone(),
            Arc::new(CheckpointServiceNoop {}),
            MySoTxValidatorMetrics::new(&Default::default()),
        );
    ...
    async fn accept_checkpoint_signature_v2() {
        let network_config =
            myso_swarm_config::network_config_builder::ConfigBuilder::new_with_temp_dir().build();
    ...
    async fn test_verify_immutable_object_claims() {
        ...
        // Test 4: Missing claim - immutable object not claimed - should fail
        {
            ...
            assert!(
                matches!(
                    err.as_inner(),
                    MySoErrorKind::ImmutableObjectNotClaimed { object_id }
        ...
        // Test 5: False claim - owned object claimed as immutable - should fail
        {
            ...
            assert!(
                matches!(
                    err.as_inner(),
                    MySoErrorKind::InvalidImmutableObjectClaim { claimed_object_id, .. }
        ...
        // Test 6: Claim not in inputs - should fail
        {
            ...
            assert!(
                matches!(
                    err.as_inner(),
                    MySoErrorKind::ImmutableObjectClaimNotFoundInInput { object_id }
                    if *object_id == immutable_id1
                ),
        ...
        // Test 7: Object not found (non-existent object) - should fail
        {
            ...
            let fake_ref = (
                non_existent_id,
                myso_types::base_types::SequenceNumber::new(),
                myso_types::digests::ObjectDigest::random(),
            );
            ...
            assert!(
                matches!(
                    err.as_inner(),
                    MySoErrorKind::UserInputError { error: UserInputError::ObjectNotFound { object_id, .. } }
        ...
        // Test 8: Version/digest mismatch for immutable object - should fail
        {
            ...
            let wrong_version_ref = (
                immutable_ref1.0,
                myso_types::base_types::SequenceNumber::from_u64(999),
                immutable_ref1.2,
            );
            ...
            assert!(
                matches!(
                    err.as_inner(),
                    MySoErrorKind::UserInputError { error: UserInputError::ObjectVersionUnavailableForConsumption { provided_obj_ref, current_version: _ } }
