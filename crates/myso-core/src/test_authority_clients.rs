...
impl AuthorityAPI for LocalAuthorityClient {
    async fn submit_transaction(
        ...
        if self.fault_config.fail_before_submit_transaction {
            return Err(MySoError::from("Mock error before submit_transaction"));
        }
        ...
        let deserialized_transaction = bcs::from_bytes::<Transaction>(&raw_request.transactions[0])
            .map_err(|e| MySoErrorKind::TransactionDeserializationError {
                error: e.to_string(),
            })?;
    ...
    async fn handle_system_state_object(
        ...
        self.state.get_myso_system_state_object_for_testing()
    }
...
pub struct MockAuthorityApi {
    delay: Duration,
    count: Arc<Mutex<u32>>,
    handle_object_info_request_result: Option<MySoResult<ObjectInfoResponse>>,
}
...
impl MockAuthorityApi {
    ...
    pub fn set_handle_object_info_request(&mut self, result: MySoResult<ObjectInfoResponse>) {
...
impl AuthorityAPI for MockAuthorityApi {
    /// Submit a new transaction to a MySo or Primary account.
    async fn submit_transaction(
        &self,
    ...
    async fn handle_transaction_info_request(
        ...
        Err(MySoErrorKind::TransactionNotFound {
