...
use myso_types::effects::{AccumulatorOperation, AccumulatorValue, TransactionEvents};
...
use myso_types::full_checkpoint_content::{CheckpointData, CheckpointTransaction};
use myso_types::messages_checkpoint::{CertifiedCheckpointSummary, CheckpointCommitment};
use myso_types::messages_consensus::ConsensusDeterminedVersionAssignments;
use myso_types::messages_grpc::ObjectInfoRequestKind;
use myso_types::move_package::TypeOrigin;
...
use myso_types::type_input::{StructInput, TypeInput};
use myso_types::{
...
fn get_registry() -> Result<Registry> {
    ...
    let kp1: MySoKeyPair =
        MySoKeyPair::Ed25519(get_key_pair_from_rng(&mut StdRng::from_seed([0; 32])).1);
    let kp2: MySoKeyPair =
        MySoKeyPair::Secp256k1(get_key_pair_from_rng(&mut StdRng::from_seed([0; 32])).1);
    let kp3: MySoKeyPair =
        MySoKeyPair::Secp256r1(get_key_pair_from_rng(&mut StdRng::from_seed([0; 32])).1);
    ...
    // ObjectID and MySoAddress are the same length
    ...
    let struct_tag = StructTag::from_str("0x2::coin::Coin<0x2::myso::MYSO>").unwrap();
    ...
    tracer
        .trace_type::<base_types::MySoAddress>(&samples)
        .unwrap();
...
#[derive(Debug, Parser)]
#[clap(
    name = "MySo format generator",
    about = "Trace serde (de)serialization to generate format descriptions for MySo types"
)]
struct Options {
...
const FILE_PATH: &str = "myso-core/tests/staged/myso.yaml";
