...
impl<S: Clone + Eq, const STRENGTH: bool> StakeAggregator<S, STRENGTH> {
    ...
    pub fn insert_generic(
        ...
        match self.data.entry(authority) {
            Entry::Occupied(oc) => {
                return InsertResult::Failed {
                    error: MySoErrorKind::StakeAggregatorRepeatedSigner {
                        signer: authority,
        ...
        if votes > 0 {
            ...
            InsertResult::Failed {
                error: MySoErrorKind::InvalidAuthenticator.into(),
            }
        }
    }
...
impl<const STRENGTH: bool> StakeAggregator<AuthoritySignInfo, STRENGTH> {
    ...
    pub fn insert<T: Message + Serialize>(
        ...
        if self.committee.epoch != sig.epoch {
            return InsertResult::Failed {
                error: MySoErrorKind::WrongEpoch {
...
mod stake_aggregator_tests {
    ...
    fn test_stake_aggregator_repeated_signer() {
        ...
        assert!(matches!(
            ...
            InsertResult::Failed {
                error
            } if matches!(error.as_inner(),  MySoErrorKind::StakeAggregatorRepeatedSigner { .. } )
        ));
        ...
        let MySoErrorKind::StakeAggregatorRepeatedSigner {
            signer,
            conflicting_sig,
        } = error.into_inner()
        else {
    ...
    fn test_stake_aggregator_from_iter_with_error() {
        ...
        let result: MySoResult<StakeAggregator<(), true>> =
