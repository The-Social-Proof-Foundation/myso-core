...
use myso_types::{
    ...
    error::{MySoError, MySoErrorKind, UserInputError},
    messages_checkpoint::{
...
struct MockAuthority {
    _name: AuthorityName,
    submit_responses: Arc<StdMutex<HashMap<TransactionDigest, Result<SubmitTxResult, MySoError>>>>,
...
impl MockAuthority {
    ...
    fn set_submit_response(
        ...
        response: Result<SubmitTxResult, MySoError>,
    ) {
    ...
    // Test 4: Transaction with errors in submission.
    {
        ...
        for (i, mock_authority) in mock_authorities.iter().enumerate() {
            if i < 2 {
                mock_authority.set_submit_response(
                    tx_digest,
                    Err(MySoErrorKind::ValidatorOverloadedRetryAfter {
                        retry_after_secs: 1,
                    }
                    .into()),
                );
