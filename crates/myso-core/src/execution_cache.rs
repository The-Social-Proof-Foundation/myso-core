...
use crate::authority::authority_store::{ExecutionLockWriteGuard, MySoLockResult};
...
use myso_types::digests::{TransactionDigest, TransactionEffectsDigest};
...
/// Should only be used for myso-tool or tests. Nodes must use build_execution_cache which
...
pub trait ObjectCacheRead: Send + Sync {
    fn get_package_object(&self, id: &ObjectID) -> MySoResult<Option<PackageObject>>;
        ...
        for (object_opt, object_ref) in objects.into_iter().zip(object_refs) {
            match object_opt {
                None => {
                    ...
                    return Err(MySoErrorKind::UserInputError { error }.into());
                }
                Some(object) => {
                    result.push(object);
                }
    ...
    fn get_lock(&self, obj_ref: ObjectRef, epoch_store: &AuthorityPerEpochStore) -> MySoLockResult;

    // This method is considered "private" - only used by multi_get_objects_with_more_accurate_error_return
    fn _get_live_objref(&self, object_id: ObjectID) -> MySoResult<ObjectRef>;
    ...
    fn check_owned_objects_are_live(&self, owned_object_refs: &[ObjectRef]) -> MySoResult;

    fn get_myso_system_state_object_unsafe(&self) -> MySoResult<MySoSystemState>;

    fn get_bridge_object_unsafe(&self) -> MySoResult<Bridge>;
...
pub trait TransactionCacheRead: Send + Sync {
    ...
    fn get_transactions_and_serialized_sizes(
        &self,
        digests: &[TransactionDigest],
    ) -> MySoResult<Vec<Option<(VerifiedTransaction, usize)>>> {
...
pub trait ExecutionCacheWrite: Send + Sync {
    ...
    fn validate_owned_object_versions(&self, owned_input_objects: &[ObjectRef]) -> MySoResult;
...
pub trait ExecutionCacheReconfigAPI: Send + Sync {
    ...
    fn expensive_check_myso_conservation(
        &self,
        old_epoch_store: &AuthorityPerEpochStore,
    ) -> MySoResult;

    fn checkpoint_db(&self, path: &Path) -> MySoResult;
    ...
    ($implementor: ident) => {
        ...
        impl BackingPackageStore for $implementor {
            fn get_package_object(
                &self,
                package_id: &ObjectID,
            ) -> MySoResult<Option<PackageObject>> {
    ...
    ($implementor: ident) => {
        ...
        impl ExecutionCacheReconfigAPI for $implementor {
            ...
            fn expensive_check_myso_conservation(
                ...
                self.store
                    .expensive_check_myso_conservation(self, old_epoch_store)
            }

            fn checkpoint_db(&self, path: &std::path::Path) -> MySoResult {
