...
impl ExecutionTimeObserver {
    pub fn spawn(
        ...
        let mut observer = Self {
            ...
            next_generation_number: SystemTime::now()
                ...
                .as_micros()
                .try_into()
                .expect("This build of myso is not supported in the year 500,000"),
        };
    ...
    fn share_observations(&mut self, to_share: Vec<(ExecutionTimeObservationKey, Duration)>) {
        ...
        if let Err(e) = self.consensus_adapter.submit_best_effort(
            ...
            if !matches!(e.as_inner(), MySoErrorKind::EpochEnded(_)) {
                epoch_store
                    .metrics
                    .epoch_execution_time_observations_dropped
...
// Key used to save StoredExecutionTimeObservations in the MySo system state object's
// `extra_fields` Bag.
