...
use myso_types::base_types::{ConciseableName, ObjectRef};
use myso_types::committee::Committee;
use myso_types::committee::CommitteeTrait;
use myso_types::crypto::{AuthoritySignInfo, AuthorityStrongQuorumSignInfo, RandomnessRound};
use myso_types::digests::{ChainIdentifier, TransactionEffectsDigest};
...
use myso_types::execution::{ExecutionTimeObservationKey, ExecutionTiming};
...
pub enum ConsensusCertificateResult {
    ...
    MySoTransaction(VerifiedExecutableTransaction),
...
impl AuthorityEpochTables {
    ...
    fn load_reconfig_state(&self) -> MySoResult<ReconfigState> {
        let state = self
            .reconfig_state
    ...
    pub fn get_all_pending_consensus_transactions(&self) -> MySoResult<Vec<ConsensusTransaction>> {
        Ok(self
    ...
    pub fn remove_executed_tx_subtle(&self, digest: &TransactionDigest) -> MySoResult {
        self.executed_transactions_to_checkpoint.remove(digest)?;
        Ok(())
    }

    pub fn get_last_consensus_index(&self) -> MySoResult<Option<ExecutionIndices>> {
        Ok(self
    ...
    pub fn get_last_consensus_stats(&self) -> MySoResult<Option<ExecutionIndicesWithStats>> {
        Ok(self.last_consensus_stats.get(&LAST_CONSENSUS_STATS_ADDR)?)
    }

    pub fn get_locked_transaction(&self, obj_ref: &ObjectRef) -> MySoResult<Option<LockDetails>> {
        Ok(self
    ...
    ) -> MySoResult<Vec<Option<LockDetails>>> {
        Ok(self
            .owned_object_locked_transactions
    ...
    fn get_all_deferred_transactions_v2(
        &self,
    ) -> MySoResult<BTreeMap<DeferralKey, Vec<VerifiedExecutableTransactionWithAliases>>> {
        Ok(self
            ...
            .chain(
                self.deferred_transactions_with_aliases_v2
                    .safe_iter()
                    // The v2 table contains the deprecated format with MySoAddress instead of u8.
...
impl AuthorityPerEpochStore {
    ...
    pub fn tables(&self) -> MySoResult<Arc<AuthorityEpochTables>> {
        match self.tables.load_full() {
            Some(tables) => Ok(tables),
            None => Err(MySoErrorKind::EpochEnded(self.epoch()).into()),
        }
    }
    ...
    ) -> MySoResult<Option<GlobalStateHash>> {
        Ok(self
            .tables()?
        ...
        let system_state = match system_state {
            ...
            MySoSystemState::V1(_) => {
                if committee.epoch() > 1 {
                    error!(
                        "`PerObjectCongestionControlMode::ExecutionTimeEstimate` cannot load execution time observations to MySoSystemState because it has an old version. This should not happen outside tests."
                    );
                }
                return itertools::Either::Left(std::iter::empty());
            }
            ...
            | MySoSystemState::SimTestDeepV2(_) => {
                return itertools::Either::Left(std::iter::empty());
            }
        };
    ...
    pub fn store_reconfig_state(&self, new_state: &ReconfigState) -> MySoResult {
        self.tables()?
            .reconfig_state
    ...
    pub fn insert_signed_transaction(&self, transaction: VerifiedSignedTransaction) -> MySoResult {
        Ok(self
            .tables()?
            .signed_transactions
    ...
    pub fn tx_key_to_digest(&self, key: &TransactionKey) -> MySoResult<Option<TransactionDigest>> {
        let tables = self.tables()?;
    ...
    pub fn get_effects_signature(
        &self,
        tx_digest: &TransactionDigest,
    ) -> MySoResult<Option<AuthoritySignInfo>> {
        let tables = self.tables()?;
    ...
    pub fn get_signed_effects_digest(
        &self,
        tx_digest: &TransactionDigest,
    ) -> MySoResult<Option<TransactionEffectsDigest>> {
        let tables = self.tables()?;
    ...
    pub fn get_transaction_cert_sig(
        &self,
        tx_digest: &TransactionDigest,
    ) -> MySoResult<Option<AuthorityStrongQuorumSignInfo>> {
    ...
    pub fn try_acquire_owned_object_locks_post_consensus(
    ...
    ) -> MySoResult<Vec<(ObjectRef, LockDetails)>> {
        ...
        for obj_ref in owned_object_refs {
            ...
            {
                return Err(MySoErrorKind::ObjectLockConflict {
                    obj_ref: *obj_ref,
    ...
    pub fn get_last_consensus_stats(&self) -> MySoResult<ExecutionIndicesWithStats> {
        assert!(
    ...
    ) -> MySoResult<Vec<(CheckpointSequenceNumber, GlobalStateHash)>> {
        self.tables()?
            .state_hash_by_checkpoint
    ...
    ) -> MySoResult<Vec<GlobalStateHash>> {
        let tables = self.tables()?;
        Ok(self
    ...
    pub fn handle_finalized_checkpoint(
        ...
        let tables = match self.tables() {
            ...
            Err(e) if matches!(e.as_inner(), MySoErrorKind::EpochEnded(_)) => return Ok(()),
            Err(e) => return Err(e),
        };
    ...
    ) -> MySoResult<HashMap<ConsensusObjectSequenceKey, SequenceNumber>> {
    ...
    ) -> MySoResult<AssignedTxAndVersions> {
    ...
    ) -> MySoResult<Vec<(DeferralKey, Vec<VerifiedExecutableTransactionWithAliases>)>> {
    ...
    ) -> Result<Vec<CheckpointSequenceNumber>, MySoError> {
        let tables = self.tables()?;
    ...
    pub fn clear_override_protocol_upgrade_buffer_stake(&self) -> MySoResult {
        warn!(
            epoch = ?self.epoch(),
    ...
    pub fn set_override_protocol_upgrade_buffer_stake(&self, new_stake_bps: u64) -> MySoResult {
        warn!(
            ?new_stake_bps,
            epoch = ?self.epoch(),
    ...
    pub fn record_capabilities_v2(&self, capabilities: &AuthorityCapabilitiesV2) -> MySoResult {
    ...
    pub fn get_capabilities_v2(&self) -> MySoResult<Vec<AuthorityCapabilitiesV2>> {
    ...
    pub(crate) fn get_new_jwks(&self, round: u64) -> MySoResult<Vec<ActiveJwk>> {
    ...
    pub(crate) fn get_randomness_last_round_timestamp(&self) -> MySoResult<Option<TimestampMs>> {
        if let Some(ts) = self
            .consensus_quarantine
            .read()
    ...
    pub fn verify_transaction_with_current_aliases(
        &self,
        tx: Transaction,
    ) -> MySoResult<VerifiedTransactionWithAliases> {
    ...
    pub(crate) fn verify_consensus_transaction(
        ...
        // Signatures are verified as part of the consensus payload verification in MySoTxValidator
    ...
    fn db_batch(&self) -> MySoResult<DBBatch> {
    ...
    ) -> MySoResult<Vec<(CheckpointHeight, PendingCheckpoint)>> {
        Ok(self
            .consensus_quarantine
            .read()
    ...
    fn pending_checkpoint_exists(&self, index: &CheckpointHeight) -> MySoResult<bool> {
        Ok(self
            .consensus_quarantine
            .read()
    ...
    pub fn get_pending_checkpoints_v2(
        &self,
        last: Option<CheckpointHeight>,
    ) -> MySoResult<Vec<(CheckpointHeight, PendingCheckpointV2)>> {
        Ok(self
            .consensus_quarantine
            .read()
    ...
    fn pending_checkpoint_exists_v2(&self, index: &CheckpointHeight) -> MySoResult<bool> {
        Ok(self
            .consensus_quarantine
            .read()
    ...
    pub fn last_built_checkpoint_builder_summary(
        &self,
    ) -> MySoResult<Option<BuilderCheckpointSummary>> {
    ...
    pub fn last_built_checkpoint_summary(
        &self,
    ) -> MySoResult<Option<(CheckpointSequenceNumber, CheckpointSummary)>> {
    ...
    pub fn get_built_checkpoint_summary(
        &self,
        sequence: CheckpointSequenceNumber,
    ) -> MySoResult<Option<CheckpointSummary>> {
        if let Some(BuilderCheckpointSummary { summary, .. }) =
    ...
    pub fn get_last_checkpoint_signature_index(&self) -> MySoResult<u64> {
        Ok(self
            .tables()?
