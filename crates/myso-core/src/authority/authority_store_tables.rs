...
use myso_types::storage::{FullObjectKey, MarkerValue};
...
pub struct AuthorityPerpetualTables {
    ...
    /// Expected total amount of MYSO in the network. This is expected to remain constant
    ...
    pub(crate) expected_network_myso_amount: DBMap<(), u64>,
...
impl AuthorityPerpetualTables {
    ...
    pub fn open(
        ...
        let configs = vec![
            ...
            (
                "expected_network_myso_amount".to_string(),
                ThConfig::new(0, 1, KeyType::uniform(1)),
            ),
            (
    ...
    // Constructs `myso_types::object::Object` from `StoreObjectWrapper`.
    // Returns `None` if object was deleted/wrapped
    pub fn object(
        &self,
        object_key: &ObjectKey,
        store_object: StoreObjectWrapper,
    ) -> Result<Option<Object>, MySoError> {
    ...
    pub fn get_latest_object_or_tombstone(
        &self,
        object_id: ObjectID,
    ) -> Result<Option<(ObjectKey, StoreObjectWrapper)>, MySoError> {
        let mut iterator = self.objects.reversed_safe_iter_with_bounds(
            Some(ObjectKey::min_for_id(&object_id)),
            Some(ObjectKey::max_for_id(&object_id)),
        )?;
    ...
    ) -> MySoResult<Option<TrustedTransaction>> {
    ...
    pub fn get_effects(&self, digest: &TransactionDigest) -> MySoResult<Option<TransactionEffects>> {
    ...
    pub fn insert_object_test_only(&self, object: Object) -> MySoResult {
    ...
    // fallible get object methods for myso-tool, which may need to attempt to read a corrupted database
    pub fn get_object_fallible(&self, object_id: &ObjectID) -> MySoResult<Option<Object>> {
        let obj_entry = self
            .objects
