...
use myso_types::base_types::ConsensusObjectSequenceKey;
...
use myso_types::executable_transaction::VerifiedExecutableTransactionWithAliases;
...
impl SharedObjVerManager {
    ...
    ) -> MySoResult<ConsensusSharedObjVerAssignment>
    where
        T: AsTx + 'a,
    {
    ...
    pub fn assign_versions_for_certificate(
        ...
        let accumulator_version = if epoch_store.accumulators_enabled() {
            ...
            let accumulator_version = *shared_input_next_versions
                .get(&(MYSO_ACCUMULATOR_ROOT_OBJECT_ID, accumulator_initial_version))
            ...
            } in shared_input_objects.iter()
            {
                let assigned_version = match cancellation_info {
                    ...
                    Some(CancelConsensusCertificateReason::DkgFailed) => {
                        if id == &MYSO_RANDOMNESS_STATE_OBJECT_ID {
...
mod tests {
    ...
    use myso_types::{MYSO_ACCUMULATOR_ROOT_OBJECT_ID, MYSO_RANDOMNESS_STATE_OBJECT_ID};
    ...
    async fn test_assign_versions_from_consensus_with_randomness() {
        ...
        assert_eq!(
            epoch_store
                .get_next_object_version(&MYSO_RANDOMNESS_STATE_OBJECT_ID, randomness_obj_version)
                .unwrap(),
            randomness_obj_version
        );
        ...
        assert_eq!(
            *shared_input_next_versions
                .get(&(MYSO_RANDOMNESS_STATE_OBJECT_ID, randomness_obj_version))
                .unwrap(),
        ...
        assert_eq!(
            ...
            vec![
                (
                    ...
                    AssignedVersions::new(
                        vec![(
                            (MYSO_RANDOMNESS_STATE_OBJECT_ID, randomness_obj_version),
                            randomness_obj_version
                        )],
                        Some(expected_accumulator_version)
                    )
    ...
    async fn test_assign_versions_from_consensus_with_cancellation() {
        ...
        // transaction.
        shared_input_next_versions
            .remove(&(MYSO_ACCUMULATOR_ROOT_OBJECT_ID, SequenceNumber::from_u64(1)));
        assert_eq!(
            shared_input_next_versions,
            HashMap::from([
            ...
            ConsensusSharedObjVerAssignment {
                assigned_versions: AssignedTxAndVersions::new(vec![
                    ...
                    (
                        ...
                        AssignedVersions {
                            shared_object_versions: vec![(
                                (MYSO_ACCUMULATOR_ROOT_OBJECT_ID, acc_version),
                                acc_version
                            )],
                            accumulator_version: Some(acc_version),
                        }
