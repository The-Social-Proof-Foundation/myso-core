...
/// A cache that maintains rejection reasons (MySoError) when validators cast reject votes for transactions
...
pub(crate) struct TransactionRejectReasonCache {
    cache: RwLock<BTreeMap<ConsensusPosition, MySoError>>,
...
impl TransactionRejectReasonCache {
    ...
    /// uniquely identifies the transaction and the `reason` (MySoError) that caused the transaction to be rejected during voting
    /// should be provided.
    pub fn set_rejection_vote_reason(&self, position: ConsensusPosition, reason: &MySoError) {
    ...
    pub fn get_rejection_vote_reason(&self, position: ConsensusPosition) -> Option<MySoError> {
        debug_assert_eq!(position.epoch, self.epoch, "Epoch mismatch");
...
mod test {
    ...
    async fn test_set_rejection_vote_reason_and_get_reason() {
        ...
        {
            let reason = MySoErrorKind::ValidatorHaltedAtEpochEnd.into();
        ...
        {
            let reason = MySoErrorKind::InvalidTransactionDigest.into();
            cache.set_rejection_vote_reason(position, &reason);
            assert_eq!(cache.get_rejection_vote_reason(position), Some(reason));
        }
        ...
        for round in 0..TOTAL_ROUNDS {
            for transaction_index in 0..5 {
                cache.set_rejection_vote_reason(
                    ...
                    &MySoErrorKind::InvalidTransactionDigest.into(),
                );
            }
        }
            ...
            for transaction_index in 0..5 {
                ...
                if round <= 2 {
                    ...
                    assert_eq!(
                        ...
                        Some(MySoErrorKind::InvalidTransactionDigest.into())
                    );
                }
            }
        }
