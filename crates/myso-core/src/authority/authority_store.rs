...
struct AuthorityStoreMetrics {
    ...
    myso_conservation_live_object_count: IntGauge,
    myso_conservation_live_object_size: IntGauge,
    myso_conservation_imbalance: IntGauge,
    myso_conservation_storage_fund: IntGauge,
    myso_conservation_storage_fund_imbalance: IntGauge,
...
impl AuthorityStoreMetrics {
    pub fn new(registry: &Registry) -> Self {
        Self {
            myso_conservation_check_latency: register_int_gauge_with_registry!(
                "myso_conservation_check_latency",
                "Number of seconds took to scan all live objects in the store for MYSO conservation check",
                registry,
            ).unwrap(),
            myso_conservation_live_object_count: register_int_gauge_with_registry!(
                "myso_conservation_live_object_count",
                "Number of live objects in the store",
                registry,
            ).unwrap(),
            myso_conservation_live_object_size: register_int_gauge_with_registry!(
                "myso_conservation_live_object_size",
                "Size in bytes of live objects in the store",
                registry,
            ).unwrap(),
            myso_conservation_imbalance: register_int_gauge_with_registry!(
                "myso_conservation_imbalance",
                "Total amount of MYSO in the network - 10B * 10^9. This delta shows the amount of imbalance",
                registry,
            ).unwrap(),
            myso_conservation_storage_fund: register_int_gauge_with_registry!(
                "myso_conservation_storage_fund",
                "Storage Fund pool balance (only includes the storage fund proper that represents object storage)",
                registry,
            ).unwrap(),
            myso_conservation_storage_fund_imbalance: register_int_gauge_with_registry!(
                "myso_conservation_storage_fund_imbalance",
...
/// S is a template on Authority signature state. This allows MySoDataStore to be used on either
/// authorities or non-authorities. Specifically, when storing transactions and effects,
/// S allows MySoDataStore to either store the authority signed version or unsigned version.
pub struct AuthorityStore {
    ...
    /// Whether to enable expensive MYSO conservation check at epoch boundaries.
    enable_epoch_myso_conservation_check: bool,
...
impl AuthorityStore {
    ...
    pub async fn open(
        ...
        let enable_epoch_myso_conservation_check = config
            .expensive_safety_check_config
            .enable_epoch_myso_conservation_check();
        ...
        let epoch_start_configuration = if perpetual_tables.database_is_empty()? {
            ...
            let epoch_start_configuration = EpochStartConfiguration::new(
                genesis.myso_system_object().into_epoch_start_state(),
    ...
    pub fn get_recovery_epoch_at_restart(&self) -> MySoResult<EpochId> {
    ...
    ) -> MySoResult<Option<TransactionEffects>> {
    ...
    pub fn effects_exists(&self, effects_digest: &TransactionEffectsDigest) -> MySoResult<bool> {
        self.perpetual_tables
            .effects
    ...
    pub fn is_tx_already_executed(&self, digest: &TransactionDigest) -> MySoResult<bool> {
        Ok(self
            .perpetual_tables
            .executed_effects
            .contains_key(digest)?)
    ...
    pub fn get_latest_marker(
        ...
        epoch_id: EpochId,
    ) -> MySoResult<Option<(SequenceNumber, MarkerValue)>> {
    ...
    pub async fn notify_read_root_state_hash(
        &self,
        epoch: EpochId,
    ) -> MySoResult<(CheckpointSequenceNumber, GlobalStateHash)> {
    ...
    pub fn deprecated_get_transaction_checkpoint(
        &self,
        digest: &TransactionDigest,
    ) -> MySoResult<Option<(EpochId, CheckpointSequenceNumber)>> {
        Ok(self
            .perpetual_tables
            .executed_transactions_to_checkpoint
            .get(digest)?)
    ...
    pub fn deprecated_multi_get_transaction_checkpoint(
        &self,
        digests: &[TransactionDigest],
    ) -> MySoResult<Vec<Option<(EpochId, CheckpointSequenceNumber)>>> {
        Ok(self
            .perpetual_tables
            .executed_transactions_to_checkpoint
            .multi_get(digests)?
    ...
    pub fn database_is_empty(&self) -> MySoResult<bool> {
    ...
    pub fn multi_object_exists_by_key(&self, object_keys: &[ObjectKey]) -> MySoResult<Vec<bool>> {
        Ok(self
            .perpetual_tables
            .objects
    ...
    fn get_object_ref_prior_to_key(
        ...
        object_id: &ObjectID,
        version: VersionNumber,
    ) -> Result<Option<ObjectRef>, MySoError> {
    ...
    ) -> Result<Vec<Option<Object>>, MySoError> {
        let wrappers = self
            .perpetual_tables
            .objects
    ...
    /// Get many objects
    pub fn get_objects(&self, objects: &[ObjectID]) -> Result<Vec<Option<Object>>, MySoError> {
    ...
    pub(crate) fn insert_genesis_object(&self, object: Object) -> MySoResult {
    ...
    fn insert_object_direct(&self, object_ref: ObjectRef, object: &Object) -> MySoResult {
    ...
    pub(crate) fn bulk_insert_genesis_objects(&self, objects: &[Object]) -> MySoResult<()> {
    ...
    pub fn bulk_insert_live_objects(
        ...
        if *expected_sha3_digest != sha3_digest {
            ...
            return Err(MySoError::from("Sha does not match"));
        }
        batch.write()?;
        Ok(())
    }
    ...
    pub fn get_epoch_start_configuration(&self) -> MySoResult<Option<EpochStartConfiguration>> {
    ...
    pub(crate) fn persist_transaction(&self, tx: &VerifiedExecutableTransaction) -> MySoResult {
    ...
    pub fn check_owned_objects_are_live(&self, objects: &[ObjectRef]) -> MySoResult {
        let locks = self
            .perpetual_tables
            .live_owned_object_markers
            .multi_get(objects)?;
    ...
    /// Returns MySoErrorKind::ObjectLockAlreadyInitialized if the lock already exists and is locked to a transaction
    ...
    pub fn initialize_live_object_markers(
        ...
        if !is_force_reset {
            ...
            if !existing_live_object_markers.is_empty() {
                ...
                return Err(MySoErrorKind::ObjectLockAlreadyInitialized {
    ...
    pub fn get_latest_object_or_tombstone(
        &self,
        object_id: ObjectID,
    ) -> Result<Option<(ObjectKey, ObjectOrTombstone)>, MySoError> {
    ...
    /// If the intent is for testing, you can use AuthorityState:: get_myso_system_state_object_for_testing.
    pub fn get_myso_system_state_object_unsafe(&self) -> MySoResult<MySoSystemState> {
        get_myso_system_state(self.perpetual_tables.as_ref())
    }

    pub fn expensive_check_myso_conservation<T>(
        self: &Arc<Self>,
        type_layout_store: T,
    ...
    {
        if !self.enable_epoch_myso_conservation_check {
            return Ok(());
        }

        let executor = old_epoch_store.executor();
        info!("Starting MYSO conservation check. This may take a while..");
        let cur_time = Instant::now();
        let mut pending_objects = vec![];
        let mut count = 0;
        let mut size = 0;
        let (mut total_myso, mut total_storage_rebate) = thread::scope(|s| {
        ...
        for object in pending_objects {
            total_storage_rebate += object.storage_rebate;
            total_myso +=
                object.get_total_myso(layout_resolver.as_mut()).unwrap() - object.storage_rebate;
        }
        info!(
        ...
        let system_state = self
            ...
            .expect("Reading myso system state object cannot fail")
            .into_myso_system_state_summary();
        let storage_fund_balance = system_state.storage_fund_total_object_storage_rebates;
        info!(
            "Total MYSO amount in the network: {}, storage fund balance: {}, total storage rebate: {} at beginning of epoch {}",
            total_myso, storage_fund_balance, total_storage_rebate, system_state.epoch
        );
        ...
        self.metrics
            .myso_conservation_imbalance
            .set((total_myso as i128 - TOTAL_SUPPLY_MIST as i128) as i64);
        ...
        if let Some(expected_myso) = self
            .perpetual_tables
            .expected_network_myso_amount
            .get(&())
            .expect("DB read cannot fail")
        {
            fp_ensure!(
                total_myso == expected_myso,
                MySoError::from(
                    format!(
                        "Inconsistent state detected at epoch {}: total myso: {}, expecting {}",
                        system_state.epoch, total_myso, expected_myso
                    )
                    .as_str()
                )
            );
...
impl GlobalStateHashStore for AuthorityStore {
    ...
    fn get_root_state_hash_for_epoch(
        &self,
        epoch: EpochId,
    ) -> MySoResult<Option<(CheckpointSequenceNumber, GlobalStateHash)>> {
        self.perpetual_tables
    ...
    fn get_root_state_hash_for_highest_epoch(
        &self,
    ) -> MySoResult<Option<(EpochId, (CheckpointSequenceNumber, GlobalStateHash))>> {
        Ok(self
            .perpetual_tables
            .root_state_hash_by_epoch
...
pub type MySoLockResult = MySoResult<ObjectLockStatus>;
