...
//! IndexStore supports creation of various ancillary indexes of state in MySoDataStore.
...
use myso_json_rpc_types::{MySoObjectDataFilter, TransactionFilter};
...
type OwnerIndexKey = (MySoAddress, ObjectID);
...
impl CoinIndexKey2 {
    ...
    pub fn new(owner: MySoAddress, coin_type: String, balance: u64, object_id: ObjectID) -> Self {
        Self {
            owner,
            coin_type,
...
pub struct IndexStoreCaches {
    per_coin_type_balance: ShardedLruCache<(MySoAddress, TypeTag), MySoResult<TotalBalance>>,
    all_balances: ShardedLruCache<MySoAddress, MySoResult<Arc<HashMap<TypeTag, TotalBalance>>>>,
    pub locks: MutexTable<MySoAddress>,
}

#[derive(Default)]
pub struct IndexStoreCacheUpdates {
    _locks: Vec<OwnedMutexGuard<()>>,
    per_coin_type_balance_changes: Vec<((MySoAddress, TypeTag), MySoResult<TotalBalance>)>,
    all_balance_changes: Vec<(MySoAddress, MySoResult<Arc<AllBalance>>)>,
}
...
pub struct IndexStoreTables {
    ...
    /// Index from myso address to transactions initiated by that address.
    transactions_from_addr: DBMap<(MySoAddress, TxSequenceNumber), TransactionDigest>,

    /// Index from myso address to transactions that were sent to that address.
    transactions_to_addr: DBMap<(MySoAddress, TxSequenceNumber), TransactionDigest>,
    ...
    /// composite key of the MySoAddress of their owner and the object ID of the object.
    ...
    address_balances: DBMap<(MySoAddress, TypeTag), ()>,
    ...
    event_by_sender: DBMap<(MySoAddress, EventId), EventIndex>,
...
impl IndexStore {
        ...
        let table_options = DBMapTableConfigMap::new(BTreeMap::from([
            (
                ...
                compaction_filter_config_by_key(
                    ...
                    |(_, id): (MySoAddress, TxSequenceNumber)| id,
                ),
            ),
            (
                ...
                compaction_filter_config_by_key(
                    ...
                    db_options.clone(),
                    pruner_watermark.clone(),
                    |(_, sequence_number): (MySoAddress, TxSequenceNumber)| sequence_number,
                ),
            ),
            (
            ...
            (
                ...
                compaction_filter_config_by_key(
                    ...
                    db_options.clone(),
                    pruner_watermark.clone(),
                    |(_, event_id): (MySoAddress, EventId)| event_id.0,
                ),
            ),
            (
    ...
    pub fn index_coin(
    ...
    ) -> MySoResult<IndexStoreCacheUpdates> {
        ...
        let mut addresses: HashSet<MySoAddress> = HashSet::new();
        addresses.extend(
        ...
        let mut balance_changes: HashMap<MySoAddress, HashMap<TypeTag, TotalBalance>> =
            HashMap::new();
        // Index coin info
            ...
            .flat_map(|(address, balance_map)| {
                balance_map.iter().map(|(type_tag, balance)| {
                    (
                        ...
                        Ok::<TotalBalance, MySoError>(*balance),
                    )
                })
            })
            .collect();
        let all_balance_changes: Vec<_> = balance_changes
            ...
            .map(|(address, balance_map)| {
                (
                    address,
                    Ok::<Arc<HashMap<TypeTag, TotalBalance>>, MySoError>(Arc::new(balance_map)),
                )
            })
            .collect();
    ...
    pub fn index_tx(
        ...
        let address_balance_updates = accumulator_events.into_iter().filter_map(|event| {
            ...
            let coin_type = myso_types::balance::Balance::maybe_get_balance_type_param(ty)?;
    ...
    pub fn get_transactions(
        ...
        let cursor = if let Some(cursor) = cursor {
            Some(
                self.get_transaction_seq(&cursor)?
                    .ok_or(MySoErrorKind::TransactionNotFound { digest: cursor })?,
            )
        } else {
            None
        };
        match filter {
            ...
            Some(_) => Err(MySoErrorKind::UserInputError {
    ...
    pub fn get_transaction_seq(
        &self,
        digest: &TransactionDigest,
    ) -> MySoResult<Option<TxSequenceNumber>> {
    ...
    pub fn all_events(
        ...
        limit: usize,
        descending: bool,
    ) -> MySoResult<Vec<(TransactionEventsDigest, TransactionDigest, usize, u64)>> {
        Ok(if descending {
            self.tables
                .event_order
    ...
    pub fn events_by_transaction(
        ...
        let seq = self
            ...
            .ok_or(MySoErrorKind::TransactionNotFound { digest: *digest })?;
        Ok(if descending {
            self.tables
                .event_order
    ...
    pub fn prune(&self, cut_time_ms: u64) -> MySoResult<TxSequenceNumber> {
        match self
            .tables
            .event_by_time
            .reversed_safe_iter_with_bounds(
    ...
    pub fn insert_genesis_objects(&self, object_index_changes: ObjectIndexChanges) -> MySoResult {
    ...
    pub fn checkpoint_db(&self, path: &Path) -> MySoResult {
    ...
    pub fn get_coin_object_balance(
        ...
        if force_disable_cache {
            ...
            .map_err(|e| {
                MySoErrorKind::ExecutionError(format!("Failed to read balance frm DB: {:?}", e))
                    .into()
            });
        }
    ...
    pub fn get_all_coin_object_balances(
        &self,
        owner: MySoAddress,
    ) -> MySoResult<Arc<HashMap<TypeTag, TotalBalance>>> {
        let force_disable_cache = read_size_from_env(ENV_VAR_DISABLE_INDEX_CACHE).unwrap_or(0) > 0;
        let metrics_cloned = self.metrics.clone();
        let coin_index_cloned = self.tables.coin_index_2.clone();
        if force_disable_cache {
    ...
    /// Read balance for a `MySoAddress` and `CoinType` from the backend database
    #[instrument(skip_all)]
    ...
    /// Read all balances for a `MySoAddress` from the backend database
    #[instrument(skip_all)]
    ...
    fn invalidate_per_coin_type_cache(
        &self,
        keys: impl IntoIterator<Item = (MySoAddress, TypeTag)>,
    ) -> MySoResult {
    ...
    fn invalidate_all_balance_cache(
        &self,
        addresses: impl IntoIterator<Item = MySoAddress>,
    ) -> MySoResult {
    ...
    fn update_per_coin_type_cache(
        &self,
        keys: impl IntoIterator<Item = ((MySoAddress, TypeTag), MySoResult<TotalBalance>)>,
    ) -> MySoResult {
        self.caches
            .per_coin_type_balance
    ...
    fn merge_balance(
        old_balance: &MySoResult<TotalBalance>,
        balance_delta: &MySoResult<TotalBalance>,
    ) -> MySoResult<TotalBalance> {
    ...
    fn update_all_balance_cache(
        &self,
        keys: impl IntoIterator<Item = (MySoAddress, MySoResult<Arc<HashMap<TypeTag, TotalBalance>>>)>,
    ) -> MySoResult {
        self.caches
            .all_balances
    ...
    fn merge_all_balance(
        old_balance: &MySoResult<Arc<HashMap<TypeTag, TotalBalance>>>,
        balance_delta: &MySoResult<Arc<HashMap<TypeTag, TotalBalance>>>,
    ) -> MySoResult<Arc<HashMap<TypeTag, TotalBalance>>> {
        if let Ok(old_balance) = old_balance {
            if let Ok(balance_delta) = balance_delta {
...
mod tests {
    ...
    async fn test_index_cache() -> anyhow::Result<()> {
        ...
        let address: MySoAddress = AccountAddress::random().into();
