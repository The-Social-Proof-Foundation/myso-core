...
use myso_types::full_checkpoint_content::{
...
pub(crate) fn store_checkpoint_locally(
    ...
    std::fs::create_dir_all(path).map_err(|err| {
        MySoErrorKind::FileIOError(format!(
    ...
    std::fs::write(path.join(file_name), compressed).map_err(|_| {
        MySoErrorKind::FileIOError("failed to save full checkpoint content locally".to_string())
    })?;
...
pub(crate) fn load_checkpoint(
    ...
    let mut events = transaction_cache_reader
        ...
        .map(|(maybe_event, tx_digest)| {
            maybe_event
                .ok_or(MySoErrorKind::TransactionEventsNotFound { digest: tx_digest }.into())
                .map(|event| (tx_digest, event))
        })
        .collect::<MySoResult<HashMap<_, _>>>()?;
    ...
    let object_set = {
        let refs = transactions
            .iter()
            .flat_map(|tx| {
                myso_types::storage::get_transaction_object_set(
                    &tx.transaction,
                    &tx.effects,
        ...
        for (idx, object) in objects.into_iter().enumerate() {
            object_set.insert(object.ok_or_else(|| {
                myso_types::storage::error::Error::custom(format!(
