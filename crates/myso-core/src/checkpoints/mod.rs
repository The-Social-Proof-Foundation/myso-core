...
use myso_types::base_types::{ConciseableName, SequenceNumber};
...
use myso_types::crypto::AuthorityStrongQuorumSignInfo;
...
impl CheckpointStore {
    ...
    fn prune_local_summaries(&self) -> MySoResult {
    ...
    pub fn reset_db_for_execution_since_genesis(&self) -> MySoResult {
...
impl<MySoError: std::error::Error + Send + Sync + 'static> From<MySoError>
...
impl CheckpointBuilder {
    ...
    async fn construct_and_execute_settlement_transactions(
        ...
        for fx in settlement_effects.iter() {
            ...
            if let Some(version) = fx
                .mutated()
                .iter()
                .find_map(|(oref, _)| (oref.0 == MYSO_ACCUMULATOR_ROOT_OBJECT_ID).then_some(oref.1))
            {
                assert!(
    ...
    ) -> MySoResult<(Vec<TransactionEffects>, HashSet<TransactionDigest>)> {
    ...
    ) -> MySoResult<Option<(TransactionDigest, TransactionEffects)>> {
    ...
    ) -> CheckpointBuilderResult<MySoSystemState> {
    ...
    ) -> MySoResult<Vec<TransactionEffects>> {
...
impl CheckpointAggregator {
    ...
    async fn run_and_notify(&mut self) -> MySoResult {
    ...
    fn run_inner(&mut self) -> MySoResult<Vec<CertifiedCheckpointSummary>> {
    ...
    fn next_checkpoint_to_certify(&self) -> MySoResult<CheckpointSequenceNumber> {
        Ok(self
            .store
            .tables
            .certified_checkpoints
    ...
    pub fn try_aggregate(
        ...
        match self.signatures_by_digest.insert(their_digest, envelope) {
            ...
            InsertResult::Failed { error }
                if matches!(
                    error.as_inner(),
                    MySoErrorKind::StakeAggregatorRepeatedSigner {
...
async fn diagnose_split_brain(
    ...
    let committee = epoch_store
        .epoch_start_state()
        .get_myso_committee_with_network_metadata();
...
pub trait CheckpointServiceNotify {
    ...
    fn notify_checkpoint(&self) -> MySoResult;
}
...
mod tests {
    ...
    use myso_types::base_types::{SequenceNumber, TransactionEffectsDigest};
    ...
    async fn test_clear_locally_computed_checkpoints_from_deletes_inclusive_range() {
        ...
        let protocol = myso_protocol_config::ProtocolConfig::get_for_max_version_UNSAFE();
        for seq in 70u64..=80u64 {
            let contents =
                myso_types::messages_checkpoint::CheckpointContents::new_with_digests_only_for_tests(
                    [myso_types::base_types::ExecutionDigests::new(
                        myso_types::digests::TransactionDigest::random(),
                        myso_types::digests::TransactionEffectsDigest::ZERO,
                    )],
                );
            let summary = myso_types::messages_checkpoint::CheckpointSummary::new(
                &protocol,
                0,
                seq,
                0,
                &contents,
                None,
                myso_types::gas::GasCostSummary::default(),
                None,
                0,
                Vec::new(),
                Vec::new(),
    ...
    pub async fn checkpoint_builder_test() {
        ...
        for digest in all_digests {
            let signature = Signature::Ed25519MySoSignature(Default::default()).into();
            state
