...
use std::sync::Arc;
use myso_types::address_alias;
use myso_types::base_types::{SequenceNumber, MySoAddress};
use myso_types::digests::SenderSignedDataDigest;
use myso_types::digests::ZKLoginInputsDigest;
use myso_types::signature_verification::{
    VerifiedDigestCache, verify_sender_signed_data_message_signatures,
};
use myso_types::storage::ObjectStore;
use myso_types::transaction::{SenderSignedData, TransactionDataAPI};
use myso_types::{
    committee::Committee,
...
type Sender = oneshot::Sender<MySoResult<VerifiedCertificate>>;
...
impl SignatureVerifier {
    ...
    pub async fn verify_cert(&self, cert: CertifiedTransaction) -> MySoResult<VerifiedCertificate> {
    ...
    pub async fn multi_verify_certs(
        &self,
        certs: Vec<CertifiedTransaction>,
    ) -> Vec<MySoResult<VerifiedCertificate>> {
    ...
    ) -> MySoResult<NonEmpty<(u8, Option<SequenceNumber>)>> {
    ...
    pub fn verify_tx_require_no_aliases(&self, signed_tx: &SenderSignedData) -> MySoResult {
    ...
    fn verify_tx(
        ...
        aliased_addresses: Vec<(MySoAddress, NonEmpty<MySoAddress>)>,
    ) -> MySoResult<Vec<u8>> {
...
fn batch_verify(
    ...
    for cert in certs {
        let idx = obligation.add_message(cert.data(), cert.epoch(), Intent::myso_app(cert.scope()));
        cert.auth_sig()
            .add_to_verification_obligation(committee, &mut obligation, idx)?;
    }

    for ckpt in checkpoints {
        let idx = obligation.add_message(ckpt.data(), ckpt.epoch(), Intent::myso_app(ckpt.scope()));
        ckpt.auth_sig()
            .add_to_verification_obligation(committee, &mut obligation, idx)?;
    }
