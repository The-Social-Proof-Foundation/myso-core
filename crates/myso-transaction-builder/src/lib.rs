...
use myso_types::{MYSO_FRAMEWORK_PACKAGE_ID, MYSO_SYSTEM_PACKAGE_ID, coin, fp_ensure};
...
impl TransactionBuilder {
    ...
    pub async fn select_gas(
        ...
        if let Some(gas) = input_gas {
            ...
            Err(anyhow!(
                "Cannot find gas coin for signer address {signer} with amount sufficient for the required gas budget {gas_budget}. If you are using the pay or transfer commands, you can use pay-myso or transfer-myso commands instead, which will use the only object as gas payment."
            ))
        }
    }
    ...
    pub async fn transfer_myso(
        ...
        let object = self.get_object_ref(myso_object_id).await?;
        let gas_price = self.0.get_reference_gas_price().await?;
        Ok(TransactionData::new_transfer_myso(
    ...
    pub fn pay_myso_tx_kind(
        &self,
        recipients: Vec<MySoAddress>,
        amounts: Vec<u64>,
    ) -> Result<TransactionKind, anyhow::Error> {
        let mut builder = ProgrammableTransactionBuilder::new();
        builder.pay_myso(recipients.clone(), amounts.clone())?;
        let pt = builder.finish();
        let tx_kind = TransactionKind::programmable(pt);
        Ok(tx_kind)
    }

    pub async fn pay_myso(
        &self,
        signer: MySoAddress,
        input_coins: Vec<ObjectID>,
        recipients: Vec<MySoAddress>,
    ...
    pub fn pay_all_myso_tx_kind(&self, recipient: MySoAddress) -> TransactionKind {
        let mut builder = ProgrammableTransactionBuilder::new();
    ...
    pub async fn pay_all_myso(
        ...
        let gas_object_ref = coin_refs.remove(0);
        let gas_price = self.0.get_reference_gas_price().await?;
        Ok(TransactionData::new_pay_all_myso(
            signer,
            coin_refs,
            recipient,
            gas_object_ref,
    ...
    pub async fn move_call_tx_kind(
        ...
        module: &str,
        function: &str,
        type_args: Vec<MySoTypeTag>,
        call_args: Vec<MySoJsonValue>,
    ) -> Result<TransactionKind, anyhow::Error> {
        let mut builder = ProgrammableTransactionBuilder::new();
    ...
    pub async fn resolve_and_checks_json_args(
        ...
        function: &Identifier,
        type_args: &[TypeTag],
        json_args: Vec<MySoJsonValue>,
    ) -> Result<Vec<Argument>, anyhow::Error> {
        let object = self.0.get_object(package_id).await?;
        let myso_types::object::Data::Package(package) = &object.data else {
            bail!(
    ...
    pub async fn publish_tx_kind(
        ...
        upgrade_admin_cap: Option<ObjectID>,
        ...
        let pt = {
            ...
            if let Some(cap_id) = upgrade_admin_cap {
    ...
    pub async fn split_coin_tx_kind(
        ...
        let package = MYSO_FRAMEWORK_PACKAGE_ID;
    ...
    pub async fn request_add_stake(
        ...
        let pt = {
            let mut builder = ProgrammableTransactionBuilder::new();
            let arguments = vec![
                builder.input(CallArg::MYSO_SYSTEM_MUT).unwrap(),
    ...
    pub async fn request_withdraw_stake(
        ...
        let staked_myso = self.get_object_ref(staked_myso).await?;
        let gas_price = self.0.get_reference_gas_price().await?;
        let gas = self
        ...
        TransactionData::new_move_call(
            ...
            vec![
                ...
                CallArg::Object(ObjectArg::ImmOrOwnedObject(staked_myso)),
            ],
            gas_budget,
            gas_price,
        )
