...
use myso_test_transaction_builder::publish_basics_package_and_make_counter;
use myso_types::MYSO_FRAMEWORK_PACKAGE_ID;
use myso_types::base_types::{FullObjectRef, ObjectRef, MySoAddress};
use myso_types::coin::PAY_JOIN_FUNC_NAME;
use myso_types::coin::PAY_MODULE_NAME;
use myso_types::coin::PAY_SPLIT_VEC_FUNC_NAME;
...
pub enum CommonTransactionCosts {
    ...
    SplitCoin(usize),
    TransferWholeCoin,
    TransferWholeMySoCoin,
    TransferPortionMySoCoin,
    SharedCounterCreate,
...
// 1. Run `cargo insta test --review` under `./myso-cost`.
...
async fn create_txes(
    ...
    //
    // Transfer Whole MySo Coin and Transfer Portion of MySo Coin
    //
    let whole_myso_coin_tx =
        TestTransactionBuilder::new(sender, gas_objects.pop().unwrap(), gas_price)
            .transfer_myso(None, MySoAddress::default())
            .build();
    let partial_myso_coin_tx =
        TestTransactionBuilder::new(sender, gas_objects.pop().unwrap(), gas_price)
            .transfer_myso(Some(10), MySoAddress::default())
            .build();
    ret.insert(
        CommonTransactionCosts::TransferWholeMySoCoin,
        whole_myso_coin_tx,
    );
    ret.insert(
        CommonTransactionCosts::TransferPortionMySoCoin,
        partial_myso_coin_tx,
    );
