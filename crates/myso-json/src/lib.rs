...
pub enum MySoJsonValueErrorKind {
...
#[derive(Debug)]
pub struct MySoJsonValueError {
    kind: MySoJsonValueErrorKind,
    val: JsonValue,
}

impl MySoJsonValueError {
    pub fn new(val: &JsonValue, kind: MySoJsonValueErrorKind) -> Self {
        Self {
            kind,
            val: val.clone(),
        }
...
impl std::error::Error for MySoJsonValueError {}

impl fmt::Display for MySoJsonValueError {
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
        let err_str = match self.kind {
            MySoJsonValueErrorKind::ValueTypeNotAllowed => {
                format!("JSON value type {} not allowed.", self.val)
            }
            MySoJsonValueErrorKind::ArrayNotHomogeneous => {
...
pub struct MySoJsonValue(JsonValue);
impl MySoJsonValue {
    pub fn new(json_value: JsonValue) -> Result<MySoJsonValue, anyhow::Error> {
    ...
    pub fn from_object_id(id: ObjectID) -> MySoJsonValue {
    ...
    pub fn to_myso_address(&self) -> anyhow::Result<MySoAddress> {
        json_value_to_myso_address(&self.0)
    }
    ...
    pub fn to_move_value(
        ...
        Ok(match (val, ty) {
                ...
                if struct_layout.type_ == ID::type_() =>
            {
                ...
                let addr = MySoAddress::from_str(s)?;
            ...
            (v, MoveTypeLayout::Address) => {
                let addr = json_value_to_myso_address(v)?;
                R::MoveValue::Address(addr.into())
            }
...
impl Debug for MySoJsonValue {
    fn fmt(&self, f: &mut Formatter<'_>) -> std::fmt::Result {
        write!(f, "{}", self.0)
    }
}

fn json_value_to_myso_address(value: &JsonValue) -> anyhow::Result<MySoAddress> {
    match value {
        JsonValue::String(s) => {
        ...
        JsonValue::Array(bytes) => {
            ...
            let bytes = value_to_byte_array(bytes)
                .ok_or_else(|| anyhow!("Invalid input: Cannot parse input into MySoAddress."))?;
            Ok(MySoAddress::try_from(bytes)?)
        }
...
fn move_value_to_json(move_value: &MoveValue) -> Option<JsonValue> {
    Some(match move_value {
        ...
        MoveValue::Signer(v) | MoveValue::Address(v) => json!(MySoAddress::from(*v).to_string()),
...
impl FromStr for MySoJsonValue {
    ...
    fn from_str(s: &str) -> Result<Self, anyhow::Error> {
        ...
        MySoJsonValue::new(serde_json::from_str(s).unwrap_or_else(|_| try_escape_array(s)))
    }
}
...
pub fn check_valid_homogeneous(val: &JsonValue) -> Result<(), MySoJsonValueError> {
...
fn check_valid_homogeneous_rec(curr_q: &mut VecDeque<&JsonValue>) -> Result<(), MySoJsonValueError> {
    ...
    while let Some(v) = curr_q.pop_front() {
        let curr = match v {
            ...
            // Not valid
            _ => {
                return Err(MySoJsonValueError::new(
                    v,
                    MySoJsonValueErrorKind::ValueTypeNotAllowed,
                ));
            }
...
pub fn primitive_type(
    ...
    Some(match param {
        ...
        SignatureToken::Datatype(struct_handle_idx) => {
            ...
            } else if resolved_struct == RESOLVED_MYSO_ID {
...
fn resolve_object_vec_arg(idx: usize, arg: &MySoJsonValue) -> Result<Vec<ObjectID>, anyhow::Error> {
    // Every elem has to be a string convertible to a ObjectID
...
pub fn resolve_move_function_args(
    ...
    function: Identifier,
    type_args: &[TypeTag],
    combined_args_json: Vec<MySoJsonValue>,
...
macro_rules! call_args {
        ($($value:expr),*) => {
        Ok::<_, anyhow::Error>(vec![$(myso_json::call_arg!($value)?,)*])
    };
    }
...
macro_rules! call_arg {
    ($value:expr) => {{
        ...
        trait MySoJsonArg {
            fn to_myso_json(&self) -> anyhow::Result<MySoJsonValue>;
        }
        // TODO: anyway to condense this?
        impl MySoJsonArg for &str {
            fn to_myso_json(&self) -> anyhow::Result<MySoJsonValue> {
                MySoJsonValue::from_str(self)
            }
        }
        impl MySoJsonArg for String {
            fn to_myso_json(&self) -> anyhow::Result<MySoJsonValue> {
                MySoJsonValue::from_str(&self)
            }
        }
        impl MySoJsonArg for myso_types::base_types::ObjectID {
            fn to_myso_json(&self) -> anyhow::Result<MySoJsonValue> {
                MySoJsonValue::from_str(&self.to_string())
            }
        }
        impl MySoJsonArg for myso_types::base_types::MySoAddress {
            fn to_myso_json(&self) -> anyhow::Result<MySoJsonValue> {
                MySoJsonValue::from_str(&self.to_string())
            }
        }
        impl MySoJsonArg for u64 {
            fn to_myso_json(&self) -> anyhow::Result<MySoJsonValue> {
                MySoJsonValue::from_bcs_bytes(
                    Some(&myso_json::MoveTypeLayout::U64),
                    &bcs::to_bytes(self)?,
                )
            }
        }
        impl MySoJsonArg for Vec<u8> {
            fn to_myso_json(&self) -> anyhow::Result<MySoJsonValue> {
                MySoJsonValue::from_bcs_bytes(None, &self)
            }
        }
        impl MySoJsonArg for &[u8] {
            fn to_myso_json(&self) -> anyhow::Result<MySoJsonValue> {
                MySoJsonValue::from_bcs_bytes(None, self)
            }
        }
...
macro_rules! type_args {
    ($($value:expr), *) => {{
        use myso_json_rpc_types::MySoTypeTag;
        use myso_types::TypeTag;
        trait MySoJsonTypeArg {
            fn to_myso_json(&self) -> anyhow::Result<MySoTypeTag>;
        }
        impl <T: core::fmt::Display> MySoJsonTypeArg for T {
            fn to_myso_json(&self) -> anyhow::Result<MySoTypeTag> {
                Ok(myso_types::parse_myso_type_tag(&self.to_string())?.into())
            }
        }
        Ok::<_, anyhow::Error>(vec![$($value.to_myso_json()?,)*])
    }};
    }
