# MySo Single Node Benchmark

This crate contains a binary for performance benchmarking a single MySo node.
...
```
cargo run --release --bin myso-single-node-benchmark -- ptb
```
...
```
cargo run --release --bin myso-single-node-benchmark -- --component baseline ptb
```
