...
fn main() {
    ...
    // Find all .proto files using walkdir (only from myso_proto_dir, not myso_rpc_proto_dir)
    let proto_ext = OsStr::new("proto");
    let mut proto_files = vec![];
    for entry in WalkDir::new(&myso_proto_dir) {
    ...
    let file_descriptors = protox::compile(proto_files, [myso_proto_dir, myso_rpc_proto_dir])
