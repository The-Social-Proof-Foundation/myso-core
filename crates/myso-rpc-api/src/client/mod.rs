...
use myso_types::messages_checkpoint::{CertifiedCheckpointSummary, CheckpointSequenceNumber};
...
pub use myso_rpc::client::HeadersInterceptor;
pub use myso_rpc::client::ResponseExt;
...
#[derive(Clone)]
pub struct Client(myso_rpc::Client);
...
impl Client {
    ...
    {
        myso_rpc::Client::new(uri).map(Self)
    }
    ...
    pub fn inner_mut(&mut self) -> &mut myso_rpc::Client {
        &mut self.0
    }

    pub fn into_inner(self) -> myso_rpc::Client {
        self.0
    }
    ...
    pub async fn get_full_checkpoint(
        ...
        let checkpoint = response
            .checkpoint
            .ok_or_else(|| tonic::Status::not_found("no checkpoint returned"))?;
        myso_types::full_checkpoint_content::Checkpoint::try_from(&checkpoint)
            .map_err(|e| status_from_error_with_metadata(e, metadata))
    }
    ...
    pub async fn get_chain_identifier(&self) -> Result<ChainIdentifier> {
        ...
        let chain_id = response
            .chain_id()
            .parse::<myso_sdk_types::Digest>()
            .map_err(|e| TryFromProtoError::invalid("chain_id", e))
            .map_err(|e| Status::from_error(e.into()))?;

        Ok(ChainIdentifier::from(
            myso_types::digests::CheckpointDigest::from(chain_id),
        ))
    }
    ...
    pub async fn get_committee(
        &self,
        epoch: Option<u64>,
    ) -> Result<myso_types::committee::Committee> {
        let mut request = proto::GetEpochRequest::default();
        if let Some(epoch) = epoch {
            request.set_epoch(epoch);
        }
    ...
    pub async fn list_delegated_stake(
        &self,
        owner: MySoAddress,
    ) -> Result<Vec<myso_rpc::client::DelegatedStake>> {
        self.0.clone().list_delegated_stake(&owner.into()).await
    }

    pub fn transaction_builder(&self) -> myso_transaction_builder::TransactionBuilder {
        myso_transaction_builder::TransactionBuilder::new(std::sync::Arc::new(self.clone()) as _)
    }
}
...
pub struct ExecutedTransaction {
    ...
    pub balance_changes: Vec<myso_sdk_types::BalanceChange>,
...
fn certified_checkpoint_summary_try_from_proto(
    ...
    let signature = myso_types::crypto::AuthorityStrongQuorumSignInfo::from(
        myso_sdk_types::ValidatorAggregatedSignature::try_from(
            checkpoint
                .signature
                .as_ref()
...
impl myso_transaction_builder::DataReader for Client {
    async fn get_owned_objects(
        &self,
        address: MySoAddress,
        object_type: move_core_types::language_storage::StructTag,
    ) -> Result<Vec<myso_types::base_types::ObjectInfo>, anyhow::Error> {
        self.list_owned_objects(address, Some(object_type))
            .map_ok(|o| myso_types::base_types::ObjectInfo::from_object(&o))
            .try_collect()
            .await
            .map_err(Into::into)
    }
