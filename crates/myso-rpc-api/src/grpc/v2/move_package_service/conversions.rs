...
// This module contains conversion functions from myso-package-resolver types to their protobuf representations.
...
pub(crate) fn convert_error(e: myso_package_resolver::error::Error) -> RpcError {
...
pub(crate) fn convert_module(
    module_name: &str,
    resolved_module: &myso_package_resolver::Module,
    package_id: &ObjectID,
) -> Result<Module> {
...
fn convert_open_signature(sig: &myso_package_resolver::OpenSignature) -> OpenSignature {
    let reference = sig.ref_.map(|r| match r {
        myso_package_resolver::Reference::Immutable => Reference::Immutable as i32,
        myso_package_resolver::Reference::Mutable => Reference::Mutable as i32,
    });
...
fn convert_open_signature_body(
    body: &myso_package_resolver::OpenSignatureBody,
) -> OpenSignatureBody {
    let (type_enum, type_name, type_params, type_param_idx) = match body {
        myso_package_resolver::OpenSignatureBody::Bool => (SignatureType::Bool, None, vec![], None),
        myso_package_resolver::OpenSignatureBody::U8 => (SignatureType::U8, None, vec![], None),
        myso_package_resolver::OpenSignatureBody::U16 => (SignatureType::U16, None, vec![], None),
        myso_package_resolver::OpenSignatureBody::U32 => (SignatureType::U32, None, vec![], None),
        myso_package_resolver::OpenSignatureBody::U64 => (SignatureType::U64, None, vec![], None),
        myso_package_resolver::OpenSignatureBody::U128 => (SignatureType::U128, None, vec![], None),
        myso_package_resolver::OpenSignatureBody::U256 => (SignatureType::U256, None, vec![], None),
        myso_package_resolver::OpenSignatureBody::Address => {
            (SignatureType::Address, None, vec![], None)
        }
        myso_package_resolver::OpenSignatureBody::Vector(inner) => {
            let inner_body = convert_open_signature_body(inner.as_ref());
            (SignatureType::Vector, None, vec![inner_body], None)
        }
        myso_package_resolver::OpenSignatureBody::Datatype(key, args) => {
        ...
        myso_package_resolver::OpenSignatureBody::TypeParameter(idx) => {
