...
impl RpcService {
    ...
    pub fn render_json(
        ...
        myso_types::proto_value::ProtoVisitor::new(self.config.max_json_move_value_size())
    ...
    pub fn render_events_to_proto(
        &self,
        events: &myso_types::effects::TransactionEvents,
        mask: &FieldMaskTree,
    ) -> TransactionEvents {
    ...
    pub fn render_clever_error(&self, effects: &mut TransactionEffects) {
        ...
        fn render(service: &RpcService, move_abort: &MoveAbort) -> Option<CleverError> {
            ...
            let package_id = location.package().parse::<myso_sdk_types::Address>().ok()?;
            let module = location.module();

            let package = {
                let object = service.reader.inner().get_object(&package_id.into())?;
                myso_package_resolver::Package::read_from_object(&object).ok()?
            };
            ...
            match clever_error.error_info {
                myso_package_resolver::ErrorConstants::None => {}
                myso_package_resolver::ErrorConstants::Rendered {
                    identifier,
                    constant,
                } => {
                ...
                myso_package_resolver::ErrorConstants::Raw { identifier, bytes } => {
                    clever_error_message.constant_name = Some(identifier);
    ...
    pub fn render_effects_to_proto<F>(
        &self,
        effects: &myso_types::effects::TransactionEffects,
        unchanged_loaded_runtime_objects: &[myso_types::storage::ObjectKey],
        object_type_lookup: F,
        mask: &FieldMaskTree,
    ) -> TransactionEffects
    where
        F: Fn(&myso_types::base_types::ObjectID) -> Option<myso_types::base_types::ObjectType>,
    {
        ...
        if mask.contains(TransactionEffects::CHANGED_OBJECTS_FIELD) {
            for changed_object in effects.changed_objects.iter_mut() {
                let Ok(object_id) = changed_object
                    .object_id()
                    .parse::<myso_types::base_types::ObjectID>()
                else {
                    continue;
                };
...
fn object_type_to_string(object_type: myso_types::base_types::ObjectType) -> String {
    match object_type {
        myso_types::base_types::ObjectType::Package => "package".to_owned(),
        myso_types::base_types::ObjectType::Struct(move_object_type) => {
