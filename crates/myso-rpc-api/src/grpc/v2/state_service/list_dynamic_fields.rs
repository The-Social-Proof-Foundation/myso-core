...
fn load_dynamic_field(
    ...
    if read_mask.contains(DynamicField::NAME_FIELD) {
        message.name = Some(
            Bcs::default()
                .with_name(myso_types::TypeTag::from(field.name_layout).to_canonical_string(true))
    ...
    if read_mask.contains(DynamicField::VALUE_FIELD) {
        message.value = Some(
            Bcs::default()
                .with_name(myso_types::TypeTag::from(field.value_layout).to_canonical_string(true))
    ...
    match field.value_metadata()? {
        ...
        DFV::ValueMetadata::DynamicObjectField(object_id) => {
                ...
                || read_mask.contains(DynamicField::CHILD_OBJECT_FIELD)
            {
                ...
                let type_tag =
                    myso_types::TypeTag::from(object.struct_tag().ok_or_else(|| {
