...
const MYSO_COIN_TREASURY: CoinTreasury = {
    ...
    treasury.total_supply = Some(myso_types::gas_coin::TOTAL_SUPPLY_MIST);
...
fn get_coin_info_from_registry(
    ...
    let treasury = if myso_types::gas_coin::GAS::is_gas(core_coin_type) {
        Some(MYSO_COIN_TREASURY)
    } else {
...
fn get_coin_info_from_index(
    service: &RpcService,
    indexes: &dyn myso_types::storage::RpcIndexes,
    coin_type: &StructTag,
    core_coin_type: &move_core_types::language_storage::StructTag,
) -> Result<Option<GetCoinInfoResponse>> {
    ...
    let metadata = if let Some(coin_metadata_object_id) = coin_info.coin_metadata_object_id {
        service
            ...
            .map(myso_types::coin::CoinMetadata::try_from)
            .transpose()
            .map_err(|_| {
                RpcError::new(
                    tonic::Code::Internal,
    ...
    } else if myso_types::gas_coin::GAS::is_gas(core_coin_type) {
        Some(MYSO_COIN_TREASURY)
    } else {
        None
    };
...
fn get_treasury_cap_info(
    service: &RpcService,
    treasury_object_id: MySoObjectID,
) -> Option<CoinTreasury> {
    ...
    let supply_state = match &obj.owner {
        ...
        AddressOwner(addr) if *addr == MySoAddress::ZERO => RpcSupplyState::Fixed,
        ConsensusAddressOwner { owner, .. } if *owner == MySoAddress::ZERO => RpcSupplyState::Fixed,
        _ => RpcSupplyState::Unknown,
    };

    myso_types::coin::TreasuryCap::try_from(obj)
        .ok()
        .map(|treasury| {
