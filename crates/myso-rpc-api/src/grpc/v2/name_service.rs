...
fn name_service_config(service: &RpcService) -> Result<NameServiceConfig> {
    match service.chain_id.chain() {
        myso_protocol_config::Chain::Mainnet => Ok(NameServiceConfig::mainnet()),
        myso_protocol_config::Chain::Testnet => Ok(NameServiceConfig::testnet()),
        myso_protocol_config::Chain::Unknown => Err(RpcError::new(
            tonic::Code::Unimplemented,
            "MySoNS not configured for this network",
        )),
    }
}
...
fn reverse_lookup_name(
    ...
    let domain = field_reverse_record_object
        .to_rust::<myso_types::dynamic_field::Field<Address, Domain>>()
        .ok_or_else(|| {
            RpcError::new(
                tonic::Code::Internal,
