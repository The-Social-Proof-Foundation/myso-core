...
use myso_types::transaction::FundsWithdrawalArg;
use myso_types::transaction::GasData;
use myso_types::transaction::ObjectArg;
use myso_types::transaction::ProgrammableTransaction;
use myso_types::transaction::Reservation;
use myso_types::transaction::TransactionData;
use myso_types::transaction::WithdrawFrom;
use myso_types::transaction::WithdrawalTypeArg;
use tap::Pipe;
...
pub fn resolve_transaction(
    ...
    let commands = ptb
        .commands
        .iter()
        .map(myso_sdk_types::Command::try_from)
...
fn resolve_unresolved_transaction(
    ...
    let expiration = expiration
        .map(myso_types::transaction::TransactionExpiration::try_from)
        .transpose()
        .map_err(|e| {
        ...
        .unwrap_or(myso_types::transaction::TransactionExpiration::None);
    ...
    Ok(TransactionData::V1(
        myso_types::transaction::TransactionDataV1 {
            kind: myso_types::transaction::TransactionKind::ProgrammableTransaction(ptb),
            sender: sender.into(),
            gas_data,
            expiration,
        },
...
fn resolve_gas_object_reference(
    ...
    let Ok(gas_coin) = myso_types::gas_coin::GasCoin::try_from(&object) else {
...
fn resolve_object_reference_with_object(
    ...
    match object.owner() {
        myso_types::object::Owner::AddressOwner(_) | myso_types::object::Owner::Immutable => {}
        _ => {
...
fn resolve_object(
    ...
    arg_idx: usize,
    object_id: Address,
    version: Option<myso_sdk_types::Version>,
    ...
    match object.owner() {
        myso_types::object::Owner::Immutable => resolve_object_reference_with_object(
            &object,
            UnresolvedObjectReference {
                object_id,
                version,
        ...
        myso_types::object::Owner::AddressOwner(_) => {
        ...
        myso_types::object::Owner::Shared { .. }
        | myso_types::object::Owner::ConsensusAddressOwner { .. } => {
            resolve_shared_input_with_object(called_packages, commands, arg_idx, object)
        }
        myso_types::object::Owner::ObjectOwner(_) => Err(RpcError::new(
            tonic::Code::InvalidArgument,
...
fn is_input_argument_receiving(
    ...
    let (receiving_package, receiving_module, receiving_struct) =
        myso_types::transfer::RESOLVED_RECEIVING_STRUCT;
...
fn arg_type_of_move_call_input(
    called_packages: &NormalizedPackages,
    move_call: &myso_sdk_types::MoveCall,
    idx: usize,
...
fn resolve_shared_input_with_object(
    ...
    let initial_shared_version = if let myso_types::object::Owner::Shared {
    ...
    Ok(ObjectArg::SharedObject {
        ...
        mutability: if mutable {
            ...
            myso_types::transaction::SharedObjectMutability::Immutable
        },
    })
}
...
struct UnresolvedInput<'a> {
    ...
    pub digest: Option<myso_sdk_types::Digest>,
                ...
                .map(|w| {
                    Ok(FundsWithdrawalArg {
                        ...
                        type_arg: WithdrawalTypeArg::Balance(
                            w.coin_type().parse::<myso_types::TypeTag>().map_err(|e| {
