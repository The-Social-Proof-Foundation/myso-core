...
use myso_rpc::proto::myso::rpc::v2::CommandOutput;
use myso_rpc::proto::myso::rpc::v2::CommandResult;
...
use myso_rpc::proto::myso::rpc::v2::SimulateTransactionResponse;
...
pub fn simulate_transaction(
    ...
    let mut transaction = match myso_sdk_types::Transaction::try_from(transaction_proto) {
        Ok(transaction) => myso_types::transaction::TransactionData::try_from(transaction)?,
    ...
    let transaction = if let Some(submask) = read_mask.subtree("transaction") {
        ...
        let transaction = myso_sdk_types::Transaction::try_from(transaction)?;
...
fn to_command_output(
    service: &RpcService,
    arg: Option<myso_types::transaction::Argument>,
    bcs: Vec<u8>,
    ty: myso_types::TypeTag,
) -> CommandOutput {
    let json = service
        .reader
        .inner()
        ...
        .flatten()
        .and_then(|layout| {
            myso_types::proto_value::ProtoVisitor::new(service.config.max_json_move_value_size())
...
fn estimate_gas_budget_from_gas_cost(
    gas_cost_summary: &myso_types::gas::GasCostSummary,
...
fn select_gas(
    service: &RpcService,
    transaction: &mut myso_types::transaction::TransactionData,
    ...
    use myso_types::transaction::TransactionDataAPI;
    use myso_types::transaction::TransactionExpiration;
    ...
    let address_balance = reader
        ...
        .map(|balance| {
            // Sum up the total MYSO reservations for the `owner` so that we can deduct that from the
            // available address balance for determining if an account as sufficient funds.
            let reserved_myso = transaction
                ...
                .filter_map(|w| {
                    ...
                    // Skip if this withdrawal isn't for MYSO
            ...
            balance.saturating_sub(reserved_myso)
        });
        ...
        *transaction.expiration_mut() = TransactionExpiration::ValidDuring {
        ...
        let input_objects = transaction
            ...
            .flat_map(|obj| match obj {
                myso_types::transaction::InputObjectKind::ImmOrOwnedMoveObject((id, _, _)) => {
                    Some(*id)
                }
                _ => None,
            })
    ...
    if selected_gas_value >= budget {
        ...
        Err(RpcError::new(
            tonic::Code::InvalidArgument,
            format!(
                "Unable to perform gas selection due to insufficient MYSO \
