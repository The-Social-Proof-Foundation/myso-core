...
pub(crate) struct Container {
    ...
    node_watch: watch::Receiver<Weak<MySoNode>>,
}
...
impl Drop for Container {
    fn drop(&mut self) {
        if let Some(handle) = self.handle.take() {
            ...
            myso_simulator::runtime::Handle::try_current().map(|h| h.delete_node(handle.node_id));
        }
    }
}
...
impl Container {
    ...
    pub async fn spawn(config: NodeConfig, _runtime: RuntimeType) -> Self {
        ...
        let handle = myso_simulator::runtime::Handle::current();
    ...
    /// Get a MySoNodeHandle to the node owned by the container.
    pub fn get_node_handle(&self) -> Option<MySoNodeHandle> {
        Some(MySoNodeHandle::new(self.node_watch.borrow().upgrade()?))
    }
