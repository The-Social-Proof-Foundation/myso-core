...
use prometheus::{
    register_counter_vec_with_registry, register_counter_with_registry, Counter, CounterVec,
    Registry,
};
...
impl SocialMetrics {
    pub fn new(registry: &Registry) -> anyhow::Result<Self> {
        ...
        let spt_u64_amount_exceeds_i64 = register_counter_vec_with_registry!(
            "myso_social_spt_u64_amount_exceeds_i64_total",
            "SPT chain JSON integer exceeded i64::MAX for a BIGINT column",
            &["field"],
            registry
        )?;
    ...
    pub fn record_event_processed(module: &str, event_type: &str) {
        if let Some(metrics) = Self::get() {
            metrics
                .events_processed
                .with_label_values(&[module, event_type])
                .inc();
        }
    }
    ...
    pub fn record_spt_u64_amount_exceeds_i64(field: &str) {
        if let Some(metrics) = Self::get() {
            metrics
                .spt_u64_amount_exceeds_i64
