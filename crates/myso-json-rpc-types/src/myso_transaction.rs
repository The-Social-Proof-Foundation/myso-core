...
use myso_types::error::{ExecutionError, MySoError, MySoResult};
...
pub struct MySoTransactionBlockResponseQuery {
    /// If None, no filter will be applied
    pub filter: Option<TransactionFilter>,
    /// config which fields to include in the response, by default only digest is included
    pub options: Option<MySoTransactionBlockResponseOptions>,
}

impl MySoTransactionBlockResponseQuery {
    pub fn new(
...
pub type TransactionBlocksPage = Page<MySoTransactionBlockResponse, TransactionDigest>;
...
pub struct MySoTransactionBlockResponseOptions {
...
impl MySoTransactionBlockResponseOptions {
    pub fn new() -> Self {
        Self::default()
    }
...
pub struct MySoTransactionBlockResponse {
    pub digest: TransactionDigest,
    /// Transaction input data
    #[serde(skip_serializing_if = "Option::is_none")]
    pub transaction: Option<MySoTransactionBlock>,
    ...
    #[serde(skip_serializing_if = "Option::is_none")]
    pub events: Option<MySoTransactionBlockEvents>,
    #[serde(skip_serializing_if = "Option::is_none")]
...
impl MySoTransactionBlockResponse {
...
impl PartialEq for MySoTransactionBlockResponse {
...
impl Display for MySoTransactionBlockResponse {
...
pub enum MySoTransactionBlockKind {
    /// A system transaction that will update epoch information on-chain.
    ChangeEpoch(MySoChangeEpoch),
    /// A system transaction used for initializing the initial state of the chain.
    Genesis(MySoGenesisTransaction),
    /// A system transaction marking the start of a series of transactions scheduled as part of a
    /// checkpoint
    ConsensusCommitPrologue(MySoConsensusCommitPrologue),
    /// A series of transactions where the results of one transaction can be used in future
    /// transactions
    ProgrammableTransaction(MySoProgrammableTransactionBlock),
    /// A transaction which updates global authenticator state
    AuthenticatorStateUpdate(MySoAuthenticatorStateUpdate),
    /// A transaction which updates global randomness state
    RandomnessStateUpdate(MySoRandomnessStateUpdate),
    /// The transaction which occurs only at the end of the epoch
    EndOfEpochTransaction(MySoEndOfEpochTransaction),
    ConsensusCommitPrologueV2(MySoConsensusCommitPrologueV2),
    ConsensusCommitPrologueV3(MySoConsensusCommitPrologueV3),
    ConsensusCommitPrologueV4(MySoConsensusCommitPrologueV4),

    ProgrammableSystemTransaction(MySoProgrammableTransactionBlock),
    // .. more transaction types go here
}

impl Display for MySoTransactionBlockKind {
    ...
    fn try_from_inner(tx: TransactionKind) -> Result<Self, anyhow::Error> {
        Ok(match tx {
            ...
            TransactionKind::ConsensusCommitPrologueV2(p) => {
                Self::ConsensusCommitPrologueV2(MySoConsensusCommitPrologueV2 {
                    epoch: p.epoch,
                    round: p.round,
                    commit_timestamp_ms: p.commit_timestamp_ms,
            ...
            TransactionKind::ConsensusCommitPrologueV3(p) => {
                Self::ConsensusCommitPrologueV3(MySoConsensusCommitPrologueV3 {
                    epoch: p.epoch,
                    round: p.round,
            ...
            TransactionKind::ConsensusCommitPrologueV4(p) => {
                Self::ConsensusCommitPrologueV4(MySoConsensusCommitPrologueV4 {
                    epoch: p.epoch,
                    round: p.round,
                    sub_dag_index: p.sub_dag_index,
                    commit_timestamp_ms: p.commit_timestamp_ms,
            ...
            TransactionKind::AuthenticatorStateUpdate(update) => {
                Self::AuthenticatorStateUpdate(MySoAuthenticatorStateUpdate {
                    epoch: update.epoch,
                    round: update.round,
                    new_active_jwks: update
                        .new_active_jwks
                        .into_iter()
                        .map(MySoActiveJwk::from)
                        .collect(),
                })
            }
            TransactionKind::RandomnessStateUpdate(update) => {
                Self::RandomnessStateUpdate(MySoRandomnessStateUpdate {
                    epoch: update.epoch,
            ...
            TransactionKind::EndOfEpochTransaction(end_of_epoch_tx) => {
                Self::EndOfEpochTransaction(MySoEndOfEpochTransaction {
                    transactions: end_of_epoch_tx
                        .into_iter()
                        .map(|tx| match tx {
                            EndOfEpochTransactionKind::ChangeEpoch(e) => {
                                MySoEndOfEpochTransactionKind::ChangeEpoch(e.into())
                            }
                            EndOfEpochTransactionKind::AuthenticatorStateCreate => {
                                MySoEndOfEpochTransactionKind::AuthenticatorStateCreate
                            }
                            EndOfEpochTransactionKind::AuthenticatorStateExpire(expire) => {
                                MySoEndOfEpochTransactionKind::AuthenticatorStateExpire(
                                    MySoAuthenticatorStateExpire {
                                        min_epoch: expire.min_epoch,
                                    },
                                )
                            }
                            EndOfEpochTransactionKind::RandomnessStateCreate => {
                                MySoEndOfEpochTransactionKind::RandomnessStateCreate
                            }
                            EndOfEpochTransactionKind::DenyListStateCreate => {
                                MySoEndOfEpochTransactionKind::CoinDenyListStateCreate
                            }
                            EndOfEpochTransactionKind::BridgeStateCreate(chain_id) => {
                                MySoEndOfEpochTransactionKind::BridgeStateCreate(
                                    (*chain_id.as_bytes()).into(),
                                )
                            }
                            EndOfEpochTransactionKind::BridgeCommitteeInit(
                                bridge_shared_version,
                            ) => MySoEndOfEpochTransactionKind::BridgeCommitteeUpdate(
                                bridge_shared_version,
                            ),
                            EndOfEpochTransactionKind::StoreExecutionTimeObservations(_) => {
                                MySoEndOfEpochTransactionKind::StoreExecutionTimeObservations
                            }
                            EndOfEpochTransactionKind::AccumulatorRootCreate => {
                                MySoEndOfEpochTransactionKind::AccumulatorRootCreate
                            }
                            EndOfEpochTransactionKind::CoinRegistryCreate => {
                                MySoEndOfEpochTransactionKind::CoinRegistryCreate
                            }
                            EndOfEpochTransactionKind::DisplayRegistryCreate => {
                                MySoEndOfEpochTransactionKind::DisplayRegistryCreate
                            }
                            EndOfEpochTransactionKind::AddressAliasStateCreate => {
                                MySoEndOfEpochTransactionKind::AddressAliasStateCreate
                            }
                            EndOfEpochTransactionKind::WriteAccumulatorStorageCost(_) => {
                                MySoEndOfEpochTransactionKind::WriteAccumulatorStorageCost
                            }
                        })
                        .collect(),
                })
        ...
        match tx {
            ...
            | TransactionKind::ProgrammableSystemTransaction(p) => {
                Ok(Self::ProgrammableTransaction(
                    MySoProgrammableTransactionBlock::try_from_with_module_cache(p, module_cache)?,
                ))
            }
        ...
        match tx {
            TransactionKind::ProgrammableSystemTransaction(p) => {
                Ok(Self::ProgrammableSystemTransaction(
                    MySoProgrammableTransactionBlock::try_from_with_package_resolver(
                        p,
                        package_resolver,
                    )
                    .await?,
...
impl From<ChangeEpoch> for MySoChangeEpoch {
...
#[enum_dispatch(MySoTransactionBlockEffectsAPI)]
#[serde(
    rename = "TransactionBlockEffects",
    rename_all = "camelCase",
    tag = "messageVersion"
)]
pub enum MySoTransactionBlockEffects {
    V1(MySoTransactionBlockEffectsV1),
}

#[enum_dispatch]
pub trait MySoTransactionBlockEffectsAPI {
    fn status(&self) -> &MySoExecutionStatus;
    fn into_status(self) -> MySoExecutionStatus;
    fn shared_objects(&self) -> &[MySoObjectRef];
    fn created(&self) -> &[OwnedObjectRef];
    fn mutated(&self) -> &[OwnedObjectRef];
    fn unwrapped(&self) -> &[OwnedObjectRef];
    fn deleted(&self) -> &[MySoObjectRef];
    fn unwrapped_then_deleted(&self) -> &[MySoObjectRef];
    fn wrapped(&self) -> &[MySoObjectRef];
    ...
    fn all_deleted_objects(&self) -> Vec<(&MySoObjectRef, DeleteKind)>;

    fn accumulator_events(&self) -> Vec<MySoAccumulatorEvent>;
}
...
pub struct MySoTransactionBlockEffectsModifiedAtVersions {
    object_id: ObjectID,
...
pub struct MySoAccumulatorEvent {
    pub accumulator_obj: ObjectID,
    pub address: MySoAddress,
    pub ty: MySoTypeTag,
    pub operation: MySoAccumulatorOperation,
    pub value: MySoAccumulatorValue,
}

impl From<AccumulatorEvent> for MySoAccumulatorEvent {
...
pub enum MySoAccumulatorOperation {
    Merge,
    Split,
}

impl From<AccumulatorOperation> for MySoAccumulatorOperation {
...
impl From<AccumulatorValue> for MySoAccumulatorValue {
...
pub struct MySoTransactionBlockEffectsV1 {
    /// The status of the execution
    pub status: MySoExecutionStatus,
    ...
    pub modified_at_versions: Vec<MySoTransactionBlockEffectsModifiedAtVersions>,
    /// The object references of the shared objects used in this transaction. Empty if no shared objects were used.
    #[serde(default, skip_serializing_if = "Vec::is_empty")]
    pub shared_objects: Vec<MySoObjectRef>,
    ...
    #[serde(default, skip_serializing_if = "Vec::is_empty")]
    pub deleted: Vec<MySoObjectRef>,
    /// Object refs of objects previously wrapped in other objects but now deleted.
    #[serde(default, skip_serializing_if = "Vec::is_empty")]
    pub unwrapped_then_deleted: Vec<MySoObjectRef>,
    /// Object refs of objects now wrapped in other objects.
    #[serde(default, skip_serializing_if = "Vec::is_empty")]
    pub wrapped: Vec<MySoObjectRef>,
    #[serde(default, skip_serializing_if = "Vec::is_empty")]
    pub accumulator_events: Vec<MySoAccumulatorEvent>,
    ...
    #[serde(default, skip_serializing_if = "Option::is_none")]
    pub abort_error: Option<MySoMoveAbort>,
}

// TODO move additional error info here

impl MySoTransactionBlockEffectsAPI for MySoTransactionBlockEffectsV1 {
    fn status(&self) -> &MySoExecutionStatus {
        &self.status
    }
    fn into_status(self) -> MySoExecutionStatus {
...
impl MySoTransactionBlockEffects {
    pub fn new_for_testing(
        transaction_digest: TransactionDigest,
        status: MySoExecutionStatus,
    ) -> Self {
        Self::V1(MySoTransactionBlockEffectsV1 {
            transaction_digest,
            status,
            gas_object: OwnedObjectRef {
                owner: Owner::AddressOwner(MySoAddress::random_for_testing_only()),
                reference: myso_types::base_types::random_object_ref().into(),
            },
            executed_epoch: 0,
...
impl TryFrom<TransactionEffects> for MySoTransactionBlockEffects {
    type Error = MySoError;

    fn try_from(effect: TransactionEffects) -> Result<Self, Self::Error> {
        Ok(MySoTransactionBlockEffects::V1(
            MySoTransactionBlockEffectsV1 {
                ...
                modified_at_versions: effect
                    ...
                    .map(|(object_id, sequence_number)| {
                        MySoTransactionBlockEffectsModifiedAtVersions {
                            object_id,
                            sequence_number,
                        }
                    })
                ...
                shared_objects: to_myso_object_ref(
                    effect
                ...
                abort_error: effect
                    .move_abort()
                    .map(|(abort, code)| MySoMoveAbort::new(abort, code)),
                accumulator_events: effect
                    .accumulator_events()
                    .into_iter()
                    .map(MySoAccumulatorEvent::from)
                    .collect(),
            },
        ))
    }
...
fn objref_string(obj: &MySoObjectRef) -> String {
    format!(
        " ┌──\n │ ID: {} \n │ Version: {} \n │ Digest: {}\n └──",
        obj.object_id,
...
impl Display for MySoTransactionBlockEffects {
    fn fmt(&self, f: &mut Formatter<'_>) -> fmt::Result {
        let mut builder = TableBuilder::default();
...
pub struct DryRunTransactionBlockResponse {
    ...
    pub events: MySoTransactionBlockEvents,
    pub object_changes: Vec<ObjectChange>,
    pub balance_changes: Vec<BalanceChange>,
    pub input: MySoTransactionBlockData,
...
pub struct MySoTransactionBlockEvents {
    pub data: Vec<MySoEvent>,
}

impl MySoTransactionBlockEvents {
    pub fn try_from(
        ...
        Ok(Self {
            data: events
                ...
                .map(|(seq, event)| {
                    ...
                    MySoEvent::try_from(event, tx_digest, seq as u64, timestamp_ms, layout)
                })
                .collect::<Result<_, _>>()?,
        })
    }
...
impl Display for MySoTransactionBlockEvents {
    fn fmt(&self, f: &mut Formatter<'_>) -> fmt::Result {
...
pub struct DevInspectArgs {
    ...
    pub gas_sponsor: Option<MySoAddress>,
...
pub struct DevInspectResults {
    ...
    #[serde(skip_serializing_if = "Option::is_none")]
    pub results: Option<Vec<MySoExecutionResult>>,
...
#[derive(Debug, Clone, Serialize, Deserialize, JsonSchema)]
#[serde(rename = "MySoExecutionResult", rename_all = "camelCase")]
pub struct MySoExecutionResult {
    /// The value of any arguments that were mutably borrowed.
    /// Non-mut borrowed values are not included
    #[serde(default, skip_serializing_if = "Vec::is_empty")]
    pub mutable_reference_outputs: Vec<(/* argument */ MySoArgument, Vec<u8>, MySoTypeTag)>,
    /// The return values from the transaction
    #[serde(default, skip_serializing_if = "Vec::is_empty")]
    pub return_values: Vec<(Vec<u8>, MySoTypeTag)>,
}
...
impl DevInspectResults {
    pub fn new(
                    ...
                    srvs.into_iter()
                        .map(|srv| {
                            ...
                            let mutable_reference_outputs = mutable_reference_outputs
                                .into_iter()
                                .map(|(a, bytes, tag)| (a.into(), bytes, MySoTypeTag::from(tag)))
                                .collect();
                            let return_values = return_values
                                .into_iter()
                                .map(|(bytes, tag)| (bytes, MySoTypeTag::from(tag)))
                                .collect();
                            MySoExecutionResult {
        ...
        Ok(Self {
            ...
            events: MySoTransactionBlockEvents::try_from(events, tx_digest, None, resolver)?,
            results,
            error,
            raw_txn_data,
            raw_effects,
...
#[derive(Eq, PartialEq, Clone, Debug, Serialize, Deserialize, JsonSchema)]
pub enum MySoTransactionBlockBuilderMode {
    /// Regular MySo Transactions that are committed on chain
    Commit,
...
impl Display for MySoExecutionStatus {
    fn fmt(&self, f: &mut Formatter<'_>) -> fmt::Result {
        match self {
...
impl MySoExecutionStatus {
    pub fn is_ok(&self) -> bool {
        matches!(self, MySoExecutionStatus::Success)
    }
    pub fn is_err(&self) -> bool {
        matches!(self, MySoExecutionStatus::Failure { .. })
    }
}

impl From<ExecutionStatus> for MySoExecutionStatus {
...
fn to_myso_object_ref(refs: Vec<ObjectRef>) -> Vec<MySoObjectRef> {
    refs.into_iter().map(MySoObjectRef::from).collect()
}
...
pub struct MySoGasData {
    pub payment: Vec<MySoObjectRef>,
    pub owner: MySoAddress,
    #[schemars(with = "BigInt<u64>")]
    #[serde_as(as = "BigInt<u64>")]
    pub price: u64,
    #[schemars(with = "BigInt<u64>")]
...
#[derive(Debug, Deserialize, Serialize, JsonSchema, Clone, PartialEq, Eq)]
#[enum_dispatch(MySoTransactionBlockDataAPI)]
#[serde(
    rename = "TransactionBlockData",
    rename_all = "camelCase",
    tag = "messageVersion"
)]
pub enum MySoTransactionBlockData {
    V1(MySoTransactionBlockDataV1),
}

#[enum_dispatch]
pub trait MySoTransactionBlockDataAPI {
    fn transaction(&self) -> &MySoTransactionBlockKind;
    fn sender(&self) -> &MySoAddress;
    fn gas_data(&self) -> &MySoGasData;
}

#[derive(Debug, Deserialize, Serialize, JsonSchema, Clone, PartialEq, Eq)]
#[serde(rename = "TransactionBlockDataV1", rename_all = "camelCase")]
pub struct MySoTransactionBlockDataV1 {
    pub transaction: MySoTransactionBlockKind,
    pub sender: MySoAddress,
    pub gas_data: MySoGasData,
}

impl MySoTransactionBlockDataAPI for MySoTransactionBlockDataV1 {
    fn transaction(&self) -> &MySoTransactionBlockKind {
        &self.transaction
    }
    fn sender(&self) -> &MySoAddress {
...
impl MySoTransactionBlockData {
    pub fn move_calls(&self) -> Vec<&MySoProgrammableMoveCall> {
        match self {
            Self::V1(data) => match &data.transaction {
                MySoTransactionBlockKind::ProgrammableTransaction(pt) => pt
                    .commands
                    .iter()
                    .filter_map(|command| match command {
                        MySoCommand::MoveCall(c) => Some(&**c),
                        _ => None,
                    })
                    .collect(),
                _ => vec![],
    ...
    fn try_from_inner(
        ...
        transaction: MySoTransactionBlockKind,
    ) -> Result<Self, anyhow::Error> {
        let message_version = data.message_version();
        let sender = data.sender();
        let gas_data = MySoGasData {
            payment: data
                .gas()
                .iter()
                .map(|obj_ref| MySoObjectRef::from(*obj_ref))
                .collect(),
        ...
        match message_version {
            1 => Ok(MySoTransactionBlockData::V1(MySoTransactionBlockDataV1 {
                transaction,
                sender,
                gas_data,
            })),
    ...
    pub fn try_from_with_module_cache(
        ...
        let transaction = MySoTransactionBlockKind::try_from_with_module_cache(
    ...
    pub async fn try_from_with_package_resolver(
        ...
        let transaction = MySoTransactionBlockKind::try_from_with_package_resolver(
            data.clone().into_kind(),
            package_resolver,
        )
        .await?;
...
impl Display for MySoTransactionBlockData {
    fn fmt(&self, f: &mut Formatter<'_>) -> fmt::Result {
        match self {
            Self::V1(data) => {
...
pub struct MySoTransactionBlock {
    pub data: MySoTransactionBlockData,
    pub tx_signatures: Vec<GenericSignature>,
}

impl MySoTransactionBlock {
    pub fn try_from(
    ...
    // TODO: the MySoTransactionBlock `try_from` can be removed after cleaning up indexer v1, so are the related
    // `try_from` methods for nested structs like MySoTransactionBlockData etc.
    pub async fn try_from_with_package_resolver(
        data: SenderSignedData,
        package_resolver: &Resolver<impl PackageStore>,
    ) -> Result<Self, anyhow::Error> {
...
impl Display for MySoTransactionBlock {
    fn fmt(&self, f: &mut Formatter<'_>) -> fmt::Result {
        let mut builder = TableBuilder::default();
        ...
        for tx_sig in &self.tx_signatures {
            builder.push_record(vec![format!(
                ...
                match tx_sig {
                    ...
                    _ => Base64::from_bytes(tx_sig.as_ref()).encoded(), // the signatures for multisig and zklogin are not mysoted to be parsed out. they should be interpreted as a whole
                }
            )]);
        }
...
#[derive(Debug, Clone, Serialize, Deserialize, JsonSchema, PartialEq, Eq)]
pub struct MySoGenesisTransaction {
    pub objects: Vec<ObjectID>,
}

#[serde_as]
#[derive(Debug, Clone, Serialize, Deserialize, JsonSchema, PartialEq, Eq)]
pub struct MySoConsensusCommitPrologue {
    #[schemars(with = "BigInt<u64>")]
    #[serde_as(as = "BigInt<u64>")]
    pub epoch: u64,
    #[schemars(with = "BigInt<u64>")]
...
#[serde_as]
#[derive(Debug, Clone, Serialize, Deserialize, JsonSchema, PartialEq, Eq)]
pub struct MySoConsensusCommitPrologueV2 {
    #[schemars(with = "BigInt<u64>")]
    #[serde_as(as = "BigInt<u64>")]
    pub epoch: u64,
    #[schemars(with = "BigInt<u64>")]
...
#[serde_as]
#[derive(Debug, Clone, Serialize, Deserialize, JsonSchema, PartialEq, Eq)]
pub struct MySoConsensusCommitPrologueV3 {
    #[schemars(with = "BigInt<u64>")]
    #[serde_as(as = "BigInt<u64>")]
    pub epoch: u64,
    #[schemars(with = "BigInt<u64>")]
...
#[serde_as]
#[derive(Debug, Clone, Serialize, Deserialize, JsonSchema, PartialEq, Eq)]
pub struct MySoConsensusCommitPrologueV4 {
    #[schemars(with = "BigInt<u64>")]
    #[serde_as(as = "BigInt<u64>")]
    pub epoch: u64,
    #[schemars(with = "BigInt<u64>")]
...
#[serde_as]
#[derive(Debug, Clone, Serialize, Deserialize, JsonSchema, PartialEq, Eq)]
pub struct MySoAuthenticatorStateUpdate {
    #[schemars(with = "BigInt<u64>")]
    #[serde_as(as = "BigInt<u64>")]
    pub epoch: u64,
    #[schemars(with = "BigInt<u64>")]
    ...
    pub new_active_jwks: Vec<MySoActiveJwk>,
}

#[serde_as]
#[derive(Debug, Clone, Serialize, Deserialize, JsonSchema, PartialEq, Eq)]
pub struct MySoRandomnessStateUpdate {
    #[schemars(with = "BigInt<u64>")]
    #[serde_as(as = "BigInt<u64>")]
    pub epoch: u64,
...
#[serde_as]
#[derive(Debug, Clone, Serialize, Deserialize, JsonSchema, PartialEq, Eq)]
pub struct MySoEndOfEpochTransaction {
    pub transactions: Vec<MySoEndOfEpochTransactionKind>,
}

#[serde_as]
#[derive(Debug, Clone, Serialize, Deserialize, JsonSchema, PartialEq, Eq)]
pub enum MySoEndOfEpochTransactionKind {
    ChangeEpoch(MySoChangeEpoch),
    AuthenticatorStateCreate,
    AuthenticatorStateExpire(MySoAuthenticatorStateExpire),
...
#[serde_as]
#[derive(Debug, Clone, Serialize, Deserialize, JsonSchema, PartialEq, Eq)]
pub struct MySoAuthenticatorStateExpire {
    #[schemars(with = "BigInt<u64>")]
    #[serde_as(as = "BigInt<u64>")]
    pub min_epoch: u64,
}
...
#[serde_as]
#[derive(Debug, Clone, Serialize, Deserialize, JsonSchema, PartialEq, Eq)]
pub struct MySoActiveJwk {
    pub jwk_id: MySoJwkId,
    pub jwk: MySoJWK,
...
impl From<ActiveJwk> for MySoActiveJwk {
...
pub enum MySoInputObjectKind {
    // A Move package, must be immutable.
    MovePackage(ObjectID),
    // A Move object, either immutable, or owned mutable.
    ImmOrOwnedMoveObject(MySoObjectRef),
...
/// commands
#[derive(Debug, Clone, Serialize, Deserialize, JsonSchema, PartialEq, Eq)]
pub struct MySoProgrammableTransactionBlock {
    /// Input objects or primitive values
    pub inputs: Vec<MySoCallArg>,
    ...
    pub commands: Vec<MySoCommand>,
}

impl Display for MySoProgrammableTransactionBlock {
...
impl MySoProgrammableTransactionBlock {
    fn try_from_with_module_cache(
        ...
        Ok(MySoProgrammableTransactionBlock {
            inputs: inputs
                .into_iter()
                .zip(input_types)
                .map(|(arg, layout)| MySoCallArg::try_from(arg, layout.as_ref()))
                .collect::<Result<_, _>>()?,
            commands: commands.into_iter().map(MySoCommand::from).collect(),
        })
    }
...
#[derive(Debug, Clone, Serialize, Deserialize, JsonSchema, PartialEq, Eq)]
#[serde(rename = "MySoTransaction")]
pub enum MySoCommand {
    /// A call to either an entry or a public Move function
    MoveCall(Box<MySoProgrammableMoveCall>),
    /// `(Vec<forall T:key+store. T>, address)`
    /// It sends n-objects to the specified address. These objects must have store
    /// (public transfer) and either the previous owner must be an address or the object must
    /// be newly created.
    TransferObjects(Vec<MySoArgument>, MySoArgument),
    /// `(&mut Coin<T>, Vec<u64>)` -> `Vec<Coin<T>>`
    /// It splits off some amounts into a new coins with those amounts
    SplitCoins(MySoArgument, Vec<MySoArgument>),
    /// `(&mut Coin<T>, Vec<Coin<T>>)`
    /// It merges n-coins into the first coin
    MergeCoins(MySoArgument, Vec<MySoArgument>),
    /// Publishes a Move package. It takes the package bytes and a list of the package's transitive
    /// dependencies to link against on-chain.
    Publish(Vec<ObjectID>),
    /// Upgrades a Move package
    Upgrade(Vec<ObjectID>, ObjectID, MySoArgument),
    /// `forall T: Vec<T> -> vector<T>`
    /// Given n-values of the same type, it constructs a vector. For non objects or an empty vector,
    /// the type tag must be specified.
    MakeMoveVec(Option<String>, Vec<MySoArgument>),
}

impl Display for MySoCommand {
    fn fmt(&self, f: &mut Formatter<'_>) -> std::fmt::Result {
        match self {
            Self::MoveCall(p) => {
                write!(f, "MoveCall({p})")
...
impl From<Command> for MySoCommand {
    fn from(value: Command) -> Self {
        match value {
            Command::MoveCall(m) => MySoCommand::MoveCall(Box::new((*m).into())),
            Command::TransferObjects(args, arg) => MySoCommand::TransferObjects(
                args.into_iter().map(MySoArgument::from).collect(),
                arg.into(),
            ),
            Command::SplitCoins(arg, args) => MySoCommand::SplitCoins(
                arg.into(),
                args.into_iter().map(MySoArgument::from).collect(),
            ),
            Command::MergeCoins(arg, args) => MySoCommand::MergeCoins(
                arg.into(),
                args.into_iter().map(MySoArgument::from).collect(),
            ),
            Command::Publish(_modules, dep_ids) => MySoCommand::Publish(dep_ids),
            Command::MakeMoveVec(tag_opt, args) => MySoCommand::MakeMoveVec(
                tag_opt.map(|tag| tag.to_string()),
                args.into_iter().map(MySoArgument::from).collect(),
            ),
            Command::Upgrade(_modules, dep_ids, current_package_id, ticket) => {
                MySoCommand::Upgrade(dep_ids, current_package_id, MySoArgument::from(ticket))
            }
        }
    }
}
...
impl From<Argument> for MySoArgument {
...
#[derive(Debug, Clone, Serialize, Deserialize, JsonSchema, PartialEq, Eq)]
pub struct MySoProgrammableMoveCall {
    ...
    pub arguments: Vec<MySoArgument>,
}
...
impl Display for MySoProgrammableMoveCall {
    fn fmt(&self, f: &mut Formatter<'_>) -> std::fmt::Result {
        let Self {
            package,
            module,
...
impl From<ProgrammableMoveCall> for MySoProgrammableMoveCall {
    fn from(value: ProgrammableMoveCall) -> Self {
        ...
        Self {
            ...
            arguments: arguments.into_iter().map(MySoArgument::from).collect(),
        }
    }
}
...
impl From<InputObjectKind> for MySoInputObjectKind {
...
impl TryInto<TypeTag> for MySoTypeTag {
    type Error = anyhow::Error;
    fn try_into(self) -> Result<TypeTag, Self::Error> {
        parse_myso_type_tag(&self.0)
    }
}

impl From<TypeTag> for MySoTypeTag {
...
pub struct MoveCallParams {
    ...
    pub function: String,
    #[serde(default)]
    pub type_arguments: Vec<MySoTypeTag>,
    pub arguments: Vec<MySoJsonValue>,
}
...
pub struct TransactionBlockBytes {
    ...
    pub gas: Vec<MySoObjectRef>,
    /// objects to be used in this transaction
    pub input_objects: Vec<MySoInputObjectKind>,
}
...
pub enum MySoCallArg {
    ...
    FundsWithdrawal(MySoFundsWithdrawalArg),
}
...
impl MySoCallArg {
    pub fn try_from(
        ...
        Ok(match value {
            CallArg::Pure(p) => MySoCallArg::Pure(MySoPureValue {
                value_type: layout.map(|l| l.into()),
                value: MySoJsonValue::from_bcs_bytes(layout, &p)?,
            }),
            CallArg::Object(ObjectArg::ImmOrOwnedObject((id, version, digest))) => {
                MySoCallArg::Object(MySoObjectArg::ImmOrOwnedObject {
                    object_id: id,
                    version,
                    digest,
                })
            ...
            CallArg::Object(ObjectArg::SharedObject {
                ...
                initial_shared_version,
                mutability,
            }) => MySoCallArg::Object(MySoObjectArg::SharedObject {
                object_id: id,
                initial_shared_version,
                mutable: mutability.is_exclusive(),
            }),
            CallArg::Object(ObjectArg::Receiving((object_id, version, digest))) => {
                MySoCallArg::Object(MySoObjectArg::Receiving {
                    object_id,
                    version,
                    digest,
                })
            }
            CallArg::FundsWithdrawal(arg) => MySoCallArg::FundsWithdrawal(MySoFundsWithdrawalArg {
                reservation: match arg.reservation {
                    Reservation::MaxAmountU64(amount) => MySoReservation::MaxAmountU64(amount),
                },
                type_arg: match arg.type_arg {
                    WithdrawalTypeArg::Balance(type_input) => {
                        MySoWithdrawalTypeArg::Balance(type_input.into())
                    }
                },
                withdraw_from: match arg.withdraw_from {
                    WithdrawFrom::Sender => MySoWithdrawFrom::Sender,
                    WithdrawFrom::Sponsor => MySoWithdrawFrom::Sponsor,
                },
            }),
        })
    }

    pub fn pure(&self) -> Option<&MySoJsonValue> {
        match self {
            MySoCallArg::Pure(v) => Some(&v.value),
            _ => None,
        }
    }
    ...
    pub fn object(&self) -> Option<&ObjectID> {
        match self {
            ...
            | MySoCallArg::Object(MySoObjectArg::Receiving { object_id, .. }) => Some(object_id),
            _ => None,
        }
    }
}
...
#[derive(Eq, PartialEq, Debug, Clone, Serialize, Deserialize, JsonSchema)]
#[serde(rename_all = "camelCase")]
pub struct MySoPureValue {
    #[schemars(with = "Option<String>")]
    #[serde_as(as = "Option<AsMySoTypeTag>")]
    value_type: Option<TypeTag>,
    value: MySoJsonValue,
}

impl MySoPureValue {
    pub fn value(&self) -> MySoJsonValue {
        self.value.clone()
    }
...
#[derive(Eq, PartialEq, Debug, Clone, Serialize, Deserialize, JsonSchema)]
#[serde(rename_all = "camelCase")]
pub enum MySoWithdrawalTypeArg {
    Balance(MySoTypeTag),
}

#[derive(Eq, PartialEq, Debug, Clone, Serialize, Deserialize, JsonSchema)]
#[serde(rename_all = "camelCase")]
pub enum MySoWithdrawFrom {
    Sender,
    Sponsor,
}
...
#[derive(Eq, PartialEq, Debug, Clone, Serialize, Deserialize, JsonSchema)]
#[serde(rename_all = "camelCase")]
pub struct MySoFundsWithdrawalArg {
    pub reservation: MySoReservation,
    pub type_arg: MySoWithdrawalTypeArg,
    pub withdraw_from: MySoWithdrawFrom,
}
...
impl From<EffectsWithInput> for MySoTransactionBlockEffects {
