...
use myso_protocol_config::{ProtocolConfig, ProtocolConfigValue, ProtocolVersion};
use myso_types::myso_serde::Readable;
use myso_types::myso_serde::{AsProtocolVersion, BigInt};
...
pub enum MySoProtocolConfigValue {
    U16(
...
impl From<ProtocolConfigValue> for MySoProtocolConfigValue {
    fn from(value: ProtocolConfigValue) -> Self {
        match value {
            ProtocolConfigValue::u16(y) => MySoProtocolConfigValue::U16(y),
            ProtocolConfigValue::u32(y) => MySoProtocolConfigValue::U32(y),
            ProtocolConfigValue::u64(x) => MySoProtocolConfigValue::U64(x),
            ProtocolConfigValue::bool(z) => MySoProtocolConfigValue::Bool(z),
        }
    }
}
...
pub struct ProtocolConfigResponse {
    ...
    pub attributes: BTreeMap<String, Option<MySoProtocolConfigValue>>,
}
    ...
    fn from(config: ProtocolConfig) -> Self {
        ProtocolConfigResponse {
            ...
            attributes: config
                .attr_map()
                .into_iter()
                .map(|(k, v)| (k, v.map(MySoProtocolConfigValue::from)))
                .collect(),
