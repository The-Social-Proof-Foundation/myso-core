...
use myso_types::error::{
    ExecutionError, MySoErrorKind, MySoObjectResponseError, MySoResult, UserInputError,
    UserInputResult,
};
...
pub struct MySoObjectResponse {
    ...
    pub data: Option<MySoObjectData>,
    #[serde(skip_serializing_if = "Option::is_none")]
    pub error: Option<MySoObjectResponseError>,
}

impl MySoObjectResponse {
    pub fn new(data: Option<MySoObjectData>, error: Option<MySoObjectResponseError>) -> Self {
        Self { data, error }
    }

    pub fn new_with_data(data: MySoObjectData) -> Self {
        Self {
            data: Some(data),
            error: None,
        }
    ...
    pub fn new_with_error(error: MySoObjectResponseError) -> Self {
        Self {
            data: None,
            error: Some(error),
        }
...
impl Ord for MySoObjectResponse {
    fn cmp(&self, other: &Self) -> Ordering {
        match (&self.data, &other.data) {
            ...
            // In this ordering those with data will come before MySoObjectResponses that are errors.
            (Some(_), None) => Ordering::Less,
            (None, Some(_)) => Ordering::Greater,
            // MySoObjectResponses that are errors are just considered equal.
            _ => Ordering::Equal,
        }
    }
}

impl PartialOrd for MySoObjectResponse {
...
impl MySoObjectResponse {
    pub fn move_object_bcs(&self) -> Option<&Vec<u8>> {
        match &self.data {
            Some(MySoObjectData {
                bcs: Some(MySoRawData::MoveObject(obj)),
                ..
            }) => Some(&obj.bcs_bytes),
            _ => None,
        }
    ...
    pub fn object_id(&self) -> Result<ObjectID, anyhow::Error> {
        match (&self.data, &self.error) {
            ...
            (None, Some(MySoObjectResponseError::NotExists { object_id })) => Ok(*object_id),
            (
                None,
                Some(MySoObjectResponseError::Deleted {
                    object_id,
                    version: _,
                    digest: _,
                }),
            ...
            _ => Err(anyhow!(
                "Could not get object_id, something went wrong with MySoObjectResponse construction."
            )),
        }
    }
...
impl TryFrom<MySoObjectResponse> for ObjectInfo {
    type Error = anyhow::Error;

    fn try_from(value: MySoObjectResponse) -> Result<Self, Self::Error> {
        let MySoObjectData {
            object_id,
            version,
            digest,
...
pub struct MySoObjectData {
    ...
    /// The type of the object. Default to be None unless MySoObjectDataOptions.showType is set to true
    ...
    /// The owner of this object. Default to be None unless MySoObjectDataOptions.showOwner is set to true
    #[serde(skip_serializing_if = "Option::is_none")]
    pub owner: Option<Owner>,
    /// The digest of the transaction that created or last mutated this object. Default to be None unless
    /// MySoObjectDataOptions.showPreviousTransaction is set to true
    #[serde(skip_serializing_if = "Option::is_none")]
    ...
    /// The Display metadata for frontend UI rendering, default to be None unless MySoObjectDataOptions.showContent is set to true
    /// This can also be None if the struct type does not have Display defined
    /// See more details in <https://forums.myso.io/t/nft-object-display-proposal/4872>
    #[serde(skip_serializing_if = "Option::is_none")]
    pub display: Option<DisplayFieldsResponse>,
    /// Move object content or package content, default to be None unless MySoObjectDataOptions.showContent is set to true
    #[serde(skip_serializing_if = "Option::is_none")]
    pub content: Option<MySoParsedData>,
    /// Move object content or package content in BCS, default to be None unless MySoObjectDataOptions.showBcs is set to true
    #[serde(skip_serializing_if = "Option::is_none")]
    pub bcs: Option<MySoRawData>,
}
...
impl MySoObjectData {
    ...
    pub fn try_into_object(
        ...
        let data = match self.bcs {
            Some(MySoRawData::MoveObject(o)) => Data::Move(unsafe {
            ...
            Some(MySoRawData::Package(p)) => Data::Package(MovePackage::new(
                p.id,
                self.version,
                p.module_map,
            ...
            _ => Err(anyhow!(
                "BCS data is required to convert MySoObjectData to Object"
            ))?,
        };
        Ok(ObjectInner {
            data,
            owner: self
                .owner
                .ok_or_else(|| anyhow!("Owner is required to convert MySoObjectData to Object"))?,
            previous_transaction: self.previous_transaction.ok_or_else(|| {
                anyhow!("previous_transaction is required to convert MySoObjectData to Object")
            })?,
            storage_rebate: self.storage_rebate.ok_or_else(|| {
                anyhow!("storage_rebate is required to convert MySoObjectData to Object")
            })?,
        }
        .into())
    }
...
impl Display for MySoObjectData {
...
impl TryFrom<&MySoObjectData> for GasCoin {
    type Error = anyhow::Error;
    fn try_from(object: &MySoObjectData) -> Result<Self, Self::Error> {
        match &object
            .content
            .as_ref()
            .ok_or_else(|| anyhow!("Expect object content to not be empty"))?
        {
            MySoParsedData::MoveObject(o) => {
                if GasCoin::type_() == o.type_ {
                    return GasCoin::try_from(&o.fields);
                }
            }
            MySoParsedData::Package(_) => {}
        }
...
impl TryFrom<&MySoMoveStruct> for GasCoin {
    type Error = anyhow::Error;
    fn try_from(move_struct: &MySoMoveStruct) -> Result<Self, Self::Error> {
        match move_struct {
            MySoMoveStruct::WithFields(fields) | MySoMoveStruct::WithTypes { type_: _, fields } => {
                if let Some(MySoMoveValue::String(balance)) = fields.get("balance")
                    && let Ok(balance) = balance.parse::<u64>()
                    && let Some(MySoMoveValue::UID { id }) = fields.get("id")
                {
...
pub struct MySoObjectDataOptions {
...
impl TryFrom<(ObjectRead, MySoObjectDataOptions)> for MySoObjectResponse {
    type Error = anyhow::Error;

    fn try_from(
        (object_read, options): (ObjectRead, MySoObjectDataOptions),
    ) -> Result<Self, Self::Error> {
        match object_read {
            ObjectRead::NotExists(id) => Ok(MySoObjectResponse::new_with_error(
                MySoObjectResponseError::NotExists { object_id: id },
            )),
            ObjectRead::Exists(object_ref, o, layout) => {
                let data = (object_ref, o, layout, options).try_into()?;
                Ok(MySoObjectResponse::new_with_data(data))
            }
            ObjectRead::Deleted((object_id, version, digest)) => Ok(
                MySoObjectResponse::new_with_error(MySoObjectResponseError::Deleted {
                    object_id,
                    version,
                    digest,
                }),
...
impl TryFrom<(ObjectInfo, MySoObjectDataOptions)> for MySoObjectResponse {
    type Error = anyhow::Error;

    fn try_from(
        (object_info, options): (ObjectInfo, MySoObjectDataOptions),
    ) -> Result<Self, Self::Error> {
        let MySoObjectDataOptions {
            show_type,
            show_owner,
        ...
        Ok(Self::new_with_data(MySoObjectData {
    ...
    )> for MySoObjectData
{
    ...
    fn try_from(
        ...
        let bcs: Option<MySoRawData> = if show_bcs {
            let data = match o.data.clone() {
                Data::Move(m) => {
                    let layout = layout.clone().ok_or_else(|| {
                        anyhow!("Layout is required to convert Move object to json")
                    })?;
                    MySoRawData::try_from_object(m, layout)?
                }
                Data::Package(p) => MySoRawData::try_from_package(p)
        ...
        let content: Option<MySoParsedData> = if show_content {
            let data = match o.data {
                Data::Move(m) => {
                    let layout = layout.ok_or_else(|| {
                        anyhow!("Layout is required to convert Move object to json")
                    })?;
                    MySoParsedData::try_from_object(m, layout)?
                }
                Data::Package(p) => MySoParsedData::try_from_package(p)?,
            };
            Some(data)
        } else {
            None
    ...
    )> for MySoObjectData
{
    ...
    fn try_from(
        ...
        let mut data: MySoObjectData = (object_ref, o, layout, options).try_into()?;
        if show_display {
...
impl MySoObjectResponse {
    ...
    pub fn object(&self) -> Result<&MySoObjectData, MySoObjectResponseError> {
        if let Some(data) = &self.data {
            Ok(data)
            ...
            Err(MySoObjectResponseError::Unknown)
        }
    }

    /// Returns the object value if there is any, otherwise an Err if
    /// the object does not exist or is deleted.
    pub fn into_object(self) -> Result<MySoObjectData, MySoObjectResponseError> {
        match self.object() {
...
impl TryInto<Object> for MySoObjectData {
    type Error = anyhow::Error;
...
impl Display for MySoObjectRef {
    fn fmt(&self, f: &mut Formatter<'_>) -> fmt::Result {
        write!(
            f,
...
impl From<ObjectRef> for MySoObjectRef {
...
pub enum MySoRawData {
    // Manually handle generic schema generation
    MoveObject(MySoRawMoveObject),
    Package(MySoRawMovePackage),
}

impl MySoData for MySoRawData {
    type ObjectType = MySoRawMoveObject;
    type PackageType = MySoRawMovePackage;
...
pub enum MySoParsedData {
    // Manually handle generic schema generation
    MoveObject(MySoParsedMoveObject),
    Package(MySoMovePackage),
}

impl MySoData for MySoParsedData {
    type ObjectType = MySoParsedMoveObject;
    type PackageType = MySoMovePackage;

    fn try_from_object(
        object: MoveObject,
        layout: MoveStructLayout,
    ) -> Result<Self, anyhow::Error> {
        Ok(Self::MoveObject(MySoParsedMoveObject::try_from_layout(
            object, layout,
        )?))
    }
    ...
    fn try_from_package(package: MovePackage) -> Result<Self, anyhow::Error> {
        ...
        for bytecode in package.serialized_module_map().values() {
            ...
            let module = move_binary_format::CompiledModule::deserialize_with_defaults(bytecode)
                .map_err(|error| MySoErrorKind::ModuleDeserializationFailure {
            ...
            let d = move_disassembler::disassembler::Disassembler::from_module_with_max_size(
                ...
                *myso_types::move_package::MAX_DISASSEMBLED_MODULE_SIZE,
            )
            .map_err(|e| MySoErrorKind::ObjectSerializationError {
                error: e.to_string(),
            })?;
            let bytecode_str =
                d.disassemble()
        ...
        Ok(Self::Package(MySoMovePackage { disassembled }))
    }
...
impl Display for MySoParsedData {
    fn fmt(&self, f: &mut Formatter<'_>) -> fmt::Result {
        let mut writer = String::new();
        match self {
            MySoParsedData::MoveObject(o) => {
...
pub trait MySoMoveObject: Sized {
...
pub struct MySoParsedMoveObject {
    #[serde(rename = "type")]
    #[serde_as(as = "MySoStructTag")]
    #[schemars(with = "String")]
    pub type_: StructTag,
    pub has_public_transfer: bool,
    pub fields: MySoMoveStruct,
}

impl MySoMoveObject for MySoParsedMoveObject {
    fn try_from_layout(
        object: MoveObject,
        layout: MoveStructLayout,
    ) -> Result<Self, anyhow::Error> {
        ...
        Ok(
            if let MySoMoveStruct::WithTypes { type_, fields } = move_struct {
                MySoParsedMoveObject {
                    type_,
                    has_public_transfer: object.has_public_transfer(),
                    fields: MySoMoveStruct::WithFields(fields),
                }
            } else {
                MySoParsedMoveObject {
...
impl MySoParsedMoveObject {
    pub fn try_from_object_read(object_read: ObjectRead) -> Result<Self, anyhow::Error> {
        let parsed_data = MySoParsedData::try_from_object_read(object_read)?;
        match parsed_data {
            MySoParsedData::MoveObject(o) => Ok(o),
            MySoParsedData::Package(_) => Err(anyhow::anyhow!("Object is not a Move object")),
        }
    }
}
...
) -> MySoResult<(StructTag, serde_json::Value)> {
    match event_data.into() {
        MySoMoveValue::Struct(move_struct) => match &move_struct {
            MySoMoveStruct::WithTypes { type_, .. } => {
                Ok((type_.clone(), move_struct.clone().to_json_value()))
            }
            _ => Err(MySoErrorKind::ObjectDeserializationError {
                error: "Found non-type MySoMoveStruct in MoveValue event".to_string(),
            }
            .into()),
        },
        MySoMoveValue::Variant(v) => Ok((v.type_.clone(), v.clone().to_json_value())),
        MySoMoveValue::Vector(_)
        | MySoMoveValue::Number(_)
        | MySoMoveValue::Bool(_)
        | MySoMoveValue::Address(_)
        | MySoMoveValue::String(_)
        | MySoMoveValue::UID { .. }
        | MySoMoveValue::Option(_) => Err(MySoErrorKind::ObjectDeserializationError {
...
impl From<MoveObject> for MySoRawMoveObject {
...
impl MySoMoveObject for MySoRawMoveObject {
    fn try_from_layout(
        object: MoveObject,
...
impl From<MovePackage> for MySoRawMovePackage {
...
pub enum MySoPastObjectResponse {
    /// The object exists and is found with this version
    VersionFound(MySoObjectData),
    /// The object does not exist
    ObjectNotExists(ObjectID),
    /// The object is found to be deleted with this version
    ObjectDeleted(MySoObjectRef),
...
impl MySoPastObjectResponse {
    /// Returns a reference to the object if there is any, otherwise an Err
    pub fn object(&self) -> UserInputResult<&MySoObjectData> {
        match &self {
    ...
    pub fn into_object(self) -> UserInputResult<MySoObjectData> {
        match self {
            Self::ObjectDeleted(oref) => Err(UserInputError::ObjectDeleted {
                object_ref: oref.to_object_ref(),
            }),
...
pub struct MySoMovePackage {
    pub disassembled: BTreeMap<String, Value>,
}

pub type QueryObjectsPage = Page<MySoObjectResponse, CheckpointedObjectID>;
pub type ObjectsPage = Page<MySoObjectResponse, ObjectID>;
...
pub struct MySoGetPastObjectRequest {
...
pub enum MySoObjectDataFilter {
    MatchAll(Vec<MySoObjectDataFilter>),
    MatchAny(Vec<MySoObjectDataFilter>),
    MatchNone(Vec<MySoObjectDataFilter>),
...
impl MySoObjectDataFilter {
    ...
    pub fn matches(&self, object: &ObjectInfo) -> bool {
        match self {
            MySoObjectDataFilter::MatchAll(filters) => !filters.iter().any(|f| !f.matches(object)),
            MySoObjectDataFilter::MatchAny(filters) => filters.iter().any(|f| f.matches(object)),
            MySoObjectDataFilter::MatchNone(filters) => !filters.iter().any(|f| f.matches(object)),
            MySoObjectDataFilter::StructType(s) => {
                ...
                // e.g. `0x2::coin::Coin` can match `0x2::coin::Coin<0x2::myso::MYSO>`
            ...
            MySoObjectDataFilter::MoveModule { package, module } => {
                matches!(&object.type_, ObjectType::Struct(s) if &ObjectID::from(s.address()) == package
                        && s.module() == module.as_ident_str())
            }
            MySoObjectDataFilter::Package(p) => {
                matches!(&object.type_, ObjectType::Struct(s) if &ObjectID::from(s.address()) == p)
            }
            MySoObjectDataFilter::AddressOwner(a) => {
                matches!(object.owner, Owner::AddressOwner(addr) if &addr == a)
            }
            MySoObjectDataFilter::ObjectOwner(o) => {
                matches!(object.owner, Owner::ObjectOwner(addr) if addr == MySoAddress::from(*o))
            }
            MySoObjectDataFilter::ObjectId(id) => &object.object_id == id,
            MySoObjectDataFilter::ObjectIds(ids) => ids.contains(&object.object_id),
            MySoObjectDataFilter::Version(v) => object.version.value() == *v,
        }
    }
}
...
pub struct MySoObjectResponseQuery {
    /// If None, no filter will be applied
    pub filter: Option<MySoObjectDataFilter>,
    /// config which fields to include in the response, by default only digest is included
    pub options: Option<MySoObjectDataOptions>,
}
...
impl MySoObjectResponseQuery {
    ...
    pub fn new_with_filter(filter: MySoObjectDataFilter) -> Self {
        Self {
            filter: Some(filter),
            options: None,
        }
    ...
    pub fn new_with_options(options: MySoObjectDataOptions) -> Self {
        Self {
            filter: None,
