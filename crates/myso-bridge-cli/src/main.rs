...
use myso_bridge::myso_transaction_builder::build_myso_transaction;
use myso_bridge::types::BridgeActionType;
use myso_bridge::utils::get_eth_provider;
use myso_bridge::utils::{EthBridgeContracts, get_eth_contracts};
use myso_bridge::utils::{
...
use myso_types::bridge::{MoveTypeCommitteeMember, MoveTypeCommitteeMemberRegistration};
...
async fn main() -> anyhow::Result<()> {
    ...
    match args.command {
        ...
        BridgeCommand::Governance {
            ...
            let myso_bridge_client =
                MySoBridgeClient::new(&config.myso_rpc_url, metrics.clone()).await?;

            let (myso_key, myso_address, gas_object_ref) = config
                .get_myso_account_info()
                .await
            ...
            // Handle MySo Side
            if chain_id.is_myso_chain() {
                let myso_chain_id = BridgeChainId::try_from(bridge_summary.chain_id).unwrap();
                assert_eq!(
                    myso_chain_id, chain_id,
                    "Chain ID mismatch, expected: {:?}, got from url: {:?}",
                    chain_id, myso_chain_id
                );
                // Create BridgeAction
                let myso_action = make_action(myso_chain_id, &cmd);
                println!("Action to execute on MySo: {:?}", myso_action);
                ...
                let bridge_arg = myso_bridge_client
                    ...
                    .await;
                let rgp = myso_bridge_client
                    .get_reference_gas_price_until_success()
                    .await;
                let id_token_map = myso_bridge_client.get_token_id_map().await.unwrap();
                ...
                let myso_sig = Signature::new_secure(
                    &IntentMessage::new(Intent::myso_transaction(), tx.clone()),
                    &myso_key,
                );
                let tx = Transaction::from_data(tx, vec![myso_sig]);
                ...
                match &resp.status {
                    myso_json_rpc_types::MySoExecutionStatus::Success => {
                        println!("MySo Transaction succeeded");
                    }
                    myso_json_rpc_types::MySoExecutionStatus::Failure { error } => {
                        println!("MySo Transaction failed: {:?}", error);
                    }
                }
                return Ok(());
            }
        ...
        BridgeCommand::ViewBridgeRegistration { myso_rpc_url } => {
            let metrics = Arc::new(BridgeMetrics::new_for_testing());
            let myso_bridge_client = MySoBridgeClient::new(&myso_rpc_url, metrics).await?;
            let bridge_summary = myso_bridge_client
                .get_bridge_summary()
                .await
                .map_err(|e| anyhow::anyhow!("Failed to get bridge summary: {:?}", e))?;
            let move_type_bridge_committee = bridge_summary.committee;
            let myso_client = Client::new(myso_rpc_url)?;
            let stakes = myso_client
                .get_committee(None)
                .await?
                .voting_rights
                .into_iter()
                .collect::<HashMap<_, _>>();
            let names = myso_client
                ...
                .map(|summary| {
                    let protocol_key =
                        AuthorityPublicKeyBytes::from_bytes(&summary.protocol_pubkey_bytes)
                            .unwrap();
                    (summary.myso_address, (protocol_key, summary.name))
                })
                .collect::<HashMap<_, _>>();
            let mut authorities = vec![];
            let mut output_wrapper = Output::<OutputMySoBridgeRegistration>::default();
            for (_, member) in move_type_bridge_committee.member_registration {
                ...
                let Ok(pubkey) = BridgeAuthorityPublicKey::from_bytes(&bridge_pubkey_bytes) else {
                    output_wrapper.add_error(format!(
                        ...
                        myso_address, bridge_pubkey_bytes
                    ));
                    continue;
                };
                ...
                let Ok(url) = from_utf8(&http_rest_url) else {
                    output_wrapper.add_error(format!(
                        ...
                        myso_address, http_rest_url
                    ));
                    continue;
                };
                let url = url.to_string();

                let (protocol_key, name) = names.get(&myso_address).unwrap();
                let stake = stakes.get(protocol_key).unwrap();
                authorities.push((name, myso_address, pubkey, eth_address, url, stake));
            }
            let total_stake = authorities
                .iter()
                .map(|(_, _, _, _, _, stake)| **stake)
                .sum::<u64>();
            let mut output = OutputMySoBridgeRegistration {
                total_registered_stake: total_stake as f32 / TOTAL_VOTING_POWER as f32 * 100.0,
                ..Default::default()
            };
            for (name, myso_address, pubkey, eth_address, url, stake) in authorities {
        ...
        BridgeCommand::ViewMySoBridge {
            ...
            let names = myso_client
                ...
                .active_validators
                .into_iter()
                .map(|summary| (summary.myso_address, summary.name))
                .collect::<HashMap<_, _>>();
            let mut authorities = vec![];
            ...
            let client = reqwest::Client::builder()
                ...
                .build()
                .unwrap();
            let mut output_wrapper = Output::<OutputMySoBridge>::default();
            for (_, member) in move_type_bridge_committee.members {
                ...
                let name = names.get(&myso_address).cloned().unwrap_or(url.clone());
            ...
            let total_stake = authorities
                ...
                .sum::<u64>();
            let mut output = OutputMySoBridge {
            ...
            for ((name, myso_address, pubkey, eth_address, url, stake, blocklisted), ping_resp) in
        ...
        BridgeCommand::Client { config_path, cmd } => {
            ...
            let config = LoadedBridgeCliConfig::load(config).await?;
            let metrics = Arc::new(BridgeMetrics::new_for_testing());
            let myso_bridge_client = MySoBridgeClient::new(&config.myso_rpc_url, metrics).await?;
            cmd.handle(&config, myso_bridge_client).await?;
            return Ok(());
        }
    }
...
#[derive(serde::Serialize, Default)]
struct OutputMySoBridgeRegistration {
