...
use myso_bridge_schema::models::ProgressStore;
use myso_bridge_schema::schema;
use myso_bridge_schema::schema::progress_store::{columns, dsl};
use myso_bridge_schema::schema::{myso_error_transactions, token_transfer, token_transfer_data};
                    ...
                    for d in data {
                        match d {
                            ...
                            ProcessedTxnData::Error(e) => {
                                diesel::insert_into(myso_error_transactions::table)
                                    .values(&e.to_db())
                                    .on_conflict_do_nothing()
                                    .execute(conn)
                                    .await?;
