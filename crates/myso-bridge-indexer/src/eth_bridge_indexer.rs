...
use myso_bridge::error::BridgeError;
use myso_bridge::eth_client::EthClient;
use myso_bridge::eth_syncer::EthSyncer;
use myso_bridge::metrics::BridgeMetrics;
use myso_bridge::retry_with_max_elapsed_time;
...
impl EthSubscriptionDatasource {
    pub async fn new(
        eth_myso_bridge_contract_addresses: Vec<EthAddress>,
        eth_client: Arc<EthClient>,
        eth_ws_url: String,
        metrics: Box<dyn IndexerMetricProvider>,
        genesis_block: u64,
        ...
        Ok(Self {
            addresses: eth_myso_bridge_contract_addresses,
            eth_client,
            eth_ws_url,
            metrics,
            genesis_block,
...
impl EthFinalizedSyncDatasource {
    pub async fn new(
        ...
        Ok(Self {
            bridge_addresses: eth_myso_bridge_contract_addresses,
            eth_http_url,
            eth_client,
            metrics,
            bridge_metrics,
...
impl DataMapper<RawEthData, ProcessedTxnData> for EthDataMapper {
    fn map(
        ...
        match bridge_event {
            EthBridgeEvent::EthMySoBridgeEvents(bridge_event) => match &bridge_event {
                EthMySoBridgeEvents::TokensDeposited(bridge_event) => {
                    info!(
                    ...
                    processed_txn_data.push(ProcessedTxnData::TokenTransfer(TokenTransfer {
                        ...
                        data: Some(TokenTransferData {
                            ...
                            amount: bridge_event.mysoAdjustedAmount,
                            is_finalized,
                        }),
                    }));
                }
                EthMySoBridgeEvents::TokensDepositedV2(bridge_event) => {
                    info!(
                        "Observed Eth Deposit at block: {}, tx_hash: {}",
                        log.block_number(),
                        log.tx_hash
                ...
                EthMySoBridgeEvents::TokensClaimed(bridge_event) => {
                    info!(
                ...
                EthMySoBridgeEvents::EmergencyOperation(f) => {
                    info!(
                ...
                EthMySoBridgeEvents::ContractUpgraded(f) => {
                    info!(
                        "Observed Eth MySoBridge Upgrade at block: {}, tx_hash: {}",
                        log.block_number(),
                        log.tx_hash
                    );
                ...
                | EthMySoBridgeEvents::Paused(_)
                | EthMySoBridgeEvents::Unpaused(_)
            ...
            EthBridgeEvent::EthBridgeConfigEvents(bridge_event) => match &bridge_event {
                ...
                EthBridgeConfigEvents::TokenAdded(_) => {
                    info!(
                        "Observed Eth AddMySoTokens at block: {}, tx_hash: {}",
                        log.block_number(),
                        log.tx_hash
                    );
