...
impl MySoTxnError {
    fn to_db(&self) -> MySoErrorTransactions {
        MySoErrorTransactions {
...
async fn get_eth_bridge_contract_addresses(
    ...
    let bridge_address = EthAddress::from_str(&config.eth_myso_bridge_contract_address)?;
...
// inline old myso-indexer-builder
