...
use myso_bridge::myso_bridge_watchdog::{
    BridgeWatchDog,
    eth_bridge_status::EthBridgeStatus,
    eth_vault_balance::{EthereumVaultBalance, VaultAsset},
    metrics::WatchdogMetrics,
    myso_bridge_status::MySoBridgeStatus,
};
use myso_bridge_indexer::config::IndexerConfig;
use myso_bridge_indexer::metrics::BridgeIndexerMetrics;
use myso_bridge_indexer::postgres_manager::get_connection_pool;
use myso_bridge_indexer::{create_eth_subscription_indexer, create_eth_sync_indexer};
...
async fn main() -> Result<()> {
    ...
    let eth_bridge_proxy_address = EthAddress::from_str(&config.eth_myso_bridge_contract_address)?;
    let mut tasks = vec![];
    ...
    let myso_bridge_client =
        Arc::new(MySoBridgeClient::new(&config.myso_rpc_url, bridge_metrics.clone()).await?);
    start_watchdog(
        config,
...
async fn start_watchdog(
    ...
    let myso_bridge_status =
        MySoBridgeStatus::new(myso_client, watchdog_metrics.myso_bridge_paused.clone());
