...
/// Example log format:
/// 2025-08-10T00:05:45.423808Z  INFO myso_indexer_alt_jsonrpc_proxy::handlers: Request: Method: mysox_getAllBalances, Params: ["0x10e8a57972082d89f8e2a31589a96da4a0ade2ac003e4f41a0f7b77dbfd752ba"] | Response body (UTF-8): {"jsonrpc":"2.0","id":47539,"result":[...]}
fn extract_from_proxy_message(message: &str) -> Option<LogEntry> {
    ...
    if request_split.len() == 2 {
        ...
        // Extract method from "Request: Method: mysox_getAllBalances, Params: ..."
...
async fn fetch_logs(
    ...
    let resp = client
        ...
        .header("X-Scope-OrgID", "myso-fleet")
        .query(&params)
        .send()
        .await?;
...
async fn run() -> Result<(), Box<dyn Error>> {
    ...
    let namespace = if net == "testnet" {
        ...
        "myso-indexer-alt-jsonrpc-proxy-mainnet".to_string()
    } else {
