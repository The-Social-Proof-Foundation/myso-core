...
static METHOD_CURSOR_POSITIONS: phf::Map<&'static str, usize> = phf_map! {
    // based on function headers in crates/myso-json-rpc-api/src/indexer.rs
    "mysox_getOwnedObjects" => 2,
    "mysox_queryTransactionBlocks" => 1,
    // based on function headers in crates/myso-json-rpc-api/src/coin.rs
    "mysox_getCoins" => 2,
    "mysox_getAllCoins" => 1,
};
...
static METHOD_LENGTHS: phf::Map<&'static str, usize> = phf_map! {
    // based on function headers in crates/myso-json-rpc-api/src/indexer.rs
    "mysox_getOwnedObjects" => 4,
    "mysox_queryTransactionBlocks" => 4,
    // based on function headers in crates/myso-json-rpc-api/src/coin.rs
    "mysox_getCoins" => 4,
    "mysox_getAllCoins" => 3,
};
...
pub async fn run_queries(
    ...
    let requests: Vec<_> = requests
        ...
        // Skip mysox_getOwnedObjects requests with MatchAny & MatchAll filters b/c it's not supported.
        .filter(|r| {
            !(r.method == "mysox_getOwnedObjects"
                && r.body_json
                    .get("params")
