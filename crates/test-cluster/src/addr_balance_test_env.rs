...
use myso_protocol_config::{OverrideGuard, ProtocolConfig, ProtocolVersion};
...
pub struct TestEnv {
    ...
    pub gas_objects: BTreeMap<MySoAddress, Vec<ObjectRef>>,
}
...
impl TestEnv {
    ...
    pub async fn fund_one_address_balance(&mut self, address: MySoAddress, amount: u64) {
        let gas = self.gas_objects[&address][0];
        let tx = TestTransactionBuilder::new(address, gas, self.rgp)
            .transfer_myso_to_address_balance(FundSource::coin(gas), vec![(amount, address)])
            .build();
    ...
    pub fn get_sender(&self, index: usize) -> MySoAddress {
        self.gas_objects.keys().copied().nth(index).unwrap()
    }

    pub fn get_sender_and_gas(&self, index: usize) -> (MySoAddress, ObjectRef) {
        let sender = self.get_sender(index);
        let gas = self.gas_objects[&sender][0];
        (sender, gas)
    }

    pub fn get_sender_and_all_gas(&self, index: usize) -> (MySoAddress, Vec<ObjectRef>) {
        let sender = self.get_sender(index);
        let gas = self.gas_objects[&sender].clone();
        (sender, gas)
    }

    pub fn tx_builder(&self, sender: MySoAddress) -> TestTransactionBuilder {
    ...
    pub fn encode_coin_reservation(
        ...
        let accumulator_obj_id = get_myso_accumulator_object_id(sender);
    ...
    pub async fn transfer_from_coin_to_address_balance(
        ...
        sender: MySoAddress,
        coin: ObjectRef,
        amounts_and_recipients: Vec<(u64, MySoAddress)>,
    ) -> MySoResult<(TransactionDigest, TransactionEffects)> {
        let tx = self
            .tx_builder(sender)
            .transfer_myso_to_address_balance(FundSource::coin(coin), amounts_and_recipients)
            .build();
    ...
    pub fn verify_accumulator_exists(&self, owner: MySoAddress, expected_balance: u64) {
        self.cluster.fullnode_handle.myso_node.with(|node| {
    ...
    pub fn get_myso_balance(&self, owner: MySoAddress) -> u64 {
        self.get_balance(owner, GAS::type_tag())
    }

    pub fn get_balance(&self, owner: MySoAddress, coin_type: TypeTag) -> u64 {
        let db_balance = self.cluster.fullnode_handle.myso_node.with({
    ...
    pub fn verify_accumulator_removed(&self, owner: MySoAddress) {
        self.cluster.fullnode_handle.myso_node.with(|node| {
            let state = node.state();
            let child_object_resolver = state.get_child_object_resolver().as_ref();
            let myso_coin_type = Balance::type_tag(GAS::type_tag());
            assert!(
                !AccumulatorValue::exists(child_object_resolver, None, owner, &myso_coin_type)
                    .unwrap(),
...
pub fn get_myso_accumulator_object_id(sender: MySoAddress) -> ObjectID {
...
pub fn verify_accumulator_exists(
    ...
    assert!(
        AccumulatorValue::exists(child_object_resolver, None, owner, &myso_coin_type).unwrap(),
        "Accumulator value should have been created"
    );

    let accumulator_object =
        AccumulatorValue::load_object(child_object_resolver, None, owner, &myso_coin_type)
            .expect("read cannot fail")
    ...
    let accumulator_value =
        AccumulatorValue::load(child_object_resolver, None, owner, &myso_coin_type)
            .expect("read cannot fail")
            .expect("accumulator should exist");
