...
use myso_swarm::memory::{Swarm, SwarmBuilder};
use myso_swarm_config::genesis_config::{
    AccountConfig, DEFAULT_GAS_AMOUNT, GenesisConfig, ValidatorGenesisConfig,
};
use myso_swarm_config::network_config::NetworkConfig;
use myso_swarm_config::network_config_builder::{
...
pub struct FullNodeHandle {
    pub myso_node: MySoNodeHandle,
    #[deprecated = "use grpc_client"]
    pub myso_client: MySoClient,
    #[deprecated = "use grpc_client"]
...
impl FullNodeHandle {
    pub async fn new(myso_node: MySoNodeHandle, json_rpc_address: SocketAddr) -> Self {
        let rpc_url = format!("http://{}", json_rpc_address);
        let rpc_client = HttpClientBuilder::default().build(&rpc_url).unwrap();

        let myso_client = MySoClientBuilder::default().build(&rpc_url).await.unwrap();
...
impl TestCluster {
    ...
    #[deprecated = "use grpc_client()"]
    pub fn myso_client(&self) -> &MySoClient {
        #[allow(deprecated)]
        &self.fullnode_handle.myso_client
    }
    ...
    pub fn get_myso_system_state(&self) -> MySoSystemState {
        self.fullnode_handle.myso_node.with(|node| {
            node.state()
                .get_myso_system_state_object_for_testing()
                .unwrap()
        })
    }
    ...
    pub fn all_node_handles(&self) -> Vec<MySoNodeHandle> {
        self.swarm
            .all_nodes()
    ...
    pub fn all_validator_handles(&self) -> Vec<MySoNodeHandle> {
        self.swarm
            .validator_nodes()
    ...
    pub async fn wait_for_run_with_range_shutdown_signal_with_timeout(
            ...
            tokio::select! {
                msg = shutdown_channel_rx.recv() =>
                {
                    match msg {
                        ...
                        Ok(None) => None,
                        Err(e) => {
                            error!("failed recv from myso-node shutdown channel: {}", e);
                            None
                        },
                    }
                },
        ...
        .await
        .expect("Timed out waiting for cluster to hit target epoch and recv shutdown signal from myso-node")
    }
    ...
    pub async fn wait_for_epoch(&self, target_epoch: Option<EpochId>) -> MySoSystemState {
    ...
    pub async fn wait_for_epoch_on_node(
        ...
        timeout(timeout_dur, async {
            ...
            if Some(epoch) == target_epoch {
                return handle.with(|node| node.state().get_myso_system_state_object_for_testing().unwrap());
            }
    ...
    pub async fn wait_for_epoch_with_timeout(
        ...
        self.wait_for_epoch_on_node(&self.fullnode_handle.myso_node, target_epoch, timeout_dur)
            .await
    }
    ...
    pub async fn sign_and_execute_transaction_directly(
        &self,
        tx_data: &TransactionData,
    ) -> MySoResult<(TransactionDigest, TransactionEffects)> {
        let mut res = self
    ...
    ) -> MySoResult<Vec<(TransactionDigest, TransactionEffects)>> {
    ...
    pub async fn execute_signed_txns_in_soft_bundle(
        ...
        for raw_result in result.results.iter() {
            let submit_result: myso_types::messages_grpc::SubmitTxResult =
                raw_result.clone().try_into()?;
            if let myso_types::messages_grpc::SubmitTxResult::Rejected { error } = submit_result {
                return Err(error);
            }
        }
    ...
    pub async fn execute_soft_bundle_with_conflicts(
        &self,
        signed_txs: &[Transaction],
    ) -> MySoResult<Vec<(TransactionDigest, WaitForEffectsResponse)>> {
        let digests: Vec<_> = signed_txs.iter().map(|tx| *tx.digest()).collect();
        ...
        let results: MySoResult<Vec<_>> = digests
            .into_iter()
    ...
    pub async fn fund_address_and_return_gas(
        ...
        let tx = context
            .sign_transaction(
                &TestTransactionBuilder::new(sender, gas, rgp)
                    .transfer_myso(amount, funding_address)
                    .build(),
            )
            .await;
    ...
    pub async fn transfer_myso_must_exceed(
        &self,
        sender: MySoAddress,
        ...
        let tx = self
            ...
            .await
            .transfer_myso(Some(amount), receiver)
            .build();
...
pub struct TestClusterBuilder {
    ...
    rpc_config: Option<myso_config::RpcConfig>,

    chain_override: Option<Chain>,

    execution_time_observer_config: Option<myso_config::node::ExecutionTimeObserverConfig>,

    state_sync_config: Option<myso_config::p2p::StateSyncConfig>,
...
impl TestClusterBuilder {
    ...
    pub fn with_state_sync_config(mut self, config: myso_config::p2p::StateSyncConfig) -> Self {
    ...
    pub fn with_execution_time_observer_config(
        mut self,
        config: myso_config::node::ExecutionTimeObserverConfig,
    ) -> Self {
    ...
    pub fn with_rpc_config(mut self, config: myso_config::RpcConfig) -> Self {
    ...
    pub async fn build(mut self) -> TestCluster {
        ...
        if !self.default_jwks {
            myso_node::set_jwk_injector(Arc::new(|_authority, provider| {
        ...
        let mut wallet_conf: MySoClientConfig =
            PersistedConfig::read(&working_dir.join(MYSO_CLIENT_CONFIG)).unwrap();
        wallet_conf.envs.push(MySoEnv {
        ...
        wallet_conf
            .persisted(&working_dir.join(MYSO_CLIENT_CONFIG))
            .save()
            .unwrap();

        let wallet_conf = swarm.dir().join(MYSO_CLIENT_CONFIG);
    ...
    async fn start_swarm(&mut self) -> Result<Swarm, anyhow::Error> {
        ...
        let network_path = dir.join(MYSO_NETWORK_CONFIG);
        let wallet_path = dir.join(MYSO_CLIENT_CONFIG);
        let keystore_path = dir.join(MYSO_KEYSTORE_FILENAME);
