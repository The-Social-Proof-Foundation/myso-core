...
pub const DATA_STORE_DIR: &str = ".myso_data_store";
...
impl FileSystemStore {
    ...
    pub fn base_path() -> Result<PathBuf, Error> {
        let home_dir = std::env::var("MYSO_DATA_STORE")
            .or_else(|_| std::env::var("MYSO_CONFIG_DIR"))
            .or_else(|_| std::env::var("HOME"))
            .or_else(|_| std::env::var("USERPROFILE"))
            .map_err(|_| {
                anyhow!(
                    "Cannot determine home directory. Define a MYSO_DATA_STORE environment variable"
                )
            })?;
