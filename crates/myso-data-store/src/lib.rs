...
//! Multi-tier caching data store for MySo blockchain data.
//!
//! This crate provides a flexible data store abstraction for retrieving and caching
//! MySo blockchain data (transactions, epochs, objects). The stores are loosely modeled
//! after the GQL schema in `crates/myso-indexer-alt-graphql/schema.graphql`.
//!
//! ## Core Traits
//!
...
/// The data provided to `myso_execution::executor::Executor::execute_transaction_to_effects`
...
/// `crates/myso-indexer-alt-graphql/schema.graphql::multiGetObjects`.
