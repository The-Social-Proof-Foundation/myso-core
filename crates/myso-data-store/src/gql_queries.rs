...
//! GQL Queries
//! Interface to the rpc for the gql schema defined in `crates\myso-indexer-alt-graphql/schema.graphql`.
...
pub(crate) mod txn_query {
    ...
    > {
        ...
        let effects: myso_types::effects::TransactionEffects = bcs::from_bytes(
            &CryptoBase64::decode(
                &effect_frag
                    .effects_bcs
...
pub(crate) mod object_query {
    ...
    impl From<GqlObjectKey> for ObjectKey {
        fn from(key: GqlObjectKey) -> Self {
            ObjectKey {
                address: MySoAddress(key.object_id.to_string()),
