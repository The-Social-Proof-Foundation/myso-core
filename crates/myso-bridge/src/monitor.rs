...
//! `BridgeMonitor` receives all `MySoBridgeEvent` and `EthBridgeEvent`
//! and handles them accordingly.

use crate::abi::{
    EthBridgeCommitteeEvents, EthBridgeConfigEvents, EthBridgeEvent, EthBridgeLimiterEvents,
    EthCommitteeUpgradeableContractEvents, EthMySoBridgeEvents,
};
...
pub struct BridgeMonitor<C> {
    ...
    myso_monitor_rx: mysten_metrics::metered_channel::Receiver<MySoBridgeEvent>,
    ...
    C: MySoClientInner + 'static,
{
    ...
    pub async fn run(self) {
        ...
        let mut latest_token_config = (*myso_token_type_tags.load().clone()).clone();

        loop {
            tokio::select! {
                myso_event = myso_monitor_rx.recv() => {
                    if let Some(myso_event) = myso_event {
                        Self::handle_myso_events(
                            myso_event,
                            &myso_client,
                            &bridge_auth_agg,
                            &bridge_paused_watch_tx,
                        ...
                        panic!("BridgeMonitor myso events channel was closed unexpectedly");
                    }
                }
    ...
    async fn handle_myso_events(
        event: MySoBridgeEvent,
        myso_client: &Arc<MySoClient<C>>,
        bridge_auth_agg: &Arc<ArcSwap<BridgeAuthorityAggregator>>,
        bridge_paused_watch_tx: &tokio::sync::watch::Sender<IsBridgePaused>,
        myso_token_type_tags: &Arc<ArcSwap<HashMap<u8, TypeTag>>>,
        ...
        info!("Received MySoBridgeEvent: {:?}", event);
        macro_rules! bump_myso_counter {
            ($action:expr) => {
                bridge_metrics
                    .observed_governance_actions
                    .with_label_values(&[$action, "myso"])
                    .inc();
            };
        }

        match event {
            MySoBridgeEvent::MySoToEthTokenBridgeV1(_) => (),
            MySoBridgeEvent::MySoToEthTokenBridgeV2(_) => (),
            MySoBridgeEvent::TokenTransferApproved(_) => (),
            MySoBridgeEvent::TokenTransferClaimed(_) => (),
            MySoBridgeEvent::TokenTransferAlreadyApproved(_) => (),
            MySoBridgeEvent::TokenTransferAlreadyClaimed(_) => (),
            MySoBridgeEvent::TokenTransferLimitExceed(_) => {
                // TODO do we want to do anything here?
            }

            MySoBridgeEvent::EmergencyOpEvent(event) => {
                bump_myso_counter!(if event.frozen {
                    "bridge_paused"
                } else {
                    "bridge_unpaused"
                });
            ...
            MySoBridgeEvent::CommitteeMemberRegistration(_) => {
                bump_myso_counter!("committee_member_registered");
            }
            MySoBridgeEvent::CommitteeUpdateEvent(_) => {
                bump_myso_counter!("committee_updated");
            }

            MySoBridgeEvent::CommitteeMemberUrlUpdateEvent(event) => {
                bump_myso_counter!("committee_member_url_updated");
            ...
            MySoBridgeEvent::BlocklistValidatorEvent(event) => {
                bump_myso_counter!(if event.blocklisted {
            ...
            MySoBridgeEvent::TokenRegistrationEvent(_) => {
                bump_myso_counter!("new_token_registered");
            }

            MySoBridgeEvent::NewTokenEvent(event) => {
                bump_myso_counter!("new_token_added");
                    ...
                    latest_token_config.entry(event.token_id)
                {
                    ...
                    myso_token_type_tags.store(Arc::new(latest_token_config.clone()));
                } else {
                    // invariant
            ...
            MySoBridgeEvent::UpdateTokenPriceEvent(_event) => {
                bump_myso_counter!("token_price_updated");
            }
    ...
    fn handle_eth_events(event: EthBridgeEvent, bridge_metrics: &Arc<BridgeMetrics>) {
        ...
        match event {
            ...
            EthBridgeEvent::EthMySoBridgeEvents(event) => match event {
                EthMySoBridgeEvents::TokensClaimed(_) => (),
                EthMySoBridgeEvents::TokensDeposited(_) => (),
                EthMySoBridgeEvents::TokensDepositedV2(_) => (),
                EthMySoBridgeEvents::Paused(_) => bump_eth_counter!("bridge_paused"),
                EthMySoBridgeEvents::Unpaused(_) => bump_eth_counter!("bridge_unpaused"),
                EthMySoBridgeEvents::Upgraded(_) => {
                    bump_eth_counter!("bridge_contract_upgraded")
                }
                EthMySoBridgeEvents::Initialized(_) => {
                    bump_eth_counter!("bridge_contract_initialized")
                }
                EthMySoBridgeEvents::ContractUpgraded(_) => {
                    bump_eth_counter!("bridge_contract_upgraded")
                }
                EthMySoBridgeEvents::EmergencyOperation(e) => {
                    if e.paused {
...
async fn get_latest_bridge_committee_with_url_update_event<C: MySoClientInner>(
    myso_client: Arc<MySoClient<C>>,
    ...
    loop {
        let Ok(Ok(committee)) = retry_with_max_elapsed_time!(
            myso_client.get_bridge_committee(),
...
async fn get_latest_bridge_committee_with_blocklist_event<C: MySoClientInner>(
    myso_client: Arc<MySoClient<C>>,
...
async fn get_latest_bridge_pause_status_with_emergency_event<C: MySoClientInner>(
    myso_client: Arc<MySoClient<C>>,
...
pub async fn subscribe_bridge_events(
    mut client: myso_rpc::Client,
    sender: mysten_metrics::metered_channel::Sender<MySoBridgeEvent>,
) {
    ...
    loop {
        ...
        while let Some(item) = subscription.next().await {
            ...
            for txn in checkpoint.checkpoint().transactions() {
                ...
                let Ok(events) = bcs_events.deserialize::<myso_types::effects::TransactionEvents>()
                else {
                    tracing::warn!(
                ...
                for event in events.data {
                    match MySoBridgeEvent::try_from_event(&event) {
                        ...
                        Err(e) => {
                            tracing::warn!(
                                "error deserializing MySoBridgeEvent from txn {txn_digest} in checkpoint {ckpt}: {e}"
                            );
                        }
                    }
                }
...
mod tests {
    ...
    async fn test_get_latest_bridge_committee_with_url_update_event() {
        ...
        let myso_client = Arc::new(MySoClient::new_for_testing(myso_client_mock.clone()));
        ...
        myso_client_mock.set_bridge_committee(summary.clone());
        ...
        myso_client_mock.set_bridge_committee(old_summary.clone());
        let timer = std::time::Instant::now();
        // update the url to "http://new.url" in 1 second
        let myso_client_mock_clone = myso_client_mock.clone();
        tokio::spawn(async move {
            tokio::time::sleep(Duration::from_secs(1)).await;
            myso_client_mock_clone.set_bridge_committee(summary.clone());
        });
        let committee = get_latest_bridge_committee_with_url_update_event(
            myso_client.clone(),
            event.clone(),
        ...
        myso_client_mock.set_bridge_committee(newer_summary.clone());
        let timer = std::time::Instant::now();
        let committee = get_latest_bridge_committee_with_url_update_event(
            myso_client.clone(),
            event.clone(),
    ...
    async fn test_get_bridge_pause_status_with_emergency_event() {
        ...
        myso_client_mock.set_is_bridge_paused(BRIDGE_PAUSED);
        let timer = std::time::Instant::now();
        assert!(
        ...
        myso_client_mock.set_is_bridge_paused(BRIDGE_UNPAUSED);
        let timer = std::time::Instant::now();
        assert!(
            !get_latest_bridge_pause_status_with_emergency_event(
                myso_client.clone(),
        ...
        tokio::spawn(async move {
            tokio::time::sleep(Duration::from_secs(1)).await;
            myso_client_mock_clone.set_is_bridge_paused(BRIDGE_UNPAUSED);
        });
        assert!(
            !get_latest_bridge_pause_status_with_emergency_event(
                myso_client.clone(),
    ...
    async fn test_update_bridge_authority_aggregation_with_url_change_event() {
        ...
        let myso_token_type_tags = Arc::new(ArcSwap::from(Arc::new(HashMap::new())));
        ...
        myso_client_mock.set_bridge_committee(new_committee_summary.clone());
        myso_monitor_tx
            .send(MySoBridgeEvent::CommitteeMemberUrlUpdateEvent(
    ...
    async fn test_update_bridge_authority_aggregation_with_blocklist_event() {
        ...
        myso_client_mock.set_bridge_committee(new_committee_summary.clone());
        myso_monitor_tx
            .send(MySoBridgeEvent::BlocklistValidatorEvent(
    ...
    async fn test_update_bridge_pause_status_with_emergency_event() {
        ...
        myso_client_mock.set_is_bridge_paused(event.frozen);
        myso_monitor_tx
            .send(MySoBridgeEvent::EmergencyOpEvent(event.clone()))
            .await
            .unwrap();
        // Wait for the monitor to process the event
        tokio::time::sleep(Duration::from_secs(1)).await;
    ...
    #[tokio::test]
    async fn test_update_myso_token_type_tags() {
        let (
            myso_monitor_tx,
            myso_monitor_rx,
            _eth_monitor_tx,
        ...
        let myso_token_type_tags = Arc::new(ArcSwap::from(Arc::new(HashMap::new())));
        let myso_token_type_tags_clone = myso_token_type_tags.clone();
        let _handle = tokio::task::spawn(
            BridgeMonitor::new(
                myso_client.clone(),
                myso_monitor_rx,
        ...
        myso_monitor_tx
            .send(MySoBridgeEvent::NewTokenEvent(event.clone()))
            .await
            .unwrap();
        // Wait for the monitor to process the event
        tokio::time::sleep(Duration::from_secs(1)).await;
        // Now expect new token type tags to appear in myso_token_type_tags
        myso_token_type_tags
            .load()
            .clone()
            .get(&event.token_id)
    ...
    fn setup() -> (
        mysten_metrics::metered_channel::Sender<MySoBridgeEvent>,
        mysten_metrics::metered_channel::Receiver<MySoBridgeEvent>,
        mysten_metrics::metered_channel::Sender<EthBridgeEvent>,
        mysten_metrics::metered_channel::Receiver<EthBridgeEvent>,
        MySoMockClient,
        Arc<MySoClient<MySoMockClient>>,
        ...
        let (myso_monitor_tx, myso_monitor_rx) = mysten_metrics::metered_channel::channel(
            ...
            &mysten_metrics::get_metrics()
                .unwrap()
                .channel_inflight
                .with_label_values(&["myso_monitor_queue"]),
        );
