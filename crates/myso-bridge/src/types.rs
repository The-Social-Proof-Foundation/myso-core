...
use crate::abi::{EthToMySoTokenBridgeV1, EthToMySoTokenBridgeV2};
...
use myso_types::bridge::{
    APPROVAL_THRESHOLD_ADD_TOKENS_ON_EVM, APPROVAL_THRESHOLD_ADD_TOKENS_ON_MYSO,
...
pub struct MySoToEthBridgeAction {
    // Digest of the transaction where the event was emitted
    pub myso_tx_digest: TransactionDigest,
    // The index of the event in the transaction
    pub myso_tx_event_index: u16,
    pub myso_bridge_event: EmittedMySoToEthTokenBridgeV1,
}

#[derive(Clone, Debug, Serialize, Deserialize, PartialEq, Eq, Hash)]
pub struct MySoToEthTokenTransfer {
    pub nonce: u64,
    ...
    pub token_id: u8,
    // The amount of tokens deposited with decimal points on MySo side
    pub amount_adjusted: u64,
}

#[derive(Clone, Debug, Serialize, Deserialize, PartialEq, Eq, Hash)]
pub struct MySoToEthTokenTransferV2 {
    pub nonce: u64,
    pub myso_chain_id: BridgeChainId,
    pub eth_chain_id: BridgeChainId,
    pub myso_address: MySoAddress,
...
#[derive(Clone, Debug, Serialize, Deserialize, PartialEq, Eq, Hash)]
pub struct EthToMySoBridgeAction {
    // Digest of the transaction where the event was emitted
    pub eth_tx_hash: EthTransactionHash,
    // The index of the event in the transaction
    pub eth_event_index: u16,
    pub eth_bridge_event: EthToMySoTokenBridgeV1,
}

#[derive(Clone, Debug, Serialize, Deserialize, PartialEq, Eq, Hash)]
pub struct EthToMySoTokenTransferV2 {
    // Digest of the transaction where the event was emitted
    pub eth_tx_hash: EthTransactionHash,
    // The index of the event in the transaction
    pub eth_event_index: u16,
    pub eth_bridge_event: EthToMySoTokenBridgeV2,
}
...
pub struct LimitUpdateAction {
    ...
    // for the limit update. For example, if chain_id is EthMainnet and sending_chain_id is MySoMainnet,
    // it means we want to update the limit for the MySoMainnet to EthMainnet route.
    pub chain_id: BridgeChainId,
...
#[derive(Clone, Debug, Serialize, Deserialize, PartialEq, Eq, Hash)]
pub struct AddTokensOnMySoAction {
    pub nonce: u64,
    pub chain_id: BridgeChainId,
    pub native: bool,
...
pub struct AddTokensOnEvmAction {
    ...
    pub token_myso_decimals: Vec<u8>,
    pub token_prices: Vec<u64>,
}
...
pub enum BridgeAction {
    /// MySo to Eth bridge action
    MySoToEthBridgeAction(MySoToEthBridgeAction),
    /// Eth to myso bridge action
    EthToMySoBridgeAction(EthToMySoBridgeAction),
    ...
    AddTokensOnMySoAction(AddTokensOnMySoAction),
    AddTokensOnEvmAction(AddTokensOnEvmAction),
    /// MySo to Eth bridge action
    MySoToEthTokenTransfer(MySoToEthTokenTransfer),
    /// MySo to Eth bridge action V2
    MySoToEthTokenTransferV2(MySoToEthTokenTransferV2),
    // /// Eth to myso bridge action V2
    EthToMySoTokenTransferV2(EthToMySoTokenTransferV2),
}
...
impl BridgeAction {
    ...
    pub fn chain_id(&self) -> BridgeChainId {
        match self {
            BridgeAction::MySoToEthBridgeAction(a) => a.myso_bridge_event.myso_chain_id,
            BridgeAction::MySoToEthTokenTransfer(a) => a.myso_chain_id,
            BridgeAction::MySoToEthTokenTransferV2(a) => a.myso_chain_id,
            BridgeAction::EthToMySoBridgeAction(a) => a.eth_bridge_event.eth_chain_id,
            BridgeAction::EthToMySoTokenTransferV2(a) => a.eth_bridge_event.eth_chain_id,
            ...
            BridgeAction::AddTokensOnMySoAction(a) => a.chain_id,
    ...
    pub fn is_governance_action(&self) -> bool {
        match self.action_type() {
            ...
            BridgeActionType::AddTokensOnMySo => true,
    ...
    pub fn action_type(&self) -> BridgeActionType {
        match self {
            BridgeAction::MySoToEthBridgeAction(_) => BridgeActionType::TokenTransfer,
            BridgeAction::MySoToEthTokenTransfer(_) => BridgeActionType::TokenTransfer,
            BridgeAction::MySoToEthTokenTransferV2(_) => BridgeActionType::TokenTransfer,
            BridgeAction::EthToMySoBridgeAction(_) => BridgeActionType::TokenTransfer,
            BridgeAction::EthToMySoTokenTransferV2(_) => BridgeActionType::TokenTransfer,
            ...
            BridgeAction::AddTokensOnMySoAction(_) => BridgeActionType::AddTokensOnMySo,
    ...
    pub fn seq_number(&self) -> u64 {
        match self {
            BridgeAction::MySoToEthBridgeAction(a) => a.myso_bridge_event.nonce,
            BridgeAction::MySoToEthTokenTransfer(a) => a.nonce,
            BridgeAction::MySoToEthTokenTransferV2(a) => a.nonce,
            BridgeAction::EthToMySoBridgeAction(a) => a.eth_bridge_event.nonce,
            BridgeAction::EthToMySoTokenTransferV2(a) => a.eth_bridge_event.nonce,
            ...
            BridgeAction::AddTokensOnMySoAction(a) => a.nonce,
    ...
    pub fn approval_threshold(&self) -> u64 {
        match self {
            BridgeAction::MySoToEthBridgeAction(_) => APPROVAL_THRESHOLD_TOKEN_TRANSFER,
            BridgeAction::MySoToEthTokenTransfer(_) => APPROVAL_THRESHOLD_TOKEN_TRANSFER,
            BridgeAction::MySoToEthTokenTransferV2(_) => APPROVAL_THRESHOLD_TOKEN_TRANSFER,
            BridgeAction::EthToMySoBridgeAction(_) => APPROVAL_THRESHOLD_TOKEN_TRANSFER,
            BridgeAction::EthToMySoTokenTransferV2(_) => APPROVAL_THRESHOLD_TOKEN_TRANSFER,
            ...
            BridgeAction::AddTokensOnMySoAction(_) => APPROVAL_THRESHOLD_ADD_TOKENS_ON_MYSO,
    ...
    pub fn update_to_token_transfer(self) -> Self {
        match self {
            BridgeAction::MySoToEthBridgeAction(a) => {
                BridgeAction::MySoToEthTokenTransfer(MySoToEthTokenTransfer {
                    nonce: a.myso_bridge_event.nonce,
                    myso_chain_id: a.myso_bridge_event.myso_chain_id,
                    eth_chain_id: a.myso_bridge_event.eth_chain_id,
                    myso_address: a.myso_bridge_event.myso_address,
                    eth_address: a.myso_bridge_event.eth_address,
                    token_id: a.myso_bridge_event.token_id,
                    amount_adjusted: a.myso_bridge_event.amount_myso_adjusted,
                })
            }
            BridgeAction::EthToMySoBridgeAction(_) => self,
            BridgeAction::EthToMySoTokenTransferV2(_) => self,
            ...
            BridgeAction::AddTokensOnMySoAction(_) => self,
            BridgeAction::AddTokensOnEvmAction(_) => self,
            BridgeAction::MySoToEthTokenTransfer(_) => self,
            BridgeAction::MySoToEthTokenTransferV2(_) => self,
        }
    }
    ...
    pub fn try_from_bridge_record(record: &MoveTypeBridgeRecord) -> BridgeResult<Self> {
        ...
        struct MySoToEthOnChainBcsPayload {
            myso_address: Vec<u8>,
            target_chain: u8,
            eth_address: Vec<u8>,
            token_type: u8,
        ...
        #[derive(Debug, Deserialize)]
        struct MySoToEthOnChainBcsPayloadV2 {
            myso_address: Vec<u8>,
            target_chain: u8,
            eth_address: Vec<u8>,
            token_type: u8,
        ...
        match *message_version {
            ...
            crate::encoding::TOKEN_TRANSFER_MESSAGE_VERSION_V2 => {
                ...
                Ok(BridgeAction::MySoToEthTokenTransferV2(
                    MySoToEthTokenTransferV2 {
                        nonce: *seq_num,
                        myso_chain_id: BridgeChainId::try_from(*source_chain)?,
                        eth_chain_id: BridgeChainId::try_from(payload.target_chain)?,
                        myso_address: MySoAddress::from_bytes(payload.myso_address)?,
...
pub fn is_route_valid(one: BridgeChainId, other: BridgeChainId) -> bool {
    if one.is_myso_chain() && other.is_myso_chain() {
        return false;
    }
    if !one.is_myso_chain() && !other.is_myso_chain() {
        return false;
    ...
    if one == BridgeChainId::EthMainnet {
        return other == BridgeChainId::MySoMainnet;
    }
    if one == BridgeChainId::MySoMainnet {
        return other == BridgeChainId::EthMainnet;
    }
    if other == BridgeChainId::EthMainnet {
        return one == BridgeChainId::MySoMainnet;
    }
    if other == BridgeChainId::MySoMainnet {
...
pub struct MySoEvents {
    pub transaction_digest: TransactionDigest,
    pub checkpoint: Option<u64>,
    pub timestamp_ms: Option<u64>,
    pub events: Vec<myso_json_rpc_types::MySoEvent>,
}

#[cfg(test)]
mod tests {
    use crate::test_utils::get_test_authority_and_key;
    use crate::test_utils::get_test_eth_to_myso_bridge_action;
    ...
    fn test_bridge_action_approval_threshold_regression_test() -> anyhow::Result<()> {
        let action = get_test_myso_to_eth_bridge_action(None, None, None, None, None, None, None);
        assert_eq!(action.approval_threshold(), 3334);

        let action = get_test_eth_to_myso_bridge_action(None, None, None, None);
        assert_eq!(action.approval_threshold(), 3334);
        ...
        let action = BridgeAction::LimitUpdateAction(LimitUpdateAction {
            nonce: 15,
            chain_id: BridgeChainId::MySoCustom,
