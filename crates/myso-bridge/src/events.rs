...
//! This file contains the definition of the MySoBridgeEvent enum, of
...
use crate::types::MySoToEthTokenTransfer;
use crate::types::MySoToEthTokenTransferV2;
...
use myso_types::bridge::MoveTypeCommitteeMemberRegistration;
...
impl TryFrom<MoveTokenRegistrationEvent> for TokenRegistrationEvent {
    ...
    fn try_from(event: MoveTokenRegistrationEvent) -> BridgeResult<Self> {
        let type_name = parse_myso_type_tag(&format!("0x{}", event.type_name)).map_err(|e| {
...
#[derive(Debug, Serialize, Deserialize, PartialEq, Eq, Clone, Hash)]
pub struct EmittedMySoToEthTokenBridgeV1 {
    pub nonce: u64,
...
#[derive(Debug, Serialize, Deserialize, PartialEq, Eq, Clone, Hash)]
pub struct EmittedMySoToEthTokenBridgeV2 {
    pub nonce: u64,
    pub myso_chain_id: BridgeChainId,
    pub eth_chain_id: BridgeChainId,
    pub myso_address: MySoAddress,
...
impl TryFrom<MoveTokenDepositedEvent> for EmittedMySoToEthTokenBridgeV1 {
    type Error = BridgeError;

    fn try_from(event: MoveTokenDepositedEvent) -> BridgeResult<Self> {
        if event.amount_myso_adjusted == 0 {
            return Err(BridgeError::ZeroValueBridgeTransfer(format!(
                "Failed to convert MoveTokenDepositedEvent to EmittedMySoToEthTokenBridgeV1. Manual intervention is required. 0 value transfer should not be allowed in Move: {:?}",
                event,
            )));
        }

        let token_id = event.token_type;
        let myso_chain_id = BridgeChainId::try_from(event.source_chain).map_err(|_e| {
            BridgeError::Generic(format!(
                "Failed to convert MoveTokenDepositedEvent to EmittedMySoToEthTokenBridgeV1. Failed to convert source chain {} to BridgeChainId",
                event.token_type,
            ))
        })?;
        let eth_chain_id = BridgeChainId::try_from(event.target_chain).map_err(|_e| {
            BridgeError::Generic(format!(
                "Failed to convert MoveTokenDepositedEvent to EmittedMySoToEthTokenBridgeV1. Failed to convert target chain {} to BridgeChainId",
                event.token_type,
            ))
        })?;
        if !myso_chain_id.is_myso_chain() {
            return Err(BridgeError::Generic(format!(
                "Failed to convert MoveTokenDepositedEvent to EmittedMySoToEthTokenBridgeV1. Invalid source chain {}",
                event.source_chain
            )));
        }
        if eth_chain_id.is_myso_chain() {
            return Err(BridgeError::Generic(format!(
                "Failed to convert MoveTokenDepositedEvent to EmittedMySoToEthTokenBridgeV1. Invalid target chain {}",
                event.target_chain
            )));
        }

        let myso_address = MySoAddress::from_bytes(event.sender_address)
            .map_err(|e| BridgeError::Generic(format!("Failed to convert MoveTokenDepositedEvent to EmittedMySoToEthTokenBridgeV1. Failed to convert sender_address to MySoAddress: {:?}", e)))?;
        ...
        Ok(Self {
            ...
            eth_address,
            token_id,
            amount_myso_adjusted: event.amount_myso_adjusted,
        })
    }
}

impl TryFrom<MoveTokenDepositedEventV2> for EmittedMySoToEthTokenBridgeV2 {
    type Error = BridgeError;
...
crate::declare_events!(
    MySoToEthTokenBridgeV1(EmittedMySoToEthTokenBridgeV1) => ("bridge::TokenDepositedEvent", MoveTokenDepositedEvent),
...
macro_rules! declare_events {
    ($($variant:ident($type:path) => ($event_tag:expr, $event_struct:path)),* $(,)?) => {
        ...
        // Try to convert a MySoEvent into MySoBridgeEvent
        impl MySoBridgeEvent {
            pub fn try_from_myso_event(event: &MySoEvent) -> BridgeResult<Option<MySoBridgeEvent>> {
                ...
                $(
                    if &event.type_ == $variant.get().unwrap() {
                        ...
                        return Ok(Some(MySoBridgeEvent::$variant(event_struct.try_into()?)));
                    }
                )*
                Ok(None)
            }

            pub fn try_from_event(event: &Event) -> BridgeResult<Option<MySoBridgeEvent>> {
                init_all_struct_tags(); // Ensure all tags are initialized
    ...
    pub fn try_into_bridge_action(self) -> Option<BridgeAction> {
        match self {
            MySoBridgeEvent::MySoToEthTokenBridgeV1(event) => {
                let EmittedMySoToEthTokenBridgeV1 {
                    nonce,
                    myso_chain_id,
                    eth_chain_id,
                    myso_address,
            ...
            MySoBridgeEvent::MySoToEthTokenBridgeV2(event) => Some(
                BridgeAction::MySoToEthTokenTransferV2(MySoToEthTokenTransferV2 {
                    nonce: event.nonce,
                    myso_chain_id: event.myso_chain_id,
                    eth_chain_id: event.eth_chain_id,
                    myso_address: event.myso_address,
                    eth_address: event.eth_address,
                    token_id: event.token_id,
                    amount_adjusted: event.amount_myso_adjusted,
                    timestamp_ms: event.timestamp_ms,
                }),
            ),
            MySoBridgeEvent::TokenTransferApproved(_event) => None,
            MySoBridgeEvent::TokenTransferClaimed(_event) => None,
            MySoBridgeEvent::TokenTransferAlreadyApproved(_event) => None,
            MySoBridgeEvent::TokenTransferAlreadyClaimed(_event) => None,
            MySoBridgeEvent::TokenTransferLimitExceed(_event) => None,
            MySoBridgeEvent::EmergencyOpEvent(_event) => None,
            MySoBridgeEvent::CommitteeMemberRegistration(_event) => None,
            MySoBridgeEvent::CommitteeUpdateEvent(_event) => None,
            MySoBridgeEvent::CommitteeMemberUrlUpdateEvent(_event) => None,
            MySoBridgeEvent::BlocklistValidatorEvent(_event) => None,
            MySoBridgeEvent::TokenRegistrationEvent(_event) => None,
...
pub mod tests {
    ...
    use crate::types::BridgeAction;
    use crate::types::MySoToEthBridgeAction;
    use alloy::primitives::Address as EthAddress;
    ...
    /// Returns a test MySoEvent and corresponding BridgeAction
    pub fn get_test_myso_event_and_action(identifier: Identifier) -> (MySoEvent, BridgeAction) {
        init_all_struct_tags(); // Ensure all tags are initialized
        let sanitized_event = EmittedMySoToEthTokenBridgeV1 {
            nonce: 1,
            myso_chain_id: BridgeChainId::MySoTestnet,
            myso_address: MySoAddress::random_for_testing_only(),
            eth_chain_id: BridgeChainId::EthSepolia,
            eth_address: EthAddress::random(),
            token_id: TOKEN_ID_MYSO,
            amount_myso_adjusted: 100,
        };
        let emitted_event = MoveTokenDepositedEvent {
            seq_num: sanitized_event.nonce,
            source_chain: sanitized_event.myso_chain_id as u8,
            sender_address: sanitized_event.myso_address.to_vec(),
            ...
            amount_myso_adjusted: sanitized_event.amount_myso_adjusted,
        };

        let tx_digest = TransactionDigest::random();
        let event_idx = 10u16;
        let bridge_action = BridgeAction::MySoToEthBridgeAction(MySoToEthBridgeAction {
            myso_tx_digest: tx_digest,
            myso_tx_event_index: event_idx,
            myso_bridge_event: sanitized_event.clone(),
        });
        let event = MySoEvent {
            type_: MySoToEthTokenBridgeV1.get().unwrap().clone(),
            ...
            sender: MySoAddress::random_for_testing_only(),
    ...
    async fn test_bridge_events_when_init() {
        ...
        for event in events.iter() {
            match MySoBridgeEvent::try_from_event(event).unwrap().unwrap() {
                MySoBridgeEvent::CommitteeMemberRegistration(_event) => mask |= 0x1,
                MySoBridgeEvent::CommitteeUpdateEvent(_event) => mask |= 0x2,
                MySoBridgeEvent::TokenRegistrationEvent(_event) => mask |= 0x4,
                MySoBridgeEvent::NewTokenEvent(_event) => mask |= 0x8,
    ...
    #[test]
    fn test_0_myso_amount_conversion_for_myso_event() {
        let emitted_event = MoveTokenDepositedEvent {
            seq_num: 1,
            source_chain: BridgeChainId::MySoTestnet as u8,
            ...
            token_type: TOKEN_ID_MYSO,
            amount_myso_adjusted: 0,
        };
        match EmittedMySoToEthTokenBridgeV1::try_from(emitted_event).unwrap_err() {
