...
pub const ETH_TO_MYSO_TX_PATH: &str = "/sign/bridge_tx/eth/myso/{tx_hash}/{event_index}";
pub const MYSO_TO_ETH_TX_PATH: &str = "/sign/bridge_tx/myso/eth/{tx_digest}/{event_index}";
pub const MYSO_TO_ETH_TRANSFER_PATH: &str =
...
pub const ADD_TOKENS_ON_MYSO_PATH: &str = "/sign/add_tokens_on_myso/{chain_id}/{nonce}/{native}/{token_ids}/{token_type_names}/{token_prices}";
pub const ADD_TOKENS_ON_EVM_PATH: &str = "/sign/add_tokens_on_evm/{chain_id}/{nonce}/{native}/{token_ids}/{token_addresses}/{token_myso_decimals}/{token_prices}";
...
pub(crate) fn make_router(
    ...
    let main_router = Router::new()
        ...
        .route(ETH_TO_MYSO_TX_PATH, get(handle_eth_tx_hash))
        .route(MYSO_TO_ETH_TX_PATH, get(handle_myso_tx_digest))
        .route(MYSO_TO_ETH_TRANSFER_PATH, get(handle_myso_token_transfer))
        .route(
...
async fn handle_myso_tx_digest(
    ...
    let future = async {
        let sig: Json<SignedBridgeAction> = handler
            .handle_myso_tx_digest(tx_digest_base58, event_idx)
            .await?;
        Ok(sig)
    };
    with_metrics!(metrics.clone(), "handle_myso_tx_digest", future).await
}
...
async fn handle_myso_token_transfer(
    ...
    let future = async {
        let sig: Json<SignedBridgeAction> = handler
            .handle_myso_token_transfer(source_chain, message_type, bridge_seq_num)
            .await?;
        Ok(sig)
    };
    with_metrics!(metrics.clone(), "handle_myso_token_transfer", future).await
}
...
async fn handle_add_tokens_on_myso(
    ...
    let future = async {
        ...
        if !chain_id.is_myso_chain() {
            return Err(BridgeError::InvalidBridgeClientRequest(
                "handle_add_tokens_on_myso only expects MySo chain id".to_string(),
            ));
        }
    ...
    with_metrics!(metrics.clone(), "handle_add_tokens_on_myso", future).await
}

#[instrument(level = "error", skip_all, fields(chain_id=chain_id, nonce=nonce, native=native, token_ids=token_ids, token_addresses=token_addresses, token_myso_decimals=token_myso_decimals, token_prices=token_prices))]
async fn handle_add_tokens_on_evm(
    Path((chain_id, nonce, native, token_ids, token_addresses, token_myso_decimals, token_prices)): Path<(
        u8,
        u64,
        u8,
        String,
    ...
    let future = async {
        ...
        if chain_id.is_myso_chain() {
            return Err(BridgeError::InvalidBridgeClientRequest(
                "handle_add_tokens_on_evm does not expect MySo chain id".to_string(),
            ));
        }
        ...
        validate_list_size(&token_myso_decimals, "token_myso_decimals")?;
        validate_list_size(&token_prices, "token_prices")?;
        ...
        let token_addresses = token_addresses
            ...
            .collect::<Result<Vec<_>, _>>()?;
        let token_myso_decimals = token_myso_decimals
            .split(',')
            .map(|s| {
                s.parse::<u8>().map_err(|err| {
                    BridgeError::InvalidBridgeClientRequest(format!(
                        "Invalid token myso decimals: {:?}",
                        err
                    ))
                })
            })
...
mod tests {
    ...
    #[tokio::test]
    async fn test_bridge_server_handle_add_tokens_on_myso_action_path() {
        let client = setup();
