...
use myso_types::MYSO_BRIDGE_OBJECT_ID;
...
use myso_types::bridge::{BridgeTrait, BridgeTreasurySummary};
use myso_types::bridge::{MoveTypeBridgeMessage, MoveTypeParsedTokenTransferMessage};
use myso_types::bridge::{
    MoveTypeCommitteeMember, MoveTypeTokenTransferPayload, MoveTypeTokenTransferPayloadV2,
};
use myso_types::collection_types::LinkedTableNode;
...
pub struct MySoClient<P> {
    inner: P,
    bridge_metrics: Arc<BridgeMetrics>,
}

pub type MySoBridgeClient = MySoClient<MySoClientInternal>;

pub struct MySoClientInternal {
    grpc_client: myso_rpc_api::Client,
}

#[derive(Clone, Debug)]
pub struct ExecuteTransactionResult {
    pub status: MySoExecutionStatus,
    pub events: Vec<MySoEvent>,
}

impl MySoBridgeClient {
    pub async fn new(rpc_url: &str, bridge_metrics: Arc<BridgeMetrics>) -> anyhow::Result<Self> {
        let grpc_client = myso_rpc_api::Client::new(rpc_url)?;
        let inner = MySoClientInternal { grpc_client };
        let self_ = Self {
            inner,
            bridge_metrics,
        };
    ...
    pub fn grpc_client(&self) -> &myso_rpc_api::Client {
    ...
    P: MySoClientInner,
{
    ...
    async fn describe(&self) -> Result<(), BridgeError> {
        ...
        tracing::info!(
            "MySoClient is connected to chain {chain_id}, current block number: {block_number}"
        );
        Ok(())
    }
    ...
    pub async fn get_bridge_action_by_tx_digest_and_event_idx_maybe(
        ...
        if event.type_.address.as_ref() != BRIDGE_PACKAGE_ID.as_ref() {
            return Err(BridgeError::BridgeEventInUnrecognizedMySoPackage);
        }
        let bridge_event = MySoBridgeEvent::try_from_myso_event(event)?
            .ok_or(BridgeError::NoBridgeEventsInTxPosition)?;
    ...
    pub async fn get_token_id_map(&self) -> BridgeResult<HashMap<u8, TypeTag>> {
        self.get_bridge_summary()
            ...
            .into_iter()
            .map(|(id, name)| {
                parse_myso_type_tag(&format!("0x{name}"))
                    .map(|name| (id, name))
                    .map_err(|e| {
        ...
        for (_, member) in move_type_bridge_committee.members {
            ...
            let base_url = from_utf8(&http_rest_url).unwrap_or_else(|_e| {
                warn!(
                    ...
                    myso_address, bridge_pubkey_bytes, http_rest_url
                );
                ""
            });
    ...
    pub async fn execute_transaction_block_with_effects(
        &self,
        tx: myso_types::transaction::Transaction,
    ...
    pub async fn get_sequence_number_from_event_id(
        ...
        let bridge_event = match MySoBridgeEvent::try_from_myso_event(event)? {
            Some(e) => e,
            None => return Ok(None),
        };

        match bridge_event {
            MySoBridgeEvent::MySoToEthTokenBridgeV1(event) => Ok(Some(event.nonce)),
            _ => Ok(None),
        }
    }
}

/// Use a trait to abstract over the MySoSDKClient and MySoMockClient for testing.
#[async_trait]
pub trait MySoClientInner: Send + Sync {
    async fn get_events_by_tx_digest(
        &self,
        tx_digest: TransactionDigest,
    ) -> Result<MySoEvents, BridgeError>;
...
#[async_trait]
impl MySoClientInner for myso_rpc_api::Client {
    async fn get_events_by_tx_digest(
        &self,
        tx_digest: TransactionDigest,
    ) -> Result<MySoEvents, BridgeError> {
        ...
        Ok(MySoEvents {
            ...
            timestamp_ms: resp
                .timestamp_opt()
                .map(|timestamp| myso_rpc::proto::proto_to_timestamp_ms(*timestamp))
                .transpose()?,
            events: resp
                .events()
                .events()
    ...
    async fn get_chain_identifier(&self) -> Result<String, BridgeError> {
        let chain_id = self
            ...
            .into_inner()
            .chain_id()
            .parse::<myso_types::digests::CheckpointDigest>()?;

        Ok(myso_types::digests::ChainIdentifier::from(chain_id).to_string())
    }

    async fn get_reference_gas_price(&self) -> Result<u64, BridgeError> {
        let mut client = self.clone();
        myso_rpc::Client::get_reference_gas_price(client.inner_mut())
            .await
            .map_err(Into::into)
    }
    ...
    async fn execute_transaction_block_with_effects(
        ...
        use myso_rpc::proto::myso::rpc::v2::ExecutedTransaction as ProtoExecutedTransaction;
        use myso_sdk_types::SignedTransaction;

        let signed_tx: SignedTransaction = tx.try_into().map_err(|e| {
            BridgeError::MySoTxFailureGeneric(format!("Failed to convert transaction: {:?}", e))
        })?;
        ...
        let myso_status = if status.success() {
            ...
            MySoExecutionStatus::Failure { error: failure_msg }
        };

        let myso_events: Vec<MySoEvent> = executed_tx
            .events()
            .events()
            .iter()
            .filter_map(|event| {
                ...
                let sender: myso_types::base_types::MySoAddress = event.sender().parse().ok()?;

                let event_type_tag: myso_types::TypeTag =
                    parse_myso_type_tag(event.event_type()).ok()?;
                let struct_tag: StructTag = match event_type_tag {
                    myso_types::TypeTag::Struct(s) => *s,
                    _ => return None,
                };
    ...
    async fn get_bridge_record(
        ...
        let record_id = {
            ...
            let key_type = myso_sdk_types::StructTag::new(
                Address::from(BRIDGE_PACKAGE_ID),
                myso_sdk_types::Identifier::from_static("message"),
                myso_sdk_types::Identifier::from_static("BridgeMessageKey"),
                vec![],
            );
        ...
        let field: myso_types::dynamic_field::Field<
    ...
    async fn get_gas_data_panic_if_not_gas(
        ...
        loop {
            let result = async {
                ...
                let obj = resp.object();
                let object: myso_types::object::Object = obj.bcs().deserialize().map_err(|e| {
...
#[async_trait]
impl MySoClientInner for MySoClientInternal {
    async fn get_events_by_tx_digest(
        &self,
        tx_digest: TransactionDigest,
    ) -> Result<MySoEvents, BridgeError> {
    ...
    async fn get_chain_identifier(&self) -> Result<String, BridgeError> {
        MySoClientInner::get_chain_identifier(&self.grpc_client).await
    }

    async fn get_reference_gas_price(&self) -> Result<u64, BridgeError> {
        MySoClientInner::get_reference_gas_price(&self.grpc_client).await
    }
...
mod tests {
    ...
    async fn get_bridge_action_by_tx_digest_and_event_idx_maybe() {
        ...
        let mock_client = MySoMockClient::default();
        let myso_client = MySoClient::new_for_testing(mock_client.clone());
        let tx_digest = TransactionDigest::random();

        // Ensure all struct tags are inited
        init_all_struct_tags();

        let sanitized_event_1 = EmittedMySoToEthTokenBridgeV1 {
            nonce: 1,
        ...
        let emitted_event_1 = MoveTokenDepositedEvent {
            ...
            source_chain: sanitized_event_1.myso_chain_id as u8,
            sender_address: sanitized_event_1.myso_address.to_vec(),
            ...
            amount_myso_adjusted: sanitized_event_1.amount_myso_adjusted,
        };
        ...
        myso_event_2.type_.module = Identifier::from_str("unrecognized_module").unwrap();
        myso_event_2.bcs = BcsEvent::new(bcs::to_bytes(&event_2).unwrap());

        // Event 3 is defined in non-bridge package
        let mut myso_event_3 = myso_event_1.clone();
        myso_event_3.type_.address = AccountAddress::random();
        ...
        let expected_action = BridgeAction::MySoToEthTokenTransfer(MySoToEthTokenTransfer {
            nonce: sanitized_event_1.nonce,
            myso_chain_id: sanitized_event_1.myso_chain_id,
            eth_chain_id: sanitized_event_1.eth_chain_id,
            myso_address: sanitized_event_1.myso_address,
            eth_address: sanitized_event_1.eth_address,
            token_id: sanitized_event_1.token_id,
            amount_adjusted: sanitized_event_1.amount_myso_adjusted,
        });
        assert_eq!(
            myso_client
        ...
        assert!(matches!(
            myso_client
                ...
                .await
                .unwrap_err(),
            BridgeError::BridgeEventInUnrecognizedMySoPackage
        ),);
        assert!(matches!(
            myso_client
        ...
        myso_event_2.type_ = MySoToEthTokenBridgeV1.get().unwrap().clone();
        mock_client.add_events_by_tx_digest(tx_digest, vec![myso_event_2]);
        myso_client
            .get_bridge_action_by_tx_digest_and_event_idx_maybe(&tx_digest, 2)
            .await
            .unwrap_err();
    ...
    async fn test_get_action_onchain_status_for_myso_to_eth_transfer() {
        telemetry_subscribers::init_for_testing();
        ...
        let myso_client =
            MySoClient::new(&test_cluster.inner.fullnode_handle.rpc_url, bridge_metrics)
                .await
                .unwrap();
        ...
        // 2. Create a MySo -> Eth Transfer, approve with validator secrets and assert its status to be Approved
        ...
        assert_eq!(bridge_event.myso_chain_id, BridgeChainId::MySoCustom);
        assert_eq!(bridge_event.eth_chain_id, BridgeChainId::EthCustom);
        assert_eq!(bridge_event.eth_address, eth_recv_address);
        assert_eq!(bridge_event.myso_address, sender);
        assert_eq!(bridge_event.token_id, TOKEN_ID_USDC);
        assert_eq!(bridge_event.amount_myso_adjusted, usdc_amount);

        let action = get_test_myso_to_eth_bridge_action(
            None,
            None,
            Some(bridge_event.nonce),
            Some(bridge_event.amount_myso_adjusted),
            Some(bridge_event.myso_address),
        ...
        let action =
            get_test_myso_to_eth_bridge_action(None, None, Some(100), None, None, None, None);
        let status = myso_client
            .inner
            .get_token_transfer_action_onchain_status(
                bridge_object_arg,
