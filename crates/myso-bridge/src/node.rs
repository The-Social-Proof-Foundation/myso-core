...
use crate::myso_bridge_watchdog::myso_bridge_status::MySoBridgeStatus;
use crate::myso_bridge_watchdog::total_supplies::TotalSupplies;
...
pub async fn run_bridge_node(
    ...
    let myso_chain_identifier = server_config
        .myso_client
        .get_chain_identifier()
        .await
        .map_err(|e| anyhow::anyhow!("Failed to get myso chain identifier: {:?}", e))?;
...
async fn start_watchdog(
    ...
    let myso_bridge_status = MySoBridgeStatus::new(
        myso_client.clone(),
        watchdog_metrics.myso_bridge_paused.clone(),
    );

    let mut observables: Vec<Box<dyn Observable + Send + Sync>> =
        vec![Box::new(eth_bridge_status), Box::new(myso_bridge_status)];
...
async fn start_client_components(
    ...
    let last_processed_bridge_event_id = myso_modules_to_watch
    ...
    let next_sequence_number = get_next_sequence_number(
        ...
        client_config.myso_bridge_next_sequence_number_override,
    )
    .await;
    ...
    let (task_handles, myso_grpc_events_rx) = MySoSyncer::new(
        client_config.myso_client,
        myso_modules_to_watch,
        metrics.clone(),
    )
    .run_grpc(
    ...
    .await
    .expect("Failed to start myso syncer");
    all_handles.extend(task_handles);
...
async fn get_next_sequence_number<C: crate::myso_client::MySoClientInner>(
    ...
    myso_client: &crate::myso_client::MySoClient<C>,
    ...
    if let Ok(Some(sequence_number)) = store.get_myso_sequence_number_cursor() {
...
fn get_myso_modules_to_watch(
    store: &std::sync::Arc<BridgeOrchestratorTables>,
    myso_bridge_module_last_processed_event_id_override: Option<EventID>,
) -> HashMap<Identifier, Option<EventID>> {
    let myso_bridge_modules = vec![
        BRIDGE_MODULE_NAME.to_owned(),
        BRIDGE_COMMITTEE_MODULE_NAME.to_owned(),
        BRIDGE_TREASURY_MODULE_NAME.to_owned(),
        BRIDGE_LIMITER_MODULE_NAME.to_owned(),
    ];
    if let Some(cursor) = myso_bridge_module_last_processed_event_id_override {
        info!("Overriding cursor for myso bridge modules to {:?}", cursor);
    ...
    let myso_bridge_module_stored_cursor = store
        .get_myso_event_cursors(&myso_bridge_modules)
        .expect("Failed to get eth myso event cursors from storage");
    let mut myso_modules_to_watch = HashMap::new();
    for (module_identifier, cursor) in myso_bridge_modules
        .iter()
        .zip(myso_bridge_module_stored_cursor)
    {
        if cursor.is_none() {
            info!(
                "No cursor found for myso bridge module {} in storage or config override, query start from the beginning.",
                module_identifier
            );
        }
        myso_modules_to_watch.insert(module_identifier.clone(), cursor);
    }
    myso_modules_to_watch
}
...
mod tests {
    ...
    async fn test_starting_bridge_node() {
        ...
        let config = BridgeNodeConfig {
            ...
            myso: MySoConfig {
                myso_rpc_url: bridge_test_cluster.myso_rpc_url(),
            ...
            eth: EthConfig {
                ...
                eth_bridge_proxy_address: bridge_test_cluster.myso_bridge_address(),
    ...
    async fn test_starting_bridge_node_with_client() {
        ...
        let client_myso_address = MySoAddress::from(kp.public());
        let sender_address = bridge_test_cluster.myso_user_address();
        // send some gas to this address
        bridge_test_cluster
            .test_cluster
            .inner
            .transfer_myso_must_exceed(sender_address, client_myso_address, 1000000000)
            .await;
        ...
        let config = BridgeNodeConfig {
            ...
            myso: MySoConfig {
                ...
                bridge_client_key_path: None,
                bridge_client_gas_object: None,
                myso_bridge_module_last_processed_event_id_override: Some(EventID {
