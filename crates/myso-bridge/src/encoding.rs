...
use crate::types::AddTokensOnMySoAction;
...
use crate::types::EthToMySoBridgeAction;
use crate::types::EthToMySoTokenTransferV2;
...
use myso_types::base_types::MYSO_ADDRESS_LENGTH;
...
pub const ADD_TOKENS_ON_MYSO_MESSAGE_VERSION: u8 = 1;
pub const ADD_TOKENS_ON_EVM_MESSAGE_VERSION: u8 = 1;

pub const BRIDGE_MESSAGE_PREFIX: &[u8] = b"MYSO_BRIDGE_MESSAGE";
...
impl BridgeMessageEncoding for MySoToEthBridgeAction {
    fn as_bytes(&self) -> Result<Vec<u8>> {
        let mut bytes = Vec::new();
        let e = &self.myso_bridge_event;
        // Add message type
        ...
        // Add source chain id
        bytes.push(e.myso_chain_id as u8);
    ...
    fn as_payload_bytes(&self) -> Result<Vec<u8>> {
        ...
        bytes.push(MYSO_ADDRESS_LENGTH as u8);
        // Add source address
        bytes.extend_from_slice(&e.myso_address.to_vec());
        // Add dest chain id
        ...
        // Add token amount
        bytes.extend_from_slice(&e.amount_myso_adjusted.to_be_bytes());

        Ok(bytes)
    }
}

impl BridgeMessageEncoding for MySoToEthTokenTransfer {
    fn as_bytes(&self) -> Result<Vec<u8>> {
        let mut bytes = Vec::new();
        // Add message type
        bytes.push(BridgeActionType::TokenTransfer as u8);
        ...
        // Add source chain id
        bytes.push(self.myso_chain_id as u8);
    ...
    fn as_payload_bytes(&self) -> Result<Vec<u8>> {
        ...
        bytes.push(MYSO_ADDRESS_LENGTH as u8);
        // Add source address
        bytes.extend_from_slice(&self.myso_address.to_vec());
        // Add dest chain id
...
impl BridgeMessageEncoding for MySoToEthTokenTransferV2 {
    fn as_bytes(&self) -> Result<Vec<u8>> {
        let mut bytes = Vec::new();
        // Add message type
        bytes.push(BridgeActionType::TokenTransfer as u8);
...
impl BridgeMessageEncoding for EthToMySoBridgeAction {
    fn as_bytes(&self) -> Result<Vec<u8>> {
        let mut bytes = Vec::new();
    ...
    fn as_payload_bytes(&self) -> Result<Vec<u8>> {
        ...
        // Add token amount
        bytes.extend_from_slice(&e.myso_adjusted_amount.to_be_bytes());

        Ok(bytes)
    }
}

impl BridgeMessageEncoding for EthToMySoTokenTransferV2 {
    fn as_bytes(&self) -> Result<Vec<u8>> {
        let mut bytes = Vec::new();
        let e = &self.eth_bridge_event;
        // Add message type
    ...
    fn as_payload_bytes(&self) -> Result<Vec<u8>> {
        ...
        // Add timestamp (convert seconds to milliseconds for MySo)
...
impl BridgeMessageEncoding for AddTokensOnMySoAction {
    fn as_bytes(&self) -> Result<Vec<u8>> {
        let mut bytes = Vec::new();
        // Add message type
        bytes.push(BridgeActionType::AddTokensOnMySo as u8);
        // Add message version
        bytes.push(ADD_TOKENS_ON_MYSO_MESSAGE_VERSION);
        // Add nonce
        bytes.extend_from_slice(&self.nonce.to_be_bytes());
        // Add chain id
        bytes.push(self.chain_id as u8);
...
impl BridgeMessageEncoding for AddTokensOnEvmAction {
    ...
    fn as_payload_bytes(&self) -> Result<Vec<u8>> {
        ...
        // Add token myso decimals
        bytes.push(u8::try_from(self.token_myso_decimals.len())?);
        for token_myso_decimal in &self.token_myso_decimals {
            bytes.push(*token_myso_decimal);
        }
...
#[cfg(test)]
mod tests {
    use crate::abi::EthToMySoTokenBridgeV1;
    ...
    fn test_bridge_message_encoding() -> anyhow::Result<()> {
        ...
        let myso_chain_id = BridgeChainId::MySoTestnet;
        let myso_tx_event_index = 1u16;
        let eth_chain_id = BridgeChainId::EthSepolia;
        let myso_address = MySoAddress::random_for_testing_only();
        let eth_address = EthAddress::random();
        let token_id = TOKEN_ID_USDC;
        let amount_myso_adjusted = 1_000_000;

        let myso_bridge_event = EmittedMySoToEthTokenBridgeV1 {
            nonce,
            myso_chain_id,
            eth_chain_id,
            myso_address,
        ...
        let encoded_bytes = BridgeAction::MySoToEthBridgeAction(MySoToEthBridgeAction {
            myso_tx_digest,
            myso_tx_event_index,
            myso_bridge_event,
        })
        ...
        let source_chain_id_bytes = vec![myso_chain_id as u8]; // len: 1

        let myso_address_length_bytes = vec![MYSO_ADDRESS_LENGTH as u8]; // len: 1
        let myso_address_bytes = myso_address.to_vec(); // len: 32
        ...
        let token_amount_bytes = amount_myso_adjusted.to_be_bytes().to_vec(); // len: 8
        ...
        combined_bytes.extend_from_slice(&myso_address_length_bytes);
        combined_bytes.extend_from_slice(&myso_address_bytes);
    ...
    #[test]
    fn test_bridge_message_encoding_regression_emitted_myso_to_eth_token_bridge_v1()
    -> anyhow::Result<()> {
        telemetry_subscribers::init_for_testing();
        let registry = Registry::new();
        mysten_metrics::init_metrics(&registry);
        ...
        let myso_chain_id = BridgeChainId::MySoTestnet;
        let eth_chain_id = BridgeChainId::EthSepolia;
        let myso_address = MySoAddress::from_str(
        ...
        let token_id = TOKEN_ID_USDC;
        let amount_myso_adjusted = 12345;
    ...
    #[test]
    fn test_bridge_message_encoding_regression_eth_to_myso_token_bridge_v1() -> anyhow::Result<()> {
        telemetry_subscribers::init_for_testing();
        let registry = Registry::new();
        mysten_metrics::init_metrics(&registry);
        ...
        let eth_address =
            EthAddress::from_str("0x00000000000000000000000000000000000000c8").unwrap();
        let token_id = TOKEN_ID_USDC;
        let myso_adjusted_amount = 12345;

        let eth_bridge_event = EthToMySoTokenBridgeV1 {
            nonce,
            myso_chain_id,
            eth_chain_id,
            myso_address,
            eth_address,
            token_id,
            myso_adjusted_amount,
        };
        let encoded_bytes = BridgeAction::EthToMySoBridgeAction(EthToMySoBridgeAction {
            eth_tx_hash,
            eth_event_index,
            eth_bridge_event,
        })
    ...
    #[test]
    fn test_bridge_message_encoding_regression_add_coins_on_myso() -> anyhow::Result<()> {
        telemetry_subscribers::init_for_testing();

        let action = BridgeAction::AddTokensOnMySoAction(AddTokensOnMySoAction {
            nonce: 0,
            chain_id: BridgeChainId::MySoCustom,
            native: false,
