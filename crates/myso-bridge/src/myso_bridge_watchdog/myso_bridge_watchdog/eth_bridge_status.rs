...
use crate::abi::EthMySoBridge::EthMySoBridgeInstance;
...
pub struct EthBridgeStatus {
    bridge_contract: EthMySoBridgeInstance<EthProvider>,
    metric: IntGauge,
}

impl EthBridgeStatus {
    pub fn new(provider: EthProvider, bridge_address: EthAddress, metric: IntGauge) -> Self {
        let bridge_contract = EthMySoBridgeInstance::new(bridge_address, provider.clone());
        Self {
            bridge_contract,
            metric,
        }
