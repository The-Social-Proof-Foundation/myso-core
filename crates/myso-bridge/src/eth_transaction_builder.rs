...
use crate::abi::{
    ...
    eth_committee_upgradeable_contract, eth_myso_bridge,
};
...
pub async fn build_eth_transaction(
    ...
    match action.data() {
        ...
        BridgeAction::AddTokensOnMySoAction(_) => {
            unreachable!();
        }
...
pub fn build_emergency_op_approve_transaction(
    ...
    let message: eth_myso_bridge::BridgeUtils::Message = action.clone().try_into()?;
    let signatures = sigs
        .signatures
        .values()
    ...
    let call = eth_myso_bridge::EthMySoBridge::executeEmergencyOpWithSignaturesCall {
        signatures,
        message,
    };
