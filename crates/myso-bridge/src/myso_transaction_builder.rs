...
pub fn build_myso_transaction_ex(
    ...
    // TODO: Check chain id?
    match action.data() {
        BridgeAction::EthToMySoBridgeAction(_) | BridgeAction::EthToMySoTokenTransferV2(_) => {
        ...
        BridgeAction::MySoToEthBridgeAction(_) => build_token_bridge_approve_transaction(
            client_address,
            gas_object_ref,
            action,
            false,
        ...
        BridgeAction::EvmContractUpgradeAction(_) => {
            // It does not need a MySo transaction to execute EVM contract upgrade
            unreachable!()
        }
        BridgeAction::AddTokensOnMySoAction(_) => build_add_tokens_on_myso_transaction(
            client_address,
            gas_object_ref,
            action,
            bridge_object_arg,
        ...
        BridgeAction::AddTokensOnEvmAction(_) => {
            // It does not need a MySo transaction to add tokens on EVM
            unreachable!()
        }
    }
}
...
fn build_token_bridge_approve_transaction(
    ...
    let (
        ...
        BridgeAction::MySoToEthBridgeAction(a) => {
            let bridge_event = a.myso_bridge_event;
            (
                bridge_event.myso_chain_id,
                bridge_event.nonce,
                bridge_event.myso_address.to_vec(),
                bridge_event.eth_chain_id,
                bridge_event.eth_address.to_vec(),
                bridge_event.token_id,
                bridge_event.amount_myso_adjusted,
                None,
            )
        }
        BridgeAction::MySoToEthTokenTransfer(a) => (
            a.myso_chain_id,
            a.nonce,
            a.myso_address.to_vec(),
            a.eth_chain_id,
        ...
        BridgeAction::MySoToEthTokenTransferV2(a) => (
            a.myso_chain_id,
            a.nonce,
            a.myso_address.to_vec(),
            a.eth_chain_id,
        ...
        BridgeAction::EthToMySoBridgeAction(a) => {
            ...
            (
                ...
                bridge_event.myso_address.to_vec(),
                bridge_event.token_id,
                bridge_event.myso_adjusted_amount,
                None,
            )
        }
        BridgeAction::EthToMySoTokenTransferV2(a) => {
            let bridge_event = a.eth_bridge_event;
            // Convert seconds to milliseconds for MySo
    ...
    if claim {
        let token_tag = myso_token_type_tags
            .get(&token_type)
...
pub fn build_add_tokens_on_myso_transaction(
    client_address: MySoAddress,
    gas_object_ref: &ObjectRef,
    action: VerifiedCertifiedBridgeAction,
    bridge_object_arg: ObjectArg,
    ...
    let (source_chain, seq_num, native, token_ids, token_type_names, token_prices) =
        match bridge_action {
            BridgeAction::AddTokensOnMySoAction(a) => (
                a.chain_id,
                a.nonce,
                a.native,
                a.token_ids,
    ...
    let message_arg = builder.programmable_move_call(
        ...
        BRIDGE_CREATE_ADD_TOKEN_ON_MYSO_MESSAGE_FUNCTION_NAME.into(),
        vec![],
        vec![
            source_chain,
            seq_num,
...
pub fn build_committee_register_transaction(
    ...
    let mut builder = ProgrammableTransactionBuilder::new();
    let system_state = builder.obj(ObjectArg::MYSO_SYSTEM_MUT).unwrap();
...
mod tests {
    ...
    async fn test_build_myso_transaction_for_token_transfer() {
        ...
        let myso_client = MySoClient::new(&test_cluster.inner.fullnode_handle.rpc_url, metrics)
            .await
            .unwrap();
        let bridge_authority_keys = test_cluster.authority_keys_clone();

        // Note: We don't call `myso_client.get_bridge_committee` here because it will err if the committee
        ...
        // 2. Test MySo -> Eth Transfer approval
    ...
    #[tokio::test(flavor = "multi_thread", worker_threads = 8)]
    async fn test_build_myso_transaction_for_emergency_op() {
        telemetry_subscribers::init_for_testing();
        let num_valdiator = 2;
        let mut bridge_keys = vec![];
        ...
        test_cluster
            .trigger_reconfiguration_if_not_yet_and_assert_bridge_committee_initialized()
            .await;
        let summary = myso_client.get_bridge_summary().await.unwrap();
    ...
    #[tokio::test(flavor = "multi_thread", worker_threads = 8)]
    async fn test_build_myso_transaction_for_committee_blocklist() {
        telemetry_subscribers::init_for_testing();
        let mut bridge_keys = vec![];
        for _ in 0..=3 {
            let (_, kp): (_, BridgeAuthorityKeyPair) = get_key_pair();
        ...
        test_cluster
            .trigger_reconfiguration_if_not_yet_and_assert_bridge_committee_initialized()
            .await;
        let committee = myso_client.get_bridge_summary().await.unwrap().committee;
    ...
    #[tokio::test(flavor = "multi_thread", worker_threads = 8)]
    async fn test_build_myso_transaction_for_limit_update() {
        telemetry_subscribers::init_for_testing();
        let mut bridge_keys = vec![];
        for _ in 0..=3 {
            let (_, kp): (_, BridgeAuthorityKeyPair) = get_key_pair();
        ...
        test_cluster
            .trigger_reconfiguration_if_not_yet_and_assert_bridge_committee_initialized()
            .await;
        let transfer_limit = myso_client
            .get_bridge_summary()
            .await
            .unwrap()
            .limiter
        ...
        .await;
        let new_transfer_limit = myso_client
            .get_bridge_summary()
            .await
            .unwrap()
            .limiter
        ...
        for limit in new_transfer_limit {
            if limit.0 == BridgeChainId::EthCustom && limit.1 == BridgeChainId::MySoCustom {
    ...
    #[tokio::test(flavor = "multi_thread", worker_threads = 8)]
    async fn test_build_myso_transaction_for_price_update() {
        telemetry_subscribers::init_for_testing();
        let mut bridge_keys = vec![];
        for _ in 0..=3 {
            let (_, kp): (_, BridgeAuthorityKeyPair) = get_key_pair();
        ...
        test_cluster
            .trigger_reconfiguration_if_not_yet_and_assert_bridge_committee_initialized()
            .await;
        let notional_values = myso_client.get_notional_values().await.unwrap();
        ...
        .await;
        let new_notional_values = myso_client.get_notional_values().await.unwrap();
