...
impl BridgeClient {
    ...
    fn bridge_action_to_path(event: &BridgeAction) -> String {
        match event {
            BridgeAction::MySoToEthBridgeAction(e) => format!(
                "sign/bridge_tx/myso/eth/{}/{}",
                e.myso_tx_digest, e.myso_tx_event_index
            ),
            BridgeAction::MySoToEthTokenTransfer(_) | BridgeAction::MySoToEthTokenTransferV2(_) => {
                format!(
                    "/sign/bridge_action/myso/eth/{source_chain}/{message_type}/{bridge_seq_num}",
                    source_chain = event.chain_id() as u8,
                    message_type = event.action_type() as u8,
                    bridge_seq_num = event.seq_number(),
                )
            }
            BridgeAction::EthToMySoBridgeAction(e) => format!(
                "sign/bridge_tx/eth/myso/{}/{}",
                Hex::encode(e.eth_tx_hash.0),
                e.eth_event_index
            ),
            BridgeAction::EthToMySoTokenTransferV2(e) => format!(
                "sign/bridge_tx/eth/myso/{}/{}",
                Hex::encode(e.eth_tx_hash.0),
                e.eth_event_index
            ),
            ...
            BridgeAction::AddTokensOnMySoAction(a) => {
                ...
                let token_prices = a
                    ...
                    .join(",");
                format!(
                    "sign/add_tokens_on_myso/{chain_id}/{nonce}/{native}/{token_ids}/{token_type_names}/{token_prices}"
                )
            }
            BridgeAction::AddTokensOnEvmAction(a) => {
                ...
                let token_prices = a
                    ...
                    .join(",");
                format!(
                    "sign/add_tokens_on_evm/{chain_id}/{nonce}/{native}/{token_ids}/{token_addresses}/{token_myso_decimals}/{token_prices}"
                )
            }
        }
    }
...
mod tests {
    ...
    use crate::{
        abi::EthToMySoTokenBridgeV1,
        crypto::BridgeAuthoritySignInfo,
        events::EmittedMySoToEthTokenBridgeV1,
        server::mock_handler::BridgeRequestMockHandler,
        test_utils::{get_test_authority_and_key, get_test_myso_to_eth_bridge_action},
    ...
    async fn test_bridge_client() {
        ...
        let action =
            get_test_myso_to_eth_bridge_action(None, Some(1), Some(1), Some(100), None, None, None);
        ...
        // Ok
        authority.base_url = "https://foo.mysobridge.io".to_string();
        let committee = Arc::new(BridgeCommittee::new(vec![authority.clone()]).unwrap());
    ...
    async fn test_bridge_client_request_sign_action() {
        ...
        mock_handler.add_myso_event_response(tx_digest, event_idx, Ok(signed_event.clone()), None);
        ...
        mock_handler.add_myso_event_response(tx_digest, event_idx, Ok(wrong_signed_action), None);
        let err = client
            .request_sign_bridge_action(action.clone())
            .await
            .unwrap_err();
        ...
        mock_handler.add_myso_event_response(tx_digest, event_idx, Ok(signed_event), None);
        ...
        mock_handler.add_myso_event_response(tx_digest, event_idx, Ok(signed_event2), None);
        let err = client
            .request_sign_bridge_action(action.clone())
            .await
            .unwrap_err();
        ...
        mock_handler.add_myso_event_response(tx_digest, event_idx, Ok(signed_event3), None);
        let err = client
            .request_sign_bridge_action(action.clone())
            .await
            .unwrap_err();
    ...
    fn test_bridge_action_path_regression_tests() {
        ...
        let myso_tx_event_index = 5;
        let action = BridgeAction::MySoToEthBridgeAction(crate::types::MySoToEthBridgeAction {
            myso_tx_digest,
            myso_tx_event_index,
        ...
        assert_eq!(
            ...
            format!(
                "sign/bridge_tx/myso/eth/{}/{}",
                myso_tx_digest, myso_tx_event_index
            )
        );

        let eth_tx_hash = TxHash::random();
        let eth_event_index = 6;
        let action = BridgeAction::EthToMySoBridgeAction(crate::types::EthToMySoBridgeAction {
            eth_tx_hash,
            eth_event_index,
        ...
        let action = BridgeAction::AddTokensOnMySoAction(crate::types::AddTokensOnMySoAction {
            nonce: 3,
            chain_id: BridgeChainId::MySoCustom,
            native: false,
