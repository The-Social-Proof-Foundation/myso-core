...
use crate::abi::{
    EthBridgeCommittee, EthBridgeConfigInstance, EthBridgeEvent, EthERC20, EthMySoBridge,
    EthMySoBridgeEvents,
};
use crate::config::{BridgeNodeConfig, EthConfig, MySoConfig, default_ed25519_key_pair};
use crate::crypto::{
...
use crate::utils::{
    ...
    publish_and_register_coins_return_add_coins_on_myso_action, wait_for_server_to_be_up,
};
...
use myso_types::committee::{ProtocolVersion, TOTAL_VOTING_POWER};
use myso_types::crypto::{EncodeDecodeBase64, KeypairTraits, ToFromBytes, get_key_pair};
...
const MYSO_BRIDGE_NAME: &str = "MySoBridge";
...
pub struct BridgeTestCluster {
    ...
    bridge_client: MySoBridgeClient,
...
impl BridgeTestClusterBuilder {
    pub fn new() -> Self {
        BridgeTestClusterBuilder {
            ...
            myso_chain_id: BridgeChainId::MySoCustom,
        }
    }
    ...
    pub fn with_myso_chain_id(mut self, chain_id: BridgeChainId) -> Self {
        self.myso_chain_id = chain_id;
        self
    }
    ...
    pub async fn build(self) -> BridgeTestCluster {
        ...
        let bridge_client =
            MySoBridgeClient::new(&test_cluster.inner.fullnode_handle.rpc_url, metrics)
                .await
                .unwrap();
...
impl BridgeTestCluster {
    ...
    pub fn bridge_client(&self) -> &MySoBridgeClient {
        &self.bridge_client
    }
    ...
    pub fn myso_user_address(&self) -> MySoAddress {
        self.test_cluster.inner.get_address_0()
    }

    pub fn myso_chain_id(&self) -> BridgeChainId {
        self.myso_chain_id
    }
    ...
    pub fn myso_bridge_address(&self) -> String {
        self.eth_environment.contracts().myso_bridge_addrress_hex()
    }
    ...
    pub fn myso_rpc_url(&self) -> String {
    ...
    /// Upgrades the MySoBridge proxy to MySoBridgeV2.
    /// This method:
    /// 1. Deploys MySoBridgeV2 implementation using forge
    ...
    pub async fn upgrade_bridge_to_v2(&self) -> anyhow::Result<()> {
        ...
        let myso_bridge_proxy = self.contracts().myso_bridge;

        info!("Upgrading MySoBridge to V2...");
        info!("  Proxy address: {:?}", myso_bridge_proxy);

        // Step 1: Deploy MySoBridgeV2 implementation using forge create
        let (_, eth_pk_hex) = self.get_eth_signer_and_private_key()?;
        let output = std::process::Command::new("forge")
            .current_dir(&sol_path)
            .args([
                "create",
                "contracts/MySoBridgeV2.sol:MySoBridgeV2",
                "--rpc-url",
                &self.eth_rpc_url(),
                "--private-key",
                &eth_pk_hex,
            ...
            .output()
            .expect("Failed to deploy MySoBridgeV2");

        if !output.status.success() {
            let stderr = String::from_utf8_lossy(&output.stderr);
            return Err(anyhow::anyhow!("Failed to deploy MySoBridgeV2: {}", stderr));
        }
        ...
        let bridge_contract = EthMySoBridge::new(myso_bridge_proxy, eth_signer.clone());
        ...
        let upgrade_action = EvmContractUpgradeAction {
            ...
            proxy_address: myso_bridge_proxy,
            new_impl_address,
        ...
        info!("MySoBridge upgraded to V2 successfully!");
...
impl DeployedSolContracts {
    ...
    pub fn myso_bridge_addrress_hex(&self) -> String {
        Self::eth_adress_to_hex(self.myso_bridge)
    }
}
...
pub(crate) async fn deploy_sol_contract(
    ...
    let contracts = DeployedSolContracts {
        myso_bridge: deployed_contracts.remove(MYSO_BRIDGE_NAME).unwrap(),
...
impl EthBridgeEnvironment {
    ...
    pub async fn get_supported_token(&self, token_id: u8) -> (EthAddress, u8, u64) {
        ...
        let token_myso_decimal = config.tokenMySoDecimalOf(token_id).call().await.unwrap();
        let token_price = config.tokenPriceOf(token_id).call().await.unwrap();
        (token_address, token_myso_decimal, token_price)
    }
}
...
pub(crate) async fn start_bridge_cluster(
        ...
        .enumerate()
    {
        ...
        let config = BridgeNodeConfig {
            ...
            myso: MySoConfig {
                myso_rpc_url: test_cluster.inner.fullnode_handle.rpc_url.clone(),
                myso_bridge_chain_id: BridgeChainId::MySoCustom as u8,
...
pub async fn get_signatures(
    ...
    let sigs = myso_bridge_client
        .get_token_transfer_action_onchain_signatures_until_success(myso_chain_id, nonce)
        .await
        .unwrap();
...
impl TestClusterWrapperBuilder {
    ...
    pub async fn build(self) -> TestClusterWrapper {
        ...
        let gas_objects_for_authority_keys = self
            ...
            .iter()
            .map(|k| {
                let address = MySoAddress::from(k.public());
        ...
        for (node, kp) in reordered_nodes.zip(self.bridge_authority_keys.iter()) {
            let validator_address = node.config().myso_address();
        ...
        if self.deploy_tokens {
            ...
            let action = publish_and_register_coins_return_add_coins_on_myso_action(
                test_cluster.wallet(),
                bridge_arg,
                vec![
            ...
            let tx = build_add_tokens_on_myso_transaction(
                sender_address,
                &test_cluster
                    .wallet
...
pub async fn initiate_bridge_eth_to_myso(
    bridge_test_cluster: &BridgeTestCluster,
    amount: u64,
    nonce: u64,
) -> Result<(), anyhow::Error> {
    initiate_bridge_eth_to_myso_internal(bridge_test_cluster, amount, nonce, false).await
}

pub async fn initiate_bridge_eth_to_myso_v2(
    bridge_test_cluster: &BridgeTestCluster,
    amount: u64,
    nonce: u64,
) -> Result<(), anyhow::Error> {
    initiate_bridge_eth_to_myso_internal(bridge_test_cluster, amount, nonce, true).await
}

async fn initiate_bridge_eth_to_myso_internal(
    bridge_test_cluster: &BridgeTestCluster,
    amount: u64,
    nonce: u64,
    use_v2: bool,
    ...
    let myso_address = bridge_test_cluster.myso_user_address();
    let myso_chain_id = bridge_test_cluster.myso_chain_id();
    let eth_chain_id = bridge_test_cluster.eth_chain_id();
    let token_id = TOKEN_ID_ETH;

    let myso_amount = (U256::from(amount) * U256::from(10).pow(U256::from(8))).to::<u64>(); // DP for Ether on MySo
    let eth_amount = U256::from(amount) * U256::from(10).pow(U256::from(18));

    let contract = EthMySoBridge::new(
        bridge_test_cluster.contracts().myso_bridge,
        eth_signer.clone(),
    );
    let myso_recipient_address = myso_address.to_vec().into();
    let tx_receipt = if use_v2 {
        let call = contract
            .bridgeETHV2(myso_recipient_address, myso_chain_id as u8)
            .value(eth_amount);
        send_eth_tx_and_get_tx_receipt(eth_signer, call.into_transaction_request()).await
    } else {
        let call = contract
            .bridgeETH(myso_recipient_address, myso_chain_id as u8)
            .value(eth_amount);
        send_eth_tx_and_get_tx_receipt(eth_signer, call.into_transaction_request()).await
    };
    ...
    let (
        ...
        EthBridgeEvent::EthMySoBridgeEvents(EthMySoBridgeEvents::TokensDeposited(event)) => (
            event.sourceChainID,
            event.destinationChainID,
            event.tokenID,
            event.mysoAdjustedAmount,
            event.senderAddress,
            event.recipientAddress.to_vec(),
            None,
        ),
        EthBridgeEvent::EthMySoBridgeEvents(EthMySoBridgeEvents::TokensDepositedV2(event)) => (
            event.sourceChainID,
            event.destinationChainID,
            event.tokenID,
            event.mysoAdjustedAmount,
    ...
    assert_eq!(destination_chain_id, myso_chain_id as u8);
    assert_eq!(emitted_token_id, token_id);
    assert_eq!(emitted_amount, myso_amount);
    assert_eq!(sender, eth_address);
    assert_eq!(recipient, myso_address.to_vec());
    if use_v2 {
    ...
    .await
    .tap_ok(|_| {
        info!("Eth to MySo bridge transfer claimed");
    })
}

pub async fn initiate_bridge_myso_to_eth(
    bridge_test_cluster: &BridgeTestCluster,
    eth_address: EthAddress,
    token: ObjectRef,
    nonce: u64,
    myso_amount: u64,
) -> Result<MySoToEthTokenTransfer, anyhow::Error> {
    match initiate_bridge_myso_to_eth_internal(
        bridge_test_cluster,
        eth_address,
        token,
        nonce,
    ...
    .await?
    {
        BridgeAction::MySoToEthTokenTransfer(action) => Ok(action),
        _ => unreachable!("Expected V1 token transfer action"),
    }
}

pub async fn initiate_bridge_myso_to_eth_v2(
    bridge_test_cluster: &BridgeTestCluster,
    eth_address: EthAddress,
    token: ObjectRef,
    nonce: u64,
    myso_amount: u64,
) -> Result<MySoToEthTokenTransferV2, anyhow::Error> {
    match initiate_bridge_myso_to_eth_internal(
        bridge_test_cluster,
        eth_address,
        token,
    ...
    .await?
    {
        BridgeAction::MySoToEthTokenTransferV2(action) => Ok(action),
        _ => unreachable!("Expected V2 token transfer action"),
    }
}

async fn initiate_bridge_myso_to_eth_internal(
    bridge_test_cluster: &BridgeTestCluster,
    eth_address: EthAddress,
    token: ObjectRef,
    nonce: u64,
    ...
    let bridge_object_arg = bridge_test_cluster
        ...
        .await;
    let myso_client = bridge_test_cluster.grpc_client();
    ...
    let resp = match deposit_eth_to_myso_package(
        myso_client,
        myso_address,
    ...
    .await
    {
        Ok(resp) => {
            if !resp.effects.status().is_ok() {
                return Err(anyhow!("MySo TX error"));
            } else {
                resp
            }
        }
    ...
    let myso_events = resp.events.unwrap().data;
    let bridge_action = myso_events
        .iter()
        .filter_map(|e| {
            let myso_bridge_event = MySoBridgeEvent::try_from_event(e).unwrap()?;
            myso_bridge_event.try_into_bridge_action()
        })
        .find(|action| {
            matches!(
                action,
    ...
    match &bridge_action {
        BridgeAction::MySoToEthTokenTransfer(action) => {
            assert_eq!(action.nonce, nonce);
            assert_eq!(action.myso_chain_id, bridge_test_cluster.myso_chain_id());
            assert_eq!(action.eth_chain_id, bridge_test_cluster.eth_chain_id());
            assert_eq!(action.myso_address, myso_address);
            assert_eq!(action.eth_address, eth_address);
            assert_eq!(action.token_id, TOKEN_ID_ETH);
            assert_eq!(action.amount_adjusted, myso_amount);
        }
        BridgeAction::MySoToEthTokenTransferV2(action) => {
            assert_eq!(action.nonce, nonce);
            assert_eq!(action.myso_chain_id, bridge_test_cluster.myso_chain_id());
            assert_eq!(action.eth_chain_id, bridge_test_cluster.eth_chain_id());
            assert_eq!(action.myso_address, myso_address);
    ...
    wait_for_transfer_action_status(
        bridge_test_cluster.bridge_client(),
        bridge_test_cluster.myso_chain_id(),
        nonce,
        BridgeActionStatus::Approved,
    )
    .await
    .unwrap();
    info!("MySo to Eth bridge transfer approved.");
...
async fn deposit_eth_to_myso_package(
    myso_client: Client,
    ...
    if use_v2 {
        ...
        builder.programmable_move_call(
            ...
            vec![myso_token_type_tags.get(&TOKEN_ID_ETH).unwrap().clone()],
            vec![
                arg_bridge,
                arg_target_chain,
                arg_target_address,
...
pub async fn initiate_bridge_erc20_to_myso(
    ...
    let allowance_call = contract.approve(bridge_test_cluster.contracts().myso_bridge, amount);
    ...
    let myso_recipient_address = bridge_test_cluster.myso_user_address();
    let myso_chain_id = bridge_test_cluster.myso_chain_id();
    let eth_chain_id = bridge_test_cluster.eth_chain_id();
    ...
    let deposit_call = contract.bridgeERC20(
        token_id,
        amount,
        myso_recipient_address.to_vec().into(),
        myso_chain_id as u8,
    );
    let tx_receipt =
    ...
    let eth_bridge_event = tx_receipt
        ...
        .find_map(EthBridgeEvent::try_from_log)
        .unwrap();
    let EthBridgeEvent::EthMySoBridgeEvents(EthMySoBridgeEvents::TokensDeposited(eth_bridge_event)) =
        eth_bridge_event
    else {
        unreachable!();
    };
    ...
    assert_eq!(eth_bridge_event.destinationChainID, myso_chain_id as u8);
    assert_eq!(eth_bridge_event.tokenID, token_id);
    assert_eq!(eth_bridge_event.mysoAdjustedAmount, myso_amount);
    assert_eq!(eth_bridge_event.senderAddress, eth_address);
    assert_eq!(
        eth_bridge_event.recipientAddress,
        myso_recipient_address.to_vec()
    );
    info!(
    ...
    .tap_ok(|_| {
        info!(
            nonce,
            token_id, amount_u64, "Eth to MySo bridge transfer claimed"
        );
    })
}
