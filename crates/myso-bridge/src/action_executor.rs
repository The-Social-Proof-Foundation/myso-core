    ...
    C: MySoClientInner + 'static,
{
    ...
    fn run_inner(
        ...
        let client_clone = self.myso_client.clone();
        let mut tasks = vec![];
    ...
    async fn should_proceed_signing(myso_client: &Arc<MySoClient<C>>) -> bool {
        let Ok(Ok(is_paused)) =
            retry_with_max_elapsed_time!(myso_client.is_bridge_paused(), Duration::from_secs(600))
        else {
    ...
    async fn handle_signing_task(
        ...
        let should_proceed = Self::should_proceed_signing(myso_client).await;
        if !should_proceed {
    ...
    async fn handle_already_processed_token_transfer_action_maybe(
        ...
        match status {
            ...
            // Although theoretically a legit MySoToEthBridgeAction should not have
            // status `NotFound`
    ...
    async fn request_signatures(
        ...
        match &action {
            BridgeAction::MySoToEthBridgeAction(_)
            | BridgeAction::MySoToEthTokenTransfer(_)
            | BridgeAction::MySoToEthTokenTransferV2(_)
            | BridgeAction::EthToMySoBridgeAction(_)
            | BridgeAction::EthToMySoTokenTransferV2(_) => (),
    ...
    // Before calling this function, `key` and `myso_address` need to be
    // verified to match.
    ...
    async fn handle_execution_task(
        ...
        let (gas_coin, gas_object_ref) =
            Self::get_gas_data_assert_ownership(*myso_address, gas_object_id, myso_client).await;
        ...
        if Self::handle_already_processed_token_transfer_action_maybe(
            myso_client, action, store, metrics,
        )
        .await
        {
        ...
        let tx_data = match build_myso_transaction(
            ...
            *bridge_object_arg,
            ...
            Err(BridgeError::UnknownTokenId(token_id)) => {
                ...
                match myso_client.get_token_id_map().await {
                    Ok(new_token_map) => {
                        ...
                        match build_myso_transaction(
                            ...
                            *bridge_object_arg,
                            ...
                            Ok(tx_data) => tx_data,
                            Err(err) => {
                                metrics.err_build_myso_transaction.inc();
                                error!(
        ...
        info!(?tx_digest, ?gas_object_ref, "Sending transaction to MySo");
        match myso_client
            ...
            .await
        {
            ...
            Err(err) => {
                error!(
                    ?action_key,
                    ?tx_digest,
                    "MySo transaction failed at signing: {err:?}"
                );
                metrics.err_myso_transaction_submission.inc();
                let metrics_clone = metrics.clone();
                ...
                spawn_logged_monitored_task!(async move {
                    ...
                    if attempt_times >= MAX_EXECUTION_ATTEMPTS {
                        metrics_clone
                            .err_myso_transaction_submission_too_many_failures
                            .inc();
    ...
    async fn handle_execution_effects(
        ...
        match &response.status {
            MySoExecutionStatus::Success => {
                ...
                info!(?tx_digest, "MySo transaction executed successfully");
                ...
                relevant_events.iter().for_each(|e| {
                    if e.type_ == *TokenTransferClaimed.get().unwrap() {
                        match action {
                            BridgeAction::EthToMySoBridgeAction(_)
                            | BridgeAction::EthToMySoTokenTransferV2(_) => {
                                metrics.eth_myso_token_transfer_claimed.inc();
                            }
                            BridgeAction::MySoToEthBridgeAction(_)
                            | BridgeAction::MySoToEthTokenTransfer(_)
                            | BridgeAction::MySoToEthTokenTransferV2(_) => {
                                metrics.myso_eth_token_transfer_claimed.inc();
                            }
                        ...
                        match action {
                            BridgeAction::EthToMySoBridgeAction(_)
                            | BridgeAction::EthToMySoTokenTransferV2(_) => {
                                metrics.eth_myso_token_transfer_approved.inc();
                            }
                            BridgeAction::MySoToEthBridgeAction(_)
                            | BridgeAction::MySoToEthTokenTransfer(_)
                            | BridgeAction::MySoToEthTokenTransferV2(_) => {
                                metrics.myso_eth_token_transfer_approved.inc();
                            }
            ...
            MySoExecutionStatus::Failure { error } => {
                ...
                metrics.err_myso_transaction_execution.inc();
                error!(
                    ?tx_digest,
                    "Manual intervention is needed. MySo transaction executed and failed with error: {error:?}"
                );
            }
        }
    }
    ...
    async fn get_gas_data_assert_ownership(
        ...
        let (gas_coin, gas_obj_ref, owner) = myso_client
...
mod tests {
    ...
    use myso_types::{base_types::random_object_ref, transaction::TransactionData};
    ...
    use crate::{
        ...
        test_utils::{
            get_test_authorities_and_run_mock_bridge_server, get_test_eth_to_myso_bridge_action,
            get_test_myso_to_eth_bridge_action, sign_action_with_key,
        },
    ...
    async fn test_onchain_execution_loop() {
        ...
        let id_token_map = (*myso_token_type_tags.load().clone()).clone();
        let tx_data = build_myso_transaction(
            myso_address,
            &gas_object_ref,
            action_certificate,
        ...
        let tx_digest = get_tx_digest(tx_data, &dummy_myso_key);

        let gas_coin = GasCoin::new_for_testing(1_000_000_000_000); // dummy gas coin
        myso_client_mock.add_gas_object_info(
            gas_coin.clone(),
            gas_object_ref,
            Owner::AddressOwner(myso_address),
        );
        ...
        let mut event = MySoEvent::random_for_testing();
    ...
    async fn test_signature_aggregation_loop() {
        ...
        let id_token_map = (*myso_token_type_tags.load().clone()).clone();
        let (action_certificate, myso_tx_digest, myso_tx_event_index) =
        ...
        loop {
            let requested_times =
                mock0.get_myso_token_events_requested(myso_tx_digest, myso_tx_event_index);
    ...
    async fn test_skip_request_signature_if_already_processed_on_chain() {
        ...
        myso_client_mock.set_action_onchain_status(&action, BridgeActionStatus::Approved);
    ...
    async fn test_skip_tx_submission_if_already_processed_on_chain() {
        ...
        myso_client_mock.set_action_onchain_status(&action, BridgeActionStatus::Pending);
    ...
    async fn test_skip_tx_submission_if_bridge_is_paused() {
        ...
        let id_token_map: HashMap<u8, TypeTag> = (*myso_token_type_tags.load().clone()).clone();
        let (action_certificate, _, _) = get_bridge_authority_approved_action(
            vec![&mock0, &mock1, &mock2, &mock3],
            vec![&secrets[0], &secrets[1], &secrets[2], &secrets[3]],
            None,
    ...
    async fn test_action_executor_handle_new_token() {
        ...
        let mut id_token_map: HashMap<u8, TypeTag> = (*myso_token_type_tags.load().clone()).clone();
        let (action_certificate, _, _) = get_bridge_authority_approved_action(
            vec![&mock0, &mock1, &mock2, &mock3],
            vec![&secrets[0], &secrets[1], &secrets[2], &secrets[3]],
            Some(new_token_id),
            false, // we need an eth -> myso action that entails the new token type tag in transaction building
        );
        ...
        myso_token_type_tags.store(Arc::new(id_token_map));
    ...
    fn mock_bridge_authority_sigs(
        ...
        myso_tx_digest: TransactionDigest,
        myso_tx_event_index: u16,
    ...
    fn get_bridge_authority_approved_action(
        ...
        let myso_tx_digest = TransactionDigest::random();
        let myso_tx_event_index = 1;
        let action = if myso_to_eth {
            ...
            get_test_eth_to_myso_bridge_action(None, None, None, token_id)
        };

        let sigs =
            mock_bridge_authority_sigs(mocks, &action, secrets, myso_tx_digest, myso_tx_event_index);
        let certified_action = CertifiedBridgeAction::new_from_data_and_sig(
            action,
            BridgeCommitteeValiditySignInfo { signatures: sigs },
        );
    ...
    fn get_tx_digest(tx_data: TransactionData, dummy_myso_key: &MySoKeyPair) -> TransactionDigest {
        let sig = Signature::new_secure(
            &IntentMessage::new(Intent::myso_transaction(), &tx_data),
            dummy_myso_key,
        );
    ...
    fn mock_transaction_response(
        myso_client_mock: &MySoMockClient,
        tx_digest: TransactionDigest,
        status: MySoExecutionStatus,
        events: Option<Vec<MySoEvent>>,
        wildcard: bool,
    ) {
        ...
        if wildcard {
            myso_client_mock.set_wildcard_transaction_response(Ok(response));
        } else {
            myso_client_mock.add_transaction_response(tx_digest, Ok(response));
        }
    }
    ...
    fn mock_transaction_error(
        ...
        if wildcard {
            myso_client_mock.set_wildcard_transaction_response(Err(error));
        } else {
            myso_client_mock.add_transaction_response(tx_digest, Err(error));
        }
    }
    ...
    async fn setup() -> (
        ...
        let (myso_address, kp): (_, fastcrypto::secp256k1::Secp256k1KeyPair) = get_key_pair();
        let myso_key = MySoKeyPair::from(kp);
        let gas_object_ref = random_object_ref();
        let temp_dir = tempfile::tempdir().unwrap();
        let store = BridgeOrchestratorTables::new(temp_dir.path());
        let myso_client_mock = MySoMockClient::default();
        let tx_subscription = myso_client_mock.subscribe_to_requested_transactions();
        ...
        let dummy_myso_key = MySoKeyPair::from(dummy_kp);
        ...
        let myso_token_type_tags = Arc::new(ArcSwap::new(Arc::new(myso_token_type_tags)));
