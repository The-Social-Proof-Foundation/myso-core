...
#
# Update myso-pg-db's generated src/schema.rs based on the schema after
...
diesel print-schema                                                           \
  --database-url "postgres://postgres:postgrespw@localhost:$PORT"             \
  --patch-file "$REPO/crates/myso-pg-db/schema.patch"             \
  > "$REPO/crates/myso-pg-db/src/schema.rs"
