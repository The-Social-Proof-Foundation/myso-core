...
`myso-indexer-alt`, as long as all the required tables are present (all
pipelines are enabled on the indexer). See the indexer's
[README](../myso-indexer-alt/README.md) for details on how to set it up.
...
When `--social-database-url` is provided (or `SOCIAL_DATABASE_URL` in production/Docker), GraphQL can query social data (profiles, posts, platforms, social graph, vesting) from the database populated by `myso-indexer-alt-social`. The core indexer and social indexer use separate databases (required). See the social indexer's [README](../myso-indexer-alt-social/README.md) for setup. The social database must have `myso-indexer-alt-social-schema` migrations applied (the social indexer applies these on startup).
...
```sh
cargo nextest run -p myso-indexer-alt-graphql
cargo nextest run -p myso-indexer-alt-reader
cargo nextest run -p myso-indexer-alt-e2e-tests -- graphql
```
...
```sh
cargo nextest run -p myso-indexer-alt-graphql -- test_schema_sdl_export
cargo nextest run -p myso-indexer-alt-graphql --features staging -- test_schema_sdl_export
cargo insta review
```
...
```sh
cargo run --bin myso-indexer-alt-graphql -- generate-config > graphql_config.toml
```
