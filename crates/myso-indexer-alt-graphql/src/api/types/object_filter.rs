...
mod tests {
    ...
    fn test_type_filter_intersection() {
        ...
        let myso = ObjectFilter {
            owner_kind: None,
            owner: None,
            type_: Some("0x2::coin::Coin<0x2::myso::MYSO>".parse().unwrap()),
        };
        ...
        // intersection order.
        assert_eq!(coin.clone().intersect(myso.clone()).unwrap(), myso);
        assert_eq!(myso.clone().intersect(coin.clone()).unwrap(), myso);
    ...
    fn test_combined_intersection() {
        ...
        assert_eq!(a_coin.clone().intersect(a_myso.clone()).unwrap(), a_myso);
