...
pub(crate) enum Error {
    ...
    Literal(#[from] myso_display::v2::FormatError),
...
impl DynamicField {
    ...
    async fn value(&self, ctx: &Context<'_>) -> Option<Result<DynamicFieldValue, RpcError>> {
        async {
            ...
            let address: MySoAddress = bcs::from_bytes(&native.value_bytes)
...
impl DynamicField {
    ...
    pub(crate) async fn by_serialized_name(
        ...
        let field_id: MySoAddress = derive_dynamic_field_id(parent, &type_, &bcs.0)?.into();
    ...
    pub(crate) async fn by_literal_name(
        ...
        #[async_trait]
        impl myso_display::v2::Store for NopStore {
            async fn object(
                &self,
                _: AccountAddress,
            ) -> anyhow::Result<Option<myso_display::v2::OwnedSlice>> {
        ...
        let root = myso_display::v2::OwnedSlice {
