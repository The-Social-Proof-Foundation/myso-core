...
pub(crate) enum Error {
    ...
    Format(myso_display::v2::FormatError),

    #[error("Path error: {0}")]
    Path(myso_display::v2::FormatError),
...
impl MoveValue {
    ...
    async fn display(&self, ctx: &Context<'_>) -> Option<Result<Display, RpcError>> {
        async {
            ...
            for (field, value) in
                myso_display::v1::Format::parse(limits.max_display_field_depth, &event.fields)
    ...
    async fn extract(
        ...
        async {
            let limits: &Limits = ctx.data()?;
            let extract = myso_display::v2::Extract::parse(limits.display(), &path)
            ...
            let interpreter = myso_display::v2::Interpreter::new(root, store);
            ...
            let Some(myso_display::v2::OwnedSlice {
                layout,
                bytes: native,
    ...
    async fn format(
        ...
        async {
            let limits: &Limits = ctx.data()?;
            let parsed = myso_display::v2::Format::parse(limits.display(), &format)
...
#[async_trait]
impl<'f, 'r> myso_display::v2::Store for DisplayStore<'f, 'r> {
    async fn object(
        &self,
        id: AccountAddress,
        ...
        Ok(Some(myso_display::v2::OwnedSlice { layout, bytes }))
    }
}
...
fn format_error(
    wrap: impl FnOnce(myso_display::v2::FormatError) -> Error,
    e: myso_display::v2::FormatError,
) -> RpcError<Error> {
    use myso_display::v2::FormatError as FE;
    match &e {
