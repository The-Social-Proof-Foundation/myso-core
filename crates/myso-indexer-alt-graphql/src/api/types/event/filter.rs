...
impl EventFilter {
    ...
    pub(crate) fn matches(&self, event: &NativeEvent) -> bool {
        if let Some(sender) = &self.sender
            && sender != &MySoAddress::from(event.sender)
        {
            return false;
        }

        if let Some(module) = &self.module {
            if module.package() != MySoAddress::from(event.package_id) {
                return false;
            }
        ...
        if let Some(type_) = &self.type_ {
            if type_.package() != MySoAddress::from(event.type_.address) {
                return false;
            }
