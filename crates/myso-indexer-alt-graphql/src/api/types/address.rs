...
#[graphql(
    name = "IAddressable",
    field(name = "address", ty = "MySoAddress"),
    field(
        name = "address_at",
    ...
    field(
        ...
        desc = "Fetch the total balance for coins with marker type `coinType` (e.g. `0x2::myso::MYSO`), owned by this address.\n\nIf the address does not own any coins of that type, a balance of zero is returned.",
    ),
    field(
        name = "balances",
...
pub(crate) struct Address {
    ...
    pub(crate) address: NativeMySoAddress,
}
...
impl Address {
    ...
    pub(crate) async fn address(&self) -> Result<MySoAddress, RpcError> {
...
impl Address {
    ...
    ///
    /// If a name is provided, it is resolved to an address using MySoNS. If the name does not
    ...
    /// Construct an address that is represented by just its identifier (`MySoAddress`).
