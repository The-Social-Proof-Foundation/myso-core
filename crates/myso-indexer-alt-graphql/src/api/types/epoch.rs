...
use myso_indexer_alt_reader::cp_sequence_numbers::CpSequenceNumberKey;
use myso_indexer_alt_reader::epochs::CheckpointBoundedEpochStartKey;
use myso_indexer_alt_reader::epochs::EpochEndKey;
use myso_indexer_alt_reader::epochs::EpochStartKey;
use myso_indexer_alt_reader::pg_reader::PgReader;
use myso_indexer_alt_schema::cp_sequence_numbers::StoredCpSequenceNumbers;
use myso_indexer_alt_schema::epochs::StoredEpochEnd;
...
use myso_types::messages_checkpoint::CheckpointCommitment;
use myso_types::myso_system_state::MYSO_SYSTEM_STATE_INNER_MODULE_NAME;
use myso_types::myso_system_state::MYSO_SYSTEM_STATE_INNER_V1_STRUCT_NAME;
use myso_types::myso_system_state::MYSO_SYSTEM_STATE_INNER_V2_STRUCT_NAME;
...
impl Epoch {
    ...
    async fn coin_deny_list(&self, ctx: &Context<'_>) -> Option<Result<Object, RpcError>> {
        async {
            ...
            Object::latest(ctx, scope, MYSO_DENY_LIST_OBJECT_ID.into()).await
        }
        .await
        .transpose()
    }
...
impl Epoch {
    ...
    async fn system_state_impl(&self, ctx: &Context<'_>) -> Result<Option<MoveValue>, RpcError> {
        ...
        let struct_name = match start.system_state.first() {
            Some(0) => MYSO_SYSTEM_STATE_INNER_V1_STRUCT_NAME,
            Some(1) => MYSO_SYSTEM_STATE_INNER_V2_STRUCT_NAME,
            _ => {
                return Ok(None);
            }
        };
        ...
        let tag = TypeTag::Struct(Box::new(StructTag {
            ...
            module: MYSO_SYSTEM_STATE_INNER_MODULE_NAME.to_owned(),
