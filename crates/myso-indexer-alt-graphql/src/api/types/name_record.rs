...
use myso_name_service::Domain as NativeDomain;
use myso_name_service::NameRecord as NativeNameRecord;
...
async fn reverse_record(
    ...
    scope: Scope,
    address: NativeMySoAddress,
) -> Result<Option<Field<NativeMySoAddress, NativeDomain>>, RpcError> {
    let config: &NameServiceConfig = ctx.data()?;
