...
pub(crate) async fn replay_transaction<S: ReadDataStore>(
    ...
    if let myso_types::transaction::TransactionKind::ProgrammableTransaction(ptb) =
...
// version to Object.
// Returns also the packages of the type parameters instantiated (e.g. `MYSO` in `Coin<MYSO>`).
