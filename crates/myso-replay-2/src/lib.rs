...
/// Load replay configuration from ~/.myso/myso_config/replay.toml file.
...
pub fn load_config_file() -> Result<ReplayConfigStable> {
    let config_dir = match myso_config_dir() {
        Ok(dir) => dir,
        Err(e) => {
...
pub fn print_effects_or_fork<W: Write>(
    ...
    if manager.member(Artifact::ForkedTransactionEffects).exists() {
        ...
        writeln!(
            w,
            "{}",
            MySoTransactionBlockEffects::try_from(tx_effects.clone())
