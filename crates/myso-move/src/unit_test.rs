    ...
    pub async fn execute(
        ...
        if !cfg!(feature = "tracing") && compute_coverage {
            return Err(anyhow::anyhow!(
                ...
                Please build the MySo CLI from source with `--features tracing` to use this flag."
            ));
        }
        ...
        if build_config.default_flavor.is_none() {
            build_config.default_flavor = Some(move_compiler::editions::Flavor::MySo);
        }
...
impl Default for MySoVMTestSetup {
    fn default() -> Self {
        Self::new()
    }
}
...
impl VMTestSetup for MySoVMTestSetup {
    type Meter<'a> = MySoGasMeter<MySoGasStatusTestWrapper>;

    fn new_meter<'a>(&'a self, execution_bound: Option<u64>) -> Self::Meter<'a> {
        MySoGasMeter(MySoGasStatusTestWrapper(
            MySoGasStatus::new(
    ...
    fn vm_config(&self) -> VMConfig {
        myso_adapter::adapter::vm_config(&self.protocol_config)
    }
...
pub struct MySoGasStatusTestWrapper(MySoGasStatus);

impl Deref for MySoGasStatusTestWrapper {
...
impl DerefMut for MySoGasStatusTestWrapper {
