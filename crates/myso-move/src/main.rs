...
use myso_move::execute_move_command;
use myso_sdk::wallet_context::WalletContext;
use myso_types::exit_main;
use tracing::debug;
...
async fn main() {
    ...
    //     MySoCommand::Console { .. } | MySoCommand::Client { .. } => {
    ...
    let _guard = telemetry_subscribers::TelemetryConfig::new()
        ...
        .with_env()
        .init();
    debug!("MySo-Move CLI version: {VERSION}");

    exit_main!(execute(args).await);
}

async fn execute(args: Args) -> anyhow::Result<()> {
    let config = myso_config_dir()?.join(MYSO_CLIENT_CONFIG);
