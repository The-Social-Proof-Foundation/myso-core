...
impl Writer {
    ...
    pub async fn create_pool(&self, req: CreatePoolRequest) -> Result<(), OrderbookError> {
        let mut conn = self
            ...
            .connect()
            .await
            .map_err(|e| OrderbookError::database(e.to_string()))?;
    ...
    pub async fn update_pool(
        ...
        if rows_affected == 0 {
            return Err(OrderbookError::not_found(format!("pool {id}")));
        }
        Ok(())
    }

    pub async fn delete_pool(&self, id: &str) -> Result<(), OrderbookError> {
        let mut conn = self
            .db
            .connect()
            .await
    ...
    pub async fn create_asset(&self, req: CreateAssetRequest) -> Result<(), OrderbookError> {
        let mut conn = self
            .db
            .connect()
            .await
    ...
    pub async fn delete_asset(&self, id: &str) -> Result<(), OrderbookError> {
        let mut conn = self
            .db
            .connect()
            .await
        ...
        if rows_affected == 0 {
            return Err(OrderbookError::not_found(format!("asset {id}")));
        }
        Ok(())
    }
}
