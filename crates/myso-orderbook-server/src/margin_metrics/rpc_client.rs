...
impl MarginRpcClient {
    pub fn new(myso_client: MySoClient, margin_package_id: &str) -> Result<Self> {
    ...
    pub async fn get_pool_state(&self, pool_id: &str, asset_type: &str) -> Result<MarginPoolState> {
        ...
        let pool_object = self
            ...
            .get_object_with_options(
                pool_object_id,
                myso_json_rpc_types::MySoObjectDataOptions::full_content().with_owner(),
            )
            .await?;
    ...
    async fn query_pool_state(
        ...
        let clock_id = ObjectID::from_hex_literal(MYSO_CLOCK_OBJECT_ID)?;
    ...
    fn extract_u64(
        &self,
        results: &[myso_json_rpc_types::MySoExecutionResult],
        index: usize,
        func_name: &str,
    ) -> Result<u64> {
        let result = results
