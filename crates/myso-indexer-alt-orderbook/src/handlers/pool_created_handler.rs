    ...
    async fn process(&self, checkpoint: &Arc<Checkpoint>) -> anyhow::Result<Vec<Self::Value>> {
        ...
        for tx in &checkpoint.transactions {
            ...
            for (index, ev) in events.data.iter().enumerate() {
                if !PoolCreatedEvent::<MYSO, MYSO>::matches_event_type(&ev.type_, self.env) {
                    continue;
                }

                let event: PoolCreatedEvent<MYSO, MYSO> = bcs::from_bytes(&ev.contents)?;
                ...
                debug!("Observed Orderbook PoolCreated {:?}", data);
                results.push(data);
            }
        }
        Ok(results)
