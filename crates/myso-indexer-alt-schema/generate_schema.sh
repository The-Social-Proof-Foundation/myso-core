...
#
# Update myso-indexer's generated src/schema.rs based on the schema after
...
diesel migration run                                                          \
  ...
  --migration-dir "$REPO/crates/myso-pg-db/migrations"

diesel migration run                                                          \
  --database-url "postgres://postgres:postgrespw@localhost:$PORT"             \
  --migration-dir "$REPO/crates/myso-indexer-alt-schema/migrations"

# Generate the schema.rs file, excluding framework tables and including the
# copyright notice.
diesel print-schema                                                           \
  --database-url "postgres://postgres:postgrespw@localhost:$PORT"             \
  --patch-file "$REPO/crates/myso-indexer-alt-schema/schema.patch"             \
  > "$REPO/crates/myso-indexer-alt-schema/src/schema.rs"
