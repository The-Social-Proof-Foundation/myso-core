...
use myso_indexer_alt_consistent_api::proto::rpc::consistent::v1alpha::owner::OwnerKind;
...
async fn test_coin_balance_change_cleanup() {
    ...
    builder.transfer_myso(a, Some(500));
...
fn create_bag(cluster: &mut FullCluster, owner: MySoAddress, ty: TypeTag, size: u64) -> ObjectRef {
    let (sender, kp, gas) = cluster
