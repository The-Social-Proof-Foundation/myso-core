...
use myso_indexer_alt_jsonrpc::config::RpcConfig as JsonRpcConfig;
...
/// Tests happy path for MySoNS resolution.
...
/// Tests MySoNS resolution response when the domain has expired.
...
impl MySoNSCluster {
    /// Sets up a full cluster with a mock MySoNS package and registries. RPC is configured to read
    ...
    async fn new() -> Self {
        ...
        // (7) Configure the RPC to read from the mock MySoNS package. Everything else is configured
    ...
    /// Send a JSON-RPC request to the cluster to resolve the given MySoNS name.
    ...
    /// Send a JSON-RPC request to the cluster to resolve the MySoNS name for a given address.
