...
use myso_indexer_alt::config::PrunerLayer;
use myso_indexer_alt_graphql::config::RpcConfig as GraphQlConfig;
use myso_indexer_alt_graphql::config::WatermarkConfig;
...
use myso_indexer_alt_e2e_tests::find::address_owned;
...
fn transfer_dust(
    ...
    let mut builder = ProgrammableTransactionBuilder::new();
    builder.transfer_myso(recipient, Some(1));
...
async fn query_transactions(cluster: &FullCluster, affected_address: MySoAddress) -> Value {
    execute_graphql_query(
        cluster,
        TRANSACTIONS_QUERY,
        Some(json!({
