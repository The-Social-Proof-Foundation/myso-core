...
use myso_indexer_alt_consistent_api::proto::rpc::consistent::v1alpha::BatchGetBalancesRequest;
use myso_indexer_alt_consistent_api::proto::rpc::consistent::v1alpha::GetBalanceRequest;
use myso_indexer_alt_consistent_api::proto::rpc::consistent::v1alpha::ListBalancesRequest;
use myso_indexer_alt_consistent_api::proto::rpc::consistent::v1alpha::ListObjectsByTypeRequest;
...
async fn test_index_address_balance_accumulates() {
    ...
    cluster.send_myso_to_address_balance(a, 100);
    cluster.send_myso_to_address_balance(a, 200);
    cluster.send_myso_to_address_balance(a, 300);

    // Send to B's address balance
    cluster.send_myso_to_address_balance(b, 400);
    cluster.send_myso_to_address_balance(b, 500);
...
async fn test_multiple_coin_types() {
    ...
    cluster.send_myso_to_address_balance(recipient, 500);
    cluster.cluster.create_checkpoint().await;
    ...
    // Recipient should have both MYSO and MY_COIN address balances
    assert_eq!(
        cluster
...
async fn test_snapshot_consistency() {
    ...
    cluster.send_myso_to_address_balance(a, 100);
    cluster.send_myso_to_address_balance(a, 200);
    cluster.send_myso_to_address_balance(b, 300);
    cluster.cluster.create_checkpoint().await;
    ...
    cluster.send_myso_to_address_balance(a, 400);
    cluster.send_myso_to_address_balance(b, 500);
    cluster.cluster.create_checkpoint().await;
...
async fn test_address_to_address_transfer() {
    ...
    let (balances, _) = cluster
        ...
        .await
        .unwrap();
    // Single balance entry for MYSO/ gas
...
async fn test_edge_cases() {
    ...
    request
        .metadata_mut()
        .insert("x-myso-checkpoint", "10".parse().unwrap());
...
impl BalanceCluster {
    ...
    fn request_gas(&mut self, requester: MySoAddress, amount: u64) -> ObjectRef {
    ...
    /// Publisher mints MYSO to send to a recipient's address balance.
    fn send_myso_to_address_balance(&mut self, recipient: MySoAddress, amount: u64) {
        ...
        let tx =
            TestTransactionBuilder::new(self.publisher, gas, self.cluster.reference_gas_price())
                .with_gas_budget(DEFAULT_GAS_BUDGET)
                .transfer_myso_to_address_balance(FundSource::coin(gas), vec![(amount, recipient)])
                .build();
    ...
    async fn list_balances(
        ...
        if let Some(checkpoint) = checkpoint {
            request
                .metadata_mut()
                .insert("x-myso-checkpoint", checkpoint.to_string().parse().unwrap());
        }
    ...
    async fn batch_get_balances(
        &mut self,
        requests: Vec<(MySoAddress, String)>,
        checkpoint: Option<u64>,
...
async fn paginate_list_balances(
    ...
    myso_indexer_alt_consistent_api::proto::rpc::consistent::v1alpha::ListBalancesResponse,
    tonic::Status,
> {
