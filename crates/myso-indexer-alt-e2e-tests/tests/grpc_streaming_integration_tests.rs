...
use myso_indexer_alt_framework::pipeline::concurrent::{self};
use myso_pg_db::Db;
...
async fn transfer_coin(
    wallet: &mut myso_sdk::wallet_context::WalletContext,
) -> myso_types::base_types::TransactionDigest {
    ...
    let tx_data = TestTransactionBuilder::new(sender, gas_obj, 1000)
        .transfer_myso(Some(1000), sender)
        .build();
