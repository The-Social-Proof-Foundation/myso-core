...
use myso_transactional_test_runner::create_adapter;
...
impl OffchainStateReader for OffchainReader {
    ...
    async fn execute_graphql(
        ...
        if show_usage {
            request = request.header(HeaderName::from_static("x-myso-rpc-show-usage"), "true");
        }
...
async fn run_test(path: &Path) -> Result<(), Box<dyn Error>> {
    ...
    let (output, mut adapter) =
        create_adapter::<MySoTestAdapter>(path, Some(Arc::new(PRE_COMPILED.clone()))).await?;
