...
use myso_indexer_alt_jsonrpc::NodeArgs;
use myso_indexer_alt_jsonrpc::RpcArgs;
use myso_indexer_alt_jsonrpc::args::SystemPackageTaskArgs;
use myso_indexer_alt_jsonrpc::config::RpcConfig;
...
impl FnDelegationTestCluster {
    ...
    async fn transfer_transaction(&self) -> anyhow::Result<(String, String, Vec<String>)> {
        ...
        let tx = self
            ...
            .await
            .transfer_myso(Some(1_000), recipient)
            .build();
    ...
    async fn privileged_transaction(&self) -> anyhow::Result<(String, String, Vec<String>)> {
        let tx: myso_types::transaction::TransactionData = self
            .onchain_cluster
            .test_transaction_builder()
            .await
    ...
    async fn get_validator_address(&self) -> MySoAddress {
...
async fn test_execution() {
    ...
    let response = test_cluster
        .execute_jsonrpc(
            "myso_executeTransactionBlock".to_string(),
            json!({
                "tx_bytes": tx_bytes,
                "signatures": sigs,
                "options": {
...
async fn test_dry_run() {
    ...
    let response = test_cluster
        .execute_jsonrpc(
            "myso_dryRunTransactionBlock".to_string(),
            json!({
                "tx_bytes": tx_bytes,
            }),
        )
...
async fn test_get_stakes_and_by_ids() {
    ...
    assert!(get_stakes_response["result"][0]["stakes"][0]["stakedMySoId"].is_string());
    let stake_id = get_stakes_response["result"][0]["stakes"][0]["stakedMySoId"]
        .as_str()
        .unwrap();
    ...
    let get_stakes_by_ids_response = test_cluster
        .execute_jsonrpc(
            "mysox_getStakesByIds".to_string(),
            json!({ "staked_myso_ids": [stake_id] }),
        )
        .await
        .unwrap();
...
async fn test_get_stakes_invalid_params() {
    ...
    let response = test_cluster
        .execute_jsonrpc(
            "mysox_getStakesByIds".to_string(),
            json!({ "staked_myso_ids": ["invalid_stake_id"] }),
        )
        .await
        .unwrap();
...
async fn test_get_validators_apy() {
    ...
    let response = test_cluster
        .execute_jsonrpc("mysox_getValidatorsApy".to_string(), json!({}))
        .await
        .unwrap();
