...
async fn test_simulate_transaction_basic() {
    ...
    let signed_tx =
        make_transfer_myso_transaction(&validator_cluster.wallet, Some(recipient), Some(1_000_000))
            .await;
...
async fn test_simulate_transaction_object_changes() {
    ...
    let output_type = output_move_obj
        ...
        .as_str()
        .unwrap();
    let myso_coin_type = GasCoin::type_().to_canonical_string(true);

    // Gas coin versions should be simulated correctly
    assert_eq!(input_state.version, 1);
    assert_eq!(output_state.version, 2);

    // Both should be MYSO coins
    assert_eq!(input_type, myso_coin_type);
    assert_eq!(output_type, myso_coin_type);
    ...
    // Created object should be MYSO coins
    ...
    let created_type = created_move_obj
        ...
        .as_str()
        .unwrap();
    assert_eq!(created_type, myso_coin_type);
}
...
async fn test_simulate_transaction_command_results() {
    ...
    let gas_objects = validator_cluster
        ...
        .await
        .unwrap();
    let tx_data = myso_types::transaction::TransactionData::new_programmable(
...
async fn test_simulate_transaction_balance_changes() {
    ...
    let signed_tx = make_transfer_myso_transaction(
...
async fn test_simulate_transaction_with_gas_selection() {
    ...
    let tx_data: myso_types::transaction::TransactionData = bcs::from_bytes(&tx_bytes).unwrap();
