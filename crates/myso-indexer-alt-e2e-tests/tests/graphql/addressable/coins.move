...
{ # Account A queries
  accountA: address(address: "@{A}") {
    ...
    # Only MYSO coin objects
    mysoCoins: objects(filter: { type: "0x2::coin::Coin<0x2::myso::MYSO>" }) {
      pageInfo { hasNextPage }
      nodes { ...O }
    }
    ...
    mysoBalance: balance(coinType: "0x2::myso::MYSO") { ...B }
