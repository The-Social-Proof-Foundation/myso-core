...
use myso_indexer_alt_e2e_tests::coin_registry::{self};
...
query GetObjects($owner: MySoAddress!, $after: String) {
...
async fn test_myso() {
    ...
    let metadata = query_metadata(&cluster, "0x2::myso::MYSO").await;
...
async fn test_fixed_supply() {
    ...
    let currency = find::address_owned_by(&fx, MYSO_COIN_REGISTRY_ADDRESS.into()).unwrap();
...
async fn query_owned_outputs(cluster: &FullCluster, owner: MySoAddress) -> LegacyCoinOutputs {
...
async fn query_objects(cluster: &FullCluster, owner: MySoAddress) -> Vec<(StructTag, ObjectRef)> {
    #[derive(Deserialize)]
    #[serde(rename_all = "camelCase")]
    struct PageInfo {
