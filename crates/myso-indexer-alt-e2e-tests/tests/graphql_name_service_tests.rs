...
use myso_indexer_alt_graphql::config::RpcConfig;
use myso_indexer_alt_jsonrpc::config::NameServiceConfig;
...
/// Tests happy path for MySoNS resolution (forward lookup).
...
/// Tests MySoNS resolution response when the domain doesn't exist or has expired.
...
async fn test_resolve_domain() {
    let mut c = MySoNSCluster::new().await;

    let nft = ObjectID::random();
    let target = MySoAddress::random_for_testing_only();
    c.add_domain(nft, &["myso", "foo"], Some(target), 1000)
        .await
...
async fn test_resolve_domain_no_target() {
    ...
    let nft = ObjectID::random();
    c.add_domain(nft, &["myso", "foo"], None, 1000)
        .await
        .expect("Failed to add domain");
...
async fn test_resolve_domain_expiry() {
    ...
    let target = MySoAddress::random_for_testing_only();
    let expiry_ms = 1000;
    c.add_domain(nft, &["myso", "foo"], Some(target), expiry_ms)
        .await
        .expect("Failed to add domain");
...
async fn test_resolve_subdomain() {
    ...
    c.add_domain(nft, &["myso", "foo", "bar"], Some(target), 0)
        .await
...
async fn test_resolve_subdomain_parent_expiry() {
    ...
    c.add_domain(nft, &["myso", "foo"], None, expiry_ms)
        .await
        .expect("Failed to add parent domain");
...
async fn test_resolve_subdomain_expiry() {
    ...
    c.add_domain(parent_nft, &["myso", "foo"], None, parent_expiry_ms)
        .await
        .expect("Failed to add parent domain");

    c.add_domain(nft, &["myso", "foo", "bar"], Some(target), expiry_ms)
        .await
        .expect("Failed to add subdomain");
...
async fn test_resolve_subdomain_bad_parent() {
    ...
    c.add_domain(nft0, &["myso", "foo"], None, 1000)
        .await
        .expect("Failed to add parent domain");

    c.add_domain(nft1, &["myso", "foo", "bar"], Some(target), 0)
        .await
        .expect("Failed to add subdomain");
...
/// Test looking up an address by its MySoNS name via Query.address(name: ...).
#[tokio::test]
...
async fn test_address_both_identifiers_error() {
    ...
    let resp = c
        .query(
            r#"
            query($address: MySoAddress!, $name: String!) {
...
impl MySoNSCluster {
    /// Sets up a full cluster with a mock MySoNS package and registries. GraphQL is configured to read
    /// from these packages and registries to resolve MySoNS names.
    ///
    ...
    async fn new() -> Self {
        ...
        // (2) Compile the mock MySoNS package.
        let mut path = PathBuf::from(env!("CARGO_MANIFEST_DIR"));
        path.extend(["packages", "mysons"]);
        ...
        // (4) Publish the mock MySoNS package.
        ...
        // (7) Configure the GraphQL RPC to read from the mock MySoNS package.
    ...
    /// Send a GraphQL request to the cluster to resolve the given MySoNS name.
    ...
    /// Send a GraphQL request to the cluster to resolve the MySoNS name for a given address.
    async fn resolve_name(&self, addr: MySoAddress) -> anyhow::Result<Value> {
        self.query(
            r#"
            query($address: MySoAddress!) {
    ...
    /// Send a GraphQL request to resolve an address by its MySoNS name.
