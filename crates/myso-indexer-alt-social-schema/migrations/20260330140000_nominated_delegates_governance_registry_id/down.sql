...
LEFT JOIN nominated_delegates n ON g.registry_type = n.registry_type AND n.status = 0
...
CREATE UNIQUE INDEX IF NOT EXISTS idx_nominees_address_type_time
    ON nominated_delegates (address, registry_type, time);

ALTER TABLE nominated_delegates DROP COLUMN IF EXISTS governance_registry_id;
