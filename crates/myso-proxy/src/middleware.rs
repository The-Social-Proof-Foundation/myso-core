...
use crate::{consumer::ProtobufDecoder, peers::MySoNodeProvider};
use axum::{
    body::Body,
    body::Bytes,
...
/// we expect myso-node to send us an http header content-length encoding.
...
/// we expect myso-node to send us an http header content-type encoding.
...
/// we expect that calling myso-nodes are known on the blockchain and we enforce
/// their pub key tls creds here
pub async fn expect_valid_public_key(
    Extension(allower): Extension<Arc<MySoNodeProvider>>,
