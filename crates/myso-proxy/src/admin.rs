...
use crate::peers::{AllowedPeer, MySoNodeProvider};
use crate::var;
use anyhow::Error;
use anyhow::Result;
...
pub fn app(
    ...
    allower: Option<MySoNodeProvider>,
...
pub fn create_server_cert_default_allow(
    hostname: String,
) -> Result<ServerConfig, myso_tls::rustls::Error> {
    let CertKeyPair(server_certificate, _) = generate_self_cert(hostname);

    ClientCertVerifier::new(AllowAll, MYSO_VALIDATOR_SERVER_NAME.to_string()).rustls_server_config(
        vec![server_certificate.rustls_certificate()],
        server_certificate.rustls_private_key(),
    )
}

/// Verify clients against myso blockchain, clients that are not found in myso_getValidators
/// will be rejected
pub fn create_server_cert_enforce_peer(
    dynamic_peers: DynamicPeerValidationConfig,
    static_peers: Option<StaticPeerValidationConfig>,
) -> Result<(ServerConfig, Option<MySoNodeProvider>), myso_tls::rustls::Error> {
    let (Some(certificate_path), Some(private_key_path)) =
        (dynamic_peers.certificate_file, dynamic_peers.private_key)
    else {
        return Err(myso_tls::rustls::Error::General(
            "missing certs to initialize server".into(),
        ));
    };
    let static_peers = load_static_peers(static_peers).map_err(|e| {
        myso_tls::rustls::Error::General(format!("unable to load static pub keys: {}", e))
    })?;
    let allower = MySoNodeProvider::new(dynamic_peers.url, dynamic_peers.interval, static_peers);
    allower.poll_peer_list();
    let c = ClientCertVerifier::new(allower.clone(), MYSO_VALIDATOR_SERVER_NAME.to_string())
