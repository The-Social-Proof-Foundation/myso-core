...
mod tests {
    ...
    use crate::{admin::CertKeyPair, config::RemoteWriteConfig, peers::MySoNodeProvider};
    use axum::Router;
    ...
    use myso_tls::{ClientCertVerifier, TlsAcceptor};
    ...
    /// myso-node client that posts data to the proxy using the protobuf format.  The server processes this
    ...
    async fn test_axum_acceptor() {
        ...
        let CertKeyPair(client_priv_cert, client_pub_key) = admin::generate_self_cert("myso".into());
        ...
        let mut allower = MySoNodeProvider::new("".into(), Duration::from_secs(30), vec![]);
        ...
        allower.get_myso_mut().write().unwrap().insert(
