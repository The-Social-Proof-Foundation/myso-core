...
/// MySoNodeProvider queries the myso blockchain and keeps a record of known validators based on the response from
/// myso_getValidators.  The node name, public key and other info is extracted from the chain and stored in this
...
#[derive(Debug, Clone)]
pub struct MySoNodeProvider {
    myso_nodes: AllowedPeers,
...
impl MySoNodeProvider {
    pub fn new(
        ...
        let myso_nodes = Arc::new(RwLock::new(HashMap::new()));
    ...
    pub fn get(&self, key: &Ed25519PublicKey) -> Option<AllowedPeer> {
        ...
        // check myso validators
        if let Some(v) = self.myso_nodes.read().unwrap().get(key) {
            return Some(AllowedPeer {
                name: v.name.to_owned(),
                public_key: v.public_key.to_owned(),
            });
    ...
    /// Get a mutable reference to the allowed myso validator map
    pub fn get_myso_mut(&mut self) -> &mut AllowedPeers {
        &mut self.myso_nodes
    }

    /// get_validators will retrieve known validators
    async fn get_validators(url: String) -> Result<MySoSystemStateSummary> {
        let rpc_method = "mysox_getLatestMySoSystemState";
        let observe = || {
    ...
    async fn get_bridge_validators(url: String) -> Result<BridgeSummary> {
        let rpc_method = "mysox_getLatestBridge";
    ...
    async fn update_myso_validator_set(&self) {
        match Self::get_validators(self.rpc_url.to_owned()).await {
            Ok(summary) => {
                let validators = extract(summary);
                let mut allow = self.myso_nodes.write().unwrap();
                allow.clear();
                allow.extend(validators);
                info!(
                    "{} myso validators managed to make it on the allow list",
                    allow.len()
                );
            }
            Err(error) => {
    ...
    async fn update_bridge_validator_set(&self, metrics_keys: MetricsPubKeys) {
        let myso_system = match Self::get_validators(self.rpc_url.to_owned()).await {
            Ok(summary) => summary,
            Err(error) => {
                JSON_RPC_STATE
                    .with_label_values(&["update_bridge_peer_count", "failed"])
                    .inc();
                error!("unable to get myso system state: {error}");
                return;
            }
        };
        match Self::get_bridge_validators(self.rpc_url.to_owned()).await {
            Ok(summary) => {
                let names = myso_system
                    .active_validators
                    .into_iter()
                    .map(|v| (v.myso_address, v.name))
                    .collect();
            ...
            Err(error) => {
                JSON_RPC_STATE
                    .with_label_values(&["update_bridge_peer_count", "failed"])
                    .inc();
                error!("unable to refresh myso bridge peer list: {error}");
            }
        };
    }
    ...
    pub fn poll_peer_list(&self) {
        ...
        tokio::spawn(async move {
            ...
            loop {
                ...
                cloned_self.update_myso_validator_set().await;
                cloned_self
...
/// extract will get the network pubkey bytes from a MySoValidatorSummary type.  This type comes from a
...
fn extract(
    summary: MySoSystemStateSummary,
    ...
    summary.active_validators.into_iter().filter_map(|vm| {
        match Ed25519PublicKey::from_bytes(&vm.network_pubkey_bytes) {
            Ok(public_key) => {
                debug!(
                    "adding public key {:?} for myso validator {:?}",
                    public_key, vm.name
                );
                Some((
                    public_key.clone(),
            ...
            Err(error) => {
                error!(
                    "unable to decode public key for name: {:?} myso_address: {:?} error: {error}",
                    vm.name, vm.myso_address
                );
...
async fn extract_bridge(
    ...
    names: Arc<BTreeMap<MySoAddress, String>>,
    ...
    let results: Vec<_> = stream::iter(committee_members)
        .filter_map(|(_, cm)| {
            ...
            async move {
                ...
                let bridge_name = names.get(&cm.myso_address).cloned().unwrap_or_else(|| {
                    warn!(
                        address =% cm.myso_address,
                        "Bridge node not found in myso committee, using base URL as the name",
                    );
...
mod tests {
    ...
    #[test]
    fn depend_on_myso_myso_system_state_summary() {
        let CertKeyPair(_, client_pub_key) = generate_self_cert("myso".into());
        // all fields here just satisfy the field types, with exception to active_validators, we use
        // some of those.
        let depends_on = MySoSystemStateSummary {
            active_validators: vec![MySoValidatorSummary {
        ...
        let r = serde_json::to_string(&ResponseBody { result: depends_on })
            .expect("expected to serialize ResponseBody{MySoSystemStateSummary}");

        let deserialized = serde_json::from_str::<ResponseBody>(&r)
            .expect("expected to deserialize ResponseBody{MySoSystemStateSummary}");
