...
impl TransactionSponsorship {
    pub fn select_gas(
        ...
        exec: &mut Executor,
        gas_coins: u32,
    ) -> (Vec<ObjectRef>, (u64, Object), MySoAddress) {
        match self {
...
impl AUTransactionGen for P2PTransferGenRandomGasRandomPriceRandomSponsorship {
    fn apply(
        ...
        // construct a p2p transfer of a random amount of MYSO
        let txn = {
            let mut builder = ProgrammableTransactionBuilder::new();
            builder.transfer_myso(recipient.initial_data.account.address, Some(self.amount));
            builder.finish()
        };
        ...
        let status: Result<ExecutionStatus, MySoError> = match run_info {
            RunInfo {
                enough_max_gas: true,
