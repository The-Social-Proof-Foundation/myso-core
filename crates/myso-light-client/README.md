This crate contains a Command Line Interface light client for MySo.
...
Alternatively, a *light client* only needs to download minimal information to authenticate blockchain state. Specifically in MySo, the light client needs to *sync* all end-of-epoch checkpoints that contain information about the committee in the next epoch. Sync involves downloading the checkpoints and checking their validity by checking their certificate. 
...
The config file for the light client takes a URL for a full node, a directory to store checkpoint summaries (that must exist) and within the directory to name of the genesis blob for the MySo network. 

```
full_node_url: "https://fullnode.mainnet.mysocial.network:443"
...
object_store_url: "https://checkpoints.mainnet.mysocial.network"
graphql_url: "https://myso-mainnet.mystenlabs.com/graphql"
...
```
$ myso-light-client --config mainnet.yaml sync
```
...
```
$ myso-light-client --config mainnet.yaml transaction -t 8RiKBwuAbtu8zNCtz8SrcfHyEUzto6zi6cMVA9t4WhWk
```
...
```
$ myso-light-client --config light_client.yaml object -o 0xa514c85e1844189a54f4bfabc0928cbcac2137b928bef61adade84bbb486fd1f
```
