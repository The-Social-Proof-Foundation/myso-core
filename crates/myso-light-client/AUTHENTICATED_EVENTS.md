...
Authenticated Events provide a cryptographically verifiable stream of Move events from MySo smart contracts. Unlike regular events, authenticated events can be verified by a light client without trusting any intermediary, making them mysotable for applications requiring trustless event consumption.

## Why Authenticated Events?

Regular MySo events are indexed and queryable, but verifying them requires trusting the RPC provider. Authenticated events solve this by:

1. **Cryptographic Commitments**: Events are committed into a Merkle Mountain Range (MMR) structure stored on-chain
2. **Checkpoint Verification**: The commitment is tied to MySo checkpoints, which are signed by the validator committee
...
Authenticated events are fully backwards-compatible with regular MySo events. An authenticated event is simply a regular event with additional metadata for verification — existing event consumers will continue to work unchanged.
...
let stream_id = MySoAddress::from(package_id);
