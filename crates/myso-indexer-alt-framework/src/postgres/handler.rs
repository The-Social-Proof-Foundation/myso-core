...
//!
//! # TODO: Consider moving FieldCount to myso-pg-db
//!
//! Currently, FieldCount lives in this framework crate but is Postgres-specific. Ideally it should
//! live in myso-pg-db. However, this creates a circular dependency:
//! - myso-indexer-alt-framework depends on myso-pg-db (for IndexerCluster and other utilities)
//! - This blanket impl needs FieldCount to implement concurrent::Handler for postgres indexers
//! - Moving FieldCount to myso-pg-db would require framework to depend on myso-pg-db (circular!)
//!
//! To fully decouple, we'd need to move all postgres-specific code (including IndexerCluster) to
//! myso-pg-db, which would be a much larger breaking change. Consider this for a future refactor.
//!
//! See: <https://github.com/the-social-proof-foundation/myso-core/pull/24055#issuecomment-3471278182>
