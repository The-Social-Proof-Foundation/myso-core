...
import {
  ...
  isValidMySoObjectId,
  MIST_PER_MYSO,
} from '@mysten/myso/utils';
import { bcs } from '@mysten/myso/bcs';
import { program } from 'commander';
import { KIOSK_LISTING, KioskClient, KioskTransaction, Network } from '@mysten/kiosk';
import { MySoClient, getFullnodeUrl } from '@mysten/myso/client';
...
const KNOWN_TYPES = {
  mysofren:
    '0x80d7de9c4a56194087e0ba0bf59492aa8e6a5ee881606226930827085ddf2332::mysofrens::MySoFren<0x80d7de9c4a56194087e0ba0bf59492aa8e6a5ee881606226930827085ddf2332::capy::Capy>',
};

/** JsonRpcProvider for the Testnet */
const client = new MySoClient({ url: getFullnodeUrl('testnet') });
...
program
  .command('list')
  .description('list an item in the Kiosk for the specified amount of MYSO')
  .argument('<item ID>', 'The ID of the item to list')
  .argument('<amount MIST>', 'The amount of MYSO to list the item for')
  .action(listItem);
...
async function newKiosk() {
  const sender = keypair.getPublicKey().toMySoAddress();
...
async function showInventory({ address, onlyDisplay, cursor, filter }) {
  const owner = address || keypair.getPublicKey().toMySoAddress();

  if (!isValidMySoAddress(owner)) {
    throw new Error(`Invalid MYSO address: "${owner}"`);
  }
...
async function showKioskContents({ id, address }) {
  ...
  if (id) {
    if (!isValidMySoObjectId(id)) {
      throw new Error(`Invalid Kiosk ID: "${id}"`);
    }

    kioskId = id;
  } else {
    const sender = address || keypair.getPublicKey().toMySoAddress();

    if (!isValidMySoAddress(sender)) {
      throw new Error(`Invalid MYSO address: "${sender}"`);
    }
  ...
  const tabledItems = items
    .map((item) => ({
      ...
      'price (MYSO)': item.listing ? formatAmount(item.listing.price) : 'N/A',
    }))
...
async function placeItem(itemId) {
  const kioskCap = await findKioskCap().catch(() => null);
  const owner = keypair.getPublicKey().toMySoAddress();

  if (kioskCap === null) {
    throw new Error('No Kiosk found for sender; use `new` to create one');
  }

  if (!isValidMySoObjectId(itemId)) {
...
async function takeItem(itemId, { address }) {
  const cap = await findKioskCap().catch(() => null);
  const receiver = address || keypair.getPublicKey().toMySoAddress();
...
/**
 * Command: `list`
 * Description: Lists an item in the Kiosk for the specified amount of MYSO
 */
...
async function purchaseItem(itemId, opts) {
  ...
  if (inputKioskId && !isValidMySoObjectId(inputKioskId)) {
...
function formatAmount(amount) {
  ...
  if (amount <= MIST_PER_MYSO) {
    return Number(amount) / Number(MIST_PER_MYSO);
  }
