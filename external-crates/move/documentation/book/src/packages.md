...
# Additional fields may be added to this section by external tools. E.g., on MySo the following sections are added:
...
tools. For example, the MySo package manager adds toolchain version information
...
flavor = <string> # The flavor of the Move compiler used to build the package, e.g. "myso"
```
...
`myso move <command> <command_flags>`. Unless a particular path is provided, all
package commands will run in the current working directory. The full list of
commands and flags for the Move CLI can be found by running `myso move --help`.
