...
pub fn build_for_driver<W: Write + Send, T, F: MoveFlavor>(
    ...
    let myso_mode = build_config.default_flavor == Some(Flavor::MySo);
    let flags = compiler_flags(build_config);
    let mut compiler = Compiler::from_package_paths(vfs_root, package_paths, vec![])
        .unwrap()
        .set_flags(flags);
    if myso_mode {
        let (filter_attr_name, filters) = myso_mode::linters::known_filters();
        compiler = compiler
            .add_custom_known_filters(filter_attr_name, filters)
            .add_visitors(myso_mode::linters::linter_visitors(lint_level))
    }
...
pub fn make_deps_for_compiler<W: Write + Send, F: MoveFlavor>(
    ...
    for pkg in packages.into_iter() {
        ...
        let config = PackageConfig {
            ...
            flavor: Flavor::from_str(pkg.flavor().unwrap_or("myso"))?,
