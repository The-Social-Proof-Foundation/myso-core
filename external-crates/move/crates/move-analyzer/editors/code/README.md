...
the `myso` binary, and restart VSCode.
...
If your `move-analyzer` binary is in a different directory than the default one (`~/.myso/bin`
on macOS or Linux, or `C:\Users\USER\.myso\bin` on Windows), you may have the extension look
...
move-analyzer binary already exists in the default location (`~/.myso/bin` on macOS or Linux, or
`C:\Users\USER\.myso\bin` on Windows), delete the existing move-analyzer binary and reinstall the
extension.
...
- If the opened Move source file is located within a buildable project, and you have the `myso`
