...
for DIST_OS VSCODE_OS in "${(@kv)SUPPORTED_OS}"; do
    # MySo distribution identifier
    MYSO_DISTRO=$NETWORK"-v"$VERSION
    # name of the MySo distribution archive file, for example myso-testnet-v1.13.0-macos-arm64.tgz
    MYSO_ARCHIVE="myso-"$MYSO_DISTRO"-"$DIST_OS".tgz"
    # a path to downloaded MySo archive
    ...
    # and the one in the MySo archive ($ARCHIVE_SERVER_BIN)
