...
export async function run(): Promise<void> {
    ...
    return new Promise((resolve, reject) => {
        // The test mysote is composed of all files ending with '.test.js'.
        glob('**/**.test.js', { cwd: testsRoot }, (err, files: ReadonlyArray<string>) => {
            if (err) {
                return reject(err);
            }

            // Add each file to the test mysote.
            files.forEach(f => mysote.addFile(path.resolve(testsRoot, f)));

            // Run the test mysote. Uncaught exceptions or a non-zero number of
            // test rejectures is considered a test mysote rejecture.
            try {
                return mysote.run(failures => {
                    if (failures > 0) {
