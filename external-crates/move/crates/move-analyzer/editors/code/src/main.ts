...
async function mysoMoveCmd(context: Readonly<Context>, cmd: string): Promise<void> {
    const version = childProcess.spawnSync(
        context.configuration.mysoPath, ['--version'], { encoding: 'utf8' },
    );
    if (version.stdout) {
        const pkgRoot = await findPkgRoot();
        if (pkgRoot !== undefined) {
            const terminalName = 'myso move';
            ...
            terminal.sendText(`${context.configuration.mysoPath} move ${cmd}`, true);
        }
    } else {
        await vscode.window.showErrorMessage(
            `A problem occurred when executing the MySo command: '${context.configuration.mysoPath}'`
            + 'Make sure that MySo CLI is installed and available, either in your global PATH, '
            + 'or on a path set via `move.myso.path` configuration option.',
        );
    }
}
...
async function buildProject(context: Readonly<Context>): Promise<void> {
    return mysoMoveCmd(context, 'build');
}
...
async function testProject(context: Readonly<Context>): Promise<void> {
    ...
    if (filter !== undefined) {
        ...
        return mysoMoveCmd(context, cmd);
    }
