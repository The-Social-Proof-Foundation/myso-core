...
export const MYSO_PATH_OPT = 'myso.path';
...
export class Configuration {
    ...
    constructor() {
        ...
        this.defaultServerDir = vscode.Uri.joinPath(vscode.Uri.file(os.homedir()), '.myso', 'bin');
    ...
    /** The path to the MySo binary. */
    get mysoPath(): string {
        const mysoBin = process.platform === 'win32' ? 'myso.exe' : 'myso';
        const mysoPath = this.configuration.get<string | null>(MYSO_PATH_OPT) ?? mysoBin;

        if (mysoPath === mysoBin) {
            return mysoPath;
        }
        if (mysoPath.startsWith('~/')) {
            return os.homedir() + mysoPath.slice('~'.length);
        }
        return path.resolve(mysoPath);
    }
