...
import {
    ...
    MYSO_PATH_OPT, SERVER_PATH_OPT, FORCE_BUNDLED_OPT, Configuration,
    AUTO_IMPORTS_OPT,
} from './configuration';
...
export class Context {
    ...
    registerOnDidChangeConfiguration(): void {
        vscode.workspace.onDidChangeConfiguration(async event => {
            ...
            const myso_path_conf = MOVE_CONF_NAME.concat('.').concat(MYSO_PATH_OPT);
            ...
            const pathsChanged = event.affectsConfiguration(server_path_conf) ||
                event.affectsConfiguration(myso_path_conf) ||
    ...
    async installServerBinary(extensionContext: vscode.ExtensionContext): Promise<boolean> {
        ...
        const cliVersionString = version(this.configuration.mysoPath, cliVersionArgs);
        ...
        if (cliVersion !== null && (highestVersion === null || semver.gt(cliVersion, highestVersion))) {
            if (bundledVersionString === null || bundledVersion === null || !this.configuration.forceBundled) {
                // Even if there is a `myso` binary on the path that has a higher version than the bundled one,
                ...
                this.resolvedServerPath = this.configuration.mysoPath;
                ...
                log.info(`User has requested to use the bundled move-analyzer v${bundledVersion.version}` +
                    ` over the CLI one v${cliVersion.version} installed at '${this.configuration.mysoPath}'`);
            }
        }
