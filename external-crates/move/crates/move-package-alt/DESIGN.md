...
provide one. By default we will use the current chain ID from `myso client` (which we will keep
...
1. The user explicitly asks for it by running `myso move update-deps`. This command will repin all
...
4. If there are multiple environments `e1`, `e2` in the manifest having chain ID `i`, we produce an
   ...
   `testnet`.
    ...
    > available. Run `myso move build --build-env e1` or `myso move build --build-env e2`
...
6. If there is no `Pub.e.toml` file, we produce an error, depending on the command:
   For build:
    ...
    >
    >   myso move build --build-env testnet
    >
    > Note: adding local networks to `Move.toml` is discouraged; see `myso client test-publish --help`
...
best-effort on that. We may provide an additional tool to help fix things up (e.g. `myso move
...
manifest. Each flavor can specify its default system dependencies (for MySo, that's `myso` and `std`).
Non-default system dependencies can be specified like that: `system_dependencies = ["myso", "std", "myso_system"]`
...
If the user runs `myso move update-deps`, we rerun resolution, pinning, fetching, and validation for
all dependencies. If they run `myso move update-deps d1 d2` we rerun these steps only for the
...
We will add a new command `myso client test-publish <pubfile>` which will publish build the root
package for one environment and then publish it to a chain using the addresses from the ephemeral
publication file.

`myso client test-publish <pubfile> --build-env <env>` will work as follows:
 ...
 - if `--build-env <env>` is omitted, it defaults to the `build-env` name of the file at `<file>`;
   ...
   > example
   >
   >   myso client test-publish <pubfile> --build-env testnet
   >
...
of examples post-bootcamp.
 ...
 - Crazy example: https://github.com/pyth-network/pyth-crosschain/tree/main/target_chains/myso/contracts
 - Left this in the tooling meeting notes, but for anyone else interested, this is the MySoGPT Move
   Dataset v1:
   [https://github.com/CMU-MySoGPT/myso-move-dataset-v1](https://github.com/CMU-MySoGPT/myso-move-dataset-v1)
...
## Impact on the packages we deploy (e.g. mysofrens)
