...
mod tests {
    ...
    async fn test_modern_using_legacy_framework() {
        let scenario = TestPackageGraph::new(["root"])
            ...
            .add_package("myso", |pkg| pkg.set_legacy().set_legacy_name("MySo"))
            .add_package("myso_system", |pkg| {
            ...
            .add_dep("myso", "std", |dep| dep.name("MoveStdLib"))
            // legacy -> legacy case (MySoSystem -> MoveStdLib (std)
            .add_dep("myso_system", "myso", |dep| dep.name("MySo"))
            .add_dep("myso_system", "std", |dep| dep.name("MoveStdLib"))
            .build();
        ...
        scenario
            .graph_for("myso_system")
            .await
            .check_rename_from()
            .unwrap();
    }

    /// modern package `bat` depends on legacy package `myso` which has legacy name `MySo` (capital S).
    ///
    /// The dependency is named `MySo`. This should be disallowed, since the legacy name shouldn't
    /// appear in a modern manifest
    #[test(tokio::test)]
    async fn modern_uses_legacy_name() {
        let scenario = TestPackageGraph::new(["bat"])
            .add_package("myso", |pkg| pkg.set_legacy().set_legacy_name("MySo"))
            .add_dep("bat", "myso", |dep| dep.name("MySo"))
            .build();

        assert_snapshot!(scenario.graph_for("bat").await.check_rename_from().unwrap_err(), @r###"
        In Move.toml, the dependency `MySo` refers to a package named `myso`. Consider renaming the dependency to `myso`:

            myso = { local = "../myso", ... }

        Alternatively, if you want to refer to `myso` as `MySo` in your source code, add a `rename-from` field to the dependency:

            MySo = { local = "../myso", rename-from = "myso", ... }
        "###);
    }
