...
const MYSO_MODE_DIR: &str = "myso_mode";
...
fn default_testing_addresses(flavor: Flavor) -> BTreeMap<String, NumericalAddress> {
    ...
    if flavor == Flavor::MySo {
        mapping.extend([("myso", "0x2"), ("myso_system", "0x3")]);
    }
    mapping
        .into_iter()
...
fn test_config(path: &Path) -> (TestKind, TestInfo, PackageConfig, Flags) {
    ...
    let flavor = if path_contains(MYSO_MODE_DIR) {
        Flavor::MySo
    } else {
        Flavor::default()
    };
...
pub fn run_test(path: &Path) -> datatest_stable::Result<()> {
    ...
    if flavor == Flavor::MySo {
        let (prefix, filters) = myso_mode::linters::known_filters();
        compiler = compiler.add_custom_known_filters(prefix, filters);
        if test_info.lint {
            compiler = compiler.add_visitors(myso_mode::linters::linter_visitors(LintLevel::All))
        }
    }
