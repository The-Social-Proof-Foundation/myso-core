...
//! ProgramInfo extension for MySo Flavor
//! Contains information that may be expensive to compute and is needed only for MySo
...
use crate::{
    ...
    myso_mode::{
        OBJECT_MODULE_NAME, MYSO_ADDR_VALUE, TRANSFER_FUNCTION_NAME, TRANSFER_MODULE_NAME,
        UID_TYPE_NAME,
    },
...
pub enum TransferKind {
    ...
    PublicTransfer(Loc),
    /// transferred within the module to an address vis `myso::transfer::transfer`
    PrivateTransfer(Loc),
}

/// MySo info for the entire program (mostly used during translation ).
#[derive(Debug, Clone)]
pub struct MySoInfo {
...
/// Same as `MySoInfo` but for a given module (store per-module in `ProgramInfo`).
#[derive(Debug, Clone)]
pub struct MySoModInfo {
    ...
    fn visit_ty(
        ...
        match ty_.inner() {
            ...
            TI::Apply(_, sp!(_, tn_), _)
                if tn_.is(&MYSO_ADDR_VALUE, OBJECT_MODULE_NAME, UID_TYPE_NAME) =>
            {
