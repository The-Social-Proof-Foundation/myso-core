...
use crate::{
    ...
    myso_mode::{
        ...
        BRIDGE_ADDR_VALUE, BRIDGE_CREATE, BRIDGE_MODULE_NAME, BOOTSTRAP_KEY_INIT,
        BOOTSTRAP_KEY_MODULE_NAME, CLOCK_MODULE_NAME,
        COIN_REGISTRY_MODULE_NAME, DENY_LIST_CREATE, DENY_LIST_MODULE_NAME, DERIVED_OBJECT_CLAIM,
        DERIVED_OBJECT_MODULE_NAME, DISPLAY_REGISTRY_MODULE_NAME, ID_LEAK_DIAG, MYSO_ADDR_NAME,
        MYSO_ADDR_VALUE, MYSO_CLOCK_CREATE, MYSO_SOCIAL_ADDR_VALUE,
        MYSO_SOCIAL_BOOTSTRAP_INIT_AT_GENESIS, MYSO_SOCIAL_BOOTSTRAP_MODULE_NAME,
        MYSO_SYSTEM_ADDR_VALUE, MYSO_SYSTEM_CREATE,
        MYSO_SYSTEM_MODULE_NAME, OBJECT_MODULE_NAME, OBJECT_NEW, OBJECT_NEW_UID_FROM_HASH,
        ORDERBOOK_ADDR_VALUE, ORDERBOOK_REGISTRY_MODULE_NAME, RANDOMNESS_MODULE_NAME,
...
pub const FRESH_ID_FUNCTIONS: &[(AccountAddress, Symbol, Symbol)] = &[
    (MYSO_ADDR_VALUE, OBJECT_MODULE_NAME, OBJECT_NEW),
    (
        MYSO_ADDR_VALUE,
        OBJECT_MODULE_NAME,
    ...
    (MYSO_ADDR_VALUE, TEST_SCENARIO_MODULE_NAME, TS_NEW_OBJECT),
];
pub const FUNCTIONS_TO_SKIP: &[(AccountAddress, Symbol, Symbol)] = &[
    (
        MYSO_SYSTEM_ADDR_VALUE,
        MYSO_SYSTEM_MODULE_NAME,
        MYSO_SYSTEM_CREATE,
    ),
    (MYSO_ADDR_VALUE, CLOCK_MODULE_NAME, MYSO_CLOCK_CREATE),
    (
        MYSO_ADDR_VALUE,
        BOOTSTRAP_KEY_MODULE_NAME,
        BOOTSTRAP_KEY_INIT,
    ),
    (
        MYSO_ADDR_VALUE,
    ...
    (MYSO_ADDR_VALUE, DENY_LIST_MODULE_NAME, DENY_LIST_CREATE),
    (BRIDGE_ADDR_VALUE, BRIDGE_MODULE_NAME, BRIDGE_CREATE),
    (
        ORDERBOOK_ADDR_VALUE,
        ORDERBOOK_REGISTRY_MODULE_NAME,
        REGISTRY_CREATE_FUNCTION_NAME,
    ),
    (MYSO_ADDR_VALUE, ACCUMULATOR_MODULE_NAME, ACCUMULATOR_CREATE),
    (
        MYSO_ADDR_VALUE,
    ...
    (
        MYSO_SOCIAL_ADDR_VALUE,
        MYSO_SOCIAL_BOOTSTRAP_MODULE_NAME,
...
impl SimpleAbsInt for IDLeakVerifierAI<'_> {
    ...
    fn exp_custom(
        ...
        if !matches!(first_value, Value::FreshID(_)) {
            ...
            let uid_msg = format!(
                "The UID must come directly from `{myso}::{object}::{new}`, or `myso::{derived}::{claim}`. \
                For tests, it can come from `{myso}::{ts}::{ts_new}`",
                myso = MYSO_ADDR_NAME,
...
fn value_for_ty(loc: &Loc, sp!(_, t): &SingleType) -> Value {
    if t.is_apply(&MYSO_ADDR_VALUE, OBJECT_MODULE_NAME, UID_TYPE_NAME)
        .is_some()
    {
        Value::NotFresh(*loc)
    } else {
