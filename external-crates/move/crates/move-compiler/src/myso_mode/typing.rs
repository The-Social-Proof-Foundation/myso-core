...
pub struct MySoTypeChecks;

impl TypingVisitorConstructor for MySoTypeChecks {
...
pub struct Context<'a> {
    ...
    myso_transfer_ident: Option<ModuleIdent>,
...
impl<'a> Context<'a> {
    fn new(env: &'a CompilationEnv, info: Arc<TypingProgramInfo>) -> Self {
        let myso_module_ident = info
            .modules
            .key_cloned_iter()
            .find(|(m, _)| m.value.is(&MYSO_ADDR_VALUE, TRANSFER_MODULE_NAME))
            .map(|(m, _)| m);
        ...
        Context {
            ...
            reporter,
            info,
            myso_transfer_ident: myso_module_ident,
            current_module: None,
            otw_name: None,
...
fn invalid_object_id_field_diag(key_loc: Loc, loc: Loc, name: DatatypeName) -> Diagnostic {
    const KEY_MSG: &str = "The 'key' ability is used to declare objects in MySo";
...
fn is_mut_clock(param_ty: &Type) -> bool {
    match &param_ty.value.inner() {
        ...
        TI::Apply(_, sp!(_, n_), _) => n_.is(&MYSO_ADDR_VALUE, CLOCK_MODULE_NAME, CLOCK_TYPE_NAME),
        TI::Unit
        | TI::Param(_)
        | TI::Var(_)
        | TI::Anything
...
fn is_entry_receiving_ty(param_ty: &Type) -> bool {
    match &param_ty.value.inner() {
        ...
        TI::Apply(_, sp!(_, n), targs)
            if n.is(&MYSO_ADDR_VALUE, TRANSFER_MODULE_NAME, RECEIVING_TYPE_NAME) =>
        {
    ...
    match &param_ty.value.inner() {
        ...
        TI::Apply(_, sp!(_, n), targs)
            if n.is(&STD_ADDR_VALUE, ASCII_MODULE_NAME, ASCII_TYPE_NAME)
                ...
                || n.is(&MYSO_ADDR_VALUE, OBJECT_MODULE_NAME, ID_TYPE_NAME) =>
        {
...
fn exp(context: &mut Context, e: &T::Exp) {
    match &e.exp.value {
        T::UnannotatedExp_::ModuleCall(mcall) => {
            ...
            if module.value.is(&MYSO_ADDR_VALUE, EVENT_MODULE_NAME)
            ...
            if module.value.is(&MYSO_ADDR_VALUE, COIN_REGISTRY_MODULE_NAME)
                && name.value() == DYNAMIC_COIN_CREATION_FUNCTION_NAME
            {
                check_dynamic_coin_creation(context, e.exp.loc, mcall)
            }

            let is_transfer_module = module.value.is(&MYSO_ADDR_VALUE, TRANSFER_MODULE_NAME);
...
fn otw_special_cases(context: &Context) -> bool {
    BRIDGE_SUPPORTED_ASSET
        ...
        || context
            .current_module()
            .value
            .is(&MYSO_ADDR_VALUE, MYSO_MODULE_NAME)
}
...
fn check_private_transfer(context: &mut Context, loc: Loc, mcall: &ModuleCall) {
    ...
    if current_module
        .value
        .is(&MYSO_ADDR_VALUE, TRANSFER_FUNCTION_NAME)
    {
