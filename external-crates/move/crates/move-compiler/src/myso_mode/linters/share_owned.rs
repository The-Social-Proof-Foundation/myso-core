...
const SHARE_FUNCTIONS: &[(AccountAddress, &str, &str)] = &[
    (MYSO_ADDR_VALUE, TRANSFER_MOD_NAME, PUBLIC_SHARE_FUN),
    (MYSO_ADDR_VALUE, TRANSFER_MOD_NAME, SHARE_FUN),
];
...
impl ShareOwnedVerifierAI<'_> {
    ...
    /// Gets MYSO info from either the current program or precompiled library
    fn get_myso_info(&self, mident: &ModuleIdent) -> &crate::myso_mode::info::MySoModInfo {
        self.info
            .modules
            .get(mident)
            .or_else(|| {
            ...
            .myso_info
            .as_ref()
            .expect("ICE typing module info expected to contain MySo info")
    }
    ...
    fn transfer_kind(
        ...
        self.get_myso_info(mident).transferred.get(datatype_name)
    }
}
