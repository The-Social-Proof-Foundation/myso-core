...
fn default_aliases(context: &mut Context) -> AliasMapBuilder {
    ...
    let myso_address = maybe_make_well_known_address(context, loc, symbol!("myso"));
    ...
    // if myso is defined and the current package is in MySo mode, add implicit myso aliases
    if let Some(myso_address) = myso_address
        && context.env().package_config(current_package).flavor == Flavor::MySo
    {
        modules.extend(
            IMPLICIT_MYSO_MODULES
                .iter()
                .copied()
                .map(|m| (myso_address, m)),
        );
        members.extend(
            IMPLICIT_MYSO_MEMBERS
                .iter()
                .copied()
                .map(|(m, mem, k)| (myso_address, m, mem, k)),
        );
    }
