...
pub struct ModuleInfo {
    ...
    pub myso_info: Option<MySoModInfo>,
}
...
impl TypingProgramInfo {
    pub fn new(
        ...
        if env
            .package_configs()
            .any(|(_, config)| config.flavor == Flavor::MySo)
        {
            let mut myso_flavor_info = MySoInfo::new(modules, &info);
            for (mident, module_info) in info.modules.key_cloned_iter_mut() {
                let uid_holders = myso_flavor_info
                    .uid_holders
                    .remove(&mident)
                    .unwrap_or_default();
                let transferred = myso_flavor_info
                    .transferred
                    .remove(&mident)
                    .unwrap_or_default();
                module_info.myso_info = Some(MySoModInfo {
                    uid_holders,
                    transferred,
                });
            }
