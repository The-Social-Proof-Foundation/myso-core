...
fn gen_unused_warnings(
    ...
    let is_myso_mode = context.env.package_config(context.current_package).flavor == Flavor::MySo;
    ...
    for (_, sname, sdef) in structs {
        ...
        if let H::StructFields::Defined(fields) = &sdef.fields {
            for (f, _) in fields {
                // skip for MySo ID fields
                if is_myso_mode && has_key && f.value() == ID_FIELD_NAME {
                    continue;
                }
                if !context
                    .used_fields
