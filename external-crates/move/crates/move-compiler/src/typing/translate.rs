...
fn unused_module_members(
    ...
    let is_myso_mode = env.package_config(mdef.package_name).flavor == Flavor::MySo;
    ...
    for (loc, name, fun) in &mdef.functions {
        ...
        if is_myso_mode && *name == myso_mode::INIT_FUNCTION_NAME {
            // a MySo-specific filter to avoid signaling that the init function is unused
            continue;
        }
