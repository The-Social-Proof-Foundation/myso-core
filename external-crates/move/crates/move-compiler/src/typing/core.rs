...
pub enum PublicForTesting {
    ...
    Entry(Loc),
    // TODO we should allow calling init in unit tests, but this would need MySo bytecode verifier
...
pub fn public_testing_visibility(
    ...
    // is_testing && (is_entry || is_myso_init)
    if !env.test_mode() {
        return None;
    }

    // TODO support myso init functions
    // let flavor = env.package_config(package).flavor;
    // flavor == Flavor::MySo && callee_name.value() == INIT_FUNCTION_NAME
